...
static sns_err_code_e sns_fill_debug_str_mask(uint64_t* debug_str_mask);
#endif
...
static void sns_diag_thread_main(void *p_arg)
{
  ...
  /* Get mask info */
  if (sns_fill_debug_str_mask(&debug_str_bit_mask) == SNS_SUCCESS)
  {
...
*/
/*=========================================================================*/
static sns_err_code_e sns_fill_debug_str_mask(sns_debug_mask_t* debug_str_mask)
{
  ...
  {
    SNS_PRINTF_STRING_ERROR_0(DBG_MOD_INIT,
                        "Length of module IDs not same as module names");
    return SNS_ERR_FAILED;
  }

  /* Initialize all modules in the mask array to disabled. */
  for ( j = 0; j < SNS_DBG_NUM_MOD_IDS; j++ )
  {
    module_mask_arr[j] = 1;
  }
    ...
    if (line[0] != '#')
    {
      ...
      if (token_str_ptr != NULL)
      {
        ...
        if ( (token_str_ptr != NULL) &&
             ( i < SNS_DBG_NUM_MOD_IDS) )
        {
          module_mask_arr[i] = (uint16_t)strtol(token_str_ptr,NULL,10);
        }
      }
    } // end of outer if
  } // end of while
  ...
  for( ; ; )
  {
    ...
    if (SNS_DEBUG_SVC_ID_V01 == smr_header.svc_num)
    {
      if ( SNS_DEBUG_STRING_ID_IND_V01 == smr_header.msg_id )
      {
        ...
        format_params_var.param1 = print_msg_ptr->param_values[0];
        format_params_var.param2 = print_msg_ptr->param_values[1];
        format_params_var.param3 = print_msg_ptr->param_values[2];
