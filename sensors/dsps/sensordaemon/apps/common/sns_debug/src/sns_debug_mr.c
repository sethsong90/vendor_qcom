                             ...
                             uint64_t bit_mask_ext)
{
...
#else  /* SNS_QMI_ENABLE not defined */
  void*            mask_qmi_msg_ptr;
  sns_smr_header_s smr_hdr;

  switch (mask_type)
  {
    case DEBUG_STR_MASK_INFO:
      mask_qmi_msg_ptr = sns_smr_msg_alloc(SNS_DBG_MOD_APPS_DIAG,sizeof(sns_diag_set_debug_mask_req_msg_v01));
      if (mask_qmi_msg_ptr == NULL)
      {
        return;
      }

      /* Memcopy the qmi message in the allocated space */
      ((sns_diag_set_debug_mask_req_msg_v01*)mask_qmi_msg_ptr)->debug_mask.mask = bit_mask;

      /* Fill SMR Header specific to mask */
      smr_hdr.msg_id = SNS_DIAG_SET_DEBUG_MASK_REQ_V01;
      smr_hdr.body_len = sizeof(sns_diag_set_debug_mask_req_msg_v01);

      break;

    case LOG_MASK_INFO:
      mask_qmi_msg_ptr = sns_smr_msg_alloc(SNS_DBG_MOD_APPS_DIAG,sizeof(sns_diag_set_log_mask_req_msg_v01));
      if (mask_qmi_msg_ptr == NULL)
      {
        return;
      }
      ...
      /* Memcopy the qmi message in the allocated space */
      ((sns_diag_set_log_mask_req_msg_v01*)mask_qmi_msg_ptr)->log_mask.mask = bit_mask;

       /* Fill SMR Header specific to mask */
      smr_hdr.msg_id = SNS_DIAG_SET_LOG_MASK_REQ_V01;
      smr_hdr.body_len = sizeof(sns_diag_set_log_mask_req_msg_v01);
  ...
  /* Fill up rest of SMR Header */
  smr_hdr.src_module = SNS_MODULE_APPS_DIAG;
  smr_hdr.priority = SNS_SMR_MSG_PRI_LOW;
  smr_hdr.txn_id = 0;
  smr_hdr.ext_clnt_id = 0;
  smr_hdr.svc_num = SNS_DIAG_DSPS_SVC_ID_V01;
  smr_hdr.msg_type = SNS_SMR_MSG_TYPE_REQ;

  if (sns_smr_set_hdr(&smr_hdr,mask_qmi_msg_ptr) == SNS_SUCCESS)
  {
    if (sns_smr_send(mask_qmi_msg_ptr) == SNS_SUCCESS)
    {
      return;
    }
  }
  sns_smr_msg_free(mask_qmi_msg_ptr);
#endif /* SNS_QMI_ENABLE */
}
...
sns_err_code_e sns_debug_mr_init(void)
{
  ...
  /* Register queues with SMR for receiving QMI messages from DSPS */
  error_code = sns_smr_register(DBG_MOD_INIT,
                                sns_diag_flag_grp,
                                SNS_DIAG_RX_SIG);
  if( SNS_SUCCESS != error_code )
  {
    ...
    SNS_PRINTF_STRING_ERROR_0(DBG_MOD_INIT,
                              "Sensors DIAG Init: SMR queue register failed");
  }
  return error_code;
#endif /* SNS_QMI_ENABLE */
}
...
sns_err_code_e sns_debug_mr_deinit(void)
{
  ...
  // TODO: Support SMR deregister
  //  error_code = sns_smr_deregister(DBG_MOD_INIT);
  error_code = SNS_SUCCESS;
  if( SNS_SUCCESS != error_code )
  {
    SNS_PRINTF_STRING_ERROR_1(DBG_MOD_INIT,
                              "Sensors DIAG Init: SMR deregister failed %d", error_code);
  }
  return error_code;
#endif /* SNS_QMI_ENABLE */
}
...
void * sns_debug_rcv(void)
{
 ...
 #else
  return (sns_smr_rcv(SNS_MODULE_APPS_DIAG));
 #endif
}
