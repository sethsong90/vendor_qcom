                                    ...
                                    uint16_t pkt_len)
{
  ...
  sns_diag_qmi_msg_s *parsed_hdr;
  sns_smr_header_s smr_hdr;
  sns_err_code_e err_code;
  void *qmi_msg_ptr = NULL;
  int32_t qmi_decode_ret_code;
  uint16_t max_decode_len = sns_smr_get_qmi_max_decode_msg_len();

  parsed_hdr = (sns_diag_qmi_msg_s*) req_pkt_ptr;

  /* Input Checks */
  if ( (parsed_hdr == NULL) ||
       (pkt_len != ( SNS_DIAG_APPS_QMI_MSG_HDR_SIZE +
                     parsed_hdr->msg_hdr.qmi_msg_size ))
       )
  {
    SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DIAG,
                              "Packet Type 1 (QMI Message) Input parameters "
                              "invalid! pkt_len = %d",
                              pkt_len);
    sns_diag_fill_smr_immediate_resp(SNS_ERR_BAD_PARM,1);
    return NULL;
  }

  /*  Fill in SMR header details from diag command packet */
  smr_hdr.dst_module = 0;
  if (parsed_hdr->msg_hdr.msg_type != SNS_SMR_MSG_TYPE_REQ)
  {
    smr_hdr.dst_module = parsed_hdr->msg_hdr.dest_module;
  }
  smr_hdr.src_module = parsed_hdr->msg_hdr.src_module;
  smr_hdr.priority = parsed_hdr->msg_hdr.priority;
  smr_hdr.txn_id = parsed_hdr->msg_hdr.txn_id;
  smr_hdr.ext_clnt_id = parsed_hdr->msg_hdr.ext_client_id;
  smr_hdr.msg_type = parsed_hdr->msg_hdr.msg_type;
  smr_hdr.svc_num = parsed_hdr->msg_hdr.srvc_num;
  smr_hdr.msg_id = parsed_hdr->msg_hdr.msg_id;
  smr_hdr.body_len = parsed_hdr->msg_hdr.qmi_msg_size;

  /* Debug checks */
  SNS_PRINTF_STRING_MEDIUM_3(SNS_DBG_MOD_DIAG,
                             "SENSOR1_Write srcmod=%d, dest_mod=%d,priority=%d",
                             parsed_hdr->msg_hdr.src_module,
                             parsed_hdr->msg_hdr.dest_module,
                             parsed_hdr->msg_hdr.priority);
  SNS_PRINTF_STRING_MEDIUM_1(SNS_DBG_MOD_DIAG,
                             "Pkt Type 1:Service num = %d",
                             parsed_hdr->msg_hdr.srvc_num);
  SNS_PRINTF_STRING_MEDIUM_1(SNS_DBG_MOD_DIAG,
                             "Pkt Type 1:Msg ID = %d",
                             parsed_hdr->msg_hdr.msg_id);
  SNS_PRINTF_STRING_MEDIUM_2(SNS_DBG_MOD_DIAG,
                             "Pkt Type 1:Msg Size = %d,SMR msg size=%d",
                             parsed_hdr->msg_hdr.qmi_msg_size,smr_hdr.body_len);
  SNS_PRINTF_STRING_FATAL_3(SNS_DBG_MOD_DIAG,
                            "Pkt Type 1:txn_id=%d,clnt id = %d, msg_type=%d",
                            parsed_hdr->msg_hdr.txn_id,
                            parsed_hdr->msg_hdr.ext_client_id,
                            parsed_hdr->msg_hdr.msg_type);

  /* Allocate memory for QMI message and fill in the details
   */
  if (parsed_hdr->msg_hdr.qmi_msg_size != 0)
  {
    //alloc memory for the max decoded msg size
    qmi_msg_ptr = sns_smr_msg_alloc(SNS_DBG_MOD_APPS_DIAG, max_decode_len);
  }
  else
  {
    qmi_msg_ptr = sns_smr_msg_alloc(SNS_DBG_MOD_APPS_DIAG,0);
  }
  if ( qmi_msg_ptr == NULL )
  {
    SNS_PRINTF_STRING_ERROR_0(SNS_DBG_MOD_DIAG,
                              "Packet Type 1 (QMI Message) SMR alloc failure!");
    sns_diag_fill_smr_immediate_resp(SNS_ERR_NOMEM,1);
    return NULL;
  }

  /* Decode QMI Message only if QMI encoded msg size is not zero */
  if (parsed_hdr->msg_hdr.qmi_msg_size != 0)
  {
    SNS_PRINTF_STRING_MEDIUM_0(SNS_DBG_MOD_DIAG,
                               "sns_diag_qmi_msg_handler_apps: Attempting "
                               "to decode QMI Message");
    qmi_decode_ret_code =
    qmi_idl_message_decode(sns_smr_get_svc_obj(smr_hdr.svc_num),
                           (qmi_idl_type_of_message_type)smr_hdr.msg_type,
                           smr_hdr.msg_id,
                           (const void*)(&parsed_hdr->msg_body),
                           parsed_hdr->msg_hdr.qmi_msg_size,
                           qmi_msg_ptr,
                           max_decode_len);
                                 // max decoded length
    /* Adjust Size after QMI decode. use the max decoded len */
    smr_hdr.body_len = max_decode_len;

    if (QMI_IDL_LIB_NO_ERR != qmi_decode_ret_code)
    {
      SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DIAG,
                                "sns_diag_qmi_msg_handler_apps: QMI Decode "
                                "failed=%d",
                                qmi_decode_ret_code);
      sns_diag_fill_smr_immediate_resp(SNS_ERR_FAILED,1);
      /* Free the memory allocated using the sns_smr_msg_alloc function to
         avoid memory leak */
  ...
  err_code = sns_smr_set_hdr(&smr_hdr,qmi_msg_ptr);

  if (err_code != SNS_SUCCESS)
  {
    SNS_PRINTF_STRING_ERROR_1(SNS_DBG_MOD_DIAG,
                              "Packet Type 1 (QMI Message) "
                              "SMR_set_header failed %d!",
                              err_code);
    /* Free the memory allocated using the sns_smr_msg_alloc function to
       avoid memory leak */
    sns_smr_msg_free(qmi_msg_ptr);
    /* Fill immediate response */
    sns_diag_fill_smr_immediate_resp(err_code,1);
    return NULL;
  }

  err_code = sns_smr_send(qmi_msg_ptr);
  /* Dont modify err code from this point.
   * The value of err_code is being used for constructing
   * the immediate response
   */
  if (err_code != SNS_SUCCESS)
  {
    SNS_PRINTF_STRING_HIGH_1(SNS_DBG_MOD_DIAG,
                             "Packet Type 1 (QMI Message) SMR_send status =%d",
                             err_code);
    /* Free the memory allocated using the sns_smr_msg_alloc function to avoid
       memory leak */
    sns_smr_msg_free(qmi_msg_ptr);
    /* Intentional fall through to fill immediate response */
  }

  SNS_PRINTF_STRING_HIGH_1(SNS_DBG_MOD_DIAG,
                           "Packet Type 1 SMR Ret Value = %d",
                           err_code);
  sns_diag_fill_smr_immediate_resp(err_code,1);
