...
sns_sam_mr_confirm_client_req( struct sns_sam_client_req* clientReqPtr,
                               ...
                               sns_sam_mr_conn_hndl mr_clnt_id )
{
  return clientReqPtr->reqModule == msgHdrPtr->src_module &&
         clientReqPtr->mrClientId == mr_clnt_id;
}
...
/*=========================================================================
  ...
  =========================================================================*/
void*
sns_sam_mr_msg_rcv( void )
{
  void* req_msg_ptr;
  sns_smr_header_s smr_msg_hdr;
  sns_err_code_e sns_smr_err;

  while( true )
  {
    req_msg_ptr = sns_smr_rcv( SNS_SAM_MODULE );

    if( NULL != req_msg_ptr )
    {
      sns_smr_err = sns_smr_get_hdr( &smr_msg_hdr, req_msg_ptr );
      if( SNS_SUCCESS != sns_smr_err )
      {
          SNS_SAM_DEBUG1( ERROR, DBG_SAM_PROC_MSG_HDR_ERR, sns_smr_err );
          sns_smr_msg_free( req_msg_ptr );
          continue;
      }
      else if( SNS_SMR_MSG_TYPE_REQ == smr_msg_hdr.msg_type )
      {
        void *resp_msg_ptr = NULL;

        sns_smr_err = sns_sam_process_client_req( req_msg_ptr,
            (sns_sam_mr_conn_hndl)smr_msg_hdr.ext_clnt_id, &resp_msg_ptr );
        if( SNS_SUCCESS != sns_smr_err )
        {
          sns_sam_report_client_error( req_msg_ptr, &resp_msg_ptr, sns_smr_err );
        }

        if( SNS_SUCCESS != sns_sam_mr_send( resp_msg_ptr, NULL ) )
        {
...
sns_sam_mr_init( void )
{
   ...
   return sns_smr_register( SNS_SAM_MODULE, sns_sam_sig_event_acc(), SNS_SAM_MSG_SIG );
}
