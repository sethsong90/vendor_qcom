...
{
   uint8_t c, i, s, k, algoIndex, algoInstId, clientReqId;
   uint32_t reportRate;
   sns_sam_sensor_req_s sensorReq[SNS_SAM_MAX_SENSORS_PER_DATA_REQ];
   int sensorReqCnt = 0;
   ...
   if (sns_sam_algo_inst_dbase[currAlgoInstId]->clientReqCount >= 1)
   {
      for(c = 0; c < algo->algoDepCount; c++)
      {
         ...
         enableMsgHdr.svc_num = algo->algoDepDbase[c];
         enableMsgHdr.ext_clnt_id = currAlgoInstId;
         enableMsgHdr.txn_id = c;
         ...
         SNS_SAM_PRINTF2( LOW, "SAM enabling sync dep algo svc %d for parent svc %d",
            algo->algoDepDbase[c], sns_sam_algo_inst_dbase[currAlgoInstId]->serviceId );
         ...
         algoIndex = sns_sam_get_algo_index(algo->algoDepDbase[c]);
         if (algoIndex >= SNS_SAM_MAX_ALGOS)
         {
            return SNS_ERR_FAILED;
         }
         ...
         if (sensorReqCnt > 0)
         {
            for(i = 0; i < sensorReqCnt; i++)
            {
               ...
               for(s = 0; s < *totalSensorCount; s++)
               {
                  if( (sensorReq[i].sensorId == totalSensorReq[s].sensorId) &&
                      (sensorReq[i].dataType == totalSensorReq[s].dataType) &&
                      (sensorReq[i].sampleRate == totalSensorReq[s].sampleRate) &&
                      (sensorReq[i].sampleQual == totalSensorReq[s].sampleQual))
                  {
                     break;
                  }
               }

               if(s >= *totalSensorCount)
               {
                  ...
                  totalSensorReq[*totalSensorCount].sensorId = sensorReq[i].sensorId;
                  totalSensorReq[*totalSensorCount].dataType = sensorReq[i].dataType;
                  totalSensorReq[*totalSensorCount].sampleRate = sensorReq[i].sampleRate;
                  totalSensorReq[*totalSensorCount].sampleQual = sensorReq[i].sampleQual;
               ...
               else
               {
                  ...
                  {
                     if( totalSensorReq[s].algoInstIds[k] == algoInstId )
                     {
                        break;
                     }
                     if( totalSensorReq[s].algoInstIds[k] == SNS_SAM_INVALID_ID )
                     {
                        totalSensorReq[s].algoInstIds[k] = algoInstId;
                        break;
                     }
                  }
                  if( k == SNS_SAM_MAX_ALGO_INSTS_PER_SENSOR )
                  {
   ...
   uint8_t parentAlgoInstId)
{
   ...
   for (i = 0; i < sns_sam_algo_dbase[algoIndex]->algoDepCount; i++)
   {
      childAlgoInstId = sns_sam_algo_inst_dbase[algoInstId]->algoReqDbase[i];
