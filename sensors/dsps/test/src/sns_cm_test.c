...
void test_invalid_writes( sensor1_handle_s *hndl)
{
  ...
  error = sensor1_write( (void*)0xBADADD,
                         &msg_hdr,
                         smgr_req );
...
void rx_hndl_from_parent(int fd, sensor1_handle_s **hndl1) {
  struct msghdr msg;
  ...
  /* Allocate a new data connetion */
  *hndl1 = (sensor1_handle_s*)((intptr_t)(*(int*)CMSG_DATA(cmsg)));

  sockfd = (int)((intptr_t)(*hndl1));

  memset(address.sun_path,0,UNIX_PATH_MAX);

  if( -1 != getsockname( sockfd, (struct sockaddr *)&address, (socklen_t*)&len ) ) {
    address.sun_path[UNIX_PATH_MAX-1] = 0;
    printf("%s: received socket fd %d name %s\n", 
           __FUNCTION__, 
           sockfd,
           address.sun_path );
    printf("Socket name: ");
    for( i = 0; i<len; i++ ) {
      printf("%c", address.sun_path[i] );
    }
    printf("\n");
  } else {
    printf("%s: received socket fd %d ERROR GETTING NAME!\n", 
           __FUNCTION__, 
           *((int*)*hndl1) );
  }
}
