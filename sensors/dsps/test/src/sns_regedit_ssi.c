...
int init_lis3dsh_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
...
int init_bma2x2_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
int init_hscdtd008_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
int init_tmd277x_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
int init_ltr55x_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
int init_mpu3050_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems );
...
uuid_t uuid_array[] = {
  ...
  {"MPU3050", SNS_REG_UUID_MPU3050, init_mpu3050_8226_config },
  ...
  {"BMA2X2", SNS_REG_UUID_BMA2X2, init_bma2x2_8226_config },
  {"HSCDTD008", SNS_REG_UUID_HSCD008, init_hscdtd008_8226_config },
  {"TMD277X", SNS_REG_UUID_TMD277X, init_tmd277x_8226_config },
  ...
  {"LIS3DSH", SNS_REG_UUID_LIS3DSH, init_lis3dsh_8226_config },
  ...
  if( regitems != NULL ) {
    for( i = 0; i < NUM_REG_ITEMS; i++ ) {
      if( regitems[i].item_id == -1 || regitems[i].item_id == item_id ) {
        ...
        regitems[i].item_data = item_data;
        break;
      }
    }
  }
...
static int init_default_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, regitems_t *regitems, dev_info_t *dev_info )
{
  ...
  {
    init_bma2x2_8226_config( cfg_array, dev_info, regitems );
    init_mpu3050_8226_config( cfg_array, dev_info, regitems );
    init_hscdtd008_8226_config( cfg_array, dev_info, regitems );
    init_pressnull_config( cfg_array, dev_info, regitems );
    //This should change to tmd277x once the driver is ready
    init_apds99xx_config( cfg_array, dev_info, regitems );
  }
  else
  {
                            ...
                            void *msg_ptr )
{
  ...
  if( msg_type == SENSOR1_MSG_TYPE_RESP ) {
    ...
    memcpy(data_info->resp_data, msg_ptr, MIN(2048, msg_hdr_ptr->msg_size));
                                       ...
                                       regitems_t *regitems )
{
  ...
  while( regitems[i].item_id != -1 ) {
    ...
    single_msg->item_id = regitems[i].item_id;
...
void print_usage( char * const filename )
{
  ...
  fprintf(stderr, "Usage: %s [-r|-w|-a] [-s|-d <SENSOR>]\n"
          ...
          "\t\t--If no sensors are selected, will configure the default MSM8974 or MSM8226 sensors\n"
          ...
          "\t\t--If no sensors are selected, will autodetect only the default MSM8974 or MSM8226 sensors\n"
          "\n"
  ...
  for(cfg_idx = 0; cfg_idx < SNS_REG_SSI_NUM_SMGR_CFG; cfg_idx++) {
    ...
    for( i = 0; i < SNS_REG_SSI_SMGR_CFG_NUM_SENSORS-1; i++ ) {
      for( j = i+1; j < SNS_REG_SSI_SMGR_CFG_NUM_SENSORS; j++ ) {
        ...
        {
          cfg->drv_cfg[i].related_sensor_index = j;
          cfg->drv_cfg[j].related_sensor_index = i;
        }
      }
    }
  }
...
int init_lis3dsh_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array,
                              dev_info_t *dev_info,
                              regitems_t *regitems )
{
  bool cfg_done = false;
...
int init_hscdtd008_8226_config( sns_reg_ssi_smgr_cfg_group_s *cfg_array, dev_info_t *dev_info, regitems_t *regitems )
{
  ...
  for(cfg_idx = 0; cfg_idx < SNS_REG_SSI_NUM_SMGR_CFG && !cfg_done; cfg_idx++)
  {
    ...
    for( i = 0; i < SNS_REG_SSI_SMGR_CFG_NUM_SENSORS; i ++ ) {
      /* the HSCDTD is a mag. So overwrite an mag part if found */
      if( cfg->drv_cfg[i].sensor_id == SNS_SMGR_ID_MAG_V01 ||
          memcmp(cfg->drv_cfg[i].drvuuid, null_uuid, sizeof(null_uuid)) == 0 ) {
        break;
      }
...
void perform_autodetect( data_t *data_info, dev_info_t *dev_info, int argc, char * const argv[] )
{
  ...
  if( use_default_config ) {
    /* Use default 8974/8226 sensors depending on msm type. To be changed based on hardware configurations
     * to be detected */
    printf("Using default 8974/8226 sensors for auto-detect.\n");
...
static sns_msm_id detect_msm()
{
  ...
  else
  {
    while( (fgets(line, sizeof(line), fp) != NULL) )
    {
      ...
      {
        if( strstr( line, "MSM 8226" ) )
        {
          printf("detected MSM 8226...\n");
          msm_id = SNS_MSM_8226;
        }
        else if( strstr( line, "MSM 8974" ) )
        {
          printf("detected MSM 8974...\n");
          msm_id = SNS_MSM_8974;
        }
        break;
      }
  ...
  /*enable the below lines once flcose start to work */
  //if( 0 != fclose( fp ) )
  //{
