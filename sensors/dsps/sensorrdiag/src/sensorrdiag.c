...
*/
void
rdiag_writable_cb(intptr_t cbdata, uint32_t service_id)
{
  ...
  {
    LOGE("%s: Sensor1 Writable Callback: Input parameter err, cbdata = %d",
         __func__, cbdata);
    delay_res_flag = 2;
    return;
  }

  context_hdl = client_association_arr[(uint32_t)cbdata];
...
*/
void
rdiag_notify_data_cb(intptr_t cbdata,
                     ...
                     void *msg_ptr)
{
  ...
  if(SNS_RDIAG_MAX_CLIENTS < cbdata)
  {
    LOGE("%s: Input parameters error, cbdata=%d", __func__, (uint32_t)cbdata);
    delay_res_flag = 1;
    return;
  }
...
*/
static sensor1_error_e
sns_rdiag_send_immed_resp(int32_t sensor1_api_ret_value, intptr_t cb_data,
...
sns_rdiag_msg_handle_write(byte *req_pkt_ptr, uint16_t pkt_len, uint8_t pkt_type)
{
     ...
     pkt_len - SNS_RDIAG_APPS_SENSOR1_MSG_WRITE_MIN_BODY_SIZE - 1)
  {
    ...
    ((uint32_t)parsed_msg_ptr->write_msgbody.cbdata > SNS_RDIAG_MAX_CLIENTS) ||
    (client_association_arr[(uint32_t)parsed_msg_ptr->write_msgbody.cbdata] == NULL))
  {
  ...
  context_hdl =
    client_association_arr[(uint32_t)parsed_msg_ptr->write_msgbody.cbdata];
...
sns_rdiag_msg_handle_close(byte *req_pkt_ptr, uint16_t pkt_len)
{
  ...
  if((pkt_len < (SNS_RDIAG_APPS_SENSOR1_MSG_CLOSE_BODY_SIZE +
      ...
      ((uint32_t)parsed_msg_ptr->close_msgbody.cbdata > SNS_RDIAG_MAX_CLIENTS) ||
      (client_association_arr[(uint32_t)parsed_msg_ptr->close_msgbody.cbdata] == NULL) )
  {
    LOGE("%s:Invalid Sensor1_Close request, pkt len = %d, cb_data= %d",
          __func__, pkt_len, parsed_msg_ptr->close_msgbody.cbdata);
    sns_rdiag_send_immed_resp(SENSOR1_EBAD_PARAM, 0, NULL, 0, 0, 1);
    return ;
  }

  context_hdl = client_association_arr[(uint32_t)parsed_msg_ptr->close_msgbody.cbdata];
  ...
  sns_rdiag_free_client_id((uint32_t)parsed_msg_ptr->close_msgbody.cbdata);
...
sns_rdiag_msg_handle_writable(byte *req_pkt_ptr, uint16_t pkt_len)
{
  ...
  context_hdl =
    client_association_arr[(uint32_t)parsed_msg_ptr->writable_msgbody.cbdata];
