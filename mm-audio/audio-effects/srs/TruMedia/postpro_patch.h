/*======================================================================*
 ...
 AND ANY RELATED DOCUMENTATION IS AT THE RECIPIENT~RS SOLE RISK:
...
#define POSTPRO_PATCH_OUTPROC_DUPE_INIT(a, b) \
    if (SRS_DoOutput()){ \
        SRS_LOG("SRS_Processing - DuplicatingThread - OutNotify_Init: %s\n", b.string()); \
        SRS_Processing::ProcessOutNotify(SRS_Processing::AUTO, a, true); \
    }

#define POSTPRO_PATCH_OUTPROC_DUPE_SAMPLES(a, fmt, buf, bsize, rate, count) \
    if ((fmt == AUDIO_FORMAT_PCM_16_BIT) && SRS_DoOutput()){ \
        SRS_Processing::ProcessOut(SRS_Processing::AUTO, a, buf, bsize, rate, count); \
    }

#define POSTPRO_PATCH_OUTPROC_DUPE_EXIT(a, b) \
    if (SRS_DoOutput()){ \
        SRS_LOG("SRS_Processing - DuplicatingThread - OutNotify_Exit: %s\n", b.string()); \
        SRS_Processing::ProcessOutNotify(SRS_Processing::AUTO, a, false); \
    }
