...
void Java_com_android_nativeaudiolatency_NativeAudioLatency_startRecording(JNIEnv* env, jclass clazz, jint latency) {
    ...
    if (property_get("ro.board.platform", value, "0") && (!strncmp(value, "msm8960", sizeof("msm8960") - 1))) {
         LOGW("recorderFrames is 320");
         recorderFrames = 320;
    }
    else if (property_get("ro.board.platform", value, "0") && (!strncmp(value, "msm8974", sizeof("msm8974") - 1))) {
        LOGW("recorderFrames is 320");
        recorderFrames = 320;
    }
    else if (property_get("ro.board.platform", value, "0") && (!strncmp(value, "msm8610", sizeof("msm8610") - 1))) {
        LOGW("recorderFrames is 320");
        recorderFrames = 320;
    }
    else if (property_get("ro.board.platform", value, "0") && (!strncmp(value, "msm8226", sizeof("msm8226") - 1))) {
        LOGW("recorderFrames is 320");
        recorderFrames = 320;
    }
