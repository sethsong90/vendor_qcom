                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
	...
	timeStampLfile += timestampInterval;
        ...
        if(readBytes!= (signed)write_buff_size) {
          DEBUG_PRINT("insert write_buff_size aligned %d\n", readBytes);
...
int main(int argc, char **argv)
{
	...
	write_buff_size = virtual_chunk_len;
        if(argc==20) {
           allocate_buf_flag = atoi(argv[19]);
        }else if(argc >= 21) {
           allocate_buf_flag = atoi(argv[19]);
           write_buff_size = atoi(argv[20]);
	}
        if (tunnel == 1)
        {
...
static int Read_Buffer (OMX_BUFFERHEADERTYPE  *pBufHdr )
{
    ...
    if(temp < packet_header_size) {
        ...
        fseek(inputBufferFile, temp, SEEK_CUR);
        current_ptr +=temp;
        bytes_read = fread(pBufHdr->pBuffer, 1, write_buff_size, inputBufferFile);
        if (bytes_read != (signed)write_buff_size)
           return -1;
	   ...
	   temp = (packet_size - packet_padding_size - temp);
           ...
           fseek(inputBufferFile,(packet_padding_size+packet_header_size), SEEK_CUR);
