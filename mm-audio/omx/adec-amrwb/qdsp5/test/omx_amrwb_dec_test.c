    ...
    if(pcmplayback && pBuffer->nFilledLen)
    {
        if(start_done == 0)
        {
            ...
            if (copy_done == 1)
            {
                ...
                {
                   if (write(m_pcmdrv_fd, pBuffer_tmp+i*pcm_buf_size, pcm_buf_size ) != (ssize_t)pcm_buf_size)
                   {
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 8)
    {
      in_filename = argv[1];
        ...
        tunnel  = atoi(argv[3]);
        DEBUG_PRINT("argv[3]- tunneling y/n = %d\n", tunnel);
    ...
    {
        DEBUG_PRINT("\ninvalid format\n");
        DEBUG_PRINT("ex: ./mm-adec-omxamr AMRINPUTFILE PCMPLAYBACK");
        DEBUG_PRINT("TUNNEL FILEWRITE SAMP-FREQ CHANNELS FORMAT \n");
	...
	DEBUG_PRINT( "PCMPLAYBACK = 0 (DISABLES PCM PLAYBACK IN NON TUNNEL MODE) \n");
        ...
        DEBUG_PRINT("FORMAT = 0 (AMRWB)\n");
	DEBUG_PRINT("FORMAT = 1 (AMRWBP)\n");
        return 0;
    }
	...
	if(tunnel == 0)
        {
            aud_comp = "OMX.qcom.audio.decoder.amrwbp";
        }
        else
        {
            aud_comp = "OMX.qcom.audio.decoder.tunneled.amrwbp";
        }
    }
...
int Init_Decoder(OMX_STRING audio_component)
{
    ...
    OMX_STRING role ="audio_decoder.amrwbp";
    ...
    DEBUG_PRINT(" Play_Decoder - tunnel = %d\n", tunnel);
...
// Modified to Set the Actual Buffer Count for input port
    ...
    {
        OMX_INDEXTYPE amrwbPlusIndex = 0;
        OMX_GetExtensionIndex(amrwb_dec_handle, "OMX.Qualcomm.index.audio.amrwbPlus", &amrwbPlusIndex);
        OMX_GetParameter(amrwb_dec_handle, amrwbPlusIndex, &amrwbPlusparam);
        amrwbPlusparam.nPortIndex   =  0;
        amrwbPlusparam.nChannels    =  ch;
        amrwbPlusparam.nSampleRate  =  sf;
        OMX_SetParameter(amrwb_dec_handle, amrwbPlusIndex, &amrwbPlusparam);
    }
    ...
    if (tunnel == 1)
    {
        ...
        OMX_SendCommand(amrwb_dec_handle, OMX_CommandPortDisable,1,0);
        wait_for_event();
    }
...
OMX_U8 buf[2048];
    ...
    if(cnt == 0)
    {
        bytes_read = fread(buf,1, 2048, inputBufferFile);
        cnt++;

        if(buf[0] == '#')
        {
DEBUG_PRINT("Ignoring the first 9 byte info...\n");
            //ignore the first 9 bytes
            memcpy(pBufHdr->pBuffer,&buf[9],2039);
            pBufHdr->nFilledLen = 2039;
            bytes_read = 2039;
        }
        else
        {
            memcpy(pBufHdr->pBuffer,&buf[0],2048);
...
{
    int error_code = 0;
	const char *outfilename = "Audio_amrwb.pcm";
    struct wav_header hdr;
    int header_len = 0;
    ...
    if((tunnel == 0) && (filewrite == 1))
    {
        ...
        outputBufferFile = fopen("Audio_amr.wav","wb");
