...
omx_adpcm_adec::omx_cmd_queue::~omx_cmd_queue()
{
    // Nothing to do
}

// omx cmd queue constructor
omx_adpcm_adec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
...
None
=============================================================================*/
bool omx_adpcm_adec::omx_cmd_queue::insert_entry(unsigned p1,
                                                 unsigned p2,
                                                 unsigned id)
{
    bool ret = true;
...
SIDE EFFECTS:
   None
=============================================================================*/
bool omx_adpcm_adec::omx_cmd_queue::pop_entry(unsigned *p1,
...
{
    return(new omx_adpcm_adec);
}
bool omx_adpcm_adec::omx_cmd_queue::get_msg_id(unsigned *id)
{
   if(m_size > 0)
   {
       *id = m_q[m_read].id;
...
void omx_adpcm_adec::process_event_cb(void *client_data, unsigned char id)
{
    DEBUG_PRINT("PE:Waiting for event's...");
    omx_adpcm_adec  *pThis = (omx_adpcm_adec *) client_data;
    (void)id;
    pThis->process_events(pThis);
}
void omx_adpcm_adec::process_events(omx_adpcm_adec *client_data)
{
    OMX_STATETYPE          state;
...
SIDE EFFECTS:
   None
=============================================================================*/
void omx_adpcm_adec::wait_for_event()
{
...
SIDE EFFECTS:
   None
=============================================================================*/
void omx_adpcm_adec::event_complete()
{
    pthread_mutex_lock(&m_event_lock);
...
// All this non-sense because of a single adpcm object
void omx_adpcm_adec::in_th_goto_sleep()
{
...
void omx_adpcm_adec::in_th_wakeup()
{
    pthread_mutex_lock(&m_in_th_lock);
...
void omx_adpcm_adec::out_th_goto_sleep()
{
...
void omx_adpcm_adec::out_th_wakeup()
{
    pthread_mutex_lock(&m_out_th_lock);
...
None
========================================================================== */
omx_adpcm_adec::omx_adpcm_adec():m_timer(NULL),
                m_tmp_meta_buf(NULL),
		m_flush_cnt(255),
		m_comp_deinit(0),
		m_bufMgr(NULL),
		...
		m_flags(0),
	        nTimestamp(0),
                output_buffer_size(OMX_ADPCM_OUTPUT_BUFFER_SIZE),
...
DEBUG_PRINT_ERROR("ADPCM component destroyed\n");
    return;
}
...
None
=============================================================================*/
void omx_adpcm_adec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if ( m_cb.EmptyBufferDone )
    {
        ...
        m_adpcm_pb_stats.ebd_cnt++;
        nNumInputBuf--;
        DEBUG_DETAIL("EBD CB:: in_buf_len=%lu nNumInputBuf=%d,ebd_cnt =%lu\n",\
            m_adpcm_pb_stats.tot_in_buf_len,
            nNumInputBuf, m_adpcm_pb_stats.ebd_cnt);
...
None
=============================================================================*/
void omx_adpcm_adec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if ( m_cb.FillBufferDone )
    {
        PrintFrameHdr(OMX_COMPONENT_GENERATE_FRAME_DONE,bufHdr);
        m_adpcm_pb_stats.fbd_cnt++;
        pthread_mutex_lock(&out_buf_count_lock);
        nNumOutputBuf--;
        DEBUG_PRINT("FBD CB:: nNumOutputBuf=%d out_buf_len=%lu bd_cnt=%lu\n",\
            nNumOutputBuf,
            m_adpcm_pb_stats.tot_out_buf_len,
            m_adpcm_pb_stats.fbd_cnt);
        m_adpcm_pb_stats.tot_out_buf_len += bufHdr->nFilledLen;
        m_adpcm_pb_stats.tot_pb_time     = bufHdr->nTimeStamp;
...
SIDE EFFECTS:
  None
=============================================================================*/
void omx_adpcm_adec::process_out_port_msg(void *client_data, unsigned char id)
{
...
SIDE EFFECTS:
  None
=============================================================================*/
void omx_adpcm_adec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned     p1;         // Parameter - 1
    unsigned     p2;         // Parameter - 2
    unsigned     ident;
...
None
=============================================================================*/
void omx_adpcm_adec::process_in_port_msg(void *client_data, unsigned char id)
{
    unsigned      p1;       // Parameter - 1
    unsigned      p2;       // Parameter - 2
    unsigned      ident;
...
OMX_ERRORTYPE omx_adpcm_adec::component_init(OMX_STRING role)
{
    ...
    incorrect param */
    memset(&m_adpcm_param, 0, sizeof(m_adpcm_param));
    m_adpcm_param.nSize = sizeof(m_adpcm_param);
    m_adpcm_param.nChannels = OMX_ADPCM_DEFAULT_CH_CFG;
    m_volume = OMX_ADPCM_DEFAULT_VOL; /* Close to unity gain */
    memset(&m_adpcm_pb_stats,0,sizeof(ADPCM_PB_STATS));
    nTimestamp = 0;
    memset(&m_adpcm_pb_stats,0,sizeof(ADPCM_PB_STATS));
    nNumInputBuf = 0;
    nNumOutputBuf = 0;
    ...
    mime_type = (OMX_STRING) malloc(sizeof("audio/adpcm"));
    if (mime_type)
    {
        strcpy(mime_type, "audio/adpcm");
    ...
    if ( !strcmp(role,"OMX.qcom.audio.decoder.adpcm") )
    {
        pcm_feedback = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    } else if ( !strcmp(role,"OMX.qcom.audio.decoder.tunneled.adpcm") )
    {
        pcm_feedback = 0;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    } else
    ...
    if (pcm_feedback)
    {
        m_tmp_out_meta_buf = (OMX_U8*)malloc(sizeof(OMX_U8)*
                             (OMX_ADPCM_OUTPUT_BUFFER_SIZE+ sizeof(META_OUT)));
    ...
    if(0 == pcm_feedback)
    {
        m_drv_fd = open("/dev/msm_adpcm",O_WRONLY);
    }
    else
    {
        m_drv_fd = open("/dev/msm_adpcm",O_RDWR);
    }
    if ( m_drv_fd < 0 )
    {
    ...
    if ( !m_ipc_to_in_th )
    {
        m_ipc_to_in_th = omx_adpcm_thread_create(process_in_port_msg,
    ...
    if ( !m_ipc_to_cmd_th )
    {
        m_ipc_to_cmd_th = omx_adpcm_thread_create(process_command_msg,
    ...
    if ( pcm_feedback )
    {
        if ( !m_ipc_to_out_th )
        {
            m_ipc_to_out_th = omx_adpcm_thread_create(process_out_port_msg,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::get_component_version(
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::send_command(OMX_IN OMX_HANDLETYPE hComp,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::send_command_proxy(OMX_IN OMX_HANDLETYPE hComp,
                                                  OMX_IN OMX_COMMANDTYPE  cmd,
                                                  OMX_IN OMX_U32       param1,
                                                  OMX_IN OMX_PTR      cmdData)
{
    ...
    if ( OMX_CommandStateSet == cmd )
    {
        ...
        else if ( OMX_StateIdle == m_state )
        {
            ...
            } else if ( OMX_StateExecuting == eState )
            {
                ...
                {
                  m_ipc_to_event_th = omx_adpcm_event_thread_create(
        ...
        DEBUG_PRINT("confg Drv for playback sample rate = 8000 \n");

        if ( ioctl(m_drv_fd, AUDIO_GET_CONFIG, &drv_config) < 0 )
              DEBUG_PRINT("AUDIO_GET_CONFIG Failed\n");

        DEBUG_PRINT("drv_config.sample_rate %d, drv_config.channel_count %d, drv_config.bits %d \
                     drv_config.buffer_size %d\n",drv_config.sample_rate,drv_config.channel_count,
                     drv_config.bits,drv_config.buffer_size);

        drv_config.sample_rate = m_adpcm_param.nSampleRate;
        drv_config.channel_count = m_adpcm_param.nChannels;
        drv_config.bits = m_adpcm_param.nBitsPerSample; // 2041 ;
        ...
        if(ioctl(m_drv_fd, AUDIO_SET_CONFIG, &drv_config) < 0)
           DEBUG_PRINT("AUDIO_SET_CONFIG Failed\n");


        if(ioctl(m_drv_fd, AUDIO_GET_PCM_CONFIG, &pcm_config) < 0)
           DEBUG_PRINT("AUDIO_GET_PCM_CONFIG Failed\n");

        pcm_config.pcm_feedback = pcm_feedback;
                DEBUG_PRINT(" configure driver mode as %d \n",pcm_feedback);

        if(ioctl(m_drv_fd, AUDIO_SET_PCM_CONFIG, &pcm_config) < 0)
            DEBUG_PRINT("AUDIO_SET_PCM_CONFIG Failed\n");
            ...
            else if ( eState == OMX_StateInvalid )
            {
                ...
                this->m_cb.EventHandler(&this->m_cmp,
                                        this->m_app_data, OMX_EventError,
                                        OMX_ErrorInvalidState,
                                        0, NULL );
                eRet = OMX_ErrorInvalidState;
            } else
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::execute_input_omx_flush()
{
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned      p1; // Parameter - 1
    unsigned      p2; // Parameter - 2
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::post_input(unsigned int p1,
                                  unsigned int p2,
                                  unsigned int id
                                 )
{
    ...
    {
        bRet = true;
        omx_adpcm_post_msg(m_ipc_to_in_th, id);
    }
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::post_command(unsigned int p1,
                                  unsigned int p2,
                                  unsigned int id
                                  )
{
    ...
    {
        bRet = true;
        omx_adpcm_post_msg(m_ipc_to_cmd_th, id);
    }
...
SIDE EFFECTS:
  None
=============================================================================*/
bool omx_adpcm_adec::post_output(unsigned int p1,
                                 unsigned int p2,
                                 unsigned int id
                                 )
{
    ...
    {
        bRet = true;
        omx_adpcm_post_msg(m_ipc_to_out_th, id);
    }
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::get_parameter(OMX_IN OMX_HANDLETYPE     hComp,
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            if ( 0 == portDefn->nPortIndex )
            {
                ...
                portDefn->format.audio.eEncoding = OMX_AUDIO_CodingADPCM;
    ...
    case OMX_IndexParamAudioPortFormat:
        {
            ...
            {
                portFormatType->eEncoding = OMX_AUDIO_CodingADPCM;
    ...
    case OMX_IndexParamAudioAdpcm:
        {
            ...
            if ( OMX_CORE_INPUT_PORT_INDEX== adpcmParam->nPortIndex )
            {
                *adpcmParam = m_adpcm_param;
            } else
            {
                DEBUG_PRINT_ERROR("get_param:OMX_IndexParamAudioadAdpcm%d\n",\
                    (int)adpcmParam->nPortIndex);
                eRet = OMX_ErrorBadPortIndex;
            }
                break;
            }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            {
                pcmparam->nSamplingRate = 8000;
                pcmparam->nChannels = m_adpcm_param.nChannels;
    ...
    case OMX_IndexParamCompBufferSupplier:
        {
            ...
            {
                DEBUG_PRINT_ERROR("get_param:IndexParamCompBufferSupp 0x%0x\n",\
                                  eRet);
                eRet = OMX_ErrorBadPortIndex;
            }
            break;
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::set_parameter(OMX_IN OMX_HANDLETYPE     hComp,
                                             OMX_IN OMX_INDEXTYPE paramIndex,
                                             OMX_IN OMX_PTR        paramData)
{
    ...
    case OMX_IndexParamAudioAdpcm:
        {
            DEBUG_PRINT("OMX_IndexParamAudioadpcm");
            m_adpcm_param = *((OMX_AUDIO_PARAM_ADPCMTYPE *) paramData);
            break;
        }
    case OMX_IndexParamPortDefinition:
        {
    ...
    case OMX_IndexParamCompBufferSupplier:
        {
            DEBUG_PRINT("set_parameter: OMX_IndexParamCompBufferSupp\n");
            OMX_PARAM_BUFFERSUPPLIERTYPE *bufferSupplierType
                = (OMX_PARAM_BUFFERSUPPLIERTYPE*) paramData;
            DEBUG_PRINT("set_parameter: OMX_IndexParamCompBufferSupp %d\n",
            ...
            {
                DEBUG_PRINT("set_param:OMX_IndexParamCompBufferSupplier\n");
                m_buffer_supplier.eBufferSupplier =
                    bufferSupplierType->eBufferSupplier;
            } else
            {
                DEBUG_PRINT_ERROR("set_param:IndexParamCompBufferSupp %0x\n", \
                                  eRet);
                eRet = OMX_ErrorBadPortIndex;
            }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            if ( OMX_CORE_OUTPUT_PORT_INDEX== pcmparam->nPortIndex )
            {
                m_adpcm_param.nChannels=pcmparam->nChannels;

                DEBUG_PRINT("set_parameter: Number of channels %lu", \
                    pcmparam->nChannels);
            } else
            {
                DEBUG_PRINT_ERROR("get_param:OMX_IndexParamAudioPcm %d\n",\
                    (int)pcmparam->nPortIndex);
                eRet = OMX_ErrorBadPortIndex;
            }
            break;
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
                                          OMX_IN OMX_INDEXTYPE configIndex,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::get_extension_index(
                                              OMX_IN OMX_HANDLETYPE      hComp,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::get_state(OMX_IN OMX_HANDLETYPE  hComp,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::component_tunnel_request(
                                    OMX_IN OMX_HANDLETYPE                hComp,
                                    OMX_IN OMX_U32                        port,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::allocate_input_buffer(
                                   OMX_IN OMX_HANDLETYPE                hComp,
  ...
  if(m_inp_current_buf_count < m_inp_act_buf_count)
  {
    ...
    if (buf_ptr != NULL)
    {
        ...
        *bufferHdr = bufHdr;
        ...
        DEBUG_PRINT("AIB:bufHdr %p bufHdr->pBuffer %p m_inp_buf_cnt=%d bytes=%lu", \
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::allocate_output_buffer
                                    (OMX_IN OMX_HANDLETYPE                hComp,
                                     OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                     OMX_IN OMX_U32                        port,
                                     OMX_IN OMX_PTR                     appData,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::allocate_buffer(
                                     OMX_IN OMX_HANDLETYPE                hComp,
                                     OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                     OMX_IN OMX_U32                        port,
                                     OMX_IN OMX_PTR                     appData,
...
SIDE EFFECTS:
  None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::use_buffer(
                                      OMX_IN OMX_HANDLETYPE            hComp,
                                      OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                      OMX_IN OMX_U32                   port,
                                      OMX_IN OMX_PTR                   appData,
...
SIDE EFFECTS:
  None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::use_input_buffer(
                                    OMX_IN OMX_HANDLETYPE            hComp,
                                    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                    OMX_IN OMX_U32                   port,
                                    OMX_IN OMX_PTR                   appData,
                                    ...
                                    OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("Inside omx_adpcm_adec::use_input_buffer");
    if(hComp == NULL)
    {
        port = 0;
        DEBUG_PRINT_ERROR("Returning OMX_ErrorBadParameter\n");
    ...
    if ( m_inp_current_buf_count < m_inp_act_buf_count )
    {
        ...
        if ( buf_ptr != NULL )
        {
            ...
            *bufferHdr = bufHdr;
            ...
            DEBUG_PRINT("use_input_buffer:bufHdr %p Buffer %p bytes=%lu", \
...
SIDE EFFECTS:
  None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::use_output_buffer(
                                   OMX_IN OMX_HANDLETYPE            hComp,
                                   OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                   OMX_IN OMX_U32                   port,
                                   OMX_IN OMX_PTR                   appData,
                                   ...
                                   OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("Inside omx_adpcm_adec::use_output_buffer");
    if ( m_out_current_buf_count < m_out_act_buf_count )
    {
...
None
=============================================================================*/
bool omx_adpcm_adec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
...
None
=============================================================================*/
bool omx_adpcm_adec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::free_buffer(OMX_IN OMX_HANDLETYPE         hComp,
                                           OMX_IN OMX_U32                 port,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::empty_this_buffer(
                                          OMX_IN OMX_HANDLETYPE         hComp,
                                          OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    ...
    {
        DEBUG_PRINT("omx_adpcm_adec::etb--> Buffer Size Invalid\n");
        return OMX_ErrorBadParameter;
    }
    if ( buffer->nVersion.nVersion != OMX_SPEC_VERSION )
    {
        DEBUG_PRINT("omx_adpcm_adec::etb--> OMX Version Invalid\n");
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::empty_this_buffer_proxy(
    OMX_IN OMX_HANDLETYPE         hComp,
    OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if (pcm_feedback &&  m_tmp_meta_buf )
    {
        data = m_tmp_meta_buf;
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::fill_this_buffer_proxy(
                                            OMX_IN OMX_HANDLETYPE         hComp,
                                            OMX_BUFFERHEADERTYPE* buffer)
{
    int byte_count = 0;
    ...
    if ( true == search_output_bufhdr(buffer) )
    {
        ...
        for ( nRead = 0; nRead < ncount; nRead++ )
        {
            ...
            if ( nRead == 0 )
            {
                ...
                DEBUG_PRINT("FTBP:NT2 NT2 Meta nRead=%d TS[%lld] TS[%lld]\n",\
    ...
    {
        DEBUG_PRINT("Invalid buffer address %p %p\n",buffer,buffer->pBuffer);
    }
    return OMX_ErrorNone;
}
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::fill_this_buffer(
                                            OMX_IN OMX_HANDLETYPE         hComp,
                                            OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    if ( buffer->nSize != sizeof(OMX_BUFFERHEADERTYPE) )
    {
        DEBUG_PRINT("omx_adpcm_adec::ftb--> Buffer Size Invalid\n");
        return OMX_ErrorBadParameter;
    }
    ...
    if ( buffer->nVersion.nVersion != OMX_SPEC_VERSION )
    {
        DEBUG_PRINT("omx_adpcm_adec::ftb--> OMX Version Invalid\n");
        return OMX_ErrorVersionMismatch;
    }
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::set_callbacks(
                                            OMX_IN OMX_HANDLETYPE        hComp,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
    if(hComp == NULL)
    {
        DEBUG_PRINT_ERROR("Returning OMX_ErrorBadParameter\n");
    ...
    deinit_decoder();
DEBUG_PRINT_ERROR("component ADPCM deinited\n");
    return OMX_ErrorNone;
}

/* ======================================================================
FUNCTION
  omx_adpcm_adec::deinit_decoder
...
========================================================================== */
void  omx_adpcm_adec::deinit_decoder()
{
    ...
    DEBUG_PRINT("STATS: in-buf-len[%lu]out-buf-len[%lu] tot-pb-time[%ld]",\
        m_adpcm_pb_stats.tot_in_buf_len,
        m_adpcm_pb_stats.tot_out_buf_len,
        m_adpcm_pb_stats.tot_pb_time);
    DEBUG_PRINT("STATS: fbd-cnt[%lu]ftb-cnt[%lu]etb-cnt[%lu]ebd-cnt[%lu]",\
        m_adpcm_pb_stats.fbd_cnt,m_adpcm_pb_stats.ftb_cnt,
        m_adpcm_pb_stats.etb_cnt,
        m_adpcm_pb_stats.ebd_cnt);
    DEBUG_PRINT("********************************\n");
    memset(&m_adpcm_pb_stats,0,sizeof(ADPCM_PB_STATS));
    ...
    {
        omx_adpcm_thread_stop(m_ipc_to_in_th);
        m_ipc_to_in_th = NULL;
    }

    if ( m_ipc_to_cmd_th != NULL )
    {
        omx_adpcm_thread_stop(m_ipc_to_cmd_th);
        m_ipc_to_cmd_th = NULL;
    }
    ...
    if ( pcm_feedback ==1 )
    {
        ...
        {
            omx_adpcm_thread_stop(m_ipc_to_out_th);
            m_ipc_to_out_th = NULL;
        }
    }
    ...
    if(m_ipc_to_event_th != NULL)
    {
          ...
          omx_adpcm_thread_stop(m_ipc_to_event_th);
    ...
    {
        DEBUG_PRINT_ERROR(" adpcm device already closed\n");
    }
    m_comp_deinit=1;
    m_is_out_th_sleep = 1;
    m_is_in_th_sleep = 1;
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::use_EGL_image(
                                     OMX_IN OMX_HANDLETYPE                hComp,
                                     OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
                                     OMX_IN OMX_U32                        port,
                                     OMX_IN OMX_PTR                     appData,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_adpcm_adec::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
                                                   OMX_OUT OMX_U8*       role,
                                                   OMX_IN OMX_U32       index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.adpcm";
...
None
=============================================================================*/
bool omx_adpcm_adec::allocate_done(void)
{
...
None
=============================================================================*/
bool omx_adpcm_adec::release_done(OMX_U32 param1)
{
    DEBUG_PRINT("Inside omx_adpcm_adec::release_done");
...
bool omx_adpcm_adec::omx_adpcm_fake_eos()
{
    int ret;
    META_IN meta_in;
    meta_in.offsetVal  = sizeof(META_IN);
    ...
    DEBUG_PRINT("omx_adpcm_fake_eos :ret=%d", ret);
    return true;
}
void omx_adpcm_adec::append_data_to_temp_buf()
{
    int bytes_read = 0;
    ...
    while( 1)
    {
        ...
        {
            DEBUG_PRINT("**********READ=-1**********************");
            DEBUG_PRINT("Append:Read=%u Residual pcm[%lu]",tot_bytes,
                                      m_bufMgr->getBufFilledSpace());
            DEBUG_PRINT("**********READ=-1**********************");
            break;
...
timer::timer(omx_adpcm_adec* base):
    ...
    m_timerinfo(NULL)
{
    ...
    int rc = pthread_create(&m_timerinfo->thr ,0, omx_adpcm_comp_timer_handler,
                             (void*)m_timerinfo );
    if(rc < 0)
{
...
void* omx_adpcm_comp_timer_handler(void *pT)
{
    TIMERINFO *pTime = (TIMERINFO*)pT;
    timer *pt = pTime->pTimer;
    omx_adpcm_adec *pb = pTime->base;
    int               rc = 0;
    ...
    while(1)
    {
        ...
        if( ((state== OMX_StatePause)) && !(pb->getSuspendFlg()))
        {
            ...
            pb->post_command(0,0,omx_adpcm_adec::OMX_COMPONENT_SUSPEND);
            pt->setTimerExpiry();
        }
        else
        {
