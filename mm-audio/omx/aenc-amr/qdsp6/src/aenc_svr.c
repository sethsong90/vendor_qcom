...
{
    struct amr_enc_ipc_info *amr_info = (struct amr_enc_ipc_info*)info;
    unsigned char id;
    int n;
 ...
 */
struct amr_enc_ipc_info *omx_amr_thread_create(
                                    message_func cb, 
                                    void* client_data,
                                    char* th_name)
{
    int r;
    int fds[2];
    struct amr_enc_ipc_info *amr_info;

    amr_info = calloc(1, sizeof(struct amr_enc_ipc_info));
    if (!amr_info)
    {
        return 0;
    }
...
void omx_amr_thread_stop(struct amr_enc_ipc_info *amr_info) {
...
void omx_amr_post_msg(struct amr_enc_ipc_info *amr_info, unsigned char id) {
    DEBUG_DETAIL("\n%s id=%d\n", __FUNCTION__,id); 
