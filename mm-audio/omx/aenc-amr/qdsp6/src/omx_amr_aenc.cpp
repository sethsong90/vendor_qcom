...
RETURN VALUE
  None.
========================================================================== */
omx_amr_aenc::omx_amr_aenc(): m_flush_cnt(255),
...
OMX_ERRORTYPE omx_amr_aenc::component_init(OMX_STRING role)
{
    ...
    frameDuration = ((OMX_AMR_OUTPUT_BUFFER_SIZE*1000) /
        (OMX_AMR_DEFAULT_SF * 2 *OMX_AMR_DEFAULT_CH_CFG));
    fcount = 0;
    nTimestamp = 0;
                                                 ...
                                                 OMX_IN OMX_PTR      cmdData)
{
    ...
    if (OMX_CommandStateSet == cmd)
    {
        ...
        if (OMX_StateLoaded == m_state)
        {
            ...
            {
                m_drv_fd = open("/dev/msm_amr_in", O_RDWR);
                if (m_drv_fd < 0)
                {
        ...
        else if (OMX_StateIdle == m_state)
        {
            ...
            } else if (OMX_StateExecuting == eState)
            {
                ...
                if(ioctl(m_drv_fd,AUDIO_GET_STREAM_CONFIG, &amrnb_str_cfg) == -1)
                {
                      DEBUG_PRINT_ERROR("ioctl AUDIO_GET_STREAM_CONFIG failed, \
                                            errno[%d] fd[%d]\n", errno, m_drv_fd);
                }
                amrnb_str_cfg.buffer_size = OMX_AMR_OUTPUT_BUFFER_SIZE;
                amrnb_str_cfg.buffer_count= OMX_CORE_NUM_OUTPUT_BUFFERS;
                if(ioctl(m_drv_fd,AUDIO_SET_STREAM_CONFIG, &amrnb_str_cfg) == -1)
                {
                     DEBUG_PRINT_ERROR("ioctl AUDIO_SET_STREAM_CONFIG failed, \
                                            errno[%d] fd[%d]\n", errno, m_drv_fd);
                }
                if(ioctl(m_drv_fd, AUDIO_GET_AMRNB_ENC_CONFIG, &amrnb_cfg) == -1)
                {
                    DEBUG_PRINT_ERROR("ioctl AUDIO_SET_AMRNB_ENC_CONFIG failed, \
                                            errno[%d] fd[%d]\n", errno, m_drv_fd);
                }
                amrnb_cfg.band_mode = m_amr_param.nBitRate;
                amrnb_cfg.dtx_enable = m_amr_param.eAMRDTXMode;
		amrnb_cfg.frame_format = m_amr_param.eAMRFrameFormat;
                if(ioctl(m_drv_fd, AUDIO_SET_AMRNB_ENC_CONFIG, &amrnb_cfg) == -1)
                {
                    DEBUG_PRINT_ERROR("ioctl AUDIO_SET_AMRNB_ENC_CONFIG failed, \
                                            errno[%d] fd[%d]\n", errno, m_drv_fd);
                }
                ...
                {
                    DEBUG_PRINT_ERROR("ioctl AUDIO_START failed, \
                                            errno[%d] fd[%d]\n", errno, m_drv_fd);
                    if ( m_drv_fd >= 0 )
                    {
                        close(m_drv_fd);
        ...
        else if (OMX_StateExecuting == m_state)
        {
            if (OMX_StateIdle == eState)
            {
                ...
                DEBUG_PRINT("SCP-->WAITDONE --> nNumOutputBuf=%d\n",nNumOutputBuf);
                                            ...
                                            OMX_IN OMX_PTR        paramData)
{
    ...
    {
      DEBUG_PRINT("OMX_IndexParamAudioAmr");
      m_amr_param = *((OMX_AUDIO_PARAM_AMRTYPE *) paramData);
      frameDuration = (((OMX_AMR_OUTPUT_BUFFER_SIZE)* 1000) /(OMX_AMR_DEFAULT_SF * 2));
      DEBUG_PRINT("frame duration of mono config = %d sampling rate = %d \n",frameDuration,OMX_AMR_DEFAULT_SF);
      break;
    }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            {
                m_amr_param.nChannels=pcmparam->nChannels;
 ...
 *     validity of buffer */
    ...
    {
        nDatalen = read(m_drv_fd, buffer->pBuffer,OMX_AMR_OUTPUT_BUFFER_SIZE);
        DEBUG_PRINT("FTBP: read buffer %p  #%d of size = %d\n",buffer->pBuffer,
            ++count, nDatalen);

        if((nDatalen < 0) || (nDatalen > OMX_AMR_OUTPUT_BUFFER_SIZE))
        {
        ...
        DEBUG_PRINT("FTBP-->nNumOutputBuf=%d \n",nNumOutputBuf);
    }
    else
    {
