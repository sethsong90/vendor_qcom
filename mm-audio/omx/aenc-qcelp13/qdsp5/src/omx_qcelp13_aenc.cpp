...
RETURN VALUE
  None.
========================================================================== */
omx_qcelp13_aenc::omx_qcelp13_aenc(): m_flush_cnt(255),
                                m_comp_deinit(0),
                                m_is_alloc_buf(0),
                                m_app_data(NULL),
                                m_drv_fd(-1),
...
omx_qcelp13_aenc::~omx_qcelp13_aenc()
{
    ...
    DEBUG_DETAIL("INSIDE DESTRUCTOR comp-deinit=%d\n",m_comp_deinit);
    //Do necessary cleanup if component not done with deinit
    if (m_comp_deinit == 0)
    {
         if((OMX_StateLoaded != m_state) && (OMX_StateInvalid != m_state))
         {
            DEBUG_PRINT_ERROR("%s,Destructor called in state[%d]\n",__FUNCTION__,\
                                                                m_state);
    ...
    DEBUG_DETAIL("OMX QCELP13 component destroyed\n");
    return;
}
...
void omx_qcelp13_aenc::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if (m_cb.EmptyBufferDone)
    {
        ...
        nNumInputBuf--;
        DEBUG_DETAIL("EBD CB:: in_buf_len=%d nNumInputBuf=%u ebd_cnt=%u\n",\
                     (unsigned int)m_qcelp13_pb_stats.tot_in_buf_len,
                     nNumInputBuf, (unsigned int)m_qcelp13_pb_stats.ebd_cnt);
...
void omx_qcelp13_aenc::flush_ack()
{
    ...
    DEBUG_DETAIL("Rxed FLUSH ACK cnt=%d\n",m_flush_cnt);
...
void omx_qcelp13_aenc::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if (m_cb.FillBufferDone)
    {
        ...
        //Time Stamp logic kept constant for tunnel mode, as DSP does not support
        if(!pcm_input) {
             if (fcount == 0) {
...
void omx_qcelp13_aenc::process_out_port_msg(void *client_data, unsigned char id)
{
    ...
    {
        DEBUG_DETAIL(" OUT: IN LOADED STATE RETURN\n");
        return;
    }
    ...
    {
        pthread_mutex_unlock(&pThis->m_outputlock);
        DEBUG_DETAIL("OUT: ***in OMX_StateLoaded so exiting\n");
        return ;
    } else
    {
        qsize = 0;
...
void omx_qcelp13_aenc::process_command_msg(void *client_data, unsigned char id)
{
    ...
    if (OMX_COMPONENT_GENERATE_EVENT == id)
    {
        if (pThis->m_cb.EventHandler)
        {
            if (OMX_CommandStateSet == p1)
            {
                ...
                pthread_mutex_unlock(&pThis->m_state_lock);
                DEBUG_DETAIL("CMD:Process->state set to %d \n", \
                    pThis->m_state);

                if(pThis->m_idle_transition)
                {
                    pThis->m_idle_transition = false;
                }
    ...
    {
        DEBUG_DETAIL("CMD: ***not in executing state so exiting\n");
        return ;
    } else
    {
...
void omx_qcelp13_aenc::process_in_port_msg(void *client_data, unsigned char id)
{
    ...
    if (state == OMX_StateLoaded)
    {
        DEBUG_DETAIL(" IN: IN LOADED STATE RETURN\n");
        return;
    }
    ...
    {
        pthread_mutex_unlock(&pThis->m_lock);
        DEBUG_DETAIL("IN: ***in OMX_StateLoaded so exiting\n");
        return ;
    } else
    {
        qsize = 0;
...
OMX_ERRORTYPE omx_qcelp13_aenc::component_init(OMX_STRING role)
{
    ...
    m_qcelp13_param.nPortIndex = OMX_CORE_OUTPUT_PORT_INDEX;
    m_qcelp13_param.nChannels = OMX_QCELP13_DEFAULT_CH_CFG;
    m_qcelp13_param.eCDMARate = OMX_AUDIO_CDMARateFull; //Current DSP does not have config
    ...
    m_pcm_param.nPortIndex = OMX_CORE_INPUT_PORT_INDEX;
    ...
    DEBUG_DETAIL("component init: role = %s\n",role);
    ...
    if(0 == pcm_input)
    {
        m_drv_fd = open("/dev/msm_voicememo", O_RDONLY);
    } else
    {
        m_drv_fd = open("/dev/msm_voicememo", O_RDWR);
    }
    #endif
    if (m_drv_fd < 0)
    {
                                           ...
                                           OMX_IN OMX_PTR      cmdData)
{
    ...
    DEBUG_DETAIL("send_command : recieved state before semwait= %lu\n",param1);
    sem_wait (&sem_States);
    DEBUG_DETAIL("send_command : recieved state after semwait\n");
    return OMX_ErrorNone;
}
    ...
    if (OMX_CommandStateSet == cmd)
    {
        ...
        if (OMX_StateLoaded == m_state)
        {
            if (OMX_StateIdle == eState)
            {
                    ...
                    {
                        DEBUG_DETAIL("SCP-->Allocate Done Complete\n");
                    } else
                    {
                        DEBUG_DETAIL("SCP-->Loaded to Idle-Pending\n");
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Loaded-->Loaded\n");
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Loaded-->WaitForResources\n");
                eRet = OMX_ErrorNone;
            }

            else if (eState == OMX_StateExecuting)
            {
                DEBUG_DETAIL("OMXCORE-SM: Loaded-->Executing\n");
                m_cb.EventHandler(&this->m_cmp,
                    this->m_app_data,
                    OMX_EventError,
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Loaded-->Pause\n");
                m_cb.EventHandler(&this->m_cmp,
                    this->m_app_data,
                    OMX_EventError,
                    OMX_ErrorIncorrectStateTransition,
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Loaded-->Invalid\n");
                m_cb.EventHandler(&this->m_cmp,
                    this->m_app_data,
                    OMX_EventError,
                    OMX_ErrorInvalidState,
        ...
        else if (OMX_StateIdle == m_state)
        {
            if (OMX_StateLoaded == eState)
            {
                if (release_done(-1))
                {
                    ...
                    {
                        DEBUG_DETAIL("SCP:Idle->Loaded,ioctl stop failed %d\n",\
                            errno);
                    }

                    nTimestamp=0;

                    DEBUG_DETAIL("SCP-->Idle to Loaded\n");
                } else
                {
                    DEBUG_DETAIL("SCP--> Idle to Loaded-Pending\n");
            ...
            } else if (OMX_StateExecuting == eState)
            {
               ...
               struct msm_audio_config pcm_cfg;
                ...
                if(ioctl(m_drv_fd,AUDIO_GET_QCELP_ENC_CONFIG ,&qcelp_cfg) == -1)
                {
                     DEBUG_PRINT("ioctl AUDIO_GET_QCELP_ENC_CONFIG failed, errno[%d]\n", errno);
                }

                qcelp_cfg.min_bit_rate = m_qcelp13_param.nMinBitRate;
                qcelp_cfg.max_bit_rate = m_qcelp13_param.nMaxBitRate;
                qcelp_cfg.cdma_rate = m_qcelp13_param.eCDMARate;
                if(ioctl(m_drv_fd,AUDIO_SET_QCELP_ENC_CONFIG ,&qcelp_cfg) == -1)
                {
                     DEBUG_PRINT("ioctl AUDIO_SET_QCELP_ENC_CONFIG failed, errno[%d]\n", errno);
                }
                if(pcm_input)
                {
                ...
                {
                    //0, 1, 2 valid for incall, any other treated as not in incall
...
#else
                struct msm_audio_voicememo_config drv_qcelp_enc_config;
                if(ioctl(m_drv_fd, AUDIO_GET_VOICEMEMO_CONFIG,&drv_qcelp_enc_config) == -1)
                {
                    DEBUG_PRINT("ioctl AUDIO_GET_VOICEMEMO_CONFIG failed, errno[%d]\n", errno);
                }
                switch(m_recPath)
                {
                    case 0:
                        drv_qcelp_enc_config.rec_type = 1;
                        break;
                    case 1:
                        drv_qcelp_enc_config.rec_type = 2;
                        break;
                    case 2 :
                        drv_qcelp_enc_config.rec_type = 3;
                        break;
                }
                drv_qcelp_enc_config.capability = CAPABILITY;
                drv_qcelp_enc_config.min_rate = m_qcelp13_param.nMinBitRate;
                drv_qcelp_enc_config.max_rate = m_qcelp13_param.nMaxBitRate;
                drv_qcelp_enc_config.frame_format = FRAMEFORMAT;
                if(ioctl(m_drv_fd, AUDIO_SET_VOICEMEMO_CONFIG, &drv_qcelp_enc_config) == -1)
                {
                    DEBUG_PRINT("ioctl AUDIO_SET_VOICEMEMO_CONFIG failed, errno[%d]\n", errno);
                }
#endif
                frameDuration = FRAME_DURATION * MILLISECOND;
                DEBUG_PRINT("ioctl AUDIO_START\n");
                ...
                DEBUG_DETAIL("OMXCORE-SM: Idle-->Executing\n");
            } else if (eState == OMX_StateIdle)
            {
                DEBUG_DETAIL("OMXCORE-SM: Idle-->Idle\n");
                m_cb.EventHandler(&this->m_cmp,
                    this->m_app_data,
                    OMX_EventError,
                    OMX_ErrorSameState,
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Idle-->WaitForResources\n");
            ...
            {
                DEBUG_DETAIL("OMXCORE-SM: Idle-->Pause\n");
            }

            else if (eState == OMX_StateInvalid)
            {
                DEBUG_DETAIL("OMXCORE-SM: Idle-->Invalid\n");
                m_state = OMX_StateInvalid;
        ...
        else if (OMX_StateExecuting == m_state)
        {
            if (OMX_StateIdle == eState)
            {
                DEBUG_DETAIL("SCP-->Executing to Idle \n");
                m_idle_transition = 1;
                if (0 == pcm_input)
                {
                    // Driver does not support flush in tunnel mode
                    //execute_omx_flush(1,false);  // Flush o/p port
                } else
                {
                                            ...
                                            OMX_INOUT OMX_PTR     paramData)
{
    ...
    case OMX_IndexParamAudioPcm:
        {
            DEBUG_PRINT("get_parameter: OMX_IndexParamAudioPcm\n");
            ...
            if (OMX_CORE_INPUT_PORT_INDEX == pcmparam->nPortIndex)
            {
                memcpy(pcmparam, &m_pcm_param,\
                    ...
                    DEBUG_PRINT("get_parameter:OMX_IndexParamAudioPcm: %lu %lu",\
                                 m_pcm_param.nChannels,m_pcm_param.nSamplingRate);
            } else
            {
    ...
    case OMX_IndexParamAudioQcelp13:
        {
            OMX_AUDIO_PARAM_QCELP13TYPE *qcelp13Param = (OMX_AUDIO_PARAM_QCELP13TYPE *) paramData;
            DEBUG_PRINT("OMX_IndexParamAudioQcelp13\n");

            if (OMX_CORE_OUTPUT_PORT_INDEX== qcelp13Param->nPortIndex)
            {
                 memcpy(qcelp13Param,&m_qcelp13_param, sizeof(OMX_AUDIO_PARAM_QCELP13TYPE));
            } else
            {
                 DEBUG_PRINT_ERROR("get_parameter:OMX_IndexParamAudioQcelp13 "\
                                   "OMX_ErrorBadPortIndex %d\n", \
                                            ...
                                            OMX_IN OMX_PTR        paramData)
{
     ...
     case OMX_IndexParamAudioQcelp13:
    {
            ...
            if (OMX_CORE_OUTPUT_PORT_INDEX== qcelp13Param->nPortIndex)
            {
                 memcpy(&m_qcelp13_param, qcelp13Param, sizeof(OMX_AUDIO_PARAM_QCELP13TYPE));
            } else
            {
                 DEBUG_PRINT_ERROR("get_parameter:OMX_IndexParamAudioQcelp13 "\
                                   "OMX_ErrorBadPortIndex %d\n", \
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            if (OMX_CORE_INPUT_PORT_INDEX== pcmparam->nPortIndex)
            {
                memcpy(&m_pcm_param,pcmparam,\
                        sizeof(OMX_AUDIO_PARAM_PCMMODETYPE));
                DEBUG_PRINT("set_parameter:OMX_IndexParamAudioPcm: %lu %lu",\
                                 m_pcm_param.nChannels,m_pcm_param.nSamplingRate);
            } else
            {
                DEBUG_PRINT_ERROR("get_parameter:OMX_IndexParamAudioPcm "
...
========================================================================== */
OMX_ERRORTYPE  omx_qcelp13_aenc::get_extension_index(OMX_IN OMX_HANDLETYPE      hComp,
    ...
    if(m_inp_current_buf_count < m_inp_act_buf_count)
    {
        ...
        if (buf_ptr != NULL)
        {
            ...
            *bufferHdr = bufHdr;
            ...
            bufHdr->pBuffer           = (OMX_U8 *)((buf_ptr) + sizeof(OMX_BUFFERHEADERTYPE));
...
/**
  ...
  @return Error status
*/
OMX_ERRORTYPE  omx_qcelp13_aenc::free_buffer(OMX_IN OMX_HANDLETYPE         hComp,
                                          OMX_IN OMX_U32                 port,
                                          OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
/**
 ...
 @return error status
 */
OMX_ERRORTYPE  omx_qcelp13_aenc::empty_this_buffer(OMX_IN OMX_HANDLETYPE         hComp,
                                                OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;

    DEBUG_PRINT("ETB:Buf:%p Len %lu TS %llu numInBuf=%d\n", \
    ...
    OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    DEBUG_PRINT("Requesting to write %lu bytes now\n",(buffer->nFilledLen+sizeof(META_IN)));
    DEBUG_PRINT("nflags: 0x%8x ts_lsw: 0x%8x ts_msw: 0x%8x\n", (unsigned int)buffer->nFlags,
            ((unsigned int) ((tmp1 & 0xFFFFFFFF00000000LL) >> 32)),
            ((unsigned int) (tmp1 & 0xFFFFFFFFLL)));
    ...
    DEBUG_PRINT("calling bufferdone nNumInputBuf = %d\n", nNumInputBuf);
    buffer_done_cb(buffer);
    return OMX_ErrorNone;
}

OMX_ERRORTYPE  omx_qcelp13_aenc::fill_this_buffer_proxy(OMX_IN OMX_HANDLETYPE         hComp,
                                                    OMX_BUFFERHEADERTYPE* buffer)
{
    static int count = 0;
    int nDatalen = 0;
    const char g_frmInfo[5] = { 1, 4, 8, 17 ,35};
    OMX_U8 buf[OMX_QCELP13_OUTPUT_BUFFER_SIZE+sizeof(META_OUT)]; // Common read buffer, with meta
    META_OUT meta_out;
    OMX_TICKS tmp1 = 0;
    ...
    if(search_output_bufhdr(buffer) == true)
    {
        if(!pcm_input)
        {
            nDatalen = read(m_drv_fd, buf,OMX_QCELP13_OUTPUT_BUFFER_SIZE);
            if(buf[1] < sizeof(g_frmInfo)) {
                memcpy(buffer->pBuffer,&buf[1],g_frmInfo[buf[1]]);
                DEBUG_PRINT("FTBP: read buffer %p  #%d of size = %d\n",buffer->pBuffer,
                        ++count, nDatalen);
            } else
                nDatalen = -1;
            if((nDatalen < 0) || (nDatalen > OMX_QCELP13_OUTPUT_BUFFER_SIZE))
            {
            ...
            {
                buffer->nFilledLen = g_frmInfo[buf[1]];
        ...
        } else
        {
            ...
            {	DEBUG_PRINT("Read %d bytes from driver \n",(nDatalen-sizeof(META_OUT)));
                ...
                DEBUG_PRINT("nflags: 0x%8x ts_msw: 0x%8x ts_lsw: 0x%8x\n",(unsigned int)buffer->nFlags,
                        ((unsigned int) ((tmp1 & 0xFFFFFFFF00000000LL) >> 32)),
                        ((unsigned int) (tmp1 & 0xFFFFFFFFLL)));
                ...
                if(buf[sizeof(META_OUT)+1] < sizeof(g_frmInfo)) {
                    memcpy(buffer->pBuffer, &buf[sizeof(META_OUT) + 1], g_frmInfo[buf[sizeof(META_OUT) +1]]);
                    buffer->nFilledLen = g_frmInfo[buf[sizeof(META_OUT) +1]];
                } else {
                    DEBUG_PRINT("frame read zero \n");
                    buffer->nFilledLen = 0;
                }
...
========================================================================== */
OMX_ERRORTYPE  omx_qcelp13_aenc::set_callbacks(OMX_IN OMX_HANDLETYPE        hComp,
...
========================================================================== */
OMX_ERRORTYPE  omx_qcelp13_aenc::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
                                                  OMX_OUT OMX_U8*        role,
