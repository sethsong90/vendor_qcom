...
OMX_ERRORTYPE omx_qcelp13_aenc::component_init(OMX_STRING role)
{
    ...
    frameDuration = ((OMX_QCELP13_OUTPUT_BUFFER_SIZE*1000) /
        (OMX_QCELP13_DEFAULT_SF * 2 *OMX_QCELP13_DEFAULT_CH_CFG));
    fcount = 0;
    nTimestamp = 0;
                                           ...
                                           OMX_IN OMX_PTR      cmdData)
{
    ...
    DEBUG_DETAIL("send_command : recieved state before semwait= %d\n",param1);
    sem_wait (&sem_States);
    ...
    if (OMX_CommandStateSet == cmd)
    {
        ...
        if (OMX_StateLoaded == m_state)
        {
            if (OMX_StateIdle == eState)
            {
                ...
                {
                    DEBUG_DETAIL("SCP:Loaded-->Idle fd[%d]\n",m_drv_fd);
        ...
        else if (OMX_StateIdle == m_state)
        {
            ...
            } else if (OMX_StateExecuting == eState)
            {
                ...
                struct msm_audio_qcelp_enc_config drv_qcelp_enc_config;

                DEBUG_DETAIL("configure Driver for QCELP13 Encoding sample rate = %d \n",OMX_QCELP13_DEFAULT_SF);
                if(ioctl(m_drv_fd, AUDIO_GET_STREAM_CONFIG, &drv_stream_config) == -1)
                {
                    DEBUG_PRINT("ioctl AUDIO_GET_STREAM_CONFIG failed, errno[%d]\n", errno);
                }
                drv_stream_config.buffer_size = OMX_QCELP13_OUTPUT_BUFFER_SIZE;
                ...
                if(ioctl(m_drv_fd, AUDIO_GET_QCELP_ENC_CONFIG, &drv_qcelp_enc_config) == -1)
                {
                    DEBUG_PRINT("ioctl AUDIO_GET_QCELP_ENC_CONFIG failed, errno[%d]\n", errno);
                }
                drv_qcelp_enc_config.min_bit_rate = m_qcelp13_param.nMinBitRate;
                drv_qcelp_enc_config.max_bit_rate = m_qcelp13_param.nMaxBitRate;
                if(ioctl(m_drv_fd, AUDIO_SET_QCELP_ENC_CONFIG, &drv_qcelp_enc_config) == -1)
                {
        ...
        else if (OMX_StateExecuting == m_state)
        {
            if (OMX_StateIdle == eState)
            {
                ...
                if(nNumOutputBuf)
                {
                    DEBUG_DETAIL("posting sem_States\n");
                    sem_post (&sem_States);
                    pthread_mutex_unlock(&out_buf_count_lock);
                    wait_for_event();
                }
                                            ...
                                            OMX_IN OMX_PTR        paramData)
{
    ...
    {
      DEBUG_PRINT("OMX_IndexParamAudioQcelp13");
      m_qcelp13_param = *((OMX_AUDIO_PARAM_QCELP13TYPE *) paramData);
      frameDuration = (((OMX_QCELP13_OUTPUT_BUFFER_SIZE)* 1000) /(OMX_QCELP13_DEFAULT_SF * 2));
      DEBUG_PRINT("frame duration of mono config = %d sampling rate = %d \n",frameDuration,OMX_QCELP13_DEFAULT_SF);
      break;
    }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            {
                m_qcelp13_param.nChannels=pcmparam->nChannels;
 ...
 *     validity of buffer */
    ...
    if(search_output_bufhdr(buffer) == true)
    {
        ...
        nDatalen = read(m_drv_fd, buffer->pBuffer,OMX_QCELP13_OUTPUT_BUFFER_SIZE);
