...
#define MIN_BITRATE 4 /* Bit rate 1 - 0.8 , 2 - x , 3 - 4.0 , 4 - 8.5 kbps*/
...
int main(int argc, char **argv) 
{
	...
	if (argc >= 1) {
		out_filename = argv[1];
		  ...
		  DEBUG_PRINT("ex: ./mm-aenc-omxevrc EVRC_OUTPUTFILE\n");
		  return 0;
	}
...
int Init_Encoder()
{
	...
	omxresult = OMX_GetHandle((OMX_HANDLETYPE*)(&evrc_enc_handle), 
			"OMX.qcom.audio.encoder.tunneled.evrc", NULL, &call_back);
         printf("test1\n");
	if (FAILED(omxresult)) {
