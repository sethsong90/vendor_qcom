...
OMX_ERRORTYPE omx_evrc_aenc::component_init(OMX_STRING role)
{
    ...
    frameDuration = ((OMX_EVRC_OUTPUT_BUFFER_SIZE*1000) /
        (OMX_EVRC_DEFAULT_SF * 2 *OMX_EVRC_DEFAULT_CH_CFG));
    fcount = 0;
    nTimestamp = 0;
    ...
    if (OMX_CommandStateSet == cmd)
    {
        ...
        else if (OMX_StateIdle == m_state)
        {
            ...
            } else if (OMX_StateExecuting == eState)
            {
                ...
                DEBUG_DETAIL("configure Driver for EVRC Encoding sample rate = %d \n",OMX_EVRC_DEFAULT_SF);
                if(ioctl(m_drv_fd,AUDIO_GET_STREAM_CONFIG,&drv_stream_config) == -1)
                {
                    DEBUG_PRINT("ioctl AUDIO_GET_STREAM_CONFIG failed, errno[%d]\n", errno);
                }
                drv_stream_config.buffer_size = OMX_EVRC_OUTPUT_BUFFER_SIZE;
                ...
                if(ioctl(m_drv_fd, AUDIO_GET_EVRC_ENC_CONFIG, &drv_evrc_enc_config) == -1)
                {
                ...
                if(ioctl(m_drv_fd, AUDIO_SET_EVRC_ENC_CONFIG, &drv_evrc_enc_config) == -1)
                {
                                            ...
                                            OMX_IN OMX_PTR        paramData)
{
     ...
     case OMX_IndexParamAudioEvrc:
    {
      ...
      frameDuration = (((OMX_EVRC_OUTPUT_BUFFER_SIZE)* 1000) /(OMX_EVRC_DEFAULT_SF * 2));
      DEBUG_PRINT("frame duration of mono config = %d sampling rate = %d \n",frameDuration,OMX_EVRC_DEFAULT_SF);
      break;
    }
    ...
    case OMX_IndexParamAudioPcm:
        {
            ...
            {
                m_evrc_param.nChannels=pcmparam->nChannels;
 ...
 *     validity of buffer */
    ...
    if(search_output_bufhdr(buffer) == true)
    {
        ...
        nDatalen = read(m_drv_fd, buffer->pBuffer,OMX_EVRC_OUTPUT_BUFFER_SIZE);
