...
RETURN VALUE
  None.
========================================================================== */
omx_evrc_aenc::omx_evrc_aenc(): m_flush_cnt(255),
                                m_comp_deinit(0),
                                m_is_alloc_buf(0),
                                m_app_data(NULL),
                                m_drv_fd(-1),
...
omx_evrc_aenc::~omx_evrc_aenc()
{
    ...
    DEBUG_DETAIL("OMX EVRC component destroyed\n");
    return;
}
...
void omx_evrc_aenc::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if (m_cb.EmptyBufferDone)
    {
        ...
        DEBUG_DETAIL("EBD CB:: in_buf_len=%d nNumInputBuf=%u ebd_cnt=%u\n",\
                     (unsigned int)m_evrc_pb_stats.tot_in_buf_len,
                     nNumInputBuf, (unsigned int)m_evrc_pb_stats.ebd_cnt);
...
OMX_ERRORTYPE omx_evrc_aenc::component_init(OMX_STRING role)
{
    ...
    m_evrc_param.nPortIndex = OMX_CORE_OUTPUT_PORT_INDEX;
    m_evrc_param.nChannels = OMX_EVRC_DEFAULT_CH_CFG;
    m_evrc_param.eCDMARate = OMX_AUDIO_CDMARateFull; //Current DSP does not have config
    ...
    if (OMX_CommandStateSet == cmd)
    {
        ...
        else if (OMX_StateIdle == m_state)
        {
            ...
            } else if (OMX_StateExecuting == eState)
            {
                ...
                if(ioctl(m_drv_fd,AUDIO_GET_EVRC_ENC_CONFIG ,&evrc_cfg) == -1)
                {
                     DEBUG_PRINT("ioctl AUDIO_GET_EVRC_ENC_CONFIG failed, errno[%d]\n", errno);
                }
                evrc_cfg.min_bit_rate = m_evrc_param.nMinBitRate;
                evrc_cfg.max_bit_rate = m_evrc_param.nMaxBitRate;
                evrc_cfg.cdma_rate =  m_evrc_param.eCDMARate;

                if(ioctl(m_drv_fd,AUDIO_SET_EVRC_ENC_CONFIG ,&evrc_cfg) == -1)
                {
                     DEBUG_PRINT("ioctl AUDIO_SET_EVRC_ENC_CONFIG failed, errno[%d]\n", errno);
                }
                if(pcm_input)
                {
...
#else
                struct  msm_audio_voicememo_config drv_evrc_enc_config;

                if(ioctl(m_drv_fd,AUDIO_GET_VOICEMEMO_CONFIG,&drv_evrc_enc_config) == -1)
                {
                ...
                {
                    case 0:
                        drv_evrc_enc_config.rec_type = 1;
                        break;
                    case 1:
                        drv_evrc_enc_config.rec_type = 2;
                        break;
                    case 2 :
                        drv_evrc_enc_config.rec_type = 3;
                        break;
                }
                drv_evrc_enc_config.capability = CAPABILITY;
                drv_evrc_enc_config.min_rate = m_evrc_param.nMinBitRate;
                drv_evrc_enc_config.max_rate = m_evrc_param.nMaxBitRate;
                drv_evrc_enc_config.frame_format = FRAMEFORMAT;
                if(ioctl(m_drv_fd,AUDIO_SET_VOICEMEMO_CONFIG, &drv_evrc_enc_config) == -1)
                {
                                            ...
                                            OMX_INOUT OMX_PTR     paramData)
{
    ...
    case OMX_IndexParamAudioEvrc:
        {
            OMX_AUDIO_PARAM_EVRCTYPE *evrcParam = (OMX_AUDIO_PARAM_EVRCTYPE *) paramData;
            DEBUG_PRINT("OMX_IndexParamAudioEvrc\n");

                if (OMX_CORE_OUTPUT_PORT_INDEX== evrcParam->nPortIndex)
                {
                    memcpy(evrcParam,&m_evrc_param, sizeof(OMX_AUDIO_PARAM_EVRCTYPE));
                } else
                {
                                            ...
                                            OMX_IN OMX_PTR        paramData)
{
     ...
     case OMX_IndexParamAudioEvrc:
    {
      ...
      if (OMX_CORE_OUTPUT_PORT_INDEX == evrcParam->nPortIndex)
      {
            memcpy(&m_evrc_param,evrcParam,sizeof(OMX_AUDIO_PARAM_EVRCTYPE));
      } else
      {
            DEBUG_PRINT_ERROR("set_parameter:OMX_IndexParamAudioEvrc "\
                   "OMX_ErrorBadPortIndex %d\n", \
                   (int)evrcParam->nPortIndex);
            eRet = OMX_ErrorBadPortIndex;
      }
...
========================================================================== */
OMX_ERRORTYPE  omx_evrc_aenc::get_extension_index(OMX_IN OMX_HANDLETYPE      hComp,
...
/**
 ...
 @return error status
 */
OMX_ERRORTYPE  omx_evrc_aenc::empty_this_buffer(OMX_IN OMX_HANDLETYPE         hComp,
                                                OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
...
OMX_ERRORTYPE  omx_evrc_aenc::fill_this_buffer_proxy(OMX_IN OMX_HANDLETYPE         hComp,
                                                    OMX_BUFFERHEADERTYPE* buffer)
{
    static int count = 0;
    int nDatalen = 0;
    const char g_frmInfo[5] = {1,3,6,11,23};
    OMX_U8 buf[OMX_EVRC_OUTPUT_BUFFER_SIZE+sizeof(META_OUT)]; // Common read buffer, with meta
    META_OUT meta_out;
    OMX_TICKS tmp1 = 0;
    ...
    if(search_output_bufhdr(buffer) == true)
    {
        if(!pcm_input)
        {
            nDatalen = read(m_drv_fd, buf,OMX_EVRC_OUTPUT_BUFFER_SIZE);
            if(buf[1] < sizeof(g_frmInfo)) {
                ...
                nDatalen = -1;
            if((nDatalen < 0) || (nDatalen > OMX_EVRC_OUTPUT_BUFFER_SIZE))
            {
        ...
        } else
        {
            ...
            nDatalen =  read(m_drv_fd, buf, OMX_EVRC_OUTPUT_BUFFER_SIZE+sizeof(META_OUT));
