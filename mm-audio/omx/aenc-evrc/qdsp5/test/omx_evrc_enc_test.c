...
/*
    An Open max test application for EVRC-NB Encoding....
*/
                           ...
                           OMX_IN OMX_PTR pEventData)
{
    ...
    switch(eEvent) {
        case OMX_EventCmdComplete:
            DEBUG_PRINT("\n OMX_EventCmdComplete event=%u data1=%lu data2=%lu\n",(OMX_EVENTTYPE)eEvent,
                nData1,nData2);
             event_complete();
             break;
                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if(bInputEosReached)
    {
      ...
      DEBUG_PRINT("omx_evrc_aenc_test: bFlushing is set to TRUE used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
      if (used_ip_buf_cnt == 0) {
        bFlushing = false;
      } else {
        DEBUG_PRINT("omx_evrc_aenc_test: more buffer to come back used_ip_buf_cnt=%d\n",used_ip_buf_cnt);
        return OMX_ErrorNone;
      }
    }
                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    {
        DEBUG_PRINT(" file write failed, not able to store EVRC encoded data \n");
        return OMX_ErrorNone;
    }
...
int main(int argc, char **argv)
{
	...
	if ( argc >=9) {
        ...
        if(recpath == 3 ) {
          ...
          DEBUG_PRINT("ex: ./mm-aenc-omxevrc OUTPUTFILE MINRATE MAXRATE CDMARATE RECORDPATH RECORDTIME INPUTFILE Tunnel\n");
...
int Init_Encoder(OMX_STRING audio_component)
{
    ...
    if (FAILED(omxresult)) {
        ...
        DEBUG_PRINT ("\nComponent is in LOADED state: %s\n", audio_component);
    }
...
int Rec_Encoder()
{
    ...
    OMX_GetExtensionIndex(evrc_enc_handle,
    ...
    OMX_SetParameter(evrc_enc_handle,recPathIndex,&recinfoparam);
#ifdef AUDIOV2
    ...
    wait_for_event();
    DEBUG_PRINT(" Start sending OMX_FILL this buffer\n");
