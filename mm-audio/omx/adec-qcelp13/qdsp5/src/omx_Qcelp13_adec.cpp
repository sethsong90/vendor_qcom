...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::transcode_data

DESCRIPTION
Trancodes the Input Qcelp13 data to DSP format.
...
OMX_U8 omx_Qcelp13_adec::transcode_data(OMX_U8 **src,
                                        OMX_U32 srcDataLen,
...
omx_Qcelp13_adec::omx_cmd_queue::~omx_cmd_queue()
{
    // Nothing to do
}

// omx cmd queue constructor
omx_Qcelp13_adec::omx_cmd_queue::omx_cmd_queue(): m_read(0),m_write(0),m_size(0)
{
    memset(m_q,      0,sizeof(omx_event)*OMX_CORE_CONTROL_CMDQ_SIZE);
}

// omx cmd queue insert
bool omx_Qcelp13_adec::omx_cmd_queue::insert_entry(unsigned p1,
                                                   unsigned p2,
                                                   unsigned id)
{
    bool ret = true;
...
None
=============================================================================*/
bool omx_Qcelp13_adec::omx_cmd_queue::pop_entry(unsigned *p1,
...
{
    return(new omx_Qcelp13_adec);
}
bool omx_Qcelp13_adec::omx_cmd_queue::get_msg_id(unsigned *id)
{
   if(m_size > 0)
   {
       *id = m_q[m_read].id;
...
void omx_Qcelp13_adec::process_event_cb(void *client_data, unsigned char id)
{
    DEBUG_PRINT("PE:Waiting for event's...");
    omx_Qcelp13_adec  *pThis = (omx_Qcelp13_adec *) client_data;
    pThis->process_events(pThis);
}
void omx_Qcelp13_adec::process_events(omx_Qcelp13_adec *client_data)
{
    OMX_STATETYPE          state;
...
None
=============================================================================*/
void omx_Qcelp13_adec::wait_for_event()
{
...
None
=============================================================================*/
void omx_Qcelp13_adec::event_complete()
{
    pthread_mutex_lock(&m_event_lock);
...
void omx_Qcelp13_adec::in_th_goto_sleep()
{
...
void omx_Qcelp13_adec::in_th_wakeup()
{
    pthread_mutex_lock(&m_in_th_lock);
...
void omx_Qcelp13_adec::out_th_goto_sleep()
{
...
void omx_Qcelp13_adec::out_th_wakeup()
{
    pthread_mutex_lock(&m_out_th_lock);
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::omx_Qcelp13_adec
...
None.
========================================================================== */
omx_Qcelp13_adec::omx_Qcelp13_adec(): m_tmp_meta_buf(NULL),
        m_flush_cnt(255),
        m_is_alloc_buf(0),
        m_residual_data_len(0),
        m_app_data(NULL),
...
omx_Qcelp13_adec::~omx_Qcelp13_adec()
{
    DEBUG_PRINT_ERROR("QCELP Object getting destroyed comp-deinit=%d\n",m_comp_deinit);
    if (mime_type)
    {
        free(mime_type);
        mime_type = NULL;
...
@return none
*/
void omx_Qcelp13_adec::buffer_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if ( m_cb.EmptyBufferDone )
    {
        ...
        m_qcelp_pb_stats.ebd_cnt++;
        nNumInputBuf--;
        DEBUG_DETAIL("EBD CB:: in_buf_len=%d nNumInputBuf=%d\n",\
            m_qcelp_pb_stats.tot_in_buf_len,
            nNumInputBuf, m_qcelp_pb_stats.ebd_cnt);
...
None
=============================================================================*/
void omx_Qcelp13_adec::flush_ack()
{
...
void omx_Qcelp13_adec::frame_done_cb(OMX_BUFFERHEADERTYPE *bufHdr)
{
    if ( m_cb.FillBufferDone )
    {
        PrintFrameHdr(OMX_COMPONENT_GENERATE_FRAME_DONE,bufHdr);
        m_qcelp_pb_stats.fbd_cnt++;
        pthread_mutex_lock(&out_buf_count_lock);
        nNumOutputBuf--;
        DEBUG_PRINT("FBD CB:: nNumOutputBuf=%d out_buf_len=%d\n",\
            nNumOutputBuf,
            m_qcelp_pb_stats.tot_out_buf_len,
            m_qcelp_pb_stats.fbd_cnt);
        m_qcelp_pb_stats.tot_out_buf_len += bufHdr->nFilledLen;
        m_qcelp_pb_stats.tot_pb_time     = bufHdr->nTimeStamp;
...
None
=============================================================================*/
void omx_Qcelp13_adec::process_out_port_msg(void *client_data, unsigned char id)
{
...
None
=============================================================================*/
void omx_Qcelp13_adec::process_command_msg(void *client_data, unsigned char id)
{
    unsigned     p1;         // Parameter - 1
    unsigned     p2;         // Parameter - 2
    unsigned     ident;
...
None
=============================================================================*/
void omx_Qcelp13_adec::process_in_port_msg(void *client_data, unsigned char id)
{
    unsigned      p1;       // Parameter - 1
    unsigned      p2;       // Parameter - 2
    unsigned      ident;
...
@return Error status
*/
OMX_ERRORTYPE omx_Qcelp13_adec::component_init(OMX_STRING role)
{
    ...
    incorrect param */
    memset(&m_qcelp_param, 0, sizeof(m_qcelp_param));
    m_qcelp_param.nSize = sizeof(m_qcelp_param);
    m_qcelp_param.nChannels = OMX_QCELP_DEFAULT_CH_CFG;

    memset(&m_pcm_param, 0, sizeof(m_pcm_param));
    m_pcm_param.nChannels = OMX_QCELP_DEFAULT_CH_CFG;
    ...
    m_volume = OMX_QCELP_DEFAULT_VOL; /* Close to unity gain */

    memset(&m_qcelp_pb_stats,0,sizeof(QCELP_PB_STATS));
    nTimestamp = 0;
    ...
    mime_type = (OMX_STRING) malloc(sizeof("audio/Qcelp13"));
    if (mime_type)
    {
        strcpy(mime_type, "audio/Qcelp13");
    ...
    if ( !strcmp(role,"OMX.qcom.audio.decoder.Qcelp13Hw") )
    {
        pcm_feedback = 1;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    } else if ( !strcmp(role,"OMX.qcom.audio.decoder.tunneled.Qcelp13Hw") )
    {
        pcm_feedback = 0;
        DEBUG_PRINT("\ncomponent_init: Component %s LOADED \n", role);
    } else
    ...
    if(0 == pcm_feedback)
    {
        m_drv_fd = open("/dev/msm_qcelp",O_WRONLY);
    }
    else
    {
        m_drv_fd = open("/dev/msm_qcelp",O_RDWR);
    }
    if ( m_drv_fd < 0 )
    {
...
@return Error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::get_component_version(
...
@return Error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::send_command(OMX_IN OMX_HANDLETYPE hComp,
...
@return error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::send_command_proxy(OMX_IN OMX_HANDLETYPE hComp,
                                                    OMX_IN OMX_COMMANDTYPE  cmd,
                                                    OMX_IN OMX_U32       param1,
                                                    OMX_IN OMX_PTR      cmdData)
{
    ...
    if ( OMX_CommandStateSet == cmd )
    {
        ...
        else if ( OMX_StateIdle == m_state )
        {
            ...
            } else if ( OMX_StateExecuting == eState )
            {
                ...
                {
                  m_ipc_to_event_th = omx_Qcelp13_event_thread_create(
                ...
                drv_config.sample_rate = 8000;//m_qcelp_param.nSampleRate;
                drv_config.channel_count = m_qcelp_param.nChannels;
                drv_config.type = 9; // Qcelp13 decoding
                if(pcm_feedback)
                {
                ...
                pcm_config.buffer_size  =
                    OMX_QCELP13_OUTPUT_BUFFER_SIZE+ sizeof(META_OUT);
	...
	bFlag = 0;
        if ( param1 == OMX_CORE_INPUT_PORT_INDEX || param1 == OMX_ALL )
        {
            ...
            else
            {
                ...
                {
                    DEBUG_PRINT("SCP: flush in Disable param1=%d state=%d \n",\
                        param1, m_state);
                    execute_omx_flush(param1);
                }
                DEBUG_PRINT("send_command_proxy:OMX_CommandPortDisable:\
...
None
=============================================================================*/
bool omx_Qcelp13_adec::execute_omx_flush(OMX_IN OMX_U32 param1, bool cmd_cmpl)
{
    bool bRet = true;
...
None
=============================================================================*/
bool omx_Qcelp13_adec::execute_input_omx_flush()
{
...
None
=============================================================================*/
bool omx_Qcelp13_adec::execute_output_omx_flush()
{
    OMX_BUFFERHEADERTYPE *omx_buf;
    unsigned      p1; // Parameter - 1
    unsigned      p2; // Parameter - 2
...
None
=============================================================================*/
bool omx_Qcelp13_adec::post_input(unsigned int p1,
                                  unsigned int p2,
                                  unsigned int id
                                  )
{
...
None
=============================================================================*/
bool omx_Qcelp13_adec::post_command(unsigned int p1,
                                    unsigned int p2,
                                    unsigned int id
                                    )
{
...
None
=============================================================================*/
bool omx_Qcelp13_adec::post_output(unsigned int p1,
                                   unsigned int p2,
                                   unsigned int id
                                   )
{
...
@return error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::get_parameter(OMX_IN OMX_HANDLETYPE     hComp,
    ...
    case OMX_IndexParamAudioQcelp13:
        {
            ...
            if ( OMX_CORE_INPUT_PORT_INDEX== Qcelp13Param->nPortIndex )
            {
                *Qcelp13Param = m_qcelp_param;
            } else
            {
                DEBUG_PRINT_ERROR("get_param:OMX_IndexParamAudioQcelp%d\n",\
...
@return error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::set_parameter(OMX_IN OMX_HANDLETYPE     hComp,
                                               OMX_IN OMX_INDEXTYPE paramIndex,
                                               OMX_IN OMX_PTR        paramData)
{
    ...
    case OMX_IndexParamAudioQcelp13:
        {
            DEBUG_PRINT("OMX_IndexParamAudioQcelp13");
            m_qcelp_param = *((OMX_AUDIO_PARAM_QCELP13TYPE *) paramData);
            break;
        }
    case OMX_IndexParamPortDefinition:
        {
    ...
    case  OMX_IndexParamAudioPortFormat:
        {
            ...
            if ( OMX_CORE_INPUT_PORT_INDEX== portFormatType->nPortIndex )
            {
                ...
                portFormatType->nPortIndex )
            {
                DEBUG_PRINT("set_param: OMX_IndexParamAudioFormat: %d\n",\
                    portFormatType->nIndex);
                portFormatType->eEncoding = OMX_AUDIO_CodingPCM;
            } else
            {
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::get_config(OMX_IN OMX_HANDLETYPE      hComp,
...
OMX Error None if successful.
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::set_config(OMX_IN OMX_HANDLETYPE      hComp,
                                            OMX_IN OMX_INDEXTYPE configIndex,
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::GetExtensionIndex
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::get_extension_index(
    OMX_IN OMX_HANDLETYPE      hComp,
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::get_state(OMX_IN OMX_HANDLETYPE  hComp,
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::ComponentTunnelRequest
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::component_tunnel_request(
    OMX_IN OMX_HANDLETYPE                hComp,
    OMX_IN OMX_U32                        port,
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::AllocateInputBuffer
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::allocate_input_buffer(
    OMX_IN OMX_HANDLETYPE                hComp,
    ...
    OMX_IN OMX_U32                       bytes)
{
    ...
    if ( buf_ptr != NULL )
    {
        ...
        *bufferHdr = bufHdr;
        ...
        DEBUG_PRINT("AIB:bufHdr %x pBuffer %x m_inp_buf_cnt=%d bytes=%d", \
...
OMX_ERRORTYPE  omx_Qcelp13_adec::allocate_output_buffer
(OMX_IN OMX_HANDLETYPE                hComp,
 OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
 OMX_IN OMX_U32                        port,
 OMX_IN OMX_PTR                     appData,
    ...
    if ( m_out_current_buf_count < m_out_act_buf_count )
    {
        ...
        if ( buf_ptr != NULL )
        {
            ...
            *bufferHdr = bufHdr;
            ...
            DEBUG_PRINT("AOB::bufHdr %x pBuffer %x m_out_buf_cnt=%d bytes=%d", \
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::AllocateBuffer
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::allocate_buffer(
    OMX_IN OMX_HANDLETYPE                hComp,
    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
    OMX_IN OMX_U32                        port,
    OMX_IN OMX_PTR                     appData,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_Qcelp13_adec::use_buffer(
    OMX_IN OMX_HANDLETYPE            hComp,
    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
    OMX_IN OMX_U32                   port,
    OMX_IN OMX_PTR                   appData,
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_Qcelp13_adec::use_input_buffer(
    OMX_IN OMX_HANDLETYPE            hComp,
    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
    OMX_IN OMX_U32                   port,
    OMX_IN OMX_PTR                   appData,
    ...
    OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("Inside omx_Qcelp13_adec::use_input_buffer");
    ...
    if ( m_inp_current_buf_count < m_inp_act_buf_count )
    {
        ...
        if ( buf_ptr != NULL )
        {
            ...
            *bufferHdr = bufHdr;
            ...
            DEBUG_PRINT("use_input_buffer:bufHdr %x Buffer %x bytes=%d", \
...
None
=============================================================================*/
OMX_ERRORTYPE  omx_Qcelp13_adec::use_output_buffer(
    OMX_IN OMX_HANDLETYPE            hComp,
    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
    OMX_IN OMX_U32                   port,
    OMX_IN OMX_PTR                   appData,
    ...
    OMX_IN OMX_U8*                   buffer)
{
    ...
    DEBUG_PRINT("Inside omx_Qcelp13_adec::use_output_buffer");
    if ( m_out_current_buf_count < m_out_act_buf_count )
    {
...
*/
bool omx_Qcelp13_adec::search_input_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
...
@return bool value indicating whether buffer is found
*/
bool omx_Qcelp13_adec::search_output_bufhdr(OMX_BUFFERHEADERTYPE *buffer)
{
...
@return Error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::free_buffer(OMX_IN OMX_HANDLETYPE         hComp,
                                             OMX_IN OMX_U32                 port,
...
@return error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::empty_this_buffer(
    OMX_IN OMX_HANDLETYPE         hComp,
    OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    ...
    {
        DEBUG_PRINT("omx_Qcelp13_adec::etb--> Buffer Size Invalid\n");
        return OMX_ErrorBadParameter;
    }
    if ( buffer->nVersion.nVersion != OMX_SPEC_VERSION )
    {
        DEBUG_PRINT("omx_Qcelp13_adec::etb--> OMX Version Invalid\n");
...
@return error status
*/
OMX_ERRORTYPE  omx_Qcelp13_adec::empty_this_buffer_proxy(
    OMX_IN OMX_HANDLETYPE         hComp,
  ...
  if(m_eos_bm)
  {
    ...
    while ( srcDataConsumed < (buffer->nFilledLen) )
    {
        ...
        if ( m_is_full_frame )
        {
            ...
            {
                DEBUG_PRINT("\nETP:Bytes Transcoded[%d]= BufSize[%d]",\
                    bytesTranscoded,OMX_TRANSCODED_OUTPUT_BUFFER_SIZE);
                if(pcm_feedback)
                {
                    if(data != NULL)
                    {
    ...
    DEBUG_PRINT("\n NT2--> Bytes Transcoded = %d", bytesTranscoded);
    if (bytesTranscoded >= 0 )
    {
        DEBUG_PRINT("ETBP:nFlags %d\n", buffer->nFlags );
        if(pcm_feedback)
        {
            if(data != NULL)
            {
...
OMX_ERRORTYPE  omx_Qcelp13_adec::fill_this_buffer_proxy(
    OMX_IN OMX_HANDLETYPE         hComp,
    OMX_BUFFERHEADERTYPE* buffer)
{
    int byte_count = 0;
    ...
    if ( true == search_output_bufhdr(buffer) )
    {
        ...
        for ( nRead = 0; nRead < ncount; nRead++ )
        {
            ...
            if ( nRead == 0 )
            {
                ...
                DEBUG_PRINT("FTBP:NT2 NT2 Meta nRead=%d TS[%ld] TS[%ld]\n",\
...
/*======================================================================
FUNCTION
omx_Qcelp13_adec::FillThisBuffer
...
========================================================================= */
OMX_ERRORTYPE  omx_Qcelp13_adec::fill_this_buffer(
    OMX_IN OMX_HANDLETYPE         hComp,
    OMX_IN OMX_BUFFERHEADERTYPE* buffer)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    if ( buffer->nSize != sizeof(OMX_BUFFERHEADERTYPE) )
    {
        DEBUG_PRINT("omx_Qcelp13_adec::ftb--> Buffer Size Invalid\n");
        return OMX_ErrorBadParameter;
    }
    ...
    if ( buffer->nVersion.nVersion != OMX_SPEC_VERSION )
    {
        DEBUG_PRINT("omx_Qcelp13_adec::ftb--> OMX Version Invalid\n");
        return OMX_ErrorVersionMismatch;
    }
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::set_callbacks(
    OMX_IN OMX_HANDLETYPE        hComp,
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::ComponentDeInit
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::component_deinit(OMX_IN OMX_HANDLETYPE hComp)
{
    if(hComp == NULL)
    {
        DEBUG_PRINT_ERROR("Returning OMX_ErrorBadParameter\n");
...
/* ======================================================================
FUNCTION
  omx_Qcelp13_adec::deinit_decoder
...
========================================================================== */
void  omx_Qcelp13_adec::deinit_decoder()
{
    ...
    DEBUG_PRINT("STATS: in-buf-len[%d]out-buf-len[%d] tot-pb-time[%ld]",\
        m_qcelp_pb_stats.tot_in_buf_len,
        m_qcelp_pb_stats.tot_out_buf_len,
        m_qcelp_pb_stats.tot_pb_time);
    DEBUG_PRINT("STATS: fbd-cnt[%d]ftb-cnt[%d]etb-cnt[%d]ebd-cnt[%d]",\
        m_qcelp_pb_stats.fbd_cnt,m_qcelp_pb_stats.ftb_cnt,
        m_qcelp_pb_stats.etb_cnt,
        m_qcelp_pb_stats.ebd_cnt);
    DEBUG_PRINT("********************************\n");
    memset(&m_qcelp_pb_stats,0,sizeof(QCELP_PB_STATS));
    ...
    if(m_ipc_to_event_th != NULL)
    {
          ...
          omx_Qcelp13_thread_stop(m_ipc_to_event_th);
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::use_EGL_image(
    OMX_IN OMX_HANDLETYPE                hComp,
    OMX_INOUT OMX_BUFFERHEADERTYPE** bufferHdr,
    OMX_IN OMX_U32                        port,
    OMX_IN OMX_PTR                     appData,
...
/* ======================================================================
FUNCTION
omx_Qcelp13_adec::ComponentRoleEnum
...
========================================================================== */
OMX_ERRORTYPE  omx_Qcelp13_adec::component_role_enum(OMX_IN OMX_HANDLETYPE hComp,
                                                     OMX_OUT OMX_U8*       role,
                                                     OMX_IN OMX_U32       index)
{
    OMX_ERRORTYPE eRet = OMX_ErrorNone;
    const char *cmp_role = "audio_decoder.Qcelp13";
...
========================================================================== */
bool omx_Qcelp13_adec::allocate_done(void)
{
...
========================================================================== */
bool omx_Qcelp13_adec::release_done(OMX_U32 param1)
{
    DEBUG_PRINT("Inside omx_qcelp_adec::release_done");
...
bool omx_Qcelp13_adec::omx_Qcelp13_fake_eos()
{
    int ret;
    META_IN meta_in;
    meta_in.offsetVal  = sizeof(META_IN);
    ...
    DEBUG_PRINT("omx_Qcelp13_fake_eos :ret=%d", ret);
    return true;
}
void omx_Qcelp13_adec::append_data_to_temp_buf()
{
    int bytes_read = 0;
...
timer::timer(omx_Qcelp13_adec* base):
    ...
    m_timerinfo(NULL)
{
    ...
    int rc = pthread_create(&m_timerinfo->thr ,0, omx_Qcelp13_comp_timer_handler,
                             (void*)m_timerinfo );
    if(rc < 0)
{
...
void* omx_Qcelp13_comp_timer_handler(void *pT)
{
    int count = 0;
    TIMERINFO *pTime = (TIMERINFO*)pT;
    timer *pt = pTime->pTimer;
    omx_Qcelp13_adec *pb = pTime->base;
    int               rc = 0;
    ...
    while(1)
    {
        ...
        if( ((state== OMX_StatePause)) && !(pb->getSuspendFlg()))
        {
            ...
            pb->post_command(0,0,omx_Qcelp13_adec::OMX_COMPONENT_SUSPEND);
            pt->setTimerExpiry();
        }
        else
        {
