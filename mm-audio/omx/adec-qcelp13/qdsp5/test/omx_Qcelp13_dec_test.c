...
void signal_handler(int sig_id) {
    ...
    if (sig_id == SIGUSR1) {
        ...
        if (bPause == true) {
            ...
            bPause = false;
            OMX_SendCommand(Qcelp13_dec_handle, OMX_CommandStateSet, OMX_StateExecuting, NULL);
        } else {
            DEBUG_PRINT("%s pause playback\n", __FUNCTION__);
            bPause = true;
            OMX_SendCommand(Qcelp13_dec_handle, OMX_CommandStateSet, OMX_StatePause, NULL);
        }
    }
}
...
int Play_Decoder()
{
    ...
    OMX_GetParameter(Qcelp13_dec_handle,OMX_IndexParamPortDefinition,&inputportFmt);
    ...
    OMX_GetExtensionIndex(Qcelp13_dec_handle,"OMX.Qualcomm.index.audio.sessionId",&index);
    ...
    if(tunnel == 0)
    {
        ...
        OMX_GetParameter(Qcelp13_dec_handle,OMX_IndexParamPortDefinition,&outputportFmt);
    ...
    OMX_SetParameter(Qcelp13_dec_handle,OMX_IndexParamAudioQcelp13,&Qcelp13param);
    ...
    error = Allocate_Buffer(Qcelp13_dec_handle, &pInputBufHdrs, inputportFmt.nPortIndex,
    ...
    if(tunnel == 0)
    {
        ...
        error = Allocate_Buffer(Qcelp13_dec_handle, &pOutputBufHdrs, outputportFmt.nPortIndex,
    ...
    OMX_SendCommand(Qcelp13_dec_handle, OMX_CommandStateSet, OMX_StateExecuting,0);
    wait_for_event();
