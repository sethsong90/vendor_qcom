...
bool COmxAmrwbplusDec::omx_cmd_queue::insert_entry(unsigned p1,
...
bool COmxAmrwbplusDec::omx_cmd_queue::pop_entry(unsigned *p1,
    unsigned *p2, unsigned *id)
{
    bool ret = true;
    if (m_size > 0)
...
bool COmxAmrwbplusDec::omx_cmd_queue::get_msg_id(unsigned *id)
{
    if(m_size > 0)
    {
        *id = m_q[m_read].id;
  ...
  while(1)
  {
    ...
    switch(drv_event.event_type)
    {
      ...
      case AUDIO_EVENT_WRITE_DONE:
      {
        ...
        if(pcm_feedback)
        {
            drv_event.event_payload.aio_buf.data_len -= sizeof(META_IN);
        }
        ...
        if(bufHdr)
        {
          ...
          post_input((unsigned) &m_cmp, (unsigned int)bufHdr, OMX_COMPONENT_GENERATE_BUFFER_DONE);
      ...
      case AUDIO_EVENT_READ_DONE:
      {
        ...
        if(bufHdr)
        {
          ...
          post_output((unsigned) &m_cmp, (unsigned int)bufHdr, OMX_COMPONENT_GENERATE_FRAME_DONE);
        ...
        if (bufHdr->nFilledLen)
        {
            ...
            if (pcm_feedback)
            {
                ...
                if(pmeta_in)
                {
                    ...
                    (((((OMX_BUFFERHEADERTYPE*)bufHdr)->nTimeStamp) >> 32) & 0xFFFFFFFF);
    ...
    do
    {
        ...
        if (qsize)
        {
            ...
            if ((ident == OMX_COMPONENT_GENERATE_ETB))
            {
                ...
                DEBUG_PRINT("Flush:Input dataq=0x%x \n", (unsigned)omx_buf);
        ...
        else if((m_ebd_q.m_size))
        {
            ...
            if(ident == OMX_COMPONENT_GENERATE_BUFFER_DONE)
            {
                omx_buf = (OMX_BUFFERHEADERTYPE *) p2;
                omx_buf->nFilledLen = 0;
                DEBUG_PRINT("Flush:ctrl dataq=0x%x \n",(unsigned) omx_buf);
...
bool COmxAmrwbplusDec::execute_output_omx_flush()
{
    ...
    do
    {
        ...
        if (qsize)
        {
            ...
            {
                omx_buf = (OMX_BUFFERHEADERTYPE *) p2;
                DEBUG_PRINT("Ouput Buf_Addr=%x TS[%lld] \n",\
...
*/
bool COmxAmrwbplusDec::post_input(unsigned int p1,
                                  unsigned int p2,
                                  unsigned int id)
{
    bool bRet = false;
...
bool COmxAmrwbplusDec::post_output( unsigned int p1,
                                    unsigned int p2,
                                    unsigned int id)
{
    bool bRet = false;
                                                ...
                                                OMX_IN OMX_PTR        paramData)
{
    ...
    switch(paramIndex)
    {
        case QOMX_IndexParamAudioAmrWbPlus:
        {
            ...
            DEBUG_PRINT("nSize %ld nPortIndex %ld nChannels %ld nBitRate %ld nSampleRate %ld \
                                                          ...
                                                          OMX_BUFFERHEADERTYPE* buffer)
{
    ...
    if(m_first_amrwbplus_header == 0)
    {
        ...
        drv_config.size_bytes = sizeof(struct msm_audio_amrwbplus_config_v2);
