                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
  ...
  timeStampLfile += timestampInterval;
        ...
        used_ip_buf_cnt++;
        DEBUG_PRINT("OMX_EmptyThisBuffer:length %lu \n Timestamp %lld\n",
...
int main(int argc, char **argv)
{
    ...
    if (argc >= 13)
    {
        ...
        BitRate= atoi(argv[4]);
        nBandMode = atoi(argv[5]);
        nDTXMode  = atoi(argv[6]);
        nFrameFormat =atoi(argv[7]);
        tunnel  = atoi(argv[8]);
        filewrite = atoi(argv[9]);
        out_filename = argv[10];
        allocate_buf_flag = atoi(argv[11]);
        write_buff_size = atoi(argv[12]);
...
static int Read_Buffer (OMX_BUFFERHEADERTYPE  *pBufHdr )
{
    ...
    int Size = fread(headerobject, 1, 8, inputBufferFile);
    if(Size <=0 ) {
        DEBUG_PRINT("NO DATA READ Parser failed\n");
        error_code = -1;
        return error_code;
    }
    int16_t framesize = ((headerobject[4] | (headerobject[5] << 8)));
    DEBUG_PRINT("headerobject[4] = %d, headerobject[5] = %d , framesize = %d\n\n", headerobject[4], headerobject[5], framesize);
    bytes_read = fread(pBufHdr->pBuffer, 1, framesize, inputBufferFile);
    current_ptr += bytes_read;
    DEBUG_PRINT("cur ptr = 0x%8x, bytes_read= %d\n", current_ptr, bytes_read);
