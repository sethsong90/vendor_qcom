                           ...
                           OMX_IN OMX_PTR pEventData)
{
   ...
   switch(eEvent) {
      ...
      case OMX_EventBufferFlag:
         ...
         DEBUG_PRINT("\n OMX_EventBufferFlag \n");
         DEBUG_PRINT("\n *********************************************\n");
         if(adec_appdata->tunnel)
         {
             adec_appdata->bInputEosReached = true;
             event_complete(adec_appdata);
         }
         else
         {
...
#ifdef AUDIOV2
strncpy(adec_mp3_inst1.device,"handset_rx",strlen("handset_rx"));
adec_mp3_inst1.control=0;
   ...
   if(adec_mp3_inst->bOutputEosReached || (adec_mp3_inst->tunnel && adec_mp3_inst->bInputEosReached)) {
...
{
    struct adec_appdata adec_mp3_inst2,adec_mp3_inst3;
    struct sigaction sa;
    pthread_t thread1_id,thread2_id,thread3_id;
    int thread1_ret=0,thread2_ret=0,thread3_ret=0;
    ...
    if(!(argc == 10 || argc == 23))
    {
          ...
          DEBUG_PRINT( "SINGLE INSTANCE \n");
          DEBUG_PRINT( "ex: ./mm-adec-omxmp3 MULTI_INSTANCE MP3INPUTFILE ParseHeader PCMPLAYBACK \n");
          DEBUG_PRINT( "TUNNEL FILEWRITE OUTFILENAME BUFFEROPTION PCMDEVICETYPE \n");
          DEBUG_PRINT( "\n\n MULTI INSTANCE\n");
          DEBUG_PRINT( "ex: ./mm-adec-omxmp3 MULTI_INSTANCE MP3INPUTFILE1 MP3INPUTFILE2 MP3INPUTFILE3 \n");
          DEBUG_PRINT( "ParseHeader1 ParseHeader2 ParseHeader3 PCMPLAYBACK1 PCMPLAYBACK2 PCMPLAYBACK3 TUNNEL1\n\n\n");
          DEBUG_PRINT( "TUNNEL2 TUNNEL3 FILEWRITE1 FILEWRITE2 FILEWRITE3 BUFFEROPTION1 BUFFEROPTION2 BUFFEROPTION3\n");
          DEBUG_PRINT( "PCMDEVICETYPE1 PCMDEVICETYPE2 PCMDEVICETYPE3\n\n");
          DEBUG_PRINT( "MULTI_INSTANCE = 0 (Single Instance) \n");
          DEBUG_PRINT( "MULTI_INSTANCE = 1 (Multi Instance) \n");
          ...
          DEBUG_PRINT( "PCMDEVICETYPE = 0 (Host PCM Device)\n");
          DEBUG_PRINT( "PCMDEVICETYPE = 1 (PCM Dec Device)\n");
          return 0;

    }


    is_multi_inst = atoi(argv[1]);
    ...
    if(is_multi_inst == 0)
    {
       if(argc == 10)
       {
            ...
            adec_mp3_inst1.in_filename = argv[2];
            adec_mp3_inst1.bParseHeader = atoi(argv[3]);
            adec_mp3_inst1.pcmplayback = atoi(argv[4]);
            adec_mp3_inst1.tunnel  = atoi(argv[5]);
            adec_mp3_inst1.filewrite = atoi(argv[6]);
            adec_mp3_inst1.out_filename = argv[7];
            adec_mp3_inst1.buffer_option = atoi(argv[8]);
            adec_mp3_inst1.pcm_device_type = atoi(argv[9]);

            //adec_mp3_inst1.out_filename = (char*)malloc(sizeof("audio.wav"));
            //strncpy(adec_mp3_inst1.out_filename,"audio.wav",strlen("audio.wav"));
            if (adec_mp3_inst1.tunnel == 1) {
              adec_mp3_inst1.pcmplayback = 0; /* This feature holds good only for non tunnel mode*/
              adec_mp3_inst1.filewrite = 0;  /* File write not supported in tunnel mode */
            }
            if(adec_mp3_inst1.tunnel == 0)
               adec_mp3_inst1.aud_comp = "OMX.qcom.audio.decoder.mp3";
            else
               adec_mp3_inst1.aud_comp = "OMX.qcom.audio.decoder.tunneled.mp3";
    ...
    else if (is_multi_inst == 1)
    {
        if (argc == 23)
        {
            adec_appdata_init(&adec_mp3_inst1);
            adec_appdata_init(&adec_mp3_inst2);
            adec_appdata_init(&adec_mp3_inst3);

            pthread_cond_init(&adec_mp3_inst1.cond, 0);
            pthread_mutex_init(&adec_mp3_inst1.lock, 0);
            pthread_cond_init(&adec_mp3_inst2.cond, 0);
            pthread_mutex_init(&adec_mp3_inst2.lock, 0);
            pthread_cond_init(&adec_mp3_inst3.cond, 0);
            pthread_mutex_init(&adec_mp3_inst3.lock, 0);

            adec_mp3_inst1.in_filename = argv[2];
            adec_mp3_inst2.in_filename = argv[3];
            adec_mp3_inst3.in_filename = argv[4];

            adec_mp3_inst1.bParseHeader = atoi(argv[5]);
            adec_mp3_inst2.bParseHeader = atoi(argv[6]);
            adec_mp3_inst3.bParseHeader = atoi(argv[7]);

            adec_mp3_inst1.pcmplayback = atoi(argv[8]);
            adec_mp3_inst2.pcmplayback = atoi(argv[9]);
            adec_mp3_inst3.pcmplayback = atoi(argv[10]);

            adec_mp3_inst1.tunnel  = atoi(argv[11]);
            adec_mp3_inst2.tunnel  = atoi(argv[12]);
            adec_mp3_inst3.tunnel  = atoi(argv[13]);

            adec_mp3_inst1.filewrite = atoi(argv[14]);
            adec_mp3_inst2.filewrite = atoi(argv[15]);
            adec_mp3_inst3.filewrite = atoi(argv[16]);

            adec_mp3_inst1.out_filename = (char*)malloc(sizeof("audio1.wav"));
            adec_mp3_inst2.out_filename = (char*)malloc(sizeof("audio2.wav"));
            adec_mp3_inst3.out_filename = (char*)malloc(sizeof("audio3.wav"));

            strncpy((char*)(adec_mp3_inst1.out_filename),"audio1.wav",strlen("audio1.wav"));
            strncpy((char*)(adec_mp3_inst2.out_filename),"audio2.wav",strlen("audio2.wav"));
            strncpy((char*)(adec_mp3_inst3.out_filename),"audio3.wav",strlen("audio3.wav"));

            adec_mp3_inst1.buffer_option = atoi(argv[17]);
            adec_mp3_inst1.buffer_option = atoi(argv[18]);
            adec_mp3_inst1.buffer_option = atoi(argv[19]);

            adec_mp3_inst1.pcm_device_type = atoi(argv[20]);
            adec_mp3_inst1.pcm_device_type = atoi(argv[21]);
            adec_mp3_inst1.pcm_device_type = atoi(argv[22]);
           ...
           if (adec_mp3_inst2.tunnel == 1) {
              adec_mp3_inst2.pcmplayback = 0; /* This feature holds good only for non tunnel mode*/
              adec_mp3_inst2.filewrite = 0;  /* File write not supported in tunnel mode */
           }

           if (adec_mp3_inst3.tunnel == 1) {
              adec_mp3_inst3.pcmplayback = 0; /* This feature holds good only for non tunnel mode*/
              adec_mp3_inst3.filewrite = 0;  /* File write not supported in tunnel mode */
           }
          ...
          if(adec_mp3_inst2.tunnel == 0)
              adec_mp3_inst2.aud_comp = "OMX.qcom.audio.decoder.mp3";
          else
              adec_mp3_inst2.aud_comp = "OMX.qcom.audio.decoder.tunneled.mp3";
          DEBUG_PRINT(" OMX test app : aud_comp instance 2= %s\n",adec_mp3_inst2.aud_comp);

          if(adec_mp3_inst3.tunnel == 0)
             adec_mp3_inst3.aud_comp = "OMX.qcom.audio.decoder.mp3";
          else
             adec_mp3_inst3.aud_comp = "OMX.qcom.audio.decoder.tunneled.mp3";
          ...
          if(adec_mp3_inst2.pcmplayback &&
              adec_mp3_inst2.pcm_device_type == PCM_DEC_DEVICE)
          {
             pthread_mutex_init(&adec_mp3_inst2.pcm_buf_lock, 0);
          }

          if(adec_mp3_inst3.pcmplayback &&
              adec_mp3_inst3.pcm_device_type == PCM_DEC_DEVICE)
          {
             pthread_mutex_init(&adec_mp3_inst3.pcm_buf_lock, 0);
          }

          DEBUG_PRINT(" OMX test app : aud_comp instance 3 = %s\n",adec_mp3_inst3.aud_comp);
        }
        else
        {
             DEBUG_PRINT( "invalid format: \n");
             DEBUG_PRINT( "ex: ./mm-adec-omxmp3 MULTI_INSTANCE MP3INPUTFILE1 MP3INPUTFILE2 MP3INPUTFILE3 \n");
             DEBUG_PRINT( "ParseHeader1 ParseHeader2 ParseHeader3 PCMPLAYBACK1 PCMPLAYBACK2 PCMPLAYBACK3 TUNNEL1\n");
             DEBUG_PRINT( "TUNNEL2 TUNNEL3 FILEWRITE1 FILEWRITE2 FILEWRITE3\n");
             DEBUG_PRINT( "MULTI_INSTANCE = 0 (Single Instance) \n");
             DEBUG_PRINT( "MULTI_INSTANCE = 1 (Multi Instance) \n");
             DEBUG_PRINT( "ParseHeader= 1 (Parses MP3 Header) \n");
             DEBUG_PRINT( "ParseHeader= 0 (Uses Default Sampling rate and channel) \n");
    ...
    {
       DEBUG_PRINT( "Invalid option entered for Instance Information\n");
       DEBUG_PRINT( "ex: ./mm-adec-omxmp3 MULTI_INSTANCE MP3INPUTFILE1 MP3INPUTFILE2 MP3INPUTFILE3 \n");
       DEBUG_PRINT( "ParseHeader1 ParseHeader2 ParseHeader3 PCMPLAYBACK1 PCMPLAYBACK2 PCMPLAYBACK3 TUNNEL1\n");
       DEBUG_PRINT( "TUNNEL2 TUNNEL3 FILEWRITE1 FILEWRITE2 FILEWRITE3\n");
       DEBUG_PRINT( "MULTI_INSTANCE = 0 (Single Instance) \n");
   ...
   if(is_multi_inst)
   {
       pthread_create (&thread2_id, NULL, &thread_function, &adec_mp3_inst2);
       pthread_create (&thread3_id, NULL, &thread_function, &adec_mp3_inst3);
   }
   ...
   if(is_multi_inst)
   {
      pthread_join (thread2_id,(void**) &thread2_ret);
      if(0 == thread2_ret)
         DEBUG_PRINT(" Thread 2 ended successfully\n");

      pthread_join (thread3_id,(void**) &thread3_ret);
      if(0 == thread3_ret)
         DEBUG_PRINT(" Thread 3 ended successfully\n");
   }
   /* Deinit OpenMAX */
   OMX_Deinit();
...
int Play_Decoder(struct adec_appdata* adec_appdata)
{
   ...
   adec_appdata->inputportFmt.nBufferCountActual = adec_appdata->inputportFmt.nBufferCountMin + 0;// 5;
   ...
   OMX_GetParameter(adec_appdata->mp3_dec_handle,index,&adec_appdata->streaminfoparam);
	...
	else
	{
		...
		if (msm_route_stream(1, adec_appdata->session_id, adec_appdata->device_id, 1))
		{
   ...
   if(adec_appdata->tunnel == 0) {
    ...
    adec_appdata->outputportFmt.nBufferCountActual = adec_appdata->outputportFmt.nBufferCountMin;
   ...
   else
   {
      ...
      if (pthread_create(&adec_appdata->m_pcmdrv_evt_thread_id, 0, (void *)process_hpcm_drv_events,
          adec_appdata) < 0)
      {
          DEBUG_PRINT_ERROR("\n Event Thread creation for PCM Dec driver FAILED\n");
          return -1;
   ...
   for (i = 0;i < adec_appdata->input_buf_cnt;i++) {
    ...
    {
        wait_for_event(adec_appdata);
        if(adec_appdata->bOutputEosReached || (adec_appdata->tunnel && adec_appdata->bInputEosReached))
        {
           adec_appdata->bReconfigureOutputPort = 0;
