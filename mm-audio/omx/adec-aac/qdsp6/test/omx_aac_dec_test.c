   ...
   (void)pEventData;
    switch(eEvent) {
        case OMX_EventCmdComplete:
            ...
            {
                fprintf(logFile,"******************************************\n");
                fprintf(logFile,"Recieved DISABLE Event Command Complete[%lu]\n",nData2);
                fprintf(logFile,"******************************************\n");
            }
            else if(OMX_CommandPortEnable == (OMX_COMMANDTYPE)nData1)
            {
                fprintf(logFile,"*********************************************\n");
                fprintf(logFile,"Recieved ENABLE Event Command Complete[%lu]\n",nData2);
                fprintf(logFile,"*********************************************\n");
            }
            else if(OMX_CommandFlush== (OMX_COMMANDTYPE)nData1)
            {
                fprintf(logFile,"*********************************************\n");
                fprintf(logFile,"Recieved FLUSH Event Command Complete[%lu]\n",nData2);
                fprintf(logFile,"*********************************************\n");
            }
        event_complete();
        break;
                              ...
                              OMX_IN OMX_BUFFERHEADERTYPE* pBuffer)
{
   ...
   fprintf(logFile," FillBufferDone #%d size %lu Flag 0x%x\n",
   ...
   if((tunnel == 0) && (filewrite == 1))
   {
       ...
       fprintf(logFile," FillBufferDone size writen to file  %d\n",bytes_writen);
    ...
    while ((opt = getopt_long(argc, argv, "-r:m:p:tds:b:c:R:n:fu:i:v:l:a:h", long_options, &option_index)) != -1)
        {
            ...
            case 'd':
                ...
                break;
            case 's':
                sbr_ps_enabled = atoi(optarg);
            case 'b':
                bsac = atoi(optarg);
                break;
            case 'c':
                configbufsize = atoi(optarg);
                break;
            case 'R':
                raw_config_bytes = atoi(optarg);
                break;
            case 'n':
                aac_channels = atoi(optarg);
                break;
            case 'f':
...
int Play_Decoder()
{
        ...
        (dual_mono_mode <= OMX_AUDIO_DUAL_MONO_MODE_FL_SR))
    {
        ...
        fprintf(logFile,"Configure DualMono: size, version,"
                    "portindex, channelconfig = %ld %ld %d\n",
...
static int Read_Buffer (OMX_BUFFERHEADERTYPE  *pBufHdr )
{
    ...
    } else if ((first_read == 1) && (raw_config_bytes > 0))
    {
        ...
        fprintf(logFile,"\nraw_given = %d, Bytes read for first read is =%d\n",
                 raw_config_bytes, bytes_read);
        first_read = 0;
    }
    else
    {
        int read_bytes = (pBufHdr->nAllocLen >  4096) ? 4096 : pBufHdr->nAllocLen;
...
static int open_audio_file ()
{
    ...
    if((tunnel == 0)&& (filewrite == 1))
    {
        ...
        fprintf(logFile," Length og wav header is %d \n",header_len );
    }
    return error_code;
}
...
void process_portreconfig ( )
{
    ...
    fprintf(logFile,">>>>PortReconfigEvent<<< new samplerate =%lu \
