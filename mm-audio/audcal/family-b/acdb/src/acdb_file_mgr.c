...
/*===========================================================================
    ...
    $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acdb/src/acdb_file_mgr.c#2 $
...
int32_t AcdbDataCmdSetData(AcdbCmdFileInfo *cmdInfo)
{
	...
	if(cmdInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved NULL input for AcdbDataCmdSetData\n");
		return ACDB_ERROR;
	}
...
int32_t AcdbDataGetDataPtr(AcdbCmdFileInfo fileInfo,uint32_t dataoffset,AcdbDataInfo *pDataInfo)
{
	...
	if(pDataInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved NULL input for AcdbDataGetDataPtr\n");
		return ACDB_ERROR;
	}
	...
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Out of bounds dataoffset value recieved\n");
		return ACDB_ERROR;
	}
	...
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Valid dataoffset recieved, but invalid data len. Trying to access beyond DATAPOOL chunk\n");
		return ACDB_ERROR;
	}
...
int32_t AcdbDataGetDevPropData(AcdbDevPropInfo *dpInfo)
{
	...
	if(dpInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved NULL input for AcdbDataGetDevPropData\n");
		return ACDB_ERROR;
	}
...
int32_t AcdbDataGetGlobalPropData(AcdbGlbalPropInfo *glbInfo)
{
	...
	if(glbInfo == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved NULL input for AcdbDataGetGlobalPropData\n");
		return ACDB_BADPARM;
	}
	if(gDataFileInfo.noOfFiles == 0)
	{
...
int32_t AcdbDataGetDevicesList(AcdbDevices *pDevs)
{
	...
	if(pDevs == NULL)
	{
		ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved NULL input for AcdbDataGetDevicesList\n");
		return ACDB_ERROR;
	}
	pDevs->noOfDevs = 0;
...
int32_t AcdbDataGetFileData(AcdbFileMgrGetFileDataReq *pInput,AcdbFileMgrResp *pOutput)
{
    ...
    {
	    ACDB_DEBUG_LOG("ACDB_FILEMGR: Recieved invalid input/output params\n");
	    return ACDB_ERROR;
    }
   ...
   case ACDBDATACMD_SET_ACDB_DATA:
	   {
		   ...
		   {
			   ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved invalid input/output params\n");
			   return ACDB_BADPARM;
		   }
   ...
   case ACDBDATACMD_GET_AVAILABLE_FILES_SLOT_COUNT:
	   {
		   ...
		   {
			   ACDB_DEBUG_LOG("ACDBFILE_MGR: Recieved invalid output params to provide the slot count info\n");
			   return ACDB_BADPARM;
		   }
