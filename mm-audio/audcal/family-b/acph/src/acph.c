 ...
 ********************************************************************************
 */
/* $Header: //source/qcom/qct/multimedia2/Audio/audcal4/acdb_sw/main/latest/acph/src/acph.c#1 $ */
/*
   -------------------------------
   |Include Files                |
   -------------------------------
                            ...
                            )
{
   ...
   {
      /**not initialized*/
	   ACDB_DEBUG_LOG("resp buffer ptr address[%uld]\n",(uint32_t)resp_buf_ptr);
      *resp_buf_length = 0;
      return;
   }
...
int32_t acph_deinit(void)
{
...

    {
       if (g_pCmdTbl != NULL && g_pCmdTbl->pNodeHead != NULL)
       {
          ...
          {
             count ++;
             ACDB_DEBUG_LOG("Node%d is not empty, address[%08X]\n",count,(uint32_t)pCur);
             pCur = pCur->pNext;
          }
          if (g_pCmdTbl->pNodeTail != NULL)
          {
        	  ACDB_DEBUG_LOG("g_pCmdTbl->pNodeTail is not NULL, address[%08X]\n",(uint32_t)g_pCmdTbl->pNodeTail);
          }
       }
    }
                              ...
                              )
{
   ...
   {
	   ACDB_DEBUG_LOG("[ACPH]->Invalid service id recieved for ACPH registration - %x\n", nService_id);
   ...
   if (g_pCmdTbl != NULL && g_pCmdTbl->pNodeHead != NULL)
   {
      ...
      if (pCur->serviceId == nService_id)
      {
         ...
         else
         {
        	 ...
        	 ACDB_DEBUG_LOG("Free first node, pNodeHead[%08X],pCur[%08X],pNext[%08X]\n",
        			 (uint32_t)g_pCmdTbl->pNodeHead,(uint32_t)pCur,(uint32_t)pCur->pNext);
        	 ACDB_MEM_FREE(pCur);
         }
