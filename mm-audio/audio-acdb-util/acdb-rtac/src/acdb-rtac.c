...
int32_t get_adm_info()
{
	...
	ret = ioctl(rtac_driver_handle, AUDIO_GET_RTAC_ADM_INFO, (unsigned int)&rtac_adm_data);
	if (ret < 0) {
...
int32_t get_adm_info_v2()
{
	int ret = 0;

	ret = ioctl(rtac_driver_handle, AUDIO_GET_RTAC_ADM_INFO_V2, (unsigned int)&rtac_adm_data_v2);
	if (ret < 0) {
		LOGE("ACDB RTAC -> ERROR: AUDIO_GET_RTAC_ADM_INFO errno: %d\n", errno);
		ret = ACPH_ERR_CSD_AUD_CMD_FAILURE;
		goto done;
...
int32_t get_voice_info()
{
	...
	ret = ioctl(rtac_driver_handle, AUDIO_GET_RTAC_VOICE_INFO, (unsigned int)&rtac_voice_data);
	if (ret < 0) {
		LOGE("ACDB RTAC -> ERROR: AUDIO_GET_RTAC_ADM_INFO errno: %d\n", errno);
			...
			uint32_t *bytes_filled)
{
	...
	for(j=0; j < rtac_adm_data.device[i].num_of_popp; j++)
		aud_strm[j+1] = rtac_adm_data.device[i].popp[j];

	*bytes_filled = (j+1) * sizeof(uint32_t);
done:
	return ret;
}

int32_t get_aud_streams_v2(ACPH_CMD_QUERY_AUD_COPP_STREAM_HANDLES_req *aud_strm_info,
			uint32_t *aud_strm,
			uint32_t *bytes_filled)
{
	int ret = 0;
	...
	for(i=0; i < rtac_adm_data_v2.num_of_dev; i++) {
		if (aud_strm_info->copp_handle == rtac_adm_data_v2.device[i].copp) {
			break;
		} else if (i == rtac_adm_data_v2.num_of_dev-1) {
			LOGE("ACDB RTAC -> ERROR: get_aud_streams, copp 0x%x does not match 0x%x in rtac driver\n",
				aud_strm_info->copp_handle, rtac_adm_data_v2.device[i].copp);
			/* default to 0 */
			i = 0;
			break;
		}
	...
	aud_strm[0] = rtac_adm_data_v2.device[i].num_of_popp;
	for(j=0; j < rtac_adm_data_v2.device[i].num_of_popp; j++) {
		aud_strm[j*2+1] = rtac_adm_data_v2.device[i].popp[j].popp;
		aud_strm[j*2+2] = rtac_adm_data_v2.device[i].popp[j].popp_topology;
	}
	...
	case ADSP_ASM_SERVICE:
		for (i=0; i < rtac_adm_data.num_of_dev; i++) {
			for (j=0; j < rtac_adm_data.num_of_dev; j++) {
				if (rtac_info[2] == rtac_adm_data.device[i].popp[j]) {
					pack_audio_get_rtac_data((ACPH_CMD_RTC_GET_CAL_DATA_req *)rtac_info, out_buf_size);
					ret = send_get_rtac(rtac_data, out_buf_size, bytes_filled,
				...
				uint32_t resp_buf_len, uint32_t *resp_buf_bytes_filled)
{
	...
	case ACPH_CMD_QUERY_AUD_COPP_STREAM_HANDLES_V2:
		ret = get_aud_streams_v2((ACPH_CMD_QUERY_AUD_COPP_STREAM_HANDLES_req *)req_buf_ptr,
					(uint32_t *)resp_buf_ptr,
					resp_buf_bytes_filled);
		goto done;
