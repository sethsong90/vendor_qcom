                            ...
                            int devid2)
{
    ...
    rc = snd_use_case_get(csd_common_data.ucm_mgr, "_verb",
                     (const char **)&use_case);
    if (rc != 0) {
        LOGE("ERROR snd_use_case_get() failed rc=%d\n");
        goto EXIT;
    }

    mod_size = snd_use_case_get_list(csd_common_data.ucm_mgr,
                                     "_enamods", &mod_list);
    LOGD("csd_alsa_disable_device() current verb=%d, mod_size=%d\n",
         use_case, mod_size);
...
static int csd_alsa_vs_set_ui_property(void)
{
    ...
    if (vs_data->ui_prop_mask & UI_PROP_FENS) {
        ...
        rc = alsa_set_mixer_ctl("Slowtalk Enable", (void *) &vs_data->slow_talk);
        if (rc != 0)
...
static int csd_alsa_vc_set_rx_volume_index(void)
{
    ...
    rc = alsa_set_rx_volume(vc_handle->volume);
...
static int csd_alsa_vs_set_mute(void)
{
    ...
    rc = alsa_set_mixer_ctl("Voice Tx Mute", (void *) &vs_handle->mute.mute_flag);
                                               ...
                                               struct voice_stream_manager *vs_handle)
{
    ...
    if (csd_common_data.ucm_mgr != NULL) {
        ...
        LOGD("\n UCM use case=%s alsa_handle_use_case=%s\n",
                     ucm_use_case, alsa_handle->useCase);
