...
protected:
   qcril_qmi_singleton_agent(const char* thread_name = NULL);
...
//===========================================================================
template <class T>
qcril_qmi_singleton_agent<T>::qcril_qmi_singleton_agent(const char* thread_name) : inited (0)
{
   if (thread_name)
   {
      strlcpy(thread_name_str, thread_name, sizeof(thread_name_str));
   }
   else
   {
      thread_name_str[0] = 0;
