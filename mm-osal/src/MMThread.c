...
)
{
    ...
    else
    {
      ...
      else
      {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "MM_Thread_CreateEx::set thread Prio to %d",prio.sched_priority);
        ...
        /*memcpy(attr.name, pstrName, PTHREAD_NAME_MAX_LEN);*/
          ...
          {
            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "MM_Thread_CreateEx::thread 0x%x created with stack %d bytes",
                         (unsigned int)*pHandle, stacksize);
          }
        }
      }
