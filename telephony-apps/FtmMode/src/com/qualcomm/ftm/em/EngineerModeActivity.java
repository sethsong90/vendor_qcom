...
public class EngineerModeActivity extends TabActivity implements
    ...
    private MSimTelephonyManager mMultiSimPhone;
    private TelephonyManager mPhone;
    private QcRilHook mQcRilHook;
    private static int mNumPhones = MSimTelephonyManager.getDefault().getPhoneCount();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
            mMultiSimPhone = (MSimTelephonyManager)mContext.getSystemService
                    (mContext.MSIM_TELEPHONY_SERVICE);
        } else {
            mPhone = (TelephonyManager)mContext.getSystemService
    ...
    public void setFtmMode(int enable, int sub) {
        ...
        if (enable == 1) {
            ...
            if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
                rat = mMultiSimPhone.getVoiceNetworkType(sub);
            } else {
                rat = mPhone.getVoiceNetworkType();
            }
            ratType[sub] = mapRatType(rat);
        }
        if (ratType[sub] == Rat.INVALID) {
            Log.e(TAG, "Invalid rat " + ratType[sub] + " sub = " + sub);
            return;
        }
        Log.d(TAG, "Sending RIL OEM HOOK ENGINEER MODE request sub:" + sub
                + "ratType:" + ratType[sub] + " enable:" + enable);
        if (!mIsQcRilHookReady) {
            Log.e(TAG, "qcril oem hook is not ready yet");
            return;
        }
        result = mQcRilHook.qcRilSetFieldTestMode(sub, (byte)ratType[sub], enable);
        if (result == false) {
    ...
    private void setupEmList() {
        for (int i = 0; i < mNumPhones; i++) {
            ...
            mTabHost.addTab(mTabHost.newTabSpec(TAB_LABEL[i])
                    .setIndicator(TAB_LABEL[i], null).setContent(intent));
        }
    }
