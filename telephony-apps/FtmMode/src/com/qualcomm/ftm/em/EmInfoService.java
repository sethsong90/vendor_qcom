...
public class EmInfoService extends OverlayService {
    ...
    private void setupListView() {
        for (int i = 0; i < mNumPhones; i++) {
            mSubTextView[i].setText(getResources().getString(mResourceString[i]));
            if (mCursor[i] != null) {
                mCursor[i].close();
            }
    ...
    private final class EmItemListAdapter extends CursorAdapter {
        ...
        public void bindView(View view, Context context, Cursor cursor) {
            ...
            if ("mcc".equals(name) || "mnc".equals(name) || "lac_id".equals(name)) {
                ((TextView) view.findViewById(R.id.info)).setText(Long.toHexString(value));
            }
        }
    }
}
