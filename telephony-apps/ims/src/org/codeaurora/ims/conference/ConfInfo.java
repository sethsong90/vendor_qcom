...
public class ConfInfo {
    ...
    public boolean isUpdateRequired(byte[] confxml) {
                    ...
                    Element.CONF_NA))
            {
                ...
                for (int index = 0; index < length; index++) {
                    userUri[index] = usersList.get(index).getAttributeValue(
                            Element.CONF_ENTITY);
                    log("Inside getUser URI list" + userUri[index]);
                }
            }
        } else {
    ...
    /*
     * To Find element in UserElementList which is match with the same source
     * element key value If there element match , return element index If no
        ...
        for (int elementIndex = 0; elementIndex < newListLength; elementIndex++) {
            ...
            if (isPartialAllowed(newSubElement.getTagName())) {
                ...
                if (key != null) {
                    ...
                    if (iIndex < 0) {
                        updateNotification(mOldSubElementList, null,
        ...
        if (mOldSubElementList != null) {
            for (int index = 0; index < mOldSubElementList.size(); index++) {
                ...
                if (temp.getTagName().equalsIgnoreCase(newSubElement.getTagName())) {
                    debugLog(
                            "Old Element List item at Index" + "[" + index + "]"
                                    + temp.getTagName());
