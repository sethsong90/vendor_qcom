 ...
 *
 */
public final class ImsMmiCode extends Handler implements MmiCode {
    static final String LOG_TAG = "IMS_MMI";
    ...
    /*
     * Service Codes carefully chosen not to match with
     * existing PS and CS MMI codes
     */
    static final String SC_BS_MT        = "156";
    static final String SC_BAICa        = "157";
    ...
    static final int EVENT_SUPP_SVC_QUERY_COMPLETE = 8;
    static final int EVENT_QUERY_ICB_COMPLETE    = 9;
    ...
    private boolean isPendingUSSD = false; //Variable not supported

    private boolean isUssdRequest = false; //Variable not supported

    private boolean isSsInfo = false;      //Variable not supported
    ...
    //Displayed as the title for a success/failure reported for enabling/disabling connected line
    static final String ColpMmi = "Connected Line ID";

    //Displayed as the title for a success/failure reported for enabling/disabling/querying/
    //resgister/erasure of ICB (Specific DN)
    static final String IcbDnMmi = "Specific Incoming Call Barring";
    //Displayed as the title for a success/failure reported for enabling/disabling/querying of
    //ICB (Anonymous)
...
*/
    ...
    /**
     * Some dial strings are defined to do non-call setup
    ...
    static ImsMmiCode newFromDialString(String dialString, ImsPhone phone, IccRecords records) {
        Matcher m;
        ImsMmiCode ret = null;

        if (SystemProperties.getBoolean("ro.config.multimode_cdma", false)) {
            /* Implement VM options for global device with CDMA enabled here */
            return ret;
        }
        ...
        if (m.matches()) {
            ret = new ImsMmiCode(phone, records);
    ...
    static boolean isScMatchesIcb(String dialString) {
...
*/
    ...
    //ICB(Anonymous Number)
    static boolean isServiceCodeIcbAnonymous(String sc) {
        return sc != null && sc.equals(SC_BAICa);
    }

    //ICB(Specific Number)
    static boolean isServiceCodeIcbSpecificDn(String sc) {
        return sc != null && sc.equals(SC_BS_MT);
    }

    static boolean isServiceCodeUnsupported(String sc) {
        return sc != null &&
                (sc.equals(SC_COLR)
                || sc.equals(SC_CNAP));
    }
    ...
    ImsMmiCode (ImsPhone phone, IccRecords records) {
        super(phone.getHandler().getLooper());
        this.phone = phone;
        this.context = phone.getContext();
        this.mIccRecords = records;
    }
        ...
        try {
            if (isShortCode()) {
                ...
                if (isInterrogate()) {
                    ...
                    phone.cm.setSuppSvc(sc, isActivate(), obtainMessage(EVENT_SET_COMPLETE, this));
                } else {
                    throw new RuntimeException ("Invalid or Unsupported MMI Code");
                }
                ...
                if (isActivate()) {
                    phone.mCi.setCLIR(CommandsInterface.CLIR_INVOCATION,
                        obtainMessage(EVENT_SET_COMPLETE, this));
                } else if (isDeactivate()) {
                    phone.mCi.setCLIR(CommandsInterface.CLIR_SUPPRESSION,
                        obtainMessage(EVENT_SET_COMPLETE, this));
                ...
                if (isInterrogate()) {
                    phone.mCi.queryCallForwardStatus(
                    ...
                    phone.mCi.setCallForward(cfAction, reason, serviceClass,
                     ...
                     isServiceCodeIcbAnonymous(sc)) {
                if (isInterrogate()) {
                    phone.mCi.queryFacilityLock(sc, null,
                            0, obtainMessage(EVENT_SUPP_SVC_QUERY_COMPLETE, this));
                } else if (isActivate() || isDeactivate()) {
                    phone.mCi.setFacilityLock(sc, isActivate(), null,
                            0, obtainMessage(EVENT_SET_COMPLETE, this));
                } else {
                    throw new RuntimeException ("Invalid or Unsupported MMI Code");
                }
            } else if (isServiceCodeIcbSpecificDn(sc)) {

                Log.d(LOG_TAG, "is ICB(specific number)");

                if (isInterrogate()) {
                    /* eg format to query ICB (specific numbers) *#SC# */
                    phone.cm.queryIncomingCallBarring(sc, SERVICE_CLASS_VOICE,
                            obtainMessage(EVENT_QUERY_ICB_COMPLETE, this));
                } else {
                    String dialingNumber = sia;
                    String[] icbNum = null;
                    int operation = 0;
                    /* eg format to activate ICB (specific numbers) *SC*DN1$DN2$DN3# */
                    if (dialingNumber != null) {
                        Log.d(LOG_TAG, "dialingNumber: " + dialingNumber);
                        icbNum = dialingNumber.split("\\$");
                        for (int i = 0; i <  icbNum.length; i++) {
                            Log.d(LOG_TAG, "icbnum: " + icbNum[i] + "at index: " + i);
                        }
                    }

                    if (isActivate()) {
                        operation = ImsQmiIF.ACTIVATE;
                    } else if (isDeactivate()) {
                        operation = ImsQmiIF.DEACTIVATE;
                    } else if (isRegister()) {
                        operation = ImsQmiIF.REGISTER;
                    } else if (isErasure()) {
                        operation = ImsQmiIF.ERASURE;
                    } else {
                        throw new RuntimeException ("Invalid or Unsupported MMI Code");
                    }

                    phone.cm.setIncomingCallBarring(operation, sc, icbNum,
                            SERVICE_CLASS_VOICE, obtainMessage(EVENT_SET_COMPLETE, this));
                }
                ...
                if (isActivate() || isDeactivate()) {
                    phone.mCi.setCallWaiting(isActivate(), serviceClass,
                            obtainMessage(EVENT_SET_COMPLETE, this));
                } else if (isInterrogate()) {
                    phone.mCi.queryCallWaiting(serviceClass,
                            obtainMessage(EVENT_QUERY_COMPLETE, this));
                } else {
                    throw new RuntimeException ("Invalid or Unsupported MMI Code");
                }
    ...
    void sendUssd(String ussdMessage) {
        /*
         * USSD is not supported on Packet based IMS network, so forward this
         * request to the current active non-ims phone
         */
        Phone voicePhone = PhoneFactory.getDefaultPhone();
        if (voicePhone != null) {
            try {
                voicePhone.dial(ussdMessage);
            } catch (CallStateException ex) {
                Log.e(LOG_TAG, "Dial returned CallStateException" + ex);
            }
        }
    }
    ...
    private CharSequence getScString() {
        if (sc != null) {
            if (isServiceCodeCallBarring(sc)) {
                ...
                return IcbDnMmi;
            } else if (isServiceCodeIcbAnonymous(sc)) {
    ...
    private void onSetComplete(AsyncResult ar){
        ...
        if (ar.exception != null) {
            state = State.FAILED;
            if (ar.result != null) {
                // onSetComplete happens for other operations like
                // CLIR for which ar.result will be null
                ...
                if (response.hasFailureCause()) {
                    ...
                    Log.d(LOG_TAG, "onSetComplete failureCause = " + failureCause);
    ...
    private void onQueryIcbComplete(AsyncResult ar) {
        StringBuilder sb = new StringBuilder(getScString());
        sb.append("\n");
        ...
        if (ar.exception != null) {
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (cbNumListType.hasServiceClass()) {
                    ImsQmiIF.ServiceClass serviceClass = cbNumListType.getServiceClass();
                    int service_class = serviceClass.getServiceClass();/* gets service class value
                                                                        * i.e 1 for Voice class etc
                                                                        */
                    Log.d(LOG_TAG, "service class: " + service_class);

                    if (cbNumListType.getCbNumListCount() == 0) {
                        sb.append(context.getText(com.android.internal.R.string.serviceDisabled));
                    } else {
                        for (int j = 0; j < cbNumListType.getCbNumListCount(); j++) {
                            ImsQmiIF.CbNumList cbNumList = cbNumListType.getCbNumList(j);
                            int status = cbNumList.getStatus();
                            String cbNum = cbNumList.getNumber();
                            Log.d(LOG_TAG, "cbNum: " + cbNum + " status: " + status);
                            sb.append("Num: " + cbNum + " status: " + status + "\n");
                        }
                    }
                } else {
                    sb.append(context.getText(com.android.internal.R.string.serviceDisabled));
    ...
    private void onQueryCfComplete(AsyncResult ar) {
        ...
        if (ar.exception != null) {
            ...
            if (infos.length == 0) {
                ...
                Log.d(LOG_TAG, "In infos.length == 0");
                sb.append(context.getText(com.android.internal.R.string.serviceDisabled));
    ...
    private void onSuppSvcQueryComplete(AsyncResult ar) {
        StringBuilder sb = new StringBuilder(getScString());
        sb.append("\n");
        String output = context.getString(com.android.internal.R.string.mmiError);
        ...
        if (ar.exception != null) {
            ...
            if (ar.result != null) {
                ...
                if (response.hasStatus()) {
                    ...
                    if (status == ImsQmiIF.ENABLED) {
                        output = context.getString(com.android.internal.R.string.serviceEnabled);
                    } else if (status == ImsQmiIF.DISABLED) {
                        output = context.getString(com.android.internal.R.string.serviceDisabled);
                    }
                }
            Log.d(LOG_TAG,"onSuppSvcQueryComplete output = " + output);
            sb.append(output);
            state = State.COMPLETE;
            }
        }
    ...
    public String toString() {
        StringBuilder sb = new StringBuilder("ImsMmiCode {");
