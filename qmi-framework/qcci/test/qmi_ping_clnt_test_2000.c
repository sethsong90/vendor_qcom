...
*/
/*=========================================================================*/
static int qmi_ping_test_main(qmi_client_type *clnt, int test_length, int test_duration_ms)
{
  uint32_t rc;
  qmi_ping_clnt_results_type results;

  printf("\n\n\nPING TEST STARTED...\n");
  rc = qmi_ping_clnt_common_sync_basic_test(clnt, test_length, test_duration_ms, 1, &results);
  if(rc == 0)
  {
    printf("Test Passed\n");
    printf("Performance: \n");
...
int main(int argc, char *argv[])
{
 ...
 /* Setup defaults */
  ...
  rc =  qmi_ping_test_main(&clnt, test_length, test_duration_ms);
  if( rc == 0 )
  {
    printf("PASS\n");
  }
