...
setString(tAniDict *dict, ANI_U8 *key, ANI_U8 *value, ANI_U8 delimiter)
{
    ...
    entry->value += sprintf((char *)entry->line, "%s", key);
    entry->value++;
    if (delimiter == '\0') {
        sprintf((char *)entry->value, "%s", value);
    } else {
        sprintf((char *)entry->value, "%c%s%c", ANI_DICT_STR_DELIMITER, value, ANI_DICT_STR_DELIMITER);
    }
