...
public class CdmaNwkAvoidance extends PreferenceActivity implements
    ...
    public void onCreate(Bundle icicle) {
        ...
        if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
            mPhoneMsim = ITelephonyMSim.Stub.asInterface(ServiceManager
                    .getService(MSIM_PHONE_SERVICE));
            mMsimTelephonyManager = (MSimTelephonyManager)getSystemService(MSIM_TELEPHONY_SERVICE);
            if (mPhoneMsim == null || mMsimTelephonyManager == null) {
                Log.e(TAG, "Unable to get Phone Services. mPhoneMsim = " + mPhoneMsim +
                        " mMsimTelephonyManager = " + mMsimTelephonyManager);
                this.finish();
            }
            mMsimTelephonyManager.listen(mPhoneStateListener,
    ...
    private boolean isCdmaPhone() {
        ...
        try {
            if (MSimTelephonyManager.getDefault().isMultiSimEnabled()) {
                if (mPhoneMsim.getActivePhoneType(CDMA_SUB) !=
