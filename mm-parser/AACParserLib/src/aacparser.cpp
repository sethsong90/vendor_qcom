...
//$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/AACParserLib/main/latest/src/aacparser.cpp#76 $
//$DateTime: 2013/09/03 22:28:23 $
//$Change: 4375310 $
...
aacParser::aacParser(uint32 pUData,
                     uint64 fsize,
                     OSCL_FILE *FilePtr,
                     bool bHttpStreaming)
{
                                                     ...
                                                     uint32* frame_time)
{
  ...
  uint16 uData = (frame[1] << 8) + frame[0];
...
void* aacParser::get_id3v1_info()
{
  return (void*)m_aac_metadata->get_id3v1();
}
void* aacParser::get_id3v2_info()
{
  return (void*)m_aac_metadata->get_id3v2();
}
//=============================================================================
                                             ...
                                             bool bUpdateTime)
{
  ...
  else
  {
    ...
    if(m_aac_format == AAC_FORMAT_ADTS)
    {
      ...
      while(!frameStartFound)
      {
                                                   ...
                                                   &frameSize,&frameTime) )
        {
          ...
          if(m_id3tagfound == true)
          {
            ...
            m_id3tagfound = false;
            metadata_id3v2_type *id3V2 = m_aac_metadata->get_id3v2();
        ...
        if(frameStartFound == false && !std_memcmp("ID3", dataBuffer+frameBufLen, std_strlen("ID3")))
        {
      ...
      *nBytesNeeded = frameBufLen;
      ...
      if( (nBytesRead - frameBufLen) >= 3 &&
          !std_memcmp("TAG", dataBuffer+frameBufLen, std_strlen("TAG")))
      {
        frameBufLen += 128;
      }
    ...
    */
    ...
    if((nBytesRead - frameBufLen) >= 3 &&
       !std_memcmp("ID3", dataBuffer+frameBufLen, std_strlen("ID3")))
    {
      uint64 prev_startOffset = m_audio_track.start;
      m_audio_track.start = m_nCurrOffset;
      PARSER_ERRORTYPE result = parse_id3();
...
PARSER_ERRORTYPE aacParser::parse_adts_file_header ( )
{
  ...
  for(iframe_count=0;iframe_count<AAC_ADTS_FRAME_PARSE_COUNT;iframe_count++)
  {
    ...
    uint16 uData = (buffer[1] << 8) + buffer[0];
    // Verify sync word and layer field.
    if (ADTS_HEADER_MASK_RESULT != (uData & ADTS_HEADER_MASK))
    {
      MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,
    ...
    if (iframe_count == 0)
    {
      ...
      */
      ...
      aac_tech_metadata.audio_object = ((buffer [2] >> 6) & 0x03)+ 1;

      const uint8 samplingFrequencyIndex = ((buffer [2] >> 2) & 0x0F);
      const uint8 channelConfiguration
              = ((buffer [2] << 2) & 0x04)
              | ((buffer [3] >> 6) & 0x03);
...
aac_format_type aacParser::getaacformattype (const uint8 *file_header) const
{
  ...
  uint16 uData = (file_header[1] << 8) + file_header[0];
  // Check if header type is ADIF
  if(!std_memcmp("ADIF", file_header,std_strlen("ADIF")))
  {
                                                   ...
                                                   uint32* frametime)
{
  ...
  uint8 buffer[AAC_ADTS_HEADER_SIZE] = {0};
  int buf_len = AAC_ADTS_HEADER_SIZE;
  tech_data_aac aac_tech_metadata;
  aac_tech_metadata.type = AUDIO_AAC;
  ...
  else
  {
    ...
    for ( i = 0; i <= ucPfe; i++ )
    {
      ...
      /* ID bit - MPEG identifier, set to ‘1’ if the audio data in the ADTS stream is MPEG-2 AAC (see ISO/IEC 13818-7)
         ...
         with OMX_AUDIO_AACPROFILETYPE struct.
      */
      aac_tech_metadata.audio_object +=1;
      uOffset += 2;
...
PARSER_ERRORTYPE aacParser::parse_file_header ()
{
  ...
  else
  {
    ...
    if(PARSER_ErrorNone == result)
    {
       metadata_id3v2_type *stid3v2 = m_aac_metadata->get_id3v2();
       if(stid3v2)
       {
