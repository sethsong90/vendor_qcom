...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/FLVParserLib/main/latest/src/flvfile.cpp#6 $
========================================================================== */
...
========================================================================== */
uint32 FLVCallbakGetData (uint64 nOffset, uint32 nNumBytesRequest,
                                 unsigned char* pData, uint32  nMaxSize,
                                 uint32  u32UserData )
{
  if(u32UserData)
  {
    FLVFile* pFLV = (FLVFile*)u32UserData;
    uint32 nread = 0;
    return( ( (nread = pFLV->FileGetData( (uint32)nOffset,
                                           nNumBytesRequest,
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 FLVFile::FileGetData(uint32 nOffset, uint32 nNumBytesRequest,
...
SIDE EFFECTS:
  None.
======================================================================*/
unsigned long FLVFile::GetNumAudioChannels(int id)
{
  uint32 nchnls = 0;
  if(m_pFLVParser)
  {
...
#ifdef VIDEO_ENABLE
    if(eCodecType == FLV_H264)
    {
      format = H264_VIDEO;
    }
    else if(eCodecType == FLV_SORENSON_H263_VIDEO)
    {
      format = H263_VIDEO;
    }
    else if(eCodecType == FLV_VP6_VIDEO)
    {
      format = VP6F_VIDEO;
    }
    else
...
/* ======================================================================
FUNCTION:
  FLVFile::getTrackAverageBitrate
...
SIDE EFFECTS:
  None.
======================================================================*/
int32 FLVFile::getTrackAverageBitrate(uint32 id)
{
  uint32 bitrate = 0;
  if(m_pFLVParser)
  {
    bitrate = m_pFLVParser->GetTrackAverageBitRate(id);
  }
  return bitrate;
}
