...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/FLVParserLib/main/latest/src/flvparser.cpp#7 $
========================================================================== */
...
extern uint32 FLVCallbakGetData (uint64, uint32,unsigned char*,uint32,uint32);

inline void FLV_REVERSE_ENDIAN(uint8* ptr,uint8 size)
{
  uint8 endpos = size-1;
  if(ptr && (size >1))
  {
...
PARSER_ERRORTYPE FLVParser::StartParsing(void)
{
  ...
  if(m_pucDataBuffer)
  {
    ...
    if(PARSER_ErrorNone == ParseFLVHeader(m_ullCurrOffset) )
    {
      ...
      if(PARSER_ErrorNone == ParseAudioInfo(m_ullCurrOffset) )
      {
        //parses if video is present in the clip.
        //If video is not present, ParseVideoInfo will return SUCCESS.
#ifdef VIDEO_ENABLE
        if(PARSER_ErrorNone == ParseVideoInfo(m_ullCurrOffset) )
#endif
        {
                                 ...
                                 int nSyncFramesToSkip)
{
  ...
  uint64           ullTime   = 0;
  FLVTrackType     eTrackType= GetTrackType(ulTrackid);
  uint32           ulBufSize = FLV_MIN_SEEK_BUF_SIZE;
  FLVTagInfo       TagInfo;
...
/* =============================================================================
FUNCTION:
 FLVParser::GetTrackAverageBitRate
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 FLVParser::GetTrackAverageBitRate(uint32 ulTrackID)
{
  uint32 rate = 0;
...
uint32 FLVParser::GetCodecHeaderSize(uint32 ulTrackId)
{
          ...
          (m_pVideoInfo->psCodecConfig))
  {
    ...
    ulCodecDataSize = (pCodecConfig->ucNumPicParams +
                       pCodecConfig->ucNumSeqParams) * sizeof(uint32);
    for ( ; ulCount < pCodecConfig->ucNumSeqParams; ulCount++)
    {
...
PARSER_ERRORTYPE  FLVParser::ParseFLVHeader(uint64 ullOffset)
{
  ...
  {
    bool bok = true;
    if(FLVCallbakGetData(ullOffset, 16,m_pucDataBuffer,DEF_DATA_BUF_SIZE,
                        (uint32)m_pUserData))
    {
      if(!m_pFLVHdr)
      {
        ...
        if(m_pFLVHdr)
        {
          ...
          if(!memcmp(m_pucDataBuffer,"FLV",FLV_SIG_BYTES_LENGTH) )
          {
            ...
            m_pFLVHdr->ucAudioPresentFlag = (nvalbyte & 0x04)>>2;
            m_pFLVHdr->ucVideoPresentFlag = (nvalbyte & 0x01);
            m_ullCurrOffset++;
            memcpy(&(m_pFLVHdr->ulDataStartOffset),
                   m_pucDataBuffer + m_ullCurrOffset, sizeof(uint32));
            FLV_REVERSE_ENDIAN((uint8*)&(m_pFLVHdr->ulDataStartOffset),
                               sizeof(uint32));
...
PARSER_ERRORTYPE  FLVParser::ParseFLVTag(uint64 ullOffset, FLVTagInfo* ptag)
{
  ...
  if(m_pucDataBuffer)
  {
    ...
    if((2 * FLV_TAG_HDR_SIZE) == ulDataRead )
    {
      ...
      FLV_REVERSE_ENDIAN((uint8*)&(pmytag->ulPrevTagSize), sizeof(uint32));
      ulIndex += sizeof(uint32);
      ...
      FLV_REVERSE_ENDIAN((uint8*)&(pmytag->ulTagDataSize), 3);
      ulIndex+=3;

      memcpy(&(pmytag->ulTime), m_pucDataBuffer + ulIndex, sizeof(uint8)*3);
      FLV_REVERSE_ENDIAN((uint8*)&(pmytag->ulTime), 3);
      ulIndex+=3;
      ...
      FLV_REVERSE_ENDIAN((uint8*)&(pmytag->ulStreamId), 3);
      ulIndex+=3;
      retError = PARSER_ErrorNone;
      ...
      if(PARSER_ErrorNone == retError)
      {
        if(TAG_TYPE_AUDIO == taginfo.ucTagType )
        {
          ...
          if(m_pAudioInfo)
          {
            ...
            uint8 ucCodecHdr  = (ucHeaderByte & 0xF0)>>4;
            uint8 nsamplerate = (ucHeaderByte & 0x0C)>>2;
            ...
            if (FLV_AAC == m_pAudioInfo->eAudioCodec)
            {
              ...
              if (0 == taginfo.ucAACFrameType)
              {
                ...
                if (m_pAudioInfo->pucCodecConfigBuf)
                {
                  ...
                  uint32 ulDataRead = FLVCallbakGetData(
                                            ullTagOffset,
                                            taginfo.ulTagDataSize,
                                            m_pAudioInfo->pucCodecConfigBuf,
                                            m_pAudioInfo->ulCodecConfigSize,
        ...
        if((!m_pMetadataInfo)&& (TAG_TYPE_SCRIPT_DATA == taginfo.ucTagType ))
        {
          ...
          if(m_pMetadataInfo)
          {
            ...
            if (ulIndex < ulDataRead)
            {
              ...
              {
                usStrLen = m_pucDataBuffer[ulIndex] << 8 |
            ...
            while(bMetaDataStart && ulIndex < ulDataRead)
            {
              ...
              if (FLV_METADATA_ECMA_ARR_TYPE == ucDataType)
              {
                ...
                FLV_REVERSE_ENDIAN((uint8*)&(ulECMACount), sizeof(uint32));
                ulIndex += sizeof(uint32);
                while(ulIndex < ulDataRead)
                {
                  char*  pDataPtr      = (char*)m_pucDataBuffer + ulIndex;
                  uint32 ulDataTypeLen = sizeof(double);
                  ...
                  if (!memcmp(pDataPtr, SCRIPTDATA_OBJECT_END, 3))
                  {
                    ...
                    ulIndex += sizeof(SCRIPTDATA_OBJECT_END);
                    break;
                  }
                  ...
                  else if(FLV_METADATA_US16_TYPE == ucDataType)
                  {
                    ...
                    memcpy(&(usValue), m_pucDataBuffer + ulIndex + usStrLen + 1,
                           sizeof(uint16));
                    FLV_REVERSE_ENDIAN((uint8*)&(usValue), sizeof(uint16));
                    fValue = usValue;
                    ulDataTypeLen = sizeof(uint16);
                  }
                  else if(FLV_METADATA_DATE_TYPE == ucDataType)
                  {
                    ...
                    memcpy(&(usLocalDateOffset),
                           ...
                           sizeof(uint16));
                    FLV_REVERSE_ENDIAN((uint8*)&(usLocalDateOffset), sizeof(uint16));
                  }
                                         ...
                                         FLVTagInfo *pTag)
{
  ...
  else if(FLV_H264 == eVideoCodec)
  {
    ...
    memcpy(&(pTag->ulCompositionTime), pucDataBuf + ulIndex,
           sizeof(uint8)*3);
    FLV_REVERSE_ENDIAN((uint8*)&(pTag->ulCompositionTime), 3);
    ulIndex += 3;
  }
  return ulIndex;
}
  ...
  do
  {
        ...
        (FLV_H264 == eCodecType))
    {
      ...
      if ((pTemp) && (pCodecConfig))
      {
        ...
        {
          NALUDatatype *pSeqParams = NULL;
          NALUDatatype *pPicParams = NULL;
  ...
  */
  while((ulCount < ucNALUParamCount) && (pNALUParam) && (ulIndex < ulBufSize))
  {
    memcpy(&pNALUParam[ulCount].usNALLength, pTemp + ulIndex,
           sizeof(uint16));
    FLV_REVERSE_ENDIAN((uint8*)&pNALUParam[ulCount].usNALLength,
                       sizeof(uint16));
    ulIndex += sizeof(uint16);
                                       ...
                                       uint32 ulBufSize)
{
  ...
  ucNALUSizeBytesLen = m_pVideoInfo->psCodecConfig->ucNALLengthMinusOne + 1;
  ...
  while(ulIndex < ulDataRead)
  {
    ...
    bool bRet = GetNextH264NALUnit(ulIndex, ucNALUSizeBytesLen, pDataBuf,
                                   &ucNALUType, &ulNALSize, ulDataRead);
