...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/FileBaseLib/main/latest/src/filebase.cpp#99 $
$DateTime: 2014/02/27 21:08:17 $
$Change: 5361780 $
                                      ...
                                      uint32 ulFrameBufSize)
{
  ...
  for( ; nCount < 2 && PARSER_ErrorNone == error; nCount++)
  {
    ...
    while( (0xFF != pucFramedata[nBufIndex]) &&
    ...
    pucFramedata = pucBuffer + (nBufIndex + 1);
    ...
    boolean crc_present = (boolean)
      ((pucFramedata[MP3HDR_CRC_OFS] & MP3HDR_CRC_M) >> MP3HDR_CRC_SHIFT);
                                     ...
                                     uint8* pucSrcBuf)
{
        ...
        (ulNALULengthSizeMinusOne <= ulBufferSize) )
  {
    ...
    (void) memmove (pucDestBuf + ulDataBufFilledLen, &ulStartCode,
                    sizeof(ulStartCode));
    ulDataBufFilledLen  += sizeof(ulStartCode);
                                    ...
                                    FileSourceFileFormat format)
{
  ...
  #else
    if(IsMPEG2File(NULL,FileFormatBuf,true))
  #endif
    {
                          ...
                          bool *pbDataUnderRun )
{
  ...
  if( pPort )
  {
    if (pPort->Seek(pos,video::iStreamPort::DS_SEEK_SET,&nOutOffset) == video::iStreamPort::DS_SUCCESS)
    {
      video::iStreamPort::DataSourceReturnCode eStatus;
      eStatus = pPort->Read(buffer,size,(int*)&nRead);
      if(pbDataUnderRun)
      {
...
bool FileBase::IsAVIFile(video::iStreamPort* pPort,uint8* pBuf,bool bUseExtension)
{
  if(pBuf)
  {
    if ( (!memcmp(pBuf, "RIFF", FOURCC_SIGNATURE_BYTES)) && (!memcmp(pBuf+8, "AVI ", FOURCC_SIGNATURE_BYTES)) )
    {
       return true;
    }
  }
...
bool FileBase::IsID3TagPresent( uint8* pucDataBuf, uint32* pulID3TagLen )
{
  ...
  if(( NULL != pProcessBuff ) && ( NULL != pulID3TagLen ) &&
     ( 0 == std_memcmp("ID3",pProcessBuff,std_strlen("ID3"))))
  {
                                        ...
                                        uint32 ulFrameBufLen)
{
  ...
  uint16 usFrameSync = (pucFrameBuff[1] << 8) + pucFrameBuff[0];

  if(( NULL != pucFrameBuff) &&
    ( 0 == std_memcmp("ADIF", pucFrameBuff,std_strlen("ADIF"))))
  {
    RetStatus = PARSER_ErrorNone;
  }
  else if((NULL != pucFrameBuff &&
           0 == std_memcmp(ucLATAMMask, pucFrameBuff,STD_SIZEOF(ucLATAMMask))))
  {
    RetStatus = PARSER_ErrorNone;
  }
