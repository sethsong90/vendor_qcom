...
const uint32 EC3_AUDIO_FORMAT_IDENTIFIER_TYPE = 0x45414333;

// VC1 Codes
// SMPTE RP227
const uint8 VC1_SEQ_START_CODE                = 0xF;
const uint8 VC1_FRAME_START_CODE              = 0xD;
//Microsoft Internal: VC-1 Compressed Video Bitstream
//Format and Decoding Process
//Table 33 is used to decode the PTYPE syntax element in the picture header.
//Table 33: Advanced Profile Picture Type VLC
...
const uint32 VC1_VIDEO_FORMAT_IDENTIFIER_TYPE = 0x56432D31;
const uint32 LPCM_AUDIO_FORMAT_IDENTIFIER     = 0x48444D56;
/*
...
const uint8 TS_DESC_HEADER_LEN = 0x06;
//^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
const uint16 MAX_INIT_PACKS_TO_PARSE      = 100;
const uint16 MAX_INIT_TSPKTS_TO_PARSE     = 500;
const uint16 MAX_PS_PACK_SIZE             = 2048;
...
const uint8  MPEG2_PACK_HDR_BYTES         = 14;
const uint8  MPEG1_PACK_HDR_BYTES         = 12;
const uint8  SYS_HDR_BYTES                = 8;
const uint8  SYS_HDR_STREAM_ID_INFO_BYTES = 4;
...
const uint32  TS_REGIS_FORMATID_AC3       = 0x41432D33;
...
#define SEEK_TIMESTAMP_GAP_TOLERANCE_MS 2000
