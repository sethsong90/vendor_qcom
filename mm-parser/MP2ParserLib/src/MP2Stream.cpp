...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/MP2ParserLib/main/latest/src/MP2Stream.cpp#97 $
========================================================================== */
...
========================================================================== */
uint32 MP2StreamCallbakGetData (uint64 nOffset, uint32 nNumBytesRequest,
                                unsigned char* pData, uint32  nMaxSize,
                                uint32  u32UserData )
{
  if(u32UserData)
  {
    MP2Stream* pMP2TStream = (MP2Stream*)u32UserData;
    /* Just return number of bytes read. Partial data skipping can be done internal to stream parser object*/
    return( pMP2TStream->FileGetData(nOffset, nNumBytesRequest, nMaxSize, pData) );
  }
  return 0;
}
  ...
  */
  ...
  if(pport)
  {
    if(pport->GetAvailableOffset(&nDownloadedBytes, &bEndOfData) == video::iSourcePort::DS_SUCCESS)
    {
      if(bEndOfData)
      {
        m_fileSize = size = nDownloadedBytes;
  ...
  if(m_pMP2StreamParser->GetTrackType(ulTrackID, &cType,&codec_type) == MP2STREAM_SUCCESS)
  {
    ...
    if( (nBytes > 0) && (retStatus == MP2STREAM_SUCCESS))
    {
      ...
      if(!bError)
      {
        ...
        {
          if((cType == TRACK_TYPE_AUDIO) && (codec_type == AUDIO_CODEC_LPCM)
            && (pucDataBuf))
          {
            if(nBytes % 2)
            {
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 MP2Stream::getMovieDuration() const
{
  uint64 nDuration = 0;
  if(!m_pMP2StreamParser)
  {
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 MP2Stream::getMovieTimescale() const
{
  uint32 nTimeScale = MPEG2_STREAM_TIME_SCALE;
  if(!m_pMP2StreamParser)
  {
    nTimeScale = 0;
  }
...
/* ======================================================================
FUNCTION:
  MP2Stream::GetMaximumBitRateForTrack(uint32 trackId)
...
/* ======================================================================
FUNCTION:
  MP2Stream::getTrackContentVersion
...
/* ======================================================================
FUNCTION:
  MP2Stream::trackRandomAccessDenied
...
/* ======================================================================
FUNCTION:
  MP2Stream::getTrackMediaDuration
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 MP2Stream::getTrackMediaDuration(uint32 id)
{
  return getMovieDurationMsec();
}
/* ======================================================================
FUNCTION:
  MP2Stream::getTrackMediaTimescale
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 MP2Stream::getTrackMediaTimescale(uint32 id)
{
  return 1000;
}
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 MP2Stream::getTrackAudioSamplingFreq(uint32 id)
{
  uint32 freq = 0;
  if(m_pMP2StreamParser)
  {
    freq = m_pMP2StreamParser->GetAudioSamplingFrequency(id);
  }
  return freq;
}
/* ======================================================================
FUNCTION:
  MP2Stream::getAudioFrameDuration
...
/* ======================================================================
FUNCTION:
  MP2Stream::getAudioSamplesPerFrame
...
/* ======================================================================
FUNCTION:
  MP2Stream::GetAudioBitsPerSample
...
SIDE EFFECTS:
  None.
======================================================================*/
unsigned long MP2Stream::GetNumAudioChannels(int id)
{
  uint32 nchnls = 0;
  if(m_pMP2StreamParser)
  {
    nchnls = m_pMP2StreamParser->GetNumberOfAudioChannels(id);
  }
  return nchnls;
}
/* ======================================================================
FUNCTION:
  MP2Stream::GetAudioEncoderOptions
    ...
    switch(type)
    {
      ...
      case TRACK_TYPE_VIDEO:
      {
          ...
          else
          {
            ...
            if(height && width)
            {
              bufsize = int32(height * width * 0.75);
            }
          }
...
int MP2Stream::GetTotalNumberOfAudioStreams()
{
  ...
  if(m_pMP2StreamParser)
  {
    ntracks = m_pMP2StreamParser->GetTotalNumberOfAudioTracks();
  }
  return ntracks;
}
 /* ======================================================================
...
int MP2Stream::GetTotalNumberOfVideoStreams()
{
  ...
  if(m_pMP2StreamParser)
  {
    ntracks = m_pMP2StreamParser->GetTotalNumberOfVideoTracks();
  }
  return ntracks;
}
 /* ======================================================================
...
PARSER_ERRORTYPE MP2Stream::getTrackDecoderSpecificInfoContent(uint32 id, uint8* buf, uint32 *pbufSize)
{
  ...
  if((m_pMP2StreamParser) && ((m_pMP2StreamParser->GetTrackType(id,&cType,&codec_type)) == MP2STREAM_SUCCESS))
  {
    if(pbufSize &&
      ((codec_type== AUDIO_CODEC_AAC) || (codec_type== VIDEO_CODEC_H264)) )
    {
      if(MP2STREAM_SUCCESS == m_pMP2StreamParser->GetTrackDecoderSpecificInfoContent(id, buf,(uint8*)pbufSize) )
      {
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 MP2Stream::GetAACAudioProfile(uint32 id)
{
  uint32 audioObject = 0;
  if(m_pMP2StreamParser)
  {
    audioObject = m_pMP2StreamParser->GetAACAudioProfile(id);
  }
  return audioObject;
}
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 MP2Stream::GetAACAudioFormat(uint32 id)
{
  uint32 audioFormat = 0;
  if(m_pMP2StreamParser)
  {
    audioFormat = m_pMP2StreamParser->GetAACAudioFormat(id);
  }
  return audioFormat;
}
...
===========================================================================*/
bool MP2Stream::GetTotalAvgBitRate(uint32 * pBitRate)
{
  return true;
}
/*! =======================================================================
...

{
  ...
  if(m_bHaveTransportRate)
  {
    ...
    *pullOffset = (uint64)((fpTransportRate*ullPBTime)/1000);
