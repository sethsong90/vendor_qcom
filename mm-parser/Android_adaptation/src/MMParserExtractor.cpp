...
status_t FileSourceWrapper::Init(iStreamPort& aStreamPort,
                                 FileSourceFileFormat eFileFormat) {
    m_pFileSource = new FileSource(FileSourceWrapper::cbFileSourceStatus, (void*)this);
    if(!m_pFileSource) {
        return ERROR_IO;
    }

    FileSourceStatus status = m_pFileSource->OpenFile(&aStreamPort,
                                                      eFileFormat, TRUE);
...
FileSourceStatus FileSourceWrapper::GetFormatBlock(uint32 id, uint8* buf, uint32 *pbufSize) {
    Mutex::Autolock autoLock(mMutex);
    return m_pFileSource->GetFormatBlock(id, buf, pbufSize);
}
...
FileSourceStatus FileSourceWrapper::GetClipMetaData(wchar_t* pMetaData,
                    ...
                    bool *pbIsUTF8) {
    Mutex::Autolock autoLock(mMutex);
    return m_pFileSource->GetClipMetaData(pMetaData, size, pSampleInfo, pbIsUTF8);
}

void FileSourceWrapper::cbFileSourceStatus(FileSourceCallBackStatus status, void* pCbData)
{
    LOGV("FileSourceWrapper::cbFileSourceStatus , pCbData = 0x%x, status = %d ===>\n", (unsigned int)pCbData, status);
...
void MMParserExtractor::setAlbumMetaElement(FileSourceMetaDataType element, uint32_t key) {
    ...
    if(albBuffer != NULL) {
        ...
        if(albStatus==FILE_SOURCE_SUCCESS && nMetaBuff > 0 ) {
            ...
            if(bIsUTF8) {
                mFileMetaData->setCString(key, (char*)albBuffer);
            }
            else {
    ...
    for (uint32_t i=0; i<numTracks; i++){
        ...
        if (status == FILE_SOURCE_SUCCESS){
            ...
            if (majorType == FILE_SOURCE_MJ_TYPE_AUDIO){
                ...
                int64 AudioDuration = m_pFileSourceWrapper->GetTrackMediaDuration(trackInfo.id);
                ...
                if (format == FILE_SOURCE_3G2) { //File is fragmented
                    ...
                    if (AudioDuration <= 0)
                        AudioDuration = m_pFileSourceWrapper->GetTrackMediaDuration(trackInfo.id);
                }
                ...
                const int64 kKeyDurationValue = AudioDuration * (1000000 /  mediaTrackInfo.audioTrackInfo.timeScale);
                ...
                if( (minorType == FILE_SOURCE_MN_TYPE_AC3) || (minorType == FILE_SOURCE_MN_TYPE_EAC3)) {
                    ...
                    configStatus = m_pFileSourceWrapper->SetConfiguration(trackInfo.id, &pitem, FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME);
                    if( configStatus != FILE_SOURCE_SUCCESS ) {
                        LOGE("Setting MP3 config %d", configStatus );
                    }
                    //Set Album Art related data
                    bSetAlbumArt = true;
                    ...
                    if((8 == mediaTrackInfo.audioTrackInfo.nBitsPerSample) &&
                       (FILE_SOURCE_MN_TYPE_PCM == minorType)) {
                        ...
                        if( configStatus != FILE_SOURCE_SUCCESS ) {
                            LOGE("Setting WAV config failed %d", configStatus );
                        }
                    }
                    ...
                    configStatus = m_pFileSourceWrapper->SetConfiguration(trackInfo.id, &pitem, FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME);
                    if( configStatus != FILE_SOURCE_SUCCESS ) {
                        LOGE("Setting AMR config failed %d", configStatus );
                    }
                ...
                int64 VideoDuration = m_pFileSourceWrapper->GetTrackMediaDuration(trackInfo.id);
                FileSourceFileFormat format = FILE_SOURCE_UNKNOWN;
                m_pFileSourceWrapper->GetFileFormat(format);
                if (format == FILE_SOURCE_3G2) { //File is fragmented
                    VideoDuration = m_pFileSourceWrapper->GetClipDuration();
                    if (VideoDuration <= 0)
                        VideoDuration = m_pFileSourceWrapper->GetTrackMediaDuration(trackInfo.id);
                }
                ...
                const int64 kKeyDurationValue = VideoDuration * (1000000 /  mediaTrackInfo.videoTrackInfo.timeScale);
                track->meta->setInt64(kKeyDuration,kKeyDurationValue);
                ...
                if (m_pFileSourceWrapper->IsDrmProtection()) {
                    ...
                    if (!m_pFileSourceWrapper->IsSeekDenied()) {
                        ...
                        for (uint64 lastSyncSampleTime = 0, syncSample = 0;
                            ...
                            lastSyncSampleTime = sampleInfo.startTime + Delta;
                            ...
                            if (FILE_SOURCE_DATA_OK != status || bytesRead <= 0){
                                ...
                                thumbnailTimestampUs = (sampleInfo.startTime * 1000000)/
                                                        track->timescale;
                ...
                int64 TextDuration =
                    m_pFileSourceWrapper->GetTrackMediaDuration(trackInfo.id);
                FileSourceFileFormat format = FILE_SOURCE_UNKNOWN;
                m_pFileSourceWrapper->GetFileFormat(format);
                if (format == FILE_SOURCE_3G2) { //File is fragmented
...
status_t MMParserSource::read(
    ...
    *out = NULL;
    ...
    if (options && options->getSeekTo(&seekTimeUs, &seekMode)) {
        ...
        seekTimems = (uint32_t) ((seekTimeUs * mTimescale) / 1000000);
    ...
    if (newBuffer) {
        ...
        if (FILE_SOURCE_DATA_OK != status){
            ...
            if (FILE_SOURCE_DATA_END == status){    //Grab this as a special case
                ...
                LOGE("FileSource::FILE_SOURCE_DATA_INSUFFICIENT \n");
                return -EAGAIN;
            }
            return ERROR_IO;
        }
        ...
        uint64 ullTime = 0;
        if(mTimescale) {
            ullTime = (sInfo.startTime * 1000000) / mTimescale;
        }
        else {
            LOGE("timescale for track %u is set as ZERO!!",
                  mTrackId);
        }
        mBuffer->meta_data()->setInt64(kKeyTime, ullTime);
        ++mCurrentSampleIndex;
        mCurrentTimeMS = sInfo.startTime;
