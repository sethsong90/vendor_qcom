/* -*- Mode: C++ -*-
  ...
  $Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/QCPParserLib/main/latest/src/qcpparser.cpp#36 $
...
#define MAX_QCELP_FILE_SIZE (1500 * 1024)
//=============================================================================
// FUNCTION DEFINITONS
//=============================================================================
//=============================================================================
...
//=============================================================================
//
qcpParser::qcpParser(uint32 pUData,
                     uint64 fsize,
                     OSCL_FILE *FilePtr){
  m_pUserData = pUData;
  m_nFileSize = fsize;
                                             ...
                                             uint32 *nBytesNeeded)
{
  ...
  if(m_hOutputModeEnum == FILE_SOURCE_MEDIA_OUTPUT_SINGLE_AUDIO_FRAME)
  {
    uint32 framesize = 0;
    uint32 frametime = 0;
    nBytesRead = QCPCallbakGetData (m_nCurrOffset, QCP_FRAME_HEADER_SIZE, m_dataBuffer, nMaxBufSize, m_pUserData );
    if(nBytesRead)
    {
      ...
      if(IAUDIO_SUCCESS == parse_frame_header(m_dataBuffer,&framesize,&frametime) && framesize)
      {
        ...
        nBytesRead = QCPCallbakGetData (m_nCurrOffset, framesize, m_dataBuffer, nMaxBufSize, m_pUserData );
  ...
  {
    nBytesRead = QCPCallbakGetData (m_nCurrOffset, nNumBytesRequest, m_dataBuffer, nMaxBufSize, m_pUserData );
    if(!(nBytesRead))
    {
...
//=============================================================================
//
PARSER_ERRORTYPE qcpParser::calculate_get_duration(uint64* uDuration)
{
  ...
  if (NULL == uDuration)
  {
    ...
    MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,
                   "qcpformatparser::get_duration: time parameter was NULL!");
    return result;
  }

  if (QCP_FORMAT_UNKNOWN == m_qcp_format)
  {
    //parse_file_header API is not called yet - get_duration API can be called only
    //after qcp format parser successfully parses the file header
    MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,
                  "qcpformatparser::get_duration: Bad state, qcp format not set!");
  ...
  {
    MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,
                     "qcpformatparser::get_duration: Bad state, qcp duration not available!");
    result = PARSER_ErrorDefault;
    return result;
  }
  result = PARSER_ErrorNone;
  // *time  = (Total Number of packets*20e-03)  > Irrespective of CBR/VBR
  *uDuration = (int64) ((int64) (m_qcp_tech_metadata.size_in_packet)*(int64) QCP_FRAME_DURATION); // as each packet is of duration 20ms
  return result;
}
