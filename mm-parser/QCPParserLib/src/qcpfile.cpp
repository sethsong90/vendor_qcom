...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/QCPParserLib/main/latest/src/qcpfile.cpp#39 $
$DateTime: 2012/09/17 11:18:52 $
$Change: 2810037 $
...
uint32 QCPCallbakGetData (uint64         nOffset,
                          ...
                          uint32        u32UserData )
{
  if(u32UserData)
  {
    QCPFile *pQCPFile = (QCPFile *)u32UserData;
    uint32 nRead = 0;
    nRead = pQCPFile->FileGetData(nOffset,
                                  nNumBytesRequest,
                                  nMaxSize,
                                  pData);
    return(nRead);
  }
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL, "QCPCallbakGetData u32UserData is NULL");
  return 0;
}
...
bool QCPFile::ParseQCPHeader()
{
  ...
  m_pqcpParser = MM_New_Args(qcpParser,((uint32)this,m_fileSize,m_QCPFilePtr));
...
PARSER_ERRORTYPE QCPFile::getTrackDecoderSpecificInfoContent(uint32 /*id*/, uint8* buf, uint32 *pbufSize)
{
  ...
  if(m_pqcpParser)
  {
    if(m_pqcpParser->GetQCPDecodeInfo(&sqcp_decode_info)==PARSER_ErrorNone)
    {
      ...
      *pbufSize = sizeof(sqcp_decode_info);
...
SIDE EFFECTS:
  None.
======================================================================*/
unsigned long QCPFile::GetNumAudioChannels(int /*id*/)
{
