...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/OGGParserLib/main/latest/src/OGGStreamParser.cpp#36 $
========================================================================== */
...
OGGStreamStatus  OGGStreamParser::ParseOGGPage(uint64 offset,OggPage* pOggPage)
{
     ...
     m_pDataBufferOffset != m_nOggDataBufferSize)
  {
     ...
     m_pDataBufferOffset = m_pDataBuffer - m_pDataCache;
  }
  else
  {
     m_pDataBufferOffset = m_nOggDataBufferSize;
  ...
  if(m_pDataCache && pOggPage && m_nOggDataBufferSize)
  {
    ...
    int nSizeNeeded = m_nOggDataBufferSize;
#ifdef OGG_PARSER_DEBUG
    ...
    if(m_nOggDataBufferFillSize <= m_pDataBufferOffset||
        (m_nOggDataBufferFillSize - m_pDataBufferOffset < MAX_PAGE_SIZE
    ...
    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
      ...
      if(!OGGStreamCallbakGetData (offset, nSizeNeeded, m_pDataCache, m_nOggDataBufferSize, (uint32) m_pUserData) )
      {
...
OGGStreamStatus  OGGStreamParser::ParseVorbisIdentificationHdr(uint32& localOffset,OggPage* pOggPage)
{
  ...
  if(pOggPage)
  {
    ...
    uint8 nIndexToWrite = m_nAudioStreams -1;
    bool bOK = true;
    if(!m_pOggAudioCodecInfo)
    {
    ...
    if(bOK)
    {
      ...
      memcpy(&(m_pOggAudioCodecInfo[nIndexToWrite].SamplingFrequency),
              m_pDataBuffer+localOffset,sizeof(uint32));
      localOffset += sizeof(uint32);
  ...
  if(pOggPage)
  {
    ...
    if(bOK)
    {
          ...
          (m_pFlacParser->StartParsing(tempoffset) == FLACPARSER_SUCCESS) )
      {
        ...
        m_pOggAudioCodecInfo[nIndexToWrite].SamplingFrequency =
        m_pFlacParser->GetAudioSamplingFrequency(pOggPage->SerialNo);
        m_pOggAudioCodecInfo[nIndexToWrite].NumberOfChannels =
        m_pFlacParser->GetNumberOfAudioChannels(pOggPage->SerialNo);
        retError = OGGSTREAM_SUCCESS;
...
OGGStreamStatus  OGGStreamParser::ParseTheoraIdentificationHdr(uint32& localOffset,OggPage* pOggPage)
{
  ...
  */
    ...
    uint8 nIndexToWrite = m_nVideoStreams -1;
    if(!m_pOggVideoCodecInfo)
    {
    ...
    if(bOK)
    {
      ...
      m_pOggVideoCodecInfo[nIndexToWrite].FmbW =
      ( ( (uint16)m_pDataBuffer[localOffset]) <<8 ) |
      ( ((uint16)m_pDataBuffer[localOffset+1]) );
      localOffset += sizeof(uint16);

      m_pOggVideoCodecInfo[nIndexToWrite].FmbH =
      ( ( (uint16)m_pDataBuffer[localOffset]) <<8) |
      ( ((uint16)m_pDataBuffer[localOffset+1]));
      localOffset += sizeof(uint16);
      ...
      uint32 frn = (((uint32)m_pDataBuffer[localOffset])<<24)  |
                   (((uint32)m_pDataBuffer[localOffset+1])<<16)|
                   (((uint32)m_pDataBuffer[localOffset+2])<<8) |
                   (((uint32)m_pDataBuffer[localOffset+3]));
      localOffset += (4*sizeof(uint8));
      ...
      m_pOggVideoCodecInfo[nIndexToWrite].CS =
      ...
      m_pOggVideoCodecInfo[nIndexToWrite].QUAL =
      ((m_pDataBuffer[localOffset] & 0xFC)>>2);

      m_pOggVideoCodecInfo[nIndexToWrite].Kfgshift =
      ((m_pDataBuffer[localOffset]  & 0x03)<<3)|
      ((m_pDataBuffer[localOffset+1]& 0xE0)>>5);

      m_pOggVideoCodecInfo[nIndexToWrite].PF =
...
SIDE EFFECTS:
  None.
=============================================================================*/
OGGStreamStatus OGGStreamParser::ParseCommentHdr(uint32& localOffset,
                                                 ogg_media_codec_type codec)
{
  ...
  if( codec != OGG_UNKNOWN_AUDIO_VIDEO_CODEC )
  {
    retError = OGGSTREAM_SUCCESS;
    #ifdef OGG_PARSER_DEBUG
    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,"ParseCommentHdr @ offset %lu",localOffset);
    #endif
    uint32 comment_hdr_start_offset = localOffset;
    uint32 vendor_length = 0;
    uint32 vendor_length_offset = localOffset + sizeof(uint8);

    if(codec == OGG_AUDIO_CODEC_VORBIS)
    {
      vendor_length_offset+= VORBIS_CODEC_SIGN_SIZE;
    }
    if(codec == OGG_VIDEO_CODEC_THEORA)
    {
      vendor_length_offset+= THEORA_CODEC_SIGN_SIZE;
    }
    ...
    if( (user_comment_list_length)&& (!m_pOggMetaData) )
    {
      m_pOggMetaData = (ogg_meta_data*)MM_Malloc( user_comment_list_length * sizeof(ogg_meta_data) );
      if(m_pOggMetaData)
      {
        memset(m_pOggMetaData,0,user_comment_list_length * sizeof(ogg_meta_data) );
    ...
    uint32  total_user_comment_length =
    (user_comment_list_length * sizeof(uint32));
    ...
    for(uint32 i = 0 ; i< user_comment_list_length;i++)
    {
      ...
      if( (user_comment_length)&& (m_pOggMetaData) )
      {
        ...
        if(tempptr)
        {
          ...
          if(index1)
          {
            ...
            for(int j = 0; j < MAX_FIELDNAMES_SUPPORTED;j++)
            {
              CONVERT_TO_UPPER(tempptr,(user_comment_length-fieldnamelength));
              uint8* fname = (uint8*)&OGGFieldNames[j][0];
              if(
                  ...
                  (!std_strncmp((const char*)fname,(const char*)tempptr,fieldnamelength) ) )
              {
                ...
                if( (nfieldvallen > 0)&& (!m_pOggMetaData[i].bAvailable) )
                {
                  m_pOggMetaData[i].pMetaData = (uint8*)MM_Malloc( nfieldvallen);
                  if(m_pOggMetaData[i].pMetaData)
                  {
                    memcpy(m_pOggMetaData[i].pMetaData,
                           m_pDataBuffer+user_comment_offset+sizeof(uint32)+fieldnamelength+1,
                           (nfieldvallen-1));
                    m_pOggMetaData[i].pMetaData[nfieldvallen-1]='\0';
                    m_pOggMetaData[i].nMetaDataFieldIndex = (uint16)j;
                    m_pOggMetaData[i].nMetaDataLength = (nfieldvallen-1);
                    m_pOggMetaData[i].bAvailable = true;
                    break;
                  }//if(m_pOggMetaData[i].pMetaData)
                }//if(nfieldvallen > 0)
              }//if(strlen((const char*)fname) == fieldnamelength)
            }//for(int i = 0; i<MAX_FIELDNAMES_SUPPORTED;i++)
          }//if(index1)
          MM_Free(tempptr);
        }//if(tempptr)
    ...
    uint32 comment_hdr_end_offset =
      user_comment_list_length_offset + sizeof(uint32) +  total_user_comment_length;

    //Add one for framebit
    if(codec == OGG_AUDIO_CODEC_VORBIS)
    {
       comment_hdr_end_offset++;
    }
    uint32 comment_hdr_size =
    comment_hdr_end_offset - comment_hdr_start_offset;

    localOffset = comment_hdr_end_offset;
    ...
    //SETUP HDR till current page end offset - VORBIS_CODEC_SIGN_SIZE -1 .
     ...
     for( ; ((( m_pOggPage->PageEndOffset - VORBIS_CODEC_SIGN_SIZE -1) >= localOffset) &&
              (m_pDataBuffer[localOffset] != VORBIS_SETUP_HDR_BYTE)); localOffset++
         );

    #ifdef OGG_PARSER_DEBUG
    MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_HIGH,
    "COMMENT_HDR vendor_length %lu user_comment_list_length %lu total_user_comment_length %lu",
    vendor_length,user_comment_list_length,total_user_comment_length);

    MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_HIGH,
    "COMMENT_HDR comment_hdr_start_offset %lu comment_hdr_end_offset %lu comment_hdr_size %lu",
    comment_hdr_start_offset,comment_hdr_end_offset,comment_hdr_size);
    #endif
        ...
        (codec == OGG_AUDIO_CODEC_VORBIS) )
    {
      ...
      (uint8*)MM_Malloc(comment_hdr_size);
       memcpy(m_pOggAudioCodecInfo[m_nAudioStreams-1].CommentHdr,
              m_pDataBuffer+comment_hdr_start_offset,
              comment_hdr_size);
      m_pOggAudioCodecInfo[m_nAudioStreams-1].nCommentHdrSize = comment_hdr_size;
    }
    if( (m_nVideoStreams) &&
        (comment_hdr_size) &&
        (codec == OGG_VIDEO_CODEC_THEORA) )
    {
      ...
      (uint8*)MM_Malloc(comment_hdr_size);
       memcpy(m_pOggVideoCodecInfo[m_nVideoStreams-1].CommentHdr,
              m_pDataBuffer+comment_hdr_start_offset,
              comment_hdr_size);
      m_pOggVideoCodecInfo[m_nVideoStreams-1].nCommentHdrSize = comment_hdr_size;
    }
  }
  return retError;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
void OGGStreamParser::GetClipMetaData(int nIndex,
                                      ...
                                      uint32* pMetadataLength)
{
  if(pMetadataLength)
  {
    ...
    for(uint32 i =0; i< m_nMetaData; i++)
    {
      if( (m_pOggMetaData[i].nMetaDataFieldIndex == nIndex) &&
          (m_pOggMetaData[i].pMetaData) )
      {
        nCount += m_pOggMetaData[i].nMetaDataLength;
        nMatched++;
      }
    }
        ...
        for(uint32 i =0; i< m_nMetaData; i++)
        {
              ...
              (m_pOggMetaData[i].pMetaData) )
          {
            memcpy(pMetadataValue+writeindex,
                   m_pOggMetaData[i].pMetaData,
                   m_pOggMetaData[i].nMetaDataLength);
            writeindex+=m_pOggMetaData[i].nMetaDataLength;
            if(nToCopy >1)
            {
...
SIDE EFFECTS:
  None.
=============================================================================*/
void  OGGStreamParser::GetClipMetaDataIndex(uint8* pFieldname,
                                            ...
                                            int* pFieldIndex)
{
  if(pFieldname && pFieldIndex && nLengthFieldName)
  {
    for(int i = 0; i < MAX_FIELDNAMES_SUPPORTED;i++)
    {
      ...
      uint8* fname = (uint8*)&OGGFieldNames[i][0];
      if(
...
uint32 OGGStreamParser::GetTrackWholeIDList(uint32* idList)
{
  ...
  for (ulCount = 0; m_pOggVideoCodecInfo && (ulCount < m_nVideoStreams); ulCount++)
  {
    (*idList) = ulCount;
    m_pOggVideoCodecInfo[ulCount].TrackId = ulCount;
    idList++;
  }
  ulCount = 0;
  for (ulCount = 0; m_pOggAudioCodecInfo && (ulCount < m_nAudioStreams); ulCount++)
  {
    (*idList) = m_nVideoStreams + ulCount;
    m_pOggAudioCodecInfo[ulCount].TrackId = ulCount + m_nVideoStreams;
    idList++;
  }
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetVideoWidth(uint32 id)
{
  uint32 width = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetVideoWidth");
#endif
  for (uint32 i = 0; m_pOggVideoCodecInfo && (i < m_nVideoStreams); i++)
  {
    if(m_pOggVideoCodecInfo[i].TrackId == id)
    {
      width = m_pOggVideoCodecInfo[i].Width;
    }
  }
  return width;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetVideoHeight(uint32 id)
{
  uint32 height = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetVideoHeight");
#endif
  for (uint32 i = 0; m_pOggVideoCodecInfo && (i < m_nVideoStreams); i++)
  {
    if(m_pOggVideoCodecInfo[i].TrackId == id)
    {
      height = m_pOggVideoCodecInfo[i].Height;
    }
  }
  return height;
}
/* =============================================================================
FUNCTION:
 OGGStreamParser::GetAudioSamplingFrequency
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetAudioSamplingFrequency(uint32 id)
{
  uint32 sfreq = 0;
#ifdef OGG_PARSER_DEBUG
  ...
  for (uint32 i = 0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
  {
    if(m_pOggAudioCodecInfo[i].TrackId == id)
    {
      sfreq = m_pOggAudioCodecInfo[i].SamplingFrequency;
    }
  }
  return sfreq;
}
/* =============================================================================
FUNCTION:
 OGGStreamParser::GetNumberOfAudioChannels
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint8  OGGStreamParser::GetNumberOfAudioChannels(uint32 id)
{
  uint8 noChannels = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetNumberOfAudioChannels");
#endif
  for (uint32 i = 0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
  {
    if(m_pOggAudioCodecInfo[i].TrackId == id)
    {
      noChannels = m_pOggAudioCodecInfo[i].NumberOfChannels;
    }
  }
  return noChannels;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetTrackAverageBitRate(uint32 id)
{
  uint32 rate = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetTrackAverageBitRate");
#endif
  for (uint32 i = 0; m_pOggVideoCodecInfo && (i < m_nVideoStreams); i++)
  {
    if(m_pOggVideoCodecInfo[i].TrackId == id)
    {
      rate = m_pOggVideoCodecInfo[i].BitRate;
    }
  }
  for (uint32 i = 0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
  {
    if(m_pOggAudioCodecInfo[i].TrackId == id)
    {
      rate = m_pOggAudioCodecInfo[i].NominalBitRate;
    }
  }
  return rate;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetTrackMaxBitRate(uint32 id)
{
  uint32 rate = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetTrackMaxBitRate");
#endif
  for (uint32 i = 0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
  {
    if(m_pOggAudioCodecInfo[i].TrackId == id)
    {
      rate = m_pOggAudioCodecInfo[i].MaximumBitRate;
    }
  }
  return rate;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 OGGStreamParser::GetTrackMinBitRate(uint32 id)
{
  uint32 rate = 0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetTrackMinBitRate");
#endif
  for (uint32 i = 0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
  {
    if(m_pOggAudioCodecInfo[i].TrackId == id)
    {
      rate = m_pOggAudioCodecInfo[i].MinimumBitRate;
    }
  }
  return rate;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
float OGGStreamParser::GetVideoFrameRate(uint32 id)
{
  float rate = 0.0;
#ifdef OGG_PARSER_DEBUG
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"GetVideoFrameRate");
#endif
  for (uint32 i = 0; m_pOggVideoCodecInfo && (i < m_nVideoStreams); i++)
  {
    if( (m_pOggVideoCodecInfo[i].TrackId == id) &&
        (m_pOggVideoCodecInfo[i].AspectRatio_Dmr) )
    {
      rate = m_pOggVideoCodecInfo[i].FrameRate;
    }
  }
  return rate;
}
...
uint8* OGGStreamParser::GetCodecHeader(uint32 id)
{
  ...
  if( (codec == OGG_AUDIO_CODEC_FLAC)|| (codec == OGG_AUDIO_CODEC_VORBIS) )
  {
    for(uint32 i =0; m_pOggAudioCodecInfo && (i < m_nAudioStreams); i++)
    {
      if(m_pOggAudioCodecInfo[i].SerialNo == ulSerialNum)
      {
        nAudioIndex = i;
        break;
      }
  ...
  if(codec == OGG_VIDEO_CODEC_THEORA)
  {
    for(uint32 i =0; m_pOggVideoCodecInfo && (i < m_nVideoStreams); i++)
    {
      if(m_pOggVideoCodecInfo[i].SerialNo == ulSerialNum)
      {
        nVideoIndex = i;
        break;
      }
...
// continued to the remaining bytes in the page to find the checksum.
  ...
  pPageBuf[25] = (nStrmCheckSum>> 24) & 0xff;
...
uint64 OGGStreamParser::FindNextPageOffset(uint64 nOffsetPrevGoodPos)
{
  ...
  uint32 nBytesNeeded = m_nOggDataBufferSize;
  ...
  while(nOffset < m_nFileSize)
  {
   ...
   if(m_nOggDataBufferFillSize < m_pDataBufferOffset||
       ...
       (nOffset >= m_nFileSize))
    {
      ...
      if(!OGGStreamCallbakGetData (nOffset, nBytesNeeded, m_pDataCache, m_nOggDataBufferSize, (uint32) m_pUserData) )
      {
        MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,"ParseOGGPage read failed..");
        m_nOggDataBufferFillSize = 0;
        m_nDataCacheFileOffset = 0;
...
bool OGGStreamParser::FindLastValidPage(uint32 trackid, uint64 fileSize, OggPage *pOggPage)
{
  ...
  if(!OGGStreamCallbakGetData (nTempDataCacheFileOffset,
                               nTempBufferFillSize,
                               m_pDataCache,
                               m_nOggDataBufferSize, (uint32) m_pUserData) )
  {
    MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,"ParseOGGPage read failed..");
