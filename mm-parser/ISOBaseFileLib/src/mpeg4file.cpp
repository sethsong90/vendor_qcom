...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/ISOBaseFileLib/main/latest/src/mpeg4file.cpp#148 $
$DateTime: 2014/02/25 20:11:12 $
$Change: 5344662 $
...
bool Mpeg4File::getBufferedDuration(uint32 id, int64 nBytes, uint64 *pBufferedTime)
{
  ...
  else
  {
    *pBufferedTime = 0;
    ...
    if((bHttpStreaming) && (m_pStreamPort) && (MAX_FILE_SIZE == nBytes))
    {
       bool bEndOfData = false;
    ...
    {
      *pBufferedTime = (uint64)(((float)preSampleInfo.time*1000.0F)/p_track->media_timescale);
      returnStatus = true;
 ...
 /* If This track is encrypted with OMA DRM V2 encryption mechanism */
        ...
        ( VIDEO_FMT_STREAM_VIDEO_HEVC == p_track->subinfo.video.format ) ) )
  {
    ...
    while (i < frameBytes)
    {
      ...
      (void) memmove (buf+bytesRead, &startCode, sizeof(startCode));
      bytesRead += sizeof(startCode);
      // Read NALU length.
      ...
      //if (bufferSize - bytesRead < NALUnitLength)
      //return OMM_E_FAILURE;

      // Skip NALU length and copy over NALU contents.
      (void) memmove (buf+bytesRead,
                      &(m_pBuffer [i + j]),
                      NALUnitLength);
...
uint64 Mpeg4File::getMediaTimestampForCurrentSample(uint32 id)
{
  ...
  uint64 time = (((m_sampleInfo[streamNum].time -
              m_baseTimeStamp[p_track->stream_num])* MILLISEC_TIMESCALE_UNIT) /
  ...
  uint64                      currentPosTimeStamp)
{
  ...
  uint64 timescaledTime = (uint64)(((float)TimeStamp/1000.0F)*p_track->media_timescale);
    ...
    */
    ...
    if(!lRewind)
    {
      for( ; frameTime<timescaledTime && retStat; )
      {
        ...
        {
          *newTimeStamp = (uint64)((float)sampleInfo->time * 1000.0F / p_track->media_timescale);
  ...
  *newTimeStamp = (uint64)(((float)sampleInfo->time*1000.0F)/
                           p_track->media_timescale);
  ...
  *bError = FALSE;
  ...
  else
  {
    ...
    *bError = TRUE;
    ...
    float fTime;
    fTime = (float)m_sampleInfo[streamNum].time / p_track->media_timescale *
            (float)TIMESCALE_BASE;
    return (uint64)fTime;
  }
                                                    ...
                                                    file_sample_info_type *pSampleInfo)
{
  ...
  if(pSinfAtom && pSinfAtom->m_pTencAtom)
  {
    ...
    pSampleInfo->sSubInfo.sCP.ucKeyIDSize =
 ...
 MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_HIGH,
  ...
  if(m_sampleInfo[streamNum].aux_sample_info_size >
    pSampleInfo->sSubInfo.sCP.ucInitVectorSize )
  {
    int NALUnitFieldLen = 0, AdjustedNALUnitFieldLength =0 ,
    ...
    NALUnitFieldLen = GetSizeOfNALLengthField(trackid);
    ...
    {
      AdjustedNALUnitFieldLength = 4 - NALUnitFieldLen ;
    }
         ...
         ulSubIndex++)
    {
      ...
      pEncSubsampleInfoData->usSizeOfClearData += AdjustedNALUnitFieldLength;
...
{
  /* Convert duration value into Milli-sec units, as output TS of MP4Parser is always in
     milli-second units. */
  return (((uint64)m_videoFmtInfo.file_info.total_movie_duration * MILLISEC_TIMESCALE_UNIT) /
...
uint64 Mpeg4File::getTrackMediaDuration(uint32 id)
{
  ...
  if ( p_track )
  {
    /* In non-DASH clips, main fragment duration will be provided as duration. But for
       DASH clips, main fragment will not contain media data so duration will always be
       ZERO. Instead total fragment's duration value can be used for the same purpose.
       Milli-second unit is the default Timescale for Timestamp used in 3gp/3g2 clips.
       So converting the values from file's timescale into milli-sec units.*/
    if(p_track->media_duration && !m_bIsDashClip)
    {
      return (uint64)(((uint64(p_track->media_duration) * MILLISEC_TIMESCALE_UNIT) /
                    p_track->media_timescale));
    }
    else
    {
      ...
      uint64 totalDur = context->file_level_data.fragment_file_total_movie_duration;
      uint64 TS = (uint64)(((totalDur * MILLISEC_TIMESCALE_UNIT) /
                         p_track->media_timescale));
      return TS;
    }
  }
...
uint32 Mpeg4File::getTrackMediaTimescale(uint32 id)
{
  ...
  if ( p_track )
  {
    /* In mpeg4file object itself, we are converting time value into mill-sec units.
       So we always return Timescale value as 1000 for mp4/3gp/3g2/DASH clips */
...
PARSER_ERRORTYPE Mpeg4File::getTrackDecoderSpecificInfoContent(uint32 id, uint8* buf, uint32 *pbufSize)
{
           ...
           ( VIDEO_FMT_STREAM_VIDEO_HEVC == p_track->subinfo.video.format ) )
  {
        ...
        ( NULL!= pHEVCInfo->pHVCCInfo->pArrayNALU->pNALU ))
    {
      if( 0 == *pbufSize )
      {
        headersize = pHEVCInfo->pHVCCInfo->ulNaluDataLength;
      }
      else if( NULL != buf)
      {
        uint32 ulArrIdx = 0;
  ...
  if ( p_track )
  {
    ...
    */
    if (p_track->type == VIDEO_FMT_STREAM_AUDIO)
    {
      ...
      case VIDEO_FMT_STREAM_AUDIO_EC3:
        format = EAC3_AUDIO;
        break;
    ...
    if(!p_track->largest_found)
    {
      ...
      */
      if(p_track->type == VIDEO_FMT_STREAM_VIDEO)
      {
        ...
        p_track->largest = FILESOURCE_MAX(p_track->largest,
      ...
      case FS_IndexParamAudioAC3:
        {
          ...
          pAC3Param->ulBitRate = \
            pTrackInfo->subinfo.audio.ec3_params.bit_rate;
          pAC3Param->ucAudioCodingMode = \
            pTrackInfo->subinfo.audio.ec3_params.aBSIInfo[0].acmod;
          pAC3Param->ucBitStreamMode = \
            pTrackInfo->subinfo.audio.ec3_params.aBSIInfo[0].bsmod;

          pAC3Param->usNumChannels = (uint16)\
            ACMOD_CHANNELS[pTrackInfo->subinfo.audio.ec3_params.aBSIInfo[0].acmod];
        }
...
bool Mpeg4File::peekMetaDataSize (uint32 fragment_num)
{
  ...
  case VIDEO_FMT_FRAGMENT_PEEK:
    ...
    if (0 == m_ulMoovBufSize)
    {
      ...
      m_ulMoovBufSize     = FILESOURCE_MIN(m_ulMoovBufSize, m_fileSize);
    }
                                            ...
                                            FileSourceMetaDataType ienumData)
{
  ...
  if((false == bFound) || (PARSER_ErrorNone != eRetType))
  {
                                                 ...
                                                 uint32*  pulDatabufLen)
{
  ...
  */
  ...
  if ((pucDataBuf) && (ulDataSize))
  {
    ...
    if (*pulDatabufLen < (ulDataSize + sizeof(uint32)))
    {
      *pulDatabufLen = ulDataSize + sizeof(uint32);
      return PARSER_ErrorInsufficientBufSize;
    }
    memset(pucDataBuf, 0, ulDataSize + sizeof(uint32));
...
{
  bool bFound = false;
  if((NULL == _metaAtom) || (NULL == dpMetaData))
  {
    return bFound;
  }
                                              ...
                                              ItunesMetaData* pMetaData)
{
  ...
  if((NULL == pulDatabufLen) || (NULL == _metaAtom) || (NULL == pMetaData) ||
     (NULL == pucDataBuf) || (NULL == pMetaData->pucMetaData))
  {
    return PARSER_ErrorInvalidParam;
  }
  ...
  */
  ...
  else if(FILE_SOURCE_MD_ALBUM_ART != ienumData)
  {
