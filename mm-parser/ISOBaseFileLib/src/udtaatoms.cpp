...
: FullAtom(fp)
{
  ...
  if ( _success )
  {
    ...
    if(_albumDataSize)
    {
      _albumData = (uint8*)MM_Malloc(_albumDataSize); /* free in destructure */
      if(_albumData)
      {
...
: FullAtom(fp)
{
  ...
  if ( _success )
  {
    ...
    if(_yrrcDataSize)
    {
      ...
      (void)OSCL_FileSeek(fp, _offsetInFile + 3 * FOURCC_SIGNATURE_BYTES,
                          SEEK_SET);
      _yrrcData = (uint8*)MM_Malloc(_yrrcDataSize); /* free in destructure */
      if(_yrrcData)
      {
...
UdtaMetaAtom::~UdtaMetaAtom()
{
  if(_metaData)
    MM_Free(_metaData);
  for (uint32 ulCount = 0; ulCount < m_uliLstAtomCount; ulCount++)
  {
            ...
            if (pMetaData->pucMetaData)
            {
                  ...
                  (ITUNES_INTEGER == pMetaData->eDataType))
              {
                ...
                snprintf((char*)pMetaData->pucMetaData,
                         ...
                         (uint32)(puciLstDataStartPtr + ulByteConsumed));
#else
      ...
      else if (DATA_TYPE == ulAtomType)
      {
        ...
        */
        ...
        copyByteSwapData((uint8*)&pMetaData->ulLocaleIndicator,
                         sizeof(uint32), pucBufPtr + ulByteConsumed, 1,
                         ulAtomSize);
        ulAtomOffset   += FOURCC_SIGNATURE_BYTES;
        ulByteConsumed += FOURCC_SIGNATURE_BYTES;
