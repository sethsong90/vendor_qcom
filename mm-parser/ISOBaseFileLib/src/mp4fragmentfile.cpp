                                               ...
                                               uint64                     currentPosTimeStampMsec)
{
  ...
  *newTimeStamp = 0;
  ...
  uint64 currentPosTimeStamp = (uint64)(((float)currentPosTimeStampMsec/1000.0F)*p_track->media_timescale);
  if ( skipNumber < 0 )
  {
    iRewind = true;
  }
  ...
  if(iRewind)
  {
    ...
    do
    {
      ...
      if(retStat)
      {
        uint64 tfraTimeMsec = (uint64)((float)tfraEntry.access_point_time * 1000.0F / p_track->media_timescale);
  ...
  uint64                      currentPosTimeStamp)
{
  ...
  uint64 timescaledTime = (uint64)(((double)TimeStamp *
                                    p_track->media_timescale) / 1000.0F);
  bool userRewind;
  uint64 scaledCurPosTime = (uint64)(((double)currentPosTimeStamp *
                                       p_track->media_timescale) / 1000.0F);
  ...
  if( (p_track->type == VIDEO_FMT_STREAM_VIDEO) && bSetToSyncSample)
  {
    ...
    */
    ...
    if(iRewind)
    {
      ...
      uint64 ullSampleTimeinMS = (uint64)((double)frameTime * 1000.0) /
      ...
      for( ; (retStat) && (ullSampleTimeinMS > TimeStamp); )
      {
        ...
        frameTime = sampleInfo->time;
        ullSampleTimeinMS = (uint64)((double)frameTime * 1000.0) /
                             p_track->media_timescale;
                ...
                */
                                      ...
                                      fragmentParsed))
                {
                                                       ...
                                                       1, sampleInfo) )
                  {
                    ...
                    */
                    timescaledTime = (uint64)(((float)(sampleInfo->time)/1000.0F)
                                              * p_track->media_timescale);
                    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
