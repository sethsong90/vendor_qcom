...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/MKAVParserLib/main/latest/src/mkavfile.cpp#45 $
========================================================================== */
...
DESCRIPTION
  Its a callback function from MKAV Parser to read the data.
...
ARGUMENTS
  ...
  ppData                  Pointer to the buffer for filling in the data
                               ...
                               uint32  ulUserData)
{
  ...
  if(ulUserData)
  {
    MKAVFile* pMKAVFile = (MKAVFile*)ulUserData;
...
========================================================================== */
bool MKAVCheckAvailableData(uint64* noffset,bool* bend,uint32 u32UserData)
{
  bool bret = false;
  if(u32UserData)
  {
    MKAVFile* pMKAVFile = (MKAVFile*)u32UserData;
...
/* ======================================================================
FUNCTION:
  MKAVFile::getTrackVideoFrameRate
...
SIDE EFFECTS:
  None.
======================================================================*/
float MKAVFile::getTrackVideoFrameRate(uint32 id)
{
  float frate = 0.0;
  if(m_pMKAVParser)
  {
    frate = m_pMKAVParser->GetVideoFrameRate(id);
  }
  return frate;
}
/* ======================================================================
...
uint16 MKAVFile::GetAudioVirtualPacketSize(int ulTrackId)
{
  ...
  if(m_pMKAVParser)
  {
    ...
    bool bAvailable = m_pMKAVParser->GetAudioTrackProperties((uint32)ulTrackId,
                                                             &AudioInfo);
...
/* ============================================================================
  ...
  @note       None.
============================================================================ */
unsigned long MKAVFile::GetAudioEncoderOptions(int ulTrackId)
{
...
/* ============================================================================
  ...
  @note       None.
============================================================================ */
unsigned long MKAVFile::GetAudioBitsPerSample(int ulTrackId)
{
  unsigned long ulBitsPerSample = 0;
  if(m_pMKAVParser)
  {
    mkav_audio_info AudioInfo;
    memset(&AudioInfo, 0, sizeof(mkav_audio_info));
...
/* ============================================================================
  ...
  @note       None.
============================================================================ */
unsigned long MKAVFile::GetNumAudioChannels(int ulTrackId)
{
...
PARSER_ERRORTYPE MKAVFile::getTrackDecoderSpecificInfoContent(uint32 id, uint8* buf, uint32 *pbufSize)
{
  ...
  uint32 size = m_pMKAVParser->GetCodecHeaderSize(id);
  uint8* header = m_pMKAVParser->GetCodecHeader(id);
...
uint32 MKAVFile::GetAACAudioProfile(uint32 ulTrackId)
{
  ...
  if(m_pMKAVParser)
  {
    if(m_pMKAVParser->GetTrackCodecType(ulTrackId) == MKAV_AAC_AUDIO)
    {
      ...
      {
        uint16 usCodecData = ( ((uint16)phdr[1]) << 8 ) |((uint16)phdr[0]);
        ulProfileData = (usCodecData & 0xF800)>>11;
      }
    }
  }
...
===========================================================================*/
bool MKAVFile::CanPlayTracks(uint64 nTotalPBTime)
{
  return true;
}
#endif//#ifdef FEATURE_FILESOURCE_3GPP_PROGRESSIVE_DNLD
                                           ...
                                           FileSourceMetaDataType ienumData)
{
  ...
  if(pTagInfo)
  {
    ...
    {
      *pulDatabufLen = pTagInfo->ulTagStringLen * sizeof(wchar_t);
    }
    else
    {
      *pulDatabufLen = sizeof(pTagInfo->ulTagBinValue) * sizeof(wchar_t);
    }
  }//if(pTagInfo)
  else
  {
