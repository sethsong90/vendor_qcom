...
/* =======================================================================
                             PERFORCE HEADER
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/MKAVParserLib/main/latest/src/mkavparser.cpp#65 $
$DateTime: 2014/01/30 01:42:42 $
$Change: 5190469 $
========================================================================== */
...
inline void MKV_REVERSE_ENDIAN(uint8* ptr,uint8 size)
{
  uint8 endpos = size-1;
  if(ptr && (size >1))
  {
...
void convertEBMLtoINT(uint8* pBuf, size_t size, int64 *pValue,
                      bool *pbIsNegative)
{
  ...
  {
    *pbIsNegative = true;
    for(uint8 nCount = 0; nCount < size; nCount++)
    {
      //1's Compliment byte after  byte.
      temp[nCount] = ~temp[nCount];
    }
  }
}
                              ...
                              bool   bIsFrameHdrPrsnt = false)
{
           ...
           MKAV_EAC3_AUDIO_CODEC == eCodecType) && (pucFrameBuf))
  {
    ...
    uint8 ucFsCod = ((pucFrameBuf[ulIndex] & 0xC0 ) >> 6 );
    uint8 ucNumBlks =((pucFrameBuf[ulIndex] & 0x30 ) >> 4 );
    if(3 == ucFsCod)
    {
    ...
    while(bContinue)
    {
      switch(m_eParserState)
      {
        case MKAVPARSER_READ_DATA:
        {
          ...
          {
            m_eParserState = eNextParserStatus;
            ulIndex        = 0;
          }
        }
        break;
        ...
        {
          ulEBMLId = EBML_DOC_ID;
          MKV_REVERSE_ENDIAN((uint8*)&ulEBMLId, sizeof(EBML_DOC_ID));
          if(memcmp(m_pDataBuffer, &ulEBMLId, sizeof(EBML_DOC_ID)) == 0)
          {
            #ifdef MKAV_PARSER_DEBUG
            MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
                       "StartParsing Located EBML DOC ID %llu", m_nCurrOffset);
            #endif
            m_nCurrOffset+= sizeof(EBML_DOC_ID);
            ulIndex      += sizeof(EBML_DOC_ID);
            ullAtomSize = AtomSize(m_pDataBuffer + ulIndex, &ucEBMLSizeBytes);
            #ifdef MKAV_PARSER_DEBUG
            MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_HIGH,
              "StartParsing ucEbmlSizeBytes %d size %llu",
              ucEBMLSizeBytes, ullAtomSize);
            #endif
            retError = ParseEBMLDocHeader((m_nCurrOffset+ ucEBMLSizeBytes),
                                           ullAtomSize);
            if(MKAV_API_SUCCESS == retError)
            {
              m_eParserState    = MKAVPARSER_READ_DATA;
              eNextParserStatus = MKAVPARSER_LOCATE_SEGMENT_HDR;
              m_nCurrOffset    += (ullAtomSize + ucEBMLSizeBytes);
            }
            else
            {
        ...
        {
          ulEBMLId = EBML_SEGMENT_ID;
          MKV_REVERSE_ENDIAN((uint8*)&ulEBMLId, sizeof(EBML_SEGMENT_ID));
          if(memcmp(m_pDataBuffer, &ulEBMLId, sizeof(EBML_SEGMENT_ID)) == 0)
          {
            #ifdef MKAV_PARSER_DEBUG
            MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
                     "StartParsing Located Segment Hdr %llu", m_nCurrOffset);
            #endif
            m_nCurrOffset += sizeof(EBML_SEGMENT_ID);
            ulIndex       += sizeof(EBML_SEGMENT_ID);
            ullAtomSize = AtomSize(m_pDataBuffer + ulIndex, &ucEBMLSizeBytes);
            #ifdef MKAV_PARSER_DEBUG
            MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_MEDIUM,
              ...
              ucEBMLSizeBytes , ullAtomSize);
            #endif
            uint64 ullEndOffset = m_nCurrOffset + ucEBMLSizeBytes +ullAtomSize;
            retError = ParseSegmentElement(m_nCurrOffset+ ucEBMLSizeBytes,
                                           ullEndOffset );
            if(MKAV_API_SUCCESS == retError)
            {
...
@param[in] N/A
@return    MKAVPARSER_SUCCESS is parsing is successful otherwise returns appropriate error.
@note      ParseByteStream needs to be called before retrieving any elementary stream specific information.
========================================================================== */
MKAV_API_STATUS MKAVParser::ParseByteStream(void)
{
  MKAV_API_STATUS retError = MKAV_API_FAIL;
  bool bContinue           = true;
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"ParseByteStream");
  if(!m_pDataBuffer)
  {
    m_pDataBuffer = (uint8*)MM_Malloc(DEF_DATA_BUF_SIZE);
  }
        ...
        case MKAVPARSER_READ_DATA:
        {
          ...
          else
          {
            ...
            ullAtomSize   = AtomSize(m_pDataBuffer + ucEBMLIdBytes,
                                     &ucEBMLSizeBytes);
            if(!memcmp(m_pDataBuffer, &EBML_DOC_ID, sizeof(EBML_DOC_ID)) )
            {
...
MKAV_API_STATUS MKAVParser::ParseEBMLDocHeader(uint64 ullOffset,
                                               uint64 ullEBMLSize)
{
  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,"ParseEBMLDocHeader %llu", ullOffset);
    ...
    if(true == bok)
    {
      ...
      else if(bNeedAllocation && ullSize)
      {
        ...
        if(ptemp)
        {
          ...
          ptemp[ullSize] = '\0';
          memcpy(ptemp, m_pDataBuffer + ulIndex, (uint32)ullSize);
          switch(ulMatchedId)
          {
                                                ...
                                                uint64 ullElementSize)
{
  MKAV_API_STATUS status = MKAV_API_FAIL;
  #ifdef MKAV_PARSER_DEBUG
  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,"ParseTracksElement %llu", ullOffset);
  #endif
  uint32 ulIndex       = 0;
  uint32 ulDataRead    = 0;
  uint32 ulBytesToRead = DEF_EBML_MAX_SIGNATURE_LENGTH_VAL;
                                                 ...
                                                 uint8         nhdrSize)
{
  ...
  {
    uint32 ulNewClusterSize = sizeof(cluster_info)*
                                                   ...
                                                   cluster_info* pClusterInfo)
{
  ...
  while( ((ullOffset + ulIndex) < ullEndOffset) && (noerror))
  {
                   ...
                   sizeof(EBML_CLUSTER_BLOCK_ID)) == 0)
    {
      ...
      if( (pBlockInfo->nFlags & EBML_LACING_ID)== EBML_LACING_ID)
      {
        #ifdef MKAV_PARSER_DEBUG
        ...
        //allocate memory to store individual frames size info.
        uint32 *pFramesSize = pBlockInfo->pFramesSize;
        if(pFramesSize)
        {
          MM_Free(pFramesSize);
          pFramesSize = NULL;
        ...
        pFramesSize=(uint32*)MM_Malloc(sizeof(uint32)*pBlockInfo->nFrames);
        pBlockInfo->pFramesSize = pFramesSize;

        if(pFramesSize)
        {
          status = CalcFrameSizes(pBlockInfo->nFrames, pBlockInfo->nFlags,
                                  ullOffset, ulDataRead, ullSize,
                                  ulIndex, pFramesSize);
        }
      }
      ...
      else if( (pBlockInfo->nFlags & XIPH_LACING_ID) == XIPH_LACING_ID)
      {
        #ifdef MKAV_PARSER_DEBUG
                                              ...
                                              cluster_info* pClusterInfo)
{
  ...
  if(noerror)
  {
    ...
    */
    ...
    if( (pSimpleBlock->nFlags & EBML_LACING_ID)== EBML_LACING_ID)
    {
      #ifdef MKAV_PARSER_DEBUG
      MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,"EBML_LACING");
      #endif
      ...
      ulIndex++;
      //allocate memory to store individual frames size info.
      uint32 *pFramesSize = pSimpleBlock->pFramesSize;
      if(pFramesSize)
      {
        MM_Free(pFramesSize);
        pFramesSize = NULL;
      ...
      pFramesSize = (uint32 *)MM_Malloc(sizeof(uint32)* pSimpleBlock->nFrames);
      pSimpleBlock->pFramesSize = pFramesSize;

      if(pFramesSize)
      {
        status = CalcFrameSizes(pSimpleBlock->nFrames, pSimpleBlock->nFlags,
                                ullOffset, ulDataRead, ullElemSize,
                                ulIndex, pFramesSize);
      }
    }
    else if( FIXED_SIZE_LACING_ID ==
    ...
    else if( (pSimpleBlock->nFlags & XIPH_LACING_ID) == XIPH_LACING_ID)
    {
      #ifdef MKAV_PARSER_DEBUG
      MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH, "XIPH_LACING");
      #endif
                                                    ...
                                                    uint64 ullElementSize)
{
  MKAV_API_STATUS status = MKAV_API_FAIL;
  #ifdef MKAV_PARSER_DEBUG
  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,"ParseTrackEntryElement %llu",
               ullOffset);
  #endif
                                     ...
                                     uint8* pBuff,     uint32 ulBufMaxSize)
{
  ...
  uint32 ulUserData = (uint32)m_pUserData;
                                        ...
                                        uint8*  pHEVCInfo)
{
  ...
  /* Codec private data for HEVC contains hvcC atom similar to mp4/3gp.
     ...
     First byte contains version, this value should be ZERO.
    ...
    */
    ...
    for(int i = 0; (i < ucNumParamSets) && (ulBufIndex + 3 <= ulCodecSize); i++)
    {
      ...
      while((ulCount < usNALSets) && (ulBufIndex + 2 <= ulCodecSize))
      {
        ...
        ulCodecConfigBufSize += sizeof(uint32) + usNALUSize;
...
uint32  MKAVParser::GetTrackBufferSize(uint32 id)
{
  ...
  for(int i = 0; (i< m_nstreams) && m_pTrackEntry; i++)
  {
        ...
        (m_pTrackEntry[i].TrackType == MKAV_TRACK_TYPE_VIDEO) )
    {
      if(m_pTrackEntry[i].pVideoInfo)
      {
        if(m_pTrackEntry[i].pVideoInfo->DisplayHeight && m_pTrackEntry[i].pVideoInfo->DisplayWidth)
        {
                                         ...
                                         mkav_stream_sample_info* psampleInfo)
{
  ...
  if((NULL == pucDataBuf) || (NULL == plBytesRead) || (NULL == psampleInfo) ||
     (0 == ulBufSize) )
  {
    return retError;
  }
      ...
      if(bok)
      {
        ...
        do
        {
                  ...
                  (m_pCurrCluster->pBlockGroup))
          {
            ...
            uint32 nCount = pBlockGroup->nBlocksParsed;
            for(; nCount < pBlockGroup->nBlockinfo; nCount++)
            {
        ...
        if(pBlockInfo && pBlockInfo->nTrackNumber == trackId && m_pCurrCluster)
        {
          bIsClusterStart = false;
                                 ...
                                 int nSyncFramesToSkip)
{
  ...
  if((pSampleInfo) && (MKAV_UNKNOWN_CODEC != GetTrackCodecType(ulTrackId)))
  {
            ...
            (m_pAllCuesInfo->pCuePointInfo) )
    {
      ...
      if(bForward || bClosest)
      {
        ...
        for (; (ulCuePoint < m_pAllCuesInfo->nCuePoints) && (!bSeekDone);
             ulCuePoint++)
        {
          /* In LA, frame work expects to seek to closest sync frame.
             Instead of reporting seek failure if there is no sync frame with
             timestamp more than requested seek time, Parser will seek to the
             last entry in CUE table and reports seek success with last entry.
          */
            ...
            )
          {
            ...
            /* Mark the flag as TRUE only if sync frame time is
               more than requested time.
               This explicit loop is required as, above "if" condition is
               disabled in LA case. */
            if((pCuePointInfo[ulCuePoint].nCueTime >= nReposTime) &&
               (MKAV_API_SUCCESS == retError))
            {
              bSeekDone = true;
              MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
                      "MKAV Parser Fwd Seek successful TIME %llu ",
               m_pAllCuesInfo->pCuePointInfo[ulCuePoint-ucPrevEntry].nCueTime);
            }
      ...
      else
      {
        ...
        cue_point_info* pCuePointInfo = m_pAllCuesInfo->pCuePointInfo;
        for (; (ulCuePoint >= 0) && (!bSeekDone); ulCuePoint--)
        {
          if(pCuePointInfo[ulCuePoint].nCueTime <= nReposTime)
          {
            retError = UpdateSeekSampleProperties(ulTrackId, pSampleInfo,
...
SIDE EFFECTS:
  None.
=============================================================================*/
float MKAVParser::GetVideoFrameRate(uint32 id)
{
  float rate = 0.0;
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM,"GetVideoFrameRate");
  return rate;
}
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint8* MKAVParser::GetCodecHeader(uint32 id)
{
          ...
          if(MKAV_AVC1_VIDEO_CODEC == m_pTrackEntry[i].CodecType)
          {
            ...
            if(!m_pTrackEntry[i].pVideoInfo->pCodecHdr)
            {
              ...
              if(pcodec)
              {
                ...
                {
                  memset(pcodec->pSPLengths,0,(pcodec->nSP * sizeof(uint16)));
                  int nTotalLength = 0;
                  pcodec->nOutData += (pcodec->nSP * AVC1_START_CODE_SIZE);
                  for(int j = 0; j < pcodec->nSP;j++)
                  {
                    memcpy(pcodec->pSPLengths+index,(m_pTrackEntry[i].pCodecPvt+noffset),sizeof(uint16));
                    MKV_REVERSE_ENDIAN((uint8*)pcodec->pSPLengths+index,sizeof(uint16));
                    noffset += sizeof(uint16);
                    if(!pcodec->pSPData)
                    {
                      pcodec->pSPData = (uint8*)MM_Malloc(*(pcodec->pSPLengths));
                      if(pcodec->pSPData)
                      {
                        memcpy(pcodec->pSPData,
                               m_pTrackEntry[i].pCodecPvt+noffset,
                               *(pcodec->pSPLengths));
                        nTotalLength += *(pcodec->pSPLengths);
                      }
                    }
                    else
                    {
                ...
                pcodec->pPPLengths = (uint16*)MM_Malloc(pcodec->nPP * sizeof(uint16));
                noffset++;
                index = 0;
                if(pcodec->pPPLengths)
                {
                  int nTotalLength = 0;
                  memset(pcodec->pPPLengths,0,(pcodec->nPP * sizeof(uint16)));
                  pcodec->nOutData += (pcodec->nPP * AVC1_START_CODE_SIZE);
                  for(int j = 0; j < pcodec->nPP;j++)
                  {
                    memcpy(pcodec->pPPLengths+index,(m_pTrackEntry[i].pCodecPvt+noffset),sizeof(uint16));
                    MKV_REVERSE_ENDIAN((uint8*)pcodec->pPPLengths+index,sizeof(uint16));
                    noffset += sizeof(uint16);
                    if(!pcodec->pPPData)
                    {
                      pcodec->pPPData = (uint8*)MM_Malloc(*(pcodec->pPPLengths));
                      if(pcodec->pPPData)
                      {
                        memcpy(pcodec->pPPData,
                               m_pTrackEntry[i].pCodecPvt+noffset,
                               *(pcodec->pPPLengths));
                        nTotalLength += *(pcodec->pPPLengths);
                      }
                    }
                    else
                    {
...
SIDE EFFECTS:
  None.
=============================================================================*/
uint32 MKAVParser::GetCodecHeaderSize(uint32 id)
{
  uint32 nsize = 0;
        ...
        if(ullSize <= (uint64)ulMaxTargetSize)
        {
             ...
             (ulMatchedId == EBML_AUDIO_OUT_SAMPL_FREQ))
          {
            ...
            if(ullSize <= sizeof(float))
            {
              ...
              if(ulMatchedId == EBML_AUDIO_SAMPLING_FREQ)
              {
                pAudioInfo->SamplingFrequency = freqval;
              }
              else
              {
                pAudioInfo->OutputSamplingFrequency = freqval;
              }
            }//if(ullSize <= sizeof(float))
          }
          if(bIsCopy)
  ...
  while( (ullOffset < ullEndOffset)&& (noerror) )
  {
    ...
    else
    {
      ...
      if(EBML_CUE_POINT == m_pDataBuffer[0])
      {
        ...
        {
          uint32 ulAllocSize = sizeof(cue_point_info)*
                                           ...
                                           cue_point_info* pCuePtInfo)
{
  ...
  #endif
  uint32 ulBytesToRead = sizeof(uint32)+1;
  uint32 ulDataRead    = 0;
  uint32 ulIndex       = 0;
                                           ...
                                           cue_track_posn_info* pCueTrkPosInfo)
{
  ...
  while( (ulIndex < ullElementSize)&& (noerror) )
  {
    ...
    if(!memcmp(m_pDataBuffer + ulIndex, &EBML_CUE_BLOCK_NO, sizeof(uint16) ))
    {
      #ifdef MKAV_PARSER_DEBUG
                                             ...
                                             cue_ref_info* pCueRefInfo)
{
  ...
  while( (ulIndex < ullElementSize)&& (noerror) )
  {
    ...
    if(pucMemory)
    {
      memcpy(pucMemory, m_pDataBuffer,
             FILESOURCE_MIN((uint32)ullSize,ulMaxCopySize));
      MKV_REVERSE_ENDIAN(pucMemory,
                        (uint8)FILESOURCE_MIN((uint32)ullSize,ulMaxCopySize));
    }//if(pucMemory)
  ...
  if(m_pSeekHeadInfo && m_pSeekHeadInfo->pSeekHead)
  {
    ...
    */
    ...
    if(m_pAllCuesInfo == NULL && true == bret)
    {
      ...
      if (bCueFound && (m_nCurrOffset < m_nFileSize) && (true == m_bEndOfData))
      {
        bret = false;
        MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
...
/* ============================================================================
  ...
  @note       None.
============================================================================ */
MKAV_API_STATUS MKAVParser::CalcFrameSizes(uint8   ucNumFrames,
                                           uint8   ucFlags,
                                           uint64  ullOffset,
                                           uint32  ulDataRead,
                                           uint64  ullElemSize,
                                           ...
                                           uint32* pulFramesSize)
{
  ...
  while(nCount < ucNumFrames - 1 && pulFramesSize)
  {
...
#ifdef MKAV_PARSER_DEBUG
       MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_HIGH,
                    "simpleblock: EBML_LACING #Framesize[%d] = %lu",
    ...
    else if( (ucFlags & XIPH_LACING_ID) == XIPH_LACING_ID)
    {
      ...
      #ifdef MKAV_PARSER_DEBUG
      MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_HIGH,
                   "simpleblock: XIPH_LACING #Framesize[%d] = %lu",
                   nCount, pulFramesSize[nCount]);
      #endif
      nCount++;
    }
                                                   ...
                                                   uint64 &ullClusterSize)
{
  ...
  do
  {
    ...
    else
    {
      ...
      ucHdrsize = ucEBMLIdBytes + ucEBMLSizeBytes;
...
MKAV_API_STATUS MKAVParser::UpdateCodecDetails( )
{
  ...
  for(int i = 0; (i< m_nstreams) && m_pTrackEntry; i++)
  {
    if((m_pSampleInfo) && (m_pSegmentInfo))
    {
      ...
      m_pSampleInfo[i].nTimeCodeScale = (double)m_pSegmentInfo->nTimeCodeScale;
    }
    else
    {
      MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH,
