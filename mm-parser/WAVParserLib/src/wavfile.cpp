...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/WAVParserLib/main/latest/src/wavfile.cpp#41 $
$DateTime: 2013/07/26 04:23:09 $
$Change: 4168490 $
========================================================================== */
...
void WAVFile::InitData()
{
  ...
  m_pDataBuffer      = NULL;
  m_nDataBufSize     = 0;
  m_eConfigItem      = FILE_SOURCE_MEDIA_DISABLE_PCM_SAMPLE_UPGRADE;
}
...
unsigned long WAVFile::GetAudioBitsPerSample(int)
{
...
//
//INPUT PARAMETERS:
//->nOffset - offset position in file read
//->nNumBytesRequest - bytes needs to be read
//->pData - pointer to buffer
//->nMaxSize - maximum size of read bytes request
//->u32UserData - WAVFile object pointer
//RETURN VALUE
//  No of bytes read
//
//SIDE EFFECTS
...
uint32 WAVCallbakGetData (uint64         nOffset,
                          ...
                          uint32        u32UserData )
{
  ...
  if(u32UserData)
  {
    WAVFile *pWAVFile = (WAVFile *)u32UserData;
    return( ( (nRead = pWAVFile->FileGetData((uint64)nOffset, nNumBytesRequest,
               nMaxSize, pData))== nNumBytesRequest )?nNumBytesRequest:nRead);
  }
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL, "WAVCallbakGetData u32UserData is NULL");
  return 0;
}
...
bool WAVFile::ParseWAVHeader()
{
  ...
  m_pwavformatParser = MM_New_Args(wavformatParser,((uint32)this,m_fileSize,m_WAVFilePtr));
  ...
  if (PARSER_ErrorNone == retStatus)
  {
    *pulBufSize = ulOutDataSize;
    ...
    if(FILE_SOURCE_MEDIA_ENABLE_PCM_SAMPLE_UPGRADE == m_eConfigItem)
    {
      ...
      // Calculate output buffer size based on data read size
      if (WavTechData.bits_per_sample == 24)
      {
        *pulBufSize = (ulOutDataSize * 4) / 3;
        for( ; ulIndex < ulOutDataSize; ulIndex +=3)
        {
          ulSampleData = 0;
          memcpy(&ulSampleData, (m_pDataBuffer + ulIndex), 3);
          ulSampleData = (((uint32)ulSampleData) << 8);
          memcpy((pucDataBuf + ulOPBufSize), &ulSampleData, 4);
          ulOPBufSize += 4;
        }
      }
      else if (WavTechData.bits_per_sample == 8)
      {
        *pulBufSize = ulOutDataSize * 2;
        uint16* pWBuf = (uint16*)pucDataBuf;
        /* Upgrade 8bit signed PCM to 16bit PCM data format.
           Following calculation will convert 8bit data into 16bit signed
           PCM format. */
        while(ulIndex < ulOutDataSize)
        {
          *pWBuf++ = ((uint16)m_pDataBuffer[ulIndex++] - 0x80) * 0x100;
        }
      }
      MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_MEDIUM,
                   "getNextMediaSample 24bit sample data %lu upgraded to %lu",
                   ulOutDataSize, ulOPBufSize);
    }
  }
  else
  {
...
//  None.
//======================================================================
unsigned long WAVFile::GetNumAudioChannels(int /*id*/)
{
...
SIDE EFFECTS:
  None.
======================================================================*/
PARSER_ERRORTYPE WAVFile::getTrackDecoderSpecificInfoContent(uint32 id, uint8* buf,
                                                             uint32 *pbufSize)
{
  if(m_pwavformatParser)
  {
