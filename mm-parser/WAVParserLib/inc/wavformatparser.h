...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/WAVParserLib/main/latest/inc/wavformatparser.h#15 $
...
extern uint32 WAVCallbakGetData (uint64 nOffset,
...
class wavformatParser : public IAudioParser
{
  ...
  public:
    //Constructor
    wavformatParser(uint32 pUData,uint64 fsize,OSCL_FILE *FilePtr);
    //Distructor
  ...
  private:
    ...
    //Read FACT chunk
    PARSER_ERRORTYPE  read_fact_chunk (uint32 *offset,uint32* chunk_size);
    //Read DATA chunk
    PARSER_ERRORTYPE  read_data_chunk (uint32 *offset,uint32* chunk_size);
    //Read PAD chunk
    PARSER_ERRORTYPE  read_pad_chunk (uint32 *offset,uint32* chunk_size);
    //Locate the chunk  types
    PARSER_ERRORTYPE  locate_chunk (const char*,uint32,uint32*,uint32*);
