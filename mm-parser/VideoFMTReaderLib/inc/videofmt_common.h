...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/VideoFMTReaderLib/main/latest/inc/videofmt_common.h#41 $
 ...
 */
typedef struct{
  uint8  fscod;           /* Sampling frequency code   */
  uint8  bsid;            /* Bit stream identification */
  uint8  bsmod;           /* Bit stream mode           */
  uint8  acmod;           /* Audio coding mode         */
  uint8  lfeon;           /* Lower frequency effect ON */
  uint8  num_dep_subs;    /* No# of dependent sub streams that are
                           * are associated with independent sub stream */
  uint16 ch_location;     /* Channel locations         */
}video_fmt_ec3_bsi_info_type;

typedef struct {
  uint16 bit_rate;        /* data-rate in kbit/s       */
  uint8  num_ind_subs;    /* No# of sub-stream present in bit-stream */
  video_fmt_ec3_bsi_info_type aBSIInfo[MAX_NUM_IND_SUBSTREAM]; /* bit-stream information  */
}video_fmt_ec3_params_type;
...
typedef struct {
  ...
  video_fmt_ec3_params_type      ec3_params;
