...
Notice that changes are listed in reverse chronological order.
   ...
   $Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/VideoFMTReaderLib/main/latest/src/videofmt_mp4r_read.c#47 $
   $DateTime: 2013/05/29 21:54:47 $
   $Change: 3839557 $
...
void process_get_data_state (void *server_data)
{
  ...
  */
  ...
  {
    MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_ERROR,"stream_process: get_data overrun!");
    stream->cb_info.get_data.num_bytes = stream->get_data_size;
  }
...
void process_get_sample_info_state (void *server_data, boolean *exit_loop)
{
  ...
  {
    int32 i;
    uint32 aux_sample_count =0,bytes = 0;
    uint64 aux_offset = 0,cur_offset = 0;
    int32 trun_count = 0;
    trun = &stream->trun[stream->current_trun];
    saio = &stream->saio;
    saiz = &stream->saiz;
...
)
{
  ...
  */
  ...
  case VIDEO_FMT_DATA_UNIT_BYTE:
    ...
    stream->read_offset = offset;
    if((TRUE == context->isDashClip) || (stream->main_fragment_bytes && (offset >= stream->main_fragment_bytes)
