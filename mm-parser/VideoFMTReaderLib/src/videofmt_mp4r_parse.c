...
Notice that changes are listed in reverse chronological order.
  ...
  $Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/VideoFMTReaderLib/main/latest/src/videofmt_mp4r_parse.c#92 $
...
static video_fmt_atom_field_type damr_fields[] =
{
  { 5, 2, 0},      /* mode set           */
  { 7, 1, 0},      /* mode change period */
  { 8, 1, 0}       /* frames per sample  */
};
static video_fmt_consume_atom_structure_type damr =
{
...
static video_fmt_consume_atom_structure_type dawp =
{
...
)
{
  ...
  */
  ...
  if(!h264_info->avcc_info)
  {
    alloc_atom.size = sizeof(video_fmt_mp4r_avcc_info);
    context->callback_ptr( VIDEO_FMT_ALLOC,
                           context->client_data,
                          (video_fmt_status_cb_info_type*)&alloc_atom,
                           NULL);
    if(alloc_atom.ptr == NULL)
  ...
  if(h264_info->avcc_info)
  {
    ...
    {
      alloc.size = h264_info->avcc_info->num_seq_param * sizeof(video_fmt_nalu_data_type);
      context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                             (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
      if(alloc.ptr == NULL)
    ...
    {
      alloc.size = h264_info->avcc_info->num_pic_param * sizeof(video_fmt_nalu_data_type);
      context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                             (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
      if(alloc.ptr == NULL)
...
/*Setting the structure variable indicating memory had been allocated*/
    h264_info->vwid_alloc.memory_ptr = alloc_atom.ptr;
  ...
  if(!h264_info->vwid_info)
  {
    alloc_atom.size = sizeof(video_fmt_vwid_info);
    context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                            (video_fmt_status_cb_info_type *) &alloc_atom,
                            NULL );
    if(alloc_atom.ptr == NULL)
  ...
  if(h264_info->vwid_info)
  {
    ...
    if((bytes_consumed+1) <= h264_info->vwid_alloc.allocated_size)
    {
      ...
      h264_info->vwid_info->min_temporal_id = ((temp_value1 & 0x38)>>3);
      h264_info->vwid_info->max_temporal_id = temp_value1 & 0x07;
    }
    ...
    if(h264_info->vwid_info->num_views)
    {
      alloc.size = h264_info->vwid_info->num_views * sizeof(video_fmt_view_info);
      context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                              (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
      if(alloc.ptr == NULL)
      ...
      for(i=0; (i<h264_info->vwid_info->num_views) && (bytes_consumed < atom_stack_top->atom.size); i++)
      {
        ...
        h264_info->vwid_info->p_view_info[i].base_view_type = ((temp_value2 & 0x0C00)>>10);
        h264_info->vwid_info->p_view_info[i].num_ref_views  = temp_value2 & 0x03FF;

        //check if there are additional ref. views pointed by this view
        if(h264_info->vwid_info->p_view_info[i].num_ref_views)
        {
          alloc.size = h264_info->vwid_info->p_view_info[i].num_ref_views * sizeof(uint16);
          context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                              (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
          if(alloc.ptr == NULL)
...
)
{
  ...
  */
  ...
  if(h264_info->mvcc_alloc.memory_allocated == FALSE)
  {
    ...
    /*Setting the structure variable indicating memory had been allocated*/
    h264_info->mvcc_alloc.memory_ptr = alloc_atom.ptr;
  ...
  if(!h264_info->mvcc_info)
  {
    alloc_atom.size = sizeof(video_fmt_mp4r_mvcc_info);
    context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                            (video_fmt_status_cb_info_type *) &alloc_atom,
                            NULL );
    if(alloc_atom.ptr == NULL)
  ...
  if(h264_info->mvcc_info)
  {
    ...
    */
    h264_info->mvcc_info->len_minus_one = value_byte & 0x03;
    h264_info->mvcc_info->complete_representation = (value_byte & 0x80)>>7;
    h264_info->mvcc_info->explicit_au_track = (value_byte & 0x40)>>6;
    ...
    {
      alloc.size = h264_info->mvcc_info->num_seq_param * sizeof(video_fmt_nalu_data_type);
      context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                              (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
      if(alloc.ptr == NULL)
    ...
    {
      alloc.size = h264_info->mvcc_info->num_pic_param * sizeof(video_fmt_nalu_data_type);
      context->callback_ptr ( VIDEO_FMT_ALLOC, context->client_data,
                              (video_fmt_status_cb_info_type *) &alloc,
                              NULL );
      if(alloc.ptr == NULL)
 ...
 )
{
  ...
  if (source == NULL || destination == NULL)
  {
    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_ERROR,
                 ...
                 or context are NULL, context %lu",
                 (uint32)context);
    video_fmt_mp4r_failure(context);
    return 0;
  }
...
)
{
  ...
  case VIDEO_FMT_MP4R_PROCESS_ATOM_SUBSTATE_INIT:
    ...
    memset(&pAudio_info->ec3_params, 0, sizeof(pAudio_info->ec3_params));
    ...
    {
      pStream_info->subinfo.audio.format = VIDEO_FMT_STREAM_AUDIO_EC3;
    }
  ...
  )
{
  ...
  video_fmt_mp4r_atom_pos_type  *pAtom_stack_top;
  video_fmt_stream_info_type    *pStream_info;
  video_fmt_ec3_params_type  *pEC3_info;
  uint32 ulAC3SpecifcData = 0;
  uint8 ucData[4];
  ...
  pEC3_info = &pStream_info->subinfo.audio.ec3_params;
  memset(pEC3_info, 0, sizeof(video_fmt_ec3_params_type));
   ...
   */
  pEC3_info->aBSIInfo[0].fscod = ( ( ucData[2] >> 6 ) & 0x03 );
  //bsid [5 bits ]Bit stream identification
  pEC3_info->aBSIInfo[0].bsid  = ( ( ucData[2] >> 1 ) & 0x1F );
  // bsmod [3 bits ] Bit stream mode
  pEC3_info->aBSIInfo[0].bsmod = ( ( ucData[2] & 0x01) << 2 )| \
                                 ( ( ucData[1] >> 6 ) & 0x03 );
  // acmod [ 3 bits ] Audio coding mode
  pEC3_info->aBSIInfo[0].acmod = ( ( ucData[1] >> 3 ) & 0x07 );
  // update number of channel based on acmod
  pStream_info->subinfo.audio.num_channels = \
    ACMOD_CHANNELS[pEC3_info->aBSIInfo[0].acmod];
  //lfeon [ 1 bits ] Low frequency effects channel ON
  pEC3_info->aBSIInfo[0].lfeon = ( ( ucData[1] >> 2 ) & 0x01 );
  //bit_rate_code [ 5 bits ]
  ucBitRateCode = ( ( ucData[1] & 0x03 ) << 3 ) | ((ucData[0] >> 5 ) & 0x07);
  pEC3_info->bit_rate = AC3_BITRATE_CODE[ucBitRateCode];
  ...
  )
{
  ...
  */
  ...
  ulDataConsumed +=2;
  // data-rate: 13 bits
  pEC3_info->bit_rate = ( ucBitRate >> 3 );
  // number of independent sub stream is sub-stream + 1: 3 bits
  pEC3_info->num_ind_subs = ( ( ucBitRate << 13 ) & 0x07 ) + 1;

  for( nIndex = 0; nIndex < pEC3_info->num_ind_subs ; nIndex++)
  {
    memset(ucData, 0, sizeof(uint32));
    ...
    //fscod: 2bits
      pEC3_info->aBSIInfo[nIndex].fscod = ( ( ucData[2] >> 6 ) & 0x03 );
    //bsid: 5 bits
      pEC3_info->aBSIInfo[nIndex].bsid = ( ( ucData[2] >> 1 ) & 0x1F );
    //bsmod: 5 bits
      pEC3_info->aBSIInfo[nIndex].bsmod = ( ( ucData[2] & 0x01) << 4 )| \
                                     ( ( ucData[1] >> 4 ) & 0xF );
    //acmod: 3 bits
      pEC3_info->aBSIInfo[nIndex].acmod = ( ( ucData[1] >> 1 ) & 0x07 );
    //lfeon: 1 bits
      pEC3_info->aBSIInfo[nIndex].lfeon = ( ucData[1] & 0x01 );
    // update number of channel based on acmod + lfeon information
      pStream_info->subinfo.audio.num_channels = \
        ACMOD_CHANNELS[pEC3_info->aBSIInfo[nIndex].acmod] + \
        (pEC3_info->aBSIInfo[nIndex].lfeon ? 1: 0);
    //number of dependent sub-stream
      pEC3_info->aBSIInfo[nIndex].num_dep_subs = ( ( ucData[0] >> 1 ) & 0xF);


      ulDataConsumed +=3;
      if ( pEC3_info->aBSIInfo[nIndex].num_dep_subs > 0)
      {
        video_fmt_mp4r_consume_data( pContext,
                                     (uint8*)&ucChLocation,
                                     1,
                                     pContext->byte_swap_needed,
                                     VIDEO_FMT_MP4R_IN_BUFFER);
        pEC3_info->aBSIInfo[nIndex].ch_location = ( ucData[0] & 0x01 )<< 8;
        pEC3_info->aBSIInfo[nIndex].ch_location = \
          (pEC3_info->aBSIInfo[nIndex].ch_location | ucChLocation );
        ulDataConsumed += 1;
      }
  ...
  case VIDEO_FMT_MP4R_PROCESS_ATOM_SUBSTATE_INIT:
    ...
    video_fmt_mp4r_consume_data( pContext,
                                (byte *)&usChannelCount,
                                 ...
                                 pContext->byte_swap_needed,
                                 VIDEO_FMT_MP4R_IN_BUFFER );
    pStream_info->subinfo.audio.num_channels = usChannelCount;
...
)
{
  ...
  uint32 ulDTSCodecSpecifcData = 0;
  uint8 ucData[4];
  uint8 ucBitRateCode = 0;
  uint32 ulDataConsumed = 0;
...
)
{
  ...
  */
  ...
  if(pHEVCInfo->sHVCCAtomAlloc.memory_allocated == FALSE)
  {
    ...
    pContext->callback_ptr ( VIDEO_FMT_ALLOC,
                            ...
                            (video_fmt_status_cb_info_type *) &sAllocMem,
                             NULL );
    ...
    /*Setting the structure variable indicating memory had been allocated*/
    pHEVCInfo->sHVCCAtomAlloc.memory_ptr = sAllocMem.ptr;
  ...
  */
  ...
  if( !pHEVCInfo->pHVCCInfo )
  {
    ...
    sAllocMem.size = sizeof(video_fmt_mp4r_hvcc_info);
    pContext->callback_ptr( VIDEO_FMT_ALLOC,
                            pContext->client_data,
                           (video_fmt_status_cb_info_type*)&sAllocMem,
                            NULL);
  ...
  if( pHEVCInfo->pHVCCInfo )
  {
    ...
    if( ( ulBytesConsumed+1 ) <= pHEVCInfo->sHVCCAtomAlloc.allocated_size )
    {
      ulBytesConsumed += video_fmt_consume_data( \
                             ...
                             (byte*)&ucTempData,
                              ...
                              FALSE,
                              pContext);
      pHEVCInfo->pHVCCInfo->ucProfileSpace = (ucTempData & 0xC0 ) >> 6;
      pHEVCInfo->pHVCCInfo->ucTierFlag = (ucTempData & 0x20 ) >> 5;
      pHEVCInfo->pHVCCInfo->ucProfileIDC = (ucTempData & 0x1F);
    ...
    */
    ...
    if( pHEVCInfo->pHVCCInfo->ucNumOfNALArray)
    {
      memset(&sAllocMem, 0, sizeof(sAllocMem));
      sAllocMem.size = \
        pHEVCInfo->pHVCCInfo->ucNumOfNALArray * sizeof(video_fmt_nalu_array_type);
        ...
        if( pHEVCInfo->pHVCCInfo->pArrayNALU[ulArrIdx].usNumNALU )
        {
          ...
          sAllocMem.size = \
            pHEVCInfo->pHVCCInfo->pArrayNALU[ulArrIdx].usNumNALU * \
                        sizeof(video_fmt_nalu_data_type);
  ...
  )
{
  ...
  case VIDEO_FMT_MP4R_PROCESS_ATOM_SUBSTATE_INIT:
    ...
    if( SAMR_TYPE == pAtom_stack_top->atom.type )
    {
      pAudio_Info->format = VIDEO_FMT_STREAM_AUDIO_AMR;
      pAudio_Info->sampling_frequency = 8000;
    }
    else if( SAWB_TYPE == pAtom_stack_top->atom.type )
    {
      pAudio_Info->format = VIDEO_FMT_STREAM_AUDIO_AMR_WB;
      pAudio_Info->sampling_frequency = 16000;
    }
    // Skip whole AMRSampleEntry box as no useful information in it
    video_fmt_mp4r_skip_data (pContext, 28, VIDEO_FMT_MP4R_IN_BUFFER);
    // Process child atom, in case DAMR present.
    pAtom_stack_top->process_substate = \
      VIDEO_FMT_MP4R_PROCESS_ATOM_SUBSTATE_CHILDREN;
    break;
