...
SIDE EFFECTS:
  None.
=============================================================================*/
aviParser::aviParser(avi_uint32 pUData,avi_uint64 fileSize,bool bDiscardAudIndex,bool bDiscardVidIndex,bool bHttpStreaming)
{
  ...
  if(chunkType == AVI_CHUNK_AUDIO)
  {
    if( (GetAudioInfo(ulTrackID,&ainfo) == AVI_SUCCESS) && (ainfo.strhAudio.dwRate) )
    {
      if(GetAudioTrackSummaryInfo(ulTrackID,&audioTrackInfo)== AVI_SUCCESS)
      {
        ...
        {
          avi_uint64 ullAudioBytesFromStart = 0;
          avi_uint32 ulEntriesSkipped = 0;
              ...
              m_base_indx_tbl[ulTrackID].nEntriesInUse ; ulSuperIndex++)
          {
            ...
            {
              float ulPrevFrameTS = 0;
              avi_uint32 ulPrevIndxChunkIndxEntry =0;
                  ...
                  ulStdIndex++ )
              {
                ...
                {
                  ulAudioTS = (( ( (float)ainfo.strhAudio.dwScale/
                                      (float)ainfo.strhAudio.dwRate)
                               * 1000.f) * (float)ulEntriesSkipped );
                }
                else if(audioTrackInfo.audioBytesPerSec)
                {
                  ulAudioTS = ((float)ullAudioBytesFromStart /
                      (float)audioTrackInfo.audioBytesPerSec *
                      1000.0f);
                }
                if((0!= ulPrevFrameTS)
                   && (ullReposTime >= ulPrevFrameTS)
                   && (ulAudioTS >= ullReposTime))
                {
                  // Declaring this pointer only to make code more readable, no other use.
                  avi_indx_chunk_index_entry* pCurrIndxEntry = m_base_indx_tbl[ulTrackID].pIXIndexChunk[ulSuperIndex].pIndxChunkIndexEntry;
                  ...
                  if((bSearchForward) || (!ulSuperIndex && !ulStdIndex))
                  {
                    ...
                    outputEntry->nTimeStamp =  ulAudioTS;
                  ...
                  else
                  {
                    ...
                    outputEntry->nTimeStamp =  ulPrevFrameTS;
                    outputEntry->dwAudFrameCount = ulEntriesSkipped -1;
                  }
                  retError = AVI_SUCCESS;
                    MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,
                        "SeekInSuperIndex matched Audio ts %f",
                        ulAudioTS);
                  break;
                }
                ulPrevFrameTS = ulAudioTS;
            ...
            if(m_base_indx_tbl[ulTrackID].pIXIndexChunk)
            {
                  ...
                  (uint32)m_base_indx_tbl[ulTrackID].pIXIndexChunk[ulSuperIndex].nEntriesInUse)
              {
                    ...
                    ulStdIndex++)
                {
                  ...
                  {
                    ulVideoTS =    ( ( (float)vinfo.strhVideo.dwScale/(float)vinfo.strhVideo.dwRate)*
                                   (float)nTotalChunksJumped*1000.0f);
                    if ( (ullReposTime) < (uint32)ulVideoTS)
                    {
                      bKeyFrameFound = true;
                      numOfChunksToJump = ulStdIndex;
                      // Key Frame found details
                      MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_LOW,
                          "seekInSuperIndex Key Frame offset=%lu",
                          (uint32)
                    ...
                    else
                    {
                      ...
                      outputEntry->nTimeStamp = ulVideoTS;
                ...
                if(bKeyFrameFound)
                {
                  ...
                  retError = AVI_SUCCESS;
                  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,
                      "Forward SeekInSuperIndex matched Video TS:%f",
                      ulVideoTS);
                  break;
                }
                else
                {
                  // We should not update numOfChunksToJump here.
                  // We reach here only when ulStdIndex loop is over and we
                  // did not find a keyframe now we will look for a
                  // key frame in the next IndexChunk so
                  // totalchunksjumped should be updated
            ...
            if(m_base_indx_tbl[ulTrackID].pIXIndexChunk)
            {
                 ...
                 (uint32)m_base_indx_tbl[ulTrackID].pIXIndexChunk[ulSuperIndex].nEntriesInUse)
              {
                    ...
                    ulStdIndex--)
                {
                  ...
                  if(m_base_indx_tbl[ulTrackID].pIXIndexChunk[ulSuperIndex].pIndxChunkIndexEntry[ulStdIndex].bKeyFrame)
                  {
                    ulVideoTS = (((float)vinfo.strhVideo.dwScale/
                ...
                if(bKeyFrameFound)
                {
                  ...
                  retError = AVI_SUCCESS;
                  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,
                      "Backward SeekInSuperIndex matched Video TS:%f",
                      ulVideoTS);
                  break;
                }
                else
                {
                  // We should not update numOfChunksToJump here.
                  // We reach here only when ulStdIndex loop is over
                  // and we did not find a keyframe now we will look
                  // for a key frame in the next IndexChunk so
                  // totalchunksjumped should be updated
                  nTotalChunksJumped -= m_base_indx_tbl[ulTrackID].pIXIndexChunk[ulSuperIndex].nEntriesInUse;
                  continue;
                }
        ...
        case AVI_CHUNK_AUDIO:
            {
              ...
              {
                MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,
                              "Matched Audio index entry TS %f",matchedEntry.nTimeStamp);
        ...
        case AVI_CHUNK_VIDEO:
            {
...
#endif
             {
              ...
              {
                MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM,
                              "Matched Video index entry TS %f",matchedEntry.nTimeStamp);
                m_nCurrOffset = matchedEntry.dwOffset;
                m_nCurrentChunkDataSize = 0;
            ...
            */
                ...
                ((nVideoSyncSkipped == nSyncFramesToSkip)&&(nSyncFramesToSkip > 0))   )
            {
              ...
              */
              ...
              MM_MSG_PRIO2(MM_FILE_OPS, MM_PRIO_LOW,
              "seekInIDX1 Matched Video Entry: dwVidFrameCount %d TS %f",
          ...
          */
              ...
              ((nAudioSyncSkipped == nSyncFramesToSkip)&&(nSyncFramesToSkip > 0))   )
          {
            ...
            */
            ...
            MM_MSG_PRIO3(MM_FILE_OPS, MM_PRIO_LOW,
            "seekInIDX1 Matched Audio Entry: dwAudFrameCount %d TS %f nTotalSizeInBytes %llu",
            outputEntry->dwAudFrameCount,outputEntry->nTimeStamp,audioBytesFromStart);
