 ...
 */
FileSource::FileSource(FileSourceCallbackFuncType callBack, void * pClientData):
m_pHelper(NULL)
{
  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_LOW, "FileSource::FileSource 0x%x", this);
  m_pHelper = MM_New_Args( FileSourceHelper, (callBack,pClientData) );
  MM_Debug_Initialize();
}
                                             ...
                                             bool *pbIsUTF8)
{
  ...
  if(m_pHelper)
  {
    status = m_pHelper->GetClipMetaData(pMetaData,pLength,ienumData, pbIsUTF8);
  }
  return status;
}
/*! ======================================================================
...
FileSourceStatus FileSource::GetFormatBlock(uint32 id,uint8* buf,uint32 *pbufSize)
{
  FileSourceStatus status = FILE_SOURCE_FAIL;
  if(m_pHelper)
  {
    status = m_pHelper->GetFormatBlock(id,buf,pbufSize);
  }
  return status;
}
/*! ======================================================================
...
FileSourceStatus FileSource::SeekAbsolutePosition( const int tAbsoluteTime,
                                                  bool bSeekToSync,
                                                  int64 nCurrPlayTime)
{
  FileSourceStatus status = FILE_SOURCE_FAIL;
