...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/FileSource/main/latest/src/filesourcehelper.cpp#154 $$
$DateTime: 2014/03/25 03:27:54 $
$Change: 5535728 $
 ...
 */
FileSourceHelper::FileSourceHelper(FileSourceHelperCallbackFuncType callBack, void * pClientData) :
m_pIStreamPort(NULL),
m_pIxStream(NULL),
                                             ...
                                             bool     blookforcodecconfig)
{
  ...
  if((m_bEveryThingOK) && (FS_STATE_INIT == m_eState) )
  {
    ...
    /* Post the event to the file open handlder */
...
uint64 FileSourceHelper::GetMediaCurrentPosition(uint32 id)
{
  ...
  if (mediaTimeScale > 0)
  {
    StartTime = (MILLISEC_TIMESCALE_UNIT * mediaTS)/ mediaTimeScale;
  }
  return StartTime;
}
                                                  ...
                                                  FileSourceConfigItemEnum ienumData)
{
       ...
       ( IS_FILESOURCE_STATUS_OK(m_eState) == true ) )
  {
    ...
    case FILE_SOURCE_MEDIA_STRIP_AUDIO_HEADER:
      {
        status = clipFileBase->GetAudioOutputMode((bool*)&pItem->nresult,
                                                  ienumData);
      }
    ...
    case FILE_SOURCE_MEDIA_NUM_DRM_SYSTEM_SUPPORTED:
      {
        status = clipFileBase->GetNumberOfDRMSupported( (uint32*)&pItem->nresult );
      }
...
FileSourceStatus FileSourceHelper::SetConfiguration(uint32 id, FileSourceConfigItem* pItem, FileSourceConfigItemEnum ienumData)
{
  ...
  if( (ienumData == FILE_SOURCE_MEDIA_BASETIME)&& pItem)
  {
    ...
    {
      nRet = clipFileBase->setBaseTime(id,pItem->nresult);
      if(nRet)
      {
        status = FILE_SOURCE_SUCCESS;
      }
  ...
  else if(pItem && clipFileBase )
  {
        ...
        (GetFileFormat(fileFormat) == FILE_SOURCE_SUCCESS))
    {
      ...
      if  (((fileFormat==FILE_SOURCE_AVI)&&((ucAudioType==MP3_AUDIO) ||
          (ucAudioType==AC3_AUDIO)||(ucAudioType==AAC_ADTS_AUDIO))) ||
          ...
          ((fileFormat==FILE_SOURCE_DASH_MP2TS)&&(ucAudioType==AC3_AUDIO)))
      {
        if(!m_pAudioDataBuffer)
        {
          ...
          if(m_pAudioDataBuffer && m_pAudioFileHandle)
          {
  ...
  if(pLength && clipFileBase)
  {
    ...
    switch (ienumData)
    {
      ...
      default:
        ...
        if (pbIsUTF8 && pMetaData)
        {
          *pbIsUTF8 = clipFileBase->isMetaDatainUTF8();
        }
      break;
    }
...
FileSourceStatus FileSourceHelper::GetMediaTrackInfo(uint32 id,MediaTrackInfo* info)
{
  ...
  MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_MEDIUM, "FileSource::getMediaTrackInfo id =%lu",id);
  if(info)
  {
...
uint64  FileSourceHelper::GetTrackMediaDuration(uint32 id)
{
  ...
  if (trackFileBase)
  {
    ...
    if (ulTimescale)
    {
      trackDuration = (MILLISEC_TIMESCALE_UNIT * ullDur) / ulTimescale;
    }
  }
  return trackDuration;
}
...
uint64  FileSourceHelper::GetClipDuration()
{
  ...
  if(m_pAudioFileHandle)
  {
    ...
    if (ulTimeScale)
    {
      ullAudioDur = (ullDuration * MILLISEC_TIMESCALE_UNIT) / ulTimeScale;
    }
  }
  ...
  if(m_pVideoFileHandle)
  {
    ...
    if (ulTimeScale)
    {
      ullVideoDur = (ullDuration * MILLISEC_TIMESCALE_UNIT) / ulTimeScale;
    }
  }
  ...
  if(m_pTextFileHandle)
  {
    ...
    if (ulTimeScale)
    {
      ullTextDur = (ullDuration * MILLISEC_TIMESCALE_UNIT) / ulTimeScale;
    }
  }
...
FileSourceStatus FileSourceHelper::GetFormatBlock(uint32 id,uint8* buf,uint32 *pbufSize)
{
  FileSourceStatus status = FILE_SOURCE_FAIL;
  if(!IS_FILESOURCE_STATUS_OK(m_eState))
  {
...
FileSourceStatus FileSourceHelper::SeekAbsolutePosition(const int tAbsoluteTime,
                                                        bool bSeekToSync,
                                                        int64 nCurrPlayTime)
{
  FileSourceStatus status = FILE_SOURCE_FAIL;
  ...
  else if((!nRet) || (!tAbsoluteTime))
  {
    ...
    status = FILE_SOURCE_SUCCESS;
    /* Post the event to the seek file handlder */
    ...
    if (numTracks)
    {
      ...
      for (uint32 itr = 0; (itr < FILESOURCE_MIN(numTracks,FILE_SOURCE_MAX_NUM_TRACKS)) && (idList); itr++)
      {
        ...
        if( (IS_AUDIO_CODEC(codecType)) && (m_nTrackIdInfoIndexToUse < FILE_SOURCE_MAX_NUM_TRACKS) )
        {
          ...
          uint8 ucDelayString[32];
          uint32 ulSize     = sizeof(ucDelayString);
          uint64 ullDelay   = 0;
    ...
    if (numTracks)
    {
      ...
      for (uint32 itr = 0; (itr < FILESOURCE_MIN(numTracks,FILE_SOURCE_MAX_NUM_TRACKS)) && (idList); itr++)
      {
        ...
        if( (IS_VIDEO_CODEC(codecType)) && (m_nTrackIdInfoIndexToUse < FILE_SOURCE_MAX_NUM_TRACKS) )
        {
          ...
          m_videoTrackInfo[index].timeScale = ullTimeScale;

          // total duration for the track in milliseconds
          if (ullTimeScale)
          {
            uint64 ullDur = m_pVideoFileHandle->getTrackMediaDuration(trackId);
            ullDur = (MILLISEC_TIMESCALE_UNIT * ullDur) / ullTimeScale;
    ...
    if (numTracks)
    {
      ...
      for (uint32 itr = 0; (itr < FILESOURCE_MIN(numTracks,FILE_SOURCE_MAX_NUM_TRACKS)) && (idList); itr++)
      {
        ...
        if( (IS_TEXT_CODEC(codecType)) && (m_nTrackIdInfoIndexToUse < FILE_SOURCE_MAX_NUM_TRACKS) )
        {
          ...
          m_textTrackInfo[index].timeScale = ulTimeScale;
                                                           ...
                                                           uint32 *size, int *numSample)
{
  ...
  if(buf && size && numSample && m_pAudioFileHandle)
  {
    ...
    if((bool)audioconfigItem.nresult)
    {
    ...
    if(GetFileFormat(fileFormat) == FILE_SOURCE_SUCCESS)
    {
          ...
          ((fileFormat == FILE_SOURCE_DASH_MP2TS) && (ucAudioType == AC3_AUDIO))))
      {
        ...
        while(1)
        {
                ...
                (ulFrameBytes > (m_pAudioDataBuffer->nDataSize - m_pAudioDataBuffer->nReadIndex)))
          {
            ...
            *size = m_pAudioFileHandle->getTrackMaxBufferSizeDB(id);
            ...
            if(status == FILE_SOURCE_DATA_OK)
            {
              ...
              if(*size > m_pAudioDataBuffer->nDataBufSize)
              {
              ...
              memcpy(m_pAudioDataBuffer->pDataBuff+m_pAudioDataBuffer->nDataSize,buf,*size);
              m_pAudioDataBuffer->nDataSize += *size;
            }
            else
            {
              MM_MSG_PRIO1(MM_FILE_OPS, MM_PRIO_HIGH,
          ...
          if(ulFrameBytes && (ulFrameBytes <= (m_pAudioDataBuffer->nDataSize - m_pAudioDataBuffer->nReadIndex)))
          {
            break;
          }
        }
        memcpy(buf,m_pAudioDataBuffer->pDataBuff+m_pAudioDataBuffer->nReadIndex,ulFrameBytes);
        *size = ulFrameBytes;
        m_pAudioDataBuffer->nReadIndex += ulFrameBytes;
        m_pAudioDataBuffer->nDataSize -= m_pAudioDataBuffer->nReadIndex;
        memmove(m_pAudioDataBuffer->pDataBuff,m_pAudioDataBuffer->pDataBuff+ m_pAudioDataBuffer->nReadIndex,
          m_pAudioDataBuffer->nDataSize);
        ...
        if(ucAudioType == AAC_ADTS_AUDIO)
        {
          ...
          {
            MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "FileSourceHelper::GetNextAudioFrame \
              AAC_ADTS_AUDIO bStripAudioHeader");
            memmove(buf,buf+AAC_ADTS_HEADER_SIZE,(*size)-AAC_ADTS_HEADER_SIZE);
            *size -= AAC_ADTS_HEADER_SIZE;
          }
        }
      }
      else
                                                  ...
                                                  bool bprocesstimestamp)
{
  ...
  if(pAudBuff && pAudBuff->pDataBuff && m_pAudioFileHandle)
  {
    ...
    {
      for(uint32 i = 0; pAudBuff->nDataSize && (i < (pAudBuff->nDataSize-1)); i++)
      {
        ...
        if(bok)
        {
          ...
          if(ulFrameBytes)
          {
            if(bprocesstimestamp)
            {
              ...
              pAudBuff->nDelta = (MILLISEC_TIMESCALE_UNIT *
    ...
    else if(ucAudioType == AC3_AUDIO)
    {
      for(uint32 i = 0; pAudBuff->nDataSize && (i < (pAudBuff->nDataSize-1)); i++)
      {
        if((pucFramedata[i] == 0x0b)&&(pucFramedata[i+1] == 0x77))
        {
          //next 2 bytes is CRC
          i+= 4;
          uint8 ucFscod =  ( (pucFramedata[i]& 0xC0)>>6);
          uint8 ucFrmsizecod =( pucFramedata[i] & 0x3F);

          uint32 ulBitrate = m_pAudioFileHandle->getTrackAverageBitrate(ulTrackid);
          uint32 ulFreq = AC3_FSCODE_RATE[ucFscod];
          ulFreq = m_pAudioFileHandle->getTrackAudioSamplingFreq(ulTrackid);
          ulFrameBytes = AC3_FRAME_SIZE_CODE[ucFrmsizecod].ulFrameSize[ucFscod] * 2;
          pAudBuff->nFrameTime = (pAudBuff->nDelta+pAudBuff->nBaseTime);
          pAudBuff->nBaseTime += pAudBuff->nDelta;
          if(ulBitrate)
          {
            pAudBuff->nDelta = (float)((ulFrameBytes * 8*1000)/ulBitrate);
          }
          break;
    ...
    else if(ucAudioType == AAC_ADTS_AUDIO)
    {
      ...
      {
        uint16 uData = 0;
        uData = (pucFramedata[(i)+1] << 8) + pucFramedata[(i)+0];
...
uint32 FileSourceHelper::GetAACAudioInfo(uint8* pucBuf, float *pFrameTime)
{
  ...
  {
    ucAudioObject = ((pucBuf[(ulIndex)+2] >> 6) & 0x03)+ 1;
    ...
    ucChannelConfig = ((pucBuf [(ulIndex)+2] << 2) & 0x04)| ((pucBuf [(ulIndex)+3] >> 6) & 0x03);
    ...
    if((ullFrameLength) && (!bError))
    {
      ...
      {
        *pFrameTime = (((float)AAC_SAMPLES_PER_DATA_BLOCK * 1000)/
                                  (float)AAC_SAMPLING_FREQUENCY_TABLE[ucSamplingFreqIndex]);
      }
...
bool FileSourceHelper::FillMP3TechHdr(uint8* buf,void* phdr,uint32 trackid)
{
  ...
  if(m_pAudioFileHandle && phdr && buf)
  {
   ...
   ((buf[MP3HDR_VERSION_OFS] & MP3HDR_VERSION_M) >> MP3HDR_VERSION_SHIFT);
    ...
    ucSampleindex = ((buf[MP3HDR_SAMPLERATE_OFS] &
  ...
  if ( pFileBase )
  {
    ...
    if (returnCode == PARSER_ErrorNone)
    {
      ...
      if(!gotTS)
      {
        ...
        if (ulTimeScale > 0)
        {
          StartTime = (MILLISEC_TIMESCALE_UNIT * ulSampleTime) / ulTimeScale;
          EndTime = StartTime + (MILLISEC_TIMESCALE_UNIT * delta) /ulTimeScale;
        }
      }
...
void FileSourceHelper::HandleOpenFileCommand(void)
{
  ...
  if(m_nAudioTracks || m_nVideoTracks || m_nTextTracks)
  {
    if(m_nAudioTracks)
    {
  ...
  else
  {
    ...
    if( m_fileSourceHelperCallBackFunc && (m_hFileFormatRequested == FILE_SOURCE_RAW))
    {
      MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_HIGH, "FileSource::HandleOpenFileCommand reporting OPEN_COMPLETE");
      FileSourceFileFormat format = FILE_SOURCE_UNKNOWN;
      (void)GetFileFormat(format);
    ...
    else if (m_fileSourceHelperCallBackFunc && (!m_bOpenPending) )
    {
    ...
    *pbError = false;
    if(m_pAudioFileHandle == pFileBase)
    {
      if(m_pAudioDataBuffer)
      {
        ...
        memset(m_pAudioDataBuffer,0,sizeof(audio_data_buffer));
          m_pAudioDataBuffer->nBaseTime = (float)ullReposTime;
        }
    }
  }
  return eRetStatus;
...
PARSER_ERRORTYPE FileSourceHelper::SeekAbsoluteAPI(uint64& ullReposTime)
{
  ...
  // Time to Seek
  ullReposTime = m_nSeekAbsoluteTime;
  ullCurtPosTS = m_currentPosTimeStamp;
  if(m_nPlaybackTime != -1)
  {
    ullCurtPosTS = m_nPlaybackTime;
  }
                                         ...
                                         uint32* pulBufferSize )
{
  ...
  else
  {
    ...
    if(eRetStatus == FILE_SOURCE_SUCCESS)
    {
      ...
      {
        eRetStatus = FILE_SOURCE_FAIL;
        MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "ValidateAACFile failed");
      }
    }
                                                      ...
                                                      uint32* pulBufferSize )
{
  ...
  {
    uint32 ulAtomSize = 0;
    memcpy(&ulAtomSize,(uint8*)pucDataBuffer,sizeof(uint32));
    ulAtomSize = REVERSE_BYTES_ORDER(ulAtomSize);
    if( ulAtomSize > *pulBufferSize )
    {
      *pulBufferSize = ulAtomSize;
      eRetStatus = FILE_SOURCE_DATA_NOTAVAILABLE;
    }
    else if( true == FileBase::Is3GPP2File(NULL,pucDataBuffer,*pulBufferSize,false ))
    {
      eRetStatus = FILE_SOURCE_SUCCESS;
    }
  }
                                                    ...
                                                    uint32* pulBufferSize )
{
  ...
  else if( true == FileBase::IsMPEG2File(NULL,pucDataBuffer, false,
                                         *pulBufferSize, &bIsProgStream ))
  {
    if(bIsProgStream)
    {
      eRetStatus = FILE_SOURCE_SUCCESS;
