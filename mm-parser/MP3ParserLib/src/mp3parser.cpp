...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/MP3ParserLib/main/latest/src/mp3parser.cpp#69 $
$DateTime: 2013/09/03 22:55:24 $
$Change: 4375459 $
...
//=============================================================================
//
mp3Parser::mp3Parser(uint32 pUData, uint64 fsize,OSCL_FILE *FilePtr,
                     bool bHTTPStreaming)
{
...
uint64 mp3Parser::GetClipDurationInMsec()
{
  ...
  if(!m_seek_function_defined)
  {
    ...
    if( NULL != m_pseek)
    {
      uDuration = m_pseek->get_duration();
...
//  None.
//=============================================================================
uint64 mp3Parser::calc_frame_time (const struct tech_data_mp3
                                       &header_info) const
{
   ...
   frame_time = ((MP3_SAMPLES_TABLE
                [(m_header_info.version == MP3_VER_1 ? 0 : 1)]
                [header_info.layer] * (uint64)1000))
                                             ...
                                             bool bUpdateTime)
{
  ...
  while(false == frameStartFound)
  {
    ...
    if(!nBytesRead)
    {
      ...
      nBytesRead = MP3CallbakGetData (m_nCurrOffset, nNumBytesRequest, m_dataBuffer, nMaxBufSize, m_pUserData, bEndOfData);
      nBytesSkipped = 0;
    }
    ...
    if(false == frameStartFound && !std_memcmp("ID3", m_dataBuffer+nFrameBufLen, std_strlen("ID3")))
    {
  ...
  *nBytesNeeded = nFrameBufLen;
  ...
  if( (nBytesRead - nFrameBufLen) >= 3 &&
      !std_memcmp("TAG", m_dataBuffer+nFrameBufLen, std_strlen("TAG")))
  {
    nFrameBufLen += 128;
  }
  ...
  if((nBytesRead - nFrameBufLen) >= 3 &&
     !std_memcmp("ID3", m_dataBuffer+nFrameBufLen, std_strlen("ID3")))
  {
      uint64 prevStartOffset = m_audio_track.start;
      m_audio_track.start = m_nCurrOffset;
      PARSER_ERRORTYPE status = parse_id3(m_MP3FilePtr, m_nFileSize);
...
PARSER_ERRORTYPE mp3Parser::parse_file_header ()
{
   ...
   bytes_read = MP3CallbakGetData(offset, MAX_READ_LENGTH, buffer, MAX_READ_LENGTH,m_pUserData, bEndOfData);
   ...
   while ( (bytes_read > (m_header_size -1)) && (MP3_PARSE_FRAME_COUNT > count) )
   {
      ...
      if (find_mp3_frame_sync(buffer+read_offset, bytes_read,m_header_info))
      {
        ...
        if(m_is_vbr)
         ...
         /* If one complete frame worth of data also is not available, then
            we will return data underrun or eof event without further processing */
         if(!count && bytes_consumed > bytes_read && bytes_consumed < MAX_READ_LENGTH)
         {
           MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_FATAL,"parse_file_header FileRead failed..");
           MM_Free(buffer);
           buffer = NULL;
                                                    ...
                                                    tech_data_mp3 &hdr_info) const
{
  ...
  else
  {
    ...
    ucSampleIndex = ((frame[MP3HDR_SAMPLERATE_OFS] &
  ...
  else
  {
    ...
    else
    {
      ...
      *file_position = 0;
      ...
      if( (!m_is_vbr) && (error == PARSER_ErrorNone) )
      {
        // CBR file
        ...
        if(time >= 1000)
        {
           frame_count = (time / 1000) *
              m_header_info.samplerate /
              MP3_SAMPLES_TABLE[(m_header_info.version == MP3_VER_1 ? 0 : 1)]
                                  [m_header_info.layer];
        }

        /* Fraction of second movement. */
        frame_count += ((time % 1000) * m_header_info.samplerate) /
                       ((uint64)1000 * MP3_SAMPLES_TABLE
                       [(m_header_info.version == MP3_VER_1 ? 0 : 1)]
                       [m_header_info.layer]);

        *file_position = (m_audio_track.start +
                         (calc_frame_length()* frame_count));

      }
      else if( (m_vbr_header) && (error == PARSER_ErrorNone) )
      {
        //VBR file - vbr header is avilable
  ...
  *SeekedTime   = 0;
  ...
  while(ElapsedTime <= RequestedTime)
  {
    ...
    if(!nBytesRead)
    {
      ...
      nBytesRead = MP3CallbakGetData (CurrentOffset, nNumBytesRequest, dataBuffer, nMaxBufSize, m_pUserData, bEndOfData);
      nBytesSkipped = 0;
      FrameBufLen = 0;
    }
