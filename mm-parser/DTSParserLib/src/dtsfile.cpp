...
$Header: //source/qcom/qct/multimedia2/Video/Source/FileDemux/DTSParserLib/main/latest/src/dtsfile.cpp#10 $
$DateTime: 2013/07/18 01:16:25 $
$Change: 4118601 $
...
PARSER_ERRORTYPE cDTSFile::ParseFrameHeaderDTS(uint8* pucFrameBuff)
{
  ...
  if ( NULL != pucFrameBuff)
  {
    ...
    uint8 ucNumBlks = 1 + (uint8)
       GetBitsFromBuffer(7, ulCurBitOffset, pucFrameBuff, DTS_AUDIO_BUF_SIZE);
    ulCurBitOffset += 7;
    m_SyncFrameInfo.usSamplesperFrame = ucNumBlks *
    ...
    m_SyncFrameInfo.usFrameSize = 1 + (uint16)
...
PARSER_ERRORTYPE cDTSFile::ParseFrameHeaderDTSSubStream(uint8* pucFrameBuff)
{
  ...
  */
  ...
  if ( NULL != pucFrameBuff)
  {
    ...
    uint8 nExtSSIndex = (uint8)GetBitsFromBuffer(2, ulCurrOffset, pucFrameBuff,
      DTS_AUDIO_BUF_SIZE);
    ulCurrOffset += 2;
                                             ...
                                             uint32 &rulIndex)
{
  ...
  {
    uint32 ulTotalDataRead    = 0;
    uint32 ulPaddingBytes     = 0;
    /* Check whether Frame size is 4byte aligned or not. */
    if ((ulFrameSize % FOURCC_SIGNATURE_BYTES) != 0)
    {
      ulPaddingBytes = FOURCC_SIGNATURE_BYTES -
                       (ulFrameSize % FOURCC_SIGNATURE_BYTES);
    }

    /* Check whether frame is 4byte aligned or not. If it is not 4byte aligned,
       then Parser will add padding at end of each frame. */
    if (ulPaddingBytes == 0)
    {
      ulOutDataSize = FileGetData(m_nOffset, *pulBufSize,
                                  *pulBufSize, pucDataBuf);
      /* Give data in multiples of frame size */
      ulNumFrames      = ulOutDataSize / ulFrameSize;
      ulOutDataSize   = ulNumFrames * ulFrameSize;
      ulTotalDataRead = ulOutDataSize;
    }
    else
    {
      /* Ensure Frame size worth of buffer is available. */
      while(*pulBufSize >
            (ulOutDataSize + ulFrameSize + ulPaddingBytes))
      {
        uint32 ulDataRead = FileGetData(ullOffset, ulFrameSize,
                                        *pulBufSize, pucDataBuf);
        ulOutDataSize   += ulDataRead;
        ulTotalDataRead += ulDataRead;
        ullOffset += ulDataRead;
        ulNumFrames++; //Counter for Number of Frames
        //If read failed, break the loop!!
        if (ulDataRead < ulFrameSize)
        {
          break;
        }
        //Add padding bytes at end of each frame
        memset(pucDataBuf + ulOutDataSize, 0, ulPaddingBytes);
        ulOutDataSize += ulPaddingBytes;
      }
    }
    m_nOffset += ulTotalDataRead;
  }
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 cDTSFile::getMediaTimestampForCurrentSample(uint32 id)
{
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM,
              "cDTSFile::getMediaTimestampForCurrentSample");
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 cDTSFile::getMovieDuration() const
{
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "cDTSFile::getMovieDuration");
  return m_totalDuration;
}

/* ======================================================================
FUNCTION:
  cDTSFile::getMovieTimescale
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 cDTSFile::getMovieTimescale() const
{
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "cDTSFile::getMovieTimescale");
...
SIDE EFFECTS:
  None.
======================================================================*/
unsigned long cDTSFile::GetNumAudioChannels(int id)
{
...
/* ======================================================================
FUNCTION:
  cDTSFile::getTrackMediaDuration
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 cDTSFile::getTrackMediaDuration(uint32 id)
{
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "cDTSFile::getTrackMediaDuration");
  return m_totalDuration;
}

/* ======================================================================
FUNCTION:
  cDTSFile::getTrackMediaTimescale
...
SIDE EFFECTS:
  None.
======================================================================*/
uint32 cDTSFile::getTrackMediaTimescale(uint32 id)
{
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "cDTSFile::getTrackMediaTimescale");
  return MILLISEC_TIMESCALE_UNIT;
}
                                         ...
                                         bool bIsLittleEndian )
{
  ...
  for( nCount = 0; nCount < ulBufLength; nCount++  )
  {
    ...
    {
      uint16 usBitsToRead = FILESOURCE_MIN( CHAR_SIZE - usConsumedBits,
                                            usAvailBits );

      // Make way to keep required number of bits in ucPresentChar
      ucPresentChar <<= usBitsToRead;
      // Read required num of bits from Tempchar variable
      ucPresentChar |= ( ucTempChar >> (usAvailBits - usBitsToRead) );
      // Delete the bits that are taken into PresentChar value
      ucTempChar = ucTempChar & (0xFF >>
                                 (CHAR_SIZE - usAvailBits + usBitsToRead));
      //Update In and Out Bits based on number of  bits read.
      usAvailBits -= usBitsToRead;
      usConsumedBits += usBitsToRead;
    }
    ...
    usConsumedBits += usAvailBits;
    ucPresentChar <<= usAvailBits;
    ucPresentChar |= ucTempChar;
  }
  return;
}
...
PARSER_ERRORTYPE cDTSFile::ParseRev2AuxData(uint8* pucFrameBuff)
{
  ...
  m_SyncFrameInfo.nRev2AUXDataByteSize = 1 + (uint8)
     GetBitsFromBuffer(7, ulCurBitOffset, pucFrameBuff, DTS_AUDIO_BUF_SIZE);
  ulCurBitOffset += 7;
