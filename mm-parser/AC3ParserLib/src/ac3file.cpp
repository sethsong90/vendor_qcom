...
uint32 AC3File::getFrameSize(uint8 *pucFrameBuff)
{
  ...
  else if(bFrameMarker)
  {
    ...
    //Frame Size will not include header word. Parser need to add it explicitly
    ulFrameSize += 1;
  }
    ...
    if ((1 == ucStrType) && (true == m_bIsAC3File) )
    {
      ...
      if (ucChnMap)
      {
        ...
        uint16 usExtChnMap = (uint16)GetBitsFromBuffer(16, usOffset,
                                                       ucBuf, ulSize);
        m_SyncFrameInfo.usNumChannels += GetExtraChannelCount(usExtChnMap);
        usOffset += 16;
      }
    }
                                             ...
                                             uint32 &rulIndex)
{
  ...
  uint64 duration = 0;
  MM_MSG_PRIO(MM_FILE_OPS, MM_PRIO_MEDIUM, "AC3File::getNextMediaSample");
    ...
    *pulBufSize = 0;
        ...
        nOutDataSize > AC3_HEADER_SIZE; count++)
    {
      ...
      if(!frameSize && !*pulBufSize)
      {
        ...
        {
          nBytesSkipped++;
          if(!frameSize && nBytesSkipped == nOutDataSize)
          {
...
SIDE EFFECTS:
  None.
======================================================================*/
uint64 AC3File::resetPlayback(  uint64 repos_time, uint32 id, bool bSetToSyncSample,
...
SIDE EFFECTS:
  None.
======================================================================*/
unsigned long AC3File::GetNumAudioChannels(int id)
{
