...
int init_transport_bdroid(boolean on) {
    ...
    if (on) {
        ...
        /*TODO: Get the BD address from persists */
        /*st = get_bdaddress(bdaddr);

        if (st != STATUS_SUCCESS) {
          ALOGE("Failed : get_bdaddress() ");
        }*/
...
/*
request_status get_bdaddress(unsigned char* bd_addr) {
    static nv_persist_item_type my_nv_item;
    static nv_persist_stat_enum_type cmd_result;
    request_status st = STATUS_SUCCESS;
    ...
    if (NV_SUCCESS  != cmd_result)
    {
          ...
          st = STATUS_NO_RESOURCES;
          // Might need to add logic to randomize it for FTM
    }
    else
    {
