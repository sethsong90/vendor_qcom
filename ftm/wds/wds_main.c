...
int main(int argc, char *argv[])
{
  ...
  fd_transport_UART = init_uart(uart_tty_interface);
  ...
  do
  {
    ...
    if (FD_ISSET(fd_transport_UART,&readfds)) {
      nbytes = 0;
      ntotal = 0;
      do
      {
        nbytes = read(fd_transport_UART, (char *)(&(bufArr[ntotal])), 1);
        ntotal += nbytes;
