...
static boolean mct_pipeline_populate_query_cap_buffer(mct_pipeline_t *pipeline)
{
  ...
  memcpy(&hal_data->hdr_bracketing_setting, &local_data->imaging_cap.hdr_bracketing_setting,
  ...
  if( local_data->sensor_cap.sensor_format != FORMAT_BAYER) {
    hal_data->min_required_pp_mask &= ~CAM_QCOM_FEATURE_SHARPNESS;
    hal_data->min_required_pp_mask &= ~CAM_QCOM_FEATURE_EFFECT;
  }
