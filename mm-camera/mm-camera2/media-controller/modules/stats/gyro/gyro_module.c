  ...
  unsigned int session_id)
{
  ...
  if (MCT_OBJECT_REFCOUNT(module) == 0) {
    CDBG("%s: Init DSPS", __func__);
    if (dsps_proc_init() == 0) {
      dsps_set_data_t dsps_set_data;

      dsps_set_data.msg_type = DSPS_ENABLE_REQ;
      dsps_set_data.sensor_feature.mode = ENABLE_ALL;
      if (dsps_proc_set_params(&dsps_set_data) != 0) {
        CDBG("%s: dsps enable req failed", __func__);
        goto dsps_enable_error;
      }
    }
    else {
      CDBG("%s: dsps_proc_init() failed", __func__);
      goto dsps_init_error;
    }
  }
