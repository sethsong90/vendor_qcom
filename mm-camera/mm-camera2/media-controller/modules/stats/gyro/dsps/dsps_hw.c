...
void dsps_cirq_add(void *config, void *data, uint8_t seqnum)
{
  ...
  //dump_time(__FUNCTION__);
  CDBG("valid1 = %d, valid2 = %d, sample_len1 = %u, sample_len2 = %u, seqnum %d",
       sensor_data->frame_info_valid, sensor_data->frame_info2_valid,
       sensor_data->frame_info.sample_len, sensor_data->frame_info2.sample_len, seqnum);

  idx = 0;
  if (sensor_data->frame_info_valid)
  {
    dsps_config->queue[dsps_config->q_pos].data.sample_len =
      sensor_data->frame_info.sample_len;

    for (i = 0; i < sensor_data->frame_info.sample_len; i++)
    {
      dsps_config->queue[dsps_config->q_pos].data.sample[idx] = sensor_data->frame_info.sample[i];
      idx++;
    }

    if (sensor_data->frame_info2_valid)
    {
      for (i = 0; i < sensor_data->frame_info2.sample_len; i++)
      {
        dsps_config->queue[dsps_config->q_pos].data.sample[idx] = sensor_data->frame_info2.sample[i];
        idx++;
      }
      dsps_config->queue[dsps_config->q_pos].data.sample_len +=
        sensor_data->frame_info2.sample_len;
    }
  }
...
int dsps_cirq_search(void *config, void *data, uint8_t id)
{
  ...
  //dump_time(__FUNCTION__);
  //LOGE("Searching for seqnum %d", id);
  ...
  switch (msg_type) {
    ...
    case SENSOR1_MSG_TYPE_IND:
      if (msg_hdr->service_number == SNS_SMGR_SVC_ID_V01) {
        ...
        CDBG_DSPS("DSPS Indication Received\n");
...
void dump_time(const char *id)
{
  ...
  CDBG_ERROR("%s, %s, time, %llu, (ms)", __FUNCTION__,id,
