...
static chromatix_parms_type chromatix_test_parms = {
#include "libchromatix/chromatix_s5k3l1yx/snapshot/chromatix_s5k3l1yx_snapshot.h"
};

static const uint8_t g_gamma_table[1024] =
{
...
static const uint8_t g_low_gamma_table[1024] =
{
...
/**
 * Function: denoise_test_event_handler
 *
 ...
 * Notes: none
 **/
int denoise_test_event_handler(void* p_appdata, img_event_t *p_event)
{
  denoise_test_t *p_test = (denoise_test_t *)p_appdata;
 ...
 * Notes: none
 **/
int denoise_test_init(denoise_test_t *p_test)
{
  int rc = IMG_SUCCESS;
  img_param_type param;
  p_test->base->p_comp = &p_test->base->comp;
  p_test->base->p_core_ops = &p_test->base->core_ops;
  p_test->info_3a.aec_real_gain = 2.0;
  p_test->info_3a.lux_idx = 150;
  p_test->info_3a.wb_g_gain = 1.0;
  ...
  rc = IMG_COMP_INIT(p_test->base->p_comp, (void *)p_test,
    (void *)&p_test->mode);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  ...
  rc = IMG_COMP_SET_CB(p_test->base->p_comp, denoise_test_event_handler);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  }

  rc = IMG_COMP_SET_PARAM(p_test->base->p_comp, QWD_3A_INFO,
    (void *)&p_test->info_3a);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  ...
  rc = IMG_COMP_SET_PARAM(p_test->base->p_comp, QWD_CHROMATIX,
    (void *)&chromatix_test_parms);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  }
  ...
  SUBSAMPLE_TABLE(g_gamma_table,
    1024,
    p_test->gamma.table,
    64,
    0);
  rc = IMG_COMP_SET_PARAM(p_test->base->p_comp, QWD_GAMMA_TABLE,
    (void *)&p_test->gamma);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  ...
  /*subsample gamma table*/
  SUBSAMPLE_TABLE(g_low_gamma_table,
    1024,
    p_test->low_gamma.table,
    64,
    0);
  rc = IMG_COMP_SET_PARAM(p_test->base->p_comp, QWD_LOW_GAMMA_TABLE,
    (void *)&p_test->low_gamma);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] rc %d", __func__, __LINE__, rc);
    return rc;
  }
 ...
 * Notes: none
 **/
int denoise_test_start(denoise_test_t *p_test)
{
  int i = 0;
  int rc = IMG_SUCCESS;
 ...
 * Notes: none
 **/
int denoise_test_stop(denoise_test_t *p_test)
{
  return IMG_SUCCESS;
}

/**
 * Function: denoise_test_deinit
 *
 ...
 * Notes: none
 **/
int denoise_test_deinit(denoise_test_t *p_test)
{
  int rc = IMG_SUCCESS;
 ...
 * Notes: none
 **/
int denoise_test_finish(denoise_test_t *p_test)
{
  int rc = IMG_SUCCESS;
...
int denoise_test_execute(denoise_test_t *p_test)
{
  ...
  rc = denoise_test_init(p_test);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] ", __func__, __LINE__);
    return rc;
  }

  rc = denoise_test_start(p_test);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] ", __func__, __LINE__);
    return rc;
  }

  rc = denoise_test_finish(p_test);
  if (rc != IMG_SUCCESS) {
    IDBG_ERROR("%s:%d] ", __func__, __LINE__);
    return rc;
  }
