...
void module_faceproc_client_send_roi_event(faceproc_client_t *p_client,
  faceproc_result_t *p_result, uint32_t frame_id)
{
  ...
  uint32_t frame_id)
{
  ...
  module_faceproc_client_send_roi_event(p_client, p_out_result, frame_id);
  pthread_mutex_unlock(&p_client->result_mutex);
}
  ...
  uint32_t buf_idx, uint32_t frame_id)
{
  ...
  rc = IMG_COMP_Q_BUF(p_comp, p_frame, IMG_TYPE_MAIN);
  if (IMG_ERROR(rc)) {
