  ...
  float ratio)
{
  ...
  for (i=0; i<16; i++) {
    pv_out->g_table.table_pos[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_green[0] * pv1->table_pos[i],
      pv2->scale_factor_green[0] * pv2->table_pos[i], ratio));
    pv_out->b_table.table_pos[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_blue[0] * pv1->table_pos[i],
      pv2->scale_factor_blue[0] * pv2->table_pos[i], ratio));
    pv_out->r_table.table_pos[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_red[0] * pv1->table_pos[i],
      pv2->scale_factor_red[0] * pv2->table_pos[i], ratio));
  }

  for (i=0; i<8; i++) {
    pv_out->g_table.table_neg[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_green[1] * pv1->table_neg[i],
      pv2->scale_factor_green[1] * pv2->table_neg[i], ratio));
    pv_out->b_table.table_neg[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_blue[1] * pv1->table_neg[i],
      pv2->scale_factor_blue[1] * pv2->table_neg[i], ratio));
    pv_out->r_table.table_neg[i] =
      Round(LINEAR_INTERPOLATION(pv1->scale_factor_red[1] * pv1->table_neg[i],
 ...
 *
 **/
isp_ops_t *abf40_open(uint32_t version)
{
