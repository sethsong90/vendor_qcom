...
static void vfe_stats_dump(void *vaddr, uint32_t len, int msg_id)
{
  ...
  case MSG_ID_STATS_BG:
    snprintf(bufp, 128, "/data/bg_%d.yuv", bg_cnt++);
    if(bg_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BE:
    snprintf(bufp, 128, "/data/be_%d.yuv", be_cnt++);
    if(be_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BF:
    snprintf(bufp, 128, "/data/bf_%d.yuv", bf_cnt++);
    if(bf_cnt > 50) {
      return;
    }
    break;
  case MSG_ID_STATS_BHIST:
    snprintf(bufp, 128, "/data/bhist_%d.yuv", bhist_cnt++);
    if(bhist_cnt > 50) {
      return;
    }
    break;
