 ...
 **/
static int af_stats_set_params (void *ctrl, uint32_t param_id,
 ...
 *  Return 0 for Success, negative if ERROR
 **/
static int af_stats_get_params (void *ctrl, uint32_t param_id,
  void *in_params, uint32_t in_param_size,
 ...
 *  Return 0 for Success, negative if ERROR
 **/
static int af_stats_action (void *ctrl, uint32_t action_code,
 ...
 *
 *  Set initial values in the af stats entry
 *
 *
 *  Return 0 for Success, negative if ERROR
 **/
static int af_stats_init (void *ctrl, void *in_params, isp_notify_ops_t *notify_ops)
{
  isp_stats_entry_t *entry = ctrl;
...
/** af_stats_destroy:
 *    @ctrl: Pointer to the entry struct for af stats
 *
 *  Free the memory for the af stats module
 *
 *
 * Return 0 on Success, negative on ERROR
 **/
static int af_stats_destroy (void *ctrl)
{
  isp_stats_entry_t *entry = ctrl;
  free(entry->reg_cmd);
  free(entry);
 ...
 *
 *  Allocate and Initialize memory for af stats module
 *
 *
 *  Return pointer to isp ops. NULL if ERROR
 **/
isp_ops_t *af_stats44_open(isp_stats_mod_t *stats,
  enum msm_isp_stats_type stats_type)
{
  int rc = 0;
  isp_stats_entry_t *entry = NULL;
  ISP_StatsAf_CfgCmdType *cmd = NULL;

  entry = malloc(sizeof(isp_stats_entry_t));
  if (!entry) {
    CDBG_ERROR("%s: no mem for af\n",  __func__);
    return NULL;
  }
  cmd = malloc(sizeof(ISP_StatsAf_CfgCmdType));
  if (!cmd) {
    CDBG_ERROR("%s: no mem\n",  __func__);
    free(entry);
    return NULL;
  ...
  entry->ops.init = af_stats_init;
  /* destroy the module object */
  entry->ops.destroy = af_stats_destroy;
  /* set parameter */
  entry->ops.set_params = af_stats_set_params;
  /* get parameter */
  entry->ops.get_params = af_stats_get_params;
  entry->ops.action = af_stats_action;
  return &entry->ops;
}
