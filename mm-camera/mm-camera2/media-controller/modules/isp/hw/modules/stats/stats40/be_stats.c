  ...
  uint32_t in_param_size)
{
  ...
  pcmd->rgnWidth   = FLOOR2(camif_window_w_t/BE_GRID_H) - 1;
  pcmd->rgnHeight  = FLOOR2(camif_window_h_t/BE_GRID_V) - 1;
  pcmd->rgnHOffset = (camif_window_w_t - BE_GRID_H * (pcmd->rgnWidth + 1))/2;
  pcmd->rgnVOffset = (camif_window_h_t - BE_GRID_V * (pcmd->rgnHeight + 1))/2;
  pcmd->rgnHNum    = BE_GRID_H - 1;
  pcmd->rgnVNum    = BE_GRID_V - 1;
  pcmd->rMax       = 255 - PIX_VALUE_OFFSET;
  pcmd->grMax      = 255 - PIX_VALUE_OFFSET;
  pcmd->bMax       = 255 - PIX_VALUE_OFFSET;
  pcmd->gbMax      = 255 - PIX_VALUE_OFFSET;
  ...
  if (pix_settings->tintless_data->is_supported &&
    ...
    stats_cfg.stat_elem_w = FLOOR2(stats_cfg.camif_win_w / BE_GRID_H);
    stats_cfg.stat_elem_h = FLOOR2(stats_cfg.camif_win_h / BE_GRID_V);
    stats_cfg.num_stat_elem_cols = BE_GRID_H;
    stats_cfg.num_stat_elem_rows = BE_GRID_V;
    stats_cfg.stats_type = STATS_TYPE_BE;
    ...
    rc = entry->notify_ops->notify(entry->notify_ops->parent,
           entry->notify_ops->handle, ISP_HW_MOD_NOTIFY_BE_CONFIG,
