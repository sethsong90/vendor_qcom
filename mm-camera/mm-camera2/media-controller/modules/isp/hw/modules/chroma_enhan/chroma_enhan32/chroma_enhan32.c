  ...
  chromatix_color_conversion_type *cv_data)
{
  ...
  case AWB_CCT_TYPE_TL84_A: {
    ratio = GET_INTERPOLATION_RATIO(trigger_info.mired_color_temp,
              trigger_info.trigger_A.mired_start,
              trigger_info.trigger_A.mired_end);
    util_color_conversion_interpolate(cv_table_TL84, cv_table_A, cv_data, ratio);
  }
    break;
  ...
  case AWB_CCT_TYPE_D65_TL84: {
    ...
    util_color_conversion_interpolate(cv_table_D65,cv_table_TL84, cv_data, ratio);
  }
    break;
  ...
  isp_hw_pix_setting_params_t *in_params, uint32_t in_param_size)
{
  ...
  CDBG("%s: wb %d", __func__, in_params->wb_mode);
