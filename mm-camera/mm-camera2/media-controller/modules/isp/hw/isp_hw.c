...
void isp_hw_send_info_to_metadata(isp_hw_t *isp_hw)
{
  ...
  /* Send gamma tables */
  CDBG("%s:%d] gamma[0] %x", __func__, __LINE__, (unsigned int)&p_gamma[0]);
    ...
    case ISP_EVENT_SOF:{
      ...
      if (GET_ISP_MAIN_VERSION(isp_hw->init_params.isp_version)
        ...
        if (metadump_enable) {
           ...
           if (rc < 0)
             CDBG_ERROR("%s: prepare meta entry error! rc = %d\n", __func__, rc);
        }
      }
  ...
  isp_hw_cap_t *cap, int id)
{
  ...
  case ISP_MSM8974_V2: {
    cmd2.cfg_cmd = (void *)&reg_cfg_cmd;
    cmd2.cfg_data = (void *)&soc_hw_ver;
    cmd2.cmd_len = sizeof(soc_hw_ver);
    cmd2.num_cfg = 1;

    reg_cfg_cmd.cmd_type = GET_SOC_HW_VER;
    ...
    soc_hw_ver_minor = (soc_hw_ver & 0x000000FF);

    if (soc_hw_ver_minor == 2) {
      cap->isp_info.max_pix_clk = ISP_PIX_CLK_400MHZ;
    } else {
      cap->isp_info.max_pix_clk = ISP_PIX_CLK_320MHZ;
    }
  }
    break;
  case ISP_MSM8974_V3: {
    cap->isp_info.max_pix_clk = ISP_PIX_CLK_450MHZ;
  }
    break;
  default: {
    cap->isp_info.max_pix_clk = ISP_PIX_CLK_320MHZ;
