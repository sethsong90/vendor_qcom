...
{
  isp_flash_params_t *flash_params = &pipeline->pix_params.cfg_and_3a_params.cfg.flash_params;
  aec_proc_flash_sensitivity_t *flash_sensitivity =
    (aec_proc_flash_sensitivity_t *)(((stats_update_t *) saved_params)->aec_update.flash_sensitivity);
  pipeline->pix_params.cfg_and_3a_params.trigger_input.stats_update.aec_update =
    ((stats_update_t *) saved_params)->aec_update;
  if (((stats_update_t *) saved_params)->aec_update.est_state == AEC_EST_START ||
    ((stats_update_t *) saved_params)->aec_update.est_state == AEC_EST_DONE ||
    ((stats_update_t *) saved_params)->aec_update.est_state == AEC_EST_DONE_FOR_AF ||
    ((stats_update_t *) saved_params)->aec_update.est_state == AEC_EST_DONE_SKIP) {
 ...
 *  Return 0 on Success, negative on ERROR
 **/
static int isp_pipeline_set_besthot(isp_pipeline_t *pipeline, int32_t *bestshot,
  uint32_t in_params_size)
{
  int i, num, rc = 0;
  uint16_t *module_ids = NULL;
  ...
  void *in_params, uint32_t in_params_size)
{
  ...
  case ISP_PIX_SET_BESTSHOT: {
    ...
    rc = isp_pipeline_set_besthot(pipeline, (int32_t *)in_params,
           in_params_size);
  }
    break;
