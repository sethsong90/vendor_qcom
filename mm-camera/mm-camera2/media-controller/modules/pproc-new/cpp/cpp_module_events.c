  ...
  unsigned int identity, isp_buf_divert_t *isp_buf)
{
  ...
  cpp_module_put_new_ack_in_list(ctrl, key, 1, num_events);
  ...
  mct_event_t* event)
{
  ...
  if (stats_update->flag & STATS_UPDATE_AEC) {
    ...
    chromatix_ptr = session_params->module_chromatix.chromatixPtr;
  ...
  mct_event_t* event)
{
    ...
    session_params->module_chromatix.chromatixComPtr =
      chromatix_param->chromatixComPtr;
    session_params->module_chromatix.chromatixPtr =
  ...
  uint32_t identity, int32_t value)
{
  ...
  for(i = 0; i < CPP_MODULE_MAX_STREAMS; i++) {
    if(session_params->stream_params[i]) {
      ...
      if (session_params->module_chromatix.chromatixPtr != NULL) {
        chromatix_ptr = session_params->module_chromatix.chromatixPtr;
        cpp_hw_params_asf_interpolate(
