...
public class WFDNative {
	...
	public static void eventCallback(String eventName, Object[] objectArray) {
		...
		for (int i=0;i<array_length;i++) {
			Log.d(TAG, "\tobjectArray["+i+"] = "+objectArray[i].toString());
		}
		if (listener == null) {
		...
		if ("Error".equalsIgnoreCase(eventName)) {
		    ...
		    if (objectArray.length == 3) {
		        ...
		        } else if ("NEGOTIATING".equalsIgnoreCase(state)) {
		    ...
		    if (objectArray.length >= 2) {
			    ...
			    Log.d(TAG, "MMStreamStarted" + width + height + hdcp);

                            Bundle b = new Bundle(4);
                            b.putString("event", var);
                            b.putString("width", (String) objectArray[1]);
                            b.putString("height", (String) objectArray[2]);
                            b.putString("hdcp", Integer.toString(hdcp));
