...
IGraphicBufferProducer* ExtendedRemoteDisplay::createSurface(int width, int height)
{
    ...
    mpBufferQueue = mpSurfaceMediaSrc->getBufferQueue();

    if(mpBufferQueue == NULL)
    {
        EXTRDLOGE("Unable to Create a Buffer Q from SurfaceMediaSource");
        return NULL;
    }

    EXTRDLOGH("EXTRD returning Surface texture");

    return mpBufferQueue.get();
}

int ExtendedRemoteDisplay::destroySurface(IGraphicBufferProducer* pBufQ)
{
    if(pBufQ == mpSurfaceMediaSrc->getBufferQueue().get())
    {
        //----------------------------------------------------------------------
