...
static jobject gWFDNativeObject;
static jclass gWFDNativeClazz;

char strarray[WFD_NATIVE_STRING_ARR_SIZE][WFD_NATIVE_STRING_LEN];
...
JNIEXPORT jint JNI_OnLoad(JavaVM *vm, void *reserved)
{
     ...
     * Cache instances of classes, so that we can later retrieve class references using GetObjectClass() JNI function.
     * These objects must be protected from garbage-collecting using NewGlobalRef(),
     * as that guarantees that they will remain available to different threads during JVM lifetime. */
    jmethodID constructor = env->GetMethodID(gWFDNativeClazz, "<init>", "()V");
    if(!constructor) {
        LOGE("Failed to get %s constructor", "com/qualcomm/wfd/WFDNative");
        return JNI_FALSE;
    }
    jobject jNativeObj = env->NewObject(gWFDNativeClazz, constructor);
    if(!jNativeObj) {
        LOGE("Failed to create %s object", "com/qualcomm/wfd/WFDNative");
        return JNI_FALSE;
    }
    gWFDNativeObject = env->NewGlobalRef(jNativeObj);
...
{
    JNIEnv *jniEnv = gEventCallbackArgs.jniEnv;
    nsecs_t eventTime = 0;
    int angle = 0;
    bool injectEvent = true;
    gEventCallbackArgs.cached_VM->AttachCurrentThread(&jniEnv, NULL);
...
{
    //No need to reattach to VM since this
    //should have been done by now for the UIBC thread
    if(!gEventCallbackArgs.isAttached)
    {
        LOGE("Something horribly wrong if thread is not attached!");
        return false;
    }
    LOGD("sendHIDEvent");
    JNIEnv *env = gEventCallbackArgs.jniEnv;
    jsize length = len;
    ...
    else
    {
        ...
        //What to do??  Detach the UIBC thread or continue?
        /*if (gEventCallbackArgs.isAttached)
            gEventCallbackArgs.isAttached = false;
...
void registerUibcAdaptorCallbacks()
{
  registerUibcCallbacks(&AttachVM,&SendEvent,&sendHIDEvent);
}
/*
  ...
  (JNIEnv *env, jclass cls, jint mode)
{
	...
	return setAVPlaybackMode((AVPlaybackMode) mode);
}
  ...
  (JNIEnv *env, jclass cls, jobject thisDevice)
{
    ...
    registerUibcAdaptorCallbacks();

    WfdDevice *dev = new WfdDevice;
    if (dev) {
       wfdDeviceToNative(env, thisDevice, dev);
       bWFDenableflag = enableWfd(dev, &jni_stringarray_callback);
       delete dev;
    }
...
{
    /*char pMac[30];
    int macLen = env->GetStringLength(jstrMacAddr);
    env->GetStringUTFRegion(jstrMacAddr, 0, macLen, pMac);*/
  ...
  (JNIEnv *env, jclass cls, jobject jobj)
{
    ...
    if (!getResolution(&(gResolution.width),&(gResolution.height))) {
  ...
  (JNIEnv *env, jclass cls, jobject javaSurface)
{
    ...
    if (javaSurface) {
    	nativeSurface = (Surface*) env->GetIntField(javaSurface, gSurface.surface);
    	LOGD("Native surface pointer %p", nativeSurface);
    } else {
    	LOGD("Java surface is null");
    }
    setSurface(nativeSurface);
}
/*
 * Class:     com_qualcomm_wfd_WFDNative
  ...
  (JNIEnv *env, jclass clazz, jint angle)
{
    ...
    if(sendUIBCEvent(&rotationEvent)) {
      return JNI_TRUE;
    } else {
  ...
  (JNIEnv *env, jclass cls, jint formatType, jint value, jintArray resParams)
{
    ...
    bool retStatus = setResolution(formatType, value, nativeResParamArr);
    if (retStatus)
    {
  ...
  (JNIEnv *env, jclass cls, jint transportType, jint bufferLengthMs, jint port)
{
	...
	negotiateRtpTransport(transportType, bufferLengthMs, port);
}
...
{
    jsize len = cfgItems.size();
    //Create jint array to tide over platform dependent
    //definition of ints
    jint* tempArr = new jint[len];
    for(jint i=0;i< len;i++) {
        tempArr[i] = cfgItems.at(i);
        LOGI("cfgItems is %d",cfgItems.at(i));
    }
...
{
    int ret = executeRuntimeCommand(cmd);
    if(ret == 0)
      return JNI_FALSE;
    else
      return JNI_TRUE;
  ...
  (JNIEnv * env, jclass cls, jintArray result)
{
    ...
    jint numProf = 0;
    uint32_t* comRes = getCommonResolution(&numProf);
    if(!comRes)
    {
        nativeResult[0]= -1;
  ...
  (JNIEnv *env, jclass cls, jintArray result)
{
    ...
    jint nativeResult[1] = {0};
    uint32_t* negRes = getNegotiatedResolution();
    if(!negRes)
    {
        nativeResult[0] = -1;
        env->SetIntArrayRegion(result,0,1,nativeResult);
...
static void callback_handler(jstring eventName, jobjectArray oarr)
{
	...
	jclass adaptorClass = env->GetObjectClass(gWFDNativeObject);
	if (!adaptorClass) {
	...
	jmethodID method = env->GetStaticMethodID(adaptorClass, "eventCallback", "(Ljava/lang/String;[Ljava/lang/Object;)V");
	if (!method) {
	...
	env->CallStaticVoidMethod(adaptorClass, method, eventName, oarr);
