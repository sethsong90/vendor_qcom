                                 ...
                                 OMX_U32 nModuleId)
  {
     ...
     if (result == OMX_ErrorNone)
     {
        if(pConfig->sOutputobj.outputType == MUXER_OUTPUT_RTP)
        {
          ...
          sNwInfo.DestIp          = pConfig->sOutputobj.outputInfo.rtpInfo.ipaddr1;
          sNwInfo.Port0           = pConfig->sOutputobj.outputInfo.rtpInfo.portno1;
          ...
          m_pSourcePort = MM_New_Args(CRTPEncoder,
                                      (&sNwInfo,pConfig->sAudioParams.bitrate +
                                       pConfig->svideoParams.bitrate));
          if (m_pSourcePort)
          {
            pSource = (QOMX_DATAINTERFACETYPE *)malloc(sizeof(QOMX_DATAINTERFACETYPE) + sizeof(int));
            if (pSource)
            {
     ...
     if (result == OMX_ErrorNone)
     {
            ...
            if (result == OMX_ErrorNone)
            {
               if(omxPortAudio.eDomain == OMX_PortDomainAudio)
               {
                  MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "Num Audio buffers potdef = %ld", omxPortAudio.nBufferCountMin);
                   m_nAudioBuffers = MAX(m_nAudioBuffers, omxPortAudio.nBufferCountMin);
        ...
        if(m_bVideoPortEnabled)
        {
            ...
            {
               m_nVideoBuffers = MAX(m_nVideoBuffers, omxPortVideo.nBufferCountMin) ;
...
//       MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "WFDMMSource::MuxerEmptyBufferDone %d",pBuffer->nFilledLen );
     ...
     if( m_bAudioPortEnabled)
     {
         ...
         if (m_bAudioUseBuffer == OMX_TRUE)
         {
            ...
            for (i = 0; i < m_nAudioBuffers; i++)
            {
               ...
               MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "USE BUFFER INPUT: %p %p %ld %d", m_hMuxer,&m_pAudioBufferHdrs[i],m_pAudioBuffers[i]->nAllocLen,(int)m_pAudioBuffers[i]->pBuffer  );
     ...
     if(m_bVideoPortEnabled)
     {
         ...
         if (m_bVideoUseBuffer == OMX_TRUE)
         {
            ...
            for (i = 0; i < m_nVideoBuffers; i++)
            {
                ...
                MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_HIGH, "USE BUFFER INPUT: %p %p %ld %d", m_hMuxer,&m_pVideoBufferHdrs[i],m_pVideoBuffers[i]->nAllocLen,(int)m_pVideoBuffers[i]->pBuffer  );
  ...
  OMX_ERRORTYPE Muxer::UpdateRTPPort(MUX_RTPinfo *pRTPInfo)
  {
      ...
      m_pSourcePort = MM_New_Args (CRTPEncoder, (pRTPInfo->portno1,
                                      pRTPInfo->ipaddr1,
                                     ((m_sConfig.sAudioParams.bitrate +
                                       m_sConfig.svideoParams.bitrate)),
                                      pRTPInfo->bPortTypeUDP));
      if (m_pSourcePort)
      {
          pSource->nInterfaceSize = 4;
          pSource->nSize = sizeof(QOMX_DATAINTERFACETYPE) + sizeof(int);
  ...
  /////////////////////////////////////////////////////////////////////////////
   void Muxer::SendEventCallback(RTPEventType eEvent,
                                 RTPStatusType eStatus, int nData)
   {
