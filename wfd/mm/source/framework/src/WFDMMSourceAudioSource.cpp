...
#define PAGE_SIZE 4096 // page size in linux which has to be the minimum size for mmap
#define AUDIO_ION_BUF_SIZE 4096 // page size in linux which has to be the minimum size for mmap
...
#define AAC_HW_PARAM_PERIOD_SIZE 1024
...
inline bool IS_PROXY_IN_USE() {
  ...
  AUDIO_DEVICE_OUT_BLUETOOTH_A2DP,""))
      == android_audio_legacy::AudioSystem::DEVICE_STATE_AVAILABLE) ||
...
*//*==========================================================================*/
 ...
 :
    ...
    m_pAACInfo(NULL)
  {
      ...
      fp_aac = NULL;
      m_bDumpAAC = IS_DUMP_ENABLE(AAC_DUMP);
#endif
#ifdef WFD_DUMP_AUDIO_DATA
      fp_wav = NULL;
...
*//*==========================================================================*/
  AudioSource::~AudioSource()
  {
    ...
    if(m_eAudioCodingType == OMX_AUDIO_CodingAAC)
    {
      ...
      {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "AudioSource :: Failed to close AAC ENC");
      }
#ifdef AAC_DUMP_ENABLE
      if (fp_aac)
      {
...
*//*==========================================================================*/
      ...
      void *appData)
  {
      ...
      getCfgItem(AUDIO_AV_SYNC_DEL_KEY,(int*)(&m_nAudioDelay));
      {
          ...
          pEventHandlerFn != NULL)
      {
          ...
          {
              uBufferSize = AAC_HW_PARAM_PERIOD_SIZE * 2 * m_PcmConfig.channels;
              m_nFrameTimeUnit = (AAC_HW_PARAM_PERIOD_SIZE *
              ...
              m_nAudioBufsize = AAC_HW_PARAM_PERIOD_SIZE * m_PcmConfig.channels * 2;
      ...
      if(m_bDumpWav)
      {
          fp_wav= fopen("/data/audiodump.wav","wb");
          if(!fp_wav)
          {
              MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
          ...
          else
          {
              ...
              hdr.fmt_sz = 16;
              hdr.audio_format = 1;
              hdr.num_channels = m_PcmConfig.channels;
              hdr.sample_rate = 48000;
              hdr.bits_per_sample = 16;
              hdr.byte_rate = (48000 * m_PcmConfig.channels * hdr.bits_per_sample) / 8;
              hdr.block_align = ( hdr.bits_per_sample * m_PcmConfig.channels ) / 8;
              hdr.data_id = ID_DATA;
              hdr.data_sz = 2147483648LL;
              hdr.riff_sz = hdr.data_sz + 44 - 8;
              fwrite(&hdr,1, sizeof(hdr),fp_wav);
      ...
      if(m_bDumpAAC)
      {
        fp_aac = fopen("/data/aacdump.aac","wb");
        if(!fp_aac)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                          "AudioSource:aacdump fopen failed");
            m_bDumpAAC = OMX_FALSE;
        }
      }
#endif
...
*//*==========================================================================*/
  ...
  OMX_ERRORTYPE AudioSource::Audio_Handler (void * audioHandle)
  {
      ...
      if( OMX_AUDIO_CodingPCM == pThis->m_eAudioCodingType )
      {
        ...
        uint32 sampling_rate = pThis->m_nSamplingFreq;
        pThis->pBufferHdr->pBuffer[index] |= (uint8)((sampling_rate == 48000 ? 2 :
      ...
      if (pThis->bKeepGoing == OMX_TRUE )
      {
         ...
         if(result == OMX_ErrorNone)
         {
            ...
            if(pThis->m_bDumpWav)
            {
              if (pThis->fp_wav)
              {
                  fwrite(pThis->m_pAudioDataBfr, 1, pThis->m_nAudioBufsize, pThis->fp_wav);
              }
            }
  #endif
            ...
            if(pThis->m_eAudioCodingType == OMX_AUDIO_CodingPCM)
            {
                ...
                if(OMX_FALSE == bInsertSilence)
                {
                  ...
                  */
                  ...
                  for(int count = 0; count < (nCopyCount >> 1); count++)
                  {
                      nVal = *pSrc++;
                      *pDst++ = (nVal >> 8) | (nVal << 8);
                  }
            ...
            else if(pThis->m_eAudioCodingType == OMX_AUDIO_CodingAAC)
            {
                ...
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,
                             "Audio_Handler:Start AAC encoding @ %lu",
                              lTime);

                pThis->pBufferHdr->nFilledLen =  0;
                pThis->pBufferHdr->nFlags = 0x00;

                result = pThis->EncodeAAC(pThis->m_pAudioDataBfr,
                                   pThis->pBufferHdr->pBuffer);

                if(result == OMX_ErrorNone)
                {
                    // iF the call to encode is success sOutArg numbytes
                    //carries the number of bytes encoded.
                    pThis->pBufferHdr->nFilledLen =
                            ((struct aacSampleInfo*)pThis->m_pAACInfo)->sOutArg.numOutBytes;
                    pThis->pBufferHdr->nFlags = 0x00;
                }
                MM_Time_GetTime(&lTime);
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "Audio_Handler\
                  : Complete AAC encoding @ = %lu",lTime);
  #ifdef AAC_DUMP_ENABLE
                if(pThis->m_bDumpAAC)
                {
                    if (pThis->fp_aac)
                    {
                        fwrite(pThis->pBufferHdr->pBuffer, 1,
                           pThis->pBufferHdr->nFilledLen, pThis->fp_aac);
                    }
                }
  #endif
...
// 2. Append Private Data as Extra Data to the end of Elementary Stream Buffer
          ...
          pThis->m_pHdcpHandle->m_eHdcpSessionStatus == HDCP_STATUS_SUCCESS )
      {
        ...
        unsigned long ulStatus = pThis->m_pHdcpHandle->WFD_HdcpDataEncrypt(STREAM_AUDIO ,
                                                    ...
                                                    (OMX_U8 *)pThis->m_pHDCPInputBuffers[index].fd, // clear data
                                                    (OMX_U8 *)pThis->m_pHDCPInputBuffers[index].fd, // Encrypted data data
                                                    pThis->pBufferHdr->nFilledLen);
          if( ulStatus != 0)
          {
          ...
          if (pThis->pBufferHdr != NULL )
          {
              if (pThis->pBufferHdr->nFilledLen)
              {
                     ...
                     (OMX_U64)nTimeStamp - (OMX_U64)pThis->m_nCurrTime > nDriftCutoff)
                  {
                     ...
                     int nTotalDrift = (nTimeStamp - pThis->m_nCurrTime);
                     ...
                     if(pThis->m_nTimeErrAdjustCnt && pThis->m_nFrameTimeUnit)
                     {
                         ...
                         int numFrames = nTotalDrift/pThis->m_nFrameTimeUnit;
              ...
              {
                  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
                   "Audio_Handler: AAC returned 0 bytes, pushing buffer");
...
void AudioSource::AudioSource_sleep_for_audioframe_interval(unsigned long drift)
{
  ...
  {
    periodSize = AAC_HW_PARAM_PERIOD_SIZE;
  }
...
OMX_ERRORTYPE AudioSource::ConfigureAudioProxyDevice()
{
  ...
  {
    m_PcmConfig.period_size = AAC_HW_PARAM_PERIOD_SIZE;
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
...
*//*========================================================================*/
  OMX_ERRORTYPE AudioSource::ConfigureAudioComponents()
  {
      ...
      if(m_eAudioCodingType == OMX_AUDIO_CodingAAC)
      {
          result = ConfigureAACEncoder();
          if (OMX_ErrorNone != result)
          {
              MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                          "AudioSource::Configering AAC encoder failed");
              return result;
          }
      }
...
*//*==========================================================================*/
  OMX_ERRORTYPE AudioSource::ConfigureAACEncoder()
  {
    ...
    eError = aacEncoder_SetParam((HANDLE_AACENCODER)m_hAACEncoder,
                                  AACENC_CHANNELMODE,
                                  GET_CHANNEL_MODE(m_PcmConfig.channels));
    ...
    */
    ...
    pAACInfo->nOutBufSize[0]            = AAC_HW_PARAM_PERIOD_SIZE
                                          * 2 * m_PcmConfig.channels;
    pAACInfo->nInBufSize[0]             = AAC_HW_PARAM_PERIOD_SIZE
                                          * 2 * m_PcmConfig.channels;

    pAACInfo->nOutElemSize[0]           = sizeof(OMX_U8);
    pAACInfo->nInSamples                = AAC_HW_PARAM_PERIOD_SIZE
                                          * m_PcmConfig.channels;
...
/*==============================================================================
         ...
         DESCRIPTION:
*//**       @brief         Encodes the given data into AAC
*//**
...
*//*==========================================================================*/
OMX_ERRORTYPE AudioSource::EncodeAAC(void* pIn, void* pOut)
{
    aacSampleInfo *pAACInfo = (aacSampleInfo*)m_pAACInfo;

    if(!pAACInfo || !pIn || !pOut)
    {
      return OMX_ErrorBadParameter;
    }

    //Initialize Input Buffer
    pAACInfo->sInBuff.bufs              = (void**)(&pIn);

    //Initialize Output Buffer
    pAACInfo->sOutBuff.bufs              = (void**)(&pOut);
    ...
    {
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                  "Audio_Handler:AAC encode fails");
        return OMX_ErrorDynamicResourcesUnavailable;
    }
    return OMX_ErrorNone;
}
              ...
              case AUDIO_SOURCE_PLAY_EVENT:
                  {
                      ...
                      if(ret != 0)
                      {
                          ...
                          {
                              m_pEventHandlerFn(m_appData, m_nModuleId,
                                                WFDMMSRC_ERROR, OMX_ErrorInsufficientResources, NULL);
                          }
                          break;
                      }
  ...
  *//*==========================================================================*/
                            ...
                            OMX_U32 nPortIndex)
  {
    ...
    if( (NULL != pBufHdr ) && (NULL != pucPESPvtHeader))
    {
      ...
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "AudioSource::ulAddr[%lu]", ulAddr);

      /* Aligned address to DWORD boundary */
      ulAddr = (ulAddr + 0x3) & (~0x3);
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "AudioSource::Aligned ulAddr[%lu]", ulAddr);
