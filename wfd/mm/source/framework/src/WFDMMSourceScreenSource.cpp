    ...
    void *appData)
{
    ...
    if(pConfig)
    {
            ...
            pEventHandlerFn != NULL )
        {
            ...
            m_nIntraPeriod = pConfig->nIntraPeriod;
    ...
    if(m_eScrnSrcState ==  WFDMM_SCREENSOURCE_STATE_INIT)
    {
      ...
      {
        V4L2_dump_file = NULL;
        V4L2_dump_file = fopen("/data/v4l2_dump_user.264","wb");
        if(!V4L2_dump_file)
        {
...
//set CBR_VFR
   ...
   //set peak bitrate
    ...
    for (unsigned int control_index = 0; control_index < ARRAY_SZ(controls); ++control_index)
    {
        if (controls[control_index].id == V4L2_CID_MPEG_VIDEO_H264_I_PERIOD)
        {
          controls[control_index].value = m_nIntraPeriod;
          MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
                      "WFDMMSourceScreenSource::setting I-Frame period %d   %d \n",
                       controls[control_index].id, controls[control_index].value);
          break;
        }
    }
...
void ScreenSource::DeInitializeV4l2Codec()
{
  ...
  int nExitCode = 0;
    ...
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Calling setWfdStatus");

    setWfdStatus(0);

    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"Done calling setWfdStatus");
}
