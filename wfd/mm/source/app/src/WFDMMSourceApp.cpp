...
int main (int argc, char *argv[])
{
  ...
  if( WFD_negotiated_capability != NULL)
  {
    ...
    if(WFD_negotiated_capability->video_config.video_config.h264_codec != NULL)
    {
        ...
        ((WFD_negotiated_capability->video_config.video_config.h264_codec)+ 0)->h264_level = OMX_VIDEO_AVCLevel32;
    ...
    if(argc == 10)
    {
       ...
       WFD_negotiated_capability->transport_capability_config.port1_id = atoi(argv[2]);
     //  nSleep = atoi(argv[3]);
       ...
       MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM, "WFDSource in 4 arguments %ld %d",
    ...
    else if( argc == 3)
    {
       ...
       WFD_negotiated_capability->peer_ip_addrs.ipv4_addr1= inet_addr(argv[1]);
       WFD_negotiated_capability->transport_capability_config.port1_id = atoi(argv[2]);
       MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM, "WFDSource in 3 arguments %ld %d",
                   WFD_negotiated_capability->peer_ip_addrs.ipv4_addr1,
                    WFD_negotiated_capability->transport_capability_config.port1_id );
    }
    else
    ...
    if(WFD_negotiated_capability->video_config.video_config.h264_codec != NULL)
    {
      if(nTCPFLag)
      {
       ...
       ((WFD_negotiated_capability->video_config.video_config.h264_codec)+ 0)->supported_cea_mode = nTCPFLag;
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "WFDSource cea mode is %ld",(((WFD_negotiated_capability->video_config.video_config.h264_codec)+ 0)->supported_cea_mode));
      }
      else
      {
