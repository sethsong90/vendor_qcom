...
OMX_ERRORTYPE OMX_FileMux::Set_Target_State(OMX_STATETYPE eState)
{
    ...
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,"Mux, state change req, current = %ld, next = %ld", (int32)m_eState, (int32)m_eTargetState);
                                    ...
                                    OMX_IN OMX_U32 nSizeBytes)
{
    ...
    if (nPortIndex == (OMX_U32) OMX_MUX_INDEX_PORT_AUDIO)
    {
        ...
        if(!pAudPortConfig->pBuffHeaderArray)
        {
            ...
            nAllocSize = sizeof(OMX_BOOL) * pAudPortDef->nBufferCountActual;
                                ...
                                OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_LOW,"freeing 0x%x", (unsigned int)pBuffer->pBuffer);
    ...
    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"could not find buffer");
    return OMX_ErrorNone;
}
...
)
{
    ...
    */
    ...
    {
        nStatus = pFileMux->MUX_Flush(nPortIndex);
    }
    if(nStatus == MUX_DONE)
    {
         m_pCallbacks->EventHandler(m_hSelf, m_pAppData,
...
)
{
    switch(eIndex)
    {
        ...
        case OMX_IndexParamContentURI:
        {
            ...
            nBufSize = pParam->nSize - sizeof(pParam->nVersion)-
                                                         sizeof(pParam->nSize);
        ...
        case QOMX_FilemuxIndexConfigMediaInfo:
        {
            if(OMX_MUX_CMD_SET_STD_OMX_PARAM == nCode)
            {
                ...
                */
                ...
                {
                    tempCdataDuration = (OMX_TICKS*)(&ptempMediaInfo->cData[0]);
                    nFileDurationLimit = *tempCdataDuration;
                    return OMX_ErrorNone;
                }
                else if (ptempMediaInfo->eTag == QOMX_MediaInfoSize)
                {
                    tempCdataSize = (OMX_U64 *)(&ptempMediaInfo->cData[0]);
            ...
            if(OMX_MUX_CMD_SET_STD_OMX_PARAM == nCode)
            {
                ...
                */
                if(pFileMux)
                {
                    ...
                    pFileMux->MUX_update_streamport((uint32)pIStreamPort);
                }
            }
            else
            {
        ...
        case OMX_IndexParamAudioPortFormat:
        {
            ...
            if(OMX_MUX_CMD_SET_STD_OMX_PARAM == nCode)
            {
                ...
                */
                if(nFileFormat == MUX_FMT_MP4 &&
                       ( pParam->eEncoding != OMX_AUDIO_CodingPCM &&
                         ...
                         pParam->eEncoding != QOMX_AUDIO_CodingEVRCB &&
                         pParam->eEncoding != QOMX_AUDIO_CodingEVRCWB))
                {
                ...
                */
                ...
                {
                    if(AUDIO_COMPRESSION_FORMAT == QOMX_AUDIO_CodingEVRCB)
                    {
                        nFileFormat = MUX_FMT_EVRCB;
                    }
                    else if(AUDIO_COMPRESSION_FORMAT == QOMX_AUDIO_CodingEVRCWB)
                    {
    ...
    */
       ...
       pBuffHdr->nFlags & OMX_BUFFERFLAG_CODECCONFIG)
    {
        ...
        */
        ...
        pFileMux->MUX_write_header(nStreamType, true, pBuffHdr->nFilledLen,
        ...
        */
        ...
        if(nheaderLen && pHeader)
        {
            ...
            */
            ...
            pFileMux->MUX_write_header(nStreamType,
                                       nheaderLen,
                                       (uint8*)pHeader);
            sStreamInfo[nStreamType].bHeaderReceived = OMX_TRUE;
        }
    ...
    else if(pTmpVidBufferHdr)
    {
        ...
        */
        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                     "OMX_FileMux ETB video StartTime %ld %ld!!!",
                     (uint32)((uint64)pBuffHdr->nTimeStamp >> 32),
                     (uint32)((uint64)pBuffHdr->nTimeStamp ) );
        MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                     "OMX_FileMux ETB video EndTime %ld %ld!!!",
         ...
         (AUDIO_COMPRESSION_FORMAT) == OMX_AUDIO_CodingAAC)
        {
            ...
            */
              ...
              nFileFormat == MUX_FMT_MP4)
            {
                ...
                pFileMux->MUX_write_header(nStreamType, 2,
...
)
{
    ...
    */
    ...
    {
        pSampleInfo->size  = pBuffHdr->nFilledLen - pBuffHdr->nOffset;
    }
    else
    {
        pSampleInfo->size  = 0;
    }
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM, "OMX_FileMux ETB About to push %ld %ld!!!",
                     pSampleInfo->size, pBuffHdr->nOffset);
    pSampleInfo->delta  = nDelta;


    if(m_FmtPvtInfo.nExtraSize != 0)
    {
        pSampleInfo->fmt_pvtdata_offset = m_FmtPvtInfo.nExtraOffset;
        pSampleInfo->fmt_pvtdata_size   = m_FmtPvtInfo.nExtraSize;
    ...
    {
        pSampleInfo->extra_data_offset = m_HDCPInfo.nExtraOffset;
        pSampleInfo->extra_data_size   = m_HDCPInfo.nExtraSize;
...
MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_MEDIUM, "OMX_FileMux ETB sAVSyncInfo.nAudioStartTime,pBuffHdr->nTimeStamp,pSampleInfo->time %lld %lld %ld!!!",
    sAVSyncInfo.nAudioStartTime,pBuffHdr->nTimeStamp,pSampleInfo->time);
MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, "OMX_FileMux ETB pStreams[nStreamNum].media_timescale = %ld!",
pStreams[nStreamNum].media_timescale);
    }
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM, "OMX_FileMux ETB Audio About to push time = %ld %ld!!!",
    ...
    ----------------------------------------------------------------------------
    */
    nStatus = pFileMux->MUX_Process_Sample(nStreamNum,
                             1,
                             pSampleInfo,
                             pBuffHdr->pBuffer + pBuffHdr->nOffset,
                             (void*)pBuffHdr);
...
)
{
    ...
    */
    ...
    if((pBuffHdr->pBuffer + pBuffHdr->nAllocLen -
         (OMX_U8*)pExtra  <  sizeof(OMX_OTHER_EXTRADATATYPE))
       || ((((OMX_U32)(&pExtra->data[0]) + pExtra->nDataSize - 1)
    ...
    while(pExtra->eType != OMX_ExtraDataNone)
    {
        ...
        ------------------------------------------------------------------------
        */
        if(pExtra->eType == QOMX_ExtraDataVideoMultiSliceInfo)
        {
        ...
        if(pExtra->eType == QOMX_ExtraDataHDCPEncryptionInfo)
        {
          MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,
          ...
          if(pExtra->nSize > sizeof(OMX_OTHER_EXTRADATATYPE))
          {
            ...
            m_HDCPInfo.nExtraOffset =
                             (uint32)pExtra + sizeof(OMX_OTHER_EXTRADATATYPE)-4
              - (uint32)pBuffHdr->pBuffer;
...
)
{
    ...
    psliceinfoptr = (OMX_U32*)(&pExtra->data);
    ...
    while(nnumslices--)
    {
        ...
        */
        MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_ERROR,
            "Sliceinfo pdataptr = %x, slice offset = %d nslicesize = %ld !!!",
             (unsigned int)pdataptr, (pdataptr - pBuffHdr->pBuffer), nslicesize);
          ...
          (pdataptr + 3) < (pBuffHdr->pBuffer + pBuffHdr->nAllocLen))
        {
            ...
            pdataptr[0] = ((nslicesize - 4) >> 24);
            pdataptr[1] = ((nslicesize - 4) >> 16) & 0xff;
            pdataptr[2] = ((nslicesize - 4) >> 8 ) & 0xff;
            pdataptr[3] = ((nslicesize - 4)     ) & 0xff;
        }
        else
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
...
*//*============================================================================
*/
int OMX_FileMux::OMX_FileMux_AVSyncFindNumAudioPacketsToInsert
(
    OMX_U32  nPortIndex
)
{
    ...
    */
    if(paudiobuffer && naudiobufferdata)
    {
        for(OMX_U32 i = 0; i < npacketinsert; i++)
        {
            ...
            psampleinfo->size = naudiobufferdata;
            psampleinfo->delta = ndelta;
            psampleinfo->sync = true;
    ...
    switch (status)
    {
        ...
        case CLOSE_MUX_FAIL:
        {
                 ...
                 m_eTargetState == OMX_StateIdle))
            {
                ...
                MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
                  "OMX_FileMux::OMX_FileMux_ProcessFileMuxCb state changed from current %ld, to next = %ld",
...
)
{
    ...
    pParams->file_duration_limit = (OMX_U32)nFileDurationLimit;
    pParams->file_size_limit = (OMX_U32)nFileSizeLimit;
    ...
    pParams->stream_bitrate = GET_VIDEO_BITRATE;
    ...
    pParams->encrypt_param.nEncryptVersion = nEncryptTypeConfigParameters.nEncryptVersion;
    ...
    */
    ...
    case QOMX_FORMAT_MP4:
        ...
        pParams->num_compat_brands = sizeof (MP4_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_AMC:
        ...
        pParams->num_compat_brands = sizeof (AMC_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_3GP:
        ...
        pParams->num_compat_brands = sizeof (_3GP_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_3G2:
        ...
        pParams->num_compat_brands = sizeof (_3G2_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_K3G:
        ...
        pParams->num_compat_brands = sizeof (K3G_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_SKM:
        ...
        */
        ...
        pParams->num_compat_brands = sizeof (SKM_COMPAT_BRANDS)
    ...
    case QOMX_FORMAT_AAC:
        ...
        */
        ...
        pParams->stream_bitrate = (arrPortConfig + OMX_MUX_INDEX_PORT_AUDIO)->
                                        sFormatSpecificInfo.sAACInfo.nBitRate;
        break;
    ...
    case QOMX_FORMAT_WAVE:
        ...
        pParams->sampling_rate = (arrPortConfig + OMX_MUX_INDEX_PORT_AUDIO)->
                                   sFormatSpecificInfo.sPCMInfo.nSamplingRate;
        pParams->block_align   =
                          (pParams->bits_per_sample+7)/8*pParams->num_channels;
        break;
...
)
{
    ...
    */
    ...
    if (OMX_TRUE == bVideoPresent)
    {
        ...
        pStreams[nIndex].subinfo.video.frame_rate =
                                                 GET_VIDEO_FRAMERATE;
        ...
        pStreams[nIndex].interlace = nIndex;
        ...
        pStreams[nIndex].max_samples  = MAX(MMI_MAX_SAMPLES_PER_STORE,
                                            nSpaceLimitThreshold *
                                            GET_VIDEO_FRAMERATE);

        if(nFragmentDuration)
        {
            pStreams[nIndex].max_chunks = MAX(
        ...
        pStreams[nIndex].max_samples_rate = GET_VIDEO_FRAMERATE;


        pStreams[nIndex].max_table_stores = MMI_MAX_STREAM_DURATION *
                                            (GET_VIDEO_BITRATE >> 3) /
                                            pStreams[nIndex].chunk_size /
...
OMX_BOOL OMX_FileMux::OMX_FileMux_PopulateAudioCommonParams(void)
{
    ...
    if (OMX_TRUE == bAudioPresent)
    {
        ...
        */
        ...
        pStreams[MMI_AUDIO_STREAM_NUM].buffer_size =
        ...
        pStreams[MMI_AUDIO_STREAM_NUM].chunk_size =
                                       (OMX_FileMux_GetAudioBitrate()/8) *
        ...
        pStreams[MMI_AUDIO_STREAM_NUM].max_chunks = MAX(
                                          DEFAULT_CHUNKS_TABLE_SIZE,
                                         (nSpaceLimitThreshold * 1000) /
                                          DESIRED_INTERLACE_RATE + 1);
...
)
{
    ...
    case OMX_AUDIO_CodingAAC:
        ...
        */
        ...
        pStreams[MMI_AUDIO_STREAM_NUM].media_timescale =
                   pAudioConfig->sAACInfo.nSampleRate;
    ...
    case OMX_AUDIO_CodingPCM:
        ...
        pParams->bits_per_sample =
               (uint16)(arrPortConfig + OMX_MUX_INDEX_PORT_AUDIO)->
                                sFormatSpecificInfo.sPCMInfo.nBitPerSample;
        pStreams[MMI_AUDIO_STREAM_NUM].media_timescale =
                   pAudioConfig->sPCMInfo.nSamplingRate;
        pParams->num_channels    =
             (uint8)(arrPortConfig + OMX_MUX_INDEX_PORT_AUDIO)->
                                    sFormatSpecificInfo.sPCMInfo.nChannels;
...
)
{
    ...
    case OMX_AUDIO_CodingAMR:
       return ((pAudioConfig->sAMRWBInfo.eAMRBandMode == OMX_AUDIO_AMRBandModeNB7)
             ...
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB15)
             ? 4000
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB14)
             ? 24000
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB13)
             ? 24000
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB12)
             ? 24000
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB11)
             ? 18000
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB10)
             ? 13600
             : (pAudioConfig->sAMRWBInfo.eAMRBandMode == QOMX_AUDIO_AMRBandModeWB9)
             ? 4000
...
void OMX_FileMux::OMX_FileMux_PrintStatistics()
{
    ...
    */
    ...
    if(IS_VIDEO_PORT_USED)
    {
        ...
        MM_MSG_PRIO1(MM_STATISTICS, MM_PRIO_HIGH,
        "Mux Stats:  Video Average Bitrate          = %lf",
        (((OMX_U64)sMuxStats.nVideoBytes * 8 * 1000000.0) /
                             sMuxStats.nVideoDuration)
        );

        if(sMuxStats.nVideoDuration)
        MM_MSG_PRIO1(MM_STATISTICS, MM_PRIO_HIGH,
        "Mux Stats:  Video Average FPS              = %lf",
        (((OMX_U64)sMuxStats.nNumVidFramesWritten * 1000000.0)/
                        sMuxStats.nVideoDuration)
        );
    }
    return;
