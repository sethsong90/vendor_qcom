...
private:
    ...
    static bool decryptCb(int, int, int, int, int, char*, int);

    bool decrypt(int, int, int, char*, int , int nStreamId = SINK_VIDEO_TRACK_ID);

    static void avInfoCb(int handle, avInfoType *pInfo);
    ...
    static void CmdThreadEntry(int pHandle,
                               unsigned int nCmd);
    ...
    static void IDRDelayTimerCb(void *);
