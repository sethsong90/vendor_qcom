...
uint64 WFDMMSinkStatistics::GetFrameIPTime(OMX_TICKS nFramePTS)
{
    ...
    int i;
    for(i = mTailStatsArray ; (i+1) % MAX_STATS_ARRAY!= mHeadStatsArray ;
       i = (i+1) % MAX_STATS_ARRAY)
    {
...
int  WFDMMSinkStatistics::PrintStatistics(void)
{
    ...
    WFDMMLOGE1("Total Number of Video Frames dropped in the session= %ld",
        mTotalVideoFramesDropped);
    WFDMMLOGE1("Total Number of Audio Frames dropped in the session= %ld",
        mTotalAudioFramesDropped);
    WFDMMLOGE1("Number of Video Frames dropped after 5 seconds= %ld",
        mTotalVideoFramesDroppedAfterFiveSeconds);
    WFDMMLOGE1("Number of Audio Frames dropped after 5 seconds = %ld",
        mTotalAudioFramesDroppedAfterFiveSeconds);
