...
#define AUDIO_AVSYNC_DROP_WINDOW -110000
#define VIDEO_AVSYNC_DROP_WINDOW -200000
                                             ...
                                             OMX_BUFFERHEADERTYPE *pBuffer)
{
    ...
    */
    ...
    WFDMMLOGH("WFDMMSinkRenderer DeliverInput");
    ...
    */
    ...
    CRITICAL_SECT_ENTER
    if(mnLatestTimeStamp < (uint64)pBuffer->nTimeStamp)
    {
        mnLatestTimeStamp = pBuffer->nTimeStamp;
    }
    CRITICAL_SECT_LEAVE
...
bool WFDMMSinkRenderer::createAudioResources()
{
    ...
    if(nFlags & AUDIO_OUTPUT_FLAG_TUNNEL)
    {
        ...
        AudioSystem::acquireAudioSessionId(mnAudioSessId);
    }

    nFlags |= AUDIO_OUTPUT_FLAG_FAST;

    WFDMMLOGD2("SinkRenderer Audio SampleRate %lu Channels %lu",
    ...
    */
    if(mCfg.bHasVideo)
    {
        ...
        mpVideoThread->Start(VideoThreadEntry, -2,
                             32768, (int)this, "WFDSinkVideorenderer");
        ...
        mpGraphicPollThread->Start(GraphicThreadEntry, -2,
                             32768, (int)this, "WFDSinkGraphicrenderer");
    ...
    if(mCfg.bHasAudio)
    {
        ...
        mpAudioThread->Start(AudioThreadEntry, -2,
                             65536*2, (int)this, "WFDSinkAudioRenderer");
...
bool WFDMMSinkRenderer::RenderVideo(frameInfoType *pFrameInfo)
{
    ...
    BufferDim_t bufferDim = {pPictInfo->rect.nWidth, pPictInfo->rect.nHeight};
                                    ...
                                    uint32  bufferId)
{
    ...
    WFDMMLOGH1("Searching for Buffer Id = %lu", bufferId);
...
)
{
    ...
    if((int)(*((int*)pCmpValue)) == (int)pGraphicBuffer->handle)
    {
        //Found what we are looking for
        WFDMMLOGH1("Buffer Found %lu", (uint32)pCmpValue);
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
void WFDMMSinkRenderer::VideoThreadEntry(int pThis,
    ...
    while(ISSTATEOPENED || ISSTATECLOSING)
    {
        ...
        if(pendingQsize)
        {
               ...
               pFrameInfo != NULL)
            {
                ...
                if(((int64)timedecider <= (int64)VIDEO_AVSYNC_DROP_WINDOW &&
                ...
                && (uint64)pFrameInfo->pBuffer->nTimeStamp <= mnFlushTimeStamp))
                {
                    ...
                    */
                    ...
                    WFDMMLOGE2(
                       "Renderer drop Videoframe TS = %llu, lateby = %lld",
                  ...
                  !mCfg.bAVSyncMode)
                {
                    ...
                    WFDMMLOGD("PushVideoFrame to render");
                    mnVideoDropCount = 0;
                    /*----------------------------------------------------------
                    ...
                    */
                    ...
                    WFDMMLOGH("PushVideoFrame to render");
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
void WFDMMSinkRenderer::AudioThreadEntry(int pThis,
                                            unsigned int nSignal)
{
    CHECK_ARGS_RET_VOID_1(pThis);
...
void WFDMMSinkRenderer::AudioThread(unsigned int nSignal)
{
    ...
    while(ISSTATEOPENED || ISSTATECLOSING)
    {
        ...
        if(pFrameInfo)
        {
            ...
            if(pFrameInfo->pBuffer->nTimeStamp <= INITIAL_AUDIO_DROP_WINDOW)
            {
                bAVSyncDrop = true;
            }
            else if (nDropCount)
            ...
            if((( (int64)timedecider <= (int64)AUDIO_AVSYNC_DROP_WINDOW)&&
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
void WFDMMSinkRenderer::GraphicThreadEntry(int pThis,
                                            unsigned int nSignal)
{
    CHECK_ARGS_RET_VOID_1(pThis);
...
void WFDMMSinkRenderer::GraphicThread(unsigned int nSignal)
{
    ...
    while((ISSTATEOPENED || ISSTATECLOSING) && !ISERROR)
    {
        ...
        if(mnNumVidQedBufs - mnNumVidDQedBufs > 2)
        {
            ...
            if (nRet != 0)
            {
                WFDMMLOGE("failed to dequeue Buffer");
                MM_Timer_Sleep(2);
                continue;
            }
            else
            {
                AccessRenderQ(&pFrameInfo, (uint32)pBuf->handle, search);
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
int WFDMMSinkRenderer::AccessRenderQ(frameInfoType **pPkt, uint32 handle,
                                       actionType bAction)
{
    if(pPkt == NULL && bAction != size)
    {
...
OMX_ERRORTYPE WFDMMSinkRenderer::Stop()
{
    ...
    uint32 nAudPendingSize = mAudioQ->GetSize();
...
void  WFDMMSinkRenderer::AudioTrackCb(int  nEvent,
                                      ...
                                      void *pInfo)
{
    WFDMMLOGE1("AudioTrack CallBack event = %d", nEvent);
    return;
}
...
void WFDMMSinkRenderer::prepareDumpFiles()
{
    ...
    if(mbDumpWav)
    {
        mpFpWAV= fopen("/data/wfdsinkaudiodump.wav","wb");
        if(!mpFpWAV)
        {
    ...
    if(mbDumpAAC)
    {
        mpFpAAC = fopen("/data/wfdsinkaacdump.aac","wb");
        if(!mpFpAAC)
        {
...
void WFDMMSinkRenderer::restartRendering(bool bFlush)
{
    if(mbPaused && bFlush)
    {
        setFlushTimeStamp(mnLatestTimeStamp);
    }
    mbPaused = false;
}
