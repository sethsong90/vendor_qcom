...
WFDMMSink::WFDMMSink()
{
    ...
    FILE *pDumpFile = fopen("/data/rtpdump.ts", "rb");
    if(pDumpFile != NULL)
    {
        fclose(pDumpFile);
        pDumpFile = NULL;
        if(MM_File_Delete("/data/rtpdump.ts") != 0)
    ...
    if(m_pCmdThread->Start(CmdThreadEntry, -2, 32768 * 4,
                           (int)this, "SinkCmdThread"))
    {
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::CmdThreadEntry(int pHandle,unsigned int nCmd)
{
        ...
        if( m_pNegotiatedCap->pSurface != NULL )
        {
            ...
            */
            if(m_pVideoSurface != NULL)
            {
                ...
                ----------------------------------------------------------------
                */
                int nErr = native_window_set_scaling_mode(m_pWindow.get(),
                                  NATIVE_WINDOW_SCALING_MODE_SCALE_TO_WINDOW);
                if(nErr)
                {
                    WFDMMLOGH("Failed to set Scaling mode");
                }
            }
        }
        else
...
)
{
    ...
    */
    if(pNegotiatedCap->video_method != WFD_VIDEO_H264 &&
        pNegotiatedCap->audio_method != WFD_AUDIO_LPCM &&
        pNegotiatedCap->audio_method != WFD_AUDIO_AAC
    ...
    */
    ...
    if(play(WFD_STREAM_AV, NULL) != WFD_STATUS_SUCCESS)
    {
        WFDMMLOGE("WFD Failed to prepare Play");
        CRITICAL_SECT_LEAVE;
        RETURNFAIL;
    }
  ...
  //                 content_protection_config.content_protection_ake_port : 0;
    ...
    m_pMediaSrc = MM_New_Args(WFDMMSinkMediaSource,
                              (SINK_MODULE_MEDIASOURCE,
                               eventHandlerCb, FBD,avInfoCb,(int)this));
...
bool WFDMMSink::createRenderer()
{
    ...
    WFDMMLOGE1("AVSync Mode = %d",sConfig.bAVSyncMode);
                                           ...
                                           )
{
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
        "WFDMMSource::getFrameResolutionRefreshRate %ld %ld %ld",
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::FBD(int handle, int moduleIdx, int trackID,
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::EBD(int handle, int moduleIdx, int trackID,
                             OMX_BUFFERHEADERTYPE *pBuffer)
{
    if(handle && pBuffer)
    {
    ...
    case WFDMMSINK_PACKETLOSS:
        {
            ...
            if(nCurrTime - m_nTimeLastIdrReq >= IDR_MIN_INTERVAL)
            {
                WFDMMLOGH("WFDMMSink IDR Callback");
                sCallBacks.idr_cb(this);
                m_nTimeLastIdrReq = nCurrTime;
                if(m_hIDRDelayTimer && !nData)
                {
                    /*----------------------------------------------------------
                    MM Timer quirks, if this is not the timer callback itself
                    delete the timer if timer has already expired
                    ------------------------------------------------------------
                    */
                    MM_Timer_Release(m_hIDRDelayTimer);
                    if(m_pMediaSrc)
                    {
                        m_pMediaSrc->receiveIDRTimerNotification(TIMER_RELEASED);
                    }
                    m_hIDRDelayTimer = NULL;
                }
            }
            ...
            {
                if(m_hIDRDelayTimer && !m_bIDRTimerPending && !nData)
                {
                    /*----------------------------------------------------------
                    MM Timer quirks, if this is not the timer callback itself
                    delete the timer if timer has already expired
                ...
                /*--------------------------------------------------------------
                   update pending IDR Req so that when timer expires it can
                   request for IDR
                ----------------------------------------------------------------
                */
                m_bPendingIDRReq = true;

                if(!m_hIDRDelayTimer)
                {
                    WFDMMLOGD("IDRreq too soon, start delay timer");
                    MM_Timer_Create(IDR_MIN_INTERVAL -
                                    (nCurrTime - m_nTimeLastIdrReq) + 1, 0,
                                    WFDMMSink::IDRDelayTimerCb,
                                    (void *)this, &m_hIDRDelayTimer);
                    if(m_pMediaSrc)
                    {
                        m_pMediaSrc->receiveIDRTimerNotification(TIMER_CREATED);
                    }
                }
                else
                {
                    WFDMMLOGD("IDRreq timer created already waiting to expire");
                }
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
bool WFDMMSink::decryptCb(int handle, int streamType, int input, int output,
                          ...
                          int size, char* pIV, int IVSize, int nStreamId)
{
    ...
    HDCPStatusType eRet = m_pHDCPManager->decrypt(pIVdata,
                              (unsigned char*)input,
                              (unsigned char*)output, size, nStreamId);
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::avInfoCb(int handle, avInfoType *pInfo)
{
    WFDMMSink *pMe = (WFDMMSink*)handle;
...
/*==============================================================================
         ...
         FUNCTION:          IDRDelayTimerCb

         DESCRIPTION:
*//**       @brief          Callback on expiry of IDR timer. Sends delayed IDR
                            request.
*//**
@par     DEPENDENCIES:
                            None
*//*
         PARAMETERS:
*//**       @param          pThis. this pointer of WFDMMSink object
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::IDRDelayTimerCb(void *pThis)
{
    WFDMMSink *pMe = (WFDMMSink *)pThis;
...
/*==============================================================================
         ...
         FUNCTION:          IDRDelayTimer

         DESCRIPTION:
*//**       @brief          Sends IDR request to RTSP layer
*//**
@par     DEPENDENCIES:
                            None
*//*
...
@par     SIDE EFFECTS:
                            None
*//*==========================================================================*/
void WFDMMSink::IDRDelayTimer()
{
    if(IS_MM_STATE_PLAY)
    {
        if(m_bPendingIDRReq)
        {
            WFDMMLOGH("IDRReq Timer Sending Delayed IDR Req");
            if(m_pMediaSrc)
            {
                m_pMediaSrc->receiveIDRTimerNotification(TIMER_EXPIRED);
            }
        }
        else
        {
            WFDMMLOGH("IDRReq Timer No pending IDR Req");
            m_bIDRTimerPending = false;
        }
    }
}
