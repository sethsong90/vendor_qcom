...
#define WFD_HDCP_OMX_ERROR_UNDEFINED         0x80001001 //same as OMX_ErrorUndefined
...
m_bKeepGoing(true)
{
    ...
    if( QC_HDCP_CP_ErrorNone == ulStatus )
    {
      ...
      if ( QC_HDCP_CP_ErrorNone == ulStatus)
      {
        ...
        if(m_ucHdcpSecondaryStreamType)
        {
        ...
        if( QC_HDCP_CP_ErrorNone != ulStatus ||
          ((QC_HDCP_CP_ErrorNone != ulRetVal)&& m_ucHdcpSecondaryStreamType))
        {
...
CWFD_HdcpCp::~CWFD_HdcpCp(void)
{
  /*De Init */
    ...
    MM_MSG_PRIO5(MM_GENERAL, MM_PRIO_HIGH,"Total HDCP Statistics: Avge Encrypt time %lld, frame size %lld,"
...
HDCP_CP_ERRORTYPE CWFD_HdcpCp::WFD_HdcpSessionInit( /*EventCbFunction EventCb*/ )
{
  ...
  /* Init the current Transmitter/Receiver */
  if( SOURCE_DEVICE == m_eDeviceType )
  {
    ulStatus = DX_HDCP_Tsmt_Init(/*EventCb*/NotifyHdcpCpEvents);
  }
  else if (PRIMARY_SINK_DEVICE == m_eDeviceType )
  {
    ulStatus = DX_HDCP_Rcv_Init(/*EventCb*/NotifyHdcpCpEvents);
  }
...
#ifdef WFD_HDCP_ENABLED
  DxUint32 ulStatus = 0;
  /* Close the current Transmitter/Receiver */
  if( SOURCE_DEVICE == m_eDeviceType )
  {
  ...
  unsigned long  ulPayloadLen )
{
       ...
       (NULL != aPESPvtHeader))
  {
    ...
    unsigned char ucHdcpStreamID =0;
  ...
  do{
    ulStatus = DX_HDCP_Tsmt_Encrypt( m_ucHdcpSessionID,
                                     ucHdcpStreamID,
                                     aPESPvtHeader,
                                     pucPESClrPayload,
                                     pucPESEncrytPayload,
...
HDCP_CP_ERRORTYPE CWFD_HdcpCp::WFD_HdcpOpenStream(
                                                   unsigned char ucContentStreamID,
                                                   unsigned char ucStreamType
                                                 )
{
  HDCP_CP_ERRORTYPE eError = QC_HDCP_CP_ErrorUndefined;
  ...
  ulStatus =  DX_HDCP_Tsmt_Open_Stream( m_ucHdcpSessionID,
                                        ucContentStreamID,
                                        m_eHdcpStreamType,
                                        &ucStreamID );
  if( STREAM_AUDIO == ucStreamType)
   ...
   unsigned long ulRetryCnt)
{
  HDCP_CP_ERRORTYPE eError = QC_HDCP_CP_ErrorUndefined;
#ifdef WFD_HDCP_ENABLED
  DxUint32 ulStatus = QC_HDCP_CP_ErrorUndefined;
  ...
  uint32 nTotalTime = ulRetryCnt * HDCP_CP_CONNECTION_TIMEOUT;
  uint32 nCurrTime = 0;
  uint32 nTempTime = 0;
      ...
      case DX_HDCP_EVENT_UNAUTHENTICATED_CONNECTION:
        {
          ...
          if(CWFD_HdcpCp::m_pEventHandlerFn)
          {
               CWFD_HdcpCp::m_pEventHandlerFn(CWFD_HdcpCp::m_pAppData,
                                 CWFD_HdcpCp::m_uModuleId,
                                 WFDMMSRC_ERROR,
                                 (OMX_ERRORTYPE)WFD_HDCP_OMX_ERROR_UNDEFINED,
                                 0);
          }
        }
        break;
...
static HDCP_CP_ERRORTYPE WFD_HdcpCpCleanUp(CWFD_HdcpCp *pHDCPContext)
{
  HDCP_CP_ERRORTYPE ulStatus = QC_HDCP_CP_ErrorUndefined;
   ...
   if(pHDCPContext->m_ucHdcpSecondaryStreamType)
   {
...
static HDCP_CP_ERRORTYPE WFD_HdcpCpReconnect(CWFD_HdcpCp *pHDCPContext)
{
  ...
  if ( QC_HDCP_CP_ErrorNone == ulStatus)
  {
    ...
    if( QC_HDCP_CP_ErrorNone != ulStatus ||
      ((QC_HDCP_CP_ErrorNone != ulRetVal)&& pHDCPContext->m_ucHdcpSecondaryStreamType))
    {
      MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_FATAL, "HDCP Open Stream Failed err"\
