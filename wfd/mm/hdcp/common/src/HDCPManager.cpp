...
{
#ifdef WFD_HDCP_ENABLED
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "HDCPManager Initialize");
...
)
{
    ...
    if(pIpAddr)
    {
        ...
        if(m_ipAddr == -1) m_ipAddr = 0;

        if(m_ipAddr)
        {
            m_ipAddrArray[3] = (DxUint8)(m_ipAddr >> 24);
            m_ipAddrArray[2] = (DxUint8)(m_ipAddr >> 16);
            m_ipAddrArray[1] = (DxUint8)(m_ipAddr >> 8);
            m_ipAddrArray[0] = (DxUint8)(m_ipAddr);
        }
    }
    m_nPortNum = portNum;
    /**-------------------------------------------------------------------------
            ...
            */ 
            ...
            else
            {
                ...
                else
                {
                    ...
                    m_ipAddrArray[3]= (addr >> 24) & 0xff;
                    m_ipAddrArray[2]= (addr >> 16) & 0xff;
                    m_ipAddrArray[1]= (addr >> 8) & 0xff;
                    m_ipAddrArray[0]=  addr & 0xff;
...
)
{
    ...
    if(m_pDisconnectSignal)
    {
        ...
        {
           MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
            "HDCPManager:teardownHDCPSession DxClose Session failed %ud",
             nStatus);
        }
        ...
        {
           MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
              "HDCPManager:teardownHDCPSession DxClose failed %ud",
               nStatus);
        }
                ...
                case m_nHDCPManListenEvent:
                {
                    ...
                    nStatus = DX_HDCP_Rpt_Init(
                     ...
                     if (m_eTopologyStatus == HDCP_UPSTREAM_CLOSE)
                     {
                          ...
                          */
                         MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                             "Disconnect issued, no need to proceed further!",
                              m_eTopologyStatus);
                         break;
                     }
                     m_eTopologyStatus = HDCP_IN_PROGRESS;
                    ...
                    {
                        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                           "Auth might have failed, do not continue %ld",
                           bContinueToDXCalls);
                    ...
                    while(m_eState == HDCP_STATE_CONNECTING)
                    {
                       ...
                       nStatus = DX_HDCP_Rpt_Listen((DxUint8 *)m_ipAddrArray,
                                                    (DxUint16)m_nPortNum);
                        if(nStatus)
                        {
                            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                                   "HDCPManager:Listen failed %ud",nStatus);
                        }
                        else
                        {
                            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,
                                   ...
                                   unsigned long nLen)
{
    ...
    if( pIn && pOut && nLen )
    {
        ...
        nStatus = DX_HDCP_Rpt_Decrypt( pesPrivateData,
                                      pIn,
                                      pOut,
                                      nLen,
                                      lStream);
                                   ...
                                   int nStreamId)
{
    ...
    else
    {
        MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_ERROR,
               "HDCPManager:Decrypt Err Invalid Args,PES Private Data"
                     " = 0x%x, pIn = 0x%x, pOut = 0x%x, pLen = %d",
                                     pesPrivateData, pIn, pOut, nLen );
    }
    m_eState = HDCP_STATE_CONNECTED;
    HDCPMAN_CRITICAL_SECT_LEAVE
...
void HDCPManager::hdcp_update_topology(void *msg1)
{
    ...
    {
       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                "HDCPManager: Rpt No need for update topo %ld",
                 m_bPropagationStatus);
