...
string  getValueForTag(string buffer, string tag)
{
   ...
   if ((begin = buffer.find(tagStart)) != string::npos)
   {
      if ((end = buffer.find(tagEnd)) != string::npos)
      {
         ...
         char *resStr = (char *)MM_Malloc(sizeof(char)*(len+1));
         if(!resStr)
         {
...
void  parseFrameSkipping(string fsBuffer)
{
   ...
   if(pReadCfgCapabilities->frameSkippingCapability.frameSkipValid)
   {
     for(int index=0; index < pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.num_h264_profiles; index++)
     {
        ...
        ( pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.h264_codec[index]).frame_rate_control_support |=(pReadCfgCapabilities->frameSkippingCapability.frameSkipIntvl << 1);
     }
   }
}
...
void  parseRTPDumpEnable(string buffer)
{
   ...
   /*Check the status of bRTPDumpEnable and appropriately set the
   key persist.sys.enable_RTPDumps to 1 or 0*/
   bRTPDumpEnable ? property_set("persist.sys.enable_RTPDumps","1")
                  : property_set("persist.sys.enable_RTPDumps","0");
...
void  parseVideoHeader(string vidheadbuffer)
{
   ...
   pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.preferred_display_mode_supported
      = (unsigned short)atoi((getValueForTag(vidheadbuffer, "PreferredDisplaySupport")).c_str());
   pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.num_h264_profiles
      = (unsigned short)atoi((getValueForTag(vidheadbuffer, "H264Profiles")).c_str());
}
...
{
   int fpsSupported, uMaxHRes, uMaxVRes, nativeModes = 0 ;
   ...
   (pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.h264_codec[index]).h264_profile = (unsigned int )atoi((getValueForTag(videoBuffer, "Profile")).c_str());
/*    = profile.compare("CBP")?
...
void  parseAudioCodec(string audioBuffer, audioCodecName type)
{
   switch(type)
   {
      case LPCM:
      {
         ...
         if( valid && ( samplingFreq == LPCM_MODE_FREQ_1 || samplingFreq == LPCM_MODE_FREQ_2 ) && ( channels == AUDIO_MIN_CHANNELS ) )
         {
            ...
            pReadCfgCapabilities->pCfgCapabilities->audio_config.lpcm_codec.decoder_latency = (unsigned short)atoi((getValueForTag(audioBuffer, "Latency")).c_str());
         }
         else
         {
      ...
      case AAC:
      {
                         ...
                         && ( ( channels%2 ) == 0 ) ) )
         {
            ...
            (unsigned short)atoi((getValueForTag(audioBuffer, "Latency")).c_str());
         }
         else
         {
            // Invalid AAC Values
   ...
   {
      // AAC is of priority check if it is set and assign the audio method to it
      if(pReadCfgCapabilities->pCfgCapabilities->audio_config.aac_codec.supported_modes_bitmap)
         pReadCfgCapabilities->pCfgCapabilities->audio_method = WFD_AUDIO_AAC;
      // AC3 is 2nd priority
      else if(pReadCfgCapabilities->pCfgCapabilities->audio_config.dolby_digital_codec.supported_modes_bitmap)
...
void  parseCfg(const char *filename, readConfigFile *readCfgCaps)
{
   ...
   {
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"parserCfg:open config file successful");
   }
   else
   {
   ...
   buffer = (char *) MM_Malloc(sizeof(char)*length);
   if(!buffer)
   {
...
void ParseCfgForBitrate(int mode, int bit, char *pFilename, int *minBitrate, int *maxBitrate)
{
    ...
    *maxBitrate = 0;
    ...
    pFileBuf = (char*)MM_Malloc(nSize + 4);
...
void updateVar(const char* key, const cfgKeys cfgKey, const string keyValue)
{
  if(keyValue.length() && key)
  {
    ...
    if(ret.second == false)
    {
        MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_ERROR,"updateVar:: Element %s already exists with %d",(ret.first->first).c_str(),ret.first->second.second);
    }
    else
    {
        ...
        if(cfgItems.size() && cfgItemsStr.size())
        {
            ...
            {//Key is present in map
                *pVal = cfgItemsStr[(string(key))].second;
                MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_ERROR,"getCfgItem:: Value of %s is %d",key,*pVal);
                return 0;
            }
            else
            {
...
void  dumpcapabilitiesRead()
{
   ...
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "   AC3 Bitmap = %lu",  pReadCfgCapabilities->pCfgCapabilities->audio_config.dolby_digital_codec.supported_modes_bitmap);
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "  LPCM Bitmap = %lu",  pReadCfgCapabilities->pCfgCapabilities->audio_config.lpcm_codec.supported_modes_bitmap);
   ...
   for(int index=0; index < pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.num_h264_profiles; index++)
   {
     ...
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, " CEA Bitmap = %lu", ( pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.h264_codec[index]).supported_cea_mode);
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, " VESA Bitmap = %lu", ( pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.h264_codec[index]).supported_vesa_mode);
     MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "  HH Bitmap = %lu", ( pReadCfgCapabilities->pCfgCapabilities->video_config.video_config.h264_codec[index]).supported_hh_mode);
   ...
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "IDR Request Intvl = %lu",pReadCfgCapabilities->idrRequestCapability.idrIntvl);
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Frame Skip Valid  = %d",pReadCfgCapabilities->frameSkippingCapability.frameSkipValid);
   MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "Frame Skip Intvl  = %lu",pReadCfgCapabilities->frameSkippingCapability.frameSkipIntvl);
