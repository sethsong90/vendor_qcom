...
int getP2pInterfaceName(char* pInterfaceName, int size)
{
    ...
    ifc.ifc_len = sizeof(struct ifreq) * MAX_IFS;
    ifc.ifc_req = ifs;
...
int getLinkSpeed(char *macaddr, unsigned int len, bool& queryIfName) {
    ...
    iwrrequest.u.data.length = strlen((char*)inputstring);
    ...
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
                 "NetUtils:Linkspeed result = %d, linkspeed = %d",
                    ...
                    int *nPort1, int *nPort2, bool tcp)
{
   ...
   if(bPair)
   {
            ...
            portNum < RTP_PORT_END_NUM; portNum+=2)
       {
           ...
           ret = bind(RTPSocket, (sockaddr *)&saddr, sizeof(saddr));
           ...
           else
           {
               ...
               ret = bind(RTCPSocket, (sockaddr *)&saddr, sizeof(saddr));
               if(ret < 0)
               {
                   close(RTPSocket);
                   close(RTCPSocket);
   ...
   else
   {
       ...
       ret = getsockname(RTPSocket, (struct sockaddr *)&saddr, (socklen_t*)(&size));
       if(ret == 0)
       {
           if(nPort1)
           {
...
int getLocalIpSocket (int sock, char* IP)
{
    ...
    int ret = getsockname(sock, (struct sockaddr *)&saddr, (socklen_t*)(&size));
    if(ret == 0)
    {
