...
#undef LOG_TAG
#endif
#define LOG_TAG "MMCapability_CPP"
#define LOG_NDEBUG  0
...
void MMCapability::getResolutionFromBitmap(WFD_h264_codec_config_t* pCapability_h264_codec, uint32 numH264)
{
    ...
    MM_MSG_PRIO3(MM_GENERAL,MM_PRIO_HIGH,"MMCapability::getResolutionRefreshRate %lu %lu %lu",cea_mode, vesa_mode, hh_mode);
...
char* MMCapability::getKeyValue(char* pKey) {
    ...
    if (strcmp(pKey, "wfd_audio_codecs") == 0) {
        ...
        if (pCapability->audio_config.lpcm_codec.supported_modes_bitmap != 0) {
            snprintf(audioStr,256, "LPCM %.8lx %.2x", pCapability->audio_config.lpcm_codec.supported_modes_bitmap, pCapability->audio_config.lpcm_codec.decoder_latency);
        ...
        if (pCapability->audio_config.dolby_digital_codec.supported_modes_bitmap != 0) {
            ...
            snprintf(audioStr,256, "AC3 %.8lx %.2x", pCapability->audio_config.dolby_digital_codec.supported_modes_bitmap, pCapability->audio_config.dolby_digital_codec.decoder_latency);
            strlcat(strABNF, audioStr, sizeof(strABNF));
        }
        return strABNF;
    }
    ...
    if (strcmp(pKey, "wfd_video_formats") == 0) {
        ...
        for (int i=0; i<pCapability->video_config.video_config.num_h264_profiles; i++) {
            ...
            snprintf(videoStr, 256, "%.2x %.2x %.8lx %.8lx %.8lx %.2x %.4x %.4x %.2x ",
...
bool MMCapability::setKeyValue(char* pKey, char* pValue) {
    ...
    if (strcmp(pKey, "wfd_video_formats") == 0) {
        ...
        while (pch != NULL) {
            ...
            MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"MMCapability_CPP: setKeyValue cea %lu",pCapability->video_config.video_config.h264_codec[numH264].supported_cea_mode);
            MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"MMCapability_CPP: setKeyValue hh  %lu",pCapability->video_config.video_config.h264_codec[numH264].supported_hh_mode);
            MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_HIGH,"MMCapability_CPP: setKeyValue vesa %lu",pCapability->video_config.video_config.h264_codec[numH264].supported_vesa_mode);
...
void MMCapability::configure(rtspWfd &RtspWfd) {
    ...
    LOGD("Configure Peer MM Capability using received RTSP wfd");
    ...
    if (audioStr.length() > 0) {
        LOGD("Configure wfd_audio_codecs...  %s", audioStr.c_str());
    ...
    if (videoStr.length() > 1) {
        LOGD("Configure wfd_video_formats...  %s", videoStr.c_str());
    ...
    if (RtspWfd.uibcCap.getValid()) {
        ss.str("");
        ss << RtspWfd.uibcCap;
        //LOGD("configure %s: %s", RtspWfd.uibcCap.getName().c_str(), ss.str().c_str());
        uibcStr = ss.str();
    }

    if (uibcStr.length() > 0) {
        LOGD("Configure wfd_uibc_capability...  %s", uibcStr.c_str());
