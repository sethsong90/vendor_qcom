      ...
      switch( *pEvent )
      {
        case THREAD_CONNECT_SIGNAL:
        {
          if (m_eUIBCState == UIBC_INIT)
          {
            ...
            {
              int32 duration = 100;
              MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, " UIBCPacketTransmitter:Connect failed. Trying to connect again after %ld ms", duration);
              if(m_pConnectTimer == NULL)
              {
        ...
        default:
        {
          MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "UIBCPacketTransmitter:Invalid signal %lu", *pEvent);
        }
      }
    }
  }
...
int32 UIBCPacketTransmitter::makeConnection()
{
   ...
   MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:make connection with port %hu, IP %lu", m_nSourcePort, m_nSourceIPaddr);

   if ( 0 != connect( m_nSocket, (struct sockaddr*)&addr, sizeof(addr)) )
   {
     connectStatus = errno;
     MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR, " UIBCPacketTransmitter:socket connect failed with error %ld %s",
     ...
     if(connectStatus == EINPROGRESS)
     {
       ...
       int32 selectStatus = select(m_nSocket+1, NULL, &fdWriteSet, NULL, &timeout);
       MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM, " UIBCPacketTransmitter:select return status %ld",selectStatus);
...
void UIBCPacketTransmitter::processEventsList()
{
  ...
  while(qmm_ListIsEmpty(&m_hEventsList) != 1)
  {
    ...
    if(pLink != NULL)
    {
      ...
      {
         pLink->event.parms.zoom_event.coordinate_x *= m_negotiated_height;
         pLink->event.parms.zoom_event.coordinate_y *= m_negotiated_width;
      ...
      if(pLink->event.type == WFD_UIBC_TOUCH)
      {
         ...
         {
             pLink->event.parms.touch_event.coordinate_x[pointerIndex] *= m_negotiated_width;
             pLink->event.parms.touch_event.coordinate_y[pointerIndex] *= m_negotiated_height;
             pointerIndex++;
         }
      ...
      if(packetLength > 0)
      {
        ...
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "UIBCPacketTransmitter:sendPacket:sent %ld bytes", numBytesSent);
      }
      else
      {
...
int32 UIBCPacketTransmitter::sendPacket(uint8* pUIBCPacket, int32 packetLength )
{
  ...
  if( nBytesSent < packetLength)
  {
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "UIBCPacketTransmitter:Error %ld in sending packet", errorCode);
    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR, "UIBCPacketTransmitter:Number of bytes requested for send %ld, actual bytes send %ld",packetLength, nBytesSent);
  }
...
int32 UIBCPacketTransmitter::createSocket()
{
  ...
  else
  {
    ...
    flags = 1;
    setsockopt(nSocket,IPPROTO_TCP,TCP_NODELAY,(char*)&flags,sizeof(int));
  }
  return nSocket;
}
