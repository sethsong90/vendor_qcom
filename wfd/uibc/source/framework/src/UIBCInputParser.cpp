                                           ...
                                           WFD_uibc_capability_t *pNegotiatedCapability)
{
  ...
  uint16 usLength = uibcPacket[index]<<8|uibcPacket[index+1];
  index += 2;
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                 "UIBCInputParser:ParseGenericInput  Generic input type %d",ucIeType);

    MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH, "UIBCInputParser:ParseGenericInput UIBC negotiated_height %lu  negotiated_width %lu",
  ...
  case UIBC_GENERIC_ID_TOUCH_MOVE:
    ...
    for (int i = 0; i < inputEvent->parms.touch_event.num_pointers;i++)
    {
      ...
      inputEvent->parms.touch_event.coordinate_x[i]/= pNegotiatedCapability ->negotiated_width;
      index+=2;
      ...
      inputEvent->parms.touch_event.coordinate_y[i]/= pNegotiatedCapability ->negotiated_height;
      index+=2;
  ...
  case UIBC_GENERIC_ID_KEY_UP:
    ...
    inputEvent->parms.key_event.key_code_1 = uibcPacket[index]<<8|uibcPacket[index+1];
    index+=2;
    inputEvent->parms.key_event.key_code_2 = uibcPacket[index]<<8|uibcPacket[index+1];
    index+=2;
    break;
  case UIBC_GENERIC_ID_ZOOM:
    ...
    if(pNegotiatedCapability ->negotiated_height != 0)
    inputEvent->parms.zoom_event.coordinate_x /= pNegotiatedCapability ->negotiated_width;
    index+=2;
    inputEvent->parms.zoom_event.coordinate_y  = uibcPacket[index]<<8|uibcPacket[index+1];
    if(pNegotiatedCapability ->negotiated_width != 0)
    inputEvent->parms.zoom_event.coordinate_y /= pNegotiatedCapability ->negotiated_height;
    index+=2;
  ...
  case UIBC_GENERIC_ID_SCROLL_HORIZONTAL:
    ...
    inputEvent->parms.scroll_event.num_pixels_scrolled  = uibcPacket[index]<<8|uibcPacket[index+1];
    index+=2;
    break;
                                       ...
                                       uint8&  HIDType)
{
  ...
  usLength        = uibcPacket[index]<<8|uibcPacket[index+1];
  index += 2;
