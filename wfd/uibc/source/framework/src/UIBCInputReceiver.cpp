...
void UIBCInputReceiver::Init()
{
  ...
  {
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
                 "UIBCInputReceiver:Error in creating thread = %ld",
                 threadStatus);
    return;
  }
...
int32 UIBCInputReceiver::CreateSocket(int16 port)
{
  ...
  if(nSocket > 0)
  {
    ...
    int reuseoption = 1;
    if(0 != setsockopt(nSocket,SOL_SOCKET,SO_REUSEADDR,&reuseoption,sizeof(reuseoption)))
    {
    ...
    if( 0 != bind(nSocket, (const struct sockaddr *)&addr, sizeof(addr)))
    {
...
int32 UIBCInputReceiver::SetSocketOptions(int32 socket)
{
...
#ifndef WIN32
  socklen_t optionLength = sizeof(recvBufferSize);
  ...
  if( recvBufferSize < (UIBC_PACKET_MAXIMUM_SIZE * 2) )
  {
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,
                 "UIBCInputReceiver:Setting socket receive buffer size %ld",
                 recvBufferSize);
...
int32 UIBCInputReceiver::ReceiveUIBCPacket(uint8* Buffer, uint32 nFetchSize)
{
  ...
  MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_HIGH,
               "UIBCInputReceiver:ReceiveUIBCPacket:start readOffset %ld,write Offset %ld",
  ...
  MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
               "UIBCInputReceiver:before recvFrom readOffset %ld,write Offset %ld, nFetchSize %ld",
                m_packetReadIndex, m_packetWriteIndex, nFetchSize);

#ifndef WIN32
  nBytes = recvfrom(socket, Buffer + m_packetWriteIndex + 1, nFetchSize,
    flags, NULL, NULL);
#else
...
#endif
  MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
               "UIBCInputReceiver:recvfrom packet length recvd %ld",
               nBytes);
  if (nBytes > 0)
  {
  ...
  MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_HIGH,
               "UIBCInputReceiver:After recvFrom readOffset %ld,write Offset %ld,nBytes %ld",
...
int32  UIBCInputReceiver::AcceptTCPConnection(int32 tcpSocket)
{
  ...
  if( 0 == listen(tcpSocket, 1) )
  {
   ...
   // fcntl(tcpSocket, F_SETFL, O_NONBLOCK);
    ...
    {
      acceptedSocket = accept(tcpSocket,(sockaddr*) &addr, (socklen_t*)&addrsize);
      if( acceptedSocket >= 0)
      {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,
                     "UIBCInputReceiver:Accepted connection with socket %ld",
                     acceptedSocket);
        m_bListenForConn = false;
        break;
      }
...
boolean UIBCInputReceiver::ParseAndInjectInput()
{
  while (m_packetWriteIndex > (m_packetReadIndex+ UIBC_PACKET_HEADER_LEN ))
  {
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"UIBCInputReceiver:ParseNinjectInput before parsing readIndex %ld",m_packetReadIndex);
  //Sanity Check
    if (((m_UIBCPacket[m_packetReadIndex]&0xE0)>>5) != UIBC_WFD_VERSION)
  {
    ...
    m_packetReadIndex++;
    uint16 size = m_UIBCPacket[m_packetReadIndex]<<8|m_UIBCPacket[m_packetReadIndex+1];
    ...
    if (size > availableSize)
    {
      ...
      MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_MEDIUM,"UIBCInputReceiver:ParseNinjectInput less data %ld is available for total size %d",availableSize, size);
      m_packetReadIndex -=2;
      return FALSE;
    }
    m_packetReadIndex+=2;
      ...
      MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,
        ...
        if(len >= 0 && m_packetReadIndex >= 0 && ((m_packetReadIndex + len)< UIBC_PACKET_MAXIMUM_SIZE) && m_pHIDInjector){
           ...
           if(usageType == HID_REPORT) {
            ...
            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR,
               "!!!UIBCInputReceiver:Out of bound access! due to index %ld or rogue length %d", m_packetReadIndex, len);
            return FALSE;
        }
#endif
    break;
  ...
  default:
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,"UIBCInputReceiver:ParseNinjectInput after parsing readIndex %ld",m_packetReadIndex);
  } // end while
  return TRUE;
}
