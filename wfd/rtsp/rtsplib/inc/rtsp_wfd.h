...
static const int numSupportedWfdParams = (sizeof(wfdTable) / sizeof(wfdTable[0]));
...
static const int numSupportedInputs = (sizeof(inputTable) / sizeof(inputTable[0]));
static const int numSupportedPaths = (sizeof(pathTable) / sizeof(pathTable[0]));
...
class clientPorts {
public:
    clientPorts() : profile(""), rtpPort0(0), rtcpPort0(0), rtpPort1(0), valid(false), name("ClientRtpPorts"), tcp(false), renegotiated(false) {}
    clientPorts(unsigned port0, unsigned port1) : profile(""), rtpPort0(port0), rtpPort1(port1), rtcpPort0(0), valid(true), tcp (false), renegotiated(false),name("ClientRtpPorts") {}
...
public:
    ...
    rtspWfd(audioCodec lpcm,
            ...
            displayEdid edid,
            idrRequest idr) :
            audioLpcm(lpcm), audioAac(aac), audioEac(eac), audioDts(dts),
            h264Cbp(cbp), h264Chp(chp), h264Chi444p(chi444p),contentProtection(cp),
            uri(uri), client(client), uibcCap(cap), uibcSet(set), standbyCap(SRCap),coupledSink(couple),
            audioRoute(rte), timing(tmg), edid(edid), idrReq(idr), isPrimarySink(false),
            isSecondarySink(false) {}
