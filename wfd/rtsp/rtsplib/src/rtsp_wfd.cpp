...
RTSPStringStream &operator>>(RTSPStringStream &stream, audioCodec &audio)
{
    ...
    stream >> latS;
    audio.latency = stringToNumHex(latS);
...
RTSPStringStream &operator>>(RTSPStringStream &stream, h264Codec &video)
{
    ...
    if (hResS != none) {
        video.maxHres = stringToNumHex(hResS);
    } else {
        video.maxHres = 0;
    }
    if (vResS != none) {
        video.maxVres = stringToNumHex(vResS);
    } else {
        video.maxVres = 0;
    }
    ...
    video.latency = stringToNumHex(latencyS);
    video.minSliceSize = stringToNumHex(minS);
...
{
    unsigned res = videoHeader.getNativeRes();
...
void rtspWfd::wfdParse(rtspWfdParams param, string line)
{
    ...
    {
        string data = findStart(listStr, '=');
        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB :: Inside wd_playback_control");
        ...
        if(control == "play") {
            ...
            MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB :: wd_playback_control flush_timing found = %llu",flush_timing);
...
void rtspWfd::parseAudioXml(string buffer, audioCodecName type)
{
    ...
    case lpcm:
    {
        ...
        unsigned latency = (unsigned short)atoi((getNvp(buffer, "Latency")).c_str());
    ...
    case dts:
    {
        string name = getNvp(buffer, "Name");
        bitset<BITSET_8> modes(getNvp(buffer, "Modes"));
        unsigned short latency = atoi((getNvp(buffer, "Latency")).c_str());
        bool valid = atoi((getNvp(buffer, "Valid")).c_str()) ? true : false;
