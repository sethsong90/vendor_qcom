...
{
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: isKeepAlive : Entered");
  if(!m_brecvKeepAlive)
  {
  ...
  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: isKeepAlive : Exiting");
}
...
void rtspBaseState::releaseKeepAliveTimer()
{
  if(m_hTimer)
  {
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: Releasing keepalive timer");
        ...
        if (isAck(tmp)) {
            if (tmp->valid & wfdValid) {
                if (tmp->state.wfdSupp == false) {
                  ...
                  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTSP_LIB :: Error: WFD not supported");
                  if (parsed.next)
                     paramsFree(parsed.next);
                  return;
                }
...
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "RTSP_LIB :: %s: Invalid Source Request", name.c_str());
}
...
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,  "RTSP_LIB ::  %s: Invalid Source Response", name.c_str() );
}
...
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH,  "RTSP_LIB ::  %s: Invalid Sink Request", name.c_str() );
}
...
void rtspM3::response(rtspFSM *s, rtspSink *b)
{
    ...
    b->processParsedMesg(&parsed);
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "RTSP_LIB :: %s: globalError = %d",globalError);
    ERROR_CHECK;
...
void rtspM7::response(rtspFSM *s, rtspSink *b)
{
    ...
    while (tmp) {
        if (tmp->mesg.subCmd == invalidCmd) {
            if(tmp->mesg.cmd == getParameterCmd)
            {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_LOW, "RTSP_LIB :: rtspM7::response : received keepalive");
                m_brecvKeepAlive = true;
                return;
            }
                ...
                b->instance->recvCmdApi(playCmd, b->session, noError);
                if (m_hTimer == NULL) {
                  MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB :: keepAliveTimerValue timer created");
...
{
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "RTSP_LIB ::  %s: Invalid Sink Response", name.c_str() );
}
