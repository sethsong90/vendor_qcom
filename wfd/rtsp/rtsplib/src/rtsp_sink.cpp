...
void rtspSink::getIntersect(bitset<WFD_MAX_SIZE> parsedWfdBitmap)
{
    ...
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::getIntersect standbyCap "\
                                           "parsed = %d StandbyBitChk = %d" \
                                           "isect.standbyCap = %d",
                                           parsedWfdBitmap,
                                           standbyBitChk,
...
void rtspSink::applySettings(rtspParams *parsed)
{
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::parsed wfd = %d",parsed->state.wfd.getValidWfd());
    if (parsed->valid & sessionValid) {
        ...
        if(timeoutStr != NULL)
        {
          int size = (timeoutStr - str)/sizeof(char);
    ...
    if (parsed->mesg.cmd == setParameterCmd &&
        ...
        if (parsed->state.wfd.client.getValid() && getFSMState()!= M4 && getFSMState()!= M3) {
          MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::applySettings client port update");
        ...
        if(isect.standbyCap.getValid() && getFSMState()== M4) {
          ...
          parsed->state.wfd.standbyCap.setValid(true);
           MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::applySettings parsed wfd = %d",parsed->state.wfd.standbyCap.getValid());
          }
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,"RTSP_LIB ::parsed isect wfd = %d",parsed->state.wfd.getValidWfd());
