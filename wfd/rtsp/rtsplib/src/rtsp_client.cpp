...
void rtspClient::setupClientSocket(string ipaddr)
{
    ...
    memset((void*)&saddr, sizeof(sockaddr_in), 0);
    ...
    if (!globalError)
        {
            MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "rtsp_client Connected");
        }
    else
        {
        ...
        int nRetVal;
        int retryCount=0;
        nRetVal = PargeCfgForIntValueForKey(WFD_CFG_FILE, (char *)"RetryCount",
                                      (int*)(&retryCount));
...
*/
                while (rLen >= sizeof(rtspApiMesg)) {
                    ...
                    break;
                    default:
                        MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH, "RTSP_LIB :: Unsupported cmd");
                    break;
                    }
