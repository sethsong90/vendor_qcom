        ...
        for (int i = 1; i < numSupportedWfdParams; i++) {
            if (GetP->mesg.wfdParams[i]) {
                ...
                case wfd_connector_type:
                   MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_ERROR,"RTSP_LIB :: ConnectorType = %d ",state->state.wfd.connectorType);

                   ss << wfdTable[i].wfdName << ": " << "0" << state->state.wfd.connectorType << crlf;
                break;
#ifndef LEGACY_TCP
                case wd_initial_buffer:
                    MM_MSG_PRIO(MM_GENERAL,MM_PRIO_ERROR,"RTSP_LIB :: wd_initial_buffer ");
