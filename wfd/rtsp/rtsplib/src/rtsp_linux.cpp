...
SOCKET SOCK()
{
    ...
    else
    {
        ...
        setsockopt(retval, SOL_IP, IP_TOS, &prio, sizeof(prio));
        struct timeval tv;
        tv.tv_sec = 5;
        tv.tv_usec = 0;
        setsockopt(retval, SOL_SOCKET, SO_SNDTIMEO,(struct timeval *)&tv,sizeof(struct timeval));
        setsockopt(retval, SOL_SOCKET, SO_RCVTIMEO,(struct timeval *)&tv,sizeof(struct timeval));
    }
    return retval;
}
        ...
        if(EINPROGRESS == errno || EWOULDBLOCK == errno)
        {
          ...
          if (selectStatus > 0) {
            if(FD_ISSET(sock, &fdReadSet))
            {
              ...
              if(errorNum == 0)
              {
                MM_MSG_PRIO(MM_GENERAL, MM_PRIO_ERROR, "RTSP_LIB :: connect :: connection established");
                globalError = 0;
                return;
              }
              else
