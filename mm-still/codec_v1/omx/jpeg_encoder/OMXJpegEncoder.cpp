  ...
  QIBuffer *aOutputBuffer)
{
  ...
  if (enable_mobicat > 0) {
    ...
    if (mobicatStr == 0) {
       QIDBG_ERROR("%s:%d] Error in composing mobicat string", __func__,
         __LINE__);
     ...
     m_exifParams.SetMobicatFlag(enable_mobicat);
...
int OMXJpegEncoder::CompleteMainImage()
{
  ...
  if (!m_abort_flag && (OMX_FALSE == m_releaseFlag)) {
    ...
    if (NULL != m_memOps.get_memory) {
      ...
      jpeg_out->size = getEstimatedExifSize() + m_outputMainImage->FilledLen();
      ...
      if ((m_thumbnailInfo.input_height != 0) &&
        ...
        lret = writeExifData(m_outThumbImage, &lOutBuf);
      } else {
        lret = writeExifData(NULL, &lOutBuf);
      }
      if (lret != OMX_ErrorNone) {
      ...
      memcpy(lOutBuf.Addr() + lOutBuf.FilledLen(),
