...
typedef struct jpeg_encoder_t
{
    ...
    os_timer_t             timer;
#endif
    /* Pending thread for destroy */
    os_thread_t            pending_thread_id;
...
int jpege_get_default_config(jpege_cfg_t  *p_config)
{
    ...
    STD_MEMSET(p_config, 0, sizeof(jpege_cfg_t));
    ...
    int32_t       mobicat_data_length)
{
    ...
    JPEG_DBG_LOW("%s,%d,mobicat_data_length=%d\n",__func__,__LINE__,mobicat_data_length);
    ...
    exif_info_obj_t  *p_exif_info)
{
    ...
    if (JPEG_SUCCEEDED(rc))
    {
        ...
        if (p_cfg->thumbnail_present && p_encoder->p_thumbnail_buf)
        {
            uint32_t thumbnail_size;
            JDBG("value is %p", p_encoder->source.p_thumbnail);
            ...
            if (JPEG_SUCCEEDED(rc))
            {
                ...
                rc = jpege_try_engines(obj,
                        JPEG_ENCODER_PREF_SOFTWARE_ONLY);
                if (!JPEG_SUCCEEDED(rc)) {
                    JPEG_DBG_HIGH("jpege_try_engines failed");
                    os_mutex_unlock(&(p_encoder->mutex));
                    return rc;
                }
                rc = jpeg_buffer_allocate(
                        (jpeg_buffer_t) p_encoder->p_thumbnail_buf,
                        thumbnail_size, false);
                if (JPEG_SUCCEEDED(rc)) {
                    rc = p_encoder->engine.start(&(p_encoder->engine),
...
void jpege_destroy(jpege_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_encoder)
        {
            ...
            if (p_encoder->p_thumbnail_buf)
            {
            /* wait for main thread to exit before destoying the jpeg object*/
    ...
    void               *p_arg)  // An optional argument for that event.
{
    ...
    case JPEG_EVENT_DONE:
        ...
        if (p_encoder->state == JPEGE_ENCODING_THUMBNAIL)
        {
            ...
            p_encoder->pending_thread_id = os_thread_self();
            JPEG_DBG_MED("pending thread %d", (int)p_encoder->pending_thread_id);

            if (JPEG_SUCCEEDED(rc))
            {
                 rc = jpege_try_engines(p_obj->encoder, p_encoder->config.preference);
                 JPEG_DBG_MED("%s:jpege_try_engines succeded with preference %d",__func__,p_encoder->config.preference);

                if (JPEG_SUCCEEDED(rc))
                {
                    JPEG_DBG_ERROR("echoes %p %p %p %p", p_encoder->config,
                              p_encoder->source,  p_encoder->engine_dst,p_encoder->engine_dst.p_buffers);
                    rc = p_encoder->engine.start(&(p_encoder->engine), &(p_encoder->config.main_cfg),
                              p_encoder->source.p_main, &(p_encoder->engine_dst));
    ...
    uint8_t             last_buf_flag) // The flag to indicate last output buffer
{
    ...
    if (p_encoder->state != JPEGE_ENCODING_THUMBNAIL)
    {
        if (p_encoder->config.target_filesize)
        {
            ...
            p_buf->offset = 0;
            // Set reference to output buffer
...
static int jpege_validate_image_data(jpege_img_data_t *p_img)
{
    ...
    if ((p_img->color_format > MONOCHROME) && (p_img->color_format < JPEG_BITSTREAM_H2V2))
    {
    ...
    if (p_img->fragment_cnt < 1 || p_img->fragment_cnt > MAX_FRAGMENTS)
        return JPEGERR_EBADPARM;
    if (p_img->color_format == MONOCHROME)
    {
    ...
    if (p_img->color_format <= YCBCRLP_H1V1)
    {
        for (i = 0; i < p_img->fragment_cnt; i++)
        {
            JPEG_DBG_ERROR("echoes in validate image dta %p ",
                           p_img->p_fragments[i].color.yuv.luma_buf );
            if (p_img->p_fragments[i].num_of_planes == 2)
            {
                if (!((jpeg_buf_t *)(p_img->p_fragments[i].color.yuv.luma_buf))->ptr ||
            ...
            else if (p_img->p_fragments[i].num_of_planes == 3)
            {
                if (!((jpeg_buf_t *)(p_img->p_fragments[i].color.yuv.luma_buf))->ptr ||
                    !((jpeg_buf_t *)(p_img->p_fragments[i].color.yuv.chroma_buf))->ptr ||
                    !((jpeg_buf_t *)(p_img->p_fragments[i].color.yuv.cr_buf))->ptr)
                {
                    JPEG_DBG_ERROR("jpege_validate_image_data: broken fragment (%d)\n", i);
                    return JPEGERR_EBADPARM;
                }
