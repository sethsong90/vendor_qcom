...
static void jpege_engine_sw_downscale_vertical_chroma_line(
    ...
    uint8_t **pp_output_line,
    jpege_scale_t *p_scale_info);
//TODO Following decl required?
// jpeg fetch to line buffer funtion ptr
static void (* jpege_engine_sw_downscale_fetch_luma_to_line_buffer_func)(
	jpege_scale_t *p_scale_info);
static void (* jpege_engine_sw_downscale_fetch_chroma_to_line_buffer_func)(
	jpege_scale_t *p_scale_info);
    ...
    uint32_t  output_width)
{
    ...
    /* set the table for 16 points */
    static const uint16_t div_q10[17] = {1024, 1024, 512, 341, 256, 205, 171,
      146, 128, 113, 102, 93, 85, 78, 73, 68, 64};
	...
	while (curr_index --)
	{
        ...
        if (count >= crop_width)
        {
			...
			*(p_output_line ++) = (uint8_t)((hori_luma_accumulator * div_q10[step])
				                             >> DIGIT_Q10);
    ...
    jpege_scale_t *p_scale_info)
{
	...
	if (p_scale_down->vert_luma_count >= crop_height)
    {
		...
		{
			// average of N/M  values of the input
			*(p_output_line ++) = (uint8_t)(((*(p_vert_luma_accum ++)) * div_q10[vert_luma_step])
				                               >> DIGIT_Q10);
		}
		// reset accumulator
		...
		// update index
        p_scale_down->vert_luma_row_index ++;
		// reset step
	...
	uint32_t  output_chroma_width)
{
	...
	while (curr_index --)
	{
        ...
        {
			// average of N/M  values of the input
			*(p_output_line ++) = (uint8_t)((horiCbAccumulator * div_q10[step])
				                             >> DIGIT_Q10);

			*(p_output_line ++) = (uint8_t)((horiCrAccumulator * div_q10[step])
				                             >> DIGIT_Q10);
    ...
    jpege_scale_t *p_scale_info)
{
	...
	if (p_scale_down->vert_chroma_count >= crop_chroma_height)
    {
		...
		while (curr_index --)
		{
			*(p_output_line ++) = (uint8_t)(((*(p_vert_chroma_accum ++)) * div_q10[vert_chroma_step])
				                             >> DIGIT_Q10);
		...
		// update index
        p_scale_down->vert_chroma_row_index ++;
        // reset step
	...
	jpege_scale_t *p_scale_info)
{
	...
	p_scale_info->scale.down.vert_luma_row_index = 0;
	p_scale_info->scale.down.vert_chroma_row_index = 0;
