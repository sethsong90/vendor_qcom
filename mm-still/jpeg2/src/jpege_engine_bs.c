...
extern jpege_engine_profile_t jpege_engine_bs_profile = {jpege_engine_bs_create, jpege_engine_bs_name};
    ...
    jpege_engine_event_handler_t  p_handler)
{
    ...
    STD_MEMSET(p_engine, 0, sizeof(jpege_engine_bs_t));  // Zero out the entire structure
    ...
    jpege_engine_dst_t   *p_dest)
{
    ...
    rc = os_thread_create(&p_engine->thread,  jpege_engine_bs_thread, (void *)p_engine);
                                     ...
                                     jpege_engine_dst_t *p_dest)
{
    ...
    p_engine->bitstream_size =
        ((jpeg_buf_t *)p_source->p_fragments[0].color.bitstream.bitstream_buf)->size;//offset;
    ...
    if ((p_config->rotation_degree_clk > 0) || (p_config->scale_cfg.enable))
    {
        ...
        (void)STD_MEMSET(&source, 0, sizeof(source));
