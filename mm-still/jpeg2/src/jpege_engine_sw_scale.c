...
#define DOWNSCALE_MAX_FACTOR_DIGIT  4 // support up to 1/8 downscale
                                            ...
                                            jpege_img_data_t   *p_source)
{
    ...
    switch (p_engine->rotation)
    {
        case 0:
        {
            ...
            p_scale_info->rotated_luma_fetch_increment = p_source->width;
            p_scale_info->rotated_luma_src_increment = p_source->width;
            p_scale_info->rotated_chroma_fetch_increment
                = p_scale_info->rotated_luma_fetch_increment / rotated_ratio;
            p_scale_info->rotated_chroma_src_increment
                = p_scale_info->rotated_luma_src_increment / rotated_ratio;
            break;
        }
        case 90:
        {
            ...
            p_scale_info->rotated_chroma_fetch_increment
                = (int32_t)p_scale_info->rotated_luma_fetch_increment / rotated_ratio;
        ...
        case 180:
        {
            ...
            p_scale_info->rotated_chroma_src_increment
                = (int32_t)p_scale_info->rotated_luma_src_increment / rotated_ratio;
            break;
        }
        case 270:
        {
    ...
    if (p_scale_info->crop_flag == true)
    {
        ...
        switch (p_engine->rotation)
        {
            case 0:
            {
                p_scale_info->p_luma_input += p_source->width * crop_top + crop_left;
                p_scale_info->p_chroma_input += 2 * chroma_width * chroma_crop_top
                                                           + 2 * chroma_crop_left;
                break;
            }
            case 90:
            {
                p_scale_info->p_luma_input += p_source->width * crop_bottom + crop_left;
                p_scale_info->p_chroma_input += 2 * chroma_width * chroma_crop_bottom
                                                           + 2 * chroma_crop_left;
                break;
            }
            case 180:
            {
                p_scale_info->p_luma_input += p_source->width * crop_bottom + crop_right;
                p_scale_info->p_chroma_input += 2 * chroma_width * chroma_crop_bottom
                                                           + 2 * chroma_crop_right;
                break;
            }
            case 270:
            {
                p_scale_info->p_luma_input += p_source->width * crop_top + crop_right;
                p_scale_info->p_chroma_input += 2 * chroma_width * chroma_crop_top
                                                           + 2 * chroma_crop_right;
                break;
            }
    ...
    else
    {
        ...
        switch ( p_engine->rotation)
        {
            ...
            case 90:
            {
                p_scale_info->p_luma_input += p_source->width * (p_source->height - 1);
                p_scale_info->p_chroma_input += chroma_width * 2 * (chroma_height - 1);
                break;
            }
            case 180:
            {
                p_scale_info->p_luma_input += p_source->width * p_source->height - 1;
                p_scale_info->p_chroma_input += (chroma_width * chroma_height - 1) * 2;
                break;
            }
            case 270:
            {
