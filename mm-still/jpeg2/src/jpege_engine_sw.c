...
static OS_THREAD_FUNC_RET_T OS_THREAD_FUNC_MODIFIER
             jpege_engine_sw_output_thread  (OS_THREAD_FUNC_ARG_T);
...
static int   jpege_engine_sw_flush_output(jpege_engine_sw_t *p_engine);
...
jpege_engine_profile_t jpege_engine_sw_profile =
    ...
    jpege_engine_hybrid_output_handler_t  p_output_handler)
{
    ...
    STD_MEMSET(p_engine, 0, sizeof(jpege_engine_sw_t));  // Zero out the entire structure
    ...
    if (os_thread_create(&(p_engine->output_thread),
                       jpege_engine_sw_output_thread,
                       (void *)p_engine))
    {
        //destroy cond & free engine
    ...
    jpege_engine_hybrid_cfg_t   *p_hybrid_cfg)
{
    ...
    rc = os_thread_create(&p_engine->thread, jpege_engine_sw_encode,
    (void *)p_engine);
    ...
    jpege_engine_hybrid_obj_t *p_obj)
{
    if (p_obj)
    {
        ...
        if (p_engine)
        {
            if (p_engine->p_internal_buf)
            {
                jpeg_buffer_destroy((jpeg_buffer_t *)&(p_engine->p_internal_buf));
            }
            if (p_engine->jpegeBitStreamState.buffer)
            {
                JPEG_FREE(p_engine->jpegeBitStreamState.buffer);
                STD_MEMSET(&(p_engine->jpegeBitStreamState), 0,
                           sizeof(bitstream_t));
            }
    ...
    jpege_engine_sw_encode(OS_THREAD_FUNC_ARG_T arg)
{
    ...
    jpege_engine_sw_t *p_engine = arg;
                                          ...
                                          int16_t *currMCUChromaDctOutput)
{
    ...
    JPEG_DBG_LOW("jpege_engine_sw_fetch_mcu: horiMCUIndex=%d, vertMCUIndex=%d\n",
    ...
    JPEG_DBG_LOW("jpege_engine_sw_fetch_mcu: inputYPtr=0x%x, currMCUYStartAddr=0x%x\n",
    ...
    for (i=0; i < p_engine->rotatedMCUHeight/BLOCK_HEIGHT; i++)
    {
        ...
        for (j=0; j < p_engine->rotatedMCUWidth/BLOCK_WIDTH; j++)
        {
            ...
            JPEG_DBG_LOW("jpege_engine_sw_fetch_mcu: currYBlockStartAddr=0x%x\n",
    ...
    JPEG_DBG_LOW("jpege_engine_sw_fetch_mcu: inputCbCrPtr=0x%x, currMCUCbCrStartAddr=0x%x\n",
...
#if 0  /* No one is calling it */
static jpeg_buf_t *jpege_engine_sw_get_next_dest_buf(jpege_engine_sw_t *p_engine)
{
    jpeg_buf_t *p_next_buf = p_engine->p_dest_buffer;

    // find out which is the next destination buffer
    if (p_engine->dest.buffer_cnt > 1)
    {
        p_engine->dest_buffer_index = (p_engine->dest_buffer_index + 1) %
            p_engine->dest.buffer_cnt;
        p_next_buf = p_engine->dest.p_buffers[p_engine->dest_buffer_index];
    }

    p_engine->p_dest_buffer = p_next_buf;
    return p_next_buf;
}
#endif

static OS_THREAD_FUNC_RET_T OS_THREAD_FUNC_MODIFIER
    jpege_engine_sw_output_thread(OS_THREAD_FUNC_ARG_T p_arg)
{
    int rc;
    jpege_engine_sw_t *p_engine = (jpege_engine_sw_t *)p_arg;

    for (;;)
    {
        jpege_engine_output_t *p_output;

        os_mutex_lock(&(p_engine->mutex));

        // Block if no output is pending
        while (!p_engine->pending_outputs[p_engine->pending_output_q_head].is_valid &&
               !p_engine->thread_exit_flag)
        {
            os_cond_wait(&(p_engine->output_cond), &(p_engine->mutex));
        }
        ...
        // Check output buffer to see if it is last output buffer
        if (p_output->is_last_buffer)
        {
            // Signal encode thread that last buffer handling done
            // So encode thread can go to finish afterwards
            os_cond_signal(&(p_engine->final_output_cond));
            // Clean up the final output flag
            p_engine->final_output_flag = false;
            // Clean up the last buffer flag
            p_output->is_last_buffer = false;
        }

        p_output->is_valid = false;
        // Signal encode thread that output thread consume
        os_cond_signal(&(p_engine->consume_cond));
        ...
        if (JPEGERR_SUCCESS != rc)
        {
            ...
            JPEG_DBG_ERROR("jpege_engine_sw_output_thread: Engine output handler return failed.");
        }
        os_mutex_unlock(&(p_engine->mutex));
    }
...
static int jpege_engine_sw_flush_output(jpege_engine_sw_t *p_engine)
{
    ...
    os_mutex_lock(&(p_engine->mutex));
    while (p_engine->pending_outputs[p_engine->pending_output_q_tail].is_valid && p_engine->abort_flag != true)
    {
        // If queue is full,
        // Wait output thread to consume
        os_cond_wait(&(p_engine->consume_cond), &(p_engine->mutex));
        JPEG_DBG_ERROR("jpege_engine_sw_flush_output: fatal error: queue is full\n");
    }

    if (p_engine->abort_flag == true)
    {
        p_engine->error_flag = true;
        os_mutex_unlock(&(p_engine->mutex));
        JPEG_DBG_ERROR("jpege_engine_sw_flush_output: aborted.\n");
        return JPEGERR_EFAILED;
    }
    ...
    {
        // Set output with is_last_buffer flag,
        // output thread will signal encode thread if is_last_buffer flag is set
        // while encode thread waits the output thread to output it if final_output_flag is set
    ...
    // Signal thread to handle the output
    os_cond_signal(&(p_engine->output_cond));
    os_mutex_unlock(&(p_engine->mutex));
...
#if 0 /* No one is using it */
void jpege_engine_sw_emit_byte(jpege_engine_sw_t *p_engine, uint8_t byte)
{
    jpeg_buf_t *p_dst_buf = p_engine->p_dest_buffer;

    // Accumulate if possible, otherwise flush the accumulated data
    if (p_dst_buf->offset == p_dst_buf->size)
    {
        if (JPEG_FAILED(jpege_engine_sw_flush_output(p_engine)))
        {
            os_mutex_lock(&(p_engine->mutex));
            p_engine->error_flag = true;
            os_mutex_unlock(&(p_engine->mutex));
        ...
        // Get the next available destination buffer to use
        p_dst_buf = jpege_engine_sw_get_next_dest_buf(p_engine);

        // Wait for it to be empty before reusing
        jpeg_buffer_wait_until_empty(p_dst_buf);
        p_dst_buf->offset = 0;
    }
    p_dst_buf->ptr[p_dst_buf->offset] = byte;
    p_dst_buf->offset++;
}
#endif
                                              ...
                                              jpege_img_cfg_t   *p_config)
{
    ...
    case 90:
    {
        ...
        p_engine->currOrigin_chroma    = (chromaHeight - BLOCK_HEIGHT) * chromaWidth * 2;
        /* setup relative step */
