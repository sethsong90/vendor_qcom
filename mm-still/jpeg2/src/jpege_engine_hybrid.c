...
#define DEFAULT_PIECE_HEIGHT 480
...
static void jpege_engine_hybrid_create(jpege_engine_obj_t *p_obj, jpege_obj_t encoder)
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_create\n");
}
    ...
    jpege_engine_event_handler_t  p_handler)
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_init\n");
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_abort done\n");
    return JPEGERR_SUCCESS;
}
    ...
    jpege_engine_obj_t *p_obj)
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_destroy done\n");
}
    ...
    jpege_engine_dst_t   *p_dest)
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_check_start_param \n");
   ...
   jpege_engine_dst_t *p_dest)
{
    ...
    while (remaining_height > 0)
    {
        ...
        p_hybrid->output_buf_size = 4 * p_job->engine_cfg.output_MCUs * MCU_height * MCU_width;
        ...
        if (JPEG_FAILED(rc))
        {
            JPEG_DBG_ERROR("jpege_engine_hybrid_configure: failed to reset job queue\n");
            return rc;
        }
        p_hybrid->job_count ++;
   ...
   jpege_engine_dst_t *p_dest)
{
    ...
    while (remaining_height > 0)
    {
       ...
       // Job configurations
        ...
        p_job->engine_cfg.output_buffer_length = p_hybrid->p_output_buf[src_index]->size - p_hybrid->p_output_buf[src_index]->start_offset -
            p_hybrid->p_output_buf[src_index]->offset;
    ...
    void                      *p_arg)  // An optional argument for that event.
{
    ...
    //fprintf(stderr,"jpege_engine_hybrid_event_handler  \n");
    ...
    case JPEG_EVENT_DONE:
        //fprintf(stderr,"jpege_engine_hybrid_event_handler  JPEG_EVENT_DONE \n");
        p_hybrid->job_done ++;
        ...
        else if (p_hybrid->job_scheduled < p_hybrid->job_count)
        {
            ...
            if (JPEG_FAILED(rc))
            {
                JPEG_DBG_ERROR("jpege_engine_hybrid_event_handler: failed to reset job queue\n");
                os_mutex_unlock(&(p_hybrid->mutex));
                return rc;
            }
    ...
    uint32_t             buf_offset) // The flag to indicate last output buffer
{
    ...
    if (p_hybrid->output_buf_received_cnt == p_hybrid->output_buf_cnt)
    {
        ...
        while (i < p_hybrid->output_buf_cnt)
        {
            bytes_remaining = p_hybrid->p_output_buf[i]->offset - p_hybrid->p_output_buf[i]->start_offset;
            bytes_written = 0;
