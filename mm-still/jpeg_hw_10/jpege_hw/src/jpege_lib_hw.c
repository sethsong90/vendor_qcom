                                                ...
                                                jpege_cmd_scale_cfg *scale_cfg)
{
  ...
  p_hw_cmd->data = nRegVal;
  JPEG_HW_PR_ERR("we_buffer_cfg PLN0_WR_BUFFER_SIZE = %d\n", p_hw_cmd->data);
  p_hw_cmd++;

  //PLN0_WR_STRIDE
  nRegVal = pIn->hw_buf_size / l_hw_buf_height;
  p_hw_cmd->data = nRegVal;
  JPEG_HW_PR_ERR("we_buffer_cfg PLN0_WR_STRIDE = %d\n", p_hw_cmd->data);
  p_hw_cmd++;
  p_hw_cmd++;
  p_hw_cmd++;
  ...
  nRegVal = pIn->hw_buf_size / l_hw_buf_height;
  p_hw_cmd->data = nRegVal;
  JPEG_HW_PR_ERR("we_buffer_cfg PLN0_WR_HSTEP = %d\n", p_hw_cmd->data);
  p_hw_cmd++;

  //PLN0_WR_VSTEP
  nRegVal = l_hw_buf_height;
  p_hw_cmd->data = nRegVal;
  JPEG_HW_PR_ERR("we_buffer_cfg PLN0_WR_VSTEP = %d\n", p_hw_cmd->data);
  p_hw_cmd++;
