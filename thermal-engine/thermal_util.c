...
#define SYSFS_DEFAULT_CORE_TEMP_LIMIT "/sys/module/msm_thermal/default_cpu_temp_limit"
...
enum therm_msm_id therm_get_msm_id(void)
{
	...
	if (!msm_id_init) {
		...
		get_soc_info(buf, SYSFS_PLATFORMID,
			     SYSFS_PLATFORMID_DEPRECATED);
		soc_id = atoi(buf);
...
static int therm_get_default_temp_limit(void)
{
	static int default_core_temp_limit;
	static uint8_t msm_default_temp_init;
	int ret = 0;

	if (!msm_default_temp_init) {
		char buf[MAX_SOC_INFO_NAME_LEN];

		ret = read_line_from_file(SYSFS_DEFAULT_CORE_TEMP_LIMIT, buf,
						MAX_SOC_INFO_NAME_LEN);
		if (ret < 0) {
			msg("Error getting default temp limit %d\n", ret);
			default_core_temp_limit = 0;
			msm_default_temp_init = 1;
			return ret;
		}
		default_core_temp_limit = CONV(atoi(buf));
		msm_default_temp_init = 1;
		dbgmsg("%s: Default core temp limit request is %d\n",
			__func__, default_core_temp_limit);

	}
	return default_core_temp_limit;
}
						...
						enum algo_type algo)
{
	...
	int default_core_temp_limit = therm_get_default_temp_limit();
	if (default_core_temp_limit <= 0)
		return;

	switch(algo) {
		case SS_ALGO_TYPE:
			for (i = 0; i < cfg_size; i++) {
				struct ss_setting *ss = &cfg[i].data.ss;

				if (0 != strncmp(cpu_sensor, ss->sensor,
						 strlen(cpu_sensor)))
					continue;

				if (ss->set_point == default_core_temp_limit)
					continue;

				set_point_delta =
					ss->set_point - ss->set_point_clr;
				ss->set_point = default_core_temp_limit;
				ss->set_point_clr =
					default_core_temp_limit - set_point_delta;
			}
			break;
		case PID_ALGO_TYPE:
			for (i = 0; i < cfg_size; i++) {
				struct pid_setting *pid = &cfg[i].data.pid;

				if (0 != strncmp(cpu_sensor, pid->sensor,
						 strlen(cpu_sensor)))
					continue;

				if (pid->set_point == default_core_temp_limit)
					continue;

				set_point_delta =
					pid->set_point - pid->set_point_clr;
				pid->set_point = default_core_temp_limit;
				pid->set_point_clr =
					default_core_temp_limit - set_point_delta;
			}
			break;
		default:
			msg(
			"%s: No support for temp limit adjust for algo %d\n",
				__func__, algo);
			break;
	}
}
...
int read_int_list_from_file(const char *path, int *arr, uint32_t arr_sz)
{
	...
	while ((int_str != NULL) && (ints_read < arr_sz)) {
