...
#define ERR_SAMPLING_VAL    INT_MIN
...
#define ERR_PCONST_VAL      -1.0
#define ERR_ICONST_VAL      -1.0
#define ERR_DCONST_VAL      -1.0
#define ERR_ERRWEIGHT_VAL   -1.0
...
static int verify_string(char c)
{
...
static int verify_number(char c)
{
	if (c >= '0' && c <= '9')
		return 0;
	return 1;
...
static int verify_whitespace(char c)
{
	/* Check for comment
...
static void append_token_text(char c)
{
...
static char **init_sensor_list(char **prev, int cnt, int size)
{
	int i, j;
	char **ret;
...
static int *init_weight_list(int *prev, int cnt, int size)
{
	int i;
	int *ret;
	/* If size is equal to sensor_info_arr_len then prev
...
static int init_v_settings(struct setting_info *section)
{
	...
	section->data.v.list_size = sensor_info_arr_len;
...
static int init_eq_settings(struct setting_info *section)
{
	...
	for (i = 0; i < INTEGERS_MAX; i++) {
		section->data.eq.sampling_list[i] = ERR_INT_VAL;
		section->data.eq.temp_range_list[i] = ERR_INT_VAL;
	}
	...
	section->data.eq.list_size = sensor_info_arr_len;
	return 0;
}
...
{
	int i;
	int *sampling_list = section->data.eq.sampling_list;
	int *temp_range_list = section->data.eq.temp_range_list;

	if (sampling_list[0] != ERR_INT_VAL)
		section->data.eq.sampling_period_ms = sampling_list[0];
	if (sampling_list[1] != ERR_INT_VAL)
		section->data.eq.long_sampling_period_ms = sampling_list[1];
	if (sampling_list[2] != ERR_INT_VAL)
		section->data.eq.long_sampling_cnt = sampling_list[2];
	if (sampling_list[3] != ERR_INT_VAL)
		section->data.eq.pre_cal_delay_ms = sampling_list[3];
	if (temp_range_list[0] != ERR_INT_VAL)
		section->data.eq.temp_range = temp_range_list[0];
	if (temp_range_list[1] != ERR_INT_VAL)
		section->data.eq.sensor_delta = temp_range_list[1];
	if (temp_range_list[2] != ERR_INT_VAL)
...
static int parse_settings(void *section, char *desc,
				struct table *gen_table, int table_size)
{
	int i;
...
static int validate_eq(struct setting_info *s)
{
	...
	if (s->data.eq.pre_cal_delay_ms < 0)
		s->data.eq.pre_cal_delay_ms = 0;
	return 0;
}
...
{
	int unique_fields = 0;
	int max_fields = ARRAY_SIZE(global_table) + ARRAY_SIZE(pid_table) +
