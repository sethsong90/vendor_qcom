...
#define MAX_TM_INSTANCES_SUPPORTED 64
				        ...
				        int num_thresholds)
{
	...
	for (i = 0; i < num_thresholds; i++) {
		uint8_t j = 0;
		int num_actions = clnt_t[i].num_actions;
...
{
	int a_idx[num_cfg_cnt];
	int c_idx;
	uint8_t notification = 0;

	if (reserved == NULL ||
	    num_cfg_cnt <= 0 ||
	    num_cfg_cnt > MAX_CONFIG_INSTANCES_SUPPORTED)
		return -1;
			   ...
			   void                  *data)
{
	...
	if (((int)data) >= tm_cnt) {
		msg("%s: unexpected idx %d", __func__, (int)data);
		return;
	}

	dbgmsg("%s: Update recieved %s %d", __func__,
	       tm_states[(int)data].setting->desc,
	       reading);

	/* Notify the waiting thread */
	pthread_mutex_lock(&wait_mutex);
	THRESH_MASK_SET((int)data);
	pthread_cond_broadcast(&wait_cond);
	pthread_mutex_unlock(&wait_mutex);
}
