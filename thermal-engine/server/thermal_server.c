				      ...
				      int client_fd)
{
	...
	while (1) {
		...
		if (rc <= 0) {
			msg("Thermal-Server:%s: recv failed, received byte %d\n",
			     __func__, rc);
			ret = -1;
			break;
		}
...
int thermal_server_init(void)
{
	...
	snprintf(server_log.sun_path, UNIX_PATH_MAX, " %s", UI_LOCALSOCKET_NAME);
	...
	rc = bind(sockfd_server_log,(struct sockaddr  const *)&server_log, sizeof(sa_family_t) + 1 + strlen(UI_LOCALSOCKET_NAME));
	if (rc != 0) {
		msg("Thermal-Server: Server log socket: bind error - %s", strerror(errno));
		goto error;
	}
...
#ifdef ANDROID
	chown(server_addr_recv.sun_path, AID_ROOT, AID_SYSTEM);
#endif
