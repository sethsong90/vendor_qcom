...
static void *do_request_to_thermal(void *data)
{
	...
	while(1) {
		...
		rc = connect(sockfd_client_send, (struct sockaddr *)&client_addr_send,
				     sizeof(sa_family_t) + strlen(send_sock_path));
		if (rc != 0) {
...
static int connect_to_socket(char *client_name, struct sockaddr_un *client_addr)
{
	...
	rc = connect(sockfd_client, (struct sockaddr *)client_addr,
		     sizeof(sa_family_t) + strlen(req_clients[i].send_sock_path));
	if (rc != 0) {
		close(sockfd_client);
		return -1;
	}
				   ...
				   int *num_configs)
{
	...
	if (rc <= 0) {
		msg("Thermal-Lib-Client:%s: recv failed, received byte %d\n",
		     __func__, rc);
		return -1;
	}
	...
	if (thermal_msg->msg_type == ALGO_IDX_MAX && thermal_msg->req_data > 0) {
		if (thermal_msg->req_data < MAX_CONFIG_INSTANCES_SUPPORTED) {
			*configs =
			(struct config_instance *)malloc(sizeof(struct config_instance) * \
						         thermal_msg->req_data);
			if (*configs == NULL) {
				msg("%s: malloc failed for fields", __func__);
				return -1;
			}
			*num_configs = thermal_msg->req_data;
			return 0;
		}
				       ...
				       struct config_instance *config)
{
	...
	socket_data->config.disable = INT_MIN;
	socket_data->config.sampling = INT_MIN;
			       ...
			       unsigned int config_size)
{
	...
	config_set_data.req_data = config_size;
