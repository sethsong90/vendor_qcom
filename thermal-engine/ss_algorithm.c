...
static int handle_sampling_adjust(struct ss_algo_t *ss_info, int algo_idx, int sampling)
{
	int s_idx = 0;
	int ret_val = 0;
	...
	if (!ss_info->disabled &&
	    (sampling_groups[ss_info->sampling_group_id].ss_clnt_state &
							 (0x1 << algo_idx))) {
		sampling_groups[s_idx].ss_clnt_state |= (0x1 << algo_idx);
...
static int handle_disable_config_adjust(struct ss_algo_t *ss_info, int idx, int disable)
{
	int ret_val = 0;
			  ...
			  int reading, void *data)
{
	...
	dbgmsg("%s: SS Id %s, Update recieved %s %d", __func__,
	       algo_clnt[(int)data].desc,
	       algo_clnt[(int)data].setting->sensor,
	       reading);
...
static void increase_mitigation_lvl(struct ss_algo_t *instance)
{
	...
	if (instance->curr_lvl >= (instance->num_of_lvls - 1)) {
...
static void handle_thresh_sig(void)
{
	...
	for (idx = 0; idx < clnt_cnt; idx++) {
		...
		if (algo_clnt[idx].state == 0) {
			...
			sampling_groups[s_id].ss_clnt_state &= (~(0x1 << idx));
			/* Flush error history */
			set_curr_err(&algo_clnt[idx].err_info, 0, ERR_HISTORY);
		} else {
...
static void *algo_monitor(void *data)
{
	...
	if (clnt_cnt < MAX_INSTANCES_SUPPORTED)
		thresh_reached_mask = (1 << clnt_cnt) - 1;
	else
