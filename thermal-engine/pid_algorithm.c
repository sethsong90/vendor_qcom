...
static void set_curr_err(struct err_history_t *err_info, int err, int sample_cnt)
{
	int idx;
	...
	for (idx = 0; idx < sample_cnt; idx++) {
		err_info->err_I_total -= get_err(err_info, idx+1);
	}
			      ...
			      int reading, void *data)
{
	...
	dbgmsg("%s: PID Id %s, Update recieved %s %d", __func__,
	       pid_algo_clnt[(int)data].desc,
	       pid_algo_clnt[(int)data].setting->sensor,
	       reading);
...
static void pid_timer_expired(union sigval sval)
{
	...
	sampling_group_mask |= (0x1 << sval.sival_int);
	pthread_cond_broadcast(&wait_cond);
	pthread_mutex_unlock(&wait_mutex);
}
...
static int config_parameter_set_notify(int num_cfg, void *data, void *reserved)
{
	...
	if (reserved == NULL ||
	    num_cfg <= 0 ||
	    num_cfg > MAX_CONFIG_INSTANCES_SUPPORTED)
		return -1;
...
static int handle_sampling_adjust(struct pid_algo_t *pid_info, int algo_idx, int sampling)
{
	int s_idx = 0;
	int ret_val = 0;
	...
	if (!pid_info->disabled &&
	    (sampling_groups[pid_info->sampling_group_id].pid_clnt_state &
							 (0x1 << algo_idx))) {
		sampling_groups[s_idx].pid_clnt_state |= (0x1 << algo_idx);
...
static int handle_disable_config_adjust(struct pid_algo_t *pid_info, int idx, int disable)
{
	int ret_val = 0;
...
static int validate_client_config(struct thermal_msg_data *client_config)
{
	...
	if (client_config->config.fields_mask & DISABLE_FIELD) {
		if (client_config->config.disable < 0 ||
				client_config->config.disable > 1) {
...
static void handle_config_set_sig(void)
{
	...
	for (i = 0; i < cfg_set_cnt; i++) {
		a_idx[i] = validate_client_config(&client_config[i]);
		if (a_idx[i] < 0) {
			pthread_mutex_unlock(&config_set_mutex);
			return;
		}
...
static void handle_override_sig(void)
{
	...
	for (idx = 0; idx < pid_clnt_cnt; ++idx) {
		...
		/* Reset Adjustment */
		pid_algo_clnt[idx].prev_adjustment = 0;
...
static void handle_timer_sig(void)
{
	...
	for (sample_id = 0; sample_id < sampling_group_count; sample_id++) {
		if ((local_sampling_group_mask & (0x1 << sample_id)) == 0)
			continue;

		sample_cnt = (((curr_time.tv_sec -
		...
		for (idx = 0; idx < pid_clnt_cnt; idx++) {
			...
			dbgmsg("%s: PID Id %s, Prev Adj. %dkHz, E0 %.1fC, "
			       ...
			       pid_algo_clnt[idx].desc,
			       pid_algo_clnt[idx].prev_adjustment, E0, E1, E2);

			if ((E0 == 0) || ((E0 > 0) && (E1 < 0)) ||
			    ((E0 < 0) && (E1 > 0))) {
				dbgmsg("%s: PID Id %s Reset Prev Adj.",
				       __func__, pid_algo_clnt[idx].desc);
				pid_algo_clnt[idx].prev_adjustment = 0;
			}
			...
			adjustment = pid_algo_clnt[idx].prev_adjustment +
				(int)(((pid_algo_clnt[idx].setting->p_const *
					(E0 - E1)) +
				       (pid_algo_clnt[idx].setting->i_const *
					E0 * Ts) +
				       ((pid_algo_clnt[idx].setting->d_const / Ts) *
					(E0 - (2 * E1) + E2))) * Units);

			pid_algo_clnt[idx].prev_adjustment = adjustment;
		...
		sampling_group_mask &= (~(0x1 << sample_id));
		pthread_mutex_unlock(&wait_mutex);
	}
...
static void handle_thresh_sig(void)
{
	...
	for (idx = 0; idx < pid_clnt_cnt; idx++) {
		...
		thresh.notify_cb_data = (void*)(int)idx;
		if (pid_algo_clnt[idx].state == 0) {
			...
			sampling_groups[s_id].pid_clnt_state &= (~(0x1 << idx));
			/* Flush error history */
			set_curr_err(&pid_algo_clnt[idx].err_info, 0, I_SAMPLES);
		} else {
			thresh.thresh.low_valid = 1;
			thresh.thresh.low = pid_algo_clnt[idx].active_set_point_clr;
			sampling_groups[s_id].pid_clnt_state |= (0x1 << idx);
		}
...
static void *pid_algo_monitor(void *data)
{
	...
	if (pid_clnt_cnt < MAX_PID_INSTANCES_SUPPORTED)
		thresh_reached_mask = (1 << pid_clnt_cnt) - 1;
	else
