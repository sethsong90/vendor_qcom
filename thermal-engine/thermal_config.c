...
static char *verify_sensor(char *pvalue, char **sensor_ptr, int cnt)
{
	int i;
...
static int *parse_weight_values(char **ppos, int *num_weight)
{
	...
	while (pvalue != NULL) {
		weight_ptr[nresult] = strtod(pvalue, NULL);
		dbgmsg("Found weight '%d'\n", weight_ptr[nresult]);
		sum += weight_ptr[nresult];
		nresult++;
		if (nresult >= sensor_info_arr_len)
			break;
		pvalue = strtok_r(NULL, "\t \r\n", &psave);
	}
	if (sum != 100)
		...
		default:
			if (in_field != FIELD_IDX_MAX) {
				...
				case OVERRIDE:
					...
					section->data.tm.override = values[0];
					break;
				}
		...
		default:
			if (in_field != FIELD_IDX_MAX) {
				...
				case OVERRIDE:
					...
					section->data.pid.override = values[0];
					break;
				}
				in_field = FIELD_IDX_MAX;
				break;
		...
		default:
			if (in_field != FIELD_IDX_MAX) {
				...
				case OVERRIDE:
					...
					section->data.ss.override = values[0];
					break;
				}
				in_field = FIELD_IDX_MAX;
				break;
			...
			if (in_field != FIELD_IDX_MAX) {
				...
				case SAMPLING:
					...
					section->data.eq.pre_cal_delay_ms =
						(int)values[3];
					if (section->data.eq.pre_cal_delay_ms < 0)
						section->data.eq.pre_cal_delay_ms = 0;
					break;
				case TEMP_RANGE:
					num = parse_values(&pos, values);
					if ((num != 3) || (values[0] < 0) ||
		...
		default:
			if (in_field != FIELD_IDX_MAX) {
				...
				case SAMPLING:
					...
					section->data.v.sampling_period_ms = (int)values[0];
					break;
				case SET_POINT:
					num = parse_values(&pos, values);
					if (num != 1) {
					...
					section->data.v.set_point = values[0];
					break;
				case SET_POINT_CLR:
					num = parse_values(&pos, values);
					if (num != 1) {
					...
					section->data.v.set_point_clr = values[0];
					break;
				case SENSOR_FIELD:
					result_str = NULL;
					if (parse_sensor_field(&pos, &result_str) != 0) {
...
{
	if (settings->sample_period_ms != -1)
