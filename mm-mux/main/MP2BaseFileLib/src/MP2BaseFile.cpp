...
boolean MP2BaseFile::Output (const uint8  *data, uint32 offset, uint32 len, bool bEOF)
{
 ...
 // MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_HIGH, "MP2BaseFile::Output length  %d", len);
   ...
   case MUX_METHOD_PORT:
       ...
       if(m_output_handle.OputPort.pOputStream != NULL)
       {
          eRet = m_output_handle.OputPort.pOputStream->WriteBlockData(m_TS_Stream_buf,
                      m_nTS_Stream_bufsz, 0, bEOF, (int*)&nBytesWritten);
       }

       if(eRet != video::iSourcePort::DS_SUCCESS ||
          nBytesWritten != m_nTS_Stream_bufsz)
       {
           m_nTS_Stream_bufsz = 0;
           if(nBytesWritten == -1)
           {
                                        ...
                                        const uint8  *sample_data)
{
  ...
  MM_MSG_PRIO4(MM_GENERAL, MM_PRIO_MEDIUM,
                                 "MP2BaseFile::MUX_Process_Sample stream number  %d Size %d tim = %lu, extra_data_size =%lu",
         ...
         if(m_Params->streams[stream_number].type == MUX_STREAM_VIDEO)
         {
            ...
            MM_MSG_PRIO2(MM_STATISTICS, MM_PRIO_MEDIUM,
             "WFD:STATISTICS:  MP2BaseFile::MUX_Process_Sample stream number %d \
              StartTime = %u", (int)stream_number,(unsigned long)muxStats.nStartTime);
         }
    }
    #endif /* ENABLE_MUX_STATS */
    // WFD:STATISTICS -- end
            ...
            */
                                 ...
                                 == VIDEO_PVTDATA_TYPE_FILLERNALU_ENCRYPTED)
            {
              ...
              if(current_sample_info.fmt_pvtdata_size >
                       1/*Sig Byte*/ + current_sample_info.fmt_pvtdata_ptr[1])
              {
                ...
                if(nPESPvtSize > 0)
                {
                  ...
                  MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,"MP2BaseFile FillerNALU: Current Extra Size = %lu",
            ...
            if(bExtraValid)
            {
                                    ...
                                    <= current_sample_info.fmt_pvtdata_size)
              {
                ...
                MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_MEDIUM,"MP2BaseFile FillerNALU: Current Sample Size = %lu",
    ...
    else
    {
        ...
        while(nTempBufSize >= sizeof(uint32))
        {
           ...
           nNalSize = MUX_FindNextPattern(pData + nPos + sizeof(uint32),
                                          nTempBufSize - sizeof(uint32),
                                          0x00000001);

           ((uint8 *)pData)[nPos]     = (nNalSize >> 24);
...
MUX_STATUS MP2BaseFile::MUX_update_streamport(uint32 streamport)
{
  ...
  m_output_handle.OputPort.pOputStream = (video::iStreamPort*)streamport;
...
void MP2BaseFile::GeneratePATPacket()
{
  ...
  if(m_MP2TSPacket)
  {
    ...
    m_MP2TS_table_continuty_counter[0] = m_MP2TS_table_continuty_counter[0] + 1;
    ...
    m_MP2TSPacket[index] |= nCRC >> 24;
    index += 1;
    m_MP2TSPacket[index] |= (nCRC >> 16) &0xFF;
    index += 1;
    m_MP2TSPacket[index] |= (nCRC >> 8) & 0XFF;
    index += 1;
    m_MP2TSPacket[index] |= (nCRC & 0xFF);
    index += 1;
...
//    index += 1;
    ...
    m_MP2TS_table_continuty_counter[1] = m_MP2TS_table_continuty_counter[1] + 1;
    ...
    if(m_bVideoPresent)
    {
          ...
          MUX_STREAM_VIDEO_H264)
      {
        ...
        {
          section_length += m_nAVCHrdDescrLen;
        }
      }
    }
    if(m_bAudioPresent)
       ...
       (m_Params->encrypt_param.type == MUX_ENCRYPT_TYPE_HDCP))
    {
      ...
      index += 1;
      /* HDCP_version */
      m_MP2TSPacket[index] = m_Params->encrypt_param.nEncryptVersion;
      index += 1;
    }
    else /*No descriptors*/
    {
                                             ...
                                             const uint32 offset_sample_data)
{
  ...
  if(m_MP2TSPacket)
  {
    ...
    if(m_adaptation_field < 4)
    {
      ...
      m_MP2TSPacket[index] |= (nAdaptationControl << 4);
    }

    m_MP2TSPacket[index] |= m_MP2TS_continuty_counter[stream_number];
    m_MP2TS_continuty_counter[stream_number] = ((m_MP2TS_continuty_counter[stream_number] + 1) % 16);
    index += 1;
    ...
    if(index < TS_PKT_SIZE)
    {
      ...
      if(sample_info->extra_data_size != 0)
      {
        ...
        {
          bytes_to_copy -= bytes_to_copy % 16;
        }
      }

      sample_data_offset = bytes_to_copy;

      bytes_to_copy += PES_Header_Size;
      ...
      else if((index + bytes_to_copy) < TS_PKT_SIZE )
      {
        ...
        if(m_MP2TSPacket[4])
        {
           bytes_to_stuff = TS_PKT_SIZE - (index + bytes_to_copy);
        }
        else
        {
           bytes_to_stuff = TS_PKT_SIZE - (index + bytes_to_copy + 2);
        }
        ...
        if(m_MP2TSPacket[4])
        {
          m_MP2TSPacket[4] = m_MP2TSPacket[4] + (bytes_to_stuff);
        }
        else
        {
          m_MP2TSPacket[4] = m_MP2TSPacket[4] + (bytes_to_stuff+1);
        ...
        if(bytes_to_stuff < TS_PKT_SIZE)
        {
          ...
          for( loop_index = index; loop_index < (index + bytes_to_stuff); loop_index++)
          {
            m_MP2TSPacket[loop_index] = 0XFF;
          }
                                            ...
                                            const uint32 offset_sample_data)
{
 ...
 // MM_MSG_PRIO(MM_GENERAL, MM_PRIO_MEDIUM, "MP2BaseFile::GenerateMP2TSVideoPacket");
  ...
  if(m_MP2TSPacket)
  {
    ...
    m_MP2TSPacket[index] |= m_MP2TS_continuty_counter[stream_number];
    m_MP2TS_continuty_counter[stream_number] =
    ...
    bytes_to_copy = TS_PKT_SIZE - index;
    ...
    if( PES_Header_Size + index + n_data_len < TS_PKT_SIZE )
    {
        ...
        {
             m_MP2TSPacket[4] += (uint8)(n_stuff_len);
        }
        if(n_stuff_len)
        {
                                             ...
                                             MUX_sample_info_type  *sample_info)
{
  ...
  if(m_MP2PCRTSPacket)
  {
    ...
    if(m_adaptation_field < 4)
    {
      ...
      // Adaptation_field_control - 2 bits
      m_MP2PCRTSPacket[index] |= (nAdaptationControl << 4);
    }
    ...
    if(1)
    {
      ...
      if(bputPCR)
      {
          ...
          m_MP2PCRTSPacket[index] |= (uint8)((PCR_Ext >> 8) & 0x1);
          index += 1;
    ...
    if(index < TS_PKT_SIZE)
    {
      ...
      if(index < TS_PKT_SIZE )
      {
        ...
        {
           bytes_to_stuff = TS_PKT_SIZE - index;
        }
        else
        {
           bytes_to_stuff = TS_PKT_SIZE - (index + 2);
        }
        ...
        {
          m_MP2PCRTSPacket[4] = m_MP2PCRTSPacket[4] + 1;
          /* Adaptation header  1- Byte */
  ...
  if(PES_Header)
  {
      ...
      if(m_bAdjustTSforCompliance)
      {
        ...
        else if(m_Params->streams[stream_number].type == MUX_STREAM_AUDIO)
        {
          ...
          */
          ...
          {
            MM_MSG_PRIO2(MM_GENERAL, MM_PRIO_ERROR, "MP2BaseFile Error Audio Timestamp %d %d",
    ...
    switch (m_PTS_DTS_Flags)
    {
      case 0:
        ...
        if(extraDataValid)
        {
          PES_Header[index] += (1+sample_info->extra_data_size);
        }
        index += 1;
        break;
      ...
      case 2:
        ...
        index += 1;
        //PES_header_data_length
        PES_Header[index] = 0X05+ header_length_stuff;
        ...
        //PTS[32...30]
        PES_Header[index] |= (PTS_Base >> 29);
        // marker_bit - 1 bit, always 1
        PES_Header[index] |= 0X01;
        index += 1;
        //PTS[29....22]
        //PES_Header[index] |= (PTS_Base >> 21) & 0xff;
        PES_Header[index] |= (PTS_Base >> 22);
        index += 1;
        //PTS[21....15]
        PES_Header[index] |= (PTS_Base >> 14);
        // marker_bit - 1 bit, always 1
        PES_Header[index] |= 0X01;
        index += 1;
        //PTS[14....7]
        PES_Header[index] |= (PTS_Base >> 7);
        index += 1;
        //PTS[6....0]
        PES_Header[index] |= (PTS_Base << 1);
        // marker_bit - 1 bit, always 1
        PES_Header[index] |= 0X01;
        index += 1;
        break;
      ...
      case 3:
        ...
        index += 1;
        //PES_header_data_length
        PES_Header[index] = 0X0A+header_length_stuff;
        if(extraDataValid)
        {
          PES_Header[index] += (1+sample_info->extra_data_size);
        }
    ...
    if(extraDataValid)
    {
      ...
      index += sample_info->extra_data_size;
  }
    if(m_Params->streams[stream_number].type == MUX_STREAM_AUDIO)
    {
      if(m_Params->streams[stream_number].subinfo.audio.format == MUX_STREAM_AUDIO_PCM)
  ...
  while(sample_data_offset < (header_size + PES_Header_Size))
  {
    if(m_MP2TSPacket)
    {
      ...
      if(index < TS_PKT_SIZE)
      {
        ...
        {
          // 2 bytes - 1 byte adaptation_field_length, 1 byte adaptation header
          uint8 bytes_to_stuff = TS_PKT_SIZE - (index + bytes_to_copy + 2);
          // Adaptation_field_control - 2 bits
          m_MP2TSPacket[3] |= (MUX_MP2_ADAPTATON_AND_PAYLOAD << 4);
          m_MP2TSPacket[4] = m_MP2TSPacket[4] + (bytes_to_stuff + 1);
          m_MP2TSPacket[5] |= 0X00;
