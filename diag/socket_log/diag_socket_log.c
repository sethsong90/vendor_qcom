...
int main(int argc, char *argv[])
{
	...
	int pid, pid_fd;
	char pid_buff[10];
	char pid_file[32] = "/data/diag_logs/diag_socket_pid";
	...
	/* Make sure the default directory exists so the diag_socket_pid file
	 * can be placed there
	 */
	if (mkdir("/data/diag_logs", 0777))
		if (errno != EEXIST)
			DIAG_LOGE("diag_socket_log: Fail creating directory for diag_socket_pid file");
