...
#ifdef FEATURE_WINMOB
static  HANDLE g_testapp_thread_hdl;			/* Diag task thread handle */
#else
...
//removed WM specific code 
#ifdef FEAUTRE_WINMOB
static DWORD StartSecondApp(LPVOID param);
#else
static void *StartSecondApp(void* param);
#endif

PACK(void *) dummy_func_no_subsys (PACK(void *)req_pkt,
  uint16 pkt_len);

PACK(void *) dummy_func_75 (PACK(void *)req_pkt,
  uint16 pkt_len);

PACK(void *) dummy_func_128 (PACK(void *)req_pkt,
  uint16 pkt_len);

PACK(void *) dummy_func_128_delay (PACK(void *)req_pkt,
  uint16 pkt_len);

PACK(void *) dummy_func_75_test (PACK(void *)req_pkt,
  uint16 pkt_len);
...
#ifdef FEATURE_WINMOB
   hLSM = LoadLibrary(L"Diag_LSM");
    if( NULL == hLSM )
    {
       printf(L"TestApp_MultiThread: LoadLibrary failed");
       return -1;
    }
    printf("TestApp_MultiThread: Loaded Diag_LSM successfully \n");

    ptr_func_init = (ptr_Diag_LSM_Init)GetProcAddress(hLSM,L"Diag_LSM_Init");
    if(NULL == ptr_func_init)
    {
       printf("TestApp_MultiThread: GetProcAddress failed");
       return -1;
    }
    printf("TestApp_MultiThread: GetProcAdress for Diag_LSM_Init succeeded.");

    bInit_Success = (ptr_func_init)(NULL);
#else
...
//removed WM specific code 
   #ifdef FEATURE_WINMOB
   g_testapp_thread_hdl = CreateThread(NULL, 0, StartSecondApp, (LPVOID)0, 0, NULL);
   #else
   pthread_create( &fd_testapp_thread_hdl, NULL, StartSecondApp, NULL );
   #endif
//removed WM specific code 
   #ifdef FEATURE_WINMOB
   if (g_testapp_thread_hdl == NULL)
   #else
  ...
  DIAGPKT_DISPATCH_TABLE_REGISTER (DIAGPKT_NO_SUBSYS_ID, test_tbl_1); // send_data 143 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER (DIAG_SUBSYS_TEST_CLIENT_1, test_tbl_2); // send_data 75 67 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER (DIAG_SUBSYS_TEST_CLIENT_MT, test_tbl_2); // send_data 75 69 0 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER (DIAG_SUBSYS_ANALOG, test_tbl_2); // send_data 75 66 0 0 0 0 0																		// 
  DIAGPKT_DISPATCH_TABLE_REGISTER_V2 (DIAG_SUBSYS_CMD_VER_2_F, DIAG_SUBSYS_TEST_CLIENT_1, test_tbl_2); // send_data 128 67 0 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER_V2 (DIAG_SUBSYS_CMD_VER_2_F,DIAG_SUBSYS_TEST_CLIENT_1, test_tbl_3);  // send_data 128 67 1 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER_V2_DELAY(DIAG_SUBSYS_CMD_VER_2_F, DIAG_SUBSYS_TEST_CLIENT_MT, test_tbl_4); //send_data 128 69 2 0 0 0 0 0 0
  DIAGPKT_DISPATCH_TABLE_REGISTER_V2_DELAY(DIAG_SUBSYS_CMD_VER_2_F, DIAG_SUBSYS_TEST_CLIENT_1, test_tbl_4); // send_data 128 67 2 0 0 0 0
...
MSG_TS(MSG_SSID_DIAG, MSG_LEGACY_HIGH, "TEST MSG WITH TIMESTAMP",
                    473040000000ULL);
...
#ifdef FEATURE_WINMOB
   ptr_func_DeInit = (ptr_Diag_LSM_DeInit)GetProcAddress(hLSM,L"Diag_LSM_DeInit");

  if(NULL == ptr_func_DeInit)
  {
      return -1;
  }
   (ptr_func_DeInit)();

   /* Free the library. This will decrement a reference count, the module will be unloaded only 
...
PACK(void *) dummy_func_no_subsys (
  PACK(void *)req_pkt,
  uint16 pkt_len
)
{
...
PACK(void *) dummy_func_128_delay (
  PACK(void *) req_pkt,
  uint16 pkt_len
)
{
  ...
  */
  ...
  rsp = diagpkt_subsys_alloc_v2_delay (rsp_ssid, DIAG_TEST_APP_F_128_DELAY, delay_rsp_id, sizeof(diagtestapp_delayed_rsp_type));
