...
int main(int argc, char *argv[])
{
	...
	char buffer[30],pid_buff[10];
	char pid_file[32] = "/data/diag_logs/diag_pid";
	struct timeval tv;
	time_t curtime;
	 ...
	 * can be placed there
	 */
	if (mkdir("/data/diag_logs", 0777))
		if (errno != EEXIST)
			DIAG_LOGE("diag_mdlog: Fail creating directory for"
				"diag_pid file");

	/*
	 * Determine our process id and write it to the file so that
	 * when we want to stop diag_mdlog we know its process id
	 */
	unlink(pid_file);
	if ((pid_fd = open(pid_file, O_RDWR | O_CREAT | O_EXCL | O_SYNC,
								0777)) < 0) {
		DIAG_LOGE("\n diag_mdlog: Unable to open Pid File,"
	...
	if (!dir_set) {
		...
		for (z = 0; z < NUM_PROC; z++) {
			strlcpy(output_dir[z], "/sdcard/diag_logs/",
							FILE_NAME_LEN);
...
/* test case 1
	while (1) {
		printf("\n ------------------------------------------------------- \n");
		printf(" To Switch logging mode \n \t Enter 1 for USB \n \t Enter 2 for SD \n \t Enter 3 for NO logging \n \t Enter 4 to Exit \n");
		scanf("%d", &choice);
		...
		case 2:
			printf(" \n SD mode \n");
			diag_switch_logging(MEMORY_DEVICE_MODE, output_dir);
			break;
		case 3:
			printf(" \n NO logging mode \n");
			diag_switch_logging(NO_LOGGING_MODE, NULL);
			break;
		case 4:
