...
)
{
  ...
  {
	 #ifdef FEATURE_WINMOB
		RETAILMSG(1,(TEXT("log_alloc: mask check returned FALSE \n")));
	   #else
...
void log_commit (PACK(void *)ptr)
{
  if (ptr)
  {
    ...
    /* Set pointer to begining of diag pkt, not the log */
	 ...
	 if(!WriteFile(ghWinDiag, (LPCVOID) ptr, DIAG_REST_OF_DATA_POS+LOG_DIAGPKT_OFFSET + phdr_ptr->len, &NumberOfBytesWritten, NULL)) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
#else
...
#ifdef FEATURE_WINMOB
				DWORD NumberOfBytesWritten = 0;
				if(!WriteFile(ghWinDiag, (LPCVOID) pdiag_data, DIAG_REST_OF_DATA_POS+FPOS (diag_log_rsp_type, log) + length, &NumberOfBytesWritten, NULL)) {
#else
				int NumberOfBytesWritten = 0;
...
#ifdef FEATURE_WINMOB
			RETAILMSG(1,(TEXT("log_submit: mask check returned FALSE \n")));
#else
