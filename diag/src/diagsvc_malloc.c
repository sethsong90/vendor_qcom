...
pthread_mutex_t gaDiagSvcMalloc_mutex;
...
void *DiagSvc_Malloc(size_t size, int Svc_ID)
{
	...
	case(GEN_SVC_ID):
		if (gnDiagSvcMalloc_Initialized)
		{
			...
			{
				if ((gaDiagSvcMalloc[i].in_use == 0) && (size <= DIAGSVC_MALLOC_ITEM_SIZE))
				{
					gaDiagSvcMalloc[i].in_use = 1;
					ret = gaDiagSvcMalloc[i].ptr;
					break;
				}
			}
	...
	case(PKT_SVC_ID):
		if (gnDiagSvcMalloc_Initialized)
		{
			...
			for (i=0; i < DIAGSVC_MALLOC_MAX_ITEMS ; i++)
			{
				if ((gaDiagSvcPktMalloc[i].in_use == 0) && (size <= DIAGSVC_MALLOC_ITEM_SIZE))
				{
					gaDiagSvcPktMalloc[i].in_use = 1;
					ret = gaDiagSvcPktMalloc[i].ptr;
					break;
				}
			}
			pthread_mutex_unlock( &gaDiagSvcMalloc_mutex );
