...
/* WM7 */
	...
	static HANDLE ghEvent_Shared_Mask = NULL;
#endif
   ...
   #endif
   {
      ...
      if(pEvent)
      {
        //removed WM specific code
         ...
         if(!WriteFile(ghWinDiag, (LPCVOID) pEvent, Alloc_Len, &NumberOfBytesWritten, NULL)) /*TODO: Check the Numberofbyteswritten against number of bytes we wanted to write?*/
		#else
...
event_alloc (event_id_enum_type id, uint8 payload_length, int* pAlloc_Len)
{
 ...
 /*    int idx = 0;
	   ...
	   #ifdef FEATURE_WINMOB
		RETAILMSG(1,(TEXT("In event_alloc: event_mask = %x \n"),event_mask));
	   #else
  ...
  if(event_mask)
    {
	   #ifdef FEATURE_WINMOB
		RETAILMSG(1,(TEXT("Attempting to Read event_mask \n")));
	   #else
       ...
       {
       #ifdef FEATURE_WINMOB
		   RETAILMSG(1,(TEXT("event_mask[%d]=%d\n"),idx,event_mask[idx]));
	   #else
...
// removed masking for Phase I
	...
	{
#ifdef FEATURE_WINMOB
		RETAILMSG(1,(TEXT("event_alloc: mask check returned FALSE \n")));
#else
...
// alloc_len = FPOS (event_store_type, payload.payload) + payload_length;
	// Prototyping Diag 1.5 WM7: Adding a uint32 so the windiag driver can identify this as an event.
