...
/** @brief get the flags for the session
 *  @param[in]
 *  @return flags
 */
uint32_t DASHMMIMediaInfo::flags() const
{
  uint32_t flag = 0;
  int64 audDur = 0, vidDur = 0;
  ...
  if (audDur && vidDur)
  {
    flag = (MediaExtractor::CAN_SEEK | MediaExtractor::CAN_SEEK_BACKWARD | MediaExtractor::CAN_SEEK_FORWARD | MediaExtractor::CAN_PAUSE);
  }
...
sp<MetaData> DASHMMIMediaInfo::GetMetaData(TrackTable::TrackInfo *track, size_t index,uint32_t mPortIndex)
{
     ...
     !m_trackTable.DoesExist(MMI_HTTP_VIDEO_PORT_INDEX) && (mPortIndex == MMI_HTTP_VIDEO_PORT_INDEX ))
  {
    ...
    if (ret == MMI_S_COMPLETE)
    {
      ...
      QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "DASHMMIMediaInfo::GetMetaData vid format(%d) %d port %ld",code,
      ...
      !m_trackTable.DoesExist(MMI_HTTP_AUDIO_PORT_INDEX) && (mPortIndex == MMI_HTTP_AUDIO_PORT_INDEX ))
  {
    ...
    ret = m_mmDASHMMIInterface->postCmd(handle,MMI_CMD_GET_STD_OMX_PARAM,&data);
    if (ret == MMI_S_COMPLETE)
    {
      ...
      QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                    "DASHMMIMediaInfo::GetMetaData aud format(%d) %d port %ld",code,
      ...
      !m_trackTable.DoesExist(MMI_HTTP_OTHER_PORT_INDEX) && (mPortIndex == MMI_HTTP_OTHER_PORT_INDEX ))
  {
    ...
    if (ret == MMI_S_COMPLETE)
    {
      ...
      QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "DASHMMIMediaInfo::GetMetaData text format(%d) %d port %ld",code,
  ...
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "DASHMMIMediaInfo::GetMetaData trying to set port %ld",
                track->m_nPort);
           ...
           (track->m_nPort == MMI_HTTP_AUDIO_PORT_INDEX))
  {
    ...
    switch(paramAudioStruct.eEncoding)
    {
      case OMX_AUDIO_CodingAAC:
      {
        ...
        data.nParamIndex = (MMI_IndexType) OMX_IndexParamAudioAac;
        aacFmt.nPortIndex = paramAudioStruct.nPortIndex;
        data.pParamStruct = &aacFmt;
    ...
    ret = m_mmDASHMMIInterface->postCmd(handle,MMI_CMD_GET_STD_OMX_PARAM,&data);
      ...
      case OMX_AUDIO_CodingMP3:
      {
        ...
        data.nParamIndex = (MMI_IndexType) OMX_IndexParamAudioMp3;
        mp3Fmt.nPortIndex = paramAudioStruct.nPortIndex;
        data.pParamStruct = &mp3Fmt;
