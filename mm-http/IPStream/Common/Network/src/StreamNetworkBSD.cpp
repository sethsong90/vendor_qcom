...
{
  int result = recv(sockfd, (char *)buf, len, 0);
/* check for user aborts */
  if (IsAborted())
  {
    /* return failure */
...
{
  int result = ::recvfrom(sockfd, (char *)buf, len, 0, fromaddr,(socklen_t *) fromlen);/*Edited for QNX*/
  /* check for user aborts */
  if (IsAborted())
  {
    /* return failure */
...
{
  int result = ::send(sockfd, (const char *)buf, len, MSG_NOSIGNAL);
  /* check for user aborts */
  if (IsAborted())
  {
    /* return failure */
...
{
  int result = ::sendto(sockfd, (char *)buf, len, 0, toaddr, tolen);;
  /* check for user aborts */
  if (IsAborted())
  {
    /* return failure */
...
)
{
  ...
  int result = ::ioctl((int)handle, request, (unsigned long *) arg_ptr);
  /* check for user aborts */
  if (IsAborted())
  {
    /* return failure */
