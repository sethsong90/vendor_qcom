...
public:
    ...
    * @return const char*
    */
   const char *ValueFor(uint32 method,
                        const char *name);
    ...
    */
   bool FindFirst(uint32 whichMethods, const char *&name, const char *&value);
...
private:
   ...
   int findHeaderEntry(const uint32 whichMethodsAffected, const char *nameToFind);
   int findFreeHeaderEntry();
   void freeAllHeaders();
   int freeHeader(const uint32 whichMethodsAffected, const char *nameToFree);
