...
public:
  ...
  bool StoreCookie(char *SetCookieValue, const char *Url);

  // Public API to get Cookie from cookie manager
  bool GetCookies(char *url, char *pHeader, uint32& pHeaderLen,
                           char *pValue, uint32& pValueLen);
private:
  ...
  bool  parsemonth(const char *cpsz, unsigned short int *pnMonth);

  void  FillRelativePath(char *relativePath, int relativePathBufSize,
  ...
  void  FillHostName(char *hostName, int hostNameBufSize,
                         int& hostNameBufSizeRequested, const char *ptrHost);
  void  FillHostPortPath(char *uri, char *hostName, int hostNameBufSize,
                             int& hostNameBufSizeRequested, unsigned short & portNumber,
                             char *relativePath, int relativePathBufSize,
                             int& relativePathBufSizeRequested);

  bool GetUrlHostPortRelativePath(const char *url, int urlLen,
