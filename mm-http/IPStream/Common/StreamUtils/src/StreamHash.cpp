...
StreamHash::Insert(int key, void* val)
{
  ...
  {
    rslt = InsertHelper((void *)key, val);
  }
  else
  {
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        if (TYPE_INT == m_KeyType)
        {
          hashKey = ComputeHashIndexForInt((int)key);
          (elem->m_HashKey).keyInt = (int)key;
        }
