...
URL::UrlErrCode URL::StripControlChars()
{
  ...
  if (((uint32)newLength) != m_urlLen)
  {
    QTV_MSG_PRIO2(QTVDIAG_RTSP, QTVDIAG_PRIO_HIGH,
      "Modified original length from %ld to %d",
      m_urlLen, newLength);
  }
  m_urlLen = newLength;
...
URL::UrlErrCode URL::GetHost(char * host,int maxHostLen) const
{
  ...
  if (std_strstr(m_url, "//[") == NULL)
  {
    ...
    uint32 ipAddrLen = QTV_MIN((unsigned int)(server_port_ptr - server_ip_ptr) + 1, sizeof(ip_addr));
  ...
  if (NULL != host)
  {
    ...
    {
       int len = QTV_MIN(maxHostLen-1,server_port_ptr - server_ip_ptr + 1);
...
URL::UrlErrCode URL::GetPort(uint32 * port) const
{
  ...
  int port_str_len = QTV_MIN(PORT_STRING_SIZE,((clip_name - server_port_ptr) + 1));
 ...
 * @return URL::UrlErrCode
 */
URL::UrlErrCode URL::GetClipName(char * clipName, int size) const
{
  ...
  if ((ip_addr_type_ptr = std_strstr(m_url, "//[::FFFF:")) != NULL)
  {
    ...
    if (URL_OK == m_errCode)
    {
      ...
      (void)std_strlcpy(ip_addr_type_ptr + 2,
                        ip_addr_type_ptr + 10,
                        port_ptr - ip_addr_type_ptr - 9);

      (void)std_strlcpy(port_ptr - 8,
                        port_ptr + 1,
                        turl + url_len - port_ptr);
    }
  }
  return m_errCode;
}
