...
bool HTTPCookieMgr::CreateCookie(HTTPCookie **ppCkList, NVPair *pwnvp,
                              ...
                              double creatTime)
{
   int nNameLen = std_strlen(pwnvp->pcName);
   int nValueLen = std_strlen(pwnvp->pcValue);
   int nSize;

   HTTPCookie *me = (HTTPCookie *)QTV_Malloc((sizeof(HTTPCookie) +
                    (nNameLen + nValueLen + 1 + nDomainLen + 1 + nPathLen + 1)) * sizeof(char));
   ...
   memset(me, 0, sizeof(HTTPCookie) + nNameLen + nValueLen + nDomainLen + nPathLen);
   ...
   me->value = me->CookieName + nNameLen + 1;
...
)
{
  ...
  bool           result             = true;
  uint32         maxAge             = -1;
  int            hostNameBufLen     = 0;
  int            relativePathBufLen = 0;
  ...
  if(((char *)0 == Url) || ((char *)0 == pCookieString))
  {
    result = false;
  }
  else
  {
    int cookieStringSize = std_strlen(pCookieString);
  ...
  if(result)
  {
   ...
   if ((true == result) && ((char *)0 != cpszMaxAge))
   {
     ...
     else
     {
       ...
       tExpires = currMSeconds + (maxAge * (1000));
   ...
   if(result)
   {
     ...
     {
       char *pcSlash;
       nPathLen = std_strlen(up.cpcPath);
       cpszPath = up.cpcPath;
   ...
   if(result)
   {
     if ((char *)0 != cpszDomain)
     {
       ...
       if(result)
       {
         ...
         nHostLen = std_strlen(up.cpcHost);
...
bool HTTPCookieMgr::IsCookieValidForURL(HTTPCookie *me, char *url)
{
  ...
  char *portStr = NULL;
   ...
   if((result) && (std_strnicmp(me->domain, UrlDomain, nDomainLen)))
   {
     result = false;
   }
...
)
{
  ...
  if(NULL == pValue)
  {
    ...
    while ((HTTPCookie *)0 != (pCk = *lfp))
    {
      ...
      if (IsCookieValidForURL(pCk, url))
      {
        if (0 != nCookies)
        {
                                ...
                                NVPair *awavp, int nMaxPairs)
{
   ...
   for (psz = *ppszIn, nNumPairs = 0;
      ...
      wavp.pcValue = (char *)std_memchr(wavp.pcName,cSep,psz - wavp.pcName);
 ...
 *   month index
 */
bool HTTPCookieMgr::parsemonth(const char *cpsz, unsigned short int *pnMonth)
{
...
double HTTPCookieMgr::ParseCookieDate(char *pszIn)
{
   ...
   if(!parseuint32(pszDay,10,&j.m_nDay) ||
     !parsemonth(pszMonth,(unsigned short int *)&j.m_nMonth) ||
     !parseuint32(pszYear,10,&j.m_nYear) ||
     !parseuint32(pszYear,10,&j.m_nYear) ||
     !parseuint32(pszYear,10,&j.m_nYear) ||
     !parseuint32(pszYear,10,&j.m_nYear) ||
     !parseuint32(pszHour,10,&j.m_nHour) ||
     !parseuint32(pszMinute,10,&j.m_nMinute) ||
     !parseuint32(pszSecond,10,&j.m_nSecond)) {
      return 0.0;
   }
...
(
  char *relativePath,
  int relativePathBufSize,
  int& relativePathBufSizeRequested,
...
(
 char *uri,
 char *hostName, int hostNameBufSize,
 int& hostNameBufSizeRequested,
 unsigned short & portNumber,
 char *relativePath, int relativePathBufSize,
 int& relativePathBufSizeRequested
)
{
  char *posHost = NULL;
                          ...
                          int& relativePathBufSizeRequested)
{
  ...
  if (urlLen < 0)
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
      "HTTPCookieStore::GetHostAndPortFromUrl - UrlLen '%d' invalid",
      urlLen);
  ...
  else
  {
    ...
    {
      std_strlcpy(localUrl, url, urlLen);
      result = true;
    }
  }
