...
public:
  ...
  //Sets the pause signal. When thread receives pause signal it modifies the state to idle
  void Pause();
  //Sets the resume signal. When thread receives resume signal it modifies the state from idle to running
  void Resume();
...
private:
  ...
  public:
    ...
    virtual bool DataRequest(int32 portIdx, void* pBuffHdr);
