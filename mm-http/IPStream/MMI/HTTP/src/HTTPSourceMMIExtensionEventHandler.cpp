  ...
  else
  {
    ...
    if (m_nEventWriteIndex < ARR_SIZE(m_eventQ))
    {
      ...
      QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "MMIEventQManager - Queued event to index %lu, "
                     "%lu events remain", m_nEventReadIndex, m_nOutstandingEvents );
      m_nEventWriteIndex = (m_nEventWriteIndex + 1) % ARR_SIZE(m_eventQ);
    }
    else
    {
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                     "MMIEventQManager - Invalid MMI event queue write index %lu",
                     m_nEventWriteIndex );
    }
    (void)MM_CriticalSection_Leave(m_hEventQLock);
  }
...
)
{
  ...
  else
  {
    ...
    if (m_nEventReadIndex < ARR_SIZE(m_eventQ))
    {
      ...
      QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "MMIEventQManager - Peeked event from index %lu, "
                     "%lu events remain", m_nEventReadIndex, m_nOutstandingEvents );
    }
    (void)MM_CriticalSection_Leave(m_hEventQLock);
  }
...
)
{
  ...
  else
  {
    ...
    if (m_nEventReadIndex < ARR_SIZE(m_eventQ))
    {
      ...
      m_nOutstandingEvents--;
      QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "MMIEventQManager - Dequeued event from index %lu, "
                     "%lu events remain", m_nEventReadIndex, m_nOutstandingEvents );
      m_nEventReadIndex = (m_nEventReadIndex + 1) % ARR_SIZE(m_eventQ);
    }
    else
    {
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                     "MMIEventQManager - Invalid MMI event queue read index %lu",
                     m_nEventReadIndex );
    }
    (void)MM_CriticalSection_Leave(m_hEventQLock);
  }
...
)
{
  ...
  if (GetNotify())
  {
    HTTPProtocolEvent protocolEvent = { protocolData->m_serverCode,
    ...
    if (rsltCode)
    {
      ...
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "HTTP Protocol Event Queued: serverCode:%ld", protocolData->m_serverCode );
    }
  }
  else
  {
    QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_LOW,
                  "HTTPProtocolEventManager: Dropping the Protocol Event with Code:%ld",
    ...
    if(bNotify)
    {
      if(pHTTPSourceMMIHelper)
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
          "ProcessBufferingStatus: Notify watermark event on port %lu as reached watermark type %d",
...
{
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
    "HTTPSourceMMIExtensionEventHandler::HTTPQOEEventManager::FillQOEData eventID = %lu",eventID);
  ...
  switch(eventID)
  {
    case QOMX_HTTP_IndexParamQOEPlay:
      if(!dataPtr)
      {
        size = sizeof(QOMX_QOE_DATA_PLAY);
      }else if(dataPlay)
      {
    ...
    case QOMX_HTTP_IndexParamQOESwitch:
      if(!dataPtr)
      {
        size = sizeof(QOMX_QOE_DATA_SWITCH);
      }else if(dataSwitch)
      {
                   ...
                   const uint32 eventID)
{
  ...
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
      "HTTPSourceMMIExtensionEventHandler::HTTPQOEEventManager::UpdateQOEData eventID = %lu",eventID);
    char* pVideoURL = NULL;
    uint32 nBandwidth = 0;
    uint32 nBufCount = 0;
    int    nURLSize = 0;
    int    nIPAddrSize = 0;
    int    nStopPhraseSize = 0;
    ...
    switch(eventID)
    {
      case QOMX_HTTP_IndexParamQOEPlay:
        ...
        if(dataPlay)
        {
          ...
          dataPlay->size = sizeof(QOMX_QOE_DATA_PLAY);
      ...
      case QOMX_HTTP_IndexParamQOEPeriodic:
        {
          ...
          int nSize = sizeof(QOMX_QOE_DATA_PERIODIC)+nURLSize+1+nIPAddrSize+1;
          if(dataPeriodic)
          {
      ...
      case QOMX_HTTP_IndexParamQOEStop:
        {
          ...
          int nSize = sizeof(QOMX_QOE_DATA_STOP)+nURLSize+1+nStopPhraseSize+1;
          if(dataStop)
          {
            QTV_Free(dataStop);
          }
                              ...
                              void *pClientData)
{
  ...
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
    "HTTPSourceMMIExtensionEventHandler::HTTPQOEEventManager::NotifyQOEEvent eventID = %lu",eventID);
    status = NotifyEvent(
              pHTTPSourceMMIHelper,
              portID,
              eventID,
