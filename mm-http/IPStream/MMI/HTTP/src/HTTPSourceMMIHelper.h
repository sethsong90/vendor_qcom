...
public:
  ...
  virtual int32 NotifyDataEvent(const HTTPDataCommand HTTPCommand,
  ...
  void NotifyMmi(unsigned int nEvtCode,
                 unsigned int nEvtStatus,
                 unsigned int nPayloadLen,
                 void *pEvtData,
                 void *pClientData);

  bool GetDurationBuffered(int32 portIdx, uint64 &duration);
...
private:
  ...
  void ProcessNotifyWatermarkEventStatus(const HTTPDownloadStatus HTTPStatus,
                                         void* pUserData);
  int32 ProcessDataRequestStatus(int32 portIdx,
                                 void *pBuffHdrParam);
  int32 ProcessFlushDataStatus(int32 portIdx,
                               void *pBuffHdrParam);
private:
