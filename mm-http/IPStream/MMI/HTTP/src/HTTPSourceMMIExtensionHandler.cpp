                                 ...
                                 MMI_OmxParamCmdType* pOmxParam)
{
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamStreamingProxyServer)
  {
    ...
    if (pURI)
    {
      int uriSize = pURI->nSize - FPOS(OMX_PARAM_CONTENTURITYPE, contentURI[0]);
      if (uriSize >= 0)
      {
        int32 proxyURILenReq = 0;
        ret = MMI_S_EBADPARAM;
        if (pHTTPController)
        {
          ...
          {
            pURI->nSize = proxyURILenReq;
            QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                          "HTTPMMIExtensionHandler Insufficient uriSize %d (required %lu)", uriSize, pURI->nSize);
            ret = MMI_S_COMPLETE;
          }
        }
      }
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamContentProtectionInfo)
  {
    ...
    else
    {
      ...
      {
        QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                  " Invalid nSize %lu, minSize %d",
                  pContentProtectionInfo->nSize, sizeof(QOMX_PARAM_STREAMING_CONTENTPROTECTIONINFO));
      }
      else
      {
        ret = MMI_S_COMPLETE;
        uint32 contentProtectionSize = 0;
        ...
        HTTPDownloadStatus eStatus =
                    m_pHTTPSourceMMI->m_pHTTPDataInterface->GetContentProtectElem(
                          drmType,
                          contentProtectionSize,
                          NULL);
        if (HTTPCommon::HTTPDL_SUCCESS == eStatus)
        {
          if (contentProtectionSize > 0)
          {
            if (pContentProtectionInfo->nContentProtectionInfoSize >= contentProtectionSize &&
                pContentProtectionInfo->nSize >=
                (FPOS(QOMX_PARAM_STREAMING_CONTENTPROTECTIONINFO, cContentProtectionData[0])) +
            ...
            else
            {
              ...
              pContentProtectionInfo->nContentProtectionInfoSize = contentProtectionSize + 1;
            }
          }
          else
          {
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamPsshInfo)
  {
              ...
              pPSSHInfo->nPortIndex <= MMI_HTTP_OTHER_PORT_INDEX ))
    {
      if (pPSSHInfo->nSize < sizeof(QOMX_PARAM_STREAMING_PSSHINFO))
      {
        QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                  " Invalid nSize %lu, minSize %d",
                  pPSSHInfo->nSize, sizeof(QOMX_PARAM_STREAMING_DRMINFO));
      }
      else
      {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                        "Invalid portIndex: %lu",pPSSHInfo->nPortIndex);
    }
  }
    ...
    if (pCallback && pCallback->nSize == sizeof(OMX_CONFIG_CALLBACKREQUESTTYPE))
    {
      if ((int)pCallback->nIndex == (int)QOMX_HTTP_IndexConfigStreamingProtocolEvent)
      {
        ...
        else
        {
          ret = MMI_S_EBADPARAM;
          QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
          "HTTPMMIExtensionHandler Invalid port index %lu",
          pCallback->nPortIndex );
        }
      }
      else
      {
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamVideoVc1)
  {
    QOMX_VIDEO_PARAM_VC1TYPE* pVc1Format = NULL;
    pVc1Format = (QOMX_VIDEO_PARAM_VC1TYPE*)pOmxParam->pParamStruct;
    if (!pVc1Format ||
        !m_pHTTPSourceMMI->IsValidPort(pVc1Format->nPortIndex, OMX_PortDomainVideo) ||
        (sizeof(QOMX_VIDEO_PARAM_VC1TYPE) != pVc1Format->nSize))
    {
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "HTTP_IndexParamVideoVc1: pVc1Format/port index is invalid" );
    }
    else
    {
      pPortInfo = &m_pHTTPSourceMMI->m_portVideo[pVc1Format->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pVc1Format, QOMX_VIDEO_PARAM_VC1TYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pVc1Format, pPortInfo->m_pPortSpecificData, sizeof(QOMX_VIDEO_PARAM_VC1TYPE));
      }
      ret = MMI_S_COMPLETE;
    }
  }
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamVideoVp6)
  {
    QOMX_VIDEO_PARAM_VPTYPE* pVpFormat;
    pVpFormat = (QOMX_VIDEO_PARAM_VPTYPE*)pOmxParam->pParamStruct;
    if (!pVpFormat ||
        !m_pHTTPSourceMMI->IsValidPort(pVpFormat->nPortIndex, OMX_PortDomainVideo) ||
        (sizeof(QOMX_VIDEO_PARAM_VPTYPE) != pVpFormat->nSize))
    {
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "HTTP_IndexParamVideoVP: pVpFormat/port index is invalid" );
    }
    else
    {
      pPortInfo = &m_pHTTPSourceMMI->m_portVideo[pVpFormat->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pVpFormat, QOMX_VIDEO_PARAM_VPTYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pVpFormat, pPortInfo->m_pPortSpecificData, sizeof(QOMX_VIDEO_PARAM_VPTYPE));
      }
      ret = MMI_S_COMPLETE;
    }
  }
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamVideoSpark)
  {
    QOMX_VIDEO_PARAM_SPARKTYPE* pSparkFormat;
    pSparkFormat = (QOMX_VIDEO_PARAM_SPARKTYPE*)pOmxParam->pParamStruct;
    if (!pSparkFormat ||
        !m_pHTTPSourceMMI->IsValidPort(pSparkFormat->nPortIndex, OMX_PortDomainVideo) ||
        (sizeof(QOMX_VIDEO_PARAM_SPARKTYPE) != pSparkFormat->nSize))
    {
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "HTTP_IndexParamVideoSPARK: pSparkFormat/port index is invalid" );
    }
    else
    {
      pPortInfo = &m_pHTTPSourceMMI->m_portVideo[pSparkFormat->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pSparkFormat, QOMX_VIDEO_PARAM_SPARKTYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pSparkFormat, pPortInfo->m_pPortSpecificData, sizeof(QOMX_VIDEO_PARAM_SPARKTYPE));
      }
      ret = MMI_S_COMPLETE;
    }
  }
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamVideoDivx)
  {
    QOMX_VIDEO_PARAM_DIVXTYPE* pDivxFormat;
    pDivxFormat = (QOMX_VIDEO_PARAM_DIVXTYPE*)pOmxParam->pParamStruct;
    if (!pDivxFormat || !m_pHTTPSourceMMI->IsValidPort(pDivxFormat->nPortIndex, OMX_PortDomainVideo) ||
        (sizeof(QOMX_VIDEO_PARAM_DIVXTYPE) != pDivxFormat->nSize))
    {
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "HTTP_IndexParamVideoDivx: pDivxFormat/port index is invalid" );
    }
    else
    {
      pPortInfo = &m_pHTTPSourceMMI->m_portVideo[pDivxFormat->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pDivxFormat, QOMX_VIDEO_PARAM_DIVXTYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pDivxFormat, pPortInfo->m_pPortSpecificData, sizeof(QOMX_VIDEO_PARAM_DIVXTYPE));
      }
      ret = MMI_S_COMPLETE;
    }
  }
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamAudioEvrcb || nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamAudioEvrcWb)
  {
    ret = MMI_S_EBADPARAM;
    OMX_AUDIO_PARAM_EVRCTYPE* pEvrcFormat;
    pEvrcFormat = (OMX_AUDIO_PARAM_EVRCTYPE*)pOmxParam->pParamStruct;
    if (!pEvrcFormat || !m_pHTTPSourceMMI->IsValidPort(pEvrcFormat->nPortIndex, OMX_PortDomainAudio) ||
        (sizeof(OMX_AUDIO_PARAM_EVRCTYPE) != pEvrcFormat->nSize))
    {
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "OMX_IndexParamAudioEvrc: pEvrcFormat/port index is invalid" );
    }
    else
    {
      pPortInfo = &m_pHTTPSourceMMI->m_portAudio[pEvrcFormat->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pEvrcFormat, OMX_AUDIO_PARAM_EVRCTYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pEvrcFormat, pPortInfo->m_pPortSpecificData, sizeof(OMX_AUDIO_PARAM_EVRCTYPE));
      }
      ret = MMI_S_COMPLETE;
    }
  }
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamFrameSizeQuerySupported)
  {
    ...
    else
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                        "HTTPMMIExtensionHandler FrameSize query on a non video port %lu",
                        pFrameSizeType->nPortIndex);
        ret = MMI_S_EBADPARAM;
      }

      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_FATAL,
        "QOMX_HTTP_IndexParamFrameSizeQuerySupported port %u, ret %lu", (int)pFrameSizeType->nPortIndex, ret);
    }
  }
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigStreamingDownloadProgressUnitsType)
  {
          ...
          sizeof(QOMX_CONFIG_STREAMING_DOWNLOADPROGRESSUNITS))
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                          "HTTPSourceMMIExtensionHandler:Invalid PortIndex:%lu",
                          pDownloadUnitsType->nPortIndex);
          ret = MMI_S_EBADPARAM;
      }
    }
  }
          ...
          sizeof(QOMX_CONFIG_STREAMING_DOWNLOADPROGRESSTYPE))
    {
          ...
          (pDownloadProgress->nPortIndex == OMX_ALL))
      {
        if(GetDownloadProgress(pDownloadProgress->nPortIndex,
                                pDownloadProgress->nCurrentStartOffset,
                                pDownloadProgress->nDataDownloaded))
        {
          ret = MMI_S_COMPLETE;
        }
      }
          ...
          if(AdpatationPropertiesLen > 0)
          {
            ...
            if(pDashPropertyInfo->nPropertiesSize >= AdpatationPropertiesLen)
            {
              m_pHTTPSourceMMI->m_pHTTPController->GetMediaProperties((char *)pDashPropertyInfo->cDashProperties,
                                                                        pDashPropertyInfo->nPropertiesSize);
            }
            else
            {
  ...
  else if (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigDASHResumeDiscontinuity)
  {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Invalid PortIndex: %lu", pResumeDiscontinuity->nPortIndex );
      ret = MMI_S_EBADPARAM;
    }
  }
  else if (nParamIndex == (OMX_INDEXTYPE) QOMX_HTTP_IndexConfigDASHRepositionRange)
  {
             ...
             pReposRange->nPortIndex <= MMI_HTTP_OTHER_PORT_INDEX ))
    {
          ...
          m_pHTTPSourceMMI->m_pHTTPSourceMMITrackHandler)
      {
        ...
        if (m_pHTTPSourceMMI->m_pHTTPDataInterface->GetConfig(mediaType, HTTPCommon::HTTP_ATTR_REPOSITION_RANGE, val))
        {
          ...
          ret = MMI_S_COMPLETE;
          QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                      "HTTPMMIExtensionHandler HTTPCommon::HTTP_ATTR_REPOSITION_RANGE %lld, %lld ", pReposRange->nMin, pReposRange->nMax);
        }
        else
        {
          QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Invalid PortIndex: %lu", pReposRange->nPortIndex );
      ret = MMI_S_EBADPARAM;
    }
  }
...
uint32 HTTPSourceMMIExtensionHandler::ProcessMMISetStdExtnParam(MMI_OmxParamCmdType* pOmxParam)
{
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamStreamingProxyServer)
  {
    ...
    if (pURI)
    {
      int uriLen = std_strlen((const char*)pURI->contentURI);

      if ((uriLen > 0 &&
      pURI->nSize >= (uriLen + 1 + FPOS(OMX_PARAM_CONTENTURITYPE, contentURI[0]))))
      {
        if (pHTTPController &&
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexStreamingConfigWaterMark)
  {
          ...
          pBufferingWaterMark->nSize == sizeof(QOMX_BUFFERINGWATERMARKTYPE))
    {
      ...
      if(ret == MMI_S_COMPLETE)
      {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
              "HTTPMMIExtensionHandler: Invalid Port Index:%lu",pBufferingWaterMark->nPortIndex);
          ret = MMI_S_EBADPARAM;
        }
      }
    }
  ...
  else if(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamQOE)
  {
    ...
    m_eventHandler.m_HTTPQOEEventMgr.SetQOENotifcation(pQOEEventReg->bNotify);
    ret = MMI_S_COMPLETE;
  }
else if((nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigStreamingProtocolEvent) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigMediaInfo) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamSeekAccess) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamFrameSizeQuerySupported) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigStreamingDownloadProgress) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamCompleteDashAdaptationProperties) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexParamDashMPD) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigDASHResumeDiscontinuity) ||
          (nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigDASHRepositionRange) ||
(nParamIndex == (OMX_INDEXTYPE)QOMX_HTTP_IndexConfigStreamingDownloadProgressUnitsType))
  {
    QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                   "HTTPSourceMMIExtensionHandler - Set not allowed on read-only parameter %d",
                   pOmxParam->nParamIndex );
  }
  return ret;
}
 ...
 * @return  MMI_S_COMPLETE on success or MMI_S_ENOTIMPL otherwise
 */
uint32 HTTPSourceMMIExtensionHandler::UpdateVideoPort(OMX_U32 PortIndex,
                                                      MediaTrackInfo &trackInfo)
{
  uint32 ret = MMI_S_EBADPARAM;
  HTTPSourceMMI::HttpSourceMmiPortInfo* pPortAddr =
    &m_pHTTPSourceMMI->m_portVideo[PortIndex];

  QOMX_VIDEO_CODINGTYPE eCompressionFormat = (QOMX_VIDEO_CODINGTYPE) pPortAddr->m_portDef.format.video.eCompressionFormat;

  switch(eCompressionFormat)
  {
    case QOMX_VIDEO_CodingVC1:
    {
      pPortAddr->m_pPortSpecificData =
        (QOMX_VIDEO_PARAM_VC1TYPE*)QTV_Malloc(sizeof(QOMX_VIDEO_PARAM_VC1TYPE));
      if ( NULL == pPortAddr->m_pPortSpecificData )
      {
        break;
      }
      QOMX_VIDEO_PARAM_VC1TYPE* pVC1Param =
        (QOMX_VIDEO_PARAM_VC1TYPE*)pPortAddr->m_pPortSpecificData;
      QOMX_STRUCT_INIT(*pVC1Param, QOMX_VIDEO_PARAM_VC1TYPE);
      pVC1Param->nPortIndex = MMI_HTTP_VIDEO_PORT_INDEX + PortIndex;
    }
    break;
    ...
    {
      pPortAddr->m_pPortSpecificData =
        (QOMX_VIDEO_PARAM_DIVXTYPE*)QTV_Malloc(sizeof(QOMX_VIDEO_PARAM_DIVXTYPE));
      if ( NULL == pPortAddr->m_pPortSpecificData )
      {
        break;
      }
      QOMX_VIDEO_PARAM_DIVXTYPE* pDivxParam =
        (QOMX_VIDEO_PARAM_DIVXTYPE*)pPortAddr->m_pPortSpecificData;
      QOMX_STRUCT_INIT(*pDivxParam, QOMX_VIDEO_PARAM_DIVXTYPE);
      UpdateDivxVideoPort(PortIndex, pDivxParam, trackInfo);
    }
    break;

    case QOMX_VIDEO_CodingVP:
    {
      pPortAddr->m_pPortSpecificData =
        (QOMX_VIDEO_PARAM_VPTYPE*)QTV_Malloc(sizeof(QOMX_VIDEO_PARAM_VPTYPE));
      if (NULL == pPortAddr->m_pPortSpecificData )
      {
        break;
      }
      QOMX_VIDEO_PARAM_VPTYPE* pVp6Param =
        (QOMX_VIDEO_PARAM_VPTYPE*)pPortAddr->m_pPortSpecificData;
      QOMX_STRUCT_INIT(*pVp6Param, QOMX_VIDEO_PARAM_VPTYPE);
      pVp6Param->nPortIndex = MMI_HTTP_VIDEO_PORT_INDEX + PortIndex;
      pVp6Param->eFormat = QOMX_VIDEO_VPFormat6;
      pVp6Param->eProfile = QOMX_VIDEO_VPProfileSimple;
    }
    break;

    case QOMX_VIDEO_CodingSpark:
    {
      pPortAddr->m_pPortSpecificData =
        (QOMX_VIDEO_PARAM_SPARKTYPE*)QTV_Malloc(sizeof(QOMX_VIDEO_PARAM_SPARKTYPE));
      if (NULL == pPortAddr->m_pPortSpecificData )
      {
        break;
      }
      QOMX_VIDEO_PARAM_SPARKTYPE* pSparkParam =
        (QOMX_VIDEO_PARAM_SPARKTYPE*)pPortAddr->m_pPortSpecificData;
      QOMX_STRUCT_INIT(*pSparkParam, QOMX_VIDEO_PARAM_SPARKTYPE);
      pSparkParam->nPortIndex = MMI_HTTP_VIDEO_PORT_INDEX + PortIndex;
      pSparkParam->eFormat = QOMX_VIDEO_SparkFormatUnused;
    }
    break;
...
)
{
  ...
  {
    QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                  "GetProtocolEvent: Invalid nSize %lu, minSize %d",
                  protocolEvent.nSize, sizeof(QOMX_CONFIG_STREAMING_PROTOCOLEVENTTYPE));
    return result;
  }
  ...
  if (true == isQueryforSizes)
  {
    ...
    {
      protocolEvent.nEntityBodySize = eEvent.entityBody.get_size();
      protocolEvent.nReasonPhraseSize = eEvent.reasonPhrase.get_size();
      ...
      QTV_MSG_PRIO3(QTVDIAG_STREAMING, QTVDIAG_PRIO_MED,
                    "GetHTTPProtocolEvent: Query result urisize %lu, "
                    "entitybody size %lu, reason size %lu",
                    protocolEvent.nContentUriSize,
                    protocolEvent.nEntityBodySize,
                    protocolEvent.nReasonPhraseSize);
  ...
  else if (protocolEvent.nSize >= (FPOS(QOMX_CONFIG_STREAMING_PROTOCOLEVENTTYPE, protocolEventText[0]))+
                                       ...
                                       protocolEvent.nContentUriSize + 1)
  {
    if (m_eventHandler.m_HTTPProtocolEventMgr.GetHTTPProtcolEvent(eEvent))
    {
      ...
      {
        unsigned int reasonPhraseLen = std_strlen(pReasonPhrase);
        maxToCopy = QTV_MIN(protocolEvent.nReasonPhraseSize, reasonPhraseLen);
      ...
      {
        unsigned int entityBodyLen = std_strlen(pEntityBody);
        maxToCopy = QTV_MIN(protocolEvent.nEntityBodySize, entityBodyLen);
  ...
  else
  {
    QTV_MSG_PRIO3( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                   ...
                   "nSize %lu, nReasonPhraseSize %lu, nEntityBodySize %lu",
                   protocolEvent.nSize,
                   protocolEvent.nReasonPhraseSize, protocolEvent.nEntityBodySize );
  }
...
)
{
  ...
  static const int messageHeadersOffset =
    FPOS(QOMX_STREAMING_PROTOCOLHEADERSTYPE, messageHeaders[0]);
  ...
  int maxBytesForMsgHeaders =
    protocolHeadersEvent.nSize - messageHeadersOffset;

  int totalBytesForMsgHeaders = protocolHeadersEvent.nMessageClassSize +
  ...
  {
    QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
      "GetProtocolHeadersEvent: Invalid nSize %lu, minSize %d",
       protocolHeadersEvent.nSize, sizeof(QOMX_STREAMING_PROTOCOLHEADERSTYPE));
  ...
  if (false == isInvalidParam)
  {
    if (true == isQueryforSizes)
    {
      ...
      if (m_eventHandler.m_HTTPProtocolHeadersEventMgr.PeekHTTPProtocolHeadersEvent(eEvent))
      {
        ...
        protocolHeadersEvent.nMessageClassSize =
          (msgClass ? std_strlen(msgClass) : 0);

        protocolHeadersEvent.nMessageAtributesSize =
          (msgAttrs ? std_strlen(msgAttrs) : 0);


        QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_HIGH,
          "GetProtocolHeadersEvent: Query result msgClassSize %lu, msgHdrsSize %lu",
          protocolHeadersEvent.nMessageClassSize, protocolHeadersEvent.nMessageAtributesSize);
    ...
    else
    {
      ...
      if (m_eventHandler.m_HTTPProtocolHeadersEventMgr.GetHTTPProtocolHeadersEvent(eEvent))
      {
        ...
        unsigned int msgClassSize = (msgClass ? std_strlen(msgClass) : 0);
        unsigned int msgHdrsSize = (msgHdrs ? std_strlen(msgHdrs) : 0);
        ...
        {
          protocolHeadersEvent.nMessageClassSize =
            QTV_MIN(protocolHeadersEvent.nMessageClassSize, msgClassSize);
        ...
        {
          protocolHeadersEvent.nMessageAtributesSize =
            QTV_MIN(protocolHeadersEvent.nMessageAtributesSize, msgHdrsSize);
        ...
        QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_MED,
          "GetProtocolHeadersEvent: Populate msgClassSize %lu, msgHdrsSize %lu",
          protocolHeadersEvent.nMessageClassSize, protocolHeadersEvent.nMessageAtributesSize);
...
/**@brief Updates Divx port information
  *@param[in] PortIndex - port index to be updated
              pDivxParam - Divx omx video param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMIExtensionHandler::UpdateDivxVideoPort(OMX_U32 videoPortIndex,
                                                        QOMX_VIDEO_PARAM_DIVXTYPE* pDivxParam,
                                                        MediaTrackInfo &trackInfo)
{
  if (NULL == pDivxParam)
  ...
  pDivxParam->nPortIndex = MMI_HTTP_VIDEO_PORT_INDEX + videoPortIndex;
  switch(trackInfo.videoTrackInfo.videoCodec)
  {
    case FILE_SOURCE_MN_TYPE_DIVX311:
      pDivxParam->eFormat = QOMX_VIDEO_DIVXFormat311;
      break;
    case FILE_SOURCE_MN_TYPE_DIVX40:
      pDivxParam->eFormat = QOMX_VIDEO_DIVXFormat4;
      break;
    case FILE_SOURCE_MN_TYPE_DIVX50_60:
      pDivxParam->eFormat = QOMX_VIDEO_DIVXFormat5;
      break;
    default:
      break;
  }
...
void HTTPSourceMMIExtensionHandler::NotifyWatermarkEvent(uint32 portIdxAndWatermarkType)
{
      ...
      portID <= MMI_HTTP_OTHER_PORT_INDEX)
  {
    ...
    QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                   "HTTPBuffering Notification on registration: portID:%lu,"
                   "WMType: %d, WMLevel: %lu, WMUnits: %d. Notification Sent %d",
                   portID,
                   bufferStatus.eCurrentWaterMark,
                   bufferStatus.nCurrentLevel,
  ...
  else
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                  "NotifyWatermarkEvent Invalid portID %lu", portID);
  }
}
  ...
  uint32 portID, QOMX_BUFFERINGSTATUSTYPE* pBufferStatus)
{
  ...
  if (m_pHTTPSourceMMI && m_pHTTPSourceMMI->m_pHTTPController && trackHdlr)
  {
    ...
    bool isOk = m_pHTTPSourceMMI->m_pHTTPSourceMMIHelper->GetDurationBuffered(
        pBufferStatus->nPortIndex, duration);
    ...
    QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "GetCurrentWatermarkStatus port %lu: watermarkType %d, level = %lu",
      portID, (int)pBufferStatus->eCurrentWaterMark, pBufferStatus->nCurrentLevel);
  }
...
)
{
  ...
  {
    case QOMX_MediaInfoTagTitle:
    case QOMX_MediaInfoTagAuthor:
    case QOMX_MediaInfoTagDescription:
    case QOMX_MediaInfoTagCopyright:
    case QOMX_MediaInfoTagDateCreated:
    case QOMX_MediaInfoTagGenre:
      {
    ...
    case QOMX_MediaInfoTagTrackNum:
      {
             ...
             pMediaInfo->nPortIndex <= MMI_HTTP_OTHER_PORT_INDEX ))
        {
                                                                         ...
                                                                         trackID))
          {
            ...
            pMediaInfo->nDataSize = sizeof(OMX_U32);
            rsltCode = MMI_S_COMPLETE;
          }
          else
          {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Invalid portIndex: %lu",pMediaInfo->nPortIndex);
          rsltCode = MMI_S_EBADPARAM;
        }
      }
      break;
    ...
    {
      //Get the Stream Type
      uint32 reqLen = sizeof(QOMX_MEDIASTREAMTYPE);

      if (pMediaInfo->cData && pMediaInfo->nDataSize >= reqLen)
      {
        rsltCode = MMI_S_EFAIL;
        QOMX_MEDIASTREAMTYPE *pStreamType;
...
)
{
  ...
  if (protocolHeader.nSize < sizeof(QOMX_CONFIG_STREAMING_PROTOCOLHEADERTYPE))
  {
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
      "GetProtocolHeader: Invalid nSize %lu, minSize %d",
       protocolHeader.nSize, sizeof(QOMX_CONFIG_STREAMING_PROTOCOLHEADERTYPE));
    return result;
  }
  ...
  if (protocolHeader.nHeaderNameSize > 0 &&
      protocolHeader.nSize >= (FPOS(QOMX_CONFIG_STREAMING_PROTOCOLHEADERTYPE, messageHeader[0])) +
        ...
        if (pHdrName)
        {
                                             ...
                                             pHeaderValue, headervalueSize))
          {
            ...
            if (protocolHeader.nHeaderValueSize && reqdBufSize)
            {
              ...
              headervalueSize = protocolHeader.nHeaderValueSize + 1;
  ...
  else
  {
    QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "HTTPSourceMMIExtensionHandler Invalid input parameter - "
                   "nSize %lu, nMessageClassSize %lu, nHeaderNameSize %lu, nHeaderValueSize %lu",
                   protocolHeader.nSize, protocolHeader.nMessageClassSize,
                   protocolHeader.nHeaderNameSize, protocolHeader.nHeaderValueSize );
  }
...
{
  uint32 result = MMI_S_EBADPARAM;
  int msgHdrLen = std_strlen((const char*)protocolHeader.messageHeader);
...
)
{
  ...
  else
  {
   ...
   else  if (pPSSHInfo->nSize >= (FPOS(QOMX_PARAM_STREAMING_PSSHINFO, cPSSHData[0])) +
                                 pPSSHInfo->nPsshDataBufSize)
   {
