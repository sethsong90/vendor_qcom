                                        ...
                                        OMX_U32 nCode, OMX_PTR pData)
{
  ...
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
               "HTTPSourceMMI::HTTPSourceMMICmd, code (relative to MMI_CMD_BASE) = %lu",
               (nCode - MMI_CMD_BASE));
  ...
  switch (nCode)
  {
    ...
    case MMI_CMD_GET_CUSTOM_PARAM:
      ...
      break;
    case MMI_CMD_USE_BUFFER:
      ret = pHttpSource->UseBuffer(pData);
      break;
    case MMI_CMD_EMPTY_THIS_BUFFER:
      ret = pHttpSource->EmptyThisBuffer(pData);
      break;
    ...
    case MMI_CMD_FLUSH:
      ...
      break;
    case MMI_CMD_ENABLE_PORT:
      ret = pHttpSource->EnablePort(pData);
      break;
    case MMI_CMD_DISABLE_PORT:
      ret = pHttpSource->DisablePort(pData);
      break;
...
HTTPSourceMMI::HTTPSourceMMI()
{
  ...
  if(m_pStopPhrase)
  {
    ...
    std_strlcpy(m_pStopPhrase,"\0",sizeof("\0"));
  }

  // Set Default URL for conformance test
  SetURL("http://mpeg4sol28.qualcomm.com/External/httplive/hesnot.3gp-20110218-123656.m3u8");
...
OMX_U32 HTTPSourceMMI::GetParam(void* parameter)
{
  ...
  switch (nOmxIndex)
  {
    ...
    {
      ret = MMI_S_EBADPARAM;
      OMX_AUDIO_PARAM_QCELP13TYPE* pQcelpFormat;
      pQcelpFormat = (OMX_AUDIO_PARAM_QCELP13TYPE*)pOmxParam->pParamStruct;
      if (!pQcelpFormat || !IsValidPort(pQcelpFormat->nPortIndex, OMX_PortDomainAudio) ||
          (sizeof(OMX_AUDIO_PARAM_QCELP13TYPE) != pQcelpFormat->nSize))
      {
        QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "OMX_IndexParamAudioAmr: pQcelpFormat/port index is invalid" );
        break;
      }
      else if(IsSeekPending())
      {
      ...
      pPortInfo = &m_portAudio[pQcelpFormat->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pQcelpFormat, OMX_AUDIO_PARAM_QCELP13TYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pQcelpFormat, pPortInfo->m_pPortSpecificData, sizeof(OMX_AUDIO_PARAM_QCELP13TYPE));
      }
      ret = MMI_S_COMPLETE;
    }
    break;
    ...
    case OMX_IndexParamAudioEvrc:
    {
      ...
      if (!pEvrcFormat || !IsValidPort(pEvrcFormat->nPortIndex, OMX_PortDomainAudio) ||
      ...
      pPortInfo = &m_portAudio[pEvrcFormat->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
    ...
    {
      ret = MMI_S_EBADPARAM;
      OMX_AUDIO_PARAM_MIDITYPE* pMidiFormat;
      pMidiFormat = (OMX_AUDIO_PARAM_MIDITYPE*)pOmxParam->pParamStruct;
      if (!pMidiFormat || !IsValidPort(pMidiFormat->nPortIndex, OMX_PortDomainAudio) ||
          (sizeof(OMX_AUDIO_PARAM_MIDITYPE) != pMidiFormat->nSize))
      {
        QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "OMX_IndexParamAudioMidi: pMidiFormat/port index is invalid" );
        break;
      }
      else if(IsSeekPending())
      {
      ...
      pPortInfo = &m_portAudio[pMidiFormat->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pMidiFormat, OMX_AUDIO_PARAM_MIDITYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pMidiFormat, pPortInfo->m_pPortSpecificData, sizeof(OMX_AUDIO_PARAM_MIDITYPE));
      }
      ret = MMI_S_COMPLETE;
    }
    break;
    ...
    {
      ret = MMI_S_EBADPARAM;
      OMX_AUDIO_PARAM_PCMMODETYPE* pPcmFormat;
      pPcmFormat = (OMX_AUDIO_PARAM_PCMMODETYPE*)pOmxParam->pParamStruct;
      if (!pPcmFormat || !IsValidPort(pPcmFormat->nPortIndex, OMX_PortDomainAudio) ||
          (sizeof(OMX_AUDIO_PARAM_PCMMODETYPE) != pPcmFormat->nSize))
      {
        QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "OMX_IndexParamAudioPcm: pPcmFormat/port index is invalid" );
        break;
      }
      else if(IsSeekPending())
      {
      ...
      pPortInfo = &m_portAudio[pPcmFormat->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pPcmFormat, OMX_AUDIO_PARAM_PCMMODETYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pPcmFormat, pPortInfo->m_pPortSpecificData, sizeof(OMX_AUDIO_PARAM_PCMMODETYPE));
      }
      ret = MMI_S_COMPLETE;
    }
    break;
    ...
    {
      OMX_VIDEO_PARAM_H263TYPE* pH263Format;
      pH263Format = (OMX_VIDEO_PARAM_H263TYPE*)pOmxParam->pParamStruct;
      ret = MMI_S_EBADPARAM;

      if (!pH263Format || !IsValidPort(pH263Format->nPortIndex, OMX_PortDomainVideo) ||
          (sizeof(OMX_VIDEO_PARAM_H263TYPE) != pH263Format->nSize))
      {
        QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "OMX_IndexParamVideoH263: pH263Format/port index is invalid" );
        break;
      }
      else if(IsSeekPending())
      {
      ...
      pPortInfo = &m_portVideo[pH263Format->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
      QOMX_STRUCT_INIT(*pH263Format, OMX_VIDEO_PARAM_H263TYPE);
      if (pPortInfo && pPortInfo->m_pPortSpecificData)
      {
        memcpy(pH263Format, pPortInfo->m_pPortSpecificData, sizeof(OMX_VIDEO_PARAM_H263TYPE));
      }
      ret = MMI_S_COMPLETE;
    }
    break;
  ...
  switch (nMMIIndex)
  {
    case MMI_IndexDomainDef:
    {
      ...
      switch ( pDomainDef->nPortIndex )
      {
        ...
        {
          switch ( pDomainDef->format.audio.eEncoding)
          {
#ifdef FEATURE_HTTP_AMR
    ...
    case MMI_IndexBuffersReq:
    {
      ...
      switch ( pParamBuffReq->nPortIndex )
      {
        ...
        case MMI_HTTP_VIDEO_PORT_INDEX:
        {
          ...
          pParamBuffReq->nDataSize = pPortInfo->m_portDef.nBufferSize;
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                      "GetCustomParam: Video Port Buffer Size %lu",pParamBuffReq->nDataSize );
          ret = MMI_S_COMPLETE;
        }
        break;
...
/**@brief Use this buffer
*@param[in] parameter - command data
*@param[out]
*@return success status
*/
OMX_U32 HTTPSourceMMI::UseBuffer(void* parameter)
{
  OMX_U32 ret = MMI_S_EBADPARAM;
  ...
  MMI_UseBufferCmdType* pUseBufferCmd = (MMI_UseBufferCmdType*)parameter;
  switch( pUseBufferCmd->nPortIndex)
  {
    case MMI_HTTP_VIDEO_PORT_INDEX:
    case MMI_HTTP_AUDIO_PORT_INDEX:
    case MMI_HTTP_IMAGE_PORT_INDEX:
...
/**@brief EmptyThisBuffer
*@param[in] parameter - command data
*@param[out]
*@return success status
*/
OMX_U32 HTTPSourceMMI::EmptyThisBuffer(void* parameter)
{
  OMX_U32 ret = MMI_S_EBADPARAM;
  ...
  {
    QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                  "EmptyThisBuffer Error: Param NULL" );
    return ret;
  }
  ...
  {
    QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "EmptyThisBuffer: port/buffer size/buffer ptr is invalid" );
    return ret;
  }
  ...
  else
  {
    ...
    else
    {
      ...
      QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                    "HTTPMMIFillThisBuffer for port %lu pBufferHdr 0x%p pBuffer 0x%p ret %d",
                     portIdx,
                    (void *)pBuffHdr,
...
OMX_U32 HTTPSourceMMI::GetSample(int32 portIdx, OMX_BUFFERHEADERTYPE *pBuffHdr)
{
  ...
  else if(ret==MMI_S_PENDING)
  {
    if (bPortSwitching)
    {
      ...
      if (eStatus == HTTPCommon::HTTPDL_SUCCESS)
      {
        ...
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Switched streams on track %ld mediaType %d, %ld/%ld tracks",
            ...
            for (uint32 index = 0; index < nNewNumTracks; index++)
            {
              if (pTrackInfo + index == NULL)
              {
                QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                               "Error: Track info for track %lu is NULL", index );
                continue;
              }
                ...
                if ( pTrackInfo[index].majorType != majorType )
                {
                  ...
                  if (eStatus != HTTPCommon::HTTPDL_SUCCESS)
                  {
                    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                                   "Switch failed on track %lu mediaType %d",
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                       "Switching streams on track %ld, waiting!", trackId );
      }
      else
      {
        SetPortSwitchingFlag(pPortInfo,false);
        ...
        if (m_pHTTPSourceMMIHelper)
        {
          ...
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Switch stream failed %d on track %ld, aborting!",
                         eStatus, trackId );
        }
      }
    }
        ...
        HTTPSourceMMITrackHandler::PLAYING)
    {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                    "Suppress watermark for track %ld", trackId);
    }
...
*@param[in] parameter - port data
*@param[out]
*@return success status
*/
OMX_U32 HTTPSourceMMI::EnablePort(void* parameter)
{
  OMX_U32 ret = MMI_S_COMPLETE;
  MMI_PortCmdType* pPortCmd = (MMI_PortCmdType*)parameter;
  HttpSourceMmiPortInfo* pPortInfo = NULL;
  ...
  switch(pPortCmd->nPortIndex)
  {
    case MMI_HTTP_VIDEO_PORT_INDEX:
    {
      if (IsValidPort(pPortCmd->nPortIndex, OMX_PortDomainVideo))
      {
        pPortInfo = &m_portVideo[ pPortCmd->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
        pPortInfo->m_portDef.bEnabled = OMX_TRUE;
      }
    }
    break;
    case MMI_HTTP_AUDIO_PORT_INDEX:
    {
      if ( IsValidPort(pPortCmd->nPortIndex, OMX_PortDomainAudio))
      {
        pPortInfo = &m_portAudio[ pPortCmd->nPortIndex - MMI_HTTP_AUDIO_PORT_INDEX];
        pPortInfo->m_portDef.bEnabled = OMX_TRUE;
      }
    }
    break;
    ...
    case MMI_HTTP_IMAGE_PORT_INDEX:
    {
      if ( IsValidPort(pPortCmd->nPortIndex, OMX_PortDomainImage))
      {
        pPortInfo = &m_portImage[ pPortCmd->nPortIndex - MMI_HTTP_IMAGE_PORT_INDEX];
        pPortInfo->m_portDef.bEnabled = OMX_TRUE;
      }
    }
    break;
...
*@return success status
*/
OMX_U32 HTTPSourceMMI::DisablePort(void* parameter)
{
  OMX_U32 ret = MMI_S_COMPLETE;
  MMI_PortCmdType* pPortCmd = (MMI_PortCmdType*)parameter;
  HttpSourceMmiPortInfo* pPortInfo = NULL;
  ...
  switch(pPortCmd->nPortIndex)
  {
    case MMI_HTTP_VIDEO_PORT_INDEX:
    {
      ...
      {
        pPortInfo = &m_portVideo[pPortCmd->nPortIndex - MMI_HTTP_VIDEO_PORT_INDEX];
        pPortInfo->m_portDef.bEnabled = OMX_FALSE;
      }
    }
    break;
    case MMI_HTTP_AUDIO_PORT_INDEX:
...
OMX_U32 HTTPSourceMMI::SelectStream (OMX_U32 portIdx, OMX_U32 streamNo)
{
                                                             ...
                                                             trk, majorType))
   {
     ...
     m_pHTTPSourceMMITrackHandler->SetTrackState(trk, majorType, tState, NULL);
     ret = MMI_S_COMPLETE;
   }
   return ret;
}
...
void HTTPSourceMMI::InitPorts()
{
  ...
  for(i = 0;i<MMI_HTTP_NUM_VIDEO_PORTS;i++)
  {
    ...
    pPort->m_portDef.nSize = sizeof(OMX_PARAM_PORTDEFINITIONTYPE);
    ...
    pPort->m_portDef.nBufferSize += MMI_HTTP_MAX_PORT_ENC_BUFFER_SIZE;
  ...
  for(i = 0;i<MMI_HTTP_NUM_OTHER_PORTS;i++)
  {
    ...
    pPort->m_portDef.nBufferSize =  MMI_HTTP_OTHER_PORT_DEFAULT_BUFFER_SIZE +
                                    MMI_HTTP_TEXT_EXTRADATA_BUFFER_SIZE;
...
)
{
  ...
  QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR," updateAudioPort bSendPortEvents:%d",bSendPortEvents);
                                       ...
                                       HTTPMediaTrackInfo &trackInfo)
{
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Get Codec data failed for track %lu status %d",
...
/**@brief Updates mp3 port information
  *@param[in] PortIndex - port index to be updated
              pMp3Param - mp3 omx audio param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMI::UpdateMp3AudioPort(OMX_U32 audioPortIndex,
                                       OMX_AUDIO_PARAM_MP3TYPE* pMp3Param,
                                       MediaTrackInfo &trackInfo)
{
  if (NULL == pMp3Param)
  {
  ...
  pMp3Param->nPortIndex = MMI_HTTP_AUDIO_PORT_INDEX + audioPortIndex;
  pMp3Param->nChannels    = trackInfo.audioTrackInfo.numChannels;
  pMp3Param->nBitRate     = trackInfo.audioTrackInfo.bitRate;
  pMp3Param->nSampleRate  = trackInfo.audioTrackInfo.samplingRate;;
  pMp3Param->eChannelMode = (trackInfo.audioTrackInfo.numChannels < 2) ?
                            OMX_AUDIO_ChannelModeMono : OMX_AUDIO_ChannelModeStereo;
  return;
}

/**@brief Updates qcelp port information
  *@param[in] PortIndex - port index to be updated
              pQcelpParam - qcelp omx audio param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMI::UpdateQcelpAudioPort(OMX_U32 audioPortIndex,
                                         OMX_AUDIO_PARAM_QCELP13TYPE* pQcelpParam,
                                         MediaTrackInfo &trackInfo)
{
  pQcelpParam->nPortIndex = MMI_HTTP_AUDIO_PORT_INDEX + audioPortIndex;
  pQcelpParam->eCDMARate = OMX_AUDIO_CDMARateFull;
  pQcelpParam->nChannels = trackInfo.audioTrackInfo.numChannels;
  return;
}

/**@brief Updates pcm port information
  *@param[in] PortIndex - port index to be updated
              pPcmParam - pcm omx audio param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMI::UpdatePcmAudioPort(OMX_U32 audioPortIndex,
                                       ...
                                       MediaTrackInfo &trackInfo)
{
  ...
  pPcmParam->eNumData = OMX_NumericalDataSigned;
  pPcmParam->eEndian = OMX_EndianLittle;
  pPcmParam->bInterleaved  = OMX_TRUE;
  pPcmParam->eChannelMapping[0] = OMX_AUDIO_ChannelLF;
  pPcmParam->eChannelMapping[1] = OMX_AUDIO_ChannelRF;
  for (i=2; i<16; i++)
  {
    pPcmParam->eChannelMapping[i] = OMX_AUDIO_ChannelNone;
  }
  pPcmParam->nPortIndex = MMI_HTTP_AUDIO_PORT_INDEX + audioPortIndex;
  switch(trackInfo.audioTrackInfo.audioCodec)
  {
    ...
    case FILE_SOURCE_MN_TYPE_G711_ALAW:
      pPcmParam->ePCMMode = OMX_AUDIO_PCMModeALaw;
      break;
    case FILE_SOURCE_MN_TYPE_G711_MULAW:
      pPcmParam->ePCMMode = OMX_AUDIO_PCMModeMULaw;
      break;
    default:
      break;
  }

  pPcmParam->nChannels = trackInfo.audioTrackInfo.numChannels;
  pPcmParam->nSamplingRate = trackInfo.audioTrackInfo.samplingRate;
  return;
}

/**@brief Updates evrc port information
  *@param[in] PortIndex - port index to be updated
              pEvrcParam - evrc omx audio param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMI::UpdateEvrcAudioPort(OMX_U32 audioPortIndex,
                                        OMX_AUDIO_PARAM_EVRCTYPE* pEvrcParam,
                                        MediaTrackInfo &trackInfo)
{
  if (NULL == pEvrcParam)
  {
  ...
  pEvrcParam->nChannels = trackInfo.audioTrackInfo.numChannels;
  pEvrcParam->eCDMARate = OMX_AUDIO_CDMARateFull;
  pEvrcParam->nPortIndex = MMI_HTTP_AUDIO_PORT_INDEX + audioPortIndex;
  return;
}
...
)
{
  ...
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
      "Video Port allocated Buffer Size %lu",
      pPortAddr->m_portDef.nBufferSize);
  ...
  pPortAddr->m_portDef.format.video.xFramerate   = trackInfo.videoTrackInfo.frameRate;
  ...
  if (trackInfo.videoTrackInfo.drmInfo.isDRMProtected)
  {
    pPortAddr->m_portDef.nBufferSize += MMI_HTTP_MAX_PORT_ENC_BUFFER_SIZE;
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
      "(DRM Protected)Video Port allocated Buffer Size %lu",
      pPortAddr->m_portDef.nBufferSize);
  }

  switch (pPortAddr->m_portDef.format.video.eCompressionFormat)
  {
    case OMX_VIDEO_CodingAVC:
    {
      pPortAddr->m_pPortSpecificData =
...
#ifdef FEATURE_HTTP_WM
/**@brief Updates Wmv port information
  *@param[in] PortIndex - port index to be updated
              pWmvParam - Wmv omx audio param
              trackInfo - track info
  *@param[out]
  * @return void
  */
void HTTPSourceMMI::UpdateWmvVideoPort(OMX_U32 videoPortIndex,
                                       ...
                                       MediaTrackInfo &trackInfo)
{
  ...
  pWmvParam->nPortIndex = MMI_HTTP_VIDEO_PORT_INDEX + videoPortIndex;
  switch(trackInfo.videoTrackInfo.videoCodec)
  {
    case FILE_SOURCE_MN_TYPE_WMV1:
      pWmvParam->eFormat = OMX_VIDEO_WMVFormat7;
      break;
    case FILE_SOURCE_MN_TYPE_WMV2:
      pWmvParam->eFormat = OMX_VIDEO_WMVFormat8;
      break;
    case FILE_SOURCE_MN_TYPE_WMV3:
      pWmvParam->eFormat = OMX_VIDEO_WMVFormat9;
      break;
    default:
      break;
  }
...
bool HTTPSourceMMI::IsEndOfStream(int32 portIdx)
{
  ...
  if (pPortInfo->m_nBufferFlags & OMX_BUFFERFLAG_EOS)
  {
    ...
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                 "Eos Detected on port %ld", portIdx);
  }
  return ret;
}
...
)
{
  ...
  pPortAddr->m_portDef.nBufferSize = trackDescription.maxSampleSize;
...
bool HTTPSourceMMI::CacheSelectedRepresentations(const char *str)
{
  ...
  if (str)
  {
    const int bufSize = 1 + std_strlen(str);
