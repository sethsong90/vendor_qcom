...
/** @brief Convert the HTTP status code to MMI status code for
  ...
  */
int32 HTTPSourceMMIHelper::NotifyDataEvent
(
 const HTTPDataCommand HTTPCommand,
 int32 portIdx,
 void *pBuffHdrParam
 ...
 )
{
  ...
  if (ret != MMI_S_PENDING)
  {
    ...
    NotifyMmi(MMI_RESP_LOAD_RESOURCES, ret, 0, NULL, pUserData);
  }
}
 ...
 )
{
  ...
  uint32 nEvtCode = MMI_RESP_START;
  ...
  if (bOpenComplete && !bSuppressNotification)
  {
    ...
    nEventDataSize = sizeof(MMI_ResourceLostMsgType);
...
int32 HTTPSourceMMIHelper::ProcessDataRequestStatus
(
 int32 portIdx,
 void *pBuffHdrParam
 )
{
  ...
  ret = m_pHTTPSourceMMI->GetSample(portIdx, pBuffHdr);
  if (MMI_S_PENDING == ret)
  {
    return -1;
  }
  ...
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
    "HTTPMMIFillThisBufferDone for port %lu pBufferHdr 0x%p pBuffer 0x%p",
    FillBuffCmdResp.nPortIndex,
    (void *)pBuffHdr,
    (void *)pBuffHdr->pBuffer );
  NotifyMmi(MMI_RESP_FILL_THIS_BUFFER,
    ret, sizeof(FillBuffCmdResp),
    &FillBuffCmdResp, m_pHTTPSourceMMI->m_pClientData);
  return 0;
}

int32 HTTPSourceMMIHelper::ProcessFlushDataStatus
(
 int32 portIdx,
 void *pBuffHdrParam
 )
...
(
 unsigned int nEvtCode,
 unsigned int nEvtStatus,
 unsigned int nPayloadLen,
 void *pEvtData,
 void *pClientData
 )
{
  if (m_pHTTPSourceMMI && m_pHTTPSourceMMI->m_HTTPSourceMMIAsync_CB)
  {
    ...
    else
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_DEBUG,
        ...
        nEvtCode-MMI_MSG_BASE, nEvtStatus-MMI_S_BASE);
      m_pHTTPSourceMMI->m_HTTPSourceMMIAsync_CB(nEvtCode, nEvtStatus,
        nPayloadLen, pEvtData,
...
void HTTPSourceMMIHelper::GetQOEData(uint32& bandwidth,uint32& reBufCount,
                                     char* pVideoURL,int& nURLSize,
                                     char* pIpAddr,int& nIPAddrSize,
                                     char* pStopPhrase,int& nStopPhraseSize)
{
