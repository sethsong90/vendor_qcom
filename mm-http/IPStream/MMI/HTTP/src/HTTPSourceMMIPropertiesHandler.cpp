  ...
  else if (pKey && pProp && pProp->data)
  {
    ...
    default:
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Error: Unsupported property for Set - id %lu", id );
      result = false;
      break;
    }
  }
...
)
{
  ...
  else if (pKey && pProp)
  {
    ...
    default:
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Error: Unsupported property for Get - id %lu", id );
      break;
    }
...
)
{
  ...
  if (pConfigFilePath)
  {
    ...
    else
    {
      ...
      if (MM_File_GetSize(pConfigFile, &configFileSize) == 0 && configFileSize > 0)
      {
        pConfigFileInfo = (char*)QTV_Malloc(configFileSize + 1);
        if (pConfigFileInfo == NULL)
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
              "Error: Memory allocation failed for config file info size : %lu", configFileSize );
        }
        else
        {
          int numRead = 0;
          if (MM_File_Read(pConfigFile, pConfigFileInfo, configFileSize, &numRead) == 0 &&
              numRead > 0)
          {
            if (static_cast<int> (configFileSize) != numRead)
            {
              QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                             "Error: Cannot read config file, only read %d of %lu bytes" ,
...
)
{
  ...
  {
    bOk = true;
    int configFileLen = std_strlen(pConfigFileInfo);
    int numBytesToBeParsed = configFileLen;
    int numLines = 1;
    char* pLineStart = pConfigFileInfo;
    int nNewLineCharLen = 0;
    ...
    while (numBytesToBeParsed > 0)
    {
      ...
      if (pSeparator)
      {
        ...
        int dstBufLen = STD_MIN(sizeof(keyStr), (unsigned int)(pPropKeyEnd - pLineStart + 1));
        ...
        dstBufLen = STD_MIN(sizeof(valStr),(unsigned int)(pLineEnd - pPropValStart + 1));
