...
/** @brief Get the track list for the current clip.
*
* @param[in] pTrackMapRecipient - Reference to the callback handler for
*                                 track list
*
* @return
...
* FALSE - Otherwise
*/
bool HTTPSourceMMITrackHandler::GetTrackMap
(
 void *pTrackMapRecipient
 )
{
  QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "HTTPSourceMMITrackHandler::GetTrackMap" );
  bool bOk = false;

  if (m_pHTTPController && m_pHTTPController->IsHTTPStreamerRunning())
  {
    //Queue the GET_TRACKS request on HTTPController
    if (m_pHTTPController->GetTracks(reinterpret_cast<void *> (pTrackMapRecipient)))
    {
      //GET_TRACKS request queued, notification will be sent later
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "HTTP GET_TRACKS request queued" );
        bOk = true;
    }
    else
    {
      //Failed to queue GET_TRACKS request
      QTV_MSG_PRIO( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "Error: Failed to queue HTTP GET_TRACKS request" );
    }
  }
  else
  {
...
)
{
  QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                 "HTTPSourceMMITrackHandler::Read for media type %d track %ld ",
      ...
      ret != MMI_S_PENDING)
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                  "Read failed for media type %d track %ld"
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Read attempted on an unselected  media type %d track %ld",
                   majorType, trackIdentifier );
  }
  else if (pTrackDescription && pTrackDescription->selectionState.isMuted)
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Read attempted on a muted media type %d track %ld ",
                   majorType, trackIdentifier );
  }
  else
  {
    ...
    if (status == HTTPCommon::HTTPDL_WAITING)
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Data underrun for media type %d track %ld",
                     majorType, trackIdentifier );
      ret = MMI_S_PENDING;
    }
    else if (!HTTPSUCCEEDED(status))
    {
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "CanPlayTrack() failed %d for media type %d track %ld",
    ...
    else
    {
      ...
      case FILE_SOURCE_DATA_OK:
            ...
            &&  httpSampleInfo.sSubInfo.sContentProtection.ucIsEncrypted)
        {
          ...
          if (psshInfo)
          {
            ...
            psshInfo->nSize = sizeof(QOMX_PARAM_STREAMING_PSSHINFO);

            if (FillExtraDataForExtraSampleInfo(nPortIndex,
                                           pBuffHdr,
                                           psshInfo,
                                          (QOMX_EXTRA_SAMPLE_INFO *)&httpSampleInfo.sSubInfo.sContentProtection
                                         )!= MMI_S_COMPLETE)
            {
              ret = MMI_S_EFAIL;
            }
      ...
      case FILE_SOURCE_DATA_INSUFFICIENT:
        ...
        if (!bSwitch)
        {
          ...
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Re-buffering started for track %ld",
                         trackIdentifier);
        }
        break;
      ...
      QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     ...
                     "size %d bytes, timestamp %lu msec, isSyncPoint %d httpstatus %d",
          ...
          m_pHTTPSourceMMIPropertiesHandler && m_pHTTPSourceMMIPropertiesHandler->IsMediaSampleLoggingAllowed())
      {
        ...
        sampleInfo.absTS = (uint32)((float)(httpSampleInfo.startTime * 0.001F) * pTrackDescription->timeScale);;
...
/** @brief Cancels any and all Read() calls which may be waiting for data.
* This is not supported currently.
*/
void HTTPSourceMMITrackHandler::CancelAllReads()
{
}
 ...
 )
{
    ...
    if (pTrackDescription)
    {
          ...
          !pTrackDescription->selectionState.isMuted)
      {
        ...
        else
        {
          QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                         "GetDownloadAndCurrentMediaPos failed for track %ld majorType %d",
                         trackID, majorType );
        }
      }
      else
      {
        QTV_MSG_PRIO4( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Track %ld majorType %d not in a good state, isSelected/isMuted %d/%d",
                       trackID, majorType,
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Invalid track %ld or media type %d",
                   trackID, majorType );
  }


  QTV_MSG_PRIO3( QTVDIAG_STREAMING, QTVDIAG_PRIO_MEDIUM,
                 "Buffering progress - %llu msec for track %ld mediaType %d",
      ...
      if( trackDescription.majorType == majorType )
      {
              ...
              trackDescription.bitRate != trackInfo.audioTrackInfo.bitRate)))
        {
          ...
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Codec change seen in between playback for track %ld - old minortype/new minortype %d/%d",
          ...
          (eStatus == HTTPCommon::HTTPDL_DATA_END))
  {
          ...
          it.advance() )
    {
      ...
      if ( (int)trackDescription.majorType == (int)majorType )
      {
        ...
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Codec removed for track %ld major/minor %d/%d",
...
)
{
  ...
  case HTTPCommon::HTTP_VIDEO_TYPE:
    {
      QTV_MSG_PRIO7( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "Video track %lu info: codec type %d, bitrate %lu bps, duration %lu msec, "
        "height %lu, width %lu, framerate %lu fps",
        TrackInfo.nTrackID,
        trackDescription.minorType,
  ...
  case HTTPCommon::HTTP_AUDIO_TYPE:
    {
       QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
       "Audio track %lu info: codec type %d,channel %d, bitrate %lu bps, duration %lu msec,"
        "samplingrate %lu sps",
        TrackInfo.nTrackID,
        trackDescription.minorType,
  ...
  case HTTPCommon::HTTP_TEXT_TYPE:
    {
      QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "Text track %lu info: codec type %d, duration %lu msec,"
         "height %lu, width %lu",
        TrackInfo.nTrackID,
        trackDescription.minorType,
        TrackInfo.audioTrackInfo.duration,
...
)
{
  ...
  QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
               "Queried using nUniqueID %d portIndex %lu mediaType %d majorType %d",
...
)
{
  ...
  int numTracks = m_pDataInterface->GetMediaTrackInfo(pTrackInfo);
    ...
    for (int index = 0; index < numTracks; index++)
    {
      ...
      else
      {
        ...
        if ( pTrackInfo[index].bSelected )
        {
          ...
          {
            QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "unable to copy the track details for track %lu",
  ...
  else
  {
    ...
    switch (pTrackDescription->playState)
    {
      case BUFFERING:
      {
        ...
        if ( pTrackDescription->reBuffering )
        {
          preroll = m_pHTTPController->GetRebufferPreroll();
        }
        else
        {
          preroll = m_pHTTPController->GetInitialPreroll();
        }

        (void)GetDownloadAndCurrentMediaPos(pTrackDescription->majorType,
                                            downloadPos,
                                            currPos);
        int buffDuration = downloadPos - currPos;
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                       "media type %d Track %ld: Buffering data - "
                       "available data (msec) %d, preroll (msec) %d",
                       majorType, pTrackDescription->trackID,
                       buffDuration, preroll );
        if (buffDuration >= (int)preroll)
        {
          result = HTTPCommon::HTTPDL_SUCCESS;
          nextPlayState = PLAYING;
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Buffering ended for media type %d track %ld",
                         majorType, pTrackDescription->trackID );
        }
      }
      break;
      ...
      {
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Bad play state for media type %d track %ld",
                       majorType, pTrackDescription->trackID );
...
{
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "HTTPSourceMMITrackHandler::GetMediaduration for port:%lu",
                portIdx );
...
{
  QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
  "HTTPSourceMMITrackHandler::GetTrackID for port:%lu",portIdx );
...
)
{
  ...
  if (pBuffHdr && pTrackDescription)
  {
    ...
    uint32 ulAddr = (uint32)( pBuffHdr->pBuffer) +  pBuffHdr->nFilledLen;
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                  "FillExtraDataForSubTitles::ulAddr[%lux]", ulAddr);

    uint32 requiredExtraDataLen  = MMI_HTTP_TEXT_EXTRADATA_BUFFER_SIZE;
    ...
    if (requiredExtraDataLen <= (pBuffHdr->nAllocLen - pBuffHdr->nFilledLen))
    {
      ...
      pSubTitleExtraData->nSize = sizeof(OMX_OTHER_EXTRADATATYPE)+
                                  (sizeof(OMX_U8)* sizeof(QOMX_SUBTITILE_DIMENSIONS)) - 4;
      pSubTitleExtraData->nDataSize = sizeof(QOMX_SUBTITILE_DIMENSIONS);
      QOMX_SUBTITILE_DIMENSIONS *dimensionsOffset = (QOMX_SUBTITILE_DIMENSIONS *)pSubTitleExtraData->data;
      ...
      ulAddr += pSubTitleExtraData->nSize;
      ...
      if (httpSampleInfo.sSubInfo.sSubTitle.subSampleCount > 0)
      {
        ...
        if (eCoding != OMX_IMAGE_CodingUnused)
        {
          ...
          if (pSubTitleSubExtraData)
          {
            ...
            pSubTitleSubExtraData->nSize = sizeof(OMX_OTHER_EXTRADATATYPE) +
                                           (sizeof(OMX_U8) * sizeof(QOMX_SUBTITLE_SUB_INFOTYPE)) - 4;
            pSubTitleSubExtraData->nDataSize = sizeof(QOMX_SUBTITLE_SUB_INFOTYPE);
            QOMX_SUBTITLE_SUB_INFOTYPE *subsOffset = (QOMX_SUBTITLE_SUB_INFOTYPE *)pSubTitleSubExtraData->data;
            ...
            ulAddr += pSubTitleSubExtraData->nSize;
            /* Aligned address to DWORD boundary */
            ulAddr = (ulAddr + 0x3) & (~0x3);
          }
        }
      ...
      if(pNoMoreExtraData)
      {
        pNoMoreExtraData->nSize = sizeof(OMX_OTHER_EXTRADATATYPE);
    ...
    {
       QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "InSufficient Buffer for Subtitile Extra Data, required(%lu), provided(%lu)",
                     requiredExtraDataLen,pBuffHdr->nSize );
       ret = MMI_S_EBADPARAM;
    }
  }
  else
...
)
{
  ...
  {
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
        "FillExtraDataForExtraSampleInfo::pBufHdr->nAllocLen[%lu] FilledLen[%lu]",
        pBuffHdr->nAllocLen,
        pBuffHdr->nFilledLen);
    ...
    {
      /* Filling up OMX_EXTRA_SAMPLE_INFO */
      ulAddr = (uint32) ( pBuffHdr->pBuffer) +  pBuffHdr->nFilledLen;
      ...
      pPSSHInfoData = (OMX_OTHER_EXTRADATATYPE *)ulAddr;
       /*Setting OMX_BUFFERFLAG_EXTRADATA flag*/
      pBuffHdr->nFlags |= OMX_BUFFERFLAG_EXTRADATA;
      pPSSHInfoData->nSize = sizeof(OMX_OTHER_EXTRADATATYPE) +
      ...
      pPSSHInfoData->nDataSize = sizeof(QOMX_PARAM_STREAMING_PSSHINFO);
      ...
      if (pExtraSampleInfo)
      {
        ...
        pExtraSampleInfoData->nSize = sizeof(OMX_OTHER_EXTRADATATYPE) +
        ...
        pExtraSampleInfoData->nDataSize = sizeof(QOMX_EXTRA_SAMPLE_INFO);
        ...
        ulAddr += pExtraSampleInfoData->nSize;
      }
    ...
    {
      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                "InSufficient Buffer for PSSH and ExtraSampleInfo, required(%d), provided(%lu)",
