 ...
 */
static const int DEFAULT_LINGER_TIMEOUT = 0;
 ...
 */
TransportConnection::ResultCode
TransportConnectionTcp::Send(const char *buffer, int bufsize, int& numWritten)
{
  QTV_NULL_PTR_CHECK(m_pCStreamNetwork, EFAILED);
 ...
 */
TransportConnection::ResultCode
TransportConnectionTcp::Recv(char *buffer, int bufsize, int& numBytesRead)
{
  QTV_NULL_PTR_CHECK(m_pCStreamNetwork, EFAILED);
  ResultCode result = EFAILED;
...
TransportConnectionTcp::CreateNonBlockingSocket()
{
  ...
  else
  {
    ...
    if (m_pCStreamNetwork->ioctl((DEVICE_HANDLE)m_Sockfd,
                                 FIONBIO,
                                 (void *)&ioctl_arg))
    {
      QTV_MSG_PRIO(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
    ...
    else
    {
      ...
      if (m_SocketRcvSizeMax > 0 && recvBufSize < m_SocketRcvSizeMax)
      {
