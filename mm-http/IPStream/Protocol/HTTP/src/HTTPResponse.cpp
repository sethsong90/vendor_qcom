...
HTTPResponse::IncreaseRspBufferSize()
{
  ...
  if (m_BufferLen >= HTTPStackCommon::MAX_HTTP_RSP_HDR_BUF_LEN)
  {
    QTV_MSG_PRIO(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
  ...
  {
    int reallocMaxStrLen = QTV_MIN(2*m_BufferLen,
                                   HTTPStackCommon::MAX_HTTP_RSP_HDR_BUF_LEN);
    ...
    else
    {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,
                    ...
                    "Realloc'ed m_Buffer to size '%d' + 1",
                    m_BufferLen);
...
HTTPResponse::ReceiveHTTPResponse(TransportConnection* httpConnection)
{
  ...
  int numToRead = m_BufferLen - m_NumBytesReadInBuffer;

  // Sanity check
  if (numToRead < 0)
  {
    QTV_MSG_PRIO(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                 "HTTPResponse::ReceiveHTTPResponse Num bytes to read < 0");
  }
  ...
  else
  {
    ...
    else
    {
      ...
      if (numRead > 0)
      {
        ...
        {
          m_ResponseHeaderLen = (posEndOfHTTPHeader - m_Buffer) + CSTRLEN("\r\n\r\n");

          QTV_MSG_PRIO(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_LOW, "RESPONSE HEADER:");
          for (int i=0; i < m_ResponseHeaderLen; ++i)
          {
            QTV_MSG_PRIO3(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_LOW,
              "%d: ascii '%d' %c", i, m_Buffer[i], m_Buffer[i]);
          }
                      ...
                      int& readLen)
{
  ...
  if (readBuf && readBufSize > 0)
  {
    if (RSP_DOWNLOADING_DATA == m_RspState)
    {
      ...
      int numBytesOfInitalDataInBuffer = m_NumBytesReadInBuffer - m_ResponseHeaderLen;

      if (numBytesOfInitalDataInBuffer > m_ContentLength)
      {
        // case where response buffer has response hrds or data for the next Request
        numBytesOfInitalDataInBuffer = m_ContentLength;
      }
      ...
      int maxToRead =
        (((m_ContentLength >= 0) && (false == m_FlagInvalidContentLength)) ?
         QTV_MIN(m_ContentLength - m_NumDataBytesReadByClient, readBufSize) :
         readBufSize);
    ...
    if (RSP_DONE != m_RspState)
    {
      if ((m_TransferEncodingFilter.IsTransferEncodingSet() && m_TransferEncodingFilter.IsTransferEncodingComplete()) ||
          ((false == m_FlagInvalidContentLength) && (m_NumDataBytesReadByClient == m_ContentLength)) ||
  ...
  {
    TransportConnection::ResultCode rslt = httpConnection->GetBytesAvailableToRead(readLen);
    ...
    {
      int numBytesInInitalDataInBuffer = m_NumBytesReadInBuffer - m_ResponseHeaderLen;
      if (m_NumDataBytesReadByClient < numBytesInInitalDataInBuffer)
      {
        int amtToCopyFromInitalBuffer = numBytesInInitalDataInBuffer - m_NumDataBytesReadByClient;
...
HTTPResponse::IsMessageLengthSpecifierSupported() const
{
  ...
  else
  {
    ...
    else
    {
      ...
      if(GetContentType())
      {
        static const int hdrStrLen = CSTRLEN("multipart/byteranges");
...
HTTPResponse::ExtractHTTPResponseReasonPhrase()
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
         int reasonPhraseStrLen = posEndResponseLine - posOnePastEndOfStatusCode;
...
HTTPResponse::ExtractHTTPResponseCode()
{
  ...
  else
  {
    ...
    if (posVersionStart && posFirstSpace)
    {
      ...
      int reqdBufSize = versionLen * sizeof(char);
    ...
    {
      int responseCodeStrLen = posOnePastEndOfStatusCode - posFirstSpace;
...
HTTPResponse::ExtractMessageLength()
{
  ...
  if (GETCSTRHEADERVALUE("Content-Length", value, STD_SIZEOF(value), ignored))
  {
    const char * end_ptr = NULL;
    m_nTotalContentLength = m_ContentLength = (int)std_scanul(value,
                                                      10,
                                                      &end_ptr,
                                                      &nErr);
    ...
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,
      "HTTPResponse::ExtractMessageLength: Message Length: '%lld'",
       m_ContentLength);
  ...
  if (GETCSTRHEADERVALUE("Content-Range", value, STD_SIZEOF(value), ignored))
  {
    char* posContentRange = std_strchr(value, '/');
    if (posContentRange)
    {
      const char * end_ptr = NULL;
      m_nTotalContentLength = (int)std_scanul(posContentRange + 1, 10,&end_ptr, &nErr);
      if (nErr == STD_NEGATIVE)
      {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,
        "HTTPResponse::ExtractMessageLength: Total Content Length: '%d'",
...
void HTTPResponse::ExtractCookieHeadersValue()
{
  ...
  while(GetNextCookieHeaderValue(&buf, NULL, 0, valueLenReq))
  {
    ...
    if(pElement)
    {
      ...
      if(GetNextCookieHeaderValue(&buf, cookieValue, pElement->size, valueLenReq))
      {
        ...
        line = std_strstr(line, "\r\n"))
    {
      ...
      if (!std_strnicmp(line, key, keyLen) && ':' == line[keyLen])
      {
        ...
        if (valueEnd)
        {
          ...
          valueLenReq = (valueEnd - valueStart) + 1;
        }
                                            ...
                                            int valueLen, int &valueLenReq)
{
  ...
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                  "HTTPResponse::GetNextCookieHeaderValue - Invalid KeyLen '%d' ",
                  keyLen);
  }
  else
  {
...
HTTPResponse::GetHeaders(char *headerStr, int headerStrBufSize, int& reqLen)
{
  ...
  if (m_Buffer)
  {
    ...
    {
      reqLen = (int)(posEndOfHeader - posStartOFHeader) + CSTRLEN("\r\n\r\n") + 1;
    }
  }
 ...
 * @param size
 */
void HTTPResponse::CommitEntityBodyBuffer(int size)
{
  m_NumBytesInEntityBody += size;
