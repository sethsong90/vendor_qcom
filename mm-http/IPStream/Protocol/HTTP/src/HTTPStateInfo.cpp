...
)
{
  ...
  if(Url)
  {
    ...
    if(bOk)
    {
      SetHeader(RequestID, header, hlen, value, vlen);
    }
...
HTTPReturnCode HTTPStateInfo::ReceiveResponse(uint32 RequestID)
{
     ...
     m_httpRequestHandler.IsRequestDone(RequestID))
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,
                  "RequestID:%lu is already marked as done",RequestID);
    return HTTP_SUCCESS;
  }
 ...
 /* if(nReturn == HTTP_WAIT &&
  ...
  if(nReturn == HTTP_SUCCESS)
  {
    ...
    if(nReturn == HTTP_SUCCESS && m_HTTPResponse.IsResponseHeaderFullyReceived())
    {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MEDIUM,"Received Successful response for Request( %lu)",
                    RequestID);
  ...
  QTV_MSG_PRIO2(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MEDIUM,"Receive(%lu) Response status :%d",RequestID,nReturn);
  ...
  if(nReturn == HTTP_SUCCESS)
  {
    if( !m_httpRequestHandler.IsRequestDone(RequestID))
    {
      if(m_HTTPResponse.IsResponseHeaderFullyReceived())
      {
        ...
        {
          QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,"Completely Recieved Data for Request( %lu)",
                    RequestID);
        }
      }
    }
...
)
{
  ...
  {
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Get Request Status Failed:%d for RequestID:%lu",
                  Status, RequestId);
  }
  return Status;
}
...
HTTPReturnCode HTTPStateInfo::CanReuseConnection(bool &bcanReuse)
{
  ...
  if(m_Connection)
  {
    ...
    {
      if (0 == std_strcmp(oldHostName, m_httpRequestHandler.GetHostName()))
      {
...
HTTPReturnCode HTTPStateInfo::HandleReconnect( uint32 requestId )
{
  ...
  if(rsltCode == HTTP_SUCCESS)
  {
    if (m_HTTPResponse.IsDownloadingData())
    {
      ...
      int nRangeLen = sizeof(cCurrRange);
      ...
      if (m_httpRequestHandler.GetHeader(requestId,
                                         ...
                                         std_strlen(HTTPStackCommon::RANGE_KEY),
                                         cCurrRange, nRangeLen))
      {
        ...
        {
          char cNewRange[32] = {0};
          int nNumBytesDownloaded = m_HTTPResponse.GetNumBytesRead();
          int nStart = 0, nEnd = -1;
          ...
          if (*pVal == '-')
          {
            ...
            {
              rsltCode = HTTP_SUCCESS;
              (void)std_strlprintf(cNewRange, sizeof(cNewRange), "bytes=-%d", nEnd);
            }
            else
            {
          ...
          else
          {
            ...
            if (pVal++)
            {
              ...
              if (nEnd >= 0)
              {
                if (nEnd >= nStart)
                {
                  (void)std_strlprintf(cNewRange, sizeof(cNewRange), "bytes=%d-%d",
                                       nStart, nEnd);
                }
                else
                {
              ...
              {
                //Open-ended request
                (void)std_strlprintf(cNewRange, sizeof(cNewRange), "bytes=%d-", nStart);
              }
            }
          }// if (*pVal == '-')
          ...
          if (rsltCode == HTTP_SUCCESS)
          {
            ...
            if (!m_httpRequestHandler.SetHeader(requestId,
                                                HTTPStackCommon::RANGE_KEY,
                                                std_strlen(HTTPStackCommon::RANGE_KEY),
                                                cNewRange, std_strlen(cNewRange)))
            {
              rsltCode = HTTP_FAILURE;
            }
