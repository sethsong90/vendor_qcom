...
static int CompareRequestID(void *itemPtr, void *compareVal)
{
  ...
  return (requestElem->m_requestID == (uint32)compareVal)? 1: 0;
}
...
)
{
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Request with ID: %lu not found",
                   RequestID);
  }
...
)
{
  ...
  else
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                   "Request with ID: %lu not found", RequestID );
  }
...
{
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,"SetRequestfor RequestID:%lu",
                RequestID);
  ...
  if ((Url == NULL) || (UrlLen < 0) ||
      (UrlLen > HTTPStackCommon::MAX_URL_LEN))
  {
    ...
    if (StreamQ_cnt(&m_requestQ) > 1)
    {
      ...
      if(pRequestElem != StreamQ_check(&m_requestQ))
      {
        ...
        if (pPrevRequestElem && pPrevRequestElem->m_httpRequest)
        {
          ...
          {
            if((std_strcmp(pPrevHTTPRequest->GetHostName(), httpRequest->GetHostName()) == 0) &&
          ...
          else
          {
            QTV_MSG_PRIO3( QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                           "No HostName (%s) or PortNo (%d) for the last Request Elem %lu to compare with",
...
)
{
  ...
  if(httpRequest)
  {
      ...
      if(pRequestElem)
      {
        ...
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,"Creating HTTPRequest Element for RequestID:%lu",
                    RequestID);
      }
      else
      {
  ...
  {
     QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"HTTPRequest Element with RequestID:%lu already exists",
                   RequestID);
  ...
  if(pRequestElem)
  {
    if((pRequestElem->m_requestID == RequestID) && pRequestElem->m_httpRequest)
    {
      if(pRequestElem->m_httpRequest->IsRspFullyRecd())
      {
        ...
        if(StreamQ_cnt(&m_requestQ) == 1)
        {
           ...
           {
             QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                "reamining one Request(%lu) is marked for deletion,Deleting and triggering internal close connection",
      ...
      else
      {
        ...
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                      "Delete Requested for current Active RequestID:%lu",RequestID);
    ...
    else if(StreamQ_cnt(&m_requestQ) > 1)
    {
      ...
      if(pRequestElem && pRequestElem->m_httpRequest)
      {
        ...
        {
          QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Marking Request with RequestID:%lu for Deletion",RequestID);
      ...
      {
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"DeleteRequest:HTTPRequest with RequestID:%lu is not Found",
                      RequestID);
      }
    }
  }
...
)
{
  ...
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,"ComposeAndQueueRequest for RequestID:%lu",
                  RequestID);
  ...
  if (!proxyServer)
  {
    if (false == pHTTPRequest->SetHeader(
      "Host", std_strlen("Host"),
      pHTTPRequest->GetHostName(), std_strlen(pHTTPRequest->GetHostName())))
    {
      result = false;
    }
  }
  else
  {
    if (false == pHTTPRequest->SetHeader(
      "Host", std_strlen("Host"),
      proxyServer, std_strlen(proxyServer)))
    {
      result = false;
    }
  }
  ...
  if (result &&
      (false == pHTTPRequest->HeaderExistsForKey("Connection",
                                               std_strlen("Connection"))))

  {
      if (false == pHTTPRequest->SetHeader(
        "Connection", std_strlen("Connection"),
        "Keep-Alive", std_strlen("Keep-Alive")))
    {
      result = false;
    }
  }
           ...
           pHTTPRequestElem = (HTTPRequestElem *)StreamQ_next(&m_requestQ,&pHTTPRequestElem->link))
      {
        ...
        if(! bExitLoop)
        {
          ...
          if(!pHTTPRequest->IsRequestCompletelySent())
          {
            ...
            if(result == HTTP_SUCCESS)
            {
              QTV_MSG_PRIO2(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MEDIUM,"Sent Request( %lu),%s out on network",
            ...
            {
              QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Failed to send Request( %lu)",
...
)
{
  ...
  HTTPRequestElem *pRequestElem = (HTTPRequestElem *)
   ...
   {
     QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"unable to find Request with ID:%lu",RequestID);
   }
   return result;
}
...
)
{
  ...
  if(pRequestElem && pRequestElem->m_httpRequest)
  {
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MEDIUM,"SetRequestState for Request( %lu ), Rsp Status: %d",
  ...
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Request( %lu) not found",RequestID);
  }
}
  ...
  if (StreamQ_cnt(&m_requestQ) > 0)
  {
    ...
    else if(pRequestElem)
    {
      QTV_MSG_PRIO2(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MEDIUM,
                    "PrepareForReconnect: RequestID:%lu, is the currently Active Request(%lu)",
...
)
{
  ...
  if(pRequestElem && pRequestElem->m_requestID == RequestID)
  {
    ...
    {
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                    "Handle Redirect for Request(%lu)",RequestID );
      ...
      if (NULL != redirectUrl)
      {
        ...
        int hostNameBufSizeRequested = 0;
        int relativePathBufSizeRequested = 0;
  ...
  else if(pRequestElem)
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,
                   "RequestID :%lu is not the active Request(%lu)",
                    RequestID,pRequestElem->m_requestID);
  }
 ...
 * @return HTTP_SUCCESS/HTTP_FAILURE
 */
HTTPReturnCode HTTPRequestHandler::SetRequestUrl(uint32 RequestID, const char* url, int urlLen)
{
  HTTPReturnCode rsltCode = HTTP_FAILURE;
    ...
    if(pConnection)
    {
         ...
         (0 != std_strcmp(oldHostName, pHTTPRequest->GetHostName()))))
      {
        ...
        QTV_MSG_PRIO5(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,
                     "SendPendingRequests:Head Request(%lu) have different hostname: %s : %d, current connections if on %s : %d",
...
)
{
  ...
  if(pHTTPRequestElem && pHTTPRequestElem->m_httpRequest && pHTTPRequestElem->bMarkForDeletion)
  {
    ...
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,"Deleting Request with RequestID:%lu",
                  pHTTPRequestElem->m_requestID);
