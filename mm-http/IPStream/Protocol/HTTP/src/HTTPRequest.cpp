...
HTTPHeaderStruct::SetKeyAndValue(char *key, char *value)
{
  ...
  int lenKey = std_strlen(key);
  int lenValue = std_strlen(value);
...
HTTPRequest::SetHTTPCommandLine(
  const char *command, int commandLen, const char *httpUrl, int httpUrlLen)
{
  bool result = false;

  if (commandLen > HTTPStackCommon::MAX_COMMAND_LEN ||
      httpUrlLen > HTTPStackCommon::MAX_URL_LEN)
  {
    ...
    if (headerStruct)
    {
      if (value)
      {
        ...
        (void)std_strlcpy(value, headerStruct->m_Value,
                          STD_MIN(valLen, std_strlen(headerStruct->m_Value) + 1));
      }
      valLen = std_strlen(headerStruct->m_Value) + 1;
    }
  }
  ...
  const char *url)
{
  ...
  {
    const int urlLen = std_strlen(url);
    ...
    int hostNameBufLen = 0, relativePathBufLen =0;
...
HTTPRequest::PrepareRequestBuffer()
{
  ...
  else
  {
    ...
    while (headerStruct)
    {
      int keyLen = std_strlen(headerStruct->m_Key);
      int valueLen = std_strlen(headerStruct->m_Value);
    ...
    int requiredBufSize = m_LengthOfRequest + 1;
...
HTTPReturnCode
HTTPRequest::SetRequestUrl(const char* url, int urlLen)
{
  HTTPReturnCode result = HTTP_FAILURE;
...
HTTPRequest::SetHostName(const char *hostName)
{
  ...
  if (hostName)
  {
    ...
    int hostNameBufSize = std_strlen(hostName) + 1;
