                             ...
                             int relativeUrlLen)
{
  ...
  if (result == HTTP_SUCCESS)
  {
    ...
    else if (&(HTTPStackStateObjects::HTTPStateConnectedObj) == m_HTTPState )
    {
      ...
      {
        QTV_MSG_PRIO3(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,"Creating New connection,result:%d,bCreate :%d,requestId:%lu",
...
HTTPReturnCode HTTPStackHelper::GetData(uint32 requestId, char *readBuf, int readBufSize, int *readLen)
{
...
HTTPReturnCode HTTPStackHelper::SetProxyServer (const char* proxyServer, int32 proxyServerLen)
{
  HTTPReturnCode result = HTTP_FAILURE;
  ...
  if ((proxyServerLen < 0) ||
      (proxyServerLen > HTTPStackCommon::MAX_PROXY_URL_LEN))
  {
    result = HTTP_BADPARAM;

    QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
      "HTTPStackHelper::SetProxyServer : Invalid proxyServerLen '%ld'",
       proxyServerLen);
  }
  else
  {
...
(
 char* ProxyServer,
 int32 ProxyServerLen,
 int32 &ProxyServerLenReq
)
{
  ...
  if (m_ProxyServerName)
  {
    int servLen = std_strlen(m_ProxyServerName);
    if (servLen > 0)
    {
      int reqProxyStrLen = servLen + 7; //(7 characters assuming max port 65535,":","\0" )
      ...
      if (localProxyStr)
      {
        ...
        int actualProxyStrLen = std_strlen(localProxyStr);
...
HTTPStackHelper::SendRequestInternal(uint32 requestId)
{
  ...
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_MED,
                "HTTPStackHelper::SendRequestInternal(%lu)",requestId);
...
HTTPReturnCode
HTTPStackHelper::NotifyEvent(int requestId, HTTPStackNotifyCode notifyCode)
{
  HTTPReturnCode rsltCode = HTTP_FAILURE;
  ...
  */
  ...
  static const int AUTHENTICATE_KEY_LEN = std_strlen(HTTPStackCommon::AUTHENTICATE_KEY);
  static const int PROXY_AUTHENTICATE_KEY_LEN = std_strlen(HTTPStackCommon::PROXY_AUTHENTICATE_KEY);
  static const char *DELIM = ": ";
  static const int DELIM_LEN = std_strlen(DELIM);
  static const char *TERMINATE = "\r\n\r\n";
  static const int TERMINATE_LEN = std_strlen(TERMINATE);
  ...
  if(m_HTTPStateInfo.GetNetPolicy(&netPolicyInfo))
  {
    ...
    if(bSetNetPolicy)
    {
      ...
      else
      {
        QTV_MSG_PRIO3(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_ERROR,
                      "Set NetPolicy succeeded,Iface:%ld , PDPProfileNo:%d, Addressfamily:%d",
    ...
    else
    {
      QTV_MSG_PRIO3(QTVDIAG_HTTP_STACK, QTVDIAG_PRIO_HIGH,
                    "Skipping setting NetPolicy, Values in use:iface:%ld, Addrfamily:%d, pdpProfileNo:%d",
