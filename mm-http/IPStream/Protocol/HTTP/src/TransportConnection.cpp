...
#include "qtv_msg.h"
#define MAX_IPADRESS_SIZE 20
TransportConnection::TransportConnection(
  ResultCode& result, CStreamNetwork *pCStreamNetwork,
  const char *serverIP, int serverIPLen, uint16 serverPort,
  int socketRcvSizeMax, int socketRcvSize, int socketSndSize)
{
  ...
  if (SUCCESS == result)
  {
    ...
    int serverIPBufSize = serverIPLen + 1;
    ...
    m_pIPAddress = (char *)QTV_Malloc(MAX_IPADRESS_SIZE);
    if(!m_pIPAddress)
    {
      m_pIPAddress = NULL;
    }
...
TransportConnection::GetIPAddr(int& size) const
{
  if(m_pIPAddress)
  {
    sprintf(m_pIPAddress,"%d.%d.%d.%d",m_Server.sin_addr.s_addr&0xFF,(m_Server.sin_addr.s_addr&0xFF00)>>8,(m_Server.sin_addr.s_addr&0xFF0000)>>16,(m_Server.sin_addr.s_addr&0xFF000000)>>24);
    size = std_strlen(m_pIPAddress);
  }
  return m_pIPAddress;
}
