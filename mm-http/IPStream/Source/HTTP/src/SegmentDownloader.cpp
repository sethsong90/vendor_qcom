...
SegmentDownloader::~SegmentDownloader()
{
  ...
  {
    QTA::HttpHandlerFactory::DeleteInstance(m_pHTTPStack);
    m_pHTTPStack = NULL;
  }
...
)
{
  ...
  if (pSessionInfo && pHTTPStack)
  {
    ...
    m_pSharedBandwidthEstimator = pBandwidthEstimator;
...
{
  IHttpHandler* pHTTPStack = m_pDownloader->m_pHTTPStack;
  ...
  if (HTTPCommon::ParseURL(url, HTTP_DEFAULT_PORT, pLaunchURL) && pLaunchURL)
  {
    ...
    if (pSessionInfo && pHTTPStack)
    {
      ...
      if (result == HTTP_SUCCESS)
      {
        ...
        {
          (void)pHTTPStack->SetHeader(m_pDownloader->m_nHTTPRequestID, "Host", std_strlen("Host"),
        ...
        if(pURL)
        {
          ...
          (void)pHTTPStack->SetHeader(m_pDownloader->m_nHTTPRequestID, "Range", std_strlen("Range"),
            range, std_strlen(range));
        }
        else
        {
          (void)pHTTPStack->UnsetHeader(m_pDownloader->m_nHTTPRequestID, "Range", std_strlen("Range"));
        }
