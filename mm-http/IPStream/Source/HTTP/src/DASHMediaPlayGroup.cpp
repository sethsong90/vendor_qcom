...
)
{
  ...
  if (bOk)
  {
    bOk = HttpHandlerFactory::CreateInstance(
       &m_pHTTPStack, NULL,
       m_pDASHSessionInfo->sSessionInfo.GetCookieStore(), NULL,
       (m_pDASHSessionInfo->sSessionInfo.IsQtaEnabled()
        ? HttpHandlerFactory::HTTP_STACK_QTA
        : HttpHandlerFactory::HTTP_STACK_DEFAULT)) == 0;
    ...
    else
    {
      QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "HttpHandlerFactory::CreateInstance failed");
    }
  }
...
void DASHMediaPlayGroup::DeInit()
{
  ...
  if (m_pHTTPStack)
  {
    HttpHandlerFactory::DeleteInstance(m_pHTTPStack);
    m_pHTTPStack = NULL;
  }
...
)
{
  ...
  if (nRepKey < (uint64)m_nNumReps && m_pRepresentation && m_pDASHSessionInfo)
  {
    ...
    bool bResult = true;
    if (GetRepresentationHandler(nRepKey) == NULL)
    {
      ...
      {
        SetRepresentationHandler(nRepKey,pRepHandler);
      }
    }
    ...
    for (int i = 0; i < m_nNumMediaComponents; i++)
    {
      ...
      if(pDataUnitElement)
      {
        if(DOWNLOAD_PARTIAL == pDataUnitElement->state)
        {
          ...
          else
          {
            QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Error in updating partial occupancy %lu duration %lu",
                           m_cMediaReadStore[i].nPartiallyDownloadedOccupancy, pDataUnitElement->nDuration );
            m_cMediaReadStore[i].nPartiallyDownloadedOccupancy = 0;
          }
        }
        else if(DOWNLOAD_COMPLETE == pDataUnitElement->state)
        {
          ...
          {
            QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Error in updating full occupancy %lu duration %lu",
                           m_cMediaReadStore[i].nFullyDownloadedOccupancy, pDataUnitElement->nDuration );
            m_cMediaReadStore[i].nFullyDownloadedOccupancy = 0;
          }
        }
...
)
{
  ...
  if (pRepHandler)
  {
      ...
      for (int i = 0; i < m_nNumMediaComponents; i++)
      {
        ...
        if (nPbTime < nMediaPbTime)
        {
          ...
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Playback %lu msec past switch point %lu msec, ignoring switch %lu/%lu",
        ...
        while(m_cMediaReadStore[i].m_cSwitchQ.Next(pIt, sElem))
        {
          if (sElem.nSwitchTime >= nPbTime)
          {
            ...
            QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                           "Overlapping switch point, removing duplicate %lu/%lu/%lu (rep/new/old)",
        ...
        if (!m_cMediaReadStore[i].m_cSwitchQ.EnQ(sElem))
        {
          status = HTTPCommon::HTTPDL_ERROR_ABORT;
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Could not queue switch point, ignoring switch %lu/%lu",
        ...
        else
        {
          QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Queued switch point %lu/%lu/%lu/%lu/%lu",
    ...
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Rep %lu could not switch %d, ignoring switch", (uint32)nRepKey, status );
      status = HTTPCommon::HTTPDL_ERROR_ABORT;
    }
  }
...
/** @brief Get the base time for the play group.
  ...
  * @return
  */
bool DASHMediaPlayGroup::GetBaseTime(uint64 &baseTime)
{
  bool bRet = false;
...
)
{
  ...
  if (pReadStore)
  {
    ...
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Invalid (%d) rep selection in playgroup %lu",
...
)
{
  ...
  if (pReadStore)
  {
    ...
    if (pRepHandler)
    {
      ...
      if (eReadState == INIT)
      {
        ...
        if (majorType == HTTPCommon::HTTP_VIDEO_TYPE||
          ...
          if (eStatus == HTTPCommon::HTTPDL_SUCCESS)
          {
            ...
            else
            {
              QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                             ...
                             "- %lu/%lu (available/required)", majorType, nRepKey, nSize, nLength );
      ...
      if (bRead)
      {
        ...
        if (eStatus == HTTPCommon::HTTPDL_SUCCESS)
        {
          ...
          else
          {
            ...
            {
              sampleInfo.endTime = currPeriodDuration;
            }
                 ...
                 eStatus == HTTPCommon::HTTPDL_DATA_END)
        {
          ...
          {
            pReadStore->SetPlaybackPosition(currPeriodDuration);
            QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        ...
        else
        {
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                         "GetNextMediaSample - majorType %d rep %d eStatus %d playback %lu msec",
  ...
  else
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                   "Invalid media type %d in playgroup %lu",
                                               ...
                                               const uint64 nDataUnitKey)
{
  if(m_pPeriodNotifier)
  {
    m_pPeriodNotifier->NotifyDownloadTooSlow(m_nKey, nRepKey, nDataUnitKey);
  }
}
...
)
{
  ...
  if(m_bInvalidGroup)
  {
    endTime = m_pDASHSessionInfo->cMPDParser.GetDuration(m_cRepGroupInfo.getKey());
  ...
  if (pReadStore)
  {
    ...
    if(m_pDASHSessionInfo->cMPDParser.IsLive())
    {
      ...
      HTTPDownloadStatus mpdStatus =
        m_pDASHSessionInfo->cMPDParser.GetLastAvailableSegmentForRepresentation(
           m_pRepresentation[nRepKey].m_cRepInfo.getKey(),
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                   "Invalid media type %d in playgroup %lu", mediaType, m_nMajorType );
  }
  ...
  if (pReadStore)
  {
    ...
    if (pRepHandler)
    {
     ...
     (void)pRepHandler->GetDownloadPosition(mediaType, nBufferedPosition);
      ...
      {
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Invalid buffered pos %lu playback pos %lu in %d/%lu rep/playgroup",
    ...
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Invalid representation key %d in playgroup %lu", nRepKey, m_nMajorType );
    }
  }
  else
  {
    ...
    if(!bRepresentationSelected)
    {
      if(IsSeekPending())
      {
        ...
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "Seek error for media (or major) type %lu", m_nMajorType );
  ...
  while (pDataUnitElement)
  {
      ...
      ((pDataUnitElement->nStartTime + pDataUnitElement->nDuration) < nPlaybackPos))
    {
      ...
      if(DOWNLOAD_COMPLETE == pDataUnitElement->state)
      {
        ...
        {
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Error in updating full occupancy %lu duration %lu start %llu prev %llu",
           ...
           (nPlaybackPos <= (pDataUnitElement->nStartTime + pDataUnitElement->nDuration)))
    {
      ...
      if(nPlaybackPos > pDataUnitElement->nPrevPlayPos)
      {
        if(DOWNLOAD_PARTIAL == pDataUnitElement->state)
        {
          ...
          else
          {
            QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Error in updating partial occupancy %lu current %llu prev %llu",
        ...
        else if(DOWNLOAD_COMPLETE == pDataUnitElement->state)
        {
          ...
          {
            QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Error in updating full occupancy %lu current %llu prev %llu",
                           nFullyDownloadedOccupancy, nPlaybackPos, pDataUnitElement->nPrevPlayPos );
            nFullyDownloadedOccupancy = 0;
          }
        }
...
)
{
  ...
  while (eReturn == HTTPCommon::HTTPDL_WAITING)
  {
    ...
    else
    {
      ...
      uint32 nNewPoolSize = inUseListSize + HTTP_MAX_DATA_UNITS;
      ...
      if (pNewPool)
      {
        ...
        for( uint32 k = inUseListSize; k < nNewPoolSize; k++ )
        {
          m_dataUnits[k].Reset();
...
)
{
  ...
  if(pDataUnitElement)
  {
    ...
    {
      QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Error in updating partial occupancy %lu duration %lu start %llu prev %llu",
                     nPartiallyDownloadedOccupancy, pDataUnitElement->nDuration,
                     pDataUnitElement->nStartTime, pDataUnitElement->nPrevPlayPos );
      nPartiallyDownloadedOccupancy = 0;
    }
