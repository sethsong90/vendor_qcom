...
PlaylistDownloadHelper::PlaylistDownloadHelper(HTTPSessionInfo& sessionInfo,
                                              IHttpStatusNotificationHandler* pStackNotificationHandler,
  ...
  m_nRequestID(nRequestIDfromResolver)
{
  ...
  if (!m_pHTTPStack)
  {
    result = HttpHandlerFactory::CreateInstance(
       &m_pHTTPStack, pStackNotificationHandler,
       sessionInfo.GetCookieStore(), NULL,
 ...
 )
{
  ...
  URL url(GetURL());
  int launchURLLen = url.GetUrlLength() + 1;
  uint32 port = 0;
  ...
  if (url.GetPort(&port) == URL::URL_OK)
  {
    ...
    else if (pDefaultPort)
    {
      ...
      {
        //Write http://
        int arg2 = std_strlen("http://") + 1;
        int dstBufLen = STD_MIN(launchURLLen, arg2);
        int numURLBytes = std_strlcpy(pLaunchURL,
          "http://", dstBufLen);
 ...
 )
{
  ...
  if (pSelf)
  {
    ...
    if (pSelf->ParseURL(HTTP_DEFAULT_PORT, pLaunchURL) && pLaunchURL)
    {
      ...
      IHttpHandler& HTTPStack = *(pSelf->m_pHTTPStack);
      ...
      if(HTTPStack.CreateRequest(pSelf->m_nRequestID) == HTTP_SUCCESS)
      {
        ...
        {
            (void)HTTPStack.SetHeader(pSelf->m_nRequestID, "Host", std_strlen("Host"),
            hostName, std_strlen(hostName));
        }
        const char* pUserAgent = (pSelf->m_sessionInfo).GetUserAgent();
        if (pUserAgent)
        {
          (void)HTTPStack.SetHeader(pSelf->m_nRequestID, "User-Agent",
          std_strlen("User-Agent"),
                                     pUserAgent, std_strlen(pUserAgent));
        }
  ...
  if (pSelf)
  {
    ...
    if (result == HTTP_WAIT || result == HTTP_SUCCESS)
    {
      ...
      if (result == HTTP_SUCCESS)
      {
        ...
        int64 representationLen = 0;
        (void)HTTPStack.GetContentLength(pSelf->m_nRequestID, &representationLen);
  ...
  if (pSelf)
  {
    ...
    if (pSelf->m_RepresentationBuffer && numBytesToRead > 0)
    {
      ...
      if (result != HTTP_FAILURE && result != HTTP_BADPARAM)
      {
        ...
        pSelf->m_nRepresentationBytesRead += numBytesRead;
        status = HTTPCommon::HTTPDL_WAITING;
