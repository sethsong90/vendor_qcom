                                           ...
                                           int& headerValueSize)
{
  ...
  {
    const char *val = m_httpOemHeaders.ValueFor(whichMethodsAffected, headerName);

    if (val)
    {
      int valLen = 1 + std_strlen(val);
      ...
      if (NULL == headerValue)
      {
        ...
        headerValueSize = valLen;
      }
      else
      {
        // populate headerValue
        int maxToCopy = QTV_MIN(headerValueSize, valLen);
 ...
 * @return bool
 */
bool HTTPSessionInfo::SetProxyServer(const char* proxyServer, int32 ProxyServerLen)
{
  bool retVal = false;
  ...
  else
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "SetproxyServer: Invalid input; proxyServer:0x%p,proxyServerLen:%ld",
