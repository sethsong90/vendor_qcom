...
(
 ...
 IHttpStatusNotificationHandler *pHTTPStatusHandler,
...
HTTPDownloadStatus DASHAdaptor::Seek(const int64 nSeekTime)
{
  ...
  //QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "DASHAdaptor::Seek nSeekTime %lu m_nSeekTime %lu\n", nSeekTime, m_nSeekTime);
      ...
      if (HTTPCommon::HTTPDL_SUCCESS == status)
      {
        ...
        if (pPeriodToSeek)
        {
          ...
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                        "Seek called on period %p for %d", pPeriodToSeek, seekTime);
        }
        else
        {
          QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
...
HTTPDownloadStatus DASHAdaptor::InitializeAndCreatePeriod(const int64 startTime, const bool bSeek)
{
  ...
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "InitializeAndCreatePeriod startTime %lu, status %d, bEOS %d",
...
HTTPDownloadStatus DASHAdaptor::CreatePeriod(PeriodInfo& periodInfo, const bool bOpen)
{
  ...
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_ERROR_ABORT;
    ...
    if (m_pDASHSessionInfo)
    {
      ...
      if (pPeriodHander && bResult)
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                      "Created periodHandler %p for period %lu",
        ...
        if (pElem->m_pPeriodHandler && bResult == true && bOpen)
          ...
          uint64 nFirstSegmentStartTime = 0;
          bool bStartOfPlayback = IsTracksSetup() ? false : true;
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "DASHAdaptor::CreatePeriod bStartOfPlayback %d", bStartOfPlayback );

          (void)m_playlistParser.GetSegmentStartTimeForPlayback(periodInfo.getPeriodKey(),
                                                                nFirstSegmentStartTime, bStartOfPlayback);
          status = (pElem->m_pPeriodHandler)->Open(nFirstSegmentStartTime, NULL);
    ...
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "DASHAdaptor::CreatePeriod period key %lu, status %d",
...
void DASHAdaptor::DownloadComplete(uint64 nPeriodKey)
{
  ...
  // can be used then.
  QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "DASHAdaptor::DownloadComplete for period 0x%08lx%08lx",
  ...
  if(attrType < HTTPCommon::HTTP_ATTR_MAX)
  {
    if(attrType == HTTPCommon::HTTP_ATTR_DURATION)
    {
      ...
      else
      {
        ...
        {
          attrVal.int_attr_val = (end >= start) ? (end - start) : 0;
          bResult = true;
        }
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "DASHAdaptor::GetConfig HTTP_ATTR_DURATION start %lu, end %lu, duration %lu",
    ...
    else if (attrType == HTTPCommon::HTTP_ATTR_PLAYBACK_DISCONTINUITY)
    {
      ...
      if (m_pDASHAdaptor->IsTracksSetup())
      {
        ...
        if (m_pDASHAdaptor->m_playlistParser.IsLive())
        {
          //For dynamic streams, (offsetFromAvailTime + m_nBasePeriodOffset) gives the right edge
          //in MPD time scale and offset it w.r.t. base media offset as playback starts at base
          //media offset. Reduce by TSB for the left edge
          uint64 tsbToUse = 1000 * m_pDASHAdaptor->m_playlistParser.GetTimeShiftBufferDepth();

          attrVal.sReposRange.nMaxDepth = tsbToUse;

          if (offsetFromAvailTime > m_pDASHAdaptor->m_nBaseMediaOffset)
          {
            attrVal.sReposRange.nMax = offsetFromAvailTime - m_pDASHAdaptor->m_nBaseMediaOffset;
            attrVal.sReposRange.nMin = (attrVal.sReposRange.nMax > tsbToUse) ? (uint64)(attrVal.sReposRange.nMax - tsbToUse) : 0;

           //When current live point reaches end of playback(offsetFromAvailTime > end), tsb window diminishes and reaches 0.
            uint64 end = 0;
            m_pDASHAdaptor->m_playlistParser.GetEndTime(end);
            if(end > 0)
            {
              attrVal.sReposRange.nMax = STD_MIN(attrVal.sReposRange.nMax, (end - m_pDASHAdaptor->m_nBaseMediaOffset));
              attrVal.sReposRange.nMin = STD_MIN(attrVal.sReposRange.nMin, attrVal.sReposRange.nMax);
        }
      }
        }
        else
        {
          ...
          if (offsetFromAvailTime > 0)
          {
            ...
            if (m_pDASHAdaptor->m_playlistParser.GetEndTime(end))
            {
              if (end > m_pDASHAdaptor->m_nBaseMediaOffset)
              {
                attrVal.sReposRange.nMax = end - m_pDASHAdaptor->m_nBaseMediaOffset;
              }
      ...
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "DASHAdaptor::GetConfig HTTP_ATTR_REPOSITION_RANGE nMin %lu, nMax %lu",
...
HTTPDownloadStatus DASHAdaptor::BaseStateHandler::GetContentProtectElem(HTTPDrmType &drmType,
...
HTTPDownloadStatus DASHAdaptor::GetContentProtectElem(HTTPDrmType &drmType,
                                          uint32 &contentProtectionInfoSize,
                                          unsigned char* contentProtectionData)
{
  ...
  if ( m_pCurrentStateHandler)
  {
    eStatus = m_pCurrentStateHandler->GetContentProtectElem(drmType, contentProtectionInfoSize,
...
bool DASHAdaptor::SelectRepresentations(const char* SetSelectionsXML)
{
  ...
  if ( m_pCurrentStateHandler)
  {
    ...
    if (SetSelectionsXML)
    {
      int reqdSize = 1 + std_strlen(SetSelectionsXML);
...
(
  ...
  int& nIPAddrSize)
{
  this->GetNetworkBandwidth(bandwidth);
 ...
 *
 */
void DASHAdaptor::GetNetworkBandwidth(uint32& bandwidth)
{
  bandwidth = 0;
  if(m_bEstimator)
  {
    uint32 dataDownloaded = m_bEstimator->GetCummulativeDataDownloaded();
 ...
 *
 */
void DASHAdaptor::GetVideoInfo(HTTPCommon::HTTPMediaType majorType, char* pVideoURL,int& nURLSize, char* pIPAddr, int& nIPAddrSize)
{
  int idx = GetReadQArrayIdx(majorType);
  MM_CriticalSection_Enter(m_pAdaptorLock);
        ...
        else
        {
          ...
          if (pNextReadQElem)
          {
            ...
            else
            {
              ...
              {
                QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                              "Delete readQElem for majorType %d for period key %llu, 0x%x",
                              majorType, pReadQElem->m_rPeriodHandler.GetPeriodKey(),
                              (unsigned int)&pReadQElem->m_rPeriodHandler);
              ...
              if (pPHQElem)
              {
                ...
                if (bCanDelete)
                {
                  ...
                  if(majorType == HTTPCommon::HTTP_TEXT_TYPE)
                  {
                    ...
                    {
                      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                        "Delete readQElem for text for period key %llu, 0x%x",
...
)
{
    ...
    if (pElem)
    {
      ...
      if(bCont)
      {
        ...
        while (pElem)
        {
          ...
          QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                        "DASHAdaptor::GetDurationBuffered mediaType %d,  pbPos %lu, buffDur %lu",
  ...
  {
    QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "DASHAdaptor::GetDurationBuffered Cumulative Buffer Occupancy mediaType %d, pbPos %lu, buffDur %lu",
                  mediaType, (uint32)nPlaybackPosition, (uint32)nBufferedDuration);
  }
...
HTTPDownloadStatus DASHAdaptor::UpdateMediaTrackInfo()
{
  ...
  if (!IsTracksSetup())
  {
    ...
    if (pHead && pHead->m_pPeriodHandler)
    {
      ...
      (void)pPH->GetBaseTime(m_nBaseMediaOffset);
      m_nBasePeriodOffset = pPH->GetPeriodStartTime();
      m_nBaseMediaOffset += m_nBasePeriodOffset;
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "m_nBasePeriodOffset %d, m_nBaseMediaOffset %d",
                     (uint32)m_nBasePeriodOffset, (uint32)m_nBaseMediaOffset );
    }
    else
    {
      QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
  ...
  for (int idx = 0; idx < NUM_MEDIA_TYPES; ++idx)
  {
    ...
    if (status == HTTPCommon::HTTPDL_SUCCESS)
    {
      if (pTrackInfo)
      {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Insufficient space to populate track info for %lu tracks", nNumTracks );
          break;
        }
      }
      nNumTracks++;
    ...
    if (pElem->m_pPeriodHandler)
    {
      if (nPlaybackTime >= pElem->m_pPeriodHandler->GetPeriodStartTime())
      {
        while (pElem)
        {
          ...
          if (pPeriodHandler)
          {
            double periodEndTime = pPeriodHandler->GetPeriodStartTime() +
