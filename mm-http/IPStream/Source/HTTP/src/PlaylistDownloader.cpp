...
char* PlaylistDownloader::GetRepresentationText(char* url)
{
  ...
  else
  {
    for(int i=0;i<m_nNumPlaylistDownloadHelpers;i++)
    {
      ...
      if(!std_strcmp(m_pDownloadHelpersList[i].url,url))
      {
      ...
      m_nRefreshTime)
    {
      ...
      if (HTTPCommon::HTTPDL_SUCCESS == status)
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "MPD Download time %lu, curTicCount %lu", m_nDownloadTime, (uint32)curTickCount);
      }
    }
    else
    {
...
void PlaylistDownloader::SetUpdateAndRefreshTime(char* url,uint32 update_time,uint32 refresh_time)
{
  if(m_bMasterDwnlderAvailable && m_pMasterDownloader)
  {
    ...
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "PlaylistDownloader::SetUpdateAndRefreshTime %lu ms, updatetime %lu ms",
  ...
  else
  {
    ...
    for(int i=0;i<m_nNumPlaylistDownloadHelpers;i++)
    {
      if(!std_strcmp(m_pDownloadHelpersList[i].url,url))
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,"update time %lu refresh time %lu",update_time,refresh_time);
...
int PlaylistDownloader::GetElapsedTimeForPlaylistUpdate() const
{
  ...
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
    "IsPlaylistRefreshedOnTime lastMPDRecd %lu, curTime %lld, minUpdatePeriod %lld",
