...
$Header: //source/qcom/qct/multimedia2/Video/Source/IPStream/Source/HTTP/dev/DASH/src/DASHMediaRepresentationHandler.cpp#77 $
      ...
      if (pTaskParam)
      {
        ...
        if (!m_nTaskID)
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Rep [0x%06lx]: TaskMediaRepresentation could not be added to scheduler",
    ...
    if (pURL)
    {
      m_pInitSegment = QTV_New_Args(InitializationSegment,
                                    (pURL, nStartOffset, nEndOffset,
                                     m_sDASHSessionInfo, m_pHTTPStack, m_bEstimator, bResult));
    }
  }

  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Rep [0x%06lx]: Created DASH representation handler (resource manager) 0x%p result %d",
...
DASHMediaRepresentationHandler::~DASHMediaRepresentationHandler()
{
  ...
  QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Rep [0x%06lx]: Destroyed DASH representation handler (resource manager) 0x%p",
 ...
 * pts offset.
 */
bool DASHMediaRepresentationHandler::GetBaseTime(uint64& baseTime)
{
  bool rslt = HTTPResourceManager::GetBaseTime(baseTime);
  if (rslt)
  {
    baseTime = (baseTime >= m_cRepInfo.GetPTSOffset()
                ? baseTime - m_cRepInfo.GetPTSOffset()
                : 0);
  }
  ...
  uint64& nDownloadPosition)
{
  ...
  double ptsOffset = m_cRepInfo.GetPTSOffset();
...
bool DASHMediaRepresentationHandler::IsLastSegDownloadSucceed()
{
  ...
  {
    nLastSegKey = nLastSegKey & 0x000000000FFFFFFF;
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
     ...
     while(m_cCmdQ.Next(pIt, cmd))
     {
       if(cmd.sBaseCmdData.eCmd == REPRESENTATION_CMD_OPEN)
       {
         ...
         QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Rep [0x%06lx]: OPEN cmd removed in close "
       ...
       else if(cmd.sBaseCmdData.eCmd == REPRESENTATION_CMD_SEEK )
       {
          m_cCmdQ.Remove(pIt);
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Rep [0x%06lx]: SEEK cmd removed in close "
                         "as resource manager is not in closing state",
                         (uint32)((m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40));
          m_bSeekPending = false;
       }
       else if(cmd.sBaseCmdData.eCmd == REPRESENTATION_CMD_NOTIFY_SEEK )
       {
          m_cCmdQ.Remove(pIt);
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Rep [0x%06lx]: Notify SEEK cmd removed in close "
                         "as resource manager is not in closing state",
                         (uint32)((m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40));
       }
     }
...
)
{
  ...
  if (!m_pRepHandler->m_cCmdQ.EnQ(cmd))
  {
    ...
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Rep [0x%06lx]: GET_SEGINFO cmd cannot be queued",
...
HTTPDownloadStatus DASHMediaRepresentationHandler::RepresentationBaseStateHandler::Close()
{
  ...
  if (m_pRepHandler->m_pInitSegment)
  {
...
int DASHMediaRepresentationHandler::RepresentationSetupStateHandler::ProcessCmds()
{
  ...
  if (m_pRepHandler->m_pInitSegment)
  {
    ...
    byte* pBuffer = NULL;
    int nBytesToRead = HTTP_MAX_DATA_CHUNK_LEN;
    ...
    if (pDataStore->GetBuffer(pBuffer, nBytesToRead))
    {
      ...
      if (result == HTTPCommon::HTTPDL_SUCCESS || result == HTTPCommon::HTTPDL_DATA_END)
      {
        ...
        {
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Rep [0x%06lx]: Init segment download progress %llu/%lld bytes",
                         nRepMPDKey, cDownloader.GetNumBytesReceived(),
                         cDownloader.GetContentLength() );
          /* Update size of data downloaded */
          m_pRepHandler->m_bEstimator->UpdateSize(nBytesRead);
        ...
        if (HTTPSUCCEEDED(status))
        {
          ...
          if (result == HTTPCommon::HTTPDL_DATA_END)
          {
            ...
            m_pRepHandler->SetStateHandler(&m_pRepHandler->m_cReadyStateHandler);
            QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                           "Rep [0x%06lx]: Init segment download complete", nRepMPDKey );
          }
          else
          {
      ...
      else if (result == HTTPCommon::HTTPDL_WAITING)
      {
        ...
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                       "Rep [0x%06lx]: Init segment waiting for data", nRepMPDKey );
      }
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Rep [0x%06lx]: Init segment download failed, closing", nRepMPDKey );
        ...
        && bGetSegInfoCmdProcessed == false)
    {
      ...
      if (eState == SEGINFO_CMDHANDLER_IDLE)
      {
        ...
        while (nDuration > 0)
        {
          ...
          if (status == HTTPCommon::HTTPDL_WAITING)
          {
            ...
            if (pSegHandler)
            {
              uint32 nSegKey = (uint32)(pSegHandler->GetKey() & 0x000000000FFFFFFF);
              ...
              if (!m_pRepHandler->m_sDASHSessionInfo.cMPDParser.GetSegmentInfoByKey(nCompleteKey, cSegInfo))
              {
                ...
                QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                              "Failed to retrieve segment for key with period %lu",
              ...
              QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                             "Rep [0x%06lx]: Segment info requested %d on seg %lu - "
                             "nStartTime %lu msec nDuration %lu msec PTO %llu msec",
                             (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
              ...
              if (status == HTTPCommon::HTTPDL_SUCCESS || status == HTTPCommon::HTTPDL_WAITING)
              {
                if (eState == SEGINFO_CMDHANDLER_IDLE)
                {
                  ...
                  cSegInfoCmdHandler.m_nStartIdx = nSegKey;
              ...
              else
              {
                ...
                QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                               "Rep [0x%06lx]: Error in getting seg info on seg %lu, "
                               "moving on start time %lu duration %lu",
                               (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
          ...
          else if (status == HTTPCommon::HTTPDL_SUCCESS || status == HTTPCommon::HTTPDL_INSUFFICIENT_BUFFER)
          {
            ...
            else
            {
              //If resource (seghandler) not already created do so now
              HTTPResource* pSegHandler = NULL;
              uint32 nSegKey = (uint32)(nSegMPDKey & 0x000000000FFFFFFF);
          ...
          else if (status == HTTPCommon::HTTPDL_TIMEOUT)
          {
            ...
            cmd->sGetSegInfoCmdData.nDuration = nFirstAvailSegStart - nStartTime;
      ...
      else if (eState == SEGINFO_CMDHANDLER_WAITING)
      {
        ...
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                       "Rep [0x%06lx]: Waiting for segment info from seg %d",
                       (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
      ...
      if (result != HTTPCommon::HTTPDL_WAITING)
      {
        ...
        if (m_pRepHandler->m_pGroupNotifier)
        {
          ...
          if (HTTPCommon::HTTPDL_SUCCESS == statusSegInfoReady)
          {
                                           ...
                                           pSegHandler) == HTTPCommon::HTTPDL_SUCCESS)
            {
              if (pSegHandler)
              {
                ...
                if (nRepEndTime > 0.0)
                {
                  nRepEndTime += nRepEndTime + m_pRepHandler->m_cRepInfo.GetPTSOffset();

                  if (pSegHandler->GetStartTimeForLastDataUnit(nLastStartTime, nRepEndTime))
                  {
                    ...
                    {
                      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                                     "Rep [0x%06lx]: Signaling DATA_END - nLastStartTime %d",
                                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
                                     (int)nLastStartTime );
          ...
          QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                         "Rep [0x%06lx]: GET_SEGINFO cmd (nStartTime %lu msec nDuration %lu msec "
                         "nNumDataUnits %lu processing complete %d - notifying group",
                         (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
      ...
      else
      {
        ...
        if(pSegHandler && pSegHandler->GetSegmentRange(segStartTime, nDuration))
        {
             ...
             HTTPCommon::HTTPDL_ERROR_ABORT)
          {
            ...
            m_pRepHandler->NotifySeekStatus(pSegHandler->GetKey(),-1,
                                            HTTPCommon::HTTPDL_ERROR_ABORT);
  ...
  if (nSegKey == (uint32)m_pRepHandler->m_cSegInfoCmdHandler.m_nStartIdx)
  {
    if(eStatus == HTTPCommon::HTTPDL_SUCCESS || (eStatus == HTTPCommon::HTTPDL_ERROR_ABORT))
    {
      ...
      if(pSegHandler)
      {
        ...
        nSegMPDStartTime = cSegInfo.getStartTime();
        if(pSegHandler->GetSegmentRange(nSegStartTime,nSegDuration))
        {
             ...
             nSegMPDStartTime <= m_pRepHandler->m_cSegInfoCmdHandler.m_nCmdStartTime)
          {
            ...
            QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                           "Rep [0x%06lx]: GET_SEGINFO mismatch in mpd start time %lu and segment start"
                           "time %lu requesting info on previous segment. New start time for request %lld",
                           (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
    ...
    if(result != HTTPCommon::HTTPDL_WAITING)
    {
      QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
              "Rep [0x%06lx]: GET_SEGINFO cmd processed %d by seg %lu (nNumDataUnits %lu)",
              (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
      ...
      {
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "Marking segment with key %lu as NOT_FOUND", nSegKey);
  ...
  while (bLoop)
  {
    ...
    if (pSegHandler && pSegHandler->GetSegmentRange(nSegStartTime, nSegDuration))
    {
      ...
      if (nPrevSegKey < 0 || (nSegKey == (uint64)nPrevSegKey + 1))
      {
        ...
        if (nRepEndTime > 0.0)
        {
          nReqDuration =
            STD_MIN(nReqDuration,
                    (uint64)(nRepEndTime + m_pRepHandler->m_cRepInfo.GetPTSOffset()) - nReqStartTime);
        }
        uint32 nFilled = 0;
        ...
        if (nFilled > 0)
        {
          ...
          if (pDataUnitInfo)
          {
            ...
            QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                           "Rep [0x%06lx]: Num data units filled %lu/%lu from seg %lu, remaining duration %lu msec",
                           (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
      ...
      else
      {
        ...
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Rep [0x%06lx]: Non-contiguous data unit %lu/%lu (curr/prev), breaking",
                       (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
...
)
{
  ...
  if (!m_pRepHandler->m_cCmdQ.EnQ(cmd))
  {
    status = HTTPCommon::HTTPDL_ERROR_ABORT;
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Rep [0x%06lx]: Notify Seek cmd cannot be queued",
                   (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40) );
  }
  else
  {
     QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "Rep [0x%06lx]: Notify Seek cmd queued successfully cnt %lu",
                    (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
...
)
{
  ...
  while(m_pRepHandler->m_cCmdQ.Next(pIt, cmd))
  {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Rep [0x%06lx]:: SEEK cmd already queued",
                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40));
      status = HTTPCommon::HTTPDL_WAITING;
      return status;
    }
  ...
  if (!m_pRepHandler->m_cCmdQ.EnQ(cmd))
  {
    status = HTTPCommon::HTTPDL_ERROR_ABORT;
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Rep [0x%06lx]: Seek cmd cannot be queued",
                   (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40) );
  }
  else
  {
     QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "Rep [0x%06lx]: Seek cmd queued successfully cnt %lu",
                    (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
                    m_pRepHandler->m_cCmdQ.Count());
...
)
{
  ...
  while(m_pRepHandler->m_cCmdQ.Next(pIt, cmd))
  {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Rep [0x%06lx]:: OPEN cmd already queued",
                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40));
      status = HTTPCommon::HTTPDL_WAITING;
      return status;
    }
  ...
  if(!m_pRepHandler->IsOpened())
  {
    ...
    if (!m_pRepHandler->m_cCmdQ.EnQ(cmd))
    {
      status = HTTPCommon::HTTPDL_ERROR_ABORT;
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Rep [0x%06lx]: OPEN cmd cannot be queued",
                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40) );
    }
    else
    {
       QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Rep [0x%06lx]: OPEN cmd queued successfully cnt %lu",
                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
                     m_pRepHandler->m_cCmdQ.Count());
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Rep [0x%06lx]:: OPEN cmd already processed",
                     (uint32)((m_pRepHandler->m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40));
    status = HTTPCommon::HTTPDL_WAITING;
  }
...
)
{
  ...
  if (ppSegHandler)
  {
    ...
    if (pResource)
    {
      ...
      uint64 nStartTime = cSegInfo.getStartTime() + m_cRepInfo.GetPTSOffset();
      uint64 nDuration = cSegInfo.getDuration();
      pResource->SetSegmentInfo(cSegInfo,nStartTime,nDuration, nSegKey);
      ...
      {
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Rep [0x%06lx]: Could not add resource (seg handler) %d",
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Could not create resource for key %lu", (uint32) nSegKey );
    }
...
)
{
  ...
  if (pSegHandler == NULL)
  {
    QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Rep [0x%06lx]: Invalid nDataUnitKey %lu (nSegIdx %lu)",
                   (uint32)((m_cRepInfo.getKey() & 0xFFFFFF0000000000ULL) >> 40),
...
void DASHMediaRepresentationHandler::ClearCmdQ(HTTPDownloadStatus eStatus)
{
  ...
  while(m_cCmdQ.Next(pIt, cmd))
  {
    if (cmd.sBaseCmdData.eCmd == REPRESENTATION_CMD_GET_SEGINFO)
    {
      ...
      {
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Rep [0x%06lx]: GET_SEGINFO cmd (nStartTime %lu nDuration %lu) "
...
)
{
  ...
  QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Rep [0x%06lx]: Moving to ERROR %d and notifying representation",
