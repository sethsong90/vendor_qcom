...
public:
  ...
  public:
    PrintXMLStringForDebug(char *pString) :
      m_Start(pString ? std_strlen(pString) : 0), m_pString(pString)
    {
    }
    ...
    ~PrintXMLStringForDebug()
    {
      if(m_pString)
      {
        int appendedLen = std_strlen(m_pString + m_Start);
  ...
  public:
    ...
    ~XMLGeneratorForRootElem()
    {
      ...
      {
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      " xml string len %d", std_strlen(m_pString));
      }
    }
  ...
  public:
        ...
        m_pString(pString), m_Size(size), m_Tag(tag)
    {
      if(m_Tag)
      {
        ...
        {
          m_Size += std_strlen(str1);
          m_Size += std_strlen(m_Tag);
          m_Size += std_strlen(str2);

          // add the key value
          m_Size += std_strlen(tmpBuf);

          m_Size += std_strlen(str3);
        }
        else
        {
          std_strlcat(m_pString, str1, m_Size);
  ...
  public:
      ...
      m_pString(pString), m_Size(size), m_pPropertiesString(pPropertiesString)
    {
      ...
      {
        m_Size += std_strlen(str1);
        m_Size += std_strlen(m_pPropertiesString);
        m_Size += std_strlen(str2);
      }
      else
      {
  ...
  public:
      ...
      m_pString(pString), m_Size(size), m_TagName(tagName), m_Value(value)
    {
      if(m_TagName && m_Value)
      {
        ...
        if(NULL == pString)
        {
          size += std_strlen(str1);
          size += std_strlen(tagName);
          size += std_strlen(str2);
          size += std_strlen(m_Value);
          size += std_strlen(str1);
          size += std_strlen(str4);
          size += std_strlen(tagName);
          size += std_strlen(str2);
        }
        else
      ...
      if(pContentDescs)
      {
        for(int k = 0; k < numDescs; ++k)
        {
          if(false == pContentDescs[k].IsInherited())
          {
            ...
            int reqdSize = std_strlen("<");
            pContentDescs[k].GetDesc(&pDescName, &pSchemeURI, &pValue);
            reqdSize += (pDescName ? std_strlen(pDescName) : 0);
            reqdSize += std_strlen(" schemeIdUri=\"");
            reqdSize += (pSchemeURI ? std_strlen(pSchemeURI) : 0);
            reqdSize += std_strlen("\" value=\"");
            reqdSize += (pValue ? std_strlen(pValue) : 0);
            reqdSize += std_strlen("\"/>");
  ...
  class XMLGeneratorForMarlinFormatVersion
  {
  public:
    XMLGeneratorForMarlinFormatVersion(
      char *pString, int& size, const char *majorType, const char *minorType)
    {
      int reqdSize = 0;

      static const char *str1 = "<";
      static const char *str2 = ContentProtectionType::MARLIN_FORMAT_VERSION;
      static const char *str3= " major=\"";
      static const char *str4 = "\"";
      static const char *str5 = " minor=\"";
      static const char *str6 = "/>";

      if(majorType || minorType)
      {
        if(NULL == pString)
        {
          reqdSize += std_strlen(str1);
          reqdSize += std_strlen(str2);

          if(majorType)
          {
            reqdSize += std_strlen(str3);
            reqdSize += std_strlen(majorType);
            reqdSize += std_strlen(str4);
          }

          if(minorType)
          {
            reqdSize += std_strlen(str5);
            reqdSize += std_strlen(minorType);
            reqdSize += std_strlen(str4);
          }

          reqdSize += std_strlen(str6);

          size += reqdSize;
        }
        else
        {
          std_strlcat(pString, str1, size);
          std_strlcat(pString, str2, size);

          if(majorType)
          {
            std_strlcat(pString, str3, size);
            std_strlcat(pString, majorType, size);
            std_strlcat(pString, str4, size);
          }

          if(minorType)
          {
            std_strlcat(pString, str5, size);
            std_strlcat(pString, minorType, size);
            std_strlcat(pString, str4, size);
          }


          std_strlcat(pString, str6, size);
        }
      }
    }
  ...
  class XMLGeneratorForContentIDs
  {
  public:
    XMLGeneratorForContentIDs(char *pString, int& size, IPStreamList<char *>& rContentIDList) :
      m_pString(pString), m_Size(size), m_rContentIDList(rContentIDList)
    {
      if(!m_rContentIDList.IsEmpty())
      {
        static const char *str_Br1 = "<";
        static const char *str_Br2 = ">";
        static const char* str1 = ContentProtectionType::MARLIN_CONTENTIDS;
        static const char* str2 = ContentProtectionType::MARLIN_CONTENTID;
        static const char* str_CloseBr1 = "</";

        if(NULL == m_pString)
        {
          m_Size += std_strlen(str_Br1);
          m_Size += std_strlen(str1);
          m_Size += std_strlen(str_Br2);

          IPStreamList<char *>::Iterator iterEnd = m_rContentIDList.End();
          for(IPStreamList<char *>::Iterator iter = m_rContentIDList.Begin();
               iter != iterEnd; iter = iter.Next())
          {
            ...
            m_Size += std_strlen(*iter);

            m_Size += std_strlen(str_CloseBr1);
            m_Size += std_strlen(str2);
            m_Size += std_strlen(str_Br2);
          }
        }
        ...
        {
          std_strlcat(m_pString, str_Br1, m_Size);
          std_strlcat(m_pString, str1, m_Size);
          std_strlcat(m_pString, str_Br2, m_Size);
               ...
               iter != iterEnd; iter = iter.Next())
          {
            ...
            std_strlcat(m_pString, *iter, m_Size);

            std_strlcat(m_pString, str_CloseBr1, m_Size);
            std_strlcat(m_pString, str2, m_Size);
            std_strlcat(m_pString, str_Br2, m_Size);
          }
        }
  ...
  private:
    char *m_pString;
    int& m_Size;
    IPStreamList<char *>& m_rContentIDList;
  };

  class XMLGeneratorForMarlineBroadBand
  {
  public:
    XMLGeneratorForMarlineBroadBand(char *pString, int& size, ContentProtectionType::MarlinBroadBandType& rBb) :
      m_pString(pString), m_Size(size), m_rMarlinBroadBand(rBb)
    {
      static const char* str1 = "<mas:MarlinBroadbandType>";
      static const char* str2 = "<mas:SilentRightsUrl>";
      static const char* str3 = "</mas:SilentRightsUrl>";
      static const char *str4 = "<mas:PreviewRightsUrl>";
      static const char *str5 = "</mas:PreviewRightsUrl>";
      static const char *str6 = "<mas:RightsIssuerUrl>";
      static const char *str7 = "</mas:RightsIssuerUrl>";

      if(!m_rMarlinBroadBand.m_SilentRightsUrlList.IsEmpty() ||
         !m_rMarlinBroadBand.m_PreviewRightsUrlList.IsEmpty() ||
         !m_rMarlinBroadBand.m_RightsIssuerUrlList.IsEmpty())
      {
        if(NULL == m_pString)
        {
          m_Size += std_strlen(str1);
          ...
          // SilentRightsUrl
          {
            IPStreamList<char *>& rList = m_rMarlinBroadBand.m_SilentRightsUrlList;
            IPStreamList<char *>::Iterator iterEnd = rList.End();
            for(IPStreamList<char *>::Iterator iter = rList.Begin();
                 iter !=  iterEnd; iter = iter.Next())
            {
              m_Size += std_strlen(str2);
              m_Size += std_strlen(*iter);
          ...
          // PreviewRightsUrl
          {
            IPStreamList<char *>& rList = m_rMarlinBroadBand.m_PreviewRightsUrlList;
            IPStreamList<char *>::Iterator iterEnd = rList.End();
            for(IPStreamList<char *>::Iterator iter = rList.Begin();
                 iter !=  iterEnd; iter = iter.Next())
            {
              m_Size += std_strlen(str4);
              m_Size += std_strlen(*iter);
              m_Size += std_strlen(str5);
            }
          }

          // RightsIssuerUrl
          {
            IPStreamList<char *>& rList = m_rMarlinBroadBand.m_RightsIssuerUrlList;
            IPStreamList<char *>::Iterator iterEnd = rList.End();
            for(IPStreamList<char *>::Iterator iter = rList.Begin();
                 iter !=  iterEnd; iter = iter.Next())
            {
              m_Size += std_strlen(str6);
              m_Size += std_strlen(*iter);
              m_Size += std_strlen(str7);
            }
          }
        }
        else
        {
          ...
          // PreviewRightsUrl
          {
            ...
            for(IPStreamList<char *>::Iterator iter = rList.Begin();
                 iter !=  iterEnd; iter = iter.Next())
            {
              std_strlcat(m_pString, str4, m_Size);
              std_strlcat(m_pString, *iter, m_Size);
              std_strlcat(m_pString, str5, m_Size);
            }
          }
          ...
          // RightsIssuerUrl
          {
            ...
            for(IPStreamList<char *>::Iterator iter = rList.Begin();
                 iter !=  iterEnd; iter = iter.Next())
            {
              std_strlcat(m_pString, str6, m_Size);
              std_strlcat(m_pString, *iter, m_Size);
              std_strlcat(m_pString, str7, m_Size);
            }
          }
        }
      }
    ...
    ~XMLGeneratorForMarlineBroadBand()
    {
      static const char* str1 = "</mas:MarlinBroadbandType>";
  ...
  private:
    char *m_pString;
    int& m_Size;
    ContentProtectionType::MarlinBroadBandType& m_rMarlinBroadBand;
  };

  class XMLGeneratorForContentProtection
  {
  public:
    XMLGeneratorForContentProtection(char *pString, int& size, ContentProtectionType& rCpt) :
      m_pString(pString), m_Size(size), m_rCpt(rCpt)
    {
      if(m_rCpt.GetContentProtectionSource() > ContentProtectionType::CONTENT_PROTECTION_NONE &&
         m_rCpt.GetContentProtectionSource() < ContentProtectionType::CONTENT_PROTECTION_MAX)
      {
        static const char* str1 = "<ContentProtection";
        static const char* str2 = ">";

        const char *pDescName = NULL, *pSchemeURI = NULL, *pValue = NULL;
        rCpt.GetDesc(&pDescName, &pSchemeURI, &pValue);

        static const char* strS = " schemeIdUri=\"";
        static const char* strV = " value=\"";
        static const char* strE = "=";
        static const char* strQ = "\"";

        bool bAddSchemeURI = false, bAddValue = false;
        if(pSchemeURI)
        {
          if(std_strcmp(pSchemeURI, ""))
          {
            bAddSchemeURI = true;
          }
        }
        ...
        if(NULL == m_pString)
        {
          ...
          if(bAddSchemeURI)
          {
            m_Size += std_strlen(strS);
            m_Size += std_strlen(pSchemeURI);
            m_Size += std_strlen(strQ);
          }

          if(bAddValue)
          {
            m_Size += strlen(strV);
            m_Size += strlen(pValue);
            m_Size += strlen(strQ);
          }
        ...
        else
        {
          ...
          if(bAddSchemeURI)
          {
            std_strlcat(m_pString, strS, m_Size);
            std_strlcat(m_pString, pSchemeURI, m_Size);
            std_strlcat(m_pString, strQ, m_Size);
          }

          if(bAddValue)
          {
            std_strlcat(m_pString, strV, m_Size);
            std_strlcat(m_pString, pValue, m_Size);
            std_strlcat(m_pString, strQ, m_Size);
          }
        ...
        case video::ContentProtectionType::CONTENT_PROTECTION_DASH:
          ...
          break;
        case ContentProtectionType::CONTENT_PROTECTION_MARLIN:
          {
            if(rCpt.GetContentProtectionSource() == ContentProtectionType::CONTENT_PROTECTION_MARLIN)
            {
              XMLGeneratorForMarlinFormatVersion(pString,size,m_rCpt.GetMajorType(), rCpt.GetMinorType());
              XMLGeneratorForContentIDs(pString,size,m_rCpt.GetContentIDList() );
              XMLGeneratorForMarlineBroadBand(pString,size, m_rCpt.GetMarlinBroadBand());
            }
          }
          break;
        default:
    ...
    ~XMLGeneratorForContentProtection()
    {
      if(m_rCpt.GetContentProtectionSource() > ContentProtectionType::CONTENT_PROTECTION_NONE &&
         m_rCpt.GetContentProtectionSource() < ContentProtectionType::CONTENT_PROTECTION_MAX)
      {
 ...
 */
PeriodInfo *MPD::GetPeriodInfoByKey(uint64 key)
{
  PeriodInfo *periodInfo = NULL;
  int numPeriods = 0;

  PeriodInfo *periodInfoArray = getPeriodInfo(numPeriods);

  if (periodInfoArray)
  {
    for (int i = 0; i < numPeriods; ++i)
    {
      if (periodInfoArray[i].getPeriodKey() == key)
      {
        periodInfo = periodInfoArray + i;
        break;
      }
    }
  }
...
void MPD::setMinimumUpdatePeriod(double minUpdatePeriod)
{
  m_nMinimumUpdatePeriod = minUpdatePeriod;
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "Set minimumUpdatePeriod %lu", (uint32)m_nMinimumUpdatePeriod);
}
...
void MPD::setTimeShiftBufferDepth(double timeShiftBufferDepth)
{
  m_nTimeShiftBufferDepth = timeShiftBufferDepth;
  QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "Set timeShiftBufferDepth %lu", (uint32)m_nTimeShiftBufferDepth);
}
...
void MPDParser::PrintMPD()
{
  ...
  for (int i = 0; i < numPeriods; ++i)
  {
    ...
    QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "PrintMPD period idx 'key' %lu periodStart %lu, periodDuration %lu",
...
void RepresentationInfo::SegmentFuncTemplate::PrintMPDInfoForRepresentation(MPDParser* pMPDParser, PeriodInfo* pPeriodInfo, RepresentationInfo* pRepInfo)
{
  ...
  if(pRepInfo->GetSegmentTemplate())
  {
    ...
    HTTPDownloadStatus status = ((RepresentationInfo::SegmentFuncTemplate*)(pRepInfo->GetSegmentFunc()))->GetAvailabilityBoundsForSegDurationFromTemplate(
      ...
      pMPDParser->GetTimeShiftBufferDepth()*1000);
 ...
 */
MPDParser::MPDParser(HTTPSessionInfo& pSessionInfo,Scheduler& pScheduler, IHttpStatusNotificationHandler* pStackNotificationHandler, uint32 nRequestIDfromResolver,
                       IHttpHandler& HTTPStack)
...
void MPDParser::SetFetchTime()
{
  ...
  MM_Time_GetUTCTime(&m_FetchTime);
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
               "m_FetchTime set as hr:%lu, min %lu, sec %lu",
          ...
          else
          {
            ...
            else
            {
              ...
              pDownloader->SetUpdateAndRefreshTime(
                ...
                1000 * pSelf->mpd->getMinimumUpdatePeriod());
...
HTTPDownloadStatus MPDParser::GetNextPeriod(PeriodInfo *pPeriodInfo, bool& bEOS)
{
  ...
  {
    int periodIndex = periodKey - (period_Info[0].getPeriodKey() >> MPD_PERIOD_SHIFT_COUNT);
...
double MPDParser::GetOffsetFromAvailabilityTimeToStartPlayback(bool bStartOfPlayback)
{
  ...
  double tsbToUse = bStartOfPlayback ? GetTsbToUseAtStartUp() : GetTimeShiftBufferDepth()*1000;
  ...
  PeriodInfo& periodInfo, bool& bEOS, int64 startTime, bool bSearchFromStartOfPlaylist)
{
  ...
  do
  {
    ...
    if ((HTTPCommon::HTTPDL_SUCCESS == status) && (false == bEOS))
    {
      ...
      if (IsLive() && ((uint64)startTime < periodInfo.getStartTime()))
      {
        break;
      }
      bool bIsFound = false;
      ...
      if (((uint64)startTime >= periodInfo.getStartTime()) &&
           ((uint64)startTime < periodInfo.getStartTime() + 1000 * periodInfo.getDuration()))
      {
        bIsFound = true;
      }
      ...
      if (!bIsFound)
      {
        // only the last period can have unknown duration and that too for dynamic mpd only.
        // in case of static mpd, not period should have unknown duration.
        bIsFound = ((uint64)startTime >= periodInfo.getStartTime() && 0.0 == periodInfo.getDuration());
      }
    ...
    if (true == bEOS)
    {
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "No period found for startTime %lu, EOS reached on playlist parser",
                    (uint32)startTime);
      break;
    }
...
/* This function will provide the client with the rep grp’s information.
 * identified by the param key
 * @param[in] - key - rep grp's key
 * @param[out] - pRepGrp - Rep grp's information
 * @return - bool
 * true on successful retrieval false otherwise
 */
bool MPDParser::GetRepresentationGrpByKey(uint64 key,RepresentationGroup& pRepGrp)
{
  bool rslt = false;
  ...
  else
  {
    ...
    if(pPeriodInfo && period_index < (uint32)numPeriods)
    {
      ...
      {
        pRepGrp.Copy(repgrpInfo[grp_index]);
        rslt = true;
...
/* This function gets the first available segment start time for the specified period
 * @param[in] - Rep key for which segment information is queried
 * @param[out] - First available segment start time in msec
 * @return - HTTPDownloadStatus
 * HTTPDL_SUCCESS - success
 * HTTPDL_ERROR_ABORT - otherwise
*/
HTTPDownloadStatus MPDParser::GetSegmentStartTimeForPlayback(const uint64 nPeriodKey, uint64& nStartTime, bool bStartOfPlayback)
{
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_ERROR_ABORT;
  nStartTime = 0;
  ...
  if (pPeriodInfo)
  {
    uint32 nEarliestPeriodIdx =
      (uint32)((uint64)(pPeriodInfo[0].getPeriodKey() & MPD_PERIOD_MASK)>>MPD_PERIOD_SHIFT_COUNT);

    if (nPeriodIdx >= nEarliestPeriodIdx)
    {
      nPeriodIdx -= nEarliestPeriodIdx;
    }

    if (nPeriodIdx < (uint32)nNumPeriods)
    {
      PeriodInfo& rPeriodInfo = pPeriodInfo[nPeriodIdx];
      uint64 nPeriodStartTime = rPeriodInfo.getStartTime();

      //For "OnDemand", all segs become available at availability start time, for "Live"
      //add the delta w.r.t. availability start time to period start (if already past)

      int64 firstAvailableSegmentStartTimeForPeriod =
        GetFirstAvailableSegmentStartTimeForPeriod(rPeriodInfo, bStartOfPlayback);

      nStartTime = nPeriodStartTime +
        (uint64)(STD_MAX(firstAvailableSegmentStartTimeForPeriod,0));

      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                   "GetFirstAvailableSegmentStartTime - period %lu start %lu available start %lu",
                     nPeriodIdx, (uint32)nPeriodStartTime, (uint32)nStartTime );
    ...
    {
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "Invalid period index %lu", nPeriodIdx);
    }
  }
        ...
        for(int j = 0; j < numRepGrps; ++j)
        {
          ...
          if(repInfo && numReps > 0)
          {
...

            {
              ...
              if(bIncludeAll)
              {
                ...
                */
                ...
                if(res)
                {
                  ...
                  std_strlprintf(tmpBuf, sizeof(tmpBuf), "%lu", res->width);
                  {
                    MPDParserXMLPropertiesHelper::CreateXMLForString
                      generateXMLForString(pString,size,"width", tmpBuf);
                  }

                  std_strlprintf(tmpBuf, sizeof(tmpBuf), "%lu", res->height);
                  {
                    MPDParserXMLPropertiesHelper::CreateXMLForString generateXMLForString(
              ...
              {
                MPDParserXMLPropertiesHelper::XMLGeneratorForContentProtection
                  generateXMLGeneratorForContentProtection(pString,size,pRepGrps[j].GetContentProtection());
              }
            ...
            {
              int repKey = (uint32)((uint64)(repInfo[k].getKey() & MPD_REPR_MASK) >> MPD_REPR_SHIFT_COUNT);
              ...
              {
                MPDParserXMLPropertiesHelper::XMLGeneratorForContentProtection
                  generateXMLGeneratorForContentProtection(pString,size,repInfo[k].GetContentProtection());
              }
            ...
            if(nAdaptationSetIdentifier == ((repGrpInfo[j].getKey() & MPD_REPGRP_MASK) >> MPD_REPGRP_SHIFT_COUNT))
            {
              ...
              if(repInfo && numReps > 0)
              {
                ...
                if(bIsSelected)
                {
                  ...
                  {
                    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "ModifySelectedRepresentations: Selected all reps for PeriodXmlId %lu and AdaptationSetXmlId %lu",
...
/**
 * Given a period, find the segment start time which is
 * available for 'nCurrMSeconds'.
 */
HTTPDownloadStatus RepresentationInfo::SegmentFuncDefault::GetFirstAvailableSegmentStartTimeForRepresentation(MPDParser *pMPDParser,
  PeriodInfo* pPeriodInfo,
  int64& minStartTime,
  double nCurrMSeconds,
  bool& bIsCurrentTimeTooEarlyOnAllReps,
  bool& bIsCurrentTimeTooLateOnAllReps,
  ...
  bool bStartOfPlayback)
{
  ...
  double tsbToUse = bStartOfPlayback ? pMPDParser->GetTsbToUseAtStartUp() : pMPDParser->GetTimeShiftBufferDepth()*1000;
  ...
  if (segmentArray && numSegments > 0)
  {
    if(!pMPDParser->isLive())
    {
      ...
      double curSegmentStartTime = segmentArray[0].getStartTime();
      if(MAX_INT64 == minStartTime)
      {
        minStartTime = curSegmentStartTime;
      }
      else
      {
        minStartTime = QTV_MIN(minStartTime, curSegmentStartTime);
      }
    }
    else
    {
      for (uint32 l = 0; l < numSegments; ++l)
      {
        double curSegmentStartTime =
          nAvailabilityMSeconds + pPeriodInfo->getStartTime() + segmentArray[l].getStartTime();

        if (curSegmentStartTime <= nCurrMSeconds)
        {
          bIsCurrentTimeTooEarlyOnAllReps = false;
          if(nCurrMSeconds < curSegmentStartTime + segmentArray[l].getDuration())
          {
            bIsCurrentTimeTooLateOnAllReps = false;
            if (MAX_INT64 == minStartTime)
            {
              minStartTime = (int64)(curSegmentStartTime);
            }
            else if (minStartTime < (int64)(curSegmentStartTime))
            {
              minStartTime = (int64)(curSegmentStartTime);
            }
      ...
      const int64 absolutePeriodStart = nAvailabilityMSeconds + pPeriodInfo->getStartTime();
      minStartTime = (minStartTime > absolutePeriodStart ? minStartTime - absolutePeriodStart : 0);
    }
  }
 ...
 * for period.
 */
int64 MPDParser::GetFirstAvailableSegmentStartTimeForPeriod(PeriodInfo& rPeriodInfo, bool bStartOfPlayback)
{
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_ERROR_ABORT;
  int64 minStartTime = MAX_INT64;
  ...
  if (repGrpArray)
  {
    for (int j = 0; j < numRepGrps; ++j)
    {
      ...
      if (repArray)
      {
        for (int k = 0; k < numReps; ++k)
        {
                    status = repArray[k].GetSegmentFunc()->GetFirstAvailableSegmentStartTimeForRepresentation(this,
                                                                                           &rPeriodInfo,
                                                                                           minStartTime,
                                                                                           nCurrMSeconds,
                                                                                                                                                                                     bIsCurrentTimeTooEarlyOnAllReps,
                                                                                                                                                                                     bIsCurrentTimeTooLateOnAllReps,
                                                                                           &repArray[k],
                                                                                           bStartOfPlayback);
                }
...
/* This function will provide the client with all the segments for the
 * representation identified by param repKey
 * @param[out] - pSegmentInfo - array of segment info for the representation
 * @param[in/out] - numSegments - If the space is not sufficient numSegments is
 *                  populated and sent to client.
 * @param[in] - Rep key for which segment information is queried
 * @return - HTTPDownloadStatus
 * HTTPDL_SUCCESS - success
 ...
 * HTTPDL_ERROR_ABORT - otherwise
*/
HTTPDownloadStatus MPDParser::GetAllSegmentsForRepresentation(SegmentInfo* pSegmentInfo,
                                                   uint32& numSegments,uint64 repKey)
{
  // This is an unused function. If this starts being used then, support
  // needs to be added for the segment template case.
  HTTPDownloadStatus status= HTTPCommon::HTTPDL_ERROR_ABORT;
  QTV_NULL_PTR_CHECK(mpd,status);
  if(m_bAbortSet)
  {
  ...
  else
  {
    ...
    if(period_index < (uint32)numPeriods)
    {
      ...
      if(grp_index < (uint32)numGrps)
      {
        int numRepresentations = 0;
        RepresentationInfo *pReps = pRepGrp[grp_index].getRepInfo(numRepresentations);
        if(rep_index < (uint32)numRepresentations)
        {
          uint32 pNumOfSegments = 0,size=0;
          SegmentInfo* pSegment = pReps[rep_index].getSegmentInfo(pNumOfSegments,size);
          if(numSegments < pNumOfSegments || pSegmentInfo == NULL)
          {
            status = HTTPCommon::HTTPDL_OUT_OF_MEMORY;
          }
          else if(numSegments >= pNumOfSegments)
          {
            for(uint32 i=0;i< pNumOfSegments;i++)
            {
              pSegmentInfo[i].Copy(pSegment[i]);
            }
            status = HTTPCommon::HTTPDL_SUCCESS;
          }
          numSegments = pNumOfSegments;
        }
...
bool MPDParser::IsSegmentAvailable(uint64 nKey)
{
  ...
  if(isLive())
  {
    ...
    if (pPeriodInfo)
    {
      int periodIndex = periodKey - ((pPeriodInfo[0].getPeriodKey()& MPD_PERIOD_MASK) >> MPD_PERIOD_SHIFT_COUNT);

      if((periodIndex >= 0) && (periodIndex < numPeriods))
      {
        uint32 periodStartTime = pPeriodInfo[periodIndex].getStartTime();
        int numRepGrps = 0;
        ...
        if(repgrpInfo && grp_index < (uint32)numRepGrps)
        {
          ...
          if(repInfo && rep_index < (uint32)numReps)
          {
            ...
            if(bOk)
            {
              ...
              double segStartTime=availabilityMSeconds + periodStartTime + segInfo.getStartTime();


              //Check segment start time for availability
              if((segStartTime >= lowerBoundTime) && (segStartTime <= upperboundTime))
              {
                bRet = true;
              }
              else
              {
                QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Segment with starttime %lf is past availability time %lf", segStartTime,upperboundTime);
              }
            }
          }
        }
      ...
      if(bOk)
      {
             ...
             >= pPeriodInfo[periodIndex].getDuration()*1000))
        {
          QTV_MSG_PRIO5(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
            "IsLastSegment true for period %lu (idx %d) duration %lu, segStart %lu, segDuration %lu",
...
HTTPDownloadStatus RepresentationInfo::SegmentFuncDefault::GetLastSegmentKeyForRepresentation(PeriodInfo* pPeriodInfo, uint64* pSegmentInfo, RepresentationInfo* pRepInfo )
{
  QTV_NULL_PTR_CHECK(pPeriodInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
  QTV_NULL_PTR_CHECK(pRepInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
...
HTTPDownloadStatus RepresentationInfo::SegmentFuncTemplate::GetLastSegmentKeyForRepresentation(PeriodInfo* pPeriodInfo, uint64* pSegmentInfo, RepresentationInfo* pRepInfo )
{
  QTV_NULL_PTR_CHECK(pPeriodInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
  QTV_NULL_PTR_CHECK(pRepInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
  ...
  else
  {
    ...
    if(segDuration > 0)
    {
      ...
      int nLatestOffsetStartNumber = -1;

      nLatestOffsetStartNumber = (1000.0 * pPeriodInfo->getDuration()) / segDuration;

      int returnLastSegIdx = nLatestOffsetStartNumber - 1;

      if ((int)(1000.0 * pPeriodInfo->getDuration()) % (int)segDuration != 0)
      {
        // segEndTime does not line up with start of a new segment in template.
        returnLastSegIdx += 1;
      }

      if(pSegmentInfo && (pPeriodInfo->getDuration()))
      {
        uint64 mpdKey = 0;
      ...
      else if(!pPeriodInfo->getDuration())
      {
        status = HTTPCommon::HTTPDL_WAITING;
      }
    }
    ...
    uint32& numSegments,uint64 repKey,uint64 segStartTime,uint64 segEndTime,double &firstAvailableSegmentStartTime)
{
  ...
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
    "GetAllSegmentsForRepresentationRange period_index %lu, segStartTime %lu, segEndTime %lu",
    ...
    if (periodArrayIdx >= 0 && periodArrayIdx < numPeriods)
    {
      ...
      if(pRepGrp && grp_index < (uint32)numGrps)
      {
        ...
        if(pReps && rep_index < (uint32)numRepresentations)
        {
          ...
          RepresentationInfo& rRepInfo = pReps[rep_index];
          status = GetAllSegmentsForRepresentationRangePreProcess(repKey, segStartTime, currMSeconds, rRepInfo.GetTimeShiftBufferDepth());
          if(status == HTTPCommon::HTTPDL_SUCCESS)
          {
    ...
    if (periodArrayIdx >= 0 && periodArrayIdx < numPeriods)
    {
      ...
      if(pRepGrp && grp_index < (uint32)numGrps)
      {
          ...
          if(pReps && rep_index < (uint32)numRepresentations)
          {
            ...
            status = rRepInfo.GetSegmentFunc()->GetLastSegmentKeyForRepresentation(&rPeriodInfo, pSegmentInfo, &rRepInfo);
...
/* This function will provide the client with last available segment key and endtime for the
* representation identified by param repKey from the current system time
* @param[out] - pSegmentInfo - array of segment keys for segments within that range
* @param[in] - repKey - Rep key for which segment information is queried
* @param[out] - lastAvailableSegmentEndTime - End time of the last available segment
* @return - HTTPDownloadStatus
* HTTPDL_SUCCESS - success
* HTTPDL_WAITING - if mpd is not available.
* HTTPDL_INSUFFICIENT_BUFFER - if space is not sufficient
* HTTPDL_ERROR_ABORT - otherwise
*/
HTTPDownloadStatus MPDParser::GetLastAvailableSegmentForRepresentation(uint64  repKey,
                                                                       uint64  &lastAvailableSegmentStartTime,
                                                                       uint64  &lastAvailableSegmentEndTime)
{
  int lastAvailableSegmentIndex = -1;
  HTTPDownloadStatus status= HTTPCommon::HTTPDL_ERROR_ABORT;
  QTV_NULL_PTR_CHECK(mpd, HTTPCommon::HTTPDL_ERROR_ABORT);
  ...
  else
  {
    ...
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
      "GetLastAvailableSegmentForRepresentation period_index %lu rep_index %lu", period_index, rep_index);
    ...
    else
    {
      ...
      if(period_info && numPeriods > 0)
      {
        ...
        if (nMinPeriodKey > nRequestedPeriodKey)
        {
          ...
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
            "HTTPDL_DATA_END as nMinPeriodKey %lu > nRequestedPeriodKey %lu",
              ...
              if(pReps && rep_index < (uint32)numRepresentations)
              {
                ...
                if (period_info[periodArrayIdx].getDuration() > 0.0)
                {
                  if (availabilityMSeconds > 0.0)
                  {
                    ...
                    double absPeriodEndTime = availabilityMSeconds +
                      periodStartTime +
                      1000 * period_info[periodArrayIdx].getDuration();
                    double offsetOfPeriodEndFromCurrentTime =
                      (currMSeconds - pReps[rep_index].GetTimeShiftBufferDepth() >= absPeriodEndTime
                      ? currMSeconds - pReps[rep_index].GetTimeShiftBufferDepth() - absPeriodEndTime
                      : 0.0);
                    ...
                    {
                      QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                        "eos reached on period %lu, peiodDuration %f, past period end by %f msecs",
                ...
                if(!bCheckEos)
                {
                  status =
                    ...
                    lastAvailableSegmentStartTime,lastAvailableSegmentEndTime);
                }
              }//if(pReps && rep_index < numRepresentations)
            }// if(pRepGrp && grp_index < numGrps)
          }
...
HTTPDownloadStatus RepresentationInfo::SegmentFuncDefault::GetLastAvailableSegmentTimeForRepresentation(
   MPDParser *pMPDParser, PeriodInfo* pPeriodInfo, RepresentationInfo* pRepInfo,uint64& latestSegmentStartTime,
   uint64& latestSegmentEndTime)
{
  QTV_NULL_PTR_CHECK(pMPDParser, HTTPCommon::HTTPDL_ERROR_ABORT);
  QTV_NULL_PTR_CHECK(pPeriodInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
  QTV_NULL_PTR_CHECK(pRepInfo, HTTPCommon::HTTPDL_ERROR_ABORT);
  ...
  MM_Time_DateTime sCurrTime;
  MM_Time_GetUTCTime(&sCurrTime);
  double currMSeconds = StreamSourceTimeUtils::ConvertSysTimeToMSec(sCurrTime);
  ...
  if(pSegment)
  {
    ...
    for(uint32 i = 0;i < nNumSegments; i++)
    {
      ...
      if(((pSegment[i].getStartTime() + periodStartTime + availabilityMSeconds) <= currMSeconds) &&
         (currMSeconds < (pSegment[i].getStartTime() + pSegment[i].getDuration() + periodStartTime + availabilityMSeconds)))
      {
    ...
    {
      status = HTTPCommon::HTTPDL_SUCCESS;
      latestSegmentStartTime = pSegment[lastAvailableSegmentIndex].getStartTime();
      latestSegmentEndTime = pSegment[lastAvailableSegmentIndex].getStartTime() +
...
bool MPDParser::GetSegmentInfoByKey(uint64 key,SegmentInfo& pSegmentInfo)
{
  ...
  else
  {
    ...
    if (pPeriodInfo)
    {
      int periodIndex = periodKey - (pPeriodInfo[0].getPeriodKey() >> MPD_PERIOD_SHIFT_COUNT);
              ...
              if(pSegment)
              {
                ...
                if ((segmentIndex >= 0) && ((uint32)segmentIndex < numSegments))
                {
                 ...
                 else
                 {
                   ...
                   "Segment with key %lu %lu %lu %lu not found in seg array - "
                   "segmentIndex %d, numSegs %lu", periodKey, grp_index, rep_index,
                                    ...
                                    bool &bBWPresent)
{
  ...
  uint32 segBasePresentationTimeOffset = 0;
  ...
  if (bOk &&
      ...
      (!rep[RepIndex].IsSegmentAlinged() ||
       !rep[RepIndex].IsValidSAPVal()))
  {
    //ignore this representation
    QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Ignoring representation %d for live profile segmentaligned %d SAP val %d",
                 (int)RepIndex,(int)rep[RepIndex].IsSegmentAlinged(),
                 (int)rep[RepIndex].GetStartWithSAPVal());
    bOk = false;
  }
  if (bOk &&
      (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND) &&
      !rep[RepIndex].IsValidSubSegementSAPVal())
  {
    //ignore this representation
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Ignoring representation %d for ondemand profile subsegment SAP val %d",
                 (int)RepIndex,(int)rep[RepIndex].GetSubSegStartWithSAPVal());
  ...
  if (bOk && !BaseURLElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      ...
      bOk = false;
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring representation %d for ondeamand profile as"
                  "base url element is not present",(int)RepIndex);
    }
  }
  if(bOk)
  {
  ...
  if (bOk && SegmentListElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      //ignore this representation for On-demand profile
       bOk = false;
       QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring representation %d for ondeamand profile as"
                  "segment list is present",(int)RepIndex);
    }
    else
    {
  ...
  if (bOk && SegmentTemplateElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
       ...
       QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring representation %d for ondeamand profile as "
                  "segment template is present",(int)RepIndex);
    }
    else
    {
  ...
  if (bOk && rep[RepIndex].IsSegmentBaseFound())
  {
    ...
    if (rep[RepIndex].GetSegmentBase()->GetInitialisation())
    {
      ...
      rep[RepIndex].SetInitialisationSegmentRange(nStartOffset, nEndOffset);
    }
    ...
    ptsOffset = (segBaseTimeScale > 0 ? ((double)segBasePresentationTimeOffset/segBaseTimeScale)
                 : segBasePresentationTimeOffset);

    rep[RepIndex].SetPTSOffset(ptsOffset);
    indexRangeExact = rep[RepIndex].GetSegmentBase()->GetIndexRangeExact();
  }
  if (bOk && rep[RepIndex].IsSegmentListFound())
  {
    ...
    {
      ptsOffset = (timeScale > 0 ? ((double)presentationTimeOffset/timeScale)
                 :presentationTimeOffset);
    }
    rep[RepIndex].SetPTSOffset(ptsOffset);

    int numSegment = 0;
    int segmentIndex = startNumber;
    SegmentInfo *pSegment=NULL;
    bool bIsSegmentTimelineProvided = rep[RepIndex].GetSegmentList()->IsSegmentTimelineFound();
    double segDuration = rep[RepIndex].GetSegmentList()->GetDuration();
    segDuration = (timeScale > 0 ? (segDuration/timeScale)*1000:segDuration*1000);
    //SegmentUrl Present
    numSegmentURL = rep[RepIndex].GetSegmentList()->GetNumSegmentUrl();
    if (numSegmentURL > 0)
    {
      numSegment = numSegmentURL;
      if (!bIsSegmentTimelineProvided) // if segment time line not provided
      {
        ...
        else
        {
          UpdateNumSegmentsAndSegmentIndex(
            ...
            startNumber, *pMpd, pPeriodInfo[PeriodIndex], rep[RepIndex], segDuration,
            numSegmentURL);
        }
                ...
                if (mediaUrl)
                {
                  ...
                  if (bOk && pMpd && pMpd->IsLive())
                  {
                    pSegment[tempIndex].SetAvailabilityTime(pMpd->GetAvailabilityStartTime()+
                                                            pPeriodInfo[PeriodIndex].getStartTime()+
                                                            segmentStartTime);
                  }
                }
                tempIndex++;
      ...
      else // if segment time line  provided
      {
        ...
        {
          repeatCount = rep[RepIndex].GetSegmentList()->GetSegTimeLineRepeatCount(i);
        ...
        if (pSegment)
        {
          ...
          while(bOk && segIndex < numSegment)
          {
            ...
            if (repeatIndex == 0)
            {
              ...
              {
                replaceSegStartTime = rep[RepIndex].GetSegmentList()->GetSegTimeLineStartTime(tempIndex);
                segmentStartTime = (replaceSegStartTime/timeScale)*1000;
                segmentStartTime -= (ptsOffset*1000);
              }
              replaceSegDuration = rep[RepIndex].GetSegmentList()->GetSegTimeLineDuration(tempIndex);
              segDuration = (replaceSegDuration/timeScale)*1000;
            }
            if ((segmentStartTime + segDuration)>= offsetFromAvailTime  &&
                  (segmentStartTime + segDuration) <= offsetFromAvailTime + relevantPeriodDuration)
            {
              bSegmentAvail = true;
              QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                ...
                "availability window [%d - %d]",
                  ...
                  (int)(offsetFromAvailTime + relevantPeriodDuration));
              if (periodDuration > 0.0 &&
      ...
      rep[RepIndex].IsSegmentTemplateFound())
  {
    ...
    else if (rep[RepIndex].GetSegmentTemplate()->GetInitialisationTemplate())
    {
      ...
      if (initUrlTemp)
      {
        int origUrlLen = std_strlen(initUrlTemp) + 1;
        char *initUrlTemplate = (char *)QTV_Malloc(origUrlLen * 2);
        if (initUrlTemplate)
        {
          std_strlcpy(initUrlTemplate, initUrlTemp, origUrlLen);

        //replace $RepresentationID$ with Representation@id
        if (IsTemplateTagPresent(initUrlTemplate, "$RepresentationID$"))
        {
          if(rep[RepIndex].getRepIdentifier())
          {
             ReplaceIdentifierwithValueInUrl(initUrlTemplate, "$RepresentationID$",
                                            rep[RepIndex].getRepIdentifier(), origUrlLen);
          }
          else
          {
            bOk = false;
        ...
        if (IsTemplateTagPresent(initUrlTemplate, "$Bandwidth$"))
        {
          if (rep[RepIndex].getBandwidth() > 0)
          {
            char bwChar[16];
            std_strlprintf(bwChar, STD_ARRAY_SIZE(bwChar), "%lu", rep[RepIndex].getBandwidth());
             ReplaceIdentifierwithValueInUrl(initUrlTemplate, "$Bandwidth$",
                                            bwChar, origUrlLen);
          }
          else
          {
        ...
        if(IsTemplateTagPresent(initUrlTemplate, "$Bandwidth%"))
        {
          FormatandReplaceIdentifierwithValueInUrl(initUrlTemplate, "$Bandwidth%", rep[RepIndex].getBandwidth(), origUrlLen);
        }
    ...
    double segDuration = rep[RepIndex].GetSegmentTemplate()->GetDuration();
    segDuration *= 1000;
    ...
    segDuration = segDuration/timeScale;
    if (!bIsSegmentTimelineProvided) // if segment time line not provided
    {
    ...
    else // SegmentTimeline Provided
    {
      ...
      {
        repeatCount = rep[RepIndex].GetSegmentTemplate()->GetSegTimeLineRepeatCount(tempIndex);
        totalNumTimelineEntry = totalNumTimelineEntry + repeatCount + 1;
      }
      int numSegment = 0;
      GetNumAvailableSegment(numSegment, *pMpd, pPeriodInfo[PeriodIndex],
      ...
      {
        int segIndex = 0;
        int replaceStartNumber = startNumber;
        uint32 repeatIndex = 0;
        int tempTimelineIndex = 0;
        uint32 tempReplaceSegDuration = 0;
        ...
        while(bOk && tempTimelineIndex < numTimelineEntry && startIndex < numSegment)
        {
          ...
          if (mediaUrlTemp)
          {
            origMediaUrlLen = std_strlen(mediaUrlTemp) + 1;
            mediaTemplate = (char *) QTV_Malloc(origMediaUrlLen * 2);
          }
          if (mediaTemplate)
          {
            std_strlcpy(mediaTemplate, mediaUrlTemp, origMediaUrlLen);
            if (repeatIndex == 0)
            {
              if (rep[RepIndex].GetSegmentTemplate()->GetSegTimeLineStartTime(tempTimelineIndex) > replaceSegStartTime)
              {
                replaceSegStartTime = rep[RepIndex].GetSegmentTemplate()->GetSegTimeLineStartTime(tempTimelineIndex);
                tempReplaceSegDuration = rep[RepIndex].GetSegmentTemplate()->GetSegTimeLineStartTime(tempTimelineIndex) ;
                segmentStartTime = (replaceSegStartTime/timeScale)*1000;
                segmentStartTime -= (ptsOffset*1000);
              }
              replaceSegDuration = rep[RepIndex].GetSegmentTemplate()->GetSegTimeLineDuration(tempTimelineIndex);
              segDuration = (replaceSegDuration/timeScale)*1000;
            }
            ...
            if (bUpdateUsingStartNumber && (uint32)segIndex >= (startNumber - tempStartNumber))
            {
              ...
              {
                tempReplaceSegDuration = tempReplaceSegDuration + (tempStartNumber - 1)*replaceSegDuration;
              }
              else
              {
                tempReplaceSegDuration = tempReplaceSegDuration + tempStartNumber*replaceSegDuration;
              }
            ...
            if (bSegmentAvail)
            {
              if (IsTemplateTagPresent(mediaTemplate, "$RepresentationID$"))
              {
                //replace $RepresentationID$ with Representation@id
                if(rep[RepIndex].getRepIdentifier())
                {
                  ReplaceIdentifierwithValueInUrl(mediaTemplate, "$RepresentationID$",
                                       rep[RepIndex].getRepIdentifier(), origMediaUrlLen);
                }
                else
                {
                  bOk = false;
              ...
              if (bOk && IsTemplateTagPresent(mediaTemplate, "$Bandwidth$"))
              {
                //replace $Bandwidth$ with Representation@bandWidth
                if (rep[RepIndex].getBandwidth() > 0)
                {
                  char bwChar[16];
                  std_strlprintf(bwChar, STD_ARRAY_SIZE(bwChar), "%lu", rep[RepIndex].getBandwidth());
                  ReplaceIdentifierwithValueInUrl(mediaTemplate, "$Bandwidth$",
                                                bwChar, origMediaUrlLen);
                }
                else
                {
                  bOk = false;
              ...
              if(bOk && IsTemplateTagPresent(mediaTemplate, "$Bandwidth%"))
              {
                FormatandReplaceIdentifierwithValueInUrl(mediaTemplate, "$Bandwidth%",
                                                         rep[RepIndex].getBandwidth(), origMediaUrlLen);
              }

              //replace $Time$ with segmenttimeline@t
              char segmentStartTimeChar[32];
              segmentStartTimeChar[0] = '\0';
              std_strlprintf(segmentStartTimeChar,STD_ARRAY_SIZE(segmentStartTimeChar), "%lu", tempReplaceSegDuration);
              ReplaceIdentifierwithValueInUrl(mediaTemplate, "$Time$",
                                              segmentStartTimeChar, origMediaUrlLen);

              //Format & replace $Time% with segmenttimeline@t
              FormatandReplaceIdentifierwithValueInUrl(mediaTemplate, "$Time%",
                                                       tempReplaceSegDuration, origMediaUrlLen);

              //replace $Number$ with segment index
              char segIndexChar[16];
              segIndexChar[0] = '\0';
              std_strlprintf(segIndexChar, STD_ARRAY_SIZE(segIndexChar), "%d", replaceStartNumber);
              ReplaceIdentifierwithValueInUrl(mediaTemplate, "$Number$",
                                              segIndexChar, origMediaUrlLen);

              //Format & replace $Number% with segment index
              FormatandReplaceIdentifierwithValueInUrl(mediaTemplate, "$Number%",
                                                       replaceStartNumber, origMediaUrlLen);

              indexUrlTemp = rep[RepIndex].GetSegmentTemplate()->GetIndexTemplate();
              int origIndexUrlLen = 0;
              if (indexUrlTemp)
              {
                origIndexUrlLen = std_strlen(indexUrlTemp) + 1;
                indexTemplate  = (char *)QTV_Malloc(origIndexUrlLen * 3);
              }
              if (indexTemplate)
              {
                std_strlcpy(indexTemplate, indexUrlTemp, origIndexUrlLen);
                if (IsTemplateTagPresent(indexTemplate, "$RepresentationID$"))
                {
                  //replace $RepresentationID$ with Representation@id
                  if(rep[RepIndex].getRepIdentifier())
                  {
                    ReplaceIdentifierwithValueInUrl(indexTemplate, "$RepresentationID$",
                                        rep[RepIndex].getRepIdentifier(), origIndexUrlLen);
                  }
                  else
                  {
                    bOk = false;
                ...
                if (bOk && IsTemplateTagPresent(indexTemplate, "$Bandwidth$"))
                {
                  //$Bandwidth$
                  if (rep[RepIndex].getBandwidth() > 0)
                  {
                    char bwChar[16];
                    std_strlprintf(bwChar, STD_ARRAY_SIZE(bwChar), "%lu", rep[RepIndex].getBandwidth());
                    ReplaceIdentifierwithValueInUrl(indexTemplate, "$Bandwidth$", bwChar,
                                                  origIndexUrlLen);
                  }
                  else
                  {
                ...
                {
                  //Format & replace $Number% with segment index
                  FormatandReplaceIdentifierwithValueInUrl(indexTemplate, "$Bandwidth%",
                                                           rep[RepIndex].getBandwidth(), origIndexUrlLen);
                }

                //replace $Time$ if present
                char segmentStartTimeChar[MAX_TEMPLATE_ID_LEN];
                segmentStartTimeChar[0] = '\0';
                std_strlprintf(segmentStartTimeChar,STD_ARRAY_SIZE(segmentStartTimeChar), "%lu", tempReplaceSegDuration);
                ReplaceIdentifierwithValueInUrl(indexTemplate, "$Time$", segmentStartTimeChar,
                                                origIndexUrlLen);

                //Format & replace $Number% with segment index
                FormatandReplaceIdentifierwithValueInUrl(indexTemplate, "$Time%",
                                                         tempReplaceSegDuration, origIndexUrlLen);

                //replace $Number$ with segment index
                char segIndexChar[MAX_TEMPLATE_ID_LEN];
                segIndexChar[0] = '\0';
                std_strlprintf(segIndexChar, STD_ARRAY_SIZE(segIndexChar), "%d", replaceStartNumber);
                ReplaceIdentifierwithValueInUrl(indexTemplate, "$Number$", segIndexChar,
                                                origIndexUrlLen);

                //Format & replace $Number% with segment index
                FormatandReplaceIdentifierwithValueInUrl(indexTemplate, "$Number%",
                                                         replaceStartNumber, origIndexUrlLen);
                isIndexURLPresent = true;
              }
              else
              {
            ...
            segmentStartTime = segmentStartTime + segDuration;
            replaceSegStartTime = replaceSegStartTime + replaceSegDuration;
            tempReplaceSegDuration = replaceSegDuration + tempReplaceSegDuration;
           ...
           numUrls > 0 )
  {
    ...
    if (pSegment)
    {
      ...
      if (pPeriodInfo[PeriodIndex].getDuration() > 0.0)
      {
        double segDuration  = (pPeriodInfo[PeriodIndex].getDuration()/segBaseTimeScale)*1000;
                              ...
                              RepresentationGroup repGrp,int& bRepType, bool &bBWPresent)
{
  ...
  if(bOk)
  {
    ...
    if (bSAPPresent)
    {
      sapVal = startWithSAPVal;
    }
    if (bSubSegSAPPresent)
    {
      subSegSAP = subSegmentSAPVal;
    }
    if (profile)
    {
    ...
    if(width > 0 && height > 0)
    {
      ...
      {
        QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Ignoring Representation with width %lu height %lu "
                       "as resolution is more than max supported resolution %lu ",
    ...
    if(bBWPresent)
    {
      ...
      {
        QTV_MSG_SPRINTF_PRIO_2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "Filtering out rep with bw %lu as it is > %lu",
                                     ...
                                     bool& bXlinkHrefFound,bool& bXlinkActuateFound )
{
  ...
  if(bOk)
  {
    ...
    if(width > 0 && height > 0)
    {
      ...
      {
        QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "Ignoring adaptationset with width %lu height %lu "
                      "as resolution is more than max supported resolution %lu ",
                       width, height, m_sessionInfo.GetMaxSupportedRepResolution());
        return false;
      }
                                       ...
                                       uint64 GroupKey)
{
  ...
  if (bOk &&
      (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND) &&
      !repGrps[GroupIndex].IsSubSegmentAlignment())
  {
    //ignore this adaptation set(or group)
    bOk = false;
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring group %d for ondeamand profile as"
                  " subsegment aligned %d",(int)GroupIndex,
                  (int)repGrps[GroupIndex].IsSubSegmentAlignment());
  }
  if (bOk &&
      (m_MpdProfile == DASH_PROFILE_ISO_LIVE) &&
      !repGrps[GroupIndex].IsSegmentAlinged())
  {
    //ignore this adaptation set(or group)
    bOk = false;
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring group %d for live profile as"
                  " segment aligned %d",(int)GroupIndex,
                  (int)repGrps[GroupIndex].IsSegmentAlinged());
  }
  ...
  if(bOk && SegmentListElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      //ignore this Group for On-Demand Profile
      bOk = false;
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring group %d for ondeamnd profile as"
                  " segment list is present",(int)GroupIndex);
    }
    else
    {
  ...
  if (bOk && SegmentTemplateElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      //ignore this Group for On-Demand Profile
       ...
       QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Ignoring group %d for ondeamnd profile as"
                  " segment template is present",(int)GroupIndex);
    }
    else
    {
                            ...
                            bool bIsFirstPeriod)
{
  ...
  if(SegmentListElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      ...
      bOk = false;
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "Ignoring period %d for ondeamnd profile as"
                    "segment list is present at period level",
                    (int)PeriodIndex);
    }
    else
    {
  ...
  if (bOk && SegmentTemplateElement)
  {
    if (m_MpdProfile == DASH_PROFILE_ISO_ON_DEMAND)
    {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "Ignoring period %d for ondeamnd profile as"
                    "segment template is present at period level",
                    (int)PeriodIndex);
    }
    else
    {
...
bool MPDParser::UpdatePeriodInfoFromTail(MPD *pMpd, PeriodInfoCache* pPeriodInfo, int numPeriods)
{
  ...
  if(bOk && pMpd && pPeriodInfo)
  {
    ...
    for(int PeriodIndex = numPeriods - 1 ; PeriodIndex >= 0 && bOk; --PeriodIndex)
    {
      ...
      nextPeriodStartTime = (!isLastPeriod) ?  pPeriodInfo[PeriodIndex  + 1].getStartTime() : 0;

      prevPeriodDuration = (!isFirstPeriod)  ?  pPeriodInfo[PeriodIndex  - 1].getDuration() : 0;
      prevPeriodStartTime = (!isFirstPeriod) ?  pPeriodInfo[PeriodIndex  - 1].getStartTime() : 0;

      currPeriodDuration  = pPeriodInfo[PeriodIndex].getDuration();
      currPeriodStartTime = pPeriodInfo[PeriodIndex].getStartTime();
  ...
  if (bOk)
  {
    ...
    else
    {
      ...
      if (profile)
      {
        ...
        int reqdSize = std_strlen(profile) + 1;
      ...
      if(bOk)
      {
        while(MPDChildElement)
        {
          ...
          if(pMpd && !pMpd->IsLive())
          {
            ...
            currPeriodStartTime = cachedPeriodInfo[cachedPeriodInfoIndex].getStartTime();
      ...
      if ((mpd) && (mpd->IsMpdValid()))
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
                      "MPD refresh check nElapsedTime %lu, minUpdatePeriod %lu",
...
void MPDParser::AdjustKeysForNewMPD(MPD& rNewMPD, MPD& rOldMPD, bool bIsOnTime)
{
  ...
  else
  {
      ...
      numOldPeriods > 0 && numNewPeriods > 0)
    {
      ...
      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
        "Updated period_index from %lu to %lu",
  ...
  if (correspondingOldPeriodIdx >= 0 && oldPeriodInfoArray && newPeriodInfoArray)
  {
    ...
    for (uint64 i = 0; i < (uint64)numNewPeriods; ++i)
    {
      if (0 == i)
      {
        ...
        uint32 prevDuration = newPeriodInfoArray[i].getDuration();
        ...
        if (prevDuration > 0.0)
        {
          ...
          newPeriodInfoArray[i].setDuration(adjustedPeriodDuration);
        }

        QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "AdjustPeriodKeys: Adjusted periodStart from %lu to %lu, duration from %lu to %lu",
  ...
  MPD& rNewMPD, MPD& rOldMPD, int& nTimeOffsetSegment, int correspondingOldPeriodIdx)
{
    ...
    numOldPeriods > 0 && numNewPeriods > 0)
  {
    ...
    for (int i = 0; i < maxNumPeriodsToCmp; ++i)
    {
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "AdjustSegmentKeys: newPeriodIdx %lu",
          ...
          if (newRepArray)
          {
            for (int k = 0; k < numNewReps; ++k)
            {
              ...
              else
              {
                ...
                QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                  "AdjustSegmentKeys: Did not find newRep with ID '%d' in old MPD", (int)newRepID);
              }
            }
          }
        }
    ...
    if (newRepFirstURL)
    {
      ...
      if (correspondingOldSegment)
      {
        ...
        bAreSegmentsCommon = true;
         ...
         // "AdjustSegmentKeys: url '%s' in old rep with key %d starttime %f",
      ...
      else
      {
        ...
        QTV_MSG_SPRINTF_PRIO_2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "AdjustSegmentKeys: Did not find url '%s' in old rep  and newSegmentStartKey is %d ",
      ...
      for (uint32 l = 0; l < numNewSegments; ++l)
      {
        ...
        QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
          "AdjustSegmentKeys: SegmentKey period_idx %lu, idx %lu segkey %lu",
        ...
        if (bFirstNewPeriod)
        {
          ...
          newSegmentArray[l].SetStartTime(prevStartTime + nTimeOffsetSegment);
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
            "AdjustSegmentKeys: Adjusted segstartTime from %lu to %lu",
        ...
        if (oldSegment)
        {
          ...
          {
            QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
              "Sanity check for segment key failed. oldSegmentKey %lu, "
              "newSegmentKey %lu, new segmentIdx %lu", (uint32)oldSegment->getKey(),
...
void RepresentationInfo::SegmentFuncTemplate::AdjustSegmentKeys(RepresentationInfo* pOldRep, RepresentationInfo* pNewRep, uint64 newPeriodKey, int& nTimeOffsetSegment, bool bFirstNewPeriod)
{
    ...
    pNewRep->GetSegmentTemplate())
  {
    ...
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "AdjustSegmentKeys: Restore StartNumber from %lu to %lu",
    ...
    pNewRep->SetPTSOffset(nOldPTSOffset / 1000.0);
  }
}
  ...
  int maxSegments)
{
  ...
  if (rMpd.GetAvailabilityStartTime() > 0.0 && rMpd.IsLive())
  {
    ...
    int incSegments = (int)(offsetFromAvailTime / segDuration);
  ...
  else
  {
    ...
    numSegment = ceil(relevantPeriodDuration / segDuration);
   ...
   RepresentationInfo& rRepInfo, uint32 timeScale, int maxSegments, int &startNumber, bool bSegmentTemplate)
{
  ...
  if (rMpd.GetAvailabilityStartTime() > 0.0 && isLive())
  {
    offsetFromAvailTime = GetOffsetFromAvailabilityTimeForPeriod(rMpd,
      rPeriodInfo, rRepInfo.GetTimeShiftBufferDepth());
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
  ...
  while(tempTimelineIndex < numTimelineEntry)
  {
    if (repeatIndex == 0)
    {
      if (bSegmentTemplate)
      {
        if (rRepInfo.GetSegmentTemplate()->GetSegTimeLineStartTime(tempTimelineIndex) > segmentStartTime)
        {
          segmentStartTime = rRepInfo.GetSegmentTemplate()->GetSegTimeLineStartTime(tempTimelineIndex);
          presentationTimeOffset = rRepInfo.GetSegmentTemplate()->GetPresentationOffset();
          segmentStartTime -= presentationTimeOffset;
          segmentStartTime = (segmentStartTime/timeScale)*1000;
        }
        segDuration = rRepInfo.GetSegmentTemplate()->GetSegTimeLineDuration(tempTimelineIndex);
      }
      else
      {
        if (rRepInfo.GetSegmentList()->GetSegTimeLineStartTime(tempTimelineIndex) > segmentStartTime)
        {
          segmentStartTime = rRepInfo.GetSegmentList()->GetSegTimeLineStartTime(tempTimelineIndex);
          presentationTimeOffset = rRepInfo.GetSegmentList()->GetPresentationOffset();
          segmentStartTime -= presentationTimeOffset;
          segmentStartTime = (segmentStartTime/timeScale)*1000;
        }
        segDuration = rRepInfo.GetSegmentList()->GetSegTimeLineDuration(tempTimelineIndex);
      }
      segDuration = (segDuration/timeScale)*1000;
    }
    ...
    if (bSegmentTemplate)
    {
      ...
      {
        if ((uint32)startNumber > repeatIndex)
        {
...
uint64 repKey, uint64 segStartTime, double currMSeconds, double tsbDepth)
{
  QTV_NULL_PTR_CHECK(mpd, HTTPCommon::HTTPDL_ERROR_ABORT);
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_SUCCESS;
  uint32 period_index = (uint32)((repKey & MPD_PERIOD_MASK)>>MPD_PERIOD_SHIFT_COUNT);
  ...
  double timeShiftBufferDepth = tsbDepth * 1000.0;
  ...
  if(HTTPCommon::HTTPDL_SUCCESS == status)
  {
    ...
    if(currMSeconds < availabilityMSeconds)
    {
      int tmpDiff = (int)((availabilityMSeconds - currMSeconds)/1000.0);
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
        "Mpd is not available yet. currMSeconds %lu < availabilityMSeconds %lu. Ahead by %d seconds",
                     (uint32)currMSeconds, (uint32)availabilityMSeconds, tmpDiff);
      status = HTTPCommon::HTTPDL_WAITING;
    }
  }
  ...
  if(HTTPCommon::HTTPDL_SUCCESS == status)
  {
    ...
    if(period_info && periodArrayIdx >= 0 && periodArrayIdx < numPeriods)
    {
      uint64 periodStartTime = period_info[periodArrayIdx].getStartTime();
      ...
      else
      {
        ...
        if (period_info[periodArrayIdx].getDuration() > 0.0)
        {
      uint64 periodEndTime = periodStartTime + 1000 * period_info[periodArrayIdx].getDuration();
          if (absSegStartTime >= periodEndTime + timeShiftBufferDepth)
          {
            QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                          "eos reached on period %lu", period_index);
      ...
      if(HTTPCommon::HTTPDL_SUCCESS == status)
      {
        if (availabilityMSeconds > 0.0 && period_info[periodArrayIdx].getDuration() > 0)
        {
          // If the current time has gone past the end of the period-end,
          // then set status as end of stream. The segmentinfo array will
          // not be created in this case.
          double absPeriodEndTime = availabilityMSeconds +
            ...
            1000 * period_info[periodArrayIdx].getDuration();
          double offsetOfPeriodEndFromCurrentTime =
            (currMSeconds - timeShiftBufferDepth >= absPeriodEndTime
             ? currMSeconds - timeShiftBufferDepth - absPeriodEndTime
             : 0.0);
                                                    ...
                                                    double timeShiftBufferDepth)
{
  ...
  double availabilityMSeconds = mpd->GetAvailabilityStartTime();
  double availPlusPeriodStart = periodStartTime + availabilityMSeconds;
  ...
  if(pSegment && nNumSegments > 0)
  {
    ...
    for(int i = 0;i < nNumSegments; i++)
    {
      ...
      //find the available segments window for the live case.
      //The calculation takes into account timeShiftBuffer.
      double availSegStartTime = availPlusPeriodStart + pSegment[i].getStartTime();
      double availSegEndTime = availSegStartTime + pSegment[i].getDuration();

      if(availSegStartTime <= upperboundTime && availSegEndTime > lowerBoundTime)
      {
      ...
      if(availSegEndTime > upperboundTime)
      {
        break;
      }
    }//for(int i = 0;i < nNumSegments; i++)
...
uint32 MPDParser::GetTsbToUseAtStartUp() const
{
  ...
  if (m_sessionInfo.IsStartupLatencyImprovementEnabled())
  {
    ...
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "TSB to use at startup = %lu ms", nTsbToUse);
  }
...
double MPDParser::GetOffsetFromAvailabilityTimeForPeriod(const MPD& rMpd, const PeriodInfo& rPeriodInfo, double timeShiftBufferDepth)
{
  ...
  double earliestAvailTime = nAvailabilityMSeconds + periodStartTime;
  double nCurrMSeconds = StreamSourceTimeUtils::ConvertSysTimeToMSec(sCurrTime);
...
{
  char *tmpUrl = (char *)QTV_Malloc(origUrlLen*2);
  if (tmpUrl && targetUrl && idToReplace && replaceString)
  {
    ...
    while (position >= 0)
    {
      ...
      std_strlcat(tmpUrl, replaceString, std_strlen(tmpUrl) + std_strlen(replaceString) + 1);
      std_strlcpy(tmpUrl + std_strlen(tmpUrl), targetUrl + position + std_strlen(idToReplace),
        std_strlen(targetUrl) - position - 1);
      std_strlcpy(targetUrl,tmpUrl,std_strlen(tmpUrl)+1);
      tmpUrl[0] = '\0';
  ...
  while (descTypeElement)
  {
    ...
    while(attrib)
    {
      ...
      if (attrib_name)
      {
        ...
        {
          int reqBufSize = 1 + std_strlen(attrib->Value());
    ...
    if(bOk1)
    {
      if(0 == std_stricmp(adaptationSetDescriptorName, "AudioChannelConfiguration"))
      {
        ...
        if(tmpBuf2)
        {
          ...
          {
            unsigned int maxSupportedAudioSpecificConfig =
            ...
            if(maxSupportedAudioSpecificConfig > 0)
            {
              ...
              {
                QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                             "Filtering out audio channel with ASC %d > %d",
  ...
  while (descTypeElement)
  {
    ...
    if(tmpBuf1)
    {
      ...
      if(!std_stricmp(tmpBuf1, ContentProtectionType::MARLIN_SCHEME_URI))
      {
        ...
        // Marlin contentProtectionType
        TiXmlElement *fmtVersion = descTypeElement->FirstChildElement(
          ContentProtectionType::MARLIN_FORMAT_VERSION);
        if(fmtVersion)
        {
          char *tmpBufMajor = NULL, *tmpBufMinor = NULL;
          TiXmlAttribute* attrib=fmtVersion->FirstAttribute();
          while(attrib)
          {
            char *attrib_name=(char*)skip_whitespace(attrib->Name());
            if(attrib_name)
            {
              if(!std_stricmp(attrib_name,"major"))
              {
                rContentProtection.SetMarlinMajorType(attrib->Value());

              }
              else if(!std_stricmp(attrib_name,"minor"))
              {
                rContentProtection.SetMarlinMinorType(attrib->Value());
              }
            }
        ...
        // Parse Marlin ContentId's
        TiXmlElement *marlinContentIds = descTypeElement->FirstChildElement("mas:MarlinContentIds");

        if(marlinContentIds)
        {
          TiXmlElement *marlinContentId = marlinContentIds->FirstChildElement("mas:MarlinContentId");
          while(marlinContentId)
          {
            const char *text = marlinContentId->GetText();

            if(text)
            {
              rContentProtection.AddMarlinContentId(text);
            }
            else
            {
              QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Null text for contentID");
            }
            marlinContentId = descTypeElement->NextSiblingElement("mas:MarlinContentId");
          }
        }

        // MarlinBroadbandType
        TiXmlElement *marlinBroadbandType = descTypeElement->FirstChildElement("mas:MarlinBroadbandType");
        if(marlinBroadbandType)
        {
          TiXmlElement *silentRightsUrl = marlinBroadbandType->FirstChildElement("mas:SilentRightsUrl");
          while(silentRightsUrl)
          {
            const char *strSilentRightsUrl = silentRightsUrl->GetText();
            if(strSilentRightsUrl)
            {
              rContentProtection.AddSilentRightsUrl(strSilentRightsUrl);
            }
            silentRightsUrl = silentRightsUrl->NextSiblingElement("mas:SilentRightsUrl");
          }

          // PreviewRightsUrl
          TiXmlElement *previewRightsUrl = marlinBroadbandType->FirstChildElement("mas:PreviewRightsUrl");
          while(previewRightsUrl)
          {
            const char *strPreviewRightsUrl = previewRightsUrl->GetText();
            if(strPreviewRightsUrl)
            {
              rContentProtection.AddPreviewRightsUrl(strPreviewRightsUrl);
            }
            previewRightsUrl = previewRightsUrl->NextSiblingElement("mas:PreviewRightsUrl");
          }

          // RightsIssuerUrl
          TiXmlElement *rightsIssuerUrl = marlinBroadbandType->FirstChildElement("mas:RightsIssuerUrl");
          while(rightsIssuerUrl)
          {
            const char *strRightsIssuerUrl = rightsIssuerUrl->GetText();
            if(strRightsIssuerUrl)
            {
              rContentProtection.AddRightsIssuerUrl(strRightsIssuerUrl);
            }
            rightsIssuerUrl = rightsIssuerUrl->NextSiblingElement("mas:RightsIssuerUrl");
          }
        }

        // MS3
        TiXmlElement *ms3 = descTypeElement->FirstChildElement("mas:MS3");
        if(ms3)
        {
          // URIsAreTemplated
          TiXmlElement *urisAreTemplated = ms3->FirstChildElement("mas:URIsAreTemplated");
          while(urisAreTemplated)
          {
            const char *strUrisAreTemplated = urisAreTemplated->GetText();
            if(strUrisAreTemplated)
            {
              rContentProtection.AddUrisAreTemplated(strUrisAreTemplated);
            }
            urisAreTemplated = urisAreTemplated->NextSiblingElement("mas:URIsAreTemplated");
          }
        }
      }
      else
...
bool MPDParser::GetIntValueForKey(TiXmlElement *elem, int& keyValue)
{
  ...
  if(elem)
  {
    ...
    if(attrib)
    {
      ...
      {
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "attrib_name %x or attrib_value %x is null", (unsigned int)attrib_name, (unsigned int)attrib_value);
      }
    }
  }
          ...
          if(repArray && numReps > 0)
          {
            for(int k = 0; k < numReps; ++k)
            {
              if(repArray[k].IsMarkedSelected())
              {
                ...
                uint32 rep_index = (uint32)((key & MPD_REPR_MASK)>>MPD_REPR_SHIFT_COUNT);
                QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "PrintSelectedReps Selected rep %s (%lu,%lu,%lu) ",
 ...
 * available for 'nCurrMSeconds'.
 */
HTTPDownloadStatus RepresentationInfo::SegmentFuncTemplate::GetFirstAvailableSegmentStartTimeForRepresentation(MPDParser *pMPDParser,
  PeriodInfo* pPeriodInfo,
  int64& minStartTime,
  double nCurrMSeconds,
  bool& bIsCurrentTimeTooEarlyOnAllReps,
  ...
  bool bStartOfPlayback)
{
  ...
  {
    QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
      "GetFirstAvailableSegmentStartTimeForPeriod: Unexpected error. Null segmenttemplate");
  ...
  else
  {
    ...
    if(segDuration > 0)
    {
      if(!pMPDParser->IsLive())
      {
        int64 nPeriodDurationMSecs = 1000 * pPeriodInfo->getDuration();
        nEarliestOffsetStartNumber = 0;
        nLatestOffsetStartNumber =
          (nPeriodDurationMSecs % (int64)segDuration == 0
          ? ((nPeriodDurationMSecs / segDuration) - 1)
          : (nPeriodDurationMSecs / segDuration));
        status = HTTPCommon::HTTPDL_SUCCESS;
      }
      else
      {
        ...
        {
          // current time falls withing availability window of atleast one rep.
          bIsCurrentTimeTooEarlyOnAllReps = false;
        }

        if(HTTPCommon::HTTPDL_DATA_END != status)
        {
          bIsCurrentTimeTooLateOnAllReps = false;
        }
      }
      ...
      if(HTTPCommon::HTTPDL_SUCCESS == status)
      {
        int64 tmpMinStartTime = nEarliestOffsetStartNumber * segDuration;
        int64 tmpMaxEndTime = (1 + nLatestOffsetStartNumber) * segDuration;
        minStartTime = QTV_MIN(tmpMinStartTime, minStartTime);
      }
    }
    else
    {
      QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
        "GetFirstAvailableSegmentStartTimeForPeriod segDuration zero");
      status = video::HTTPCommon::HTTPDL_ERROR_ABORT;
    }
  }
  ...
  uint64& latestSegmentEndTime)
{
  ...
  else
  {
    ...
    if(!pMPDParser->isLive())
    {
      ...
      int64 segDurationMSecs = CalculateSegmentDurationFromStoredTemplate(pRepInfo);

      if(segDurationMSecs > 0)
      {
        int numSegments = (0 == periodDurationMs % segDurationMSecs
          ? periodDurationMs / segDurationMSecs
          : 1 + (periodDurationMs / segDurationMSecs));

        latestSegmentEndTime = numSegments * segDurationMSecs;
        status = HTTPCommon::HTTPDL_SUCCESS;
      }
      else
      {
    ...
    else
    {
      ...
      if(segDuration > 0)
      {
        ...
        {
          latestSegmentEndTime = segDuration * (1+nLatestOffsetStartNumber);
          latestSegmentStartTime = latestSegmentEndTime - segDuration;
        }
      }
      else
      {
        QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "GetLastAvailableSegmentEndTimeForRepresentation segDuration zero");
      }
    }
  }
   ...
   MPDParser* pMPDParser, SegmentInfo* pSegmentInfo, RepresentationInfo* pRepInfo, PeriodInfo* pPeriodInfo, uint64 nKey)
{
  ...
  if(segDuration > 0)
  {
    ...
    if(mediaTemplate)
    {
      std_strlcpy(mediaTemplate, mediaUrlTemp , origMediaUrlLen);
      //replace $RepresentationID$ with Representation@id
      if (pMPDParser->IsTemplateTagPresent(mediaTemplate, "$RepresentationID$"))
      {
        if(pRepInfo->getRepIdentifier())
        {
          pMPDParser->ReplaceIdentifierwithValueInUrl(
             mediaTemplate, "$RepresentationID$", pRepInfo->getRepIdentifier(), origMediaUrlLen);
        }
        else
        {
          bOk = false;
      ...
      if (bOk && pMPDParser->IsTemplateTagPresent(mediaTemplate, "$Bandwidth$"))
      {
        //replace $Bandwidth$ with Representation@bandWidth
        if (pRepInfo->getBandwidth() > 0)
        {
          char bwChar[16];
          std_strlprintf(bwChar, STD_ARRAY_SIZE(bwChar), "%lu", pRepInfo->getBandwidth());
          pMPDParser->ReplaceIdentifierwithValueInUrl(mediaTemplate, "$Bandwidth$", bwChar, origMediaUrlLen);
        }
        else
        {
          bOk = false;
      ...
      {
        //Format & replace $Number% with segment index
        pMPDParser->FormatandReplaceIdentifierwithValueInUrl(mediaTemplate, "$Bandwidth%",
                                                             pRepInfo->getBandwidth(), origMediaUrlLen);
      }

      //replace $Number$ segment index
      char segIndexChar[16];
      segIndexChar[0] = '\0';
      std_strlprintf(segIndexChar, STD_ARRAY_SIZE(segIndexChar), "%lu", segmentIndex + startNumber);
      pMPDParser->ReplaceIdentifierwithValueInUrl(mediaTemplate, "$Number$", segIndexChar, origMediaUrlLen);

      //Format & replace $Number% with segment index
      pMPDParser->FormatandReplaceIdentifierwithValueInUrl(mediaTemplate, "$Number%",
                                                           segmentIndex + startNumber, origMediaUrlLen);
      ...
      if (indexTemplate)
      {
        std_strlcpy(indexTemplate, indexUrlTemp, origIndexUrlLen);
        //replace $RepresentationID$ with Representation@id
        if (pMPDParser->IsTemplateTagPresent(indexTemplate, "$RepresentationID$"))
        {
          if(pRepInfo->getRepIdentifier())
          {
            pMPDParser->ReplaceIdentifierwithValueInUrl(indexTemplate, "$RepresentationID$",
                                            pRepInfo->getRepIdentifier(), origIndexUrlLen);
          }
          else
          {
            bOk = false;
        ...
        if (bOk && pMPDParser->IsTemplateTagPresent(indexTemplate, "$Bandwidth$"))
        {
          //replace $Bandwidth$ with Representation@bandwidth
          if (pRepInfo->getBandwidth() > 0)
          {
            char bwChar[16];
            std_strlprintf(bwChar, STD_ARRAY_SIZE(bwChar), "%lu", pRepInfo->getBandwidth());
            pMPDParser->ReplaceIdentifierwithValueInUrl(indexTemplate, "$Bandwidth$", bwChar, origIndexUrlLen);
          }
          else
          {
            bOk = false;
        ...
        {
          //Format & replace $Number% with segment index
          pMPDParser->FormatandReplaceIdentifierwithValueInUrl(indexTemplate, "$Bandwidth%",
                                                               pRepInfo->getBandwidth(), origIndexUrlLen);
        }

        //replace $Number$ with segment index
        char segIndexChar[MAX_TEMPLATE_ID_LEN];
        segIndexChar[0] = '\0';
        std_strlprintf(segIndexChar, STD_ARRAY_SIZE(segIndexChar), "%d", segmentIndex + (int)startNumber);
        pMPDParser->ReplaceIdentifierwithValueInUrl(indexTemplate, "$Number$", segIndexChar, origIndexUrlLen);
        isIndexURLPresent = true;

        //Format & replace $Number% with segment index
        pMPDParser->FormatandReplaceIdentifierwithValueInUrl(indexTemplate, "$Number%",
                                                             segmentIndex + (int)startNumber, origIndexUrlLen);
      }
      else
      {
        indexTemplate = mediaTemplate;
  ...
  if(pRepInfo->GetSegmentTemplate())
  {
    ...
    if(segDuration > 0)
    {
      ...
      if(!pMPDParser->isLive())
      {
        ...
        nLatestOffsetStartNumber = (1000.0 * pPeriodInfo->getDuration() - 1) / segDuration;
      }
      else
      {
        status = GetAvailabilityBoundsForSegDurationFromTemplate(
      ...
      if(HTTPCommon::HTTPDL_SUCCESS == status)
      {
        double earliestSegStartTime = nEarliestOffsetStartNumber * segDuration;
        double latestSegEndTime =  (nLatestOffsetStartNumber + 1) * segDuration;
        ...
        else
        {
          ...
          else
          {
            ...
            {
              segStartTime = earliestSegStartTime;
            }

            if(segEndTime > latestSegEndTime)
            {
              segEndTime = latestSegEndTime;
            }

            // returnFirstSegIdx is correct whether or not segStartTime
            // lines up with the the template start time.
            uint32 returnFirstSegIdx = segStartTime / segDuration;

            uint32 returnLastSegIdx = segEndTime / segDuration;
            ...
            QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
              "DEBUG: Template:offset-startNos %d - %d",
              nEarliestOffsetStartNumber, nLatestOffsetStartNumber);
  ...
  uint64 segStartTime,uint64 segEndTime, double &firstAvailableSegmentStartTime)
{
  ...
  if(pSegment)
  {
    ...
    else
    {
      ...
      if (nNumSegments > 0)
      {
        double firstSegmentStartTime =
          periodStartTime + pSegment[0].getStartTime() + availabilityMSeconds;

        double segStartTimeAbs =
          availabilityMSeconds + periodStartTime + segStartTime;

        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "GetAllSegmentsForRepresentationRange. Check Reuqest to early earliestSegmentStartTime %lu > currMSeconds %lu",
          (uint32)((int64)firstSegmentStartTime % 0x80000000),
          (uint32)((int64)currMSeconds % 0x80000000));

        if (segStartTimeAbs < firstSegmentStartTime)
        {
          ...
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
            "RequestedSegmentStart %lu < firstSegmentStartTime %lu",
            (uint32)((int64)(segStartTimeAbs) % 0x80000000), (uint32)((int64)(firstSegmentStartTime)% 0x80000000));
          status = HTTPCommon::HTTPDL_ERROR_ABORT;
        }
        else
        {
    ...
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
      "%lu segments are available within the given range", numSegments);
  ...
  double tsbToUse)
{
  ...
  double periodStartTime = pPeriodInfo->getStartTime();

  double availabilityMSeconds = mpd->GetAvailabilityStartTime();
  double availPlusPeriodStart = periodStartTime + availabilityMSeconds;

  if(upperboundTime < availPlusPeriodStart)
  {
    // case (ii)
    status = HTTPCommon::HTTPDL_WAITING;
  }
  else
  {
    ...
    if(periodDuration > 0)
    {
      absolutePeriodEndtime = availPlusPeriodStart + 1000.0 * periodDuration;
    }
    else
    {
    ...
    else
    {
      ...
      if(lowerBoundTime < availPlusPeriodStart)
      {
        lowerBoundTime = availPlusPeriodStart;
      }
