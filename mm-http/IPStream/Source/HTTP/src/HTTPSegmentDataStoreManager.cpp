 ...
 *  HTTPDL_ERROR_ABORT on failure
 */
HTTPDownloadStatus HttpDataStoreSegmentManager::Flush(int offset)
{
                                                          ...
                                                          uint64 nKey)
{
  ...
  QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "GetBuffer() result %d, key %d, pBuf %p, bufSize %d",
  ...
  int32& nNumSegments, int64 byteOffset)
{
  ...
  QTV_MSG_PRIO2(QTVDIAG_STREAMING, QTVDIAG_PRIO_HIGH,
                "GetNumAvailableSegments() numAvail %ld for byteOffset %lld",
