...
int HttpSegmentDataStoreContainer::GetSpaceInUse()
{
  ...
  while (pSegment)
  {
    if (iHTTPBase::HEAP == pSegment->GetStorageType())
    {
      ...
      int numLogicalUnits = 1 + (numBytesDownloaded / HTTPHeapManager::GetChunkSize());
  ...
  int heapSpaceInUse = totalLogicalUnitsHeap * HTTPHeapManager::GetChunkSize();
...
void HttpSegmentDataStoreContainer::PrintList(StreamQ_type* q)
{
  ...
  char strSegment[100];
  char logStr[200];
  const int lenStrSegment = sizeof(strSegment);
  const int lenLogStr = sizeof(logStr);
