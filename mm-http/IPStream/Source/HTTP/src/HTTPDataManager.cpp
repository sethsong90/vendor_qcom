...
** ======================================================================= */
                                                              ...
                                                              int numSwapSegments)
  {
      ...
      if (NULL != pFilepath)
      {
        int dstSize = std_strlen(pFilepath) + 1;
        if(m_pFilepath)
        {
  ...
  HTTPDownloadStatus HTTPDataManager::SaveDownloadedContent(const char* pFilePath)
  {
    ...
    if (HTTPCommon::HTTPDL_SUCCESS == result)
    {
      ...
      {
        QTV_Free(m_pFilepath);
        int sourceSize = std_strlen(pFilePath) + 1;
                                                ...
                                                uint64 nKey)
  {
    ...
    if (NULL != m_pHttpDataStore)
    {
      result = m_pHttpDataStore->GetBuffer(pBuffer, (int&)nBufSize, nKey);
    }
   ...
   * @return int
   */
  DataSourceReturnCode HTTPDataManager::Read(byte *pBuf, int nBufSize, int *pnRead)
  {
    DataSourceReturnCode result = DS_FAILURE;
    ...
    {
      result = m_pHttpDataStore->Read(pBuf,nBufSize,pnRead);
    }
  ...
  DataSourceReturnCode HTTPDataManager::Write(const byte *pBuf, int nBufSize, int *pnWritten)
  {
      pBuf = pBuf;
      nBufSize = nBufSize;
      pnWritten = pnWritten;
