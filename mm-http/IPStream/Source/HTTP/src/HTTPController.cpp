...
)
{
  ...
  m_pStackNotificationHandler = reinterpret_cast<IHttpStatusNotificationHandler *>(pHTTPStackStatusHandler);
  ...
  if (m_pStackNotificationHandler && cb)
  {
    if (cb->m_protocolHeaders && m_pDownloader)
    {
      ...
      if (pAuthElem)
      {
        ...
        int bufSize = (cb->m_serverMessage ? 1 + std_strlen(cb->m_serverMessage) : 0);
        if (bufSize > 0)
        {
    ...
    {
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "HTTPDownloader::Notify Notify protocolEvent with code %ld to MMI", cb->m_serverCode);
                                      ...
                                      const char *authValue)
{
  ...
  else
  {
    ...
    {
      int authKeyBufSize = 1 + std_strlen(authKey);
      int authValueBufSize = 1 + std_strlen(authValue);
        ...
        if (pElem)
        {
          ...
          if (pHTTPStack)
          {
            if (HTTP_SUCCESS == pHTTPStack->SetAuthorization(
              0, headerName, std_strlen(headerName), headerValue, std_strlen(headerValue)))
            {
...
void HTTPController::ExecuteSelectRepresentations(const char *pSelectionsXML)
{
  ...
  rslt = m_pDownloader->SelectRepresentations(
...
void HTTPController::EnableQta()
{
  if (m_pDownloader)
  {
    m_pDownloader->EnableQta();
  }
}

bool HTTPController::IsQtaEnabled() const
{
  return m_pDownloader ? m_pDownloader->IsQtaEnabled() : false;
}
 ...
 *
 */
void HTTPController::GetQOEData(uint32 &bandwidth, char* pVideoURL, int& nURLSize, char* pIpAddr, int& nIPAddrSize)
{
  if(m_pDownloader)
  {
