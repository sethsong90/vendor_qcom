...
$Header: //source/qcom/qct/multimedia2/Video/Source/IPStream/Source/HTTP/dev/DASH/src/DASHMediaSegmentHandler.cpp#122 $
...
(
  ...
  IHttpHandler* pHTTPStack,
  HTTPBandwidthEstimator *pSharedBandwidthEstimator
)
{
  bool bOk = true;
  if(!pHTTPStack)
    ...
    if(pSessionInfo)
    {
      ...
      if(m_nSegmentDownloaders > 0)
      {
        ...
        if(m_aDownloader)
        {
          ...
          {
            if (!m_aDownloader[i].Init(pSessionInfo, m_pHTTPStack, pSharedBandwidthEstimator))
            {
              bOk = false;
              break;
            }
    ...
    m_offsetAdjust(0)
{
  ...
  if (bResult)
  {
    bResult = m_cSegDownloadMgr.Init(&m_sDASHSessionInfo.sSessionInfo, pHTTPStack, pBandwidthEstimator);
  }
...
DASHMediaSegmentHandler::~DASHMediaSegmentHandler()
{
  ...
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Seg [0x%08lx%08lx]: Destroyed DASH segment handler (resource) 0x%p",
...
)
{
  ...
  if (m_pSegmentHandler->EnQCmd(cmd))
  {
    ...
    QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                   "Seg [0x%08lx%08lx]: GET_SEGDATA cmd queued successfully cnt %lu key %lu",
...
)
{
  ...
  {
    uint32 nNumTotalDataUnits = m_pSegmentHandler->m_pSidxParser->get_data_chunk_count();
    if ((uint32)nDataUnitKey < nNumTotalDataUnits)
    {
      data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
      if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(nDataUnitKey, &sFragInfo))
      {
    ...
    else
    {
     QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                    "Seg [0x%08lx%08lx]: Invalid data unit key %lu/%lu",
...
)
{
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt,&cmd))
  {
      ...
      && cmd->sGetSegDataCmdData.nDataUnitKey == nDataUnitKey)
    {
      ...
      status = HTTPCommon::HTTPDL_WAITING;
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "Seg [0x%08lx%08lx]: Cancel Data segment (key %lu)"
  ...
  if(status == HTTPCommon::HTTPDL_ERROR_ABORT)
  {
    ...
    QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Seg [0x%08lx%08lx]: CancelSegmentData - GetSegmentData Cmd"
                   "for (key %lu) not found",(uint32)(nSegMPDKey >> 32),
...
)
{
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt,&cmd))
  {
      ...
      && cmd->sGetSegDataCmdData.nDataUnitKey == nDataUnitKey)
    {
      ...
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "Seg [0x%08lx%08lx]: Continue Data segment (key %lu)",
          (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,
          (uint32)nDataUnitKey);
    }
  }
    ...
    if (status == HTTPCommon::HTTPDL_SUCCESS)
    {
      for (int i = 0; i < availiableSegmentsInfo.m_NumSegmentsInUse; i++)
      {
            ...
            nCurrStartOffset >= 0)
        {
          ...
          if (m_pSegmentHandler->m_pSidxParser)
          {
            if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(nCurrKey, &sFragInfo))
            {
            ...
            {
              QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                             "Seg [0x%08lx%08lx]: Could not get frag %llu info, returning...",
...
)
{
  ...
  if (m_pSegmentHandler->m_pSidxParser)
  {
        ...
        m_pSegmentHandler->GetSegmentRange(nSegStart, nSegDuration))
    {
      ...
      int nDataUnitStartIndex = -1, nDataUnitEndIndex = -1;
      ...
      if (status == HTTPCommon::HTTPDL_SUCCESS)
      {
        for (i = 0; i < nNumTotalDataUnits; i++)
        {
          data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
          if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(i, &sFragInfo))
          {
            ...
            else if (i == nNumTotalDataUnits - 1)
            {
      ...
      if (status == HTTPCommon::HTTPDL_SUCCESS)
      {
        for (i = nDataUnitStartIndex; i < nNumTotalDataUnits; i++)
        {
          data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
          if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(i, &sFragInfo))
          {
            ...
            else if (i == nNumTotalDataUnits - 1)
            {
              ...
              nDataUnitEndIndex = nNumTotalDataUnits;
            }
          }
          else
          {
      ...
      if (status == HTTPCommon::HTTPDL_SUCCESS)
      {
        if (nDataUnitEndIndex > nDataUnitStartIndex)
        {
          ...
          if (pDataUnitInfo && nSizeOfDataUnitInfo >= nNumDataUnitInfo)
          {
            QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                           "Seg [0x%08lx%08lx]: Data unit (fragment) info filled - seg idx %lu "
                           "nStartTime %lu nDuration %lu nNumDataUnitInfo %lu",
                           (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,
                           (uint32)m_pSegmentHandler->GetKey(),
                           (uint32)nStartTime, (uint32)nDuration, nNumDataUnitInfo );
            for (int i = nDataUnitStartIndex; i < nDataUnitEndIndex; i++)
            {
              data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
              if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(i, &sFragInfo))
              {
          ...
          else
          {
            ...
            QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                           "Seg [0x%08lx%08lx]: Insufficient buffer for dataunit info %lu/%lu (actual/required)",
        ...
        else
        {
          ...
          QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Seg [0x%08lx%08lx]: Invalid data unit range %d/%d (start/end) for %lu/%lu (start/end)",
...
)
{
  ...
  if (m_pSegmentHandler->m_pSidxParser)
  {
    ...
    if ((uint32)nDataUnitKey < nNumTotalDataUnits)
    {
      ...
      data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
      if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info((uint32)nDataUnitKey, &sFragInfo))
      {
        status = HTTPCommon::HTTPDL_SUCCESS;
        if(nPbTime == 0)
        {
  ...
  {
     QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Seg [0x%08lx%08lx]: Select key/start %lu/%lu",
                       (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey() >> 32),
                       (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey()),
...
void DASHMediaSegmentHandler::SegmentBaseStateHandler::ClearBufferedData(uint64 nStartTime)
{
  ...
  if (pDataManager)
  {
              ...
              if(sFragInfo.n_start_time >= (uint64)nStartTime)
              {
                ...
                else
                {
                  ...
                  QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                                 "Seg [0x%08lx%08lx]: Aborting segment %llu startTime %lu "
                                 "duration %lu flushTime %d",
                                 (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, nCurrKey,
                                 (uint32)starttime, (uint32)duration, (int)nStartTime);
                }

                nLastRemovedSegKey = nSegMPDKey & 0x000000000FFFFFFF;
    ...
    {
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Seg [0x%08lx%08lx]: Could not get data segment info %d",
...
)
{
  ...
  if (m_pSegmentHandler->m_pSidxParser)
  {
    ...
    {
      data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
      if (m_pSegmentHandler->m_pSidxParser && m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(nDataUnitIndex, &sFragInfo))
      {
...
)
{
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt, cmd))
  {
    ...
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Seg [0x%08lx%08lx]: OPEN cmd already queued",
                     (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey() >> 32),
                     (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey()) );
      status = HTTPCommon::HTTPDL_WAITING;
      return status;
  ...
  if (m_pSegmentHandler->EnQCmd(cmd))
  {
    ...
    QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                   "Seg [0x%08lx%08lx]: OPEN cmd queued successfully cnt %lu",
                   (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey() >> 32),
                   (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey()),
...
bool DASHMediaSegmentHandler::SegmentBaseStateHandler::GetSegmentRange(uint64& nStartTime, uint64& nDuration)
{
  ...
  if (m_pSegmentHandler->m_pSidxParser)
  {
    ...
    if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(0, &sFragInfo))
    {
      ...
      uint32 nNumTotalDataUnits = m_pSegmentHandler->m_pSidxParser->get_data_chunk_count();
      if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(nNumTotalDataUnits - 1, &sFragInfo))
      {
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt, cmd))
  {
    if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_GET_SEGINFO)
    {
      ...
      if (pDownloader && pDataManager)
      {
        ...
        int nSegStartOffset = (m_pSegmentHandler->m_pInitSegment ?
        ...
        if (status == HTTPCommon::HTTPDL_SUCCESS)
        {
          //Create sidx parser, start downloader and move to SETUP state for sidx parsing
          if (m_pSegmentHandler->m_pSidxParser)
          {
        ...
        {
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Seg [0x%08lx%08lx]: Data manager CreateSegment() failed %d for init segment",
                         (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, status );
          status = HTTPCommon::HTTPDL_ERROR_ABORT;
        }
      }
...
)
{
  ...
  if (pDownloader)
  {
    ...
    if (m_pSegmentHandler->EnQCmd(cmd))
    {
      status = HTTPCommon::HTTPDL_WAITING;
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "Seg [0x%08lx%08lx]: GET_SEGINFO cmd queued successfully cnt %lu",
                     (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey() >> 32),
                     (uint32)(m_pSegmentHandler->m_cSegmentInfo.getKey()),
                     m_pSegmentHandler->m_cCmdQ.Count() );
    }
    ...
    switch(m_pSegmentHandler->m_pSidxParser->parse_sidx())
    {
      ...
      case SIDX_PARSER_OUT_OF_MEMORY:
        ...
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Seg [0x%08lx%08lx]: Sidx parsing error. Continue with download entire segment",
                       (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
        status = HTTPCommon::HTTPDL_SUCCESS;
        break;
      ...
      case SIDX_PARSER_SIDX_NOT_AVAILABLE:
        ...
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Seg [0x%08lx%08lx]: No sidx atom present in segment. Download entire segment",
                       (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
        status = HTTPCommon::HTTPDL_SUCCESS;
        break;
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt, &cmd))
  {
    if (cmd->sBaseCmdData.eCmd == SEGMENT_CMD_GET_SEGINFO)
    {
              ...
              (pDownloader && pDownloader->GetNumBytesReceived() == 0))
      {
        ...
        if( (SegAvailabilityTimeMsec + SegDuration) > CurrMsec )
        {
        ...
        else
        {
          QTV_MSG_PRIO5(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                             "Seg [0x%08lx%08lx]:not Retrying, as SegmentAbilityTime(%lu + dutation(%lu)) is before the currentTime(%lu)",
            ...
            else
            {
              ...
              if(HTTPSUCCEEDED(status))
              {
                ...
                if(m_pSegmentHandler->m_cSegmentInfo.IsIndexURLPresent() && pDataManager)
                {
                  ...
                  data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
                  if (m_pSegmentHandler->m_pSidxParser &&
                      m_pSegmentHandler->m_pSidxParser->get_data_chunk_info((uint32)0, &sFragInfo))
                  {
                ...
                QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "Seg [0x%08lx%08lx]: Init segment downloaded and sidx parsing complete",
                  (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
              }
            }
          }
      ...
      if (status != HTTPCommon::HTTPDL_WAITING)
      {
        //Stop the downloader
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Seg [0x%08lx%08lx]: Exiting ParseSidx %d, stopping downloader",
                       (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, status );
        ...
        if (HTTPSUCCEEDED(status))
        {
          ...
          if (m_pSegmentHandler->m_pRepNotifier)
          {
            ...
            QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                           "Seg [0x%08lx%08lx]: GET_SEGINFO cmd (nStartTime %lu nDuration %lu nNumDataUnits %lu) "
...
bool DASHMediaSegmentHandler::ConstructByteRangeURL(char* origUrl, char* templateStr, int64 nStartOffset, int64 nEndOffset)
{
  ...
  {
    int origUrlLength = std_strlen(origUrl);
    int templateStrLength = std_strlen(templateStr);
...
)
{
  ...
  if ( pDownloader )
  {
          ...
          result != HTTPCommon::HTTPDL_ERROR_ABORT)
    {
      ...
      {
        QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Seg [0x%08lx%08lx] Discarded %d bytes after Sidx, (requested/downloaded) %lld/%llu bytes",
                       (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt, cmd))
  {
    if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_GET_SEGINFO)
    {
      ...
      HTTPDownloadStatus mpdStatus =
        m_pSegmentHandler->m_sDASHSessionInfo.cMPDParser.GetLastAvailableSegmentForRepresentation(
           ...
           mpdStatus == HTTPCommon::HTTPDL_DATA_END)
      {
        ...
        double SegAvailabilityTimeMsec = m_pSegmentHandler->m_cSegmentInfo.getAvailabilitytime();
        uint32 SegDuration = (uint32)m_pSegmentHandler->m_cSegmentInfo.getDuration();
        uint32 nMaxRetryDuration = SegDuration * SEGMENT_DURATION_FRACTION_FOR_RETRY_DURATION;
        uint32 SleepTime = SegDuration * SEGMENT_DURATION_FRACTION_FOR_RETRY_INTERVAL;


        if ( ((SegAvailabilityTimeMsec + nMaxRetryDuration) > currMSeconds)  && nElapsedTimeSinceLastFailure >= SleepTime) //&& nElapsedTime <  nMaxRetryDuration)
        {
          (void)pDownloader->Stop();
          if(m_pSegmentHandler->SendSegmentInfoRequest(pDownloader))
          {
            ++m_pSegmentHandler->m_numRetriesExcerised;
            QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,"Seg [0x%08lx%08lx]:SegmentSegmentInfoRetryHandler: Retrying(%d)..currTime(%lu)/MaxRetryWindow(%lu)",
                           (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey , m_pSegmentHandler->m_numRetriesExcerised, (uint32)currMSeconds,
                           (uint32)(SegAvailabilityTimeMsec + nMaxRetryDuration));
            m_pSegmentHandler->SetStateHandler(&m_pSegmentHandler->m_cParseSidxStateHandler);
          }
        }
        else if((SegAvailabilityTimeMsec + nMaxRetryDuration) <= currMSeconds)
        {
          QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Seg [0x%08lx%08lx]:SegmentSegmentInfoRetryHandler:Beyond retry window SegAvaibilityTime(%lu), CurrMsec (%lu),MaxRetryTime"
                          "(%lu),Retries attempted(%d)",
                          (uint32)(nSegMPDKey >> 32),(uint32)nSegMPDKey,(uint32)SegAvailabilityTimeMsec,
                         (uint32)currMSeconds,(uint32)(SegAvailabilityTimeMsec + nMaxRetryDuration), m_pSegmentHandler->m_numRetriesExcerised);
          eStatus = HTTPCommon::HTTPDL_ERROR_ABORT;
        }
      }
      else if ( lastAvailableSegmentStartTime >= m_pSegmentHandler->m_cSegmentInfo.getStartTime())
      {
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,"Seg [0x%08lx%08lx]:SegmentSegmentInfoRetryHandler: Stopping Retries as Next Segment with time(%llu) is available",
      ...
      {
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,"Seg [0x%08lx%08lx]:SegmentSegmentInfoRetryHandler:GetLastAvailableSegment Failed status:%d",
    ...
    if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_OPEN)
    {
      if(m_pSegmentHandler->m_bProcessOpenCmd)
      {
        ...
        if (status != HTTPCommon::HTTPDL_WAITING)
        {
          ...
          {
            QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
              "Seg [0x%08lx%08lx]: Opening file source and moving to OPENING",
              (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
            (void)m_pSegmentHandler->m_cCmdQ.Remove(pIt);
          }
          else
...
)
{
  ...
  if (status == FILE_SOURCE_OPEN_COMPLETE)
  {
    if (m_pSegmentHandler->SetupTracks() == HTTPCommon::HTTPDL_SUCCESS)
    {
      bError = false;
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "Seg [0x%08lx%08lx]: File source reported OPEN_COMPLETE and track setup successful, "
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Seg [0x%08lx%08lx]: File source reported error, notify representation",
                   (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
...
HTTPDownloadStatus DASHMediaSegmentHandler::SegmentOpenStateHandler::Seek(const int64 nSeekTime)
{
  ...
  FileSource* pFileSource = m_pSegmentHandler->GetFileSource();
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Seg [0x%08lx%08lx]: Seek %d issued on File source"
                  " moving to seeking state",(uint32)(nSegMPDKey >> 32),
                  (uint32)nSegMPDKey,(int)m_pSegmentHandler->m_nSeekTime );
...
)
{
  ...
  if (status == FILE_SOURCE_SEEK_COMPLETE)
  {
    bError = false;
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                   "Seg [0x%08lx%08lx]: File source reported SEEK_COMPLETE"
                   "moving back to OPEN", (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
    m_pSegmentHandler->m_nSeekTime = -1;

    if(pFileSource)
    {
      nSeekTime =  pFileSource->GetMediaCurrentPosition();
    }
  ...
  else if(status == FILE_SOURCE_SEEK_UNDERRUN || status == FILE_SOURCE_SEEK_UNDERRUN_IN_FRAGMENT)
  {
    if(pFileSource)
    {
      ...
      uint64 mediaPosition = pFileSource->GetMediaCurrentPosition();
      ...
      {
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Data Segment  [0x%08lx%08lx] seek underrun "
  ...
  if (bError)
  {
    ...
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Seg [0x%08lx%08lx]: File source reported error, remove the segment",
                   (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
    m_pSegmentHandler->OnError(HTTPCommon::HTTPDL_ERROR_ABORT);
  }
  ...
  if(bNotifySeek)
  {
    if(m_pSegmentHandler->m_pRepNotifier)
    {
      m_pSegmentHandler->m_pRepNotifier->NotifySeekStatus(m_pSegmentHandler->GetKey(),nSeekTime,eStatus);
    }
  }
}
  ...
  while(m_pSegmentHandler->m_cCmdQ.Next(pIt, cmd))
  {
    ...
    else if (cmd.sCheckCmdData.eCmd == SEGMENT_CMD_CHECK_DATA_AVAIL_FOR_SEEK)
    {
          ...
          ((uint64)nAvailableOffset - cmd.sCheckCmdData.nCurrDataOffset) >= SEEK_POLL_DATA))
      {
          ...
          FILE_SOURCE_FAILED(pFileSource->SeekAbsolutePosition(m_pSegmentHandler->m_nSeekTime)))
        {
          ...
          if(m_pSegmentHandler->m_pRepNotifier)
          {
            m_pSegmentHandler->m_pRepNotifier->NotifySeekStatus(nSegKey,
    ...
    else if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_PROCESS_SEEK_FAILURE)
    {
          ...
          for (uint32 i = 0; i < nNumTotalDataUnits; i++)
          {
            ...
            if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(i, &sFragInfo))
            {
              if (sFragInfo.n_start_time > (unsigned long long)nSeekTime)
              {
                ...
                QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                               "Seg [0x%08lx%08lx]: Seek failed in current fragment, moving over to"
                               " fragment %lu, new seek time %d msec",
                               (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, i, (int)nSeekTime );
                break;
              }
            }
          }// for ()
      ...
      if (bError)
      {
        ...
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Seg [0x%08lx%08lx]: Seek error, remove the segment and move over",
                       (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey );
        m_pSegmentHandler->OnError(HTTPCommon::HTTPDL_ERROR_ABORT);
      }
      ...
      if (m_pSegmentHandler->m_pRepNotifier)
      {
        m_pSegmentHandler->m_pRepNotifier->NotifySeekStatus(m_pSegmentHandler->GetKey(), nSeekTime, status);
      }
    }
...
)
{
  ...
  if (pDownloader && pDataManager)
  {
    ...
    while (status == HTTPCommon::HTTPDL_SUCCESS)
    {
      ...
      status = pDataManager->GetBuffer(pBuffer, (int32 &)nBytesAvailable, nKey);
      if (HTTPSUCCEEDED(status))
      {
        int64 nBytesRead = 0;
        ...
        if (result == HTTPCommon::HTTPDL_SUCCESS || result == HTTPCommon::HTTPDL_DATA_END)
        {
          ...
          if (nSegContentLength < 0 && (nKey != HTTP_INIT_SEG_KEY && nKey != HTTP_NONSELF_INIT_SEG_KEY))
          {
            if (m_pSidxParser)
            {
              nSegContentLength = 0;
              data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
              uint32 nNumTotalDataUnits = m_pSidxParser->get_data_chunk_count();
              for (uint32 i = 0; i < nNumTotalDataUnits; i++)
              {
                if (m_pSidxParser->get_data_chunk_info(i, &sFragInfo))
                {
          ...
          QTV_MSG_PRIO6( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Seg [0x%08lx%08lx]: Data segment (key %lu) download progress %llu/%llu/%lld bytes",
          ...
          if (HTTPSUCCEEDED(status))
          {
            ...
            if (result == HTTPCommon::HTTPDL_DATA_END || (nBytesReceived == (uint32)nContentLength))
            {
              status = HTTPCommon::HTTPDL_DATA_END;
              QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                             "Seg [0x%08lx%08lx]: Data segment (key %lu) download complete",
                             (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, (uint32)nKey );
            }
          ...
          {
            QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Seg [0x%08lx%08lx]: Data segment (key %lu) download buffer commit failed %d",
        ...
        else if (result == HTTPCommon::HTTPDL_WAITING)
        {
          status = HTTPCommon::HTTPDL_WAITING;
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                         "Seg [0x%08lx%08lx]: Data segment (key %lu) waiting for data",
                         (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, (uint32)nKey );
        }
        else if (result == HTTPCommon::HTTPDL_SEGMENT_NOT_FOUND)
        {
        ...
        else
        {
          status = HTTPCommon::HTTPDL_ERROR_ABORT;
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Seg [0x%08lx%08lx]: Data segment (key %lu) read failed %d",
      ...
      else if (status == HTTPCommon::HTTPDL_OUT_OF_MEMORY)
      {
        ...
        status = HTTPCommon::HTTPDL_WAITING;
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Seg [0x%08lx%08lx]: Data segment (key %lu) download buffer unavailable, "
        ...
        HTTPCommon::HTTPDL_WAITING == status)
    {
      ...
      if (bIsTooSlow)
      {
        if (IsDataUnit(nKey))
        {
          ...
          m_pRepNotifier->NotifyDownloadTooSlow(nSegMPDKey & 0x000000000FFFFFFF, nKey);
        }
      }
    }
  }// if (pDownloader && pDataManager)
...
)
{
  ...
  {
    status = ProcessGetSegmentDataInit(cmd.sGetSegDataCmdData.nDataUnitKey);
  }
  else if(cmd.sGetSegDataCmdData.eState == CMD_STATE_DOWNLOADING)
  {
    status = ProcessGetSegmentDataDownload(cmd.sGetSegDataCmdData.nDataUnitKey,
                                           cmd.sGetSegDataCmdData.nCmdId);
  }
...
)
{
  ...
  if (eCurrDownloadState != DATA_DOWNLOAD_ERROR)
  {
    ...
    if (pDownloader && pDataManager && bSegAvailable)
    {
      ...
      if(m_pSidxParser)
      {
        ...
        data_chunk_info sFragInfo = {0,0,0,0,0,0,0,0};
        if (m_pSidxParser &&
            m_pSidxParser->get_data_chunk_info((uint32)nDataUnitKey, &sFragInfo))
        {
      ...
      if (nStartOffset >= 0)
      {
        ...
        if (status == HTTPCommon::HTTPDL_EXISTS)
        {
          ...
          status = HTTPCommon::HTTPDL_WAITING;
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Seg [0x%08lx%08lx]: Data segment (key %lu) already exists",
        ...
        else if (status == HTTPCommon::HTTPDL_SUCCESS)
        {
                            ...
                            nDurationMs))
          {
            ...
            while(m_cCmdQ.Next(pIt,&cmd))
            {
                 ...
                 && cmd->sGetSegDataCmdData.nDataUnitKey == nDataUnitKey)
              {
                ...
                status = HTTPCommon::HTTPDL_WAITING;
                QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                               "Seg [0x%08lx%08lx]: ProcessGetSegmentDataInit GetSegmentData"
                               "(key %lu) state modified to downloading",
                               (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, (uint32)nDataUnitKey);
                break;
              }
            }
          ...
          else
          {
            ...
            status = HTTPCommon::HTTPDL_ERROR_ABORT;
            QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                           "Seg [0x%08lx%08lx]: Download (key %lu) could not be started",
                           (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, (uint32)nDataUnitKey );
          }
        }
        else
        {
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Seg [0x%08lx%08lx]: Data manager CreateSegment() failed %d (key %lu)",
    ...
    {
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Seg [0x%08lx%08lx]: Segment %ld is not available now on server",
                     (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,(uint32)nDataUnitKey );
  ...
  if (status != HTTPCommon::HTTPDL_WAITING)
  {
    ...
    if (!bError)
    {
      ...
      {
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Seg [0x%08lx%08lx]: GET_SEGDATA cmd (key %lu) processing complete %d in CMD_STATE_INIT - "
...
(
   uint64 nDataUnitKey, uint32 nCmdID
)
{
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_ERROR_ABORT;
  uint64 nSegMPDKey = m_cSegmentInfo.getKey();
    ...
    if (eCurrDownloadState != DATA_DOWNLOAD_ERROR)
    {
      ...
      if (status != HTTPCommon::HTTPDL_WAITING)
      {
        ...
        if (bLast || eNewDownloadState == DATA_DOWNLOAD_ERROR)
        {
          ...
          QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Seg [0x%08lx%08lx]: Data unit (key %lu) download state %d/%d",
                         (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, (uint32)nDataUnitKey,
  ...
  if (status != HTTPCommon::HTTPDL_WAITING)
  {
    ...
    if (!bError)
    {
      ...
      {
        QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Seg [0x%08lx%08lx]: GET_SEGDATA cmd (key %lu) processing complete %d in CMD_STATE_DOWNLOADING - "
                       "notifying representation", (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,
                       (uint32)nDataUnitKey, status );
        m_pRepNotifier->SegDataReady((uint32)GetKey(), nDataUnitKey, status);
      }
...
)
{
 ...
 //GET_SEGDATA cmd will take the state forward!)
  ...
  QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Seg [0x%08lx%08lx]: GET_SEGDATA cmd (key %lu) processing"
...
bool DASHMediaSegmentHandler::SegmentBaseStateHandler::IsDownloadComplete()
{
  ...
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                 "Seg [0x%08lx%08lx]: Download status on data unit %d",
          ...
          if (m_pSegmentHandler->m_pSidxParser)
          {
            ...
            if (m_pSegmentHandler->m_pSidxParser->get_data_chunk_info(nCurrKey, &sFragInfo))
            {
              ...
              else
              {
                ...
                QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                               "Seg [0x%08lx%08lx]: Download incomplete on data unit %d, breaking",
                               (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey, nCurrKey );
                bQueryFS = true;
...
void DASHMediaSegmentHandler::ClearCmdQ(HTTPDownloadStatus eCmdIdStatus, uint32 nCmdId)
{
  ...
  if( m_nSeekTime > 0 )
  {
    m_pRepNotifier->NotifySeekStatus(GetKey(), -1, HTTPCommon::HTTPDL_ERROR_ABORT);
  }
  m_nSeekTime = -1;
    ...
    if (pSegCmdRespArray)
    {
      while(m_cCmdQ.Next(pIt, cmd))
      {
        ...
        if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_GET_SEGINFO)
        {
          ...
          nResp++;
          QTV_MSG_PRIO4( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Seg [0x%08lx%08lx]: GET_SEGINFO cmd (nStartTime %lu nDuration %lu) "
        ...
        else if (cmd.sBaseCmdData.eCmd == SEGMENT_CMD_GET_SEGDATA)
        {
          ...
          nResp++;
          QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Seg [0x%08lx%08lx]: GET_SEGDATA cmd (key %lu) cleared - "
                         "notifying representation", (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey,
...
)
{
  ...
  if(m_pSidxParser)
  {
    ...
    (void)m_pSidxParser->get_data_chunk_info(nDataUnitKey,&sFragInfo);
    nFragStartTime = sFragInfo.n_start_time;
  }
  else
  {
 ...
 */
bool DASHMediaSegmentHandler::GetStartTimeForLastDataUnit(uint64& nStartTime, double repEndTime)
{
...
HTTPDownloadStatus DASHMediaSegmentHandler::SegmentBaseStateHandler::Close(HTTPMediaType majorType)
{
  ...
  uint64 nSegMPDKey = m_pSegmentHandler->m_cSegmentInfo.getKey();
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Seg [0x%08lx%08lx]: Close for majorType %d - ",
  ...
  if(m_pSegmentHandler->CloseComplete())
  {
    ...
    while(m_pSegmentHandler->m_cCmdQ.Next(pIt, cmd))
    {
      ...
      {
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                       "Seg [0x%08lx%08lx]: dequeing check data avail for seek cmd",
                        (uint32)(nSegMPDKey >> 32), (uint32)nSegMPDKey);
...
)
{
  ...
  uint64 nSegMPDKey = m_cSegmentInfo.getKey();
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Seg [0x%08lx%08lx]: Moving to ERROR %d and notifying representation",
...
bool DASHMediaSegmentHandler::SendSegmentInfoRequest(SegmentDownloader* pDownloader)
{
  ...
  if (pDownloader)
  {
    ...
    if (m_cSegmentInfo.GetIndexUrlRange())
    {
      ...
      // If IndexRangeExact is True, initialization segment could be present before sidx,
      // hence download init segment as a part of sidx download only
      if(m_cSegmentInfo.IsIndexRangeExact())
      {
        nStartOffset = 0;
      }
      // If init range is present and init url is missing, consider this as self init
      // segment and download sidx from the start of initialization segment
      else if ((m_cSegmentInfo.GetInitUrlRange()) && (!m_cSegmentInfo.IsInitURLPresent()))
      {
        int64 nInitStartOffset = 0;
        int64 nInitEndOffset = 0;
        char *initRange = m_cSegmentInfo.GetInitUrlRange();
        parseByteRange(initRange, nInitStartOffset, nInitEndOffset);
        if(nInitEndOffset <= nStartOffset)
        {
          nStartOffset = nInitStartOffset;
        }
      }
    }
