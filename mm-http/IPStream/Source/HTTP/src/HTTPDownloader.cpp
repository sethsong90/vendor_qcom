...
)
{
  ...
  m_pStackNotificationHandler =reinterpret_cast<IHttpStatusNotificationHandler *>(pStatusHandler);
  m_bPaused = false;
  ...
  if (bOk)
  {
    ...
    result = HttpHandlerFactory::CreateInstance(
       &m_pHTTPStack, (m_pStackNotificationHandler),
       m_sessionInfo.GetCookieStore(), NULL,
       HttpHandlerFactory::HTTP_STACK_DEFAULT);
 ...
 )
{
  ...
  m_sessionInfo.SetUserAgent("QualcommHTTPClient");
 ...
 )
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "Error: ResolveHTTPFlavor task timed out=%lu ms", callbackData.nTimeout);
 ...
 )
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
          "DownloadData task Paused - reset timer = %lu", parameter->nStartTime);
        parameter->nStartTime = pSourceClock->GetTickCount();
      }
 ...
 )
{
  ...
  else
  {
    ...
    else
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
          "TaskSeek - Paused state - reset timer = %lu", parameter->nStartTime);
        parameter->nStartTime = pSourceClock->GetTickCount();
      }
...
)
{
  ...
  if(proxyServer)
  {
    int32 proxyServerStrlen = std_strlen(proxyServer);
 ...
 * @return True if success, false otherwise
 */
bool HTTPDownloader::SelectRepresentations
(
 ...
 )
{
  ...
  QTV_NULL_PTR_CHECK(pTaskParam, HTTPCommon::HTTPDL_OUT_OF_MEMORY);
  ...
  int reqdBufSize = 1 + std_strlen(SetSelectionsXML);
 ...
 *
 */
void HTTPDownloader::GetQOEData(uint32 &bandwidth, char *pVideoURL, int& nURLSize, char *pIpAddr, int& nIPAddrSize)
{
  if(m_pDownloadHelper)
  {
