...
HTTPDownloadStatus HTTPResource::SetupTracks()
{
  ...
  int nNumTracks;
  int nNumSelectedTracks = 0;
  QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Setting up tracks for resource [%lu %lu] filesource %p",
  ...
  if ( pTrackList )
  {
    ...
    for (int index = 0; index < nNumTracks; index++)
    {
...
HTTPDownloadStatus HTTPResource::SetAudioTrackProperties()
{
  ...
  if ( pFileSource && GetSelectedMediaTrackInfo(HTTPCommon::HTTP_AUDIO_TYPE, tInfo))
  {
       ...
       HTTP_MINOR_TYPE_AAC_LOAS == tInfo.minorType)
    {
        ...
        if(HTTP_AAC_FORMAT_ADTS == (HTTPAACStreamFormat)aacCodecData.eAACStreamFormat)
        {
          ...
          if(configStatus != FILE_SOURCE_SUCCESS)
          {
            ...
            QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                      "SetConfiguration  STRIP_AUDIO_HEADER failed for track %lu", tInfo.nTrackID );
          }
          else
          {
            ...
            if(configStatus != FILE_SOURCE_SUCCESS)
            {
              status = HTTPCommon::HTTPDL_ERROR_ABORT;
              QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                        "SetConfiguration  OUTPUT_SINGLE_AUDIO_FRAME failed for track %lu", tInfo.nTrackID );
            }
          }
        }
      }
      ...
      {
           QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                          "Get AAC code info failed for track %lu", tInfo.nTrackID );
      }
    }
  }
  else
...
)
{
  ...
  if ( majorType != HTTPCommon::HTTP_UNKNOWN_TYPE )
  {
    ...
    if (false == isMajorTypeFound)
    {
      ...
      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                    "Did not find major type %d in resource %lu",
  ...
  else
  {
    for ( uint32 i = 0; i < m_nNumOfTracks; i++ )
    {
      if ( pTrackInfo + i )
      {
        ...
        {
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Get trackInfo for index %lu id %lu failed", i,
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Invalid track info failed for index %lu", i );
      }
    }
  }
  return nNumTracks;
...
)
{
  ...
  if ( pTrackMap && pFileSource )
  {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Get track info failed for track %lu", pTrackMap->m_nTrackID );
    }
  }
  else
  {
    ...
    case HTTP_MINOR_TYPE_AAC_LOAS:
      {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                         "Get AAC code info failed for track %lu", nTrackID );
        }
      }
      break;
    default:
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "No codec info avaliable for minortype %d track id %lu",
                     minorType, nTrackID );
      break;
    }
  }
...
)
{
  ...
  if ( pTrackMap && pFileSource )
  {
    ...
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Unable to get the format block %d for track %ld",
...
)
{
  HTTPCommon::HTTPDownloadStatus eStatus = HTTPCommon::HTTPDL_ERROR_ABORT;
    ...
    if ( pTrackMap && pFileSource )
    {
      ...
      {
         httpSampleInfo.startTime = (uint64)((uint32)(1<<31) + fsSampleInfo.startTime);
         httpSampleInfo.endTime = (uint64)((uint32)(1<<31) + fsSampleInfo.endTime);
      }
      else
      {
        httpSampleInfo.endTime = fsSampleInfo.endTime;
        httpSampleInfo.startTime = fsSampleInfo.startTime;
      }
      ...
      default:
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Unhandled error %d for trackID %ld", fsStatus,
...
)
{
  ...
  {
    nPlaybackPosition =
      pFileSource->GetMediaCurrentPosition(pTrackMap->m_nTrackID);
...
)
{
  ...
  for ( uint32 i = 0; i < m_nNumOfTracks; i++ )
  {
    if(m_pTrackMap[i].m_bEndOfStream == false)
    {
      ...
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "get the last retrieved sample offset failed for %ld",
                        m_pTrackMap[i].m_nTrackID );
        break;
      }
      else
    ...
    else
    {
      ...
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                     "EOS detected on track %ld ignore the minoffset",
                      m_pTrackMap[i].m_nTrackID );
    }
  }
  MM_CriticalSection_Leave(m_resourceDataLock);
 ...
 * @return
 */
 bool HTTPResource::GetBaseTime(uint64& baseTime)
{
  int64 tmpBaseTime = -1;
  bool bRet = false;
  baseTime = 0;
        ...
        if(fsStatus == FILE_SOURCE_SUCCESS)
        {
          FS_PROTECTION_SYSTEM_SPECIFIC_INFOTYPE *pPSSHInfo
              = (FS_PROTECTION_SYSTEM_SPECIFIC_INFOTYPE *)
                 QTV_Malloc(fileSourceConfigItem.nresult * sizeof(FS_PROTECTION_SYSTEM_SPECIFIC_INFOTYPE));
          if (pPSSHInfo)
          {
            ...
            for (uint32 drmIndex = 0; drmIndex < fileSourceConfigItem.nresult; drmIndex++)
            {
              if (pPSSHInfo[drmIndex].ePSDRMType == FILE_SOURCE_MARLIN_DRM)
              {
                ...
                drmInfo->eDrmType = FILE_SOURCE_MARLIN_DRM;
