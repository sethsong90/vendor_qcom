...
#else
#define DASH_STATS_FILE_PATH              "/data/dash.stats"
#endif /* WIN32 */
#endif /* DASH_STATS */
...
DASHMediaPeriodHandler::~DASHMediaPeriodHandler()
{
  ...
  QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                 "Period [0x%02lx]: Destroyed DASH period handler %p",
...
bool DASHMediaPeriodHandler::HandleAdaptationSetChangeInfo(const char *pSelectionsXml)
{
               ...
               "AdaptationSetChange: New RepGrpKeys");
  {
          ...
          iter != iterEnd; iter = iter.Next())
    {
      ...
      if (rslt)
      {
        ...
        QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "AdaptationSetChange: majorType %lu, Key %llu (NEW)",
  ...
  if (true == rslt)
  {
    ...
    {
      QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Period [0x%02lx]: AdaptationSetChange cmd cannot be queued",
                     (uint32)((m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
...
void DASHMediaPeriodHandler::SelectAdaptationSet()
{
  ...
  if (pDASMHeadElem)
  {
    ...
    uint32 nPeriodXMLKey = ((GetPeriodKey() & MPD_PERIOD_MASK) >> MPD_PERIOD_SHIFT_COUNT);
    ...
    if (nOldRepGrpInfoKey != MAX_UINT64_VAL && nNewRepGrpInfoKey != MAX_UINT64_VAL)
    {
      ...
      if (bOk)
      {
        ...
        {
          QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
            "Failed to modify SelectedReps for new adaptationset xml key %lu",
            nOldAdapXMLKey);
      ...
      {
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
          "Failed to modify SelectedReps for old adaptationset xml key %lu",
          nOldAdapXMLKey);
      }
    }
        ...
        MAX_UINT64_VAL != nRemoveKey)
    {
      if (MAX_UINT64_VAL != nAddOrReplaceGrpKey)
      {
        ...
        if (rslt)
        {
          ...
          if (pNewPlayGroup)
          {
            ...
            if (pElem)
            {
              QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "AdaptationSet Change periodKey %llu adaptationSetKey %llu on repgrp idx %lu - pending QSM notification",
            ...
            {
              QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                "Failed to allocate RepGroupQElem for majorType %lu", nMajorType);
              QTV_Delete(pNewPlayGroup);
            }
          }
          else
          {
            QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
              "Failed to allocate DMPG for majorType %lu", nMajorType);
          }
        }
        else
        {
      ...
      if (QSM::MAJOR_TYPE_VIDEO == cInfo.m_eMajorType)
      {
        cInfo.m_bSwitchable = true;
      }
...
{
  QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "AdaptationSetChangeResponse tid %lu, adapsetChangeStatus %d",
                tid, (int)s);
  ...
  if (!m_cCmdQ.EnQ(cmd))
  {
    ...
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: AdaptationSetChangeResponse cmd cannot be queued",
                   (uint32)((m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
  }
...
HTTPDownloadStatus DASHMediaPeriodHandler::PeriodBaseStateHandler::Close()
{
  ...
  QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "Period [0x%02lx]: Closing", nKey );
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                   "Period [0x%02lx]: QSM STOP called", nKey );
  ...
  if (m_pPeriodHandler)
  {
    ...
    if (m_pPeriodHandler->m_pQSM)
    {
      ...
      QTV_MSG_PRIO7(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "SegInfoReady: PH %p, grKey '%llu', repKey %llu, startTime %lu, duration %lu, numUnits %lu, qStat %d",
...
)
{
  ...
  if (pGroupInfo && nSizeOfGroupInfo >= nNumGroupInfo)
  {
    ...
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                   "Period [0x%02lx]: Group info for %lu groups obtained successfully",
  ...
  else
  {
    ...
    QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: Insufficient buffer for group info %lu/%lu (actual/required)",
                   (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
)
{
  ...
  if (pPlayGroup)
  {
    if (pPlayGroup->IsValid())
    {
      ...
      case HTTPCommon::HTTPDL_SUCCESS:
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Period [0x%02lx]: Representation info for %lu reps in group %lu obtained successfully",
      ...
      case HTTPCommon::HTTPDL_INSUFFICIENT_BUFFER:
        status = QSM::SS_STATUS_INSUFF_BUFF;
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Period [0x%02lx]: Insufficient buffer for rep info %lu/%lu (actual/required)",
      ...
      default:
        ...
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Period [0x%02lx]: Could not get rep info for group %lu", nPerKey, (uint32)nGroupKey );
      }
    }
  }
  else
  {
    status = QSM::SS_STATUS_FAILURE;
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: Invalid group/ group key %lu (actual)",
...
)
{
  ...
  if(m_pPeriodHandler)
  {
    ...
    if (m_pPeriodHandler->m_pDAInfoQuery)
    {
      ...
      uint32 nMaxDuration = ((heapSize * 8) / (float)maxBitRate) * 1000;
...
)
{
  ...
  if (pSelection && pSelectionStatus)
  {
    ...
    for (uint32 i = 0; i < nNumGroupRepresentationSelection; i++)
    {
      ...
      QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                     "Period [0x%02lx]: Selection status %d for %lu/%lu/%lu (group/rep/data unit)",
                     (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
{
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "GroupRequestsCompleted on period %x, repGrp 0x%x%x",
                (unsigned int)m_pPeriodHandler, (unsigned int)(nGroupKey >> 32), (unsigned int)nGroupKey);
  //Notify group of period end (relative to period start)
  uint64 nRelPeriodEndTime =
    m_pPeriodHandler->m_sDASHSessionInfo.cMPDParser.GetDuration(m_pPeriodHandler->GetPeriodKey());
  ...
  if(nGroupKey == ALL_GROUPS )
  {
    ...
    m_pPeriodHandler->m_AvailableTracks = 0;
  }
  else
  {
    DASHMediaPlayGroup *pPlayGroup =  m_pPeriodHandler->GetPlayGrpByKey(nGroupKey);
    ...
    if (pPlayGroup)
    {
      ...
      if(!pPlayGroup->IsValid())
      {
        switch(pPlayGroup->m_nMajorType)
        {
          case QSM::MAJOR_TYPE_AUDIO :
            m_pPeriodHandler->m_AvailableTracks &= ~AUDIO_TRACK_AVAIL;
            break;
          case QSM::MAJOR_TYPE_VIDEO:
            m_pPeriodHandler->m_AvailableTracks &= ~VIDEO_TRACK_AVAIL;
            break;
          case QSM::MAJOR_TYPE_TEXT:
            m_pPeriodHandler->m_AvailableTracks &= ~TEXT_TRACK_AVAIL;
            break;
          default:
            break;
        }
      ...
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
              "GroupRequestsCompleted | setting value = %d",m_pPeriodHandler->m_AvailableTracks);
    }
    else
    {
      QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
...
int DASHMediaPeriodHandler::PeriodIdleStateHandler::ProcessCmds()
{
  ...
  if (m_pPeriodHandler->m_cCmdQ.PeekHead(cmd))
  {
    if (cmd.sBaseCmdData.eCmd == PERIOD_CMD_OPEN)
    {
      ...
      {
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Period [0x%02lx]: QSM started (nStartTime %lu), period moving to OPENING",
                       (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
)
{
  ...
  if (!m_pPeriodHandler->m_cCmdQ.EnQ(cmd))
  {
    status = HTTPCommon::HTTPDL_ERROR_ABORT;
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: OPEN cmd cannot be queued",
                   (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
  }
  ...
  while(m_pPeriodHandler->m_cCmdQ.Next(pIt, cmd))
  {
    ...
    if (cmd.sBaseCmdData.eCmd == PERIOD_CMD_OPEN)
    {
      ...
      if (result != HTTPCommon::HTTPDL_WAITING)
      {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Period [0x%02lx]: All groups ready, period moving to OPEN",
                         (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
        ...
        {
            QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                           "Period [0x%02lx]: Period open failed, closing the period",
                           (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
        ...
        while(m_pPeriodHandler->m_cCmdQ.Next(pSeekIt, seekCmd))
        {
          if (seekCmd.sBaseCmdData.eCmd == PERIOD_CMD_SEEK)
          {
            QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
                          "Period [0x%02lx]: Open cmd dequeued as period moving from"
                          "opening to seeking",
                          (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56));
            (void)m_pPeriodHandler->m_cCmdQ.Remove(pIt);
            break;
...
/*
 * Check if the media track is active for read.
 * API is called from the adaptor to get minimum
 * global playback position, active tracks which can be read out are
 * considered for the calculation of minimum global playback time
 *
 * @param[in] majorType  major media type for which track information
 * needs to be retreived
 *
 * @return
 * TRUE - if the track is present
 */
bool DASHMediaPeriodHandler::PeriodOpenStateHandler::IsMediaTrackAvailable
(
  HTTPCommon::HTTPMediaType majorType
)
{
  ...
  if(m_pPeriodHandler)
  {
    ...
    {
      status = m_pPeriodHandler->m_AvailableTracks & TEXT_TRACK_AVAIL;
    }
    else if(majorType == HTTPCommon::HTTP_AUDIO_TYPE)
    {
      status = m_pPeriodHandler->m_AvailableTracks & AUDIO_TRACK_AVAIL;
    }
    else if(majorType == HTTPCommon::HTTP_VIDEO_TYPE)
    {
      status = m_pPeriodHandler->m_AvailableTracks & VIDEO_TRACK_AVAIL;
    }
  }
...
)
{
  ...
  if (!m_pPeriodHandler->m_cCmdQ.EnQ(cmd))
  {
    status = HTTPCommon::HTTPDL_ERROR_ABORT;
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: Seek cmd cannot be queued",
                   (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
  }
...
{
  HTTPDownloadStatus status = HTTPCommon::HTTPDL_SUCCESS;
  //Store number of available tracks
  HTTPMediaTrackInfo TrackInfo;
  HTTPDownloadStatus eStatus = HTTPCommon::HTTPDL_UNSUPPORTED;
  HTTPCommon::HTTPMediaType mediaType[] = {HTTPCommon::HTTP_AUDIO_TYPE, HTTPCommon::HTTP_VIDEO_TYPE, HTTPCommon::HTTP_TEXT_TYPE};
  for (int i = 0; i < STD_ARRAY_SIZE(mediaType); i++)
  {
    eStatus = m_pPeriodHandler->GetSelectedMediaTrackInfo(mediaType[i], TrackInfo);
    if((HTTPCommon::HTTPDL_UNSUPPORTED == eStatus) || (HTTPCommon::HTTPDL_ERROR_ABORT == eStatus))
    {
      continue;
    }
    else
    {
      if(mediaType[i] == HTTPCommon::HTTP_TEXT_TYPE)
      {
        m_pPeriodHandler->m_AvailableTracks |= TEXT_TRACK_AVAIL;
      }
      else if(mediaType[i] == HTTPCommon::HTTP_AUDIO_TYPE)
      {
        m_pPeriodHandler->m_AvailableTracks |= AUDIO_TRACK_AVAIL;
      }
  ...
  if (m_pPeriodHandler->m_cCmdQ.PeekHead(cmd))
  {
    if (cmd.sBaseCmdData.eCmd == PERIOD_CMD_SEEK)
    {
      ...
      if (result != HTTPCommon::HTTPDL_WAITING)
      {
        ...
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Period [0x%02lx]: All groups ready, period moving to OPEN from SEEK",
                         (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
          m_pPeriodHandler->SetStateHandler(&m_pPeriodHandler->m_cOpenStateHandler);
        }
        else
        {
          QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "Period [0x%02lx]: Period Seek failed closing the period reporting data_end",
                         (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
          m_pPeriodHandler->Close();
        }
      }
                                                ...
                                                bool bNotified)
{
  ...
  for (uint32 i = 0; i < QSM::MAJOR_TYPE_MAX; ++i)
  {
    ...
    if (!rQ.IsEmpty())
    {
      ...
      {
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
          "AdaptationSetChange: RepGroupQ with idx %lu. Send DL switch "
...
)
{
  ...
  if (HTTPCommon::HTTPDL_SWITCH != eStatus)
  {
    ...
    if (pPlayGroup)
    {
      ...
      QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "GetNextMediaSample 1st PG %llu, isReadable %d, 2ndPG addr %x, isReadable %d",
        pPlayGroup->GetKey(), pPlayGroup->IsReadable(), (unsigned int)p2ndPlayGrp, bCanSwitchToSecondPlayGrp);
      ...
      if(HTTPCommon::HTTPDL_DATA_END == eStatus)
      {
        ...
        // Update the track availability info
        switch(majorType)
        {
...
)
{
  ...
  {
    QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                  "GetGroupDurationBuffered failed to find grp with key %llu in period %x",
                  nGroupKey, (unsigned int)m_pPeriodHandler);
  }
  else
  {
  ...
  if (0 == logCount[majorType])
  {
    QTV_MSG_PRIO7(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                  "GetGroupPlaybackStats: ok %d, GetGroupDurationBuffered major %d, (%lu,%lu,%lu), pbPos %llu, fma %lu",
...
)
{
  ...
  {
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
      "GetMediaDurationBuffered returning FALSE for Period %x", (unsigned int)m_pPeriodHandler);
  }
  ...
  {
    QTV_MSG_PRIO7(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                  "GlobalStats: ok %d, GetGroupDurationBuffered major %d, (%lu,%lu,%lu), pbPos %llu, fma %lu",
                  bResult, majorType, nFullyDownloadedOccupancy, nPartiallyDownloadedOccupancy,
                  nTotalOccupancy, nPlaybackPosition, nForwardMediaAvailability);
  }
  ++logCount[majorType];
...
)
{
  ...
  QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                 "Period [0x%02lx]: Async fatal error %d",
...
)
{
    ...
    eStatus == HTTPCommon::HTTPDL_DATA_END)
  {
    ...
    if (pPlayGroup)
    {
      ...
      {
        QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
         "Period [0x%02lx]: Seek completed on video playgroup seek time %lu",
         (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
         (uint32)nSeekEndTime);
        ...
        if(pAudioGrp && pAudioGrp != pVideoGrp)
        {
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
             "Period [0x%02lx]: Seek started on audio playgroup seek time %lu",
             (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
             (uint32)nSeekEndTime);
        ...
        else
        {
          ...
          {
             QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
             "Period [0x%02lx]: Seek started on Text playgroup seek time %lu",
             (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
             (uint32)nSeekEndTime);
      ...
      else
      {
        ...
        {
          QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
              "Period [0x%02lx]: Seek completed on audio playgroup seek time %lu",
              (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
               (uint32)nSeekEndTime);
        ...
        else
        {
          ...
          {
            QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "Period [0x%02lx]: Seek completed on Text playgroup seek time %lu",
                (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
                 (uint32)nSeekEndTime);
           }
           else
           {
             QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                "Period [0x%02lx]: Seek completed on unknown playgroup %llu seek time %lu",
                (uint32)((m_pPeriodHandler->m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
bool DASHMediaPeriodHandler::CreatePlayGroup()
{
  ...
  QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                "CreatePlayGroup for period %llu, numGroups %lu",
  ...
  if (nNumRepGroups > 0)
  {
    ...
    if (pRepGroup)
    {
      ...
      if (HTTPSUCCEEDED(status))
      {
          ...
          if (numPlayGrps <= HTTP_MAX_PLAYGROUPS)
          {
            ...
            if (pNewPlayGroup)
            {
              ...
              if (pElem)
              {
                ...
                QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                              "RepGrpQ: Added DownloadQElem key %llu, map arrayIdx %lu to majorType(arrayIdx) %lu",
        ...
        QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                       "Period [0x%02lx]: Created %d play groups successfully from %lu rep groups in the period",
                       (uint32)((m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
/** @brief Get Base time for the period
  ...
  * @return
  */
bool DASHMediaPeriodHandler::GetBaseTime(uint64 &baseTime)
{
  bool bRet = false;
  int64 tmpBaseTime = -1;
  ...
  for (uint32 i = 0 ; i <= QSM::MAJOR_TYPE_MAX; ++i)
  {
    ...
    if (!rQ.IsEmpty())
    {
      ...
      if (pElem)
      {
        ...
        if (pPlayGroup)
        {
          bRet = pPlayGroup->GetBaseTime(baseTime);
          tmpBaseTime = tmpBaseTime >= 0 ? STD_MIN((uint64)tmpBaseTime,baseTime):baseTime;
        }
      }
    }
  }
  ...
  if(tmpBaseTime >= 0 && bRet)
  {
    baseTime = tmpBaseTime;
  }
  else
  {
    baseTime = m_nStartTime;
    bRet = true;
  }
  return bRet;
}
...
int DASHMediaPeriodHandler::GetObservedBandwidth(uint32 nInterval)
{
  ...
  if (m_pStatsFile)
  {
    ...
    char statsFileEntry[100] = {0};
    int numBytesWritten = 0;
    (void)std_strlprintf(statsFileEntry, sizeof(statsFileEntry), "%lu\t%d\t%lu\t%d\t%llu\t%d\r\n",
                         (uint32)((m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56),
...
)
{
  ...
  QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
                 "Buffer occupancy = %lu msec", (uint32)nDuration );
...
)
{
  ...
  QTV_MSG_PRIO6(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
    "GetGroupPlaybackStats: ok %d: PG 0x%x, Media type %d Current Playback time = %llu Full Occ %lu Partial Occ %lu",
...
void DASHMediaPeriodHandler::SuspendQSM()
{
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                   "Period [0x%02lx]: PERIOD_CMD_SUSPEND cmd cannot be queued",
                   (uint32)((m_cPeriodInfo.getPeriodKey() & 0xFF00000000000000ULL) >> 56) );
  }
}
...
void DASHMediaPeriodHandler::RepGroupQ::Print()
{
       ...
       iter != iterEnd; iter = iter.Next())
  {
    ...
    QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "### repGrpKey %llu, isCommitted %d, isValid %d, isReadable %d",
                  pPlayGroup->GetKey(), (*iter)->IsCommitted(),  pPlayGroup->IsValid(), pPlayGroup->IsReadable());
  }
}
