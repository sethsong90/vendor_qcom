    ...
    else
    {
      ...
      while(str)
      {
        ...
        if(!std_strncmp(str,"avc1",4))
        {
          ...
          codec_info->mcodecs[index].profile=profile;
          codec_info->mcodecs[index].level=level;
        }
...
bool PlaylistParser::ResolveURI(char *base_uri,char *relative_path,char *absolute_url,int& absolute_url_len)
{
  ...
  bool bOk = true;
  int temp_url_len=std_strlen(base_uri)+1;
  ...
  **/
  ...
  {
    int req_len=std_strlen(relative_path)+std_strlen("http:")+1;
  ...
  **/
  else if(relative_path[0]=='/')
  {
    ...
    int firstslashindex = (firstslash ? (std_strlen(temp_url)-std_strlen(firstslash)) : 0);
    int req_len=firstslashindex +std_strlen(relative_path)+1;
  ...
  **/
  else if(relative_path[0]=='.')
  {
    ...
    int lastslashindex = (lastslash ? (std_strlen(temp_url)-std_strlen(lastslash)): 0);
    ...
    while(relative_path&& relative_path[0]=='.')
    {
      ...
      else if(std_stribegins(relative_path,"../"))
      {
        ...
        if(std_strlen(temp_url) > http_const_len &&
          std_strchr(temp_url+http_const_len,'/'))
        {
          lastslash =std_strrchr(temp_url,'/');
          lastslashindex = std_strlen(temp_url)- (lastslash ? std_strlen(lastslash) : 0);
          temp_url[lastslashindex]='\0';
          relative_path+=3;
        }
        else
    ...
    int req_len=(relative_path ? (std_strlen(temp_url)+std_strlen(relative_path)+2) : 0 );
  ...
  **/
  ...
  {
    char *lastslash =std_strrchr(temp_url,'/');
    int lastslashindex = std_strlen(temp_url)-(lastslash ? std_strlen(lastslash) : 0);
    temp_url[lastslashindex]='\0';
    int req_len=std_strlen(temp_url)+std_strlen(relative_path)+2;
    if((req_len <= absolute_url_len) && (absolute_url))
    {
      std_strlcpy(absolute_url,temp_url,absolute_url_len);
      std_strlcat(absolute_url,"/",absolute_url_len);
...
bool PeriodInfo::ResizeRepInfo(int newSize)
{
  bool bOk=false;
...
/* @brief: check if Segment Timeline is provided
 * return: true if present else false
 */
bool PeriodInfo::IsSegmentTimelineFound()
{
  bool bOk = false;
  if(m_pSegmentTemplate)
  {
    bOk = m_pSegmentTemplate->IsSegmentTimelineFound();
  }
  return bOk;
}

/* @brief: returns number of segment timeline entry
 */
int PeriodInfo::GetNumSegmentTimelineEntry()
{
  int numEntry = 0;
  if(m_pSegmentTemplate)
  {
 ...
 */
bool PeriodInfo::SetSegmentTimeline(uint32 segTimelineIndex, uint32 startTime,
                         uint32 duration, uint32 repeatCount)
{
  bool bOk = false;
  if (m_pSegmentTemplate)
  {
...
void RepresentationGroup::Copy(const RepresentationGroup& repgrp_info)
{
  ...
  {
    int bufSz = 1 + std_strlen(repgrp_info.m_GrpIdentifier);
  ...
  if(repgrp_info.m_MimeType)
  {
    ...
    int bufSize = std_strlen(repgrp_info.m_MimeType) + 1;
                                       ...
                                       int subSegmentSAPVal, uint32 grpID)
{
  ...
  if(repGrpId)
  {
    int reqdBufSize = 1 + std_strlen(repGrpId);
   ...
   if(pSamplingRates)
   {
     ...
     if(m_SamplingRate)
     {
       for(int i=0;i<m_nNumSamplingRates;i++)
       {
         m_SamplingRate[i] = pSamplingRates[i];
       }
     }
   }
   if(m_Channels)
   ...
   if(mimeType)
   {
     ...
     int bufSize = std_strlen(mimeType) + 1;
     m_MimeType = (char *)QTV_Malloc(bufSize * sizeof(char));
...
/* @brief: checks if segment timeline found */
bool RepresentationGroup::IsSegmentTimelineFound()
{
  bool bOk = false;
  if(m_pSegmentTemplate)
  {
...
/* @brief: returns number of segment timeline entry*/
int RepresentationGroup::GetNumSegmentTimelineEntry()
{
  int numEntry = 0;
  if(m_pSegmentTemplate)
  {
...
/* @brief: sets segment timeline
 */
bool RepresentationGroup::SetSegmentTimeline(uint32 segTimelineIndex, uint32 startTime,
                         uint32 duration, uint32 repeatCount)
{
  bool bOk = false;
  if (m_pSegmentTemplate)
...
{
  bool bOk = false;
  for (int i = 0; i < repInfo1->GetNumMediaStreamStructEntry(); i++)
  {
    for (int j = 0; j < repInfo2->GetNumMediaStreamStructEntry(); j++)
    {
...
void RepresentationInfo::Copy(const RepresentationInfo &rep_info)
{
  ...
  {
    int reqdBufSize = 1 + std_strlen(rep_info.m_BaseURL);
  ...
  if(rep_info.m_MimeType)
  {
    ...
    int reqdSize = std_strlen(rep_info.m_MimeType) + 1;
                                    ...
                                    char*  mediaStreamStructureId)
{
   ...
   if(mimeType)
   {
     ...
     int reqdSize = std_strlen(mimeType) + 1;
...
/* @brief: Initializes the Segment Info. Allocates an array of size
'size'
*/
SegmentInfo* RepresentationInfo::InitializeSegmentInfo(uint32 numSegments,
                                                       uint32 size)
{
...
*/
void RepresentationInfo::SetInitialisationSegmentRange(int64 nStartOffset, int64 nEndOffset)
{
...
/* @brief: sets initilisation segment url
 */
void RepresentationInfo::SetInitialisation(URLType *initialisation)
{
  if(m_pSegmentTemplate)
  {
    m_pSegmentTemplate->SetInitialisation(initialisation);
...
/* @brief: sets segment timeline
 */
bool RepresentationInfo::SetSegmentTimeline(uint32 segTimelineIndex, uint32 startTime,
                         uint32 duration, uint32 repeatCount)
{
  bool bOk = false;
  if (m_pSegmentTemplate)
...
bool RepresentationInfo::SetMediaStreamStructureId(char *mediaStructID)
{
  ...
  if (mediaStructID)
  {
    int len = std_strlen(mediaStructID) +  1;
    char *temp = NULL;
    int index = 0;
    int position = 0;
    if (m_MediaStreamStructureId)
    ...
    {
      char tempVal[10];
      position = (std_strstr(mediaStructID," ") == 0)? -1 : std_strstr(mediaStructID," ") - mediaStructID;
      if (position >= 0)
      {
...
int RepresentationInfo::GetMediaStreamStructureId(int index)
{
...
int RepresentationInfo::GetNumMediaStreamStructEntry()
{
  return m_nNumMediaStreamStructEntry;
}

/* @brief: checks if valid SAP value*/
bool RepresentationInfo::IsValidSAPVal()
{
  bool bOk = true;
  if (m_nStartWithSAPVal == -1 || m_nStartWithSAPVal ==0 || m_nStartWithSAPVal > 3)
  {
    bOk = false;
  }
  return bOk;
...
/* @brief: checks if valid subsegment SAP value*/
bool RepresentationInfo::IsValidSubSegementSAPVal()
{
  bool bOk = true;
  if (m_nSubSegmentStartsWithSAP == -1 || m_nSubSegmentStartsWithSAP ==0 || m_nSubSegmentStartsWithSAP > 3)
  {
    bOk = false;
  }
  return bOk;
...
void RepresentationInfo::MarkSelected(bool bSelectRep)
{
  ...
  if(m_bIsSelected)
  {
    ...
    //  "MarkSelected Representation with id '%s' key (%lu,%lu,%lu)",
...
SegmentInfo::~SegmentInfo()
{
  ...
  if(m_DecryptionInfo)
  {
    QTV_Free(m_DecryptionInfo);
    m_DecryptionInfo = NULL;
  }
  if(m_EncryptionMethod)
  {
    QTV_Free(m_EncryptionMethod);
    m_EncryptionMethod = NULL;
  }
  if(m_pSegDataLock)
  {
...
void SegmentInfo::Copy(const SegmentInfo &segment_info)
{
  ...
  m_bEncrypted= segment_info.m_bEncrypted;
...
/* @brief: Copy Constructor
*/
SegmentInfo::SegmentInfo(const SegmentInfo &segment_info)
: m_nStarttime(0),
m_nDuration(0),
m_nAvailabilitytime(0),
m_url(NULL),
...
{
  Copy(segment_info);
  if(segment_info.m_DecryptionInfo)
  {
    m_DecryptionInfo = (char*)QTV_Malloc(std_strlen(
      segment_info.m_DecryptionInfo)+1);
    if(m_DecryptionInfo)
    {
      (void)std_strlcpy(m_DecryptionInfo,segment_info.m_DecryptionInfo,
        std_strlen(segment_info.m_DecryptionInfo)+1);
    }
  }
  if(segment_info.m_EncryptionMethod)
  {
    m_EncryptionMethod = (char*)QTV_Malloc(std_strlen(
      segment_info.m_EncryptionMethod)+1);
    if(m_EncryptionMethod)
    {
      (void)std_strlcpy(m_EncryptionMethod,segment_info.m_EncryptionMethod,
        std_strlen(segment_info.m_EncryptionMethod)+1);
    }
  }
}
/* @brief Assignment operator overloading
  ...
  if(!m_bIsAvailableForUse)
  {
    if(m_url)
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
        "Segment key [%lu %lu ]",(uint32)(m_key >> 32),
        (uint32) (m_key));

      QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MEDIUM,
        "Segment key [program %lu period %lu "
        "representation %lu segment %lu] start time %f",
        (uint32)(m_key >> 51),
...
/*
* @brief
* Returns true if the segment is encrypted
*/
bool SegmentInfo::isEncrypted()
{
  return m_bEncrypted;
}
/*
...
void SegmentInfo::SetByteRangeURL(char* pByteRangeUrl)
{
  ...
  if(pByteRangeUrl)
  {
    int length = std_strlen(pByteRangeUrl);
                          ...
                          char* initRange, bool isdiscontinuous, bool isencrypted)
{
  ...
  if (indexUrl)
  {
    ...
    {
      tmpUrlLength = std_strlen(indexUrl)+1;
    }
  }
...
void SegmentTemplateType::SetMediaTemplate(char *media)
{
  ...
  if (media)
  {
    int medialen = std_strlen(media)+1;
...
void SegmentTemplateType::SetIndexTemplate(char *index)
{
  ...
  if (index)
  {
    int indexlen = std_strlen(index)+1;
...
void SegmentTemplateType::SetInitialisationTemplate(char *initialisation)
{
  ...
  if (initialisation)
  {
    int initialisationlen = std_strlen(initialisation)+1;
...
void SegmentTemplateType::SetBSSwitchingTemplate(char *bsSwitching)
{
  ...
  if (bsSwitching)
  {
    int bsSwitchinglen = std_strlen(bsSwitching)+1;
...
void MultipleSegmentBaseType::SetBitstreamSwitching(URLType *bitstreamSwitching)
{
  if (m_pBitstreamSwitching)
  {
    if (m_pBitstreamSwitching->sourceURL)
  ...
  {
    m_pBitstreamSwitching = (URLType *)QTV_Malloc(sizeof(URLType));
    if (m_pBitstreamSwitching)
    {
      if (bitstreamSwitching->sourceURL)
      {
        int len = std_strlen(bitstreamSwitching->sourceURL) + 1;
        m_pBitstreamSwitching->sourceURL = (char *)QTV_Malloc(len);
        if (m_pBitstreamSwitching->sourceURL)
        {
          std_strlcpy(m_pBitstreamSwitching->sourceURL, bitstreamSwitching->sourceURL, len);
        }
      }
      if (bitstreamSwitching->range)
      {
        int len = std_strlen(bitstreamSwitching->range) + 1;
        m_pBitstreamSwitching->range = (char *)QTV_Malloc(len);
        if (m_pBitstreamSwitching->range)
        {
          std_strlcpy(m_pBitstreamSwitching->range, bitstreamSwitching->range, len);
        }
      }
    }
  }
...
void SegmentBaseType::SetIndexRange(char *indexrange)
{
  ...
  if (indexrange)
  {
    int indexrangelen = std_strlen(indexrange)+1;
...
void SegmentBaseType::SetInitialisation(URLType *initialisation)
{
  ...
  if (initialisation)
  {
    ...
    if (m_pInitialisation)
    {
      ...
      {
        int len = std_strlen(initialisation->sourceURL) + 1;
      ...
      {
        int len = std_strlen(initialisation->range) + 1;
...
void SegmentBaseType::SetRepresentationIndex(URLType *repindex)
{
  ...
  if (repindex)
  {
    ...
    if (m_pRepresentationIndex)
    {
      ...
      {
        int len = std_strlen(repindex->sourceURL) + 1;
      ...
      if (repindex->range)
      {
        int len = std_strlen(repindex->range) + 1;
...
void SegmentTimelineType::SetStartTime(uint32 starttime)
{
  m_nStartTime = starttime;
}

void SegmentTimelineType::SetDuration(uint32  duration)
{
  m_nDuration = duration;
}

void SegmentTimelineType::SetRepeatCount(uint32  repeatCount)
{
  m_nRepeatCount = repeatCount;
}


uint32 SegmentTimelineType::GetStartTime()
{
  return m_nStartTime;
}

uint32 SegmentTimelineType::GetDuration()
{
  return m_nDuration;
}

uint32 SegmentTimelineType::GetRepeatCount()
{
...
void SegmentListType::SetSegmentUrl(int urlIndex, char* mediaUrl,
...
void SegmentURLType::SetMediaRange(char *mediaRange)
{
  ...
  if (mediaRange)
  {
    int len = std_strlen(mediaRange) + 1;
...
void SegmentURLType::SetIndexRange(char *indexRange)
{
  ...
  if (indexRange)
  {
    int len = std_strlen(indexRange) + 1;
...
bool ContentDescriptorType::SetDesc(const char *descName, const char *schemeURI, const char* value)
{
  ...
  if (descName && schemeURI && value)
  {
    ...
    int descNameSize = 1 + std_strlen(descName);
    ...
    int schemeUriSize = 1 + std_strlen(schemeURI);
    ...
    int schemeInfoSize = 1 + std_strlen(value);
...
bool ContentDescriptorType::Copy(const ContentDescriptorType& rCdt)
{
  ...
  if (rCdt.m_DescName)
  {
    ...
    int descNameSize = 1 + std_strlen(rCdt.m_DescName);
    m_DescName = (char *)QTV_Malloc(descNameSize * sizeof(char));
  ...
  if (rCdt.m_SchemeURI)
  {
    ...
    int schemeUriSize = 1 + std_strlen(rCdt.m_SchemeURI);
    m_SchemeURI = (char *)QTV_Malloc(schemeUriSize * sizeof(char));
  ...
  {
    int schemeInfoSize = 1 + std_strlen(rCdt.m_SchemeInfo);
    m_SchemeInfo = (char *)QTV_Malloc(schemeInfoSize);
    if (m_SchemeInfo)
    {
...
ContentProtectionType::ContentProtectionType() :
  ...
  m_MarlinMajorType(NULL),
  m_MarlinMinorType(NULL)
{
...
ContentProtectionType::~ContentProtectionType()
{
  ...
  size = m_MarlinContentIdList.Size();
  for (IPStreamList<char *>::Iterator iter = m_MarlinContentIdList.Begin();
       size > 0; iter = iter.Next(), size--)
  {
    char *ptr = (char *)(*iter);
    if(ptr)
    {
  ...
  size = m_MarlinBroadBand.m_SilentRightsUrlList.Size();
  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_SilentRightsUrlList.Begin();
       size > 0; iter = iter.Next(), size--)
  {
    char *ptr = (char *)(*iter);
    if(ptr)
  ...
  size = m_MarlinBroadBand.m_PreviewRightsUrlList.Size();
  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_PreviewRightsUrlList.Begin();
       size > 0; iter = iter.Next(), size--)
  {
    char *ptr = (char *)(*iter);
    if(ptr)
 ...
 size = m_MarlinBroadBand.m_RightsIssuerUrlList.Size();
  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_RightsIssuerUrlList.Begin();
       size > 0; iter = iter.Next(), size--)
  {
    char *ptr = (char *)(*iter);
    if(ptr)
  ...
  size = m_MS3.m_URIsAreTemplatedList.Size();
  for (IPStreamList<char *>::Iterator iter = m_MS3.m_URIsAreTemplatedList.Begin();
       size > 0; iter = iter.Next(), size--)
  {
    char *ptr = (char *)(*iter);
    if(ptr)
...
void ContentProtectionType::SetMarlinMajorType(const char* majorType)
{
  if(majorType)
  {
    int reqdSize = 1 + std_strlen(majorType);

    if(m_MarlinMajorType)
    {
      QTV_Free(m_MarlinMajorType);
    }

    m_MarlinMajorType = (char *)QTV_Malloc(reqdSize * sizeof(char));
    if(m_MarlinMajorType)
    {
      std_strlcpy(m_MarlinMajorType, majorType, reqdSize);
    }
  }
}

void ContentProtectionType::SetMarlinMinorType(const char* minorType)
{
  if(minorType)
  {
    int reqdSize = 1 + std_strlen(minorType);

    if(m_MarlinMinorType)
    {
      QTV_Free(m_MarlinMinorType);
    }

    m_MarlinMinorType = (char *)QTV_Malloc(reqdSize * sizeof(char));
    if(m_MarlinMinorType)
    {
      std_strlcpy(m_MarlinMinorType, minorType, reqdSize);
    }
  }
}

void ContentProtectionType::AddMarlinContentId(const char* marlinContentId)
{
  if(marlinContentId)
  {
    int reqdSize = 1 + std_strlen(marlinContentId);
    char *strMarlinContentId = (char *)QTV_Malloc(reqdSize * sizeof(char));
    if(strMarlinContentId)
    {
      std_strlcpy(strMarlinContentId, marlinContentId, reqdSize);
      m_MarlinContentIdList.Push(strMarlinContentId);
    }
  }
}


void ContentProtectionType::AddSilentRightsUrl(const char* silentRightsUrl)
{
  if(silentRightsUrl)
  {
    int reqdSize = 1 + std_strlen(silentRightsUrl);
    char *str = (char *)QTV_Malloc(reqdSize * sizeof(char));

    if(str)
    {
      std_strlcpy(str, silentRightsUrl, reqdSize);
      m_MarlinBroadBand.m_SilentRightsUrlList.Push(str);
    }
  }
}

void ContentProtectionType::AddPreviewRightsUrl(const char* previewRightsUrl)
{
  if(previewRightsUrl)
  {
    int reqdSize = 1 + std_strlen(previewRightsUrl);
    char *str = (char *)QTV_Malloc(reqdSize * sizeof(char));

    if(str)
    {
      std_strlcpy(str, previewRightsUrl, reqdSize);
      m_MarlinBroadBand.m_PreviewRightsUrlList.Push(str);
    }
  }
}

void ContentProtectionType::AddRightsIssuerUrl(const char* rightsIssuerUrl)
{
  if(rightsIssuerUrl)
  {
    int reqdSize = 1 + std_strlen(rightsIssuerUrl);
    char *str = (char *)QTV_Malloc(reqdSize * sizeof(char));

    if(str)
    {
      std_strlcpy(str, rightsIssuerUrl, reqdSize);
      m_MarlinBroadBand.m_RightsIssuerUrlList.Push(str);
    }
  }
}

void ContentProtectionType::AddUrisAreTemplated(const char* urisAreTemplated)
{
  if(urisAreTemplated)
  {
    int reqdSize = 1 + std_strlen(urisAreTemplated);
    char *str = (char *)QTV_Malloc(reqdSize * sizeof(char));

    if(str)
    {
      std_strlcpy(str, urisAreTemplated, reqdSize);
      m_MS3.m_URIsAreTemplatedList.Push(str);
    }
  }
}
...
const char *ContentProtectionType::GetMajorType() const
{
  return m_MarlinMajorType;
}

const char *ContentProtectionType::GetMinorType() const
{
  return m_MarlinMinorType;
}

IPStreamList<char *>& ContentProtectionType::GetContentIDList()
{
  return m_MarlinContentIdList;
}

ContentProtectionType::MarlinBroadBandType& ContentProtectionType::GetMarlinBroadBand()
{
  return m_MarlinBroadBand;
}
...
void ContentProtectionType::Print()
{
  ...
  QTV_MSG_SPRINTF_PRIO_2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                         "ContentProtectionType::Print Marlin Major %s, Marlin Minor %s",
                         m_MarlinMajorType ? m_MarlinMajorType : "",
                         m_MarlinMinorType ? m_MarlinMinorType : "");

  for (IPStreamList<char *>::Iterator iter = m_MarlinContentIdList.Begin();
        iter != m_MarlinContentIdList.End();
        iter = iter.Next())
  {
    QTV_MSG_SPRINTF_PRIO_1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "ContentProtectionType::Print MarlinContentId %s", *iter);
  }

  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_SilentRightsUrlList.Begin();
        iter != m_MarlinBroadBand.m_SilentRightsUrlList.End();
        iter = iter.Next())
  {
    QTV_MSG_SPRINTF_PRIO_1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "ContentProtectionType::Print m_SilentRightsUrl %s", *iter);
  }

  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_PreviewRightsUrlList.Begin();
        iter != m_MarlinBroadBand.m_PreviewRightsUrlList.End();
        iter = iter.Next())
  {
    QTV_MSG_SPRINTF_PRIO_1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "ContentProtectionType::Print m_PreviewRightsUrl %s", *iter);
  }

  for (IPStreamList<char *>::Iterator iter = m_MarlinBroadBand.m_RightsIssuerUrlList.Begin();
        iter != m_MarlinBroadBand.m_RightsIssuerUrlList.End();
        iter = iter.Next())
  {
    QTV_MSG_SPRINTF_PRIO_1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "ContentProtectionType::Print m_RightsIssuerUrlList %s", *iter);
  }

  for (IPStreamList<char *>::Iterator iter = m_MS3.m_URIsAreTemplatedList.Begin();
        iter != m_MS3.m_URIsAreTemplatedList.End();
        iter = iter.Next())
  {
    QTV_MSG_SPRINTF_PRIO_1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
      "ContentProtectionType::Print m_URIsAreTemplated %s", *iter);
  }
}
...
bool StringValue::SetString(const char *name, const char *value)
{
  ...
  if(name && value)
  {
    ...
    int nameBufSize = 1 + std_strlen(name);
    int valueBufSize = 1 + std_strlen(value);
