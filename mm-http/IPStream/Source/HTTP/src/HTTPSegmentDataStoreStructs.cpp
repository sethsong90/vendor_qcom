 ...
 */
bool HttpSegmentDataStoreHeap::GetBuffer(byte* &pBuf, int& nBufSize)
{
  bool rslt = false;
  pBuf = NULL;
  ...
  int arrayIdx = m_NumBytesDownloaded / m_ChunkSize;
  ...
  {
    int numDownloadedInLogicalUnit = m_NumBytesDownloaded % m_ChunkSize;
    ...
    else
    {
      ...
      int maxAvailable = m_ChunkSize - numDownloadedInLogicalUnit;
  ...
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                "GetBuffer() result %d, pBuf %p, nBufSize %d",
  ...
  else
  {
    ...
    else
    {
      ...
      else
      {
        ...
        {
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
            "CommitBuffer() nNumWritten %lld larger than max allowed %d",
 ...
 */
int HttpSegmentDataStoreHeap::Read(int nOffset, byte* pBuf, int nBufSize)
{
  QTV_MSG_PRIO3(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
  ...
  if ((nOffset >= 0) && (nOffset < m_NumBytesDownloaded))
  {
    ...
    do
    {
      ...
      int readArrayIdx = nOffset / m_ChunkSize;
      ...
      int readOffset = nOffset % m_ChunkSize;
      byte *pReadPtr = m_HeapMemArray[readArrayIdx] + readOffset;

      int writeArrayIdx = m_NumBytesDownloaded / m_ChunkSize;
      ...
      else
      {
        ...
        else
        {
          QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
            ...
            "writeArrayIdx %d", readArrayIdx, writeArrayIdx);
        }
      }

      int numToRead = (nBufSize <= maxToRead ? nBufSize : maxToRead);

      if (numToRead > 0)
      {
        memcpy(pBuf, pReadPtr, numToRead);

        QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
          "HttpSegmentDataStoreHeap::Read() numRead %lld, maxToRead %lld, "
          "readArrayIdx %d, writeArrayIdx %d",
...
{
  int startArrIdx = ((nOffset)/m_ChunkSize);
  int endArrIdx = (m_NumBytesDownloaded/m_ChunkSize);
  ...
  {
    int writeOffset = nOffset % m_ChunkSize;
    std_memset(m_HeapMemArray[(nOffset/m_ChunkSize)]+writeOffset,0,m_ChunkSize - writeOffset);
  }
}
/**
 ...
 * Get ptr to buffer to write to for the segment.
 */
bool HttpSegmentDataStoreListElement::GetBuffer(byte* &pBuf, int& nBufSize)
{
 ...
 * start of the segment.
 */
int HttpSegmentDataStoreListElement::Read(int nOffset, byte* pBuf, int nBufSize)
{
