...
public:
  ...
  bool StoreDRMInfo(FileSource *pFileSource, uint32 trackId, HTTPDrmInfo *drmInfo);
   ...
   virtual void SetSegmentInfo(const SegmentInfo &cSegInfo ,const uint64 nStartTime,
                               const uint64 nDuration,const uint64 nKey) = 0;
  ...
  virtual bool GetStartTimeForLastDataUnit(uint64& nStartTime, double nRepEndTime) = 0;
...
public:
  ...
  virtual char* GetVideoURL(int& size) = 0;
...
protected:
  ...
  virtual bool GetTimeForFileOffset(HTTPMediaType majorType, uint32 nOffset, uint64& nTimeOffset);
    ...
    if ( m_pTrackMap && m_nNumOfTracks > 0 )
    {
      ...
      if ( i == m_nNumOfTracks )
      {
        QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                       "Unable to find the track map for %ld", nTrackID );
      }
    }
    else
    {
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_ERROR,
                     "Invalid resource state %p %lu",
