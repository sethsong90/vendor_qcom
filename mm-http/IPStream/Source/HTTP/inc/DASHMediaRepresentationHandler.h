...
public:
   ...
   * pts offset.
   */
  bool GetBaseTime(uint64& baseTime);
...
public:
  ...
  {
    QTV_MSG_PRIO1( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "Rep [0x%06lx]: Closing",
...
private:
  ...
  public:
    InitializationSegment(const char* pURL, uint64 nStartOffset, uint64 nEndOffset,
                          DASHSessionInfo& sDASHSessionInfo, IHttpHandler* HTTPStack,
                          HTTPBandwidthEstimator *pSharedBandwidthEstimator, bool& bOk)
      :m_pURL(pURL),
      ...
      m_pHTTPStack(HTTPStack)
    {
      ...
      bOk = m_cDownloader.Init(&sDASHSessionInfo.sSessionInfo, m_pHTTPStack, pSharedBandwidthEstimator);
    };
