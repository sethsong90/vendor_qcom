...
public:
  ...
  void SetByteRangeURL(const char* pByteRangeUrl)
  {
    ...
    if(pByteRangeUrl)
    {
      int len = (uint32)std_strlen(pByteRangeUrl);
...
private:
  ...
  public:
    ...
    virtual bool Open(const char* pURL, const char* pByteRangeUrl, const int64 nStartOffset, const int64 nEndOffset, const int nDurationMs = -1, const uint32 nHTTPRequestID = UNDEFINED_VAL_REQUESTID)
    {
      return true;
    };
  ...
  void UpdateNumBytesReceived(const uint64 nSizeRead)
  {
    ...
    m_pBandwidthEstimator->UpdateSize(nSizeRead);
    MM_CriticalSection_Leave(m_pDataLock);
  };
