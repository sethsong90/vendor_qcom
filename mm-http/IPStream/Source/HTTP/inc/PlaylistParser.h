...
public:
  ...
  */
  ...
  bool SetSegmentTimeline(uint32 segTimelineIndex, uint32 startTime,
...
public:
  ...
  void SetMarlinMajorType(const char* majorType);
  void SetMarlinMinorType(const char* minorType);

  void AddMarlinContentId(const char* marlinContentId);

  void AddSilentRightsUrl(const char* silentRightsUrl);
  void AddPreviewRightsUrl(const char* previewRightsUrl);
  void AddRightsIssuerUrl(const char* rightsIssuerUrl);

  void AddUrisAreTemplated(const char* urisAreTemplated);

  ContentProtectionSource GetContentProtectionSource() const;

  const char *GetMajorType() const;
  const char *GetMinorType() const;

  IPStreamList<char *>& GetContentIDList();

  struct MarlinBroadBandType
  {
    IPStreamList<char *> m_SilentRightsUrlList;
    IPStreamList<char *> m_PreviewRightsUrlList;
    IPStreamList<char *> m_RightsIssuerUrlList;
  };
  MarlinBroadBandType& GetMarlinBroadBand();
...
private:
  ...
  char *m_MarlinMajorType;
  char *m_MarlinMinorType;
  IPStreamList<char *> m_MarlinContentIdList;

  MarlinBroadBandType m_MarlinBroadBand;

  struct MS3Type
  {
    IPStreamList<char *> m_URIsAreTemplatedList;
  };
...
public:
  ...
  public:
    ...
    virtual HTTPDownloadStatus GetFirstAvailableSegmentStartTimeForRepresentation(
       MPDParser *pMPDParser, PeriodInfo* pPeriodInfo, int64& minStartTime,
       double nCurrMSeconds, bool& bIsCurrentTimeTooEarlyOnAllReps, bool& bIsCurrentTimeTooLateOnAllReps, RepresentationInfo* pRepInfo, bool bStartOfPlayback) = 0;

    virtual HTTPDownloadStatus GetLastAvailableSegmentTimeForRepresentation(
       MPDParser *pMPDParser, PeriodInfo* pPeriodInfo, RepresentationInfo* pRepInfo, uint64& latestSegmentStartTime, uint64& latestSegmentEndTime) = 0;
    ...
    virtual HTTPDownloadStatus GetLastSegmentKeyForRepresentation(
       PeriodInfo* pPeriodInfo, uint64* pSegmentInfo, RepresentationInfo* pRepInfo ) = 0;
  ...
  public:
    ...
    virtual HTTPDownloadStatus GetFirstAvailableSegmentStartTimeForRepresentation(
       MPDParser *pMPDParser, PeriodInfo* pPeriodInfo, int64& minStartTime,
       double nCurrMSeconds, bool& bIsCurrentTimeTooEarlyOnAllReps, bool& bIsCurrentTimeTooLateOnAllReps, RepresentationInfo* pRepInfo, bool bStartOfPlayback);

    virtual HTTPDownloadStatus GetLastAvailableSegmentTimeForRepresentation(
       MPDParser *pMPDParser, PeriodInfo* pPeriodInfo, RepresentationInfo* pRepInfo, uint64& latestSegmentStartTime, uint64& latestSegmentEndTime);
    ...
    virtual HTTPDownloadStatus GetLastSegmentKeyForRepresentation(
       PeriodInfo* pPeriodInfo, uint64* pSegmentInfo, RepresentationInfo* pRepInfo );
  ...
  */
  ...
  SegmentInfo* InitializeSegmentInfo(uint32 numSegments,uint32 size);
  ...
  void SetInitialisationSegmentRange(int64 nStartOffset, int64 nEndOffset);
  ...
  int GetMediaStreamStructureId(int index);
  bool CompareStreamStructId(char * structId1,char * structId2);
  SegmentTemplateType *GetSegmentTemplate();
  int GetNumSegmentTimelineEntry();
  int GetNumMediaStreamStructEntry();
  bool IsValidSAPVal();
  bool IsValidSubSegementSAPVal();
  ...
  */
  ...
  bool SetBaseURL(const char *baseUrl)
  {
    ...
    if (baseUrl)
    {
      ...
      int reqdSize = 1 + std_strlen(baseUrl);
...
public:
  ...
  SegmentInfo(const SegmentInfo &segment_info);
  ~SegmentInfo();
...
private:
  ...
  char* m_pInitRange;
  uint64 m_key;
  char *m_DecryptionInfo;
  char* m_EncryptionMethod;
...
public:
  ...
  /*
  * @brief
  * Returns true if session is a live streaming session
  */
  virtual bool isLive()
  {
    return m_bIsLive;
  ...
  /*
  * @brief - Sets the url for the playlist parser to start download
...
public:
  ...
  uint32 GetSegTimeLineStartTime(uint32 segmentTimelineIndex);
  uint32 GetSegTimeLineDuration(uint32 segmentTimelineIndex);
  uint32 GetSegTimeLineRepeatCount(uint32 segmentTimelineIndex);
...
public:
  ...
  void SetSegmentUrl(int segUrlIndex, char* mediaUrl,
                     char* mediaRange,char* indexUrl,char* indexRange);
