...
public:
  ...
  )
  {
    ...
    QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                   "Current playback time for PH %p = %lu msec, bOk %d", (void *)this, (uint32)nTime, bOk  );
  ...
  )
  {
    ...
    else
    {
      ...
      QTV_MSG_PRIO5(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
        "RequestNumberDataUnitsInfo PH %p, KEYS gr %lu, rep %lu,startTime %lu, duration %lu",
    ...
    else
    {
      PeriodBaseStateHandler* pCurrentStateHandler = GetStateHandler();
        ...
        QTV_MSG_PRIO6(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
          "ReadDataUnitsInfo: GrpKey %llu, repKey %llu, startTime %llu, duration %llu, nElements, %lu, nFilled %lu",
        ...
        for (uint32 i = 0; i < nFilledElements; ++i)
        {
          QTV_MSG_PRIO4(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
            "ReadDataUnitsInfo: unitKey %llu, unitStart %llu, unitDuration %llu, nFilled %lu",
  ...
  )
  {
    ...
    else
    {
      ...
      QTV_MSG_PRIO5( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_LOW,
                     "RequestDownloadDataUnit PH %p, keys gr %lu, rep %lu, dunit %lx:%lx",
...
public:
  ...
  virtual bool IsMediaTrackAvailable(HTTPCommon::HTTPMediaType majorType)
  {
    bool eStatus = false;
    PeriodBaseStateHandler* pCurrentStateHandler = GetStateHandler();
    if (pCurrentStateHandler)
    {
      eStatus = pCurrentStateHandler->IsMediaTrackAvailable(majorType);
    }
    return eStatus;
  };
  ...
  void GetVideoInfo(HTTPCommon::HTTPMediaType majorType, char* pVideoURL,int& nURLSize, char* pIPAddr, int& nIPAddrSize)
  {
    PeriodBaseStateHandler* pCurrentStateHandler = GetStateHandler();
    if (pCurrentStateHandler)
    {
  ...
  virtual HTTPDownloadStatus GetContentProtectElem(HTTPDrmType &drmType,
