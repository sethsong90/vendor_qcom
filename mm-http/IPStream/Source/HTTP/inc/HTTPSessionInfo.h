...
public:
    ...
    m_bQtaEnabled(false)
  {
    ...
    if(*maxResolution)
    {
      ...
      if(pMaxHeight)
      {
        ...
        {
          m_MaxSupportedRepResolution = STD_MIN((maxWidth * maxHeight), MAX_RESOLUTION);
        }
        QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                        "Using m_MaxSupportedRepResolution %lu", (uint32)m_MaxSupportedRepResolution);
      }
    }
    ...
    if(*maxBw)
    {
      if(atoi(maxBw) > 0)
      {
      m_MaxSupportedRepBw = STD_MIN(atoi(maxBw), HTTP_MAX_REP_BANDWIDTH);
      }

      QTV_MSG_PRIO2(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "persist.dash.max.rep.bandwidth %lu (capped to %lu)",
                       m_MaxSupportedRepBw, HTTP_MAX_REP_BANDWIDTH);
    }
#endif
    m_MaxSupportedVideoBufferSize = (m_MaxSupportedRepResolution * 3/2);
    QTV_MSG_PRIO1(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH,
                      "Using m_MaxSupportedVideoBufferSize %lu",
  ...
  void SetInitialPreroll(const uint32 preroll, const bool bConfig = true)
  {
    ...
    if (bConfig)
    {
      m_sInitialPreroll.nConfigured = m_sInitialPreroll.nComputed;
    }
  }
  ...
  void SetRebufferPreroll(const uint32 preroll, const bool bConfig = true)
  {
    ...
    if (bConfig)
    {
      m_sRebufferPreroll.nConfigured = m_sRebufferPreroll.nComputed;
    }
  }
  ...
  bool SetProxyServer(const char* proxyServer, int32 ProxyServerLen);
  ...
  void EnableQta()
  {
    QTV_MSG_PRIO(QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "EnableRequestAccelerator");
    m_bQtaEnabled = true;
  }

  bool IsQtaEnabled() const
  {
    return m_bQtaEnabled;
  }
