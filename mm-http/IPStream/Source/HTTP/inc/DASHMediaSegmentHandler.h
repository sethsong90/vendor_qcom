...
$Header: //source/qcom/qct/multimedia2/Video/Source/IPStream/Source/HTTP/dev/DASH/inc/DASHMediaSegmentHandler.h#65 $
...
public:
  ...
  {
    QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_HIGH, "Seg [0x%08lx%08lx]: Closing",
  ...
  virtual void SetSegmentInfo(const SegmentInfo &cSegInfo, const uint64 nStartTime,
                              const uint64 nDuration,const uint64 nKey)
  {
  ...
  void UpdateIPAddr(IHttpHandler* pHTTPStack)
  {
    if(!m_bIsIPAddressUpdated)
    {
      ...
      m_pIPAddr = (char*)QTV_Malloc(size+1);
      memset(m_pIPAddr,0,size+1);
      if(m_pIPAddr && ipAddr)
      {
        std_strlcpy(m_pIPAddr,ipAddr,size+1);
   ...
   */
  virtual bool GetStartTimeForLastDataUnit(uint64& nStartTime, double nRepEndTime);
...
public:
  ...
  {
    HTTPCommon::HTTPDownloadStatus eStatus = HTTPResource::GetNextMediaSample(majorType, pBuffer, nSize, sampleInfo);
  ...
  virtual bool GetBaseTime(uint64& baseTime)
  {
...
private:
  ...
  HTTPDownloadStatus ProcessGetSegmentDataInit(uint64 nDataUnitKey);
  HTTPDownloadStatus ProcessGetSegmentDataDownload(uint64 nDataUnitKey, uint32 nCmdID);
