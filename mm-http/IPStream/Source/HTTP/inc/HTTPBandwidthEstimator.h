...
public:
   ...
   */
  void SetObservationInterval(const int nInterval);
   ...
   * and cached.
   */
  void Update(int32 bytesToRead, int32 bytesRead);

  /**
   * Updates cummulative data downloaded with amount passed as argument.
   */
  void UpdateSize(int bytesDownloaded)
  {
  ...
  void StartTimer()
  {
    ...
    if(!timerRunning)
    {
      ...
      QTV_MSG_PRIO2( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
                     "Start timer 0x%x  = %ld", (unsigned int)this, (uint32)startTime );
    }
 ...
 /**
  ...
  void StopTimer()
  {
    ...
    if(timerRunning && !refCount)
    {
      ...
      QTV_MSG_PRIO3( QTVDIAG_HTTP_STREAMING, QTVDIAG_PRIO_MED,
                   "Stop timer %x = %ld,elapsed time %ld",
                   (unsigned int)this, (uint32)endTime,(uint32)timeElapsed);
    }
  ...
  uint32 GetCummulativeDataDownloaded()
  {
    uint32 nDataDownloaded = 0;
    MM_CriticalSection_Enter(m_UpdateSizeLock);
...
private:
   ...
   */
  void CalculateBandwidth(int currentBucketIdx);
   ...
   */
  uint32 m_CummulativeDataDownloaded;
