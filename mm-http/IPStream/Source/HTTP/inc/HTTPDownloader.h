...
public:
  ...
  void EnableQta()
  {
    m_sessionInfo.EnableQta();
  }

  bool IsQtaEnabled() const
  {
    return m_sessionInfo.IsQtaEnabled();
  }
  ...
  bool SelectRepresentations(const char* SetSelectionsXML, HTTPControllerCb pCallback, const HTTPControllerCbData& callbackData);

  bool GetMPDText(char *pMPDTextStr, uint32 &mpdSize);

  void GetQOEData(uint32 &bandwidth, char *pVideoURL, int& nURLSize, char *pIpAddr, int& nIPAddrSize);
...
private:
  ...
  IHttpStatusNotificationHandler *m_pStackNotificationHandler;
