...
TEST(BKM, MKDirValid) {
	EXPECT_GE(BKMCL_MkDir((uint8*)ETEST_BKMCL_VALID_DIR), 0) << "BKMCL_MkDir valid dir test failed";
}
...
TEST(BKM, MKDirInvalid2) {
	EXPECT_LT(BKMCL_MkDir((uint8*)ETEST_BKMCL_WRONG_DIR2), 0) << "BKMCL_MkDir invalid dir test 1 failed";
}
...
TEST(BKM, MKDirNameTooLong) {
	...
	ret = BKMCL_MkDir((uint8*)ETEST_BKMCL_TOO_LONG_DIR);
}
...
TEST(BKM, RmDirValid) {
	EXPECT_GE(BKMCL_MkDir((uint8*)ETEST_BKMCL_VALID_DIR2), 0) << "BKMCL_RmDir valid dir test failed in creating the dir";
	EXPECT_EQ(BKMCL_RmDir((uint8*)ETEST_BKMCL_VALID_DIR2), 0) << "BKMCL_RmDir valid dir test failed";
}
