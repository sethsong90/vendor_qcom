...
#define TZ_PR_REM_SIZE             TZ_PR_MAX_DATA_LEN - sizeof(tz_pr_cmd_type) - sizeof(long)
...
typedef struct tz_pr_decrypt_video_req_old_s
{
  /** First 4 bytes should always be command id */
  tz_pr_cmd_type          pr_cmd;
  ...
  /**< Physical address of decrypted data */
  tz_buf_array_s_old_t    out_phy;

  /**< Contains a list of block to decrypt with associated IV etc. */
  tzEncryptionMetaData_t    encryptionMetaData[TZ_PR_MAX_ENCRYPTION_UNITS];

} __attribute__ ((packed)) tz_pr_decrypt_video_req_old_t;
...
typedef struct tz_pr_get_license_state_req_s
{
  ...
  tz_pr_cmd_type            pr_cmd;
  /** Query data */
  const unsigned short      querydata[TZ_PR_REM_SIZE - (SUM(tz_pr_cmd_type,unsigned long,uint32,uint32))];
  /** Size of query data */
  unsigned long             querydata_len;
  /** Application Context */
  uint32                    app_ctx;
...
typedef struct tz_pr_joindomain_proc_resp_req_s
{
  ...
  uint8                   joindom_rsp_ptr[TZ_PR_REM_SIZE - (SUM(tz_pr_cmd_type,uint32,uint32,long)) - sizeof(tzdrmdomainid_t)];
...
typedef struct tz_pr_leavedomain_proc_resp_req_s
{
  ...
  uint8                   leavedom_rsp_ptr[TZ_PR_REM_SIZE - (SUM(tz_pr_cmd_type,uint32,uint32,long))];
