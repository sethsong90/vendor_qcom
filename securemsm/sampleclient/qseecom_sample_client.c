...
static unsigned int test_iterations  = 1000;
...
int32_t qsc_run_crypto_test(uint32_t cmd, int dummy_thread)
{
	...
	printf("Starting qsc_run_crypto_test\n");
...
int32_t qsc_run_crypto_performance_test(uint32_t dummy_cmd, int dummy_thread)
{
	...
	printf("Starting qsc_run_crypto_performance_test\n");
...
int32_t qsc_run_start_stop_app_basic_test(uint32_t dummy_cmd,int thread)
{
	...
	struct QSEECom_handle *l_QSEEComHandle = NULL;
        ...
        printf("Basic_start_stop_test: thread %d\n", thread + 1);
...
{
	int i, rc, thread_errs;
	int32_t errors = 0;
	static pthread_t threads[2];

	printf("\n\n");
	printf("----------------------------------------------\n");
	printf(" Running Multi-Client BASIC Test...\n");
	printf("----------------------------------------------\n");
...
int main(int argc, char *argv[])
{
	...
	if (NON_LEGACY_CMD_LINE_ARGS ==
		...
		if((argv[1] == 'e') ||(argv[1] == 'h') ||(argv[1] == NULL)) {
			qsc_usage();
			qsc_legacy_usage(-1);
		}
