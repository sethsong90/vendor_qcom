...
void DFA::writeTableAndAPI(raw_ostream &OS, const std::string &TargetName) {
  ...
  std::vector<int> StateEntry(states.size()+1);
  ...
  // Populate StateEntry[states.size()] since we need that value to print out
  // DFAStateEntryTable
  StateEntry[states.size()] = ValidTransitions;
  OS << "{-1, -1}\n";
