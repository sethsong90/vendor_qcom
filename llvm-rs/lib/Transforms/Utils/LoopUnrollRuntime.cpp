...
/// - Create PHI nodes at the new preheader block to combine values
...
static void ConnectProlog(Loop *L, Value *TripCount, unsigned Count,
  ...
  // The new PHI node is inserted in the new preheader basic block.
  ...
  for (succ_iterator SBI = succ_begin(Latch), SBE = succ_end(Latch);
    ...
    for (BasicBlock::iterator BBI = (*SBI)->begin();
      ...
      // Add a new PHI node to the new preheader block and add the
      ...
      // that was created.
      NewPN->addIncoming(V, PrologEnd);
      // Adding a value to the new PHI node from the original loop preheader,
      // and update the existing PHI node.
      if (L->contains(PN)) {
        ...
        // Update the existing PHI node in the exist block.
        PN->addIncoming(V, PrologEnd);
      }
    }
  }
...
/// Try to figure out if unrolling the loop will be profitable or not. This
/// is really needed because prologue if-then-else blocks are converted into
/// jump tables and register allocator is not doing a good job of coalescing
/// register copies. For now, we disallow any triangle shape loops.
///
bool llvm::UnrollRuntimeLoopProfitable(Loop *L) {
  BasicBlock *Header = L->getHeader();
  BasicBlock *Latch = L->getLoopLatch();
  TerminatorInst *Term = Header->getTerminator();
  // Look for triangle loop body.
  if (Term->getNumSuccessors() == 2) {
    BasicBlock *Succ1 = Term->getSuccessor(0);
    BasicBlock *Succ2 = Term->getSuccessor(1);
    if (Succ1 == Latch) {
      Term = Succ2->getTerminator();
      if (Term->getNumSuccessors() == 1) {
        if (Term->getSuccessor(0) == Latch)
          return false;
      }
    }
  }
...
bool llvm::UnrollRuntimeLoopProlog(Loop *L, unsigned Count, LoopInfo *LI,
  ...
  if (!UnrollRuntimeLoopProfitable(L))
    return false;
  ...
  BranchInst::Create(NewPH, NewPH, BranchVal, PreHeaderBR);
