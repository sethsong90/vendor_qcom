...
static cl::opt<bool>
NewGenSel("new-gen-sel", cl::Hidden, cl::init(true),
...
typedef std::map<PHINode*, ValuePair> PhiSet;

static void ExpandInstVect (InstVect &Is, const BasicBlock *BB) {
...
static void HoistInstructions (Instruction *Where, BasicBlock *BB,
                               const InstSet &InitialSet) {
  typedef BasicBlock::InstListType InstListType;

  InstVect Is;
  for (InstSet::const_iterator I = InitialSet.begin(), E = InitialSet.end();
       I != E; ++I) {
    Instruction *In = *I;
    if (In->getParent() == BB)
      Is.push_back(In);
  ...
  ExpandInstVect(Is, BB);

  InstSet ExpSet(Is.begin(), Is.end());


  BasicBlock::iterator It = BB->begin();
  BasicBlock::iterator End = BB->end();
  InstListType &BBIs = BB->getInstList();
  InstListType &BPIs = Where->getParent()->getInstList();

  while (It != End) {
    BasicBlock::iterator Next = next(It);
    Instruction *In = It;
    if (ExpSet.count(In)) {
      BBIs.remove(In);
      BPIs.insert(Where, In);
    }
    It = Next;
  }
}
...
static bool isSafeToSpeculativelyExecute(const BasicBlock *BB) {
  typedef BasicBlock::const_iterator iterator;
  for (iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
    if (!isa<BranchInst>(I) && !isSafeToSpeculativelyExecute(I))
      return false;
  }
  return true;
}
...
static bool CoversWholeBlock(InstSet &Ins, BasicBlock *BB) {
  for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
    Instruction *In = I;
    if (!isa<DbgInfoIntrinsic>(I)) {
      if (!isa<BranchInst>(I) && !Ins.count(In))
        return false;
    }
  }
  return true;
...
static bool TooExpensiveToSpeculate(BasicBlock *BB) {
  InstVect OpList;
  InstSet OpSet;
  ...
  for (unsigned i = 0; i < OpList.size(); ++i) {
    Instruction *In = OpList[i];
    for (User::op_iterator I = In->op_begin(), E = In->op_end(); I != E; ++I) {
      if (Instruction *OpI = dyn_cast<Instruction>(I)) {
        if (OpI->getParent() == BB && OpSet.count(In) == 0) {
          OpList.push_back(OpI);
          OpSet.insert(OpI);
        }
      }
    }
  ...
  for (unsigned i = 0, n = OpList.size(); i < n; ++i) {
    Instruction *In = OpList[i];
    unsigned C = 0;

    if (isSafeToSpeculativelyExecute(In)) {
      C = ComputeSpeculationCost(In);
    }
  ...
  return Cost > BBSpecThreshold;
}


static bool SpeculativelyExecuteBB2(BranchInst *BI) {
  if (!NewGenSel) return false;

  BasicBlock *BB1 = BI->getSuccessor(0);
  BasicBlock *BB2 = BI->getSuccessor(1);
  TerminatorInst *T1 = BB1->getTerminator();
  TerminatorInst *T2 = BB2->getTerminator();

  // Only diamonds.
  if (T1->getNumSuccessors() != 1 || T2->getNumSuccessors() != 1)
    return false;
  if (T1->getSuccessor(0) != T2->getSuccessor(0))
    return false;

  if (!isSafeToSpeculativelyExecute(BB1))
    return false;
  if (!isSafeToSpeculativelyExecute(BB2))
    return false;
  if (TooExpensiveToSpeculate(BB1) || TooExpensiveToSpeculate(BB2))
    return false;
  ...
  BasicBlock *BBJ = T1->getSuccessor(0);  // Join block.

  // Select the set of interesting phi nodes.
  InstSet ValuesToHoist;
  PhiSet Phis;

  for (BasicBlock::iterator II = BBJ->begin(), EI = BBJ->end();
       II != EI; ++II) {
    if (PHINode *PN = dyn_cast<PHINode>(II)) {
      Value *V1 = PN->getIncomingValueForBlock(BB1);
      Value *V2 = PN->getIncomingValueForBlock(BB2);
      Instruction *IV1 = dyn_cast<Instruction>(V1);
      Instruction *IV2 = dyn_cast<Instruction>(V2);
      if (!IV1 && !IV2) continue;

      if (IV1 && IV1->getParent() != BB1 && IV2 && IV2->getParent() != BB2) {
        // If neither value comes from a side node, skip.
        continue;
      }

      if (IV1) ValuesToHoist.insert(IV1);
      if (IV2) ValuesToHoist.insert(IV2);
      Phis.insert(std::make_pair(PN, std::make_pair(V1, V2)));
    } else {
      // No more PHIs.
      break;
    }
  ...
  if (ValuesToHoist.empty()) return false;
  if (!CoversWholeBlock(ValuesToHoist, BB1)) return false;
  if (!CoversWholeBlock(ValuesToHoist, BB2)) return false;

  HoistInstructions (BI, BB1, ValuesToHoist);
  HoistInstructions (BI, BB2, ValuesToHoist);

  IRBuilder<true, NoFolder> Builder(BI);
  SelectInst *SI;

  for (PhiSet::iterator PI = Phis.begin(), PE = Phis.end(); PI != PE; ++PI) {

    const ValuePair &VP = PI->second;

    Twine SelName;
    if (VP.first->hasName() && VP.second->hasName()) {
      SelName = VP.first->getName() + Twine(".") + VP.second->getName();
    } else {
      SelName = Twine("sel") + Twine(++SelCount);
    }

    SI = cast<SelectInst>(Builder.CreateSelect(BrCond, VP.first, VP.second,
                                               SelName));

    // Make the PHI node use the select for all incoming values
    // for "then" and "else" blocks.
    for (BasicBlock::iterator I = BBJ->begin();
         PHINode *PN = dyn_cast<PHINode>(I); ++I) {
      unsigned N1 = PN->getBasicBlockIndex(BB1);
      Value *V1 = PN->getIncomingValue(N1);
      if (V1 == VP.first)
        PN->setIncomingValue(N1, SI);

      unsigned N2 = PN->getBasicBlockIndex(BB2);
      Value *V2 = PN->getIncomingValue(N2);
      if (V2 == VP.second)
        PN->setIncomingValue(N2, SI);
    }
  }
...
static bool SpeculativelyExecuteBB_old(BranchInst *BI, BasicBlock *BB1) {
...
static bool SpeculativelyExecuteBB_new(BranchInst *BI, BasicBlock *BB1) {
  // Only speculatively execution a single instruction (not counting the
  // terminator) for now.
  InstVect HInsts;
  for (BasicBlock::iterator II = BB1->begin(), EI = BB1->end();
       II != EI; ++II) {
    Instruction *I = II;
    if (!isa<DbgInfoIntrinsic>(I) && !isa<BranchInst>(I))
      HInsts.push_back(I);
  ...
  for (InstVect::iterator II = HInsts.begin(), EI = HInsts.end();
       II != EI; ++II) {
    Instruction *I = *II;
    if (!isSafeToSpeculativelyExecute(I))
      return false;
  ...
  if (TooExpensiveToSpeculate(BB1)) return false;
  ...
  InstSet Is(HInsts.begin(), HInsts.end());

  if (!CoversWholeBlock(Is, BB1)) return false;

  HoistInstructions(BI, BB1, Is);
  ...
  for (unsigned i = 0, e = PHIs.size(); i != e; ++i) {
    ...
    // false value is the previously determined FalseV.
    Twine SelName;
    if (TrueV->hasName() && FalseV->hasName()) {
      SelName = TrueV->getName() + Twine(".") + FalseV->getName();
    } else {
      SelName = Twine("sel") + Twine(++SelCount);
    }
    ...
    SelectInst *SI;
    if (Invert) {
      SI = cast<SelectInst>(Builder.CreateSelect(BrCond, FalseV, TrueV,
                                                 SelName));
    } else {
      SI = cast<SelectInst>(Builder.CreateSelect(BrCond, TrueV, FalseV,
                                                 SelName));
    }
...
static bool BlockIsSimpleEnoughToThreadThrough(BasicBlock *BB) {
  ...
  // Avoid blocks that are targets of back-edges.
  typedef std::pair<const BasicBlock*, const BasicBlock*> CFGEdge;
  typedef SmallVector<CFGEdge,16> EdgeVector;

  EdgeVector BackEdges;
  FindFunctionBackedges(*BB->getParent(), BackEdges);

  for (EdgeVector::iterator I = BackEdges.begin(), E = BackEdges.end();
       I != E; ++I)
    if (I->second == BB) return false;
...
bool SimplifyCFGOpt::SimplifyCondBranch(BranchInst *BI, IRBuilder<> &Builder) {
  ...
  if (BI->getSuccessor(0)->getSinglePredecessor() != 0) {
    if (BI->getSuccessor(1)->getSinglePredecessor() != 0) {
      if (HoistThenElseCodeToIf(BI) || SpeculativelyExecuteBB2(BI))
        return SimplifyCFG(BB) | true;
    } else {
