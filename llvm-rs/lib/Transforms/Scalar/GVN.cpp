...
namespace {
  ...
  public:
    ...
    explicit GVN(bool nopre = false, bool noloads = false)
        : FunctionPass(ID), NoLoads(noloads), NoPre(nopre), MD(0) {
...
FunctionPass *llvm::createGVNPass(bool NoPre) {
  return llvm::createGVNPass(NoPre, false);
}
...
bool GVN::processNonLocalLoad(LoadInst *LI) {
  ...
  if (NoPre || !EnablePRE || !EnableLoadPRE)
    return false;
