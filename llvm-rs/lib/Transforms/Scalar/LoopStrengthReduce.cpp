...
cl::opt<bool> LSRICmpZero("lsr-icmp-zero", cl::Hidden, cl::init(false));
...
void Cost::RateRegister(const SCEV *Reg,
  ...
  if (const SCEVAddRecExpr *AR = dyn_cast<SCEVAddRecExpr>(Reg)) {
    ...
    if (TLI && isa<SCEVConstant>(LoopStep)) {
      // Check if a post-indexed load/store can be used.
      EVT VT = TLI->getValueType(AR->getType(), true);
      if (TLI->isIndexedLoadLegal(ISD::POST_INC, VT) ||
          TLI->isIndexedStoreLegal(ISD::POST_INC, VT)) {
...
/// Check if the SCEV contains a sub-SCEV (specifically, a SCEVAddRecExpr)
/// that references some other loop.
static bool isLocalSCEV(const SCEV *S, Loop *L) {
  if (const SCEVAddRecExpr *AR = dyn_cast<SCEVAddRecExpr>(S))
    if (AR->getLoop() != L)
      return false;
  if (const SCEVCastExpr *CE = dyn_cast<SCEVCastExpr>(S)) {
    if (!isLocalSCEV(CE->getOperand(), L))
      return false;
  } else if (const SCEVNAryExpr *NE = dyn_cast<SCEVNAryExpr>(S)) {
    for (unsigned i = 0, n = NE->getNumOperands(); i < n; ++i) {
      const SCEV *Op = NE->getOperand(i);
      if (!isLocalSCEV(Op, L))
        return false;
    }
  }
...
void LSRInstance::CollectFixupsAndInitialFormulae() {
  for (IVUsers::const_iterator UI = IU.begin(), E = IU.end(); UI != E; ++UI) {
    ...
    if (!isLocalSCEV(S, L))
      continue;
    if (LI.getLoopFor(UserInst->getParent()) != L)
      continue;
