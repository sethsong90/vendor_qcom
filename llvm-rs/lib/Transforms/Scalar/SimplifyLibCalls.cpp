...
namespace {
  ...
  protected:
    ...
    virtual void IncNumSimplified()  {++NumSimplified;}
  public:
    ...
    virtual void InitOptimizations();
...
//===-- LibCallOpt - library call replacement --------------------===//
// This pass replaces library calls versions based on a set of customizable
// rules.
// Currently a set of rules have been defined for math library calls.
// Math library functions are usually avaliable in single precision and
// double precision formats. But this information is not always exploited
// by programmers. This pass looks for missed opportunities in which
// precision can be dropped in favor of faster computation.
//
// For example, consider the code:
// float foo(float y) {
//   float x = (float)sin((double)y);
//   //   ...
// } in which, sin() is defined as "double sin(double)", i.e., a double
// precision floating point computation.
//
// This pass converts the above code to
//   float foo(float y) {
//   float x = sinf(y);
//   ...
// } in which, sinf() is defined as "float sinf(float)", i.e., a single
// precision floating point computation which executes faster than
// the double precision counterpart.
// ===---------------------------------------------------------------------===//

STATISTIC(NumLibCallOpt, "Number of versions of libcalls replaced");

//===----------------------------------------------------------------------===//
// Helper Functions
//===----------------------------------------------------------------------===//
static bool doArgsMatch(CallInst* Caller, FunctionType* FT) {
  for(unsigned int i=0; i<Caller->getNumArgOperands();++i) {
    FPExtInst * ArgDefInst = dyn_cast<FPExtInst>(Caller->getArgOperand(i));
    if (!ArgDefInst || ArgDefInst->getOperand(i)->getType() !=
        FT->getParamType(i)) {
      return false;
    }
  }
...
//===----------------------------------------------------------------------===//
// LibCall Verstion Selection Optimization
//===----------------------------------------------------------------------===//
namespace {
template<typename OldFuncSig,typename NewFuncSig>
struct LibCallFPVersionOpt : public LibCallOptimization {
  virtual Value *CallOptimizer(Function *Callee, CallInst *CI, IRBuilder<> &B) {
    Module* M = Callee->getParent();
    LLVMContext &Ctx = M->getContext();

    FunctionType *OldFT = TypeBuilder<OldFuncSig,true>::get(Ctx);
    FunctionType *NewFT = TypeBuilder<NewFuncSig,true>::get(Ctx);


    if (Callee->getFunctionType() != OldFT)
      return 0;

    Type * NewRT = NewFT->getReturnType();

    // Requirements for library call replacement
    // - Call return value has only one use
    // - Call return value is used truncated and match the return type of NewFT
    // - Call arguments match the ones defined in NewFT
    if (!CI->hasOneUse())
      return 0;

    Value::use_iterator ValI = CI->use_begin();
    FPTruncInst *ResUseInst = dyn_cast<FPTruncInst>(*ValI);
    if (!ResUseInst || ResUseInst->getType() != NewRT)
      return 0;

    if(CI->getNumArgOperands() != NewFT->getNumParams())
      return 0;

    if (!doArgsMatch(CI, NewFT))
      return 0;

    Function *NewFunc = dyn_cast<Function>(Callee->getParent()->
      getOrInsertFunction(Callee->getName().str() + "f", NewFT));
    if (!NewFunc)
      return 0;

    // Old call arguments that have been extended are replaced with
    // their non-extended definition
    std::vector<Value*> Args;
    for(unsigned int i=0; i<CI->getNumArgOperands();++i) {
      FPExtInst * ArgDefInst = dyn_cast<FPExtInst>(CI->getArgOperand(i));
      if(ArgDefInst)
        Args.push_back(ArgDefInst->getOperand(0));
      else
        Args.push_back(CI->getArgOperand(i));
    }
    CallInst* NewCI = CallInst::Create(NewFunc, ArrayRef<Value*>(Args),
    "", CI);
    BasicBlock::iterator ii(ResUseInst);
    ReplaceInstWithValue(ResUseInst->getParent()->getInstList(), ii,
            NewCI);
...
//===----------------------------------------------------------------------===//
// LibCall Version Selection Optimization Pass Implementation
//===----------------------------------------------------------------------===//
namespace {
class ReplaceLibCallVersions: public SimplifyLibCalls{
  LibCallFPVersionOpt<types::ieee_double(types::ieee_double),
    types::ieee_float(types::ieee_float)> DDToFFOpt;
  protected:
    virtual void IncNumSimplified()  {++NumLibCallOpt;}

public:
  explicit ReplaceLibCallVersions() : SimplifyLibCalls() {}
  void InitOptimizations();
  virtual const char *getPassName() const {
    return "Library Call Version Optimization";
  }
};
} // end anonymous namespace

void ReplaceLibCallVersions::InitOptimizations() {
  Optimizations["sin"] = &DDToFFOpt;
  Optimizations["cos"] = &DDToFFOpt;
  Optimizations["tan"] = &DDToFFOpt;
  Optimizations["asin"] = &DDToFFOpt;
  Optimizations["acos"] = &DDToFFOpt;
  Optimizations["atan"] = &DDToFFOpt;
  Optimizations["exp"] = &DDToFFOpt;
  Optimizations["exp2"] = &DDToFFOpt;
  Optimizations["exp10"] = &DDToFFOpt;
  Optimizations["log"] = &DDToFFOpt;
  Optimizations["log10"] = &DDToFFOpt;
  Optimizations["log2"] = &DDToFFOpt;
  Optimizations["logb"] = &DDToFFOpt;
  Optimizations["sqrt"] = &DDToFFOpt;
  Optimizations["cbrt"] = &DDToFFOpt;
  Optimizations["expm1"] = &DDToFFOpt;
  Optimizations["log1p"] = &DDToFFOpt;
  Optimizations["sinh"] = &DDToFFOpt;
  Optimizations["cosh"] = &DDToFFOpt;
  Optimizations["tanh"] = &DDToFFOpt;
  Optimizations["asinh"] = &DDToFFOpt;
  Optimizations["acosh"] = &DDToFFOpt;
  Optimizations["atanh"] = &DDToFFOpt;
}

Pass *llvm::createReplaceLibCallVersionsPass() {
  return new ReplaceLibCallVersions();
}
