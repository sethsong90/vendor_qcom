...
static cl::opt<int>
OpSizeNonLocalOrMoreCallersThreshold("inline-threshold-multicaller-size",
                                     cl::Hidden, cl::init(20),
cl::desc("Threshold for inlining more-than-one caller functions (-Os)"));
...
static bool hasMoreThanOneCaller(Function *F) {
  Value::use_iterator UI = F->use_begin();
  Value::use_iterator UE = F->use_end();
  if (UI == UE || ++UI == UE)
    return false;
  return true;
}
...
unsigned Inliner::getInlineThreshold(CallSite CS) const {
  ...
  // If compiling for size, don't inline functions that are called more than
  // once or don't have local linkage (except for very small functions).
  Function *Callee = CS.getCalledFunction();
  if (!Caller->isDeclaration() &&
      Caller->hasFnAttr(Attribute::OptimizeForSize) &&
      !Callee->hasFnAttr(Attribute::InlineHint) &&
      (!Callee->hasLocalLinkage() ||
        hasMoreThanOneCaller(Callee)))
    thres = OpSizeNonLocalOrMoreCallersThreshold;
