...
bool GlobalOpt::ProcessInternalGlobal(GlobalVariable *GV,
  ...
  if (GS.AccessingFunction && !GS.HasNonInstructionUser &&
      GV->getType()->getElementType()->isSingleValueType() &&
      GV->getType()->getAddressSpace() == 0) {
    if (!GS.HasMultipleAccessingFunctions &&
        GS.AccessingFunction->hasExternalLinkage()) {
      // If called from main, localize global variables
      if (GS.AccessingFunction->getName() == "main") {
        DEBUG(dbgs() << "LOCALIZING GLOBAL: " << *GV);
        Instruction &FirstI =
          const_cast<Instruction&>(*GS.AccessingFunction
                                   ->getEntryBlock().begin());
        Type *ElemTy = GV->getType()->getElementType();
        // FIXME: Pass Global's alignment when globals have alignment
        AllocaInst *Alloca =
          new AllocaInst(ElemTy, NULL, GV->getName(), &FirstI);
        ...
        ++NumLocalized;
        return true;
        // Mark the global variable to be safe to localize in loops
      }
      else
        GV->setSafeToLocalize(true);
    }
    else if (GS.HasMultipleAccessingFunctions)
      GV->setSafeToLocalizeMF(true);
  }
