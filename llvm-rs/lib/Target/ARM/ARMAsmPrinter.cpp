...
void ARMAsmPrinter::emitAttributes() {
  ...
  if (CPUString == "cortex-a8" ||
    ...
    if (Subtarget->isCortexA9()) {
      AttrEmitter->EmitTextAttribute(ARMBuildAttrs::CPU_name, "cortex-a9");
    }

    if (Subtarget->isScorpion() || Subtarget->isKrait()) {
      //Note:the assembler does not recognize cpu "scorpion" nor "krait",
      // using "cortex-a9" to avoid assembler warnings due to lack of
