...
bool ARMDAGToDAGISel::SelectLdStSOReg(SDValue N, SDValue &Base, SDValue &Offset,
  ...
  if (N.getOpcode() == ISD::MUL &&
      (!(Subtarget->isCortexA9() || Subtarget->isScorpion() ||
      Subtarget->isKrait()) || N.hasOneUse())) {
    if (ConstantSDNode *RHS = dyn_cast<ConstantSDNode>(N.getOperand(1))) {
  ...
  if (N.getOpcode() != ISD::SUB && ShOpcVal == ARM_AM::no_shift &&
      !(Subtarget->isCortexA9() || Subtarget->isScorpion() ||
      Subtarget->isKrait() || N.getOperand(0).hasOneUse())) {
...
bool ARMDAGToDAGISel::SelectAddrModePC(SDValue N,
  ...
  // when OptimizeGOT is on, the process below this line is not neccessary
  if (OptimizeGOT) return false;
