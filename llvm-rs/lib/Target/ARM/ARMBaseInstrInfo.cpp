...
CreateTargetPostRAProfitRecognizer(const InstrItineraryData *II) const {
   if (usePostRAProfitRecognizer()) {
      ...
      if (Subtarget.isScorpion() || Subtarget.isKrait())
        return (ScheduleProfitRecognizer *)
          new ARMProfitRecognizer(II, *this, Subtarget);
      return TargetInstrInfoImpl::CreateTargetPostRAProfitRecognizer(II);
   }
   return TargetInstrInfoImpl::CreateTargetPostRAProfitRecognizer(II);
}
...
isProfitableToIfCvt(MachineBasicBlock &MBB,
  ...
  // For cortex-a9, Scorpion and Krait, a weight similar to the misprediciton
  ...
  if (Subtarget.isCortexA9() || Subtarget.isScorpion() || Subtarget.isKrait())
    UnpredCost += 1;
  else
...
ARMBaseInstrInfo::getNumMicroOps(const InstrItineraryData *ItinData,
  ...
  //
  // On Cortex-A9/Scorpion, the formula is simply (#reg / 2) + (#reg % 2). If the address
  ...
  case ARM::t2STMDB_UPD: {
    ...
    if (Subtarget.isCortexA8()) {
      ...
      if (NumRegs % 2)
        ++UOps;
      return UOps;
    } else if (Subtarget.isCortexA9() || Subtarget.isScorpion() ||
      Subtarget.isKrait()) {
      UOps = (NumRegs / 2);
...
ARMBaseInstrInfo::getOperandLatency(const InstrItineraryData *ItinData,
  ...
  if (Reg == ARM::CPSR) {
    if (DefMI->getOpcode() == ARM::FMSTAT) {
      ...
      return (Subtarget.isCortexA9() || Subtarget.isScorpion() ||
        Subtarget.isKrait()) ? 1 : 20;
    }
  ...
  if (Latency > 1 &&
      ...
      Subtarget.isScorpion() || Subtarget.isKrait())) {
...
ARMBaseInstrInfo::getOperandLatency(const InstrItineraryData *ItinData,
  ...
  if (!UseNode->isMachineOpcode()) {
    ...
    if (Subtarget.isCortexA9() || Subtarget.isScorpion() ||
      Subtarget.isKrait())
...
unsigned ARMBaseInstrInfo::getInstrExtraPredCost(const MachineInstr *MI) const {
  unsigned Opc = MI->getOpcode();
  // Scorpion instructions predication cost retrieved from
  // Scorpion Code Generation Guidelines 80-VE670-2 Rev. E and
  // Scorpion PRM 80-VB867-5 Rev. D.
  // In general the instructions below introduce an additional 6 to 10 cycles
  // delay.
  // Conditional load and store multiples: LDM, STM, POP, PUSH, SRS, RFE.
  // Conditional MULT/MAC.
  // Conditional Load and store doubles (LDRD, STRD, LDREXD, STREXD).
  // Conditional store exclusives (STREX, STREXB, STREXH).
  // Conditional swaps (SWP, SWPB).
  // Conditional VFP load and store multiples (FLDMS, FLDMD, FLDMX,
  // VLDMD, FSTMS, FSTMD, FSTMX, VSTMD).
  // Conditional coprocessor ops (FMSTAT, LDC DTR, STC DTR).
  // Conditional MRRC instructions (MRRC FPS, MRRC FPD).
  // Conditional VU FMRXs (VMRS) that use external MRC logic.
  // Conditional VU instructions which transfer data to the integer GPR file
  // (FMRS, FMRDH, FMRDL, FMRRS, FMRRD).
  // Conditional data-processing S-form instructions which have a register shift.
  if (Subtarget.isScorpion()) {
    switch (Opc) {
    default:
      return 0;
      // Conditional load and store multiples: LDM, STM, POP, PUSH, SRS, RFE.
    ...
    case ARM::t2STRDi8:
      ...
      // Conditional store exclusives (STREX, STREXB, STREXH, STREXD).
    case ARM::STREX:
    case ARM::STREXB:
    case ARM::STREXD:
    case ARM::STREXH:
    ...
    case ARM::SWPB:
      ...
      // Conditional VFP load and store multiples (VLDM, VSTM, VPOP, VPUSH).
    case ARM::VLDMDDB_UPD:
    case ARM::VLDMDIA:
    case ARM::VLDMDIA_UPD:
    case ARM::VLDMQIA:
    ...
    case ARM::VMSR_FPSID:
      ...
      // Conditional VU instructions which transfer data to the integer GPR file
      // (FMRS (VMOV integer -> SP reg), FMRDH and FMRDL (SIMD scalar ->integer)
      // FMRRS (2 integer-> 2 SP), (FMRRD 2 int-> Doubleword).
      // Note : VMOVD, VMOVDcc, VMOVS, VMOVScc do not transfer data to the
      // integer GPR file and therefore are allowed conditional form.
    case ARM::VMOVDRR:
    ...
    case ARM::VMOVv8i16:
    case ARM::VMOVv8i8:
      // return some value high enough to avoid predication
      return 0xFFFF;
    ...
    // subtargets other than Scorpion/Krait
    return 0;
  }
}
