...
namespace {
  ...
  private:
    ...
    bool RemoveRedundantPICInstr(MachineInstr *MI,
                          SmallVectorImpl<MachineInstr*>& history);
    bool RemoveRedundantPICInstrs(MachineBasicBlock *MBB,
                          SmallVectorImpl<MachineInstr*>& history);
  };
...
bool ARMPreAllocLoadStoreOpt::runOnMachineFunction(MachineFunction &Fn) {
  ...
  SmallVector<MachineInstr*,16> History;
  bool Modified = false;
  for (MachineFunction::iterator MFI = Fn.begin(), E = Fn.end(); MFI != E;
       ++MFI) {
    Modified |= RemoveRedundantPICInstrs(MFI, History);
...
bool ARMPreAllocLoadStoreOpt::RemoveRedundantPICInstr(MachineInstr *MI,
                            SmallVectorImpl<MachineInstr*>& history ) {
  if (MI->getOpcode() != ARM::t2LDRpci_pic) return false;

  MachineBasicBlock *MBB = MI->getParent();
  MachineOperand& resOP = MI->getOperand(0);
  MachineOperand& cpOP = MI->getOperand(1);
  unsigned cpi = cpOP.getIndex();

  // get the GV
  MachineConstantPool* MCP = MF->getConstantPool();
  const MachineConstantPoolEntry CP = MCP->getConstants()[cpi];
  ARMConstantPoolConstant *CPC = (ARMConstantPoolConstant*) CP.Val.MachineCPVal;
  const GlobalValue* GV = CPC->getGV();

  // check if GV is in the history
  // if not there,       add into history ( GV->BB&vreg )
  // if in history,     check if the history BB is dominating this BB
  // if yes,            use the history vreg by converting to copy
  // if not,            add into history

  MachineInstr* baseMI = NULL;
  for (SmallVectorImpl<MachineInstr*>::iterator
        I = history.begin(), E = history.end(); I != E; ++ I) {
     MachineInstr* thisMI = (*I);
     if (thisMI == MI) continue;

     unsigned curCPI = thisMI->getOperand(1).getIndex();
     ARMConstantPoolConstant *thisCPC = (ARMConstantPoolConstant*)
                            MCP->getConstants()[curCPI].Val.MachineCPVal;
     const GlobalValue* thisGV = thisCPC->getGV();

     if (thisGV == GV) {
       // check domination
       MachineBasicBlock* thisMBB = thisMI->getParent();
       if (DT->dominates(thisMBB, MBB)) {
         baseMI = thisMI;
         break;
       }
     }
  ...
  if (baseMI == NULL) {
    ...
    // convert to copy
    MachineOperand& srcOP = baseMI->getOperand(0);
    unsigned srcVR = srcOP.getReg();
    unsigned vreg = resOP.getReg();
    BuildMI(*MBB, MI, MI->getDebugLoc(),
                       TII->get(TargetOpcode::COPY), vreg).addReg(srcVR);
    MBB->erase(MI);
    return true;
  }
...
bool
ARMPreAllocLoadStoreOpt::RemoveRedundantPICInstrs(MachineBasicBlock *MBB,
                                     SmallVectorImpl<MachineInstr*>& history) {
    ...
    for (MachineBasicBlock::iterator
      ...
      MachineInstr *MI = &*MII;
      bool Erased = false;
      Erased = RemoveRedundantPICInstr(MI, history);
