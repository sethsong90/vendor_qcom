...
// This option will enable PIC generation using GOTPREL and label relative
// way. And this option will enable corresponding PIC address redundant
// elimination.
cl::opt<bool>
...
SDValue ARMTargetLowering::LowerGlobalAddressELF(SDValue Op,
  ...
  if (RelocM == Reloc::PIC_) {
    ...
    if (OptimizeGOT) {
      ...
      ARMConstantPoolValue *CPV;
      if (UseGOTOFF)
          CPV = ARMConstantPoolConstant::Create(GV,
                                       ARMPCLabelIndex, ARMCP::CPValue, PCAdj);
      else
          CPV = ARMConstantPoolConstant::Create(GV,
                                       ARMPCLabelIndex, ARMCP::CPValue, PCAdj,
                                                        ARMCP::GOTPREL, true);
      ...
      SDValue PICLabel = DAG.getConstant(ARMPCLabelIndex, MVT::i32);
      SDValue addr = DAG.getNode(ARMISD::PIC_ADD, dl, PtrVT, Result, PICLabel);
      if (!UseGOTOFF) {
        addr = DAG.getLoad(PtrVT, dl, Chain, addr, MachinePointerInfo(),
                           false, false, false, 0);
      }
      return addr;
    }
...
static SDValue PerformANDToUBFXCombine(SDNode *N,
  ...
  SDValue Ubfx = DAG.getNode(ARMISD::UBFX, N->getDebugLoc(), N->getValueType(0),
                             Srl->getOperand(0), Lsb, Width);
  if (LeftShiftWidth)
    Ubfx = DCI.DAG.getNode(ISD::SHL, N->getDebugLoc(), N->getValueType(0),
