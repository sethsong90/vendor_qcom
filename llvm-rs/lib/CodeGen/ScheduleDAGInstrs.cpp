...
cl::opt<bool> EnableAASchedMI("enable-aa-sched-mi", cl::Hidden,
...
void ScheduleDAGInstrs::addPhysRegDeps(SUnit *SU, unsigned OperIdx) {
  ...
  if (!MO.isDef()) {
    ...
    // Do not clear Uses[MO.getReg()] here because we need to keep
    // track of the uses in the current instruction.
...
void ScheduleDAGInstrs::buildSchedGraph(AliasAnalysis *AA) {
  ...
  for (MachineBasicBlock::iterator MII = RegionEnd, MIE = RegionBegin;
    ...
    // Temporary disable this assert for Hexagon.
    // Do not upstream yet.
