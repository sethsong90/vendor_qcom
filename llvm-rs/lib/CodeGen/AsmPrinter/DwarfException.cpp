...
void DwarfException::EmitExceptionTable() {
  ...
  MCSymbol *TTypeBaseBegin =
    Asm->GetTempSymbol("ttype_base_begin", Asm->getFunctionNumber());
  MCSymbol *TTypeBaseEnd =
    Asm->GetTempSymbol("ttype_base_end", Asm->getFunctionNumber());
  ...
  if (HaveTTData) {
    ...
    Asm->EmitLabelDifference(TTypeBaseEnd, TTypeBaseBegin, 16);
    Asm->OutStreamer.EmitLabel(TTypeBaseBegin);
    SizeAlign = 0;
  }
  ...
  if (IsSJLJ) {
    ...
    Asm->EmitEncodingByte(dwarf::DW_EH_PE_uleb128, "Call site");

    // Add extra padding if it wasn't added to the TType base offset.
    //Asm->EmitULEB128(CallSiteTableLength, "Call site table length", SizeAlign);

    MCSymbol *CallSiteBegin =
      Asm->GetTempSymbol("call_site_begin", Asm->getFunctionNumber());
    MCSymbol *CallSiteEnd =
      Asm->GetTempSymbol("call_site_end", Asm->getFunctionNumber());

    Asm->EmitLabelDifference(CallSiteEnd, CallSiteBegin, 16);
    Asm->OutStreamer.EmitLabel(CallSiteBegin);
    ...
    for (SmallVectorImpl<CallSiteEntry>::const_iterator
      ...
      Asm->EmitLabelDifference(BeginLabel, EHFuncBeginSym, 16);
      if (VerboseAsm)
        Asm->OutStreamer.AddComment(Twine("  Call between ") +
                                    BeginLabel->getName() + " and " +
                                    EndLabel->getName());
      Asm->EmitLabelDifference(EndLabel, BeginLabel, 16);
	...
	Asm->EmitULEB128(0);
        ...
        if (VerboseAsm)
          Asm->OutStreamer.AddComment(Twine("    jumps to ") +
                                      S.PadLabel->getName());
        Asm->EmitLabelDifference(S.PadLabel, EHFuncBeginSym, 16);
      }
    ...
    Asm->OutStreamer.EmitLabel(CallSiteEnd);
  }
  ...
  Asm->OutStreamer.EmitLabel(TTypeBaseEnd);
