...
isBlockOnlyReachableByFallthrough(const MachineBasicBlock *MBB) const {
  ...
  for (MachineBasicBlock::iterator II = Pred->getFirstTerminator(),
    ...
    //
    // Bugfix 6681: Temporary bugfix. We need to change the design of bundles
    // so that they accumulate labels from the contained instructions in
    // addition to registers. The design change warrants extensive testing
    // and examination. This is a temporary fix to get around the design flaw.
    //
    if (MI.isBundle()) {
      MachineBasicBlock::instr_iterator BundleIter = *II;
      ++BundleIter;
      for (; BundleIter != Pred->end() && BundleIter->isInsideBundle();
           ++BundleIter) {
        MachineInstr &BundleMI = *BundleIter;
        // Only examine terminator instructions within the bundle
        if (!BundleMI.isTerminator())
          continue;
        // If it is not a simple branch, we are in a table somewhere.
        if (!BundleMI.isBranch() || BundleMI.isIndirectBranch())
          return false;
        // If we are the operands of one of the branches, this is not
        // a fall through.
        for (MachineInstr::mop_iterator OI = BundleMI.operands_begin(),
               OE = BundleMI.operands_end(); OI != OE; ++OI) {
