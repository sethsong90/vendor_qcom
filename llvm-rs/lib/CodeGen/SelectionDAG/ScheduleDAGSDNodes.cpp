...
static cl::opt<bool> DisableAASched("disable-aa-sched", cl::Hidden,
    cl::ZeroOrMore, cl::init(false),
    cl::desc("Disable use of AA during scheduling"));
...
/// - The two SUs could be analized as memory refs
/// - It could be positively proven that they do not alias each other
/// - or if Positive == false, that it could be proven that they do
///   alias each other.
bool SUsAlias (SelectionDAG *DAG,
    SUnit *SU1, SUnit *SU2, AliasAnalysis *AA, bool Positive=true) {
  ...
  if (SDN1->isMachineOpcode() && SDN2->isMachineOpcode()) {
    ...
    if (MMOa && MMOa->getValue() && MMOb && MMOb->getValue()) {
      ...
      // Only two volatiles should not bypass each other
      // See http://llvm.org/docs/LangRef.html#volatile
      if (MMOa->isVolatile() && MMOb->isVolatile()) {
        if (Positive) return false;
        else return true;
      }

      // TODO: Check if these are two load insn
      ...
      AliasAnalysis::AliasResult AAResult = AA->alias(
            ...
            AliasAnalysis::Location(MMOb->getValue(), Overlapb,
                                    MMOb->getTBAAInfo()));
      if (Positive && (AAResult == AliasAnalysis::NoAlias))
        return true;
      else if (!Positive && (AAResult != AliasAnalysis::NoAlias))
        return true;
    }
  }
  return false;
...
SUnit *FollowChainDep (SelectionDAG *DAG,
                       SUnit *SU, AliasAnalysis *AA,
                       const TargetInstrInfo *TII, SUnit *MMO,
                       std::vector<SUnit *> &DepList) {
  SUnit *SURes = NULL;

  if (!SU || !MMO)   return NULL;
  // If it is already lowered to MI for some reason,
  // we should not reason aboit it here
  if (SU->isInstr() || MMO->isInstr())  return NULL;
  // Already has an edge
  if (SU->isPred(MMO))  return NULL;
  // If this is a mem SU, look at aliasing
  // if the two mem locations could alias,
  // stop traversing.
  if (SUsAlias(DAG,SU,MMO,AA,false))   return SU;

  // If this is a call or any other barier, return this.
  for (SDNode *N = SU->getNode(); N; N = N->getGluedNode()) {
    if (N->isMachineOpcode()) {
      const MCInstrDesc &TID = TII->get(N->getMachineOpcode());
      if (TID.isCall() || TID.isBarrier() || TID.isTerminator())
        return SU;
    } else {
      switch(N->getOpcode()){
        default:    break;
      case ISD::TokenFactor:
      // TODO: Look at the clobber list
      case ISD::INLINEASM:
        return SU;
      }
    }
  }
  // Once SU in question is found, follow up the chain link
  // untill another mem ref that can potentially
  // alias this one or call or another barier is seen.
  // TODO: Sparser version of this traversal is possible
  for (SUnit::pred_iterator I = SU->Preds.begin(), E = SU->Preds.end();
       I != E; ++I) {
    if (I->isCtrl()) {
      SURes = FollowChainDep (DAG,I->getSUnit(), AA, TII, MMO, DepList);
      // If it is a valid node to update, add it to the update list.
      if (SURes) {
        bool found = false;
        for (unsigned ii=0; ii < DepList.size(); ii++)
            if (SURes == DepList[ii]) {
              found = true;
              break;
            }
        if (!found)  DepList.push_back(SURes);
      }
    }
  }
  return SURes;
...
void ScheduleDAGSDNodes::AddSchedEdges(AliasAnalysis *AA) {
  const TargetSubtargetInfo &ST = TM.getSubtarget<TargetSubtargetInfo>();

  // Check to see if the scheduler cares about latencies.
  bool UnitLatencies = forceUnitLatencies();
  std::vector<SUnit *> Mem_SUnits;
  ...
  for (unsigned su = 0, e = SUnits.size(); su != e; ++su) {
    ...
    for (SDNode *N = SU->getNode(); N; N = N->getGluedNode()) {
      ...
      for (unsigned i = 0, e = N->getNumOperands(); i != e; ++i) {
        ...
        // It seems convinient to mark all candidates at this time
        // to reduce search time later.
        // Tradeoff - repetitive call to AA
        if (isChain && !DisableAASched
           && SUsAlias(DAG,SU,OpSU,AA))
          Mem_SUnits.push_back(SU);
      }
    }
  }

  if (DisableAASched)    return;

  // Now, as a post pass go through marked edges and
  // remove them
  for (unsigned sd = 0, ed = Mem_SUnits.size(); sd != ed; ++sd) {
    SUnit *SU = Mem_SUnits[sd];

    // Do not delete edges leading to the root node
    // It might break some schedulers
    if(SU == &SUnits[DAG->getRoot().getNode()->getNodeId()])    continue;
    for (SUnit::pred_iterator I = SU->Preds.begin(), E = SU->Preds.end();
         I != E; ++I) {
      if (!I->isCtrl()) continue;
      SUnit *SUcand = I->getSUnit();
      if (SUsAlias(DAG,SU,SUcand,AA)) {
        DEBUG(dbgs() << "Can ignore order between these nodes: \n");
        DEBUG(dumpNode(SU));
        DEBUG(dumpNode(SUcand));
        // List of SUs to update
        std::vector<SUnit *> DepList;
        SUnit *SingleAliasingPredecessor = FollowChainDep(DAG,SUcand,AA,TII,
                                                  SU,DepList);
        // TODO: Make this more elegant.
        if (SingleAliasingPredecessor) {
          bool found = false;
          for ( unsigned ii=0; ii < DepList.size(); ii++)
            if (SingleAliasingPredecessor == DepList[ii]) {
                found = true;
                break;
            }
          if (!found) DepList.push_back(SingleAliasingPredecessor);
        }
        //Remove this dependency
        SU->removePred(*I);
        // Add new deps to dominators in order
        // to preserve transient dependency chain
        for (unsigned ii=0; ii < DepList.size(); ii++)
          SU->addPred(SDep(DepList[ii], SDep::Order,1, 0));
        // Now add sucessor dependencies
        // TODO: See if this could be reduced
        for (SUnit::succ_iterator I = SU->Succs.begin(), E = SU->Succs.end();
             I != E; ++I) {
          // Check if similar dep already exist.
          // if (SU->isSucc(MMO))  return NULL;
          if (I->isCtrl())
            I->getSUnit()->addPred(SDep(SUcand, SDep::Order,1, 0));
        }
      }
    }
  }
