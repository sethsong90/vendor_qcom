...
static cl::opt<bool> EnablePhiSpeculation("enable-basicaa-phi-speculation",
...
BasicAliasAnalysis::aliasGEP(const GEPOperator *GEP1, uint64_t V1Size,
  ...
  if (const GEPOperator *GEP2 = dyn_cast<GEPOperator>(V2)) {
    ...
    if (EnablePhiSpeculation && V1Size == V2Size) {
...
BasicAliasAnalysis::aliasPHI(const PHINode *PN, uint64_t PNSize,
  ...
  if (const PHINode *PN2 = dyn_cast<PHINode>(V2))
    if (PN2->getParent() == PN->getParent()) {
      ...
      bool ArePhisAssumedCongruent = false;
      AliasResult OrigAliasResult;
      if  (EnablePhiSpeculation && Alias == NoAlias) {
        // Assume that phis are congruent.
        // Pretend the phis do not alias and see what happens.
        assert(AliasCache.count(Locs));
        OrigAliasResult = AliasCache[Locs];
        AliasCache[Locs] = NoAlias;
        ArePhisAssumedCongruent = true;
      }
      ...
      if (EnablePhiSpeculation && Alias != NoAlias && ArePhisAssumedCongruent) {
