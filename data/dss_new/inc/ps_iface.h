...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/api/datamodem/main/latest/ps_iface.h#4 $
  $DateTime: 2011/07/21 11:09:53 $
...
/*===========================================================================
TYPEDEF PS_IFACE_STAT_OVERRIDE_F_PTR_TYPE()

DESCRIPTION
  This function is used by Mode Handlers that wants to implement its own
  statistics. MH's register this function and return the statistics values
  that are used to override any statistics that are currently stored in the
  PS iface for that mode handler.

PARAMETERS
  iface_ptr     : Pointer to the iface control block
  override_stats: Pointer to the result containing the new stats
  op_type       : Operation Type enum (GET or RESET)
...
SIDE EFFECTS
  None
===========================================================================*/
typedef void (*ps_iface_stat_override_f_ptr_type)
(
  ps_iface_type                       *iface_ptr,
  ps_iface_stat_override_type         *override_stats,
...
struct ps_iface_s
{
  ...
  struct
  {
    ...
    uint32         sock_ref_cnt;      /* number of sockets using the iface */
  ...
  /*-------------------------------------------------------------------------
    ...
    - total_open_time_since_power_up
                                : Amount of time that the iface has been up
                                  since power up, in seconds.
    - total_active_time_since_power_up
                                : Amount of time that the iface has been up
                                  and also had previous I/O, since power up,
                                  in seconds.
                                  ( total_open_time_since_power_up -
                                    amount of time that
                                    iface has been up with no I/O)
    - total_bytes_transferred_since_power_up
                                : Total bytes transferred via this iface
                                  from power up
    - bytes_transferred         : Bytes transferred via this iface
                                  since last iface up state transition
    - last_up_state_timestamp   : Time stamp of the last transition
                                  to UP state. In milliseconds.
    - first_io_timestamp        : Time stamp of the first I/O
                                  since last up state. In milliseconds.
  -------------------------------------------------------------------------*/
  ...
  struct iface_stats
  {
    uint64  bytes_rx;
    uint64  bytes_tx;
    uint64  total_bytes_transferred_since_power_up;
    ...
    uint32  queried;
    uint32  total_open_time_since_power_up;
    uint32  total_active_time_since_power_up;
    uint32  last_up_state_timestamp;
    uint32  first_io_timestamp;
  } iface_i_stats;
  ...
  /*-------------------------------------------------------------------------
    Function called to get stats from mode handler that can override the
    stats saved in PS Iface.
  -------------------------------------------------------------------------*/
  ps_iface_stat_override_f_ptr_type ps_iface_stat_override_f_ptr;
...
INLINE void ps_iface_set_is_active
(
  ps_iface_type *iface_ptr,
  boolean        is_active
)
...
RETURN VALUE
  ...
  FALSE : otherwise
===========================================================================*/
#define PS_IFACE_COMMON_IS_FLOW_ENABLED(iface_ptr, flow_ptr)             \
  ((PS_IFACE_IS_VALID(iface_ptr) && PS_FLOW_IS_VALID(flow_ptr))          \
      ? PS_IFACEI_COMMON_IS_FLOW_ENABLED(iface_ptr, flow_ptr)            \
      : 0)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (FALSE == PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    return FALSE;
  }
  ...
  return (PS_IFACE_FLOW_ENABLED(ps_iface_ptr) &&
...
SIDE EFFECTS
  None
===========================================================================*/
#define PS_IFACE_SET_V4_NET_MASK( iface_ptr, net_mask )                  \
  if (PS_IFACE_IS_VALID(iface_ptr))                                      \
  {                                                                      \
    (iface_ptr)->v4_net_info.net_mask.ps_s_addr  = net_mask;             \
  }
...
PARAMETERS
  this_iface_ptr : ptr to interface control block on which to operate on.
  dhcp6_client_handle : handle to set
