...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ps_handle_mgr.c#2 $
  $DateTime: 2011/07/20 10:24:09 $ $Author: brijeshd $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid ID %d", client_id, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid input, # items %d, min_base %d, max_possible %d",
  ...
  {
    LOG_MSG_ERROR("Client %d is inited already", client_id, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  if (max_possible_handle != 0)
  {
    ...
    {
      LOG_MSG_INVALID_INPUT("max possible handle %d is not large enough",
                            max_possible_handle, 0, 0);
      ASSERT(0);
      return;
    }
  }
  ...
  {
    LOG_MSG_ERROR("Max Base handle + # of items > pow(2, 31) - 1", 0, 0, 0);
    ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (index < 0)
  {
    LOG_MSG_ERROR("Invalid index %d, client %d", index, client_id, 0);
    ASSERT(0);
    return PS_HANDLE_MGR_INVALID_HANDLE;
  }
  ...
  if (!( client_info_ptr->is_inited))
  {
    LOG_MSG_ERROR("Client %d is not inited", client_id, 0, 0);
    ASSERT(0);
    return PS_HANDLE_MGR_INVALID_HANDLE;
  }
       ...
       client_info_ptr->max_handle_used_so_far )
  {
    ...
    {
      LOG_MSG_INFO2("Changing handle base of client %d from %d to %d",
                     client_id,
