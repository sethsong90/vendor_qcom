...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ds_Utils_Conversion.cpp#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (psErrno)
  {
    ...
    default:
    {
      LOG_MSG_ERROR( "Unknown ps err %d", psErrno, 0, 0);
      return AEE_EFAILED;
    }
  }
} /* PSErrnoGetDSErrno() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == pNumItems)
  {
    LOG_MSG_ERROR( "ProcessLenReq(): pNumItems ptr is NULL", 0, 0, 0);
    ASSERT(0);
    return QDS_EFAULT;
  }
  if (bufLen < 0)
...
(
  ...
  memcpy(DSS_CMSG_DATA(cmsg), data, cmlen - sizeof(struct dss_cmsghdr));
