...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ps_stat_logging.c#1 $
  $Author: zhasan $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("QXDM called GET_INST_DESC", 0, 0, 0);
  ...
  {
    LOG_MSG_INFO1("Invalid pkt or pkt length", 0, 0, 0);
  ...
  {
    LOG_MSG_INFO1("Invalid command code %d.",
              hget16(request_ptr), 0, 0);
  ...
  if (NULL == desc_ptr)
  {
    LOG_MSG_ERROR("Out of mem for desc", 0, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid cmd pkt size", 0, 0, 0);
    return NULL;
  }
  ...
  for (idx = 0; idx < num_instances; ++idx)
  {
    ...
    {
      LOG_MSG_ERROR ("Out of mem for inst info", 0, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("QXDM called PS_STAT_LOGGINGI_GET_INST_STAT", 0, 0, 0);
  ...
  if (NULL == req_pkt)
  {
    LOG_MSG_INFO1("Invalid request was sent by QXDM", 0, 0, 0);
    return (diagpkt_err_rsp(DIAG_BAD_LEN_F, req_pkt, pkt_len));
  }
  ...
  {
    LOG_MSG_INFO1("Invalid command code %d was sent by QXDM",
              hget16(request_ptr), 0, 0);
    return (diagpkt_err_rsp(DIAG_BAD_CMD_F, req_pkt, pkt_len));
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("QXDM called PS_STAT_LOGGINGI_RESET_INST_STAT", 0, 0, 0);
#ifdef FEATURE_DATA_PS_LOGGING_DEBUG
  memdump ((void*) req_pkt, pkt_len);
#endif
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("PS cmd to get inst descs %d", bitmask, 0, 0);
  ...
  for (idx = 0; idx < (uint32)PS_STAT_MODULE_MAX; ++idx)
  {
    ...
    if (desc_type & 0x00000001)
    {
      ...
      {
        LOG_MSG_INFO1("Callback not registered %d", idx, 0, 0);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Processing GET_INST_STAT", 0, 0, 0);
  ...
  while (NULL != inst_cmd_info_ptr)
  {
    ...
    {
      LOG_MSG_INFO1("Callback not registered %d", inst_cmd_info_ptr->type, 0, 0);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (NULL != inst_info_ptr)
  {
    ...
    {
      LOG_MSG_INFO1("Callback not registered %d", inst_info_ptr->type, 0, 0);
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_FATAL_ERROR("Can't init the stats logging module", 0, 0, 0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Registering fill description callback function for type: %d",
           func_type, 0, 0);

  if (func_type >= PS_STAT_MODULE_MAX)
  {
    LOG_MSG_INFO1("Register fill desc cback with invalid type: %d", func_type, 0, 0);
    return;
  }

  if (NULL == func_ptr)
  {
    LOG_MSG_INFO1("Register fill desc cack with void function pointer", 0, 0, 0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Registering get statistics callback function for type: %d",
           func_type, 0, 0);

  if (func_type >= PS_STAT_MODULE_MAX)
  {
    LOG_MSG_INFO1("Registering get stat cback with invalid type: %d",
              func_type, 0, 0);
    return;
  }
  ...
  if (NULL == func_ptr)
  {
    LOG_MSG_INFO1("Registering get stat cback with void function pointer",
              0, 0, 0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Registering reset statistics callback function for type: %d",
           func_type, 0, 0);

  if (func_type >= PS_STAT_MODULE_MAX)
  {
    LOG_MSG_INFO1("Registering reset stat cback with invalid type: %d",
              func_type, 0, 0);
    return;
  }
  ...
  if (NULL == func_ptr)
  {
    LOG_MSG_INFO1("Register reset stat cback with void function pointer",
              0, 0, 0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Updating inst desc. Type: %d. Handle: %d. Action: %d",
  ...
  {
    LOG_MSG_ERROR("Invalid instance type during description update", 0, 0, 0);
    return;
  }

  if (inst_update >= PS_STAT_INST_UPDATE_MAX)
  {
    LOG_MSG_ERROR("Invalid instance update action", 0, 0, 0);
    return;
  }

  if (NULL == get_desc_log_pkt_cback_arr[inst_type])
  {
    LOG_MSG_ERROR("Update inst desc - fill inst desc cback null", 0, 0, 0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (log_pkt_type)
  {
    ...
    default:
      LOG_MSG_INFO1("Invalid log type passed in to GET_LOG_PKT", 0, 0, 0);
      ASSERT(0);
      break;
  }
