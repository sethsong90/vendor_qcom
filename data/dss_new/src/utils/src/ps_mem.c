...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ps_mem.c#1 $
  $DateTime: 2011/06/17 12:02:33 $Author: $
...
static char *ps_mem_pool_name_arr[] =
{
  ...
  "DSAL_GLOBALS",
  "DSAL_PRIMARY_NET_APP",
  "DSAL_SECONDARY_NET_APP",
  "DSAL_SOCKET",
  ...
  "DSAL_EVENT_HANDLER_EXT_IP_CONFIG",
  "DSAL_EVENT_HANDLER_RATE_INTERIA",
  "DSAL_EVENT_HANDLER_MCAST_MBSCTRL",
  "DSAL_EVENT_HANDLER_BEARER_TECH",
  "DSAL_EVENT_HANDLER_SLOTT_SESS_CHNG",
  "DSAL_EVENT_HANDLER_SLOTT_RES",
  "DSAL_EVENT_HANDLER_RFC_COND",
  "DSAL_EVENT_HANDLER_QOS_PROF_CHNG",
  "DSAL_EVENT_HANDLER_QOS_MODIFY",
  "DSAL_EVENT_HANDLER_QOS_INF_CODE_UPD",
  "DSAL_EVENT_HANDLER_QOS",
  "DSAL_EVENT_HANDLER_QOS_DEL_ON_IFACE",
  "DSAL_EVENT_HANDLER_QOS_AW_UNAW",
  "DSAL_EVENT_HANDLER_QOS_ADD_ON_IFACE",
  "DSAL_EVENT_HANDLER_QOS_PRI_MOD_STAT",
  "DSAL_EVENT_HANDLER_QOS_PRI_MOD",
  "DSAL_EVENT_HANDLER_PHYS_LINK_STATE",
  "DSAL_EVENT_HANDLER_OUTAGE",
  "DSAL_EVENT_HANDLER_NETWORK_STATE",
  "DSAL_EVENT_HANDLER_NETWORK_IP",
  "DSAL_EVENT_HANDLER_MTPD_REQUEST",
  "DSAL_EVENT_HANDLER_MCAST_STATUS",
  "DSAL_EVENT_HANDLER_MCAST_REGISTER",
  "DSAL_EVENT_HANDLER_IP6PRIV_ADDR",
  "DSAL_PRIV_IPV6_ADDR",
  "DSAL_EVENT_HANDLER_IPV6_PREFIX",
...
/*---------------------------------------------------------------------------
  These data structures are used for supporting PS MEM debug APIs         

  ps_mem_debug_add_check_point()
    Create a snapshot of PS mem pool allocations. 

  ps_mem_debug_compare_check_points(chkpt1, chkpt1)
    Returns number of buffer differences between the check points. 

  ps_mem_debug_reset_check_points()
    Resets the check points. 
---------------------------------------------------------------------------*/

typedef struct
{
  int32 alloc_bufs[PS_MEM_MAX_POOLS];
} ps_mem_debug_check_point_type;

#define PS_MEM_DEBUG_MAX_CHECK_POINTS 10
static ps_mem_debug_check_point_type check_point_arr[PS_MEM_DEBUG_MAX_CHECK_POINTS];

/* Check point handle */
static int32 check_point_handle = 0;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Incorrect pool_id %d", pool_id, 0, 0);
    return -1;
  }
  ...
  {
    MEM_INC_INSTANCE_STATS(pool_id, alloc_errors, 1);
    LOG_MSG_ERROR("Incorrect mem levels, num_buf %d, high %d, low %d",
  ...
  {
    MEM_INC_INSTANCE_STATS(pool_id, alloc_errors, 1);
    LOG_MSG_ERROR("Incorrect mem size %d", buf_size, 0, 0);
    return -1;
  }
  ...
  {
    MEM_INC_INSTANCE_STATS(pool_id, alloc_errors, 1);
    LOG_MSG_ERROR("Pool %d is inited already", pool_id, 0, 0);
    return -1;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Incorrect pool_id %d", poolid, 0, 0);
    return NULL;
  }

  if (ps_mem_pool_info[poolid].size == 0)
  {
    LOG_MSG_ERROR("Pool %d not initialized", poolid, 0, 0);
    return NULL;
  }
  ...
  {
    buf_hdr = ps_system_heap_mem_alloc( ps_mem_pool_info[poolid].size +
                                          PS_MEM_BUF_HDR_SIZE );
    if( buf_hdr == NULL )
    {
      LOG_MSG_INFO1("Out of mem, pool %d requested %d",
               poolid, ps_mem_pool_info[poolid].size + PS_MEM_BUF_HDR_SIZE, 0);
  ...
  if ( buffer != NULL )
  {
         ...
         ps_mem_pool_info[poolid].num_used )
    {
      ...
      LOG_MSG_INFO1("PS mem pool id %d exceed high wm %d",
               poolid, ps_mem_pool_info[poolid].hi_watermark, 0);
    }
  ...
  else
  {
    ...
    PS_BRANCH_LEAVE_CRIT_SECTION(&ps_mem_crit_section);
    LOG_MSG_FATAL_ERROR("Out of memory in pool %d", poolid, 0, 0);
    if( ps_mem_pool_info[poolid].is_static == FALSE )
    {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid pool %d", poolid, 0, 0);
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR("Pool %d doesnt match pool %d in buf_hdr",
              poolid, buf_hdr->poolid, 0);
    return FALSE;
  }
  ...
  if( ps_mem_pool_info[poolid].is_static == TRUE )
  {
    ...
    {
      LOG_MSG_ERROR("Buf 0x%p is not of pool %d", buf_hdr, poolid, 0);
      return FALSE;
    }
    ...
    {
      LOG_MSG_ERROR("Invalid addr: buffer 0x%p pool %d", buf_hdr, poolid, 0);
      return FALSE;
    }
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  } /* static */
  else
  {
    ...
    {
      LOG_MSG_ERROR ("Invalid index %d, pool id %d", index, poolid, 0);
      return FALSE;
    }
    ...
    if ((int *) ps_mem_pool_info[poolid].first_element[index] !=
          (int *) buf_hdr)
    {
      LOG_MSG_ERROR("Invalid dynamic addr: buffer 0x%p pool %d index %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (buf == NULL)
  {
    LOG_MSG_ERROR("Null buffer", 0, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid buf 0x%p", buf, 0, 0);
    return;
  }
  ...
  *buffer = (int) ps_mem_pool_info[poolid].free_list;
  ...
  {
    LOG_MSG_INFO1("Calling low wm callback for pool %d", poolid, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if(poolid >= PS_MEM_MAX_POOLS)
  {
    LOG_MSG_ERROR("Pool ID %d meets or exceeds MAX POOLS %d",
              (uint32)poolid,(uint32)PS_MEM_MAX_POOLS,0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(poolid >= PS_MEM_MAX_POOLS)
  {
    LOG_MSG_ERROR("ps_mem_index_to_buf(): Invalid poolid %d", poolid, 0, 0);
    ASSERT(0);
    return NULL;
  }
  ...
  if (index < 0 || index >= ps_mem_pool_info[poolid].total_num)
  {
    LOG_MSG_INFO1("Index (%d) out of range for pool %d", index, poolid, 0);
    return NULL;
  }
  ...
  if( ps_mem_pool_info[poolid].is_static == TRUE )
  {
    ...
    {
      LOG_MSG_ERROR("Buf of index %d is not allocated, pool %d",
                index, poolid, 0);
      return NULL;
    }
  ...
  else
  {
    ...
    if (buffer == NULL)
    {
      LOG_MSG_ERROR("Dynamic home entry is NULL for index %d, pool %d",
                index, poolid, 0);
      return NULL;
    }
    ...
    {
      LOG_MSG_ERROR("Dynamic buffer's index %d does not match home entry %d",
                ((ps_mem_buf_hdr_type *)buffer)->index, index,0 );
      return NULL;
    }
  } /* dynamic */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Invalid buf", 0, 0, 0);
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Index range error: Buffer header 0x%p : index %d : Max %d ",
              buf_hdr, buf_hdr->index, ps_mem_pool_info[poolid].total_num - 1);
        return -1;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Unsupported ext pool_id %d", poolid, 0, 0);
    return NULL;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Invalid stat value passed to ps_mem_get_stat_i_all",0,0,0);
    ASSERT(0);
  ...
  {
    LOG_MSG_ERROR("ps_mem_get_stat_instance_all:Null value of pool_info_ptr passed",
              0,0,0);
    ASSERT(0);
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  mem_desc_ptr->name_size = (uint8)(strlen(ps_mem_pool_name_arr[handle]) + 1);

  log_pkt_fill_ptr += sizeof(ps_stat_inst_desc_mem_type);

  (void)std_strlcpy((void *)log_pkt_fill_ptr,
              ps_mem_pool_name_arr[handle],
              mem_desc_ptr->name_size);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx = (PS_MEM_DUMMY_POOL_TYPE + 1); idx < PS_MEM_MAX_POOLS; ++idx)
    {
      ...
      ps_mem_inst_fill_one_desc(idx, log_pkt_fill_ptr);
  ...
  else
  {
    ...
    ps_mem_inst_fill_one_desc(handle, log_pkt_fill_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    ...
    for (idx = (PS_MEM_DUMMY_POOL_TYPE + 1); idx < PS_MEM_MAX_POOLS; ++idx)
    {
      ...
      if (NULL == log_pkt_ptr)
      {
        LOG_MSG_ERROR("Log packet allocated for MEM failed", 0, 0, 0);
        return;
      }
  ...
  else
  {
    ...
    {
      LOG_MSG_ERROR("Invalid instance getting mem stats %d", handle, 0, 0);
      return;
    }
    /*---------------------------------------------------------------------
      Convert the handle into the instance control block
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("ps_mem_pool_free(): Invalid poolid %d", poolid, 0, 0);
    ASSERT(0);
    return;
  }
...
int32 ps_mem_debug_add_check_point
(
  void 
)
{
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_MEM_DEBUG_MAX_CHECK_POINTS >= check_point_handle)
  {
    LOG_MSG_ERROR("Max check points %d allocated already", 
                   PS_MEM_DEBUG_MAX_CHECK_POINTS, 0, 0);
    return -1;
  }

  for (index = 0; index < (int)PS_MEM_MAX_POOLS; index++)
  {
    check_point_arr[check_point_handle].alloc_bufs[index] = 
      ps_mem_pool_info[index].num_used;
  }

  return check_point_handle++;

} /* ps_mem_debug_add_check_point() */

int32 ps_mem_debug_compare_check_points
(
  int32 check_point1, 
  int32 check_point2 
)
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (check_point_handle <= check_point1 ||
      check_point_handle <= check_point2)
  {
    LOG_MSG_ERROR ("Unassigned check point handle passed, %d, %d", 
  ...
  {
    result += 
      (check_point_arr[check_point1].alloc_bufs[index] -
       check_point_arr[check_point2].alloc_bufs[index]);
  }
  ...
  if (0 != result)
  {
    ...
    for (index = 0; index < (int)PS_MEM_MAX_POOLS; index++)
    {
      if(check_point_arr[check_point1].alloc_bufs[index] !=
         check_point_arr[check_point2].alloc_bufs[index])
      {
        LOG_MSG_INFO1("Pool %d, chk point 1 %d, chk point 2 %d",
                      index, 
                      check_point_arr[check_point1].alloc_bufs[index], 
                      check_point_arr[check_point2].alloc_bufs[index]);
      }
...
} /* ps_mem_debug_compare_check_points() */


void ps_mem_debug_reset_check_points
(
  void 
)
{
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  memset (&check_point_arr, 0, sizeof(check_point_arr));
  check_point_handle = 0;
