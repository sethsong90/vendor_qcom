...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ps_utils_aka.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
/*---------------------------------------------------------------------------
  EFS config & NV item files
---------------------------------------------------------------------------*/
#define PS_UTILS_AKA_CONF_FILE_NAME    "/nv/item_files/conf/ps_config_info.conf"
#define PS_UTILS_AKA_NV_SW_FILE_NAME   "/nv/item_files/datacommon/utils/aka_in_sw"
#define PS_UTILS_AKA_NV_AUTH_FILE_NAME "/nv/item_files/datacommon/utils/aka_auth_info"
...
/*===========================================================================
FUNCTION PS_UTILS_AKA_CREATE_EFS_ITEM_FILE_PATH()

DESCRIPTION  Adds the EFS item file paths into the PS EFS config file
...
SIDE EFFECTS  None
===========================================================================*/
void ps_utils_aka_create_efs_item_file_path
(
  void
)
{
  char conf_file_path[]  = PS_UTILS_AKA_CONF_FILE_NAME;
  char item_file_path1[] = PS_UTILS_AKA_NV_SW_FILE_NAME;
  char item_file_path2[] = PS_UTILS_AKA_NV_AUTH_FILE_NAME;
  int  retval;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/

  /*-----------------------------------------------------------------------
    Put item file paths, into config file.
  -----------------------------------------------------------------------*/    
  retval = ps_create_efs_item_file(conf_file_path,
                                   item_file_path1,
                                   (fs_size_t)sizeof(item_file_path1)-1);
  if ( 0 != retval )
  {
    LOG_MSG_ERROR("Create EFS item file failed", 0, 0, 0);
    return;
  }

  retval = ps_create_efs_item_file(conf_file_path,
                                   item_file_path2,
                                   (fs_size_t)sizeof(item_file_path2)-1);
  if ( 0 != retval )
  {
    LOG_MSG_ERROR("Create EFS item file failed", 0, 0, 0);
    return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("isn_file_path is NULL", 0, 0, 0);
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO2("EFS: Creating ISN file if necessary", 0, 0, 0);
  result = ps_path_is_directory("/datacommon");
  if( 0 != result )
  {
    /* Directory doesn't exist yet */
    LOG_MSG_INFO1("Create /datacommon dir in EFS", 0, 0, 0);
    result = efs_mkdir( "/datacommon", S_IREAD|S_IWRITE|S_IEXEC);
    if ( -1 == result )
    {
  ...
  {
    /* Directory doesn't exist yet */
    LOG_MSG_INFO1("Create /datacommon/utils dir in EFS", 0, 0, 0);
    result = efs_mkdir( "/datacommon/utils", S_IREAD|S_IWRITE|S_IEXEC);
    if ( -1 == result )
    {
      LOG_MSG_ERROR("Create EFS Dir Failed: error %d", efs_errno, 0, 0);
      return efs_errno;
  ...
  /*-------------------------------------------------------------------------
    ...
    to zero bytes.
  -------------------------------------------------------------------------*/  
  config_fd = efs_open (isn_file_path, O_WRONLY|O_CREAT|O_TRUNC, ALLPERMS);
  if ( 0 > config_fd )
  {
...
)
{
  ...
  int result;
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO2("EFS store sequence number", 0, 0, 0);
  if (NULL == seqnum_arr)
  {
    LOG_MSG_ERROR("seqnum_arr is NULL", 0, 0, 0);
    return;
  }

  if(PS_UTILS_AKA_SEQ_NUM_ARRAY_ELEMENTS != seqnum_arr_num_elements)
  {
    LOG_MSG_ERROR("Invalid size %d", seqnum_arr_num_elements, 0, 0);
    return;
  }
  (void)efs_truncate (PS_UTILS_AKA_ISN_EFS_FILE_NAME, 0);
  fd = efs_open (PS_UTILS_AKA_ISN_EFS_FILE_NAME, O_WRONLY);
  
  if(0 > fd) 
  {
    if (ENOENT == efs_errno) /* ISN file does not exist */
    {
      LOG_MSG_ERROR( "EFS: ISN file not present", 0, 0, 0);
      LOG_MSG_INFO1("EFS: Creating ISN file", 0, 0, 0);
      result = ps_utils_aka_create_efs_file(PS_UTILS_AKA_ISN_EFS_FILE_NAME);
      if ( 0 != result )
      {
        LOG_MSG_ERROR("EFS: Error creating ISN file, error %d",
                      efs_errno, 0, 0);
        return;
      }
	  
      fd = efs_open (PS_UTILS_AKA_ISN_EFS_FILE_NAME, O_RDWR|O_APPEND);
      if ( 0 > fd )
      {
        LOG_MSG_ERROR("EFS: Error opening ISN file, error %d",
                       efs_errno, 0, 0);
        return;
      }
    }
    ...
    {
      LOG_MSG_ERROR("Error opening ISN file, error %d",
                    efs_errno, 0, 0);
      return;
    }
  }
  ...
  for(i = 0; i< seqnum_arr_num_elements; i++)
  {
    ...
    (void)std_strlprintf ((char *)&temp_64, 17, "%016llx", seqnum_arr[i]);
    (void)efs_write (fd, (char *)&temp_64, 16);
    /*lint -restore */
    (void)efs_write (fd, (char *)&seperator, 1);
  }
  (void) efs_close (fd);
...
)
{
  ...
  {
    LOG_MSG_ERROR("user_info_ptr is NULL", 0, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid AKA handle", 0, 0, 0);
    return;
  }

  if (NULL == aka_handle_pool[aka_handle].client_cback_ptr)
  {
    LOG_MSG_ERROR("Client callback pointer is NULL", 0, 0, 0);
  ...
  switch ( status )
  {
    ...
    {
      LOG_MSG_INFO1 ("AKA Algo CB - Sync failure", 0, 0, 0);
    ...
    case AKAALGO_SUCCESS:
    {
      ...
      {
        LOG_MSG_ERROR("Invalid RES_LEN %d", create_digest.res_len , 0, 0);
      ...
      {
        LOG_MSG_ERROR("Invalid CK_LEN %d", create_digest.ck_len, 0, 0);
        ps_utils_aka_send_failure_to_client(aka_handle);
        return;
      }
      ...
      {
        LOG_MSG_ERROR("Invalid IK_LEN %d", create_digest.ik_len, 0, 0);
        ps_utils_aka_send_failure_to_client(aka_handle);
        return;
      }
      ...
      {
        memcpy(hmac_md5_output, create_digest.res, create_digest.res_len);
      ...
      else
      {
        ...
        if (FALSE == ret_val)
        {
          LOG_MSG_ERROR("Creating HMAC_MD5 digest failed", 0, 0, 0);
          ps_utils_aka_send_failure_to_client(aka_handle);
          return;
        }
      }
      ...
      LOG_MSG_INFO1("AKA s/w Algo CB - Success", 0, 0, 0);
      aka_handle_pool[aka_handle].client_cback_ptr (aka_handle, 
    ...
    default:
    {
      LOG_MSG_ERROR ("Undefined AKA status %d", status, 0, 0);
      ps_utils_aka_send_failure_to_client(aka_handle);
      return;
    }
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Input parameters are NULL", 0, 0, 0);
    return aka_handle;
  }
  ...
  if ( 0 != retval )
  {
    LOG_MSG_ERROR("AKA in s/w or on card unknown, NV item not set. "
  ...
  if (TRUE == aka_in_sw)
  {
    ...
    if (0 != retval)
    {
      LOG_MSG_ERROR("Cannot read OP & K values from EFS", 0, 0, 0);
      PS_UTILS_AKA_FREE_HANDLE(&aka_handle_pool[aka_handle]);
      return PS_UTILS_AKA_INVALID_HANDLE;
    }
    ...
    if (ps_utils_aka_read_isn_from_efs(&aka_isn_info) == (sint15)E_SUCCESS)
    {
        ...
        {
          LOG_MSG_INFO2("Initial sequence number from EFS success", 0, 0, 0);
        }
    }
    else
    {
      LOG_MSG_ERROR("Reading initial sequence number from EFS failed"
    ...
    LOG_MSG_INFO1("Switching Task Ctx to make the s/w call async", 0, 0, 0);
  ...
  {
#ifdef FEATURE_MMGSDI_SESSION_LIB
    LOG_MSG_INFO1("Running AKA Algo on card", 0, 0, 0);
    if (FALSE == ps_utils_aka_run_algo_on_card(aka_handle, aka_rand, 
                                               rand_len, autn, autn_len))
    {
      LOG_MSG_ERROR("Run AKA algo on card failed, freeing the AKA handle", 
                    0, 0, 0);
      PS_UTILS_AKA_FREE_HANDLE(&aka_handle_pool[aka_handle]);
      return PS_UTILS_AKA_INVALID_HANDLE;
    }
...
)
{
  ...
  if(E_SUCCESS != status)
  {
    LOG_MSG_ERROR("Failed to create hash handle", 0, 0, 0);
    return status;
  }
  ...
  if(E_SUCCESS != status)
  {
    LOG_MSG_ERROR("Unable to use ARM platform", 0 , 0, 0);
    return FALSE;  
  }
  ...
  if ( E_SUCCESS != status )
  {
    LOG_MSG_ERROR("secapi_hsh_create_hmac() Failed", 0 , 0, 0);
    return FALSE;  
  }
  ...
  if ( E_SUCCESS != status )
  {
    LOG_MSG_ERROR("Failed to delte hash handle", 0 , 0, 0);
    return FALSE;  
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Invlid RES %d, CK %d or IK %d length", 
  ...
  if (NULL == key)
  {
    LOG_MSG_ERROR("PS sytem heap alloc() failed", 0, 0, 0);
    return FALSE;
  }
  ...
  if (FALSE == ret_val)
  {
    LOG_MSG_ERROR("ps_utils_aka_create_hmac() failed", 0, 0, 0);
...
)
{
  ...
  if (NULL == aka_handle_pool[aka_handle].client_cback_ptr)
  {
    LOG_MSG_ERROR("Client callback ptr is NULL", 0, 0, 0);
    return;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR("Invalid command posted", 0, 0, 0);
    return;
  }
  ...
  switch(sub_cmd)
  {
    ...
    case PS_UTILS_AKA_MMGSDI_CARD_INS_EVT:
      ...
      {
        LOG_MSG_ERROR("mmgsdi_get_all_available_apps() failed, error %d ", 
                      ret_val, 0, 0);
        return;
      }
      break;
    ...
    case PS_UTILS_AKA_MMGSDI_SESSION_REG:
      ...
      {
        LOG_MSG_INFO2("Trying to re-open session for ISIM", 0, 0, 0);
        ...
        {
          LOG_MSG_ERROR("ISIM not present, session re-open failed", 0, 0, 0);
        }
    ...
    default:
      LOG_MSG_ERROR("Invalid sub command during task switch, cmd %d ", 
...
)
{
  ...
  {
    LOG_MSG_ERROR("client_data_ptr is NULL", 0, 0, 0);
    return;
  }
  ...
  if(MMGSDI_AUTS_FAIL == auth_info_ptr->response_header.mmgsdi_status)
  {
    ...
    {
      LOG_MSG_INFO1("ISIM Algo CB - sync failure", 0, 0, 0);
      ...
      memcpy (aka_data_ptr + 1, auth_info_ptr->auts, auth_info_ptr->auts_len);
    ...
    {
      LOG_MSG_ERROR("Invalid AUTS", 0, 0, 0);
      ps_utils_aka_send_failure_to_client (aka_handle);
      return;
    }
  }
  ...
  if(MMGSDI_SUCCESS == auth_info_ptr->response_header.mmgsdi_status)
  {
    /*---------------------------------------------------------------------
      ...
      to TS 31.102 7.1.2
    -----------------------------------------------------------------------*/
    if (auth_info_ptr->res_len > PS_UTILS_AKA_MAX_RES
        || auth_info_ptr->ck_len > PS_UTILS_AKA_CK_LEN
        || auth_info_ptr->ik_len > PS_UTILS_AKA_IK_LEN)
    {
      LOG_MSG_ERROR("Invalid auth values, RES_LEN %d, CK_LEN %d, IK_LEN %d", 
    ...
    if (PS_UTILS_AKA_V1 == aka_handle_pool_ptr->aka_ver)
    {
      memcpy(hmac_md5_output, auth_info_ptr->res, auth_info_ptr->res_len);
    ...
    aka_data_len = sizeof(auth_info_ptr->res_len) + auth_info_ptr->res_len 
                   + sizeof(auth_info_ptr->ck_len) + auth_info_ptr->ck_len
                   + sizeof(auth_info_ptr->ik_len) + auth_info_ptr->ik_len;
    ...
    -----------------------------------------------------------------------*/
    offset = 0;
    aka_data_ptr[offset] = auth_info_ptr->res_len;
    offset += sizeof(auth_info_ptr->res_len);

    memcpy (aka_data_ptr + offset, auth_info_ptr->res, 
            auth_info_ptr->res_len);
    offset += auth_info_ptr->res_len;
    
    aka_data_ptr[offset] = auth_info_ptr->ck_len;
    offset += sizeof(auth_info_ptr->ck_len);

    memcpy (aka_data_ptr + offset, auth_info_ptr->ck, 
            auth_info_ptr->ck_len);
    offset += auth_info_ptr->ck_len;

    aka_data_ptr[offset] = auth_info_ptr->ik_len;
    offset += sizeof(auth_info_ptr->ik_len);

    memcpy (aka_data_ptr + offset, auth_info_ptr->ik, 
            auth_info_ptr->ik_len);
    offset += auth_info_ptr->ik_len;

    LOG_MSG_INFO2("ISIM Algo CB - Success", 0, 0, 0);
    aka_handle_pool_ptr->client_cback_ptr (aka_handle, 
               (ds_auth_aka_status_enum_type) PS_UTILS_AKA_SUCCESS,
               hmac_md5_output,
               hmac_md5_len,
  ...
  -----------------------------------------------------------------------*/
  LOG_MSG_ERROR("ISIM Algo CB - failed, error %d", 
...
)
{
  ...
  if ( NULL == cnf_ptr ) 
  {
    LOG_MSG_ERROR("cnf_ptr NULL", 0, 0, 0);
    return;
  }

  if( MMGSDI_SUCCESS != status )
  {
    LOG_MSG_ERROR("MMGSDI AKA ISIM cb, failure status %d", status, 0, 0);
  }

  if( MMGSDI_ISIM_AUTH_CNF != cnf )
  {
    LOG_MSG_ERROR("Improper command confirmation, cnf %d", cnf, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Client data is NULL", 0, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid auth info, RES_LEN %d, CK_LEN %d, IK_LEN %d",
                  isim_auth_cnf->res.data_len, 
                  isim_auth_cnf->ck.data_len, 
                  isim_auth_cnf->ik.data_len);
    LOG_MSG_ERROR("AUTS_LEN %d", isim_auth_cnf->auts.data_len, 0, 0);
    return;
  }

  memcpy ( &aka_cb.mmgsdi_info.auth_info.isim.response_header, 
           &isim_auth_cnf->response_header, 
           sizeof(mmgsdi_response_header_type) );

  if (NULL != isim_auth_cnf->res.data_ptr && 0 != isim_auth_cnf->res.data_len)
  {
    memcpy (aka_cb.mmgsdi_info.auth_info.isim.res,
  ...
  if ((NULL != isim_auth_cnf->ck.data_ptr) && (0 != isim_auth_cnf->ck.data_len))
  {
    memcpy (aka_cb.mmgsdi_info.auth_info.isim.ck,
  ...
  if ((NULL != isim_auth_cnf->ik.data_ptr) && (0 != isim_auth_cnf->ik.data_len))
  {
    memcpy (aka_cb.mmgsdi_info.auth_info.isim.ik,
  ...
  if ((NULL != isim_auth_cnf->auts.data_ptr) 
       && (0 != isim_auth_cnf->auts.data_len))
  {
    memcpy (aka_cb.mmgsdi_info.auth_info.isim.auts,
  ...
  LOG_MSG_INFO1("MMGSDI ISIM Algo CB - Switching Task Ctx", 0, 0, 0);
...
)
{
  ...
  {
    LOG_MSG_ERROR("MMGSDI returned error, %d", mmgsdi_status, 0, 0);
    ps_utils_aka_send_failure_to_client (aka_handle);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("auth_data_ptr is NULL or auth_data_len is 0", 0, 0, 0);
    ps_utils_aka_send_failure_to_client (aka_handle);
    return;
  }
  ...
  if(PS_UTILS_AKA_AUTH_SYNC_FAILURE == *auth_data_ptr)
  {
    ...
    {
      LOG_MSG_ERROR("USIM Algo CB - sync failure", 0, 0, 0);
  ...
  if(PS_UTILS_AKA_AUTH_3G_UMTS_SUCCESS == *(auth_data_ptr))
  {
    ...
    {
      LOG_MSG_ERROR("Invalid 3G RES length (%d)", res_len , 0, 0) ;
      PS_SYSTEM_HEAP_MEM_FREE (auth_data_ptr);
      ps_utils_aka_send_failure_to_client (aka_handle);
      return;
    }
    ...
    {
      LOG_MSG_ERROR("Invalid 3G CK length (%d)", ck_len , 0, 0) ;
      PS_SYSTEM_HEAP_MEM_FREE (auth_data_ptr);
      ps_utils_aka_send_failure_to_client (aka_handle);
      return;
    }
    ...
    {
      LOG_MSG_ERROR("Invalid 3G IK length (%d)", ik_len , 0, 0) ;
      PS_SYSTEM_HEAP_MEM_FREE (auth_data_ptr);
      ps_utils_aka_send_failure_to_client (aka_handle);
      return;
    }
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_ERROR("USIM Algo CB - Success", 0, 0, 0);
    aka_handle_pool_ptr->client_cback_ptr (aka_handle, 
               (ds_auth_aka_status_enum_type) PS_UTILS_AKA_SUCCESS,
               hmac_md5_output,
               hmac_md5_len,
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_ERROR("USIM Algo CB - Failure = 0x%x", 
...
)
{
  ...
  if( MMGSDI_SUCCESS != status )
  {
    LOG_MSG_ERROR("MMGSDI AKA USIM cb, failure status %d", status, 0, 0);
  }
  ...
  -----------------------------------------------------------------------*/
  memcpy ( &aka_cb.mmgsdi_info.auth_info.usim,
           usim_auth_cnf,
      ...
      && (MMGSDI_SUCCESS == response_header->mmgsdi_status)))
  {
    ...
    memcpy (aka_cb.mmgsdi_info.auth_info.usim.auth_response.data_ptr,
            usim_auth_cnf->auth_response.data_ptr,
            usim_auth_cnf->auth_response.data_len);
  }

  LOG_MSG_INFO1("MMGSDI USIM Algo CB - Switching Task Ctx", 0, 0, 0);
...
)
{
  ...
  if ((NULL == aka_rand) || (NULL == autn)
      ...
      || (autn_len > PS_UTILS_AKA_MAX_AUTN_LEN))
  {
    LOG_MSG_ERROR("Invalid RAND or AUTN parameters", 0, 0, 0);
    return FALSE;
  }
  ...
  memcpy(databuf, &rand_len, sizeof(uint8));
  offset += sizeof(uint8);

  memcpy(databuf + offset, aka_rand, rand_len);
  offset += rand_len;

  memcpy(databuf + offset, &autn_len, sizeof(uint8));
  offset += sizeof(uint8);

  memcpy(databuf + offset, autn, autn_len);
  ...
  {
    LOG_MSG_INFO1("Running MMGSDI ISIM AKA Algo", 0, 0, 0);
    retval = mmgsdi_session_isim_authenticate(
           ...
           && (TRUE == aka_cb.mmgsdi_info.session_info.session_active))
  {
    LOG_MSG_INFO1("Running MMGSDI USIM AKA Algo", 0, 0, 0);
  ...
  {
    LOG_MSG_ERROR("Neither ISIM nor USIM is present", 0, 0, 0);
    return FALSE;
  }
  ...
  {
    LOG_MSG_ERROR("mmgsdi session authenticate() returned error = %d", 
                  retval, 0, 0);
    return FALSE;
  }
...
)
{
  ...
  if( NULL == cnf_ptr )
  {
    LOG_MSG_ERROR("cnf_ptr is NULL", 0, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("MMGSDI-CLIENT ID Reg failure status %d "
  ...
  if(TRUE == isim_present)
  {
    ...
    {
      LOG_MSG_ERROR("Invalid APP data", 0, 0, 0);
      return;
    }

    memcpy(aka_cb.mmgsdi_info.session_info.aid.data_ptr,
           all_apps_info_ptr->card1_app_info[app_cnt].app_data.aid.data_ptr,
           aka_cb.mmgsdi_info.session_info.aid.data_len);
...
)
{
  ...
  if( NULL == cnf_ptr )
  {
    LOG_MSG_ERROR("MMGSDI session open failure as cnf_ptr is NULL", 0, 0, 0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Session open cb failure status %d and mmgsdi_status %d ", 
...
)
{
  ...
  if ( NULL == event )
  {
    LOG_MSG_ERROR("Input parameter is NULL", 0, 0, 0);
    return;
  }
  /*-----------------------------------------------------------------------
  ...
  switch ( event->evt )
  {
    ...
    default:
      LOG_MSG_INFO2("Received event : %d", event->evt, 0, 0);
      break;
  }
...
{
  LOG_MSG_INFO2 ("MMGSDI response call back for cnf=%d with status=%d",
                 cnf, status,0);
  return;
...
)
{
  if (NULL == event)
  {
    LOG_MSG_ERROR("Event pointer is NULL", 0, 0, 0);
    return;
  }
  ...
  switch (event->evt)
  {
    case MMGSDI_SUBSCRIPTION_READY_EVT:
      LOG_MSG_INFO2("MMGSDI_SUBSCRIPTION_READY_EVT for client_id %d", 
                    event->client_id, 0, 0);
      /*-----------------------------------------------------------------------
      ...
      {
        LOG_MSG_ERROR("Unable to open GW session with USIM", 0, 0, 0);
      }
      break;

    case MMGSDI_SESSION_CLOSE_EVT:
      LOG_MSG_INFO2("MMGSDI_SESSION_CLOSE_EVT for client_id %d", 
                     event->client_id, 0, 0);
    ...
    case MMGSDI_REFRESH_EVT:
      LOG_MSG_INFO2("MMGSDI_REFRESH_EVT for client_id %d, refresh stage %d, " 
      ...
      {
        LOG_MSG_INFO1 ("Invalid refresh stage %d", 
                       event->data.refresh.stage, 0, 0);
      }
      break;

    default:
      LOG_MSG_INFO2("Ignoring MMGSDI session event %d for client_id %d", 
...
)
{
  ...
  if (MMGSDI_SUCCESS != status)
  {
    LOG_MSG_ERROR("MMGSDI ISIM session open failed, %d", status, 0, 0);
  }
...
)
{
  ...
  if( NULL == cnf_ptr )
  {
    LOG_MSG_ERROR("MMGSDI Client ID Reg failure as cnf_ptr is NULL", 0, 0, 0);
    return ;
  }
  ...
  LOG_MSG_INFO2("MMGSDI ID & Event Registration Callback", 0, 0, 0);
...
)
{
  ...
  if(MMGSDI_SUCCESS != retval) 
  {
    LOG_MSG_ERROR("mmgsdi_client_id_and_evt_reg() error = %d", retval, 0, 0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *data_read = 0;
  ...
  while(from != to)
  {
    ...
    {
      LOG_MSG_INFO2 ("Invalid value. Abort", 0, 0, 0);
      return -1;
    }
    ...
    switch(*from)
    {
      ...
      case 'f':
        ...
        break;
      default:
        LOG_MSG_INFO1("Invalid value. Abort", 0, 0, 0);
        return -1;
    }
    temp = temp * 16;
    temp = temp + value;
  ...
  *err_num = (sint15) E_FAILURE;
          ...
          != (ret_val = ps_efs_tokenizer(efs_db_ptr, &from, &to )))
  {
    ...
    else if(PS_EFS_TOKEN_PARSE_SUCCESS == ret_val)
    {
      ...
      {
        LOG_MSG_ERROR("Error in reading. Abort", 0, 0, 0);
        *err_num = (sint15) E_FAILURE;
        return -1;
      }
      else
      {
        if(aka_isn_info_ptr->aka_algo_seqnum_arr_elements 
                                      >= PS_UTILS_AKA_SEQ_NUM_ARRAY_ELEMENTS)
        {
          LOG_MSG_ERROR("Too many elements to parse. Array overflow", 0, 0, 0);
          *err_num = (sint15) E_FAILURE;
          return -1;
        }
  ...
  LOG_MSG_INFO1("Done Parsing record EFS: Arry size %d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Error opening file ISN from EFS", 0, 0, 0);
    return (sint15) E_FAILURE;
  }
                                        ...
                                        aka_isn_info_ptr ) == -1)
  {
    ...
    {
      LOG_MSG_INFO1("Error %d parsing record from EFS.Aborting", 
                    err_num, 0, 0);
      return (sint15) E_FAILURE;
    }
    else
    {
      LOG_MSG_INFO1("EOF reached reading record from EFS", 0, 0, 0);
      return (sint15) E_SUCCESS;
    }
  }

  ps_efs_file_close(&efs_db);
  LOG_MSG_INFO1("EFS file read successfully", 0, 0, 0);
