...
Copyright (c) 2000-2011 Qualcomm Technologies, Inc. 
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/pstimer.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
/*---------------------------------------------------------------------------
  ...
  START: Timer has been started.
  CANCEL:Timer has been cancelled.
---------------------------------------------------------------------------*/
typedef enum
{
  PS_TIMER_FREE   = 0,
...
/*---------------------------------------------------------------------------
  The timer structure has the following fields:
    ...
    timer_state: This holds the state of the timer. If the timer is defined
                ...
                not yet defined, it is in PS_TIMER_FREE mode. If the timer is 
                started it is in PS_TIMER_START state. If it is cancelled
                it is in PS_TIMER_CANCEL state.
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO2("Invalid timer. Timer %d is freed.",
  ...
  {
    LOG_MSG_INFO2("Timer cookie 0x%x mismatch with handle cookie 0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /*-------------------------------------------------------------------------
      If the timer is already cancelled then we don't call
      the client callback.
    -------------------------------------------------------------------------*/
    if( pstimer->timer_state == PS_TIMER_CANCEL ||
        pstimer->timer_state == PS_TIMER_FREE )
    {
        continue;
    }
    ...
    if(cb_fcn != NULL )
    {
      LOG_MSG_INFO2 ("Timer #%d expired - calling callback",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Callback from timer API with invalid handle %d!\n",
                        (handle & PS_TIMER_HANDLE_MASK), 0, 0);
    return;
  }
  ...
  if(pstimer->extended_time_left > 0)
  {
    ...
    {
      LOG_MSG_INFO1("PS extended timer internal callback. Total time left: %d",
...
void ps_timer_init()
{
  ...
  LOG_MSG_INFO3("Max timer value supported is %d ms",
...
)
{
  ...
  -------------------------------------------------------------------------*/
  PS_LEAVE_CRIT_SECTION(&ps_timer_crit_section);
  LOG_MSG_ERROR("No more ps timers available!", 0,0,0);
...
)
{
  ...
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO3 ("Set timer #%d (%d ms)", handle, timer_len, 0);
  ...
  if( (pstimer = psi_timerhandle_to_timer(handle)) == NULL)
  {
    LOG_MSG_ERROR(" %d is not a valid handle!\n",
                  (handle & PS_TIMER_HANDLE_MASK), 0, 0);
  ...
  {
    LOG_MSG_INFO3("Removing old callback that hasn't been processed yet.",0,0,0);
    pstimer->cback_pending = FALSE;
  ...
  if(timer_len != 0)
  {
    if (timer_len > 0)
    {
      ...
      ps_platform_timer_set( pstimer->timer,
                             (uint32)timer_len );
      pstimer->timer_state = PS_TIMER_START;

    }
    else
    {
      LOG_MSG_ERROR("Negative timeout (%d) specified!\n", timer_len,0,0);
...
)
{
  ...
  timer_entry_type    *pstimer;
  /*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  LOG_MSG_INFO3 ("Cancel timer #%d", (handle & PS_TIMER_HANDLE_MASK), 0, 0);
  ...
  /*-------------------------------------------------------------------------
    This critical section would ensure that we don't post a PS command for 
    timer callback processing while we are cancelling the timer.
  -------------------------------------------------------------------------*/
  REX_DISABLE_PREMPTION(); /* Nobody should pre-empt this */
  PS_ENTER_CRIT_SECTION(&ps_timer_crit_section);
  /*-------------------------------------------------------------------------
  ...
  pstimer->timer_state = PS_TIMER_CANCEL;
...
)
{
  ...
  LOG_MSG_INFO2("freeing timer %d ", (handle & PS_TIMER_HANDLE_MASK),0,0);

  /*-------------------------------------------------------------------------
    This critical section would ensure that we don't post a PS command for 
    timer callback processing while we are freeing the timer.
  -------------------------------------------------------------------------*/
  REX_DISABLE_PREMPTION(); /* Nobody should pre-empt this */
  PS_ENTER_CRIT_SECTION(&ps_timer_crit_section);
  /*-------------------------------------------------------------------------
...
)
{
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    LOG_MSG_INFO2 ("Timer %d not running", (handle & PS_TIMER_HANDLE_MASK),0,0);
  }
  PS_LEAVE_CRIT_SECTION(&ps_timer_crit_section);
  LOG_MSG_INFO2 ("Timer %d has %d ms left",
