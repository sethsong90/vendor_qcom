...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ds_auth_qmi.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
)
{
  ...
  {
    LOG_MSG_ERROR("ds_auth_platform_run_aka_algo: rand_len %d or autn_len %d too large for QMI!", rand_len, autn_len, 0);
    *ds_errno = DS_EFAULT;
    return -1;
  }
  ...
  ind_data_ptr =
    ps_system_heap_mem_alloc(sizeof(ds_auth_aka_ind_data_type));
...
void ds_auth_platform_init()
{
  ...
  LOG_MSG_INFO2("ds_auth_platform_init()", 0,0,0);
  ...
  {
    LOG_MSG_INFO2("ds_auth_qmi_init: QMI connection init failed ", 0,0,0);
    return;
  }
  ...
  if ((qmi_auth_handle = qmi_eap_srvc_init_client(QMI_PORT_RMNET_1, 
                                                  ...
                                                  &qmi_err)) < 0) 
  {
    LOG_MSG_ERROR("ds_auth_qmi_init: Auth client init failed ", 0,0,0);
    ASSERT(0);
    return;
  }
