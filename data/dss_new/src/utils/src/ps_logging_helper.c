...
/*===========================================================================
 ...
 $Header: //source/qcom/qct/modem/datamodem/interface/utils/rel/11.03/src/ps_logging_helper.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (len > MAX_PAYLOAD)
  {
    ...
    if (NULL == log_ptr)
    {
      LOG_MSG_INFO2("log_alloc failed", 0, 0, 0);
      return;
    }
    ...
    {
      LOG_MSG_FATAL_ERROR("DSM chain is corrupted, 0x%p", item_ptr, 0, 0);
      return;
    }
...
#else /* FEATURE_DATA_PS_DATA_LOGGING */

/*---------------------------------------------------------------------------
  ps_logging_defs.h is included because that is where the function
  declaration is comdef.h is included to resolve dsm types
---------------------------------------------------------------------------*/
...
{
  /*-------------------------------------------------------------------------
    empty stub so libraries built with logging feature on will link
  -------------------------------------------------------------------------*/
} /* dpli_log_full_packet() */
