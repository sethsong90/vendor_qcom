...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_flowi.c#2 $
  $Author: smudired $ $DateTime: 2011/07/25 17:19:31 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ip_flow_ptr->field_mask & IPFLOW_MASK_DATA_RATE)
  {
    ...
    {
      if (ip_flow_ptr->data_rate.format.min_max.max_rate == 0 ||
  ...
  {
    LOG_MSG_INFO2("Warning latency variance = 0 on flow type 0x%x",
            flow_type, 0, 0);
  }
  ...
  {
    LOG_MSG_ERROR("IP_FLOW field_mask = 0x%x, Type of FLOW processed: 0x%x, "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Zero filters specified for mask 0x%x", flow_type, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  else if (fltr_spec_ptr->num_filters > MAX_FLTR_PER_REQ)
  {
    LOG_MSG_INFO1("Too many filters specified: %d, max = %d, for mask 0x%x",
  ...
  {
    LOG_MSG_INFO1("NULL list_ptr in fltr spec for mask 0x%x", flow_type, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid flow_type 0x%x is passed", flow_type, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO2("Storing Rx flow spec", 0, 0, 0);
    ...
    if (qos_spec_ptr->field_mask & QOS_MASK_RX_AUXILIARY_FLOWS)
    {
           ...
           flow_index++)
      {
        ...
        {
          LOG_MSG_ERROR("Couldn't create a buffer for Rx flow spec", 0, 0, 0);
          *ps_errno = DS_ENOMEM;
          return -1;
        }
  ...
  {
    LOG_MSG_INFO2("Storing Tx flow spec", 0, 0, 0);
    ...
    if (qos_spec_ptr->field_mask & QOS_MASK_TX_AUXILIARY_FLOWS)
    {
           ...
           flow_index++)
      {
        ...
        if (new_flow_spec_ptr == NULL)
        {
          LOG_MSG_ERROR("Couldn't create a buffer for Tx flow spec", 0, 0, 0);
          *ps_errno = DS_ENOMEM;
          return -1;
        }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Too many auxiliary flows specified: %d, max = %d, for mask 0x%x",
  ...
  {
    LOG_MSG_INFO1("Zero auxiliary flows specified for mask 0x%x", flow_type, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  else if (ip_flow_spec_ptr->aux_flow_list_ptr == NULL)
  {
    LOG_MSG_INFO1("NULL aux_flow_list_ptr for mask 0x%x", flow_type, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Storing Rx fltr spec", 0, 0, 0);
       ...
       fltr_index++)
  {
    ...
    {
      LOG_MSG_ERROR("Couldn't create a buffer for Rx filter", 0, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("App is not changing anything in QOS_MODIFY on "
             "ps_flow, 0x%p", flow_ptr, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_INFO1("Reserved bits are turned on by app in QOS_MODIFY on "
             "ps_flow, 0x%p", flow_ptr, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (qos_spec_ptr->field_mask & QOS_MASK_TX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)   /* if Tx flow is deleted */
    {
      ...
      {
        LOG_MSG_INFO1("Deleting Tx flow spec when it was not requested "
                 "previously on ps_flow, 0x%p", flow_ptr, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
      ...
      {
        LOG_MSG_INFO1("Deleting Tx flow spec while specifying one of Tx min "
      ...
      if ((!(field_mask & QOS_MASK_RX_FLOW) &&
           ...
           ((qos_spec_ptr->field_mask & QOS_MASK_RX_FLOW) &&
              (qos_spec_ptr->rx.flow_template.req_flow.field_mask ==
                IPFLOW_MASK_NONE)))
      {
        LOG_MSG_INFO1("Can't have QOS spec with out both Tx and Rx flow specs "
                 "on ps_flow, 0x%p", flow_ptr, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
      ...
      {
        LOG_MSG_INFO1("Can't modify Tx filters while deleting Tx QoS, on "
                 "ps_flow, 0x%p", flow_ptr, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
  ...
  if (qos_spec_ptr->field_mask & QOS_MODIFY_MASK_TX_FLTR_MODIFY)
  {
    if (!(field_mask & QOS_MASK_TX_FLOW))
    {
      LOG_MSG_INFO1("Modifying Tx fltr spec when it was not requested "
               "previously on ps_flow, 0x%p", flow_ptr, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
  ...
  if (qos_spec_ptr->field_mask & QOS_MASK_RX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)   /* if Rx flow is deleted */
    {
      if (!(field_mask & QOS_MASK_RX_FLOW))
      {
        LOG_MSG_INFO1("Deleting Rx flow spec when it was not requested "
                 "previously on ps_flow, 0x%p", flow_ptr, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
      ...
      {
        LOG_MSG_INFO1("Deleting Rx flow spec while specifying one of Rx min "
      ...
      {
        LOG_MSG_INFO1("Can't modify Rx filters while deleting Rx QoS, on "
                 "ps_flow, 0x%p", flow_ptr, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
  ...
  if (qos_spec_ptr->field_mask & QOS_MODIFY_MASK_RX_FLTR_MODIFY)
  {
    if (!(field_mask & QOS_MASK_RX_FLOW))
    {
      LOG_MSG_INFO1("Modifying Rx fltr spec when it was not requested "
               "previously on ps_flow, 0x%p", flow_ptr, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *flow_ptr_ptr = (ps_flow_type *) ps_mem_get_buf(PS_MEM_PS_FLOW_TYPE);
  ...
  {
    LOG_MSG_ERROR("Couldn't create a flow", 0, 0, 0);
    *ps_errno = DS_ENOMEM;
    return -1;
  }
  ...
  if (is_qos_supported)
  {
    ...
    {
      LOG_MSG_ERROR("Couldn't create qos_info_ptr for flow, 0x%p",
                *flow_ptr_ptr, 0, 0);
      *ps_errno = DS_ENOMEM;
    ...
    list_init
    (
      &((*flow_ptr_ptr)->flow_private.qos_info_ptr->rx.ipflow.aux_flow_list)
    );
    list_init
    (
      &((*flow_ptr_ptr)->flow_private.qos_info_ptr->tx.ipflow.aux_flow_list)
    );
    list_init(&((*flow_ptr_ptr)->flow_private.qos_info_ptr->rx.fltr_list));
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Flow, 0x%p, is already bound", flow_ptr, 0, 0);
  ...
  {
    LOG_MSG_ERROR("phys_link_ptr is NULL!", 0, 0, 0);
    return -1;
  }

  LOG_MSG_INFO2("Binding flow, 0x%p, to phys link, 0x%p",
                                    ...
                                    PS_PHYS_LINK_CAPABILITY_FLOW_DECOUPLED))
  {
    ...
    {
      LOG_MSG_ERROR("Invalid ref cnt, %d, on FLOW_COUPLED phys link, 0x%p",
                PS_PHYS_LINKI_GET_REF_CNT(phys_link_ptr), phys_link_ptr, 0);
    ...
    LOG_MSG_INFO2("Registering for phys link, 0x%p, events on flow, 0x%p",
            phys_link_ptr, flow_ptr, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Unbinding flow, 0x%p, from phys link, 0x%p",
          flow_ptr, PS_FLOWI_GET_PHYS_LINK(flow_ptr), 0);

  if (PS_FLOWI_GET_PHYS_LINK(flow_ptr) == NULL)
  {
    LOG_MSG_INFO2("Flow, 0x%p, is not bound to a phys link", flow_ptr, 0, 0);
    return 0;
  }
  ...
  if (!PS_PHYS_LINKI_GET_CAPABILITY(PS_FLOWI_GET_PHYS_LINK(flow_ptr),
                                    PS_PHYS_LINK_CAPABILITY_FLOW_DECOUPLED))
  {
    LOG_MSG_INFO2("De-registering for phys link, 0x%p, events on flow, 0x%p",
            PS_FLOWI_GET_PHYS_LINK(flow_ptr), flow_ptr, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Flow, 0x%p, is bound to invalid phys link", flow_ptr, 0, 0);
    ASSERT(0);
    return -1;
  }

  LOG_MSG_INFO2("Rebinding flow, 0x%p, from phys link, 0x%p to phys link, 0x%p",
...
{
  return ((PS_FLOWI_GET_ASSOC_PS_FLOW(flow_ptr) != NULL)
            ? PS_FLOW_GET_PHYS_LINK(PS_FLOWI_GET_ASSOC_PS_FLOW(flow_ptr))
            : (flow_ptr)->flow_private.phys_link_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Default flow, 0x%p, must not be deleted", flow_ptr, 0, 0);
    ASSERT(0);
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Deleting flow, 0x%p, in invalid state, 0x%x",
  ...
  -------------------------------------------------------------------------*/
  for (loop = 0; loop < FLOW_MAX_EV - FLOW_MIN_EV; loop++)
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Invalid QOS spec, mask=%d, specify Rx or Tx flow",
             qos_req_param->qos_spec->field_mask, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_INFO1("Invalid QOS spec, mask=%d, Bits outside the allowed range "
             "are set", qos_req_param->qos_spec->field_mask, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (qos_req_param->qos_spec->field_mask & QOS_MASK_RX_FLOW)
  {
    ...
    if (qos_req_param->qos_spec->field_mask & QOS_MASK_RX_AUXILIARY_FLOWS)
    {
      ...
      {
        LOG_MSG_INFO1("Invalid QOS Spec, Rx aux flow spec is invalid", 0, 0, 0);
    ...
    {
      LOG_MSG_INFO1("Invalid QOS Spec, Rx fltr spec is invalid", 0, 0, 0);
  ...
  if (qos_req_param->qos_spec->field_mask & QOS_MASK_TX_FLOW)
  {
    ...
    if (qos_req_param->qos_spec->field_mask & QOS_MASK_TX_AUXILIARY_FLOWS)
    {
      ...
      {
        LOG_MSG_INFO1("Invalid QOS Spec, Tx aux flow spec is invalid", 0, 0, 0);
        /* ps_errno is populated in ps_flowi_validate_aux_flow_spec() */
        return -1;
      }
    }
    ...
    {
      LOG_MSG_INFO1("Invalid QOS Spec, Tx fltr spec is invalid", 0, 0, 0);
      /* ps_errno is populated in ps_flowi_validate_fltr_spec() */
      return -1;
    }
  }
  else /* if TX_FLOW is not specified */
  {
    ...
    if (qos_req_param->qos_spec->field_mask & QOS_MASK_TX_AUXILIARY_FLOWS)
    {
      LOG_MSG_INFO1("TX_AUXILIARY_FLOWS is specified without TX_FLOW, mask = %d",
               qos_req_param->qos_spec->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
  ...
  {
    LOG_MSG_INFO1("QOS Spec validation failed", 0, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_flowi_is_qos_modify_spec_well_formed(flow_ptr,
                                              ...
                                              ps_errno) == -1)
  {
    LOG_MSG_INFO1("QOS spec in QOS_MODIFY is not well formed, ps_flow, 0x%p",
             flow_ptr, 0, 0);
    return -1;
  }
  ...
  if (qos_modify_mask & QOS_MASK_RX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)
    {
            ...
            QOS_MASK_RX_AUXILIARY_FLOWS)
      {
        ...
        {
          LOG_MSG_INFO1("Invalid QOS_MODIFY Spec, Rx aux flow spec is invalid",
                   0, 0, 0);
          /* ps_errno is populated in ps_flowi_validate_aux_flow_spec() */
          return -1;
        }
  ...
  else /* if RX_FLOW is not specified */
  {
    ...
    {
      LOG_MSG_INFO1("RX_MIN_FLOW is specified without RX_FLOW, mask = %d",
               qos_modify_param->qos_spec->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
      ...
      (qos_modify_mask & QOS_MODIFY_MASK_RX_FLTR_MODIFY))
  {
    ...
    {
      LOG_MSG_INFO1("Invalid QOS_MODIFY Spec, Rx fltr spec is invalid", 0, 0, 0);
      /* ps_errno is populated in ps_flowi_validate_fltr_spec() */
      return -1;
    }
  }
  ...
  if (qos_modify_mask & QOS_MASK_TX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)
    {
            ...
            QOS_MASK_TX_AUXILIARY_FLOWS)
      {
        ...
        {
          LOG_MSG_INFO1("Invalid QOS_MODIFY Spec, Tx aux flow spec is invalid",
                   0, 0, 0);
          /* ps_errno is populated in ps_flowi_validate_aux_flow_spec() */
          return -1;
        }
      ...
      (qos_modify_mask & QOS_MODIFY_MASK_TX_FLTR_MODIFY))
  {
    ...
    {
      LOG_MSG_INFO1("Invalid QOS_MODIFY Spec, Tx fltr spec is invalid", 0, 0, 0);
      /* ps_errno is populated in ps_flowi_validate_fltr_spec() */
      return -1;
    }
  }
  ...
  if (is_qos_spec_valid == FALSE)
  {
    LOG_MSG_INFO1("QOS_MODIFY Spec validation failed", 0, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Validating primary qos spec", 0, 0, 0);
  ...
  {
    LOG_MSG_INFO1("Invalid primary QOS spec, mask=0x%x, specify Rx or Tx flow",
             primary_qos_spec_ptr->field_mask, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_INFO1("Invalid primary QOS spec, mask=0x%x, Bits outside the allowed "
             "range are set", primary_qos_spec_ptr->field_mask, 0, 0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (primary_qos_spec_ptr->field_mask & QOS_MASK_TX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)   /* if Tx flow is deleted */
    {
      ...
      {
        LOG_MSG_INFO1("Deleting Tx flow spec while specifying one of Tx min "
                 "flow spec or Tx auxiliary flow specs", 0, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
  ...
  if (primary_qos_spec_ptr->field_mask & QOS_MASK_RX_FLOW)
  {
          ...
          IPFLOW_MASK_NONE)   /* if Rx flow is deleted */
    {
      ...
      {
        LOG_MSG_INFO1("Deleting Rx flow spec while specifying one of Rx min "
                 "flow spec or Rx auxiliary flow specs", 0, 0, 0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
  ...
  else
  {
    ...
    if (primary_qos_spec_ptr->field_mask & QOS_MASK_RX_MIN_FLOW)
    {
      LOG_MSG_INFO1("RX_MIN_FLOW is specified without RX_FLOW, mask = 0x%x",
               primary_qos_spec_ptr->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    if (primary_qos_spec_ptr->field_mask & QOS_MASK_RX_AUXILIARY_FLOWS)
    {
      LOG_MSG_INFO1("RX_AUXILIARY_FLOWS is specified without RX_FLOW, mask = 0x%x",
               primary_qos_spec_ptr->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
  ...
  else /* if TX_FLOW is not specified */
  {
    ...
    if (primary_qos_spec_ptr->field_mask & QOS_MASK_TX_MIN_FLOW)
    {
      LOG_MSG_INFO1("TX_MIN_FLOW is specified without TX_FLOW, mask = 0x%x",
               primary_qos_spec_ptr->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    if (primary_qos_spec_ptr->field_mask & QOS_MASK_TX_AUXILIARY_FLOWS)
    {
      LOG_MSG_INFO1("TX_AUXILIARY_FLOWS is specified without TX_FLOW, mask = 0x%x",
               primary_qos_spec_ptr->field_mask, 0, 0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (qos_spec == NULL)
  {
    LOG_MSG_ERROR("QOS_SPEC is NULL", 0, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("flow_ptr or qos_info_ptr is NULL", 0, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid fltr priority style, %d, is passed",
              fltr_priority, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  ...
  {
    LOG_MSG_ERROR("Already stored QOS Spec on ps_flow, 0x%p", flow_ptr, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  if (ps_flowi_store_flow_spec(flow_ptr->flow_private.qos_info_ptr,
                               ...
                               ps_errno) == -1)
  {
    LOG_MSG_ERROR("Flow 0x%p: Couldn't store flow spec", flow_ptr, 0, 0);
    (void) ps_flowi_delete_qos_spec(flow_ptr);
    return -1;
  }
  ...
  if (qos_spec->field_mask & QOS_MASK_RX_FLOW)
  {
    if (ps_flowi_store_rx_fltr_spec(flow_ptr->flow_private.qos_info_ptr,
                                    ...
                                    ps_errno) == -1)
    {
      LOG_MSG_ERROR("Flow 0x%p: Couldn't store Rx filter spec", flow_ptr, 0, 0);
      (void) ps_flowi_delete_qos_spec(flow_ptr);
      return -1;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("QOS_MODIFY_SPEC is NULL", 0, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Couldn't create qos_modify_info_ptr for flow, 0x%p",
              flow_ptr, 0, 0);
    *ps_errno = DS_ENOMEM;
    return -1;
  }
  ...
  if (ps_flowi_store_flow_spec(flow_ptr->flow_private.qos_modify_info_ptr,
                               ...
                               ps_errno) == -1)
  {
    LOG_MSG_ERROR("Flow 0x%p: Couldn't store modify_flow_spec", flow_ptr, 0, 0);
      ...
      PS_FLOWI_IS_MODIFY_RX_FLTR_SPEC_MODIFIED(flow_ptr))
  {
    ...
    {
      LOG_MSG_ERROR("Flow 0x%p: Couldn't store modify_rx_fltr_spec",
                flow_ptr, 0, 0);
      (void) ps_flowi_delete_qos_modify_spec(flow_ptr);
      return -1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("QOS info pointer of flow pointer is NULL",0,0,0);
    ASSERT(0);
    return -1;
  }
  /*-------------------------------------------------------------------------
  ...
  LOG_MSG_INFO1("Flow 0x%p: QOS released", flow_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1("Flow 0x%p: QOS_MODIFY_SPEC released", flow_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (is_modify)
  {
    ...
    {
      LOG_MSG_ERROR("QOS_MODIFY is not pending on ps_flow, 0x%p", flow_ptr, 0, 0);
      ASSERT(0);
      return NULL;
    }
