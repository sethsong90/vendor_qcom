...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_flowi_event.c#1 $
...
#define PS_FLOW_EVT_BUF_NUM         250
#define PS_FLOW_EVT_BUF_HIGH_WM     220
#define PS_FLOW_EVT_BUF_LOW_WM       20
#else
#define PS_FLOW_EVT_BUF_NUM         200
#define PS_FLOW_EVT_BUF_HIGH_WM     170
#define PS_FLOW_EVT_BUF_LOW_WM       20
#endif
...
#define PS_FLOW_EVT_BUF_NUM         300
#define PS_FLOW_EVT_BUF_HIGH_WM     240
#define PS_FLOW_EVT_BUF_LOW_WM       60
...
----------------------------------------------------------------------------*/
static int ps_flow_evt_buf_mem[PS_MEM_GET_TOT_SIZE_OPT
                               (
                                 PS_FLOW_EVT_BUF_NUM,
...
#define PS_FLOWI_REG_EV_WITH_PHYS_LINK(phys_link_ptr, ev_buf_ptr, event,   \
  ...
  if ((ev_buf_ptr) == NULL)                                                \
  {                                                                        \
    LOG_MSG_ERROR("Unable to allocate buffer for EV, %d", event, 0, 0);        \
    return;                                                                \
  }                                                                        \
                                                                           \
  if (ps_phys_link_event_cback_reg(phys_link_ptr, event, ev_buf_ptr) != 0) \
  {                                                                        \
    LOG_MSG_ERROR("Could not reg cback for EV %d", event, 0, 0);               \
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Flow, 0x%p, is invalid", flow_ptr, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Phys link, 0x%p is not COUPLED to a flow",
              phys_link_ptr, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Flow, 0x%p, is not bound to phys link, 0x%p",
              flow_ptr, phys_link_ptr, 0);
    ASSERT(0);
    return;
  }

  LOG_MSG_INFO2("Processing event, %d on phys link 0x%p for flow 0x%p",
  ...
  switch (event)
  {
    ...
    default:
      ASSERT(0);
      LOG_MSG_ERROR("Unknown event %d", event, 0, 0);
      break;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#if 0
  LOG_MSG_FUNCTION_ENTRY("Event %d on flow 0x%p", event, assoc_flow_ptr, 0);
#endif

  assoc_flow_cmd_ptr = ps_system_heap_mem_alloc(
  ...
  do
  {
    ...
    {
      LOG_MSG_INVALID_INPUT("Got invalid logical flow_ptr 0x%p",
    ...
    LOG_MSG_FUNCTION_EXIT("Success: Event %d on flow 0x%p", event,
                          assoc_flow_ptr, 0);
    return;
  } while(0);
...
#if 0
  LOG_MSG_FUNCTION_EXIT("Fail: Event %d on flow 0x%p",
                        event, assoc_flow_ptr, 0);
#endif
  return;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Initialize Pool
  -------------------------------------------------------------------------*/
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_PS_FLOW_EVT_CB_TYPE,
                           ps_flow_evt_buf_mem,
                           PS_FLOW_EVT_BUF_SIZE,
                           PS_FLOW_EVT_BUF_NUM,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("NULL call back function is passed", 0, 0, 0);
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if (buf_ptr == NULL)
  {
    LOG_MSG_ERROR("NULL buffer is passed", 0, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Buffer, 0x%p, is still on a queue", buf_ptr, 0, 0);
    ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid ps_flow buf 0x%p to reg evt %d", buf_ptr, event, 0);
    ASSERT(0);
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Not re-q'ing ps_flow buf 0x%p for evt %d",
              flow_ev_buf_ptr, event, 0);
    ASSERT(0);
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Callback is null in buf 0x%p for evt %d",
              flow_ev_buf_ptr, event, 0);
    ASSERT(0);
    return -1;
  }
  ...
  flow_ev_buf_ptr->event_q_ptr =
    &(flow_ptr->flow_private.event_q_array[event - FLOW_MIN_EV]);
  q_put(flow_ev_buf_ptr->event_q_ptr, &(flow_ev_buf_ptr->link));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ps_mem_is_valid(buf_ptr, PS_MEM_PS_FLOW_EVT_CB_TYPE) == FALSE)
  {
    LOG_MSG_INFO1("Invalid buf 0x%p to dereg evt %d", buf_ptr, event, 0);
    ASSERT(0);
    return;
  }
  ...
  if (!(Q_ALREADY_QUEUED(&(flow_ev_buf_ptr->link))))
  {
    LOG_MSG_INFO2("Buf 0x%p for event %d not on a queue",
            flow_ev_buf_ptr, event, 0);
    return;
  }
  ...
  else
  {
    ...
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR("Invalid flow ptr, 0x%p, passed", flow_ptr, 0, 0);
      return;
    }
  ...
  {
    LOG_MSG_ERROR("Buf 0x%p (ev %d) not reg 0x%p flow",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid flow event passed %d", event, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  for (;;)
  {
    ...
    while (event_buf_ptr != NULL)
    {
      ...
      event_buf_ptr = (ps_flow_event_buf_type *) (q_next(working_q_ptr,
                                                  &(event_buf_ptr->link)));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( ! is_default_flow)
  {
    ...
    if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.activated ==
          NULL)
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_ACTIVATED_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_ACTIVATED_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.activated
      );
    ...
    if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.suspended ==
          NULL)
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_SUSPENDED_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_SUSPENDED_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.suspended
      );
    ...
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_NULL_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_NULL_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.null
      );
    ...
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_INFO_CODE_UPDATED_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d",
                FLOW_INFO_CODE_UPDATED_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.info_code_updated
      );
    ...
    if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.modify_accepted ==
          NULL)
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_MODIFY_ACCEPTED_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_MODIFY_ACCEPTED_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.modify_accepted
      );
    ...
    if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.modify_rejected ==
          NULL)
    {
      LOG_MSG_ERROR("Out of mem for ev %d", FLOW_MODIFY_REJECTED_EV, 0, 0);
      *ps_errno = DS_ENOMEM;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_MODIFY_REJECTED_EV, 0, 0);
      ps_flow_free_event_cback_buf
      (
        flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.modify_rejected
      );
  ...
  if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.tx_enabled ==
        NULL)
  {
    LOG_MSG_ERROR("Out of mem for ev %d", FLOW_TX_ENABLED_EV, 0, 0);
    *ps_errno = DS_ENOMEM;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_TX_ENABLED_EV, 0, 0);
    ps_flow_free_event_cback_buf
    (
      flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.tx_enabled
    );
  ...
  if (flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.tx_disabled ==
        NULL)
  {
    LOG_MSG_ERROR("Out of mem for ev %d", FLOW_TX_DISABLED_EV, 0, 0);
    *ps_errno = DS_ENOMEM;
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Could not reg cback for ev %d", FLOW_TX_DISABLED_EV, 0, 0);
    ps_flow_free_event_cback_buf
    (
      flow_ptr->flow_private.logical_flow.ps_flow_ev_cback_buf.tx_disabled
    );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY("flow 0x%p", flow_ptr, 0, 0);
  ...
  LOG_MSG_FUNCTION_EXIT( "Success, flow 0x%p", flow_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_FUNCTION_ENTRY( "cmd %d called with user_data_ptr 0x%p", cmd,
                          user_data_ptr, 0);

  if (user_data_ptr == NULL)
  {
    LOG_MSG_INVALID_INPUT("NULL DS cmd ptr", 0, 0, 0);
    return;
  }

  if (cmd != DCC_LOGICAL_FLOW_ASSOC_FLOW_EV_CMD)
  {
    LOG_MSG_INVALID_INPUT("Invalid DCC cmd id %d", cmd, 0, 0);
  ...
  LOG_MSG_INFO1("Processing event, %d on assoc_ps_flow 0x%p for flow 0x%p",
  ...
  if (!PS_FLOW_IS_VALID(flow_ptr))
  {
    LOG_MSG_INFO1("Ignoring event %d as flow 0x%p is invalid",
                  event, flow_ptr, 0);
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
  ...
  {
    LOG_MSG_INFO1("Ignoring event %d as flow 0x%p is re-allocated",
                  event, flow_ptr, 0);
    PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Flow 0x%p is not associated to flow 0x%p but to 0x%p",
                  flow_ptr,
                  assoc_flow_ptr,
  ...
  if (event != FLOW_NULL_EV)
  {
    if (!PS_FLOW_IS_VALID(assoc_flow_ptr))
    {
      LOG_MSG_INFO1("Ignoring event %d on flow 0x%p as assoc flow 0x%p is "
  ...
  switch (event)
  {
    ...
    case FLOW_NULL_EV:
      ...
      if (ps_iface_delete_flow(flow_ptr->flow_private.iface_ptr,
                               ...
                               &ps_errno) != 0)
      {
        LOG_MSG_ERROR("Could not delete PS flow 0x%p", flow_ptr, 0, 0);
      }
    ...
    case FLOW_MODIFY_ACCEPTED_EV:
      ...
      {
        LOG_MSG_INFO1("Network inited qos modify accept 0x%p", flow_ptr, 0, 0);
        ...
        if (0 != ret_val)
        {
          LOG_MSG_INFO1("Fail ps_flowi_get_qos_spec_type_from_flow, flow 0x%p,"
                        "errno %d", flow_ptr, ps_errno, 0);
          break;
        }
        ...
        if (0 != ret_val)
        {
          LOG_MSG_INFO1("Fail ps_iface_modify_flow, flow 0x%p, errno %d",
                        flow_ptr, ps_errno, 0);
          break;
        }
      }
      ...
      if (ps_iface_modify_flow_accepted(flow_ptr->flow_private.iface_ptr,
                                        ...
                                        &ps_errno ) != 0 )
      {
        LOG_MSG_ERROR("Could not modify PS flow 0x%p", flow_ptr, 0, 0);
      }
    ...
    case FLOW_FLTR_AUX_INFO_UPDATED_EV:
      ...
      if (0 != ret_val)
      {
        LOG_MSG_ERROR("ps_iface_logical_flowi_process_fltr_aux_info_updated_ev() "
                  "failed, PS flow 0x%p err %d", flow_ptr, ps_errno, 0);
      }
    ...
    default:
      ASSERT(0);
      LOG_MSG_ERROR("Invalid event %d", event, 0, 0);
      break;
  } /* switch (event) */
