...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_route.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
/* extern till ps_iface.h is updated */
extern int ps_iface_get_root_iface
(
...
)
{
  ACL_TRACE_DUMP_6(ACL_LEVEL_MED,
                   "ACL Policy Parameters: Policy_Flag %d, Iface 0x%x:%x, "
                   "IP_family %d, Is_sock_orig %d, Is_Routable %d",
  ...
  ACL_TRACE_DUMP_6(ACL_LEVEL_MED,
                   "IPSec_disabled %d, Lookup_only %d, Bring_Up %d, "
                   "App Identifier %llx, data session profile ID %d, "
                   "IM_CN flag %d",
 ...
 /*------------------------------------------------------------------------
  ...
  if( ip_pkt_ptr && (ip_pkt_ptr->ip_vsn == IP_V6) )
  {
    if(PS_IN6_IS_ADDR_V4MAPPED(ip_pkt_ptr->ip_hdr.v6.hdr_body.
                                        base_hdr.dst_addr.ps_s6_addr))
    {
      temp_addr = PS_IN6_GET_V4_FROM_V4_MAPPED_V6(
                  ip_pkt_ptr->ip_hdr.v6.hdr_body.base_hdr.dst_addr.ps_s6_addr);
    ...
    else
    {
      ...
      if((PS_IN6_IS_ADDR_UNSPECIFIED(
                 ip_pkt_ptr->ip_hdr.v6.base_v6_hdr.src_addr.ps_s6_addr)
          ||
          PS_IN6_IS_ADDR_UNSPECIFIED(
                 ip_pkt_ptr->ip_hdr.v6.base_v6_hdr.dst_addr.ps_s6_addr)
         ) == FALSE )
      {
                                  ...
                                  base_hdr.dst_addr.ps_s6_addr))
        {
          ...
          -----------------------------------------------------------------*/
          if(!PS_IN6_IS_ADDR_LINKLOCAL(ip_pkt_ptr->ip_hdr.v6.hdr_body.
                                     base_hdr.src_addr.ps_s6_addr))
          {
            LOG_MSG_INFO1("SRC and DST addrs IPv6 scopes dont match", 0, 0, 0);
            return -1;
          }
        }
                                  ...
                                  base_hdr.dst_addr.ps_s6_addr))
      {
        ...
        {
          LOG_MSG_INFO1("No scopeid for linklocal IPv6 address, routing not "
                   "allowed", 0, 0, 0);
          return -1;
        }
        ...
        {
          LOG_MSG_INFO1("Route lookup failed, invalid if_ptr", 0, 0, 0);
          return -1;
        }
        ...
        if (acl_pi_ptr != NULL)
        {
          ...
          {
            LOG_MSG_INFO1("Route lookup failed, scopeid violates netpolicy",
                          0,0,0);
            return -1;
          }
        }
        ...
        {
          LOG_MSG_INFO1("Skipped route scope check as policy is NULL", 0, 0, 0);
        }
  ...
  {
    LOG_MSG_INFO1("Route lookup failed,",0,0,0);
    return -1;
  }
...
)
{
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_FATAL_ERROR( "RT: Table is full",0 , 0, 0 );
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  LOG_MSG_INFO1("RT: Adding 0x%x/0x%x gw 0x%x",
  ...
  {
    case CDMA_SN_IFACE:
    LOG_MSG_INFO1("via 1x/SN if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
      break;
    case CDMA_AN_IFACE:
    LOG_MSG_INFO1("via 1x/AN if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
      break;
    case UMTS_IFACE:
    LOG_MSG_INFO1("via UMTS if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
      break;
    case SIO_IFACE:
    LOG_MSG_INFO1("via SIO if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
      break;
    case LO_IFACE:
    LOG_MSG_INFO1("via LO if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
      break;
    default:
    LOG_MSG_INFO1("via if 0x%x:%d (metric %d)",
             if_ptr->name,
             if_ptr->instance,
             metric );
  ...
  switch( type )
  {
    ...
    default:
      LOG_MSG_FATAL_ERROR( "RT: Unknown route type %d", type, 0, 0 );
  } /* switch(route type) */
  ...
  {
    LOG_MSG_ERROR("Rt: Deleting bad v4 entry %d", index, 0, 0 );
...
)
{
  ...
  if ( gateway == NULL )
  {
    LOG_MSG_INFO1("Flushing all routes for dst 0x%x",
             destination.addr.v4.ps_s_addr, 0, 0);
  }
  else
  {
    ASSERT( gateway->type == IPV4_ADDR );
...
)
{
  ...
  ps_iface_type**           temp_deny_list_ptr = NULL;
  ...
  if ( iface_deny_count > params_ptr->deny_list_arr_size )
  {
    LOG_MSG_INFO2("%d denied ifaces > %d deny list size",
            iface_deny_count, params_ptr->deny_list_arr_size, 0);
  }
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_HIGH, "PASSED iface priority", 0, 0, 0 );

  ACL_TRACE_DUMP(ACL_LEVEL_HIGH, "Denied ifaces List:", 0, 0, 0);

  params_ptr->deny_list_ptr--;
  temp_deny_list_ptr = params_ptr->deny_list_ptr;

  for (i = temp_deny_cnt; i > 0; i--)
  {
    if (i - 3 > 0)
    {
      ACL_TRACE_DUMP_6(ACL_LEVEL_HIGH,
                       " 0x%x:%d 0x%x:%d 0x%x:%d",
                       (*temp_deny_list_ptr)->name,
                       (*temp_deny_list_ptr)->instance,
                       (*(temp_deny_list_ptr - 1))->name,
                       (*(temp_deny_list_ptr - 1))->instance,
                       (*(temp_deny_list_ptr - 2))->name,
                       (*(temp_deny_list_ptr - 2))->instance);

      temp_deny_list_ptr = temp_deny_list_ptr - 3;
      i = i - 2;
    }
    else
    {
      ACL_TRACE_DUMP(ACL_LEVEL_HIGH,
                     " 0x%x:%d",
                     (*temp_deny_list_ptr)->name,
                     (*temp_deny_list_ptr)->instance,
                     0);
      temp_deny_list_ptr = temp_deny_list_ptr - 1;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( -1 == ps_route_local_acl_policy_search_list( ROUTE_ACL_DEFAULT_OUTBOUND,
                                                    ...
                                                    &rt_result ) )
  {
    LOG_MSG_ERROR("Routing search failed on local search(phys):%d",
                ROUTE_ACL_DEFAULT_OUTBOUND,0,0);
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    return -1;
  }

  LOG_MSG_INFO2( "Search for phys Iface succeeded",
                  0, 0, 0 );
  ...
  if( acl_ptr )
  {
    LOG_MSG_INFO2("ACL Srch (phys): Found ACL match on iface 0x%x:%d with priority %d",
  ...
  while ( acl_ptr )
  {
    ...
    if ( ps_route_local_acl_policy_search_list( ROUTE_ACL_LOGICAL_OUTBOUND,
                                                ...
                                                &rt_result ) == -1 )
    {
      LOG_MSG_ERROR("Routing search failed on local search(logical):%d",
                 ROUTE_ACL_LOGICAL_OUTBOUND,0,0);
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      return -1;
    }

    LOG_MSG_INFO2( "Search for logical Iface succeeded",
                    0, 0, 0 );
    ...
    {
      LOG_MSG_INFO2("ACL Srch(logical): Found ACL match on iface 0x%x:%d with priority %d",
               ACL_IF_PTR( acl_ptr )->name,
               ACL_IF_PTR( acl_ptr )->instance,
               rt_result.priority );
      /* Preserve proxy iface attributes if specified */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (params_ptr->acl_pi_ptr->iface.kind == DSS_IFACE_ID)
  {
    ...
    {
      LOG_MSG_ERROR("ps_route_network_selection_by_policy(): Invalid iface ID",
                    params_ptr->acl_pi_ptr->iface.info.id, 0, 0);
      PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      return -1;
    }
    ...
    for (index = 0; index < (uint32) ACL_PROC_MAX; index++)
    {
      ...
      if (_acl_priority_cb_tbl[ index ])
      {
        ...
        if (-1 == _acl_priority_cb_tbl[ index ]( params_ptr,
                                                 ...
                                                 &rt_result) )
        {
          LOG_MSG_ERROR("Routing search failed on processor:%d", index, 0, 0);
        }
        else
        {
          /*-----------------------------------------------------------------
  ...
  {
    LOG_MSG_INFO2("RT: Found ACL match on iface 0x%x:%d with priority %d",
...
)
{
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_INFO1("RT: No route found for dst 0x%x",
             ip_pkt_ptr->ip_hdr.v4.dest.ps_s_addr, 0, 0 );
    return -1;
  }
  else
  {
    ...
    for ( ; deny_list_ptr >= _route_deny_list; deny_list_ptr-- )
    {
      ...
      {
        LOG_MSG_INFO1("RT: Found denied interface 0x%x:%d",
                 (*deny_list_ptr)->name,
                 (*deny_list_ptr)->instance, 0 );
        temp = -1;
        break;
      }
    }
    ...
    if ( temp != -1 )
    {
      LOG_MSG_INFO2("RT: Found table match for iface 0x%x:%d",
...
)
{
  ...
  *if_ptr = NULL;
  ...
  if(ip_pkt_ptr == NULL)
  {
    ...
    {
      ASSERT(0);
      LOG_MSG_INFO1("Null IP pkt info, addr routing not allowed", 0, 0, 0);
      return -1;
    }
  }
  ...
  LOG_MSG_INFO2("DEBUG addr_based_routing_only=%d if_ptr=0x%p",
          addr_based_routing_only, if_ptr, 0 );
  LOG_MSG_INFO2("DEBUG POLICY: flag=%d lookup_only=%d bring_up=%d",
          acl_pi_ptr->policy_flag, acl_pi_ptr->lookup_only,
          acl_pi_ptr->bring_up);
  LOG_MSG_INFO2("DEBUG POLICY: family=%d iface.kind=%d iface.id/name=0x%p",
          acl_pi_ptr->ip_family, acl_pi_ptr->iface.kind,
          acl_pi_ptr->iface.info.id);
  ...
  if( !addr_based_routing_only && acl_pi_ptr )
  {
    ...
    if( (acl_pi_ptr->bring_up == TRUE || acl_pi_ptr->lookup_only == TRUE))
    {
      ...
      if(-1 == ps_route_network_selection_by_policy( &lookup_params ))
      {
...
)
{
  ...
  if ( if_ptr == NULL )
  {
    ...
    LOG_MSG_INFO1("RT: Flushing table", 0, 0, 0 );
  ...
  else
  {
    ...
    LOG_MSG_INFO1("RT: Flushing table for iface 0x%x:%d",
             if_ptr->name, if_ptr->instance, 0);
...
)
{
  ...
  switch( priority )
  {
    ...
    case ROUTE_ACL_TOP_PRIORITY:
      LOG_MSG_FATAL_ERROR( "RT: Unsupported op", 0, 0, 0 );
      break;
...
)
{
  ...
  if ( acl_ptr == NULL)
  {
    LOG_MSG_ERROR("RT: Bad ACL Info id:%d, 0x%x", acl_id_type, &acl_id, 0 );
  }
  else
  {
    /* Found the ACL, perform the control operation here */
    acl_ptr->disabled = (acl_enable) ? FALSE : TRUE;
    LOG_MSG_INFO1("RT: ACL 0x%x enabled = %d", acl_ptr, acl_enable, 0 );
  }
...
)
{
  ...
  *if_ptr = NULL;
  ...
  {
    ASSERT(0);
    LOG_MSG_ERROR("Invalid IP protocol specified: %d",
                  ip_pkt_ptr->ip_vsn, 0, 0);
    return -1;
  }
  ...
  if (rlist_type == ROUTE_ACL_PHYSICAL_INBOUND)
  {
    is_inbound_acl = TRUE;
  }
  ...
  if( acl_match_ptr != NULL )
  {
    LOG_MSG_INFO3("Found ACL match for iface 0x%x:%d with priority %d",
             ACL_IF_PTR( acl_match_ptr )->name,
             ACL_IF_PTR( acl_match_ptr )->instance,
             best_priority );
  ...
  if ( *if_ptr == NULL )
  {
    LOG_MSG_INFO3("No ACL match found for list %d", rlist_type, 0, 0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Invalid iface", 0, 0, 0);
    return (FALSE);
  }
...
)
{
  ...
  if(acl_pi_ptr == NULL)
  {
    LOG_MSG_INFO1("NULL acl_policy_info passed to route_scope_by_policy",0,0,0);
    return (route_scope);
  }
  ...
  if( ip_pkt_ptr && (ip_pkt_ptr->ip_vsn == IP_V6) )
  {
                                        ...
                                        base_hdr.src_addr.ps_s6_addr))
    {
      ...
      temp_addr = PS_IN6_GET_V4_FROM_V4_MAPPED_V6(
                  ip_pkt_ptr->ip_hdr.v6.hdr_body.base_hdr.src_addr.ps_s6_addr);
  ...
  for (index = ROUTE_ACL_OUTBOUND_MIN; index < ROUTE_ACL_OUTBOUND_MAX; index++)
  {
    ...
    if (acl_list_ptr == NULL)
    {
      LOG_MSG_ERROR("Couldn't find acl list at idx %d", index, 0, 0);
      ASSERT(0);
      return route_scope;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid iface ptr 0x%p or ip addr ptr 0x%p",
              this_iface_ptr, ip_addr_ptr, 0);
    ASSERT(0);
    return;
  }
  ...
  {
    LOG_MSG_ERROR("Mismatched iface 0x%p and address type %d",
              this_iface_ptr, ip_addr_ptr->type, 0);
    ASSERT(0);
    return;
  }
  ...
  switch(ip_addr_ptr->type)
  {
    ...
    default:
      LOG_MSG_FATAL_ERROR("LO routing table update failed: invalid address type: %d",
                ip_addr_ptr->type,0,0);
      break;
  }

  LOG_MSG_INFO2("LO routing table updated w/iface 0x%x:%d",
  ...
  if(ps_iface_addr_family_is_v4(this_iface_ptr))
  {
    ...
    for(index=0; index < (MAX_IPV4_LO_IFACES - 1); index++)
    {
      if(global_v4_lo_route_array[index].iface_ptr == this_iface_ptr)
      {
        ...
        {
          memcpy(&global_v4_lo_route_array[index],
        ...
        LOG_MSG_INFO2("LO routing entry cleared for iface 0x%p",
                this_iface_ptr, 0, 0);
        break;
      }
    } /* end for */
  ...
  else if(ps_iface_addr_family_is_v6(this_iface_ptr))
  {
    ...
    for(index=0; index < (MAX_IPV6_LO_IFACES - 1); index++)
    {
      if(global_v6_lo_route_array[index].iface_ptr == this_iface_ptr)
      {
        ...
        {
          memcpy(&global_v6_lo_route_array[index],
        ...
        LOG_MSG_INFO2("LO routing entry cleared for iface 0x%x:%d",
                this_iface_ptr->name, this_iface_ptr->instance, 0);
        break;
      }
    } /* end for */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2( "DEBUG POLICY: family=%d iface.kind=%d iface.id/name=0x%p",
          acl_pi_ptr->ip_family, acl_pi_ptr->iface.kind,
          acl_pi_ptr->iface.info.id);
  LOG_MSG_INFO2( "DEBUG POLICY: proc_id=%d",
          acl_pi_ptr->proc_id, 0, 0);
  ...
  result = ps_route_network_selection_by_policy( &lookup_params );

  LOG_MSG_INFO1( "lookup: selection done, in_use_cfg_match [%d], in_use [%d], Iface available? [%d]",
  ...
  {
    LOG_MSG_INFO1( "lookup: in_use_cfg_match > 0, ARBITRATE! in_use_cfg_match [%d], in_use [%d]",
                   num_iface_in_use_cfg_match, num_iface_in_use, 0 );
                                  ...
                                  num_iface_in_use_cfg_match ) != 0 )
    {
      ...
      LOG_MSG_INFO1( "lookup: failed to find logical, use physical", 0, 0, 0 );
    ...
    else
    {
      ...
      LOG_MSG_INFO1( "lookup: found logical, arbitrate on logical", 0, 0, 0 );
    ...
    *arb_candidate = TRUE;
    ...
    if( dun_flag )
    {
      ...
      {
        if_ptr = tmp_if_ptr;
        LOG_MSG_INFO1( "lookup: TE-DUN, found physical", 0, 0, 0 );
      }
    }
#endif /* FEATURE_DATA_PS_ARBITRATION_MGR */
  ...
  if( result == 0 )
  {
    LOG_MSG_INFO1( "lookup: Iface available, ARBITRATION NOT NEEDED!", 0, 0, 0 );
    *arb_candidate = FALSE;
  }
  else /* Iface is not available, in_use_cfg_match list is empty */
  if ( result == -1 && num_iface_in_use > 0 )
  {
    MSG_HIGH( "lookup: in_use > 0, ARBITRATE!, in_use [%d]", num_iface_in_use, 0, 0 );
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_INFO1( "lookup: failed to find Iface",
                   0, 0, 0 );
    return NULL;
  }
  ...
  *iface_priority_mask = PS_IFACEI_GET_PRIORITY_MASK( if_ptr );
  ...
  MSG_HIGH( "lookup: done! Iface [0x%x:%x] available, prio mask [%x]",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("bring_up flag is not set", 0, 0, 0);
    *ps_errno = DS_EINVAL;
    ASSERT(0);
    return -1;
  }
  ...
  LOG_MSG_INFO2("DEBUG if_ptr=0x%p", if_ptr, 0, 0 );
  LOG_MSG_INFO2("DEBUG POLICY: flag=%d lookup_only=%d bring_up=%d",
                acl_pi_ptr->policy_flag,
                acl_pi_ptr->lookup_only,
                acl_pi_ptr->bring_up);
  LOG_MSG_INFO2("DEBUG POLICY: family=%d iface.kind=%d iface.id/name=0x%p",
                ...
                acl_pi_ptr->iface.kind,
                acl_pi_ptr->iface.info.id);
  LOG_MSG_INFO2( "DEBUG POLICY: proc_id=%d", acl_pi_ptr->proc_id, 0, 0);
  ...
  *if_ptr = NULL;
  ...
  LOG_MSG_INFO1( "bringup: selection done, in_use_cfg_match [%d], "
                 "in_use [%d], Iface available? [%d]",
                 num_iface_in_use_cfg_match, num_iface_in_use, !result );

  if (result == 0)
  {
    LOG_MSG_INFO1( "bringup: Iface available, ARBITRATION NOT NEEDED!",
                   0, 0, 0 );
    arb_flg = FALSE;
  }
  else if (num_iface_in_use_cfg_match > 0)
  {
    LOG_MSG_INFO1( "bringup: in_use_cfg_match > 0, ARBITRATE! "
  ...
  {
    LOG_MSG_INFO1( "bringup: in_use > 0, ARBITRATE!, in_use [%d]",
                   num_iface_in_use, 0, 0 );
    ...
    {
      /* found logical, not TE-DUN return logical Ifaces */
      LOG_MSG_INFO1( "lookup: found logical, NOT TE-DUN, use logical", 0, 0, 0 );
      arb_candidate_iface_arr_ptr = logical_iface_arr_ptr;
      num_arb_candidate_iface     = num_logical_iface;
    }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_INFO1( "bringup: failed to find Iface", 0, 0, 0 );
  ...
  LOG_MSG_INFO1("PS Iface bringup using prio [%d], profile [%d]",
                app_priority, acl_pi_ptr->data_session_profile_id, 0 );
  ...
  if( !arb_flg )
  {
    result = ps_iface_bring_up_cmd_ex(*if_ptr,
                                      ...
                                      ps_errno,
                                      client_data_ptr);
    LOG_MSG_INFO1( "PS Iface bringup no arb, result [%d], errno [%d]",
                   result, *ps_errno, 0 );
  }
  else
  {
    ...
    *ps_errno = DS_ENETCLOSEINPROGRESS;
    LOG_MSG_INFO1( "PS Iface bringup after arb, result [%d], errno [%d]",
                   result, *ps_errno, 0 );
  }
...
)
{
  ...
  LOG_MSG_INFO2("Register ACL policy routing func: proc=%d cb=0x%p",
          proc_id, acl_priority_cb, 0);
  ...
  *if_ptr = NULL;
  ...
  if (!addr_based_routing_only)
  {
    if (acl_pi_ptr->iface.kind == DSS_IFACE_ID)
    {
      ...
      if (ps_iface_ptr == NULL)
      {
        LOG_MSG_INFO1("ps_iface_get_handle failed, handle %d",
	         acl_pi_ptr->iface.info.id, 0, 0);
        return -1;
      }
