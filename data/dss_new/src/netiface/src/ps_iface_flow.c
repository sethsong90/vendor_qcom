...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_iface_flow.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("NULL parameters are passed", 0, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  *out_flow_ptr_ptr = NULL;
  ...
  if (phys_link_ptr != NULL)
  {
    ...
    {
      LOG_MSG_ERROR("Invalid phys link, 0x%p, is passed", phys_link_ptr, 0, 0);
      ASSERT(0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
    ...
    {
      LOG_MSG_ERROR("Iface 0x%x:%d, not associated with phys link 0x%p",
  ...
  if (ps_flowi_validate_qos_spec(qos_req_param,
                                 ...
                                 ps_errno) != 0)
  {
    LOG_MSG_ERROR("Invalid QOS spec is sent by application", 0, 0, 0);
  ...
  {
    LOG_MSG_ERROR("Couldn't create a buffer for PS flow", 0, 0, 0);
  ...
  if (ps_flowi_store_qos_spec(flow_ptr,
                              ...
                              ps_errno) != 0)
  {
    LOG_MSG_ERROR("Couldn't store qos spec in flow, 0x%p", flow_ptr, 0, 0);
  ...
  if (qos_req_param->qos_spec->field_mask & QOS_MASK_TX_FLOW)
  {
    ...
    {
      LOG_MSG_ERROR("Couldn't store Tx fltr spec in iface, 0x%x:%d, "
                "for flow 0x%p",
  ...
  if (qos_req_param->qos_spec->field_mask & QOS_MASK_RX_FLOW)
  {
    ...
    if (PS_FLOWI_GET_RX_FLTR_HANDLE(flow_ptr) ==
          PS_IFACE_IPFLTR_INVALID_HANDLE)
    {
      LOG_MSG_ERROR("Couldn't store Rx fltr spec in iface, 0x%x:%d, "
                "for flow 0x%p",
                iface_ptr->name, iface_ptr->instance, flow_ptr);
  ...
  if (PS_IFACEI_GET_NUM_SEC_FLOWS(iface_ptr) == PS_IFACE_MAX_SEC_FLOWS)
  {
    ...
    LOG_MSG_INFO1("Iface, 0x%x:%d, already has maximum possible number of "
             "ps_flows (%d). Can't add one more ps_flow",
             iface_ptr->name, iface_ptr->instance, PS_IFACE_MAX_SEC_FLOWS);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("QOS is not supported on default flow, 0x%p", flow_ptr, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_INFO1("Atmost only one QOS_MODIFY can be active on ps_flow, 0x%p",
             flow_ptr, 0, 0);
    *ps_errno = DS_EINVAL;
    return -1;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_INFO1("Invalid QOS_MODIFY spec is specified on ps_flow, 0x%p",
             flow_ptr, 0, 0);
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Couldn't store QOS_MODIFY spec in ps_flow, 0x%p", flow_ptr, 0, 0);
      ...
      PS_FLOWI_IS_MODIFY_TX_FLTR_SPEC_MODIFIED(flow_ptr))
  {
    ...
    if (PS_FLOWI_GET_MODIFY_TX_FLTR_HANDLE(flow_ptr) ==
          PS_IFACE_IPFLTR_INVALID_HANDLE)
    {
      LOG_MSG_ERROR("Couldn't store QOS_MODIFY Tx fltr spec in iface, 0x%x:%d, "
                "for ps_flow 0x%p",
                iface_ptr->name, iface_ptr->instance, flow_ptr);
  ...
  if (is_rx_fltr_modified &&
      PS_FLOWI_GET_MODIFY_RX_FLTR_HANDLE(flow_ptr) ==
        PS_IFACE_IPFLTR_INVALID_HANDLE)
  {
    LOG_MSG_ERROR("Couldn't store QOS_MODIFY Rx fltr spec in iface, 0x%x:%d, "
              "for ps_flow 0x%p",
              iface_ptr->name, iface_ptr->instance, flow_ptr);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Flow, 0x%p, doesn't belong to iface, 0x%x:%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_FLOWI_GET_CAPABILITY(flow_ptr, PS_FLOW_CAPABILITY_DEFAULT))
  {
    ...
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR("ps_flow 0x%p is not a default flow of iface 0x%x:%d",
                flow_ptr, iface_ptr->name, iface_ptr->instance);
      ASSERT(0);
      *ps_errno = DS_EFAULT;
      return -1;
  ...
  else
  {
    ...
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR("ps_flow, 0x%p, doesn't belong to iface, 0x%x:%d",
                flow_ptr, iface_ptr->name, iface_ptr->instance);
      ASSERT(0);
      *ps_errno = DS_EFAULT;
      return -1;
  ...
  LOG_MSG_INFO1("QOS MODIFY ACCEPTED on ps_flow, 0x%p", flow_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO1("QOS_MODIFY_REJECTED on ps_flow, 0x%p", flow_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr))
  {
    LOG_MSG_ERROR("Iface, 0x%p is invalid", iface_ptr, 0, 0);
    ASSERT(0);
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  *sec_flow_ptr = (ps_flow_type *) sec_flow_handle;
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Buffer (0x%p) doesn't belong to ps_flow's mem pool",
              sec_flow_handle, 0, 0);
    ASSERT(0);
    return FALSE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Filters are not installed on default flow, 0x%p",
              flow_ptr, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return -1;
  ...
  {
    LOG_MSG_ERROR("Unable to perform control operation on Tx filters on "
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("ps_flow, 0x%p is a default flow and it doesn't have "
              "Tx filters", flow_ptr, 0, 0);
    ASSERT(0);
    return -1;
  }
  ...
  -------------------------------------------------------------------------*/
  curr_fltr_buf = (ps_iface_ipfilteri_type *) tx_fltr_handle;
  fltr_q_ptr =
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(ps_iface_ptr))
  {
    LOG_MSG_ERROR("Invalid Iface ptr, 0x%p, is passed", ps_iface_ptr, 0, 0);
    ASSERT(0);
    return NULL;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Buffer (0x%X) doesn't belong to Tx filter's mem pool",
              tx_fltr_handle, 0, 0);
    ASSERT(0);
    return FALSE;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Invalid handle is passed", 0, 0, 0);
    ASSERT(0);
    return FALSE;
  }
...
/*===========================================================================
FUNCTION PS_IFACE_GET_TX_FLTR_HANDLE_EX()
...
DEPENDENCIES
  ...
  Sample usage to fetch Tx filters pertaining to QOS_REQUEST is
    TAKLOCK()
      handle = ps_iface_get_tx_fltr_handle_ex(iface_ptr, flow_ptr, FALSE)
      while (handle != NULL)
      {
        ps_iface_get_tx_fltr_by_handle_ex(iface_ptr, flow_ptr, FALSE,
                                       handle, fltr, prcd, new_handle)
        handle = new_handle
      }
    TASKFREE()
...
SIDE EFFECTS
  None
===========================================================================*/
void *ps_iface_get_tx_fltr_handle_ex
(
  ps_iface_type         * ps_iface_ptr,
  ps_flow_type          * flow_ptr,
  boolean                 is_modify
...
} /* ps_iface_get_tx_fltr_handle_ex() */


/*===========================================================================
...
/*===========================================================================
FUNCTION PS_IFACE_GET_TX_FLTR_BY_HANDLE_EX()
...
SIDE EFFECTS
  ...
  next Tx filter is fetched
===========================================================================*/
boolean ps_iface_get_tx_fltr_by_handle_ex
(
  ps_iface_type          * ps_iface_ptr,
  ps_flow_type           * flow_ptr,
  void                   * tx_fltr_handle,
...
} /* ps_iface_get_tx_fltr_by_handle_ex() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if(NULL == ip_fltr_spec)
  {
    LOG_MSG_ERROR("NULL ip_filter_spec",0,0,0);
    *ps_errno = DS_EINVAL;
    return FALSE;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Iface 0x%p invalid", iface_ptr, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EINVAL;
    return FALSE;
  }
  ...
  tx_fltr_cnt = ps_ifacei_get_tx_fltr_cnt_ex(iface_ptr, flow_ptr, is_modify);
  ...
  while ( tx_handle != NULL && num_fltrs > 0)
  {
    if (ps_ifacei_get_tx_fltr_by_handle_ex(iface_ptr,
                                          ...
                                          &new_handle) == FALSE)
    {
      LOG_MSG_INFO1("Could not get TX filter by handle for flow %d",
                                                 flow_ptr,0,0);
      return FALSE;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Null value passed to set_tx_filter_spec()",0,0,0);
    ASSERT(0);
    return FALSE;
  }
  ...
  /*-------------------------------------------------------------------------
    ...
    belong to given ps_flow
  -------------------------------------------------------------------------*/
  curr_tx_fltr_buf = (ps_iface_ipfilteri_type *) tx_fltr_handle;
  fltr_q_ptr =
  ...
  if (tx_fltr_cnt != fltr_cnt)
  {
    ...
    LOG_MSG_ERROR("The number of flows supplied 0x%d do not match with the flows"
              "stored in iface, 0x%d", ip_fltr_spec->num_filters, tx_fltr_cnt, 0);
    return FALSE;
  }

  curr_tx_fltr_buf = q_check(fltr_q_ptr);

  while (curr_tx_fltr_buf != NULL)
  {
    if (curr_tx_fltr_buf->fi_handle == tx_fltr_handle)
    {
      memcpy(&curr_tx_fltr_buf->filter,
...
/* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - */
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr))
  {
    LOG_MSG_ERROR("Invalid iface_id 0x%x: Invalid iface, 0x%p",
              iface_id, iface_ptr, 0);
    return NULL;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid iface_id 0x%x: flow cookie, %d too big",
              iface_id, flow_cookie, 0);
    flow_ptr = NULL;
  }
  else
  {
