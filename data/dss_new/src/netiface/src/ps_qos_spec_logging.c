...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_qos_spec_logging.c#1 $
  $Author: zhasan $ $DateTime: 2011/06/17 12:02:33 $
when        who    what, where, why
--------    ---    ----------------------------------------------------------
08/13/09    ar     Suppress routines if built on multiproc Apps image.
10/24/08    pp     Replaced snprintf w/ std_strlprintf.
...
---------------------------------------------------------------------------*/
#define QOS_REQUESTED_LOG_PKT_VER         3
#define QOS_GRANTED_LOG_PKT_VER           3
#define QOS_MODIFIED_LOG_PKT_VER          3
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (log_status(LOG_QOS_REQUESTED_EX_C))
  {
...
#else
    log_ptr = hput32(log_ptr, (uint32) flow_ptr);
    ...
    ret_val = std_strlcpy((char *) log_ptr,
                          net_iface,
