...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_aclrules.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_MED, "ACL PASS, pri %d ", acl_rule_val, 0, 0); 
  return;
} /* acl_trace_pass() */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  ACL_TRACE_DUMP(ACL_LEVEL_HIGH,                                            
                "Running ACL for Iface 0x%x:%d",                           
                ((ps_iface_type*)this_if_ptr)->name,                       
                ((ps_iface_type*)this_if_ptr)->instance, 0);               
  return;
} /* acl_trace_start()  */
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( IFACE_TYPE_IS_ID()  )
  {
    ...
    {
      ACL_TRACE_DUMP(ACL_LEVEL_MED,
                   "REQUIRED_IFACE_TYPE_IS: Iface ID 0x%x, Pol Iface ID "
                   "(Match)",
                   (dss_iface_id_type) ifname, 0, 0);
      return TRUE;
    }
  }
  else if ( IFACE_TYPE_IS_NAME()  )
  {
    ...
    {
      ACL_TRACE_DUMP(ACL_LEVEL_MED,
                   "REQUIRED_IFACE_TYPE_IS: Iface Name 0x%x, "
                   "Pol Iface Name (Match)", ifname, 0, 0);
      return TRUE;
    }
  }
  else if ( IFACE_TYPE_IS_GROUP(ifname)) 
  {
    if (policy_info_ptr->iface.info.name & (dss_iface_name_enum_type) ifname)
    {
      ACL_TRACE_DUMP(ACL_LEVEL_MED,
                   ...
                   "Pol Iface Group 0x%x (Match)",
                   (dss_iface_name_enum_type) ifname,
                    policy_info_ptr->iface.info.name, 0);
      return TRUE;
    }
  }
  else
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if( USE_POL_INFO( policy_info_ptr->ip_family == (unsigned int)ipfam) )
  {
    ACL_TRACE_DUMP(ACL_LEVEL_MED,
                   "IP_POL_FAMILY_IS: IP Pol Family %d, IP family, (Match)",
                   policy_info_ptr->ip_family, 0, 0);
    return TRUE;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
       ...
       ps_iface_addr_family_is_v4((ps_iface_type*) this_if_ptr)))
  {
    ACL_TRACE_DUMP(ACL_LEVEL_MED,
                   ...
                   "Iface Family %d Pol IP Family, (Match)",
                   if_family, 0, 0);
    return TRUE;
  }
    ...
    if ((uint16)policy_info_ptr->iface.info.name & (uint16)IFACE_MASK)
    {
          ...
          ((ps_iface_type*) this_if_ptr)->name)
      {
        ACL_TRACE_DUMP(ACL_LEVEL_MED,
                       ...
                       "Pol iface name 0x%x iface name, (Match)",
                       policy_info_ptr->iface.info.name, 0, 0);
        return TRUE;
      }
    }
  }
