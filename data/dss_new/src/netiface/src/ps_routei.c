...
$Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_routei.c#2 $ $DateTime: 2011/06/27 12:39:51 $ $Author: smudired $
...
)
{
  ...
  switch( rt_type )
  {
    ...
    default:
      LOG_MSG_FATAL_ERROR( "RT: Unknown route type %d", rt_type, 0, 0 );
      return -1;
  }
...
)
{
  ...
  {
    LOG_MSG_ERROR( "RTI: Del index too large (%d/%d)",
...
int routei_check_v4_table( void )
{
  ...
  for ( i = (int)routei_v4_routing_table.table_size_used - 1; i >= 0; i-- )
  {
    ...
    {
      LOG_MSG_ERROR( "RTv4: NULL iface in entry %d", i, 0, 0 );
      break;
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == if_ptr)
  {
    LOG_MSG_ERROR("routei_acl_post_processing(): got NULL Interface pointer ",
                  0, 0, 0);
    ASSERT(0);
    return;
  }

  LOG_MSG_INFO2("routei_acl_post_processing(): Iface 0x%x::%d",
  ...
  if (PS_IFACE_GET_PROC_ID(if_ptr) == PS_IFACE_PROC_ID_NONE)
  {
    ...
    {
      LOG_MSG_ERROR("routei_acl_post_processing(): Got NULL policy_info_ptr",
                    0, 0, 0);
      ASSERT(0);
    }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR("routei_acl_get_best_match(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return FALSE;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR("routei_acl_update_local_srch_results(): NULL list pointer ",
              0, 0, 0);
    ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( (NULL == item_ptr) ||
      (NULL == compare_val) )
  {
    LOG_MSG_ERROR("routei_acl_compare_rt_less(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return 0;
  }
...
/*-------------------------------------------------------------------------*/
  ...
  {
    LOG_MSG_ERROR("routei_acl_add_result_list(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( (NULL == acl_ptr) ||
       (NULL == acl_match_list_ptr) )
  {
    LOG_MSG_ERROR("routei_acl_simple_update_result(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  else
  {
    ...
    {
      LOG_MSG_ERROR( "routei_acl_simple_update_result(): NULL ACL rt pointer ",
                 0, 0, 0);
      ASSERT(0);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  if ( (NULL == item_ptr) ||
       (NULL == compare_val) )
  {
    LOG_MSG_ERROR("routei_acl_compare_rt_iface_class(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return -1;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ( (NULL == acl_ptr) ||
       (NULL == acl_match_list_ptr) )
  {
    LOG_MSG_ERROR("routei_acl_get_same_class_result(): NULL pointer ",
              0, 0, 0);
    ASSERT(0);
    return NULL;
  }
  ...
  if (NULL == if_ptr)
  {
    LOG_MSG_ERROR("routei_acl_get_same_class_result(): NULL Interface pointer ",
              0, 0, 0);
    ASSERT(0);
    return NULL;
  }
