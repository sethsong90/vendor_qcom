...
Copyright (c) 2005-2008 Qualcomm Technologies, Inc.
All Rights Reserved.
...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_iface_rx_qos_fltr.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  while (fltr_buf_ptr != NULL)
  {
    ...
    {
      LOG_MSG_ERROR("Couldn't create a buffer for Rx filter on iface 0x%x:%d",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("qos_info_ptr is NULL", 0, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  }
  ...
  {
    LOG_MSG_ERROR("Fltr priority style, %d, is not supported",
              fltr_priority, 0, 0);
    ASSERT(0);
    *ps_errno = DS_EFAULT;
    return PS_IFACE_IPFLTR_INVALID_HANDLE;
  ...
  LOG_MSG_INFO2("Adding pointers to Rx fltrs to iface 0x%x:%d",
          iface_ptr->name, iface_ptr->instance, 0);
  ...
  else
  {
    ...
    else /* Wrap around happened */
    {
      ...
      if (filter_node_ptr == NULL)
      {
        ...
        LOG_MSG_ERROR("Couldn't find a hole in precedence space iface 0x%x:%d",
                  iface_ptr->name, iface_ptr->instance, 0);
        ASSERT(0);
        *ps_errno = DS_ENOMEM;
        return PS_IFACE_IPFLTR_INVALID_HANDLE;
  ...
  LOG_MSG_INFO2("Assigning fltr precedence from %d", fltr_prcd, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Modifying pointers to Rx fltrs on iface 0x%x:%d",
          iface_ptr->name, iface_ptr->instance, 0);
  ...
  if (filter_node_ptr == NULL)
  {
    LOG_MSG_ERROR("Couldn't find filters to modify on iface 0x%x:%d. Passed "
              "fltr_handle is 0x%x",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("Deleting pointers to Rx fltrs on iface 0x%x:%d",
          iface_ptr->name, iface_ptr->instance, 0);
