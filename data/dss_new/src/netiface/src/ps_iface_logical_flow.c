...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_iface_logical_flow.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  switch (ioctl_name)
  {
    case PS_IFACE_IOCTL_QOS_REQUEST:
    {
      ...
      {
        LOG_MSG_ERROR("NULL QoS spec", 0, 0, 0);
        ASSERT(0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
    ...
    case PS_IFACE_IOCTL_QOS_REQUEST_EX:
    {
      ...
      {
        LOG_MSG_ERROR("Too many requests per IOCTL %d, MAX=%d",
                  qos_request_ex_info_ptr->num_qos_specs,
                  MAX_QOS_OPERATIONS_PER_IOCTL, 0);
        ASSERT(0);
        *ps_errno = DS_EFAULT;
        return -1;
      }
    ...
    default:
    {
      LOG_MSG_ERROR("Invalid ioctl 0x%x", ioctl_name, 0, 0);
      ASSERT(0);
      *ps_errno = DS_EFAULT;
      return -1;
    }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Iface 0x%x:%d not logical",
  ...
  for (cnt = 0; cnt < num_qos_flows; cnt++)
  {
    ...
    if (ret_val != 0)
    {
      LOG_MSG_ERROR ("Couldn't create ps_flow, err %d", *ps_errno, 0, 0 );
      break;
    }
    ...
    {
      LOG_MSG_ERROR ("Couldn't activate/configure ps_flow 0x%x, err %d",
                 out_flow_ptr_ptr[cnt], *ps_errno, 0 );
    ...
    if (default_flow_meta_info_ptr != NULL)
    {
      ...
      {
        LOG_MSG_ERROR("Couldn't alloc metainfo", 0, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ret_val != 0)
  {
    LOG_MSG_ERROR ("Couldn't modify ps_flow 0x%x, err %d",
               modify_flow_ptr, *ps_errno, 0);
    return -1;
  }
