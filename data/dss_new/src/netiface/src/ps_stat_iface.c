...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_stat_iface.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid iface_ptr 0x%p", iface_ptr, 0, 0);
    return 0;
  }

  if (NULL == log_pkt_ptr)
  {
    LOG_MSG_ERROR("Null log packet", 0, 0, 0);
    return 0;
  }
  ...
  switch(iface_ptr->name)
  {
    ...
    default:
      LOG_MSG_ERROR("Invalid iface", 0, 0, 0);
      return 0;
  } /* switch */

  (void) std_strlcpy(log_pkt_fill_ptr, name_ptr, strlen(name_ptr) + 1);

  return strlen(name_ptr) + 1;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid stat %d", stat, 0, 0);
    ASSERT(0);
  ...
  if (iface_ptr == NULL)
  {
    LOG_MSG_ERROR("NULL iface", 0, 0, 0);
    ASSERT(0);
    return E_NOT_ALLOWED ;
  }
  ...
  {
    LOG_MSG_ERROR("NULL return_value", 0, 0, 0);
    return E_NOT_ALLOWED ;
  }
  ...
  timestamp = (uint32)msclock();

  bytes_temp = iface_ptr->iface_i_stats.bytes_rx +
               iface_ptr->iface_i_stats.bytes_tx -
               iface_ptr->iface_i_stats.bytes_transferred;

  iface_i_stats->total_open_time_since_power_up  =
    iface_ptr->iface_i_stats.total_open_time_since_power_up +
      ((0 == iface_ptr->iface_i_stats.last_up_state_timestamp)
        ? 0 : (timestamp - iface_ptr->iface_i_stats.last_up_state_timestamp)
      );

  iface_i_stats->total_active_time_since_power_up =
    iface_ptr->iface_i_stats.total_active_time_since_power_up +
    timestamp - iface_ptr->iface_i_stats.first_io_timestamp;

  iface_i_stats->total_bytes_transferred_since_power_up      =
    iface_ptr->iface_i_stats.total_bytes_transferred_since_power_up +
    ((0 == iface_ptr->iface_i_stats.bytes_transferred)
      ? 0 : bytes_temp
    );

  iface_i_stats->first_io_timestamp =
    iface_ptr->iface_i_stats.first_io_timestamp;

  iface_i_stats->bytes_transferred  =
    iface_ptr->iface_i_stats.bytes_transferred;

  iface_i_stats->last_up_state_timestamp        =
    ((iface_ptr->iface_i_stats.last_up_state_timestamp == 0)
      ? (timestamp - iface_ptr->iface_i_stats.last_up_state_timestamp) : 0) / 1000;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (FALSE == PS_IFACE_IS_VALID(iface_ptr))
  {
    LOG_MSG_ERROR("Invalid iface cb 0x%p", iface_ptr, 0, 0);
    return desc_len;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (NULL == log_pkt_ptr)
  {
    LOG_MSG_ERROR("NULL log pkt", 0, 0, 0);
    ASSERT(0);
    return;
  }
  ...
  while (NULL != iface_mcast_grp_ptr)
  {
    ...
    memcpy(log_pkt_fill_ptr, &iface_mcast_v6_addr, sizeof(struct ps_in6_addr));
  ...
  iface_name_size = ps_stat_fill_iface_name(iface_ptr, log_pkt_fill_ptr);
  ...
  memcpy(iface_desc_ptr->hw_addr,
         iface_ptr->iface_private.hw_addr,
         PS_IFACE_MAX_HW_ADDR_LEN);

  iface_desc_ptr->mtu           = iface_ptr->net_info.mtu;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
         ...
         ++idx)
    {
      ...
      {
       iface_ptr->ps_iface_stat_fill_desc_f_ptr(iface_ptr, log_pkt_fill_ptr);
      }
      else
      {
        ps_stat_fill_iface_inst_desc_default_cback(iface_ptr, log_pkt_fill_ptr);
      }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx = 0; idx < MAX_SYSTEM_IFACES; ++idx)
    {
      ...
      if (NULL == log_pkt_ptr)
      {
        LOG_MSG_ERROR("Log packet allocated for IFACE failed", 0, 0, 0);
        return;
      }
  ...
  else
  {
    ...
    if (NULL == log_pkt_ptr)
    {
      LOG_MSG_ERROR("Log pkt alloc failed", 0, 0, 0);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*---------------------------------------------------------------------
    There is no valid stats for this handle, so do nothing
  ---------------------------------------------------------------------*/
  if (IFACE_UP != PS_IFACEI_GET_STATE(iface_cb_ptr))
  {
    LOG_MSG_ERROR("Attempting to reset statistics on an interface that is down. "
              "Handle: %d", handle, 0, 0);
    return;
  }

  if (PS_STAT_QUERIED == iface_cb_ptr->iface_i_stats.queried)
  {
    ps_stat_get_iface_stat_log_pkt(handle);
  }

  memset(&(iface_cb_ptr->iface_i_stats), 0,
         sizeof(iface_cb_ptr->iface_i_stats));

  if (iface_cb_ptr->ps_iface_stat_override_f_ptr != NULL)
  {
    LOG_MSG_INFO1("Resetting MH Stats Overrides", 0, 0, 0);
    iface_cb_ptr->ps_iface_stat_override_f_ptr(iface_cb_ptr, NULL,
                                            PS_IFACE_STAT_OP_TYPE_RESET);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Attempting to reset statistics on an interface that is down. "
              "Iface_ptr: 0x%p", iface_ptr, 0, 0);
    return;
  }

  memset(&(iface_ptr->iface_i_stats), 0,
         sizeof(iface_ptr->iface_i_stats));

  if (iface_ptr->ps_iface_stat_override_f_ptr != NULL)
  {
    LOG_MSG_INFO1("Resetting MH Stats Overrides", 0, 0, 0);
    iface_ptr->ps_iface_stat_override_f_ptr(iface_ptr, NULL,
                                            PS_IFACE_STAT_OP_TYPE_RESET);
  }
...
)
{
  ...
  ps_iface_stat_override_type      stat_override;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (!PS_IFACE_IS_VALID(iface_ptr))
  {
    LOG_MSG_ERROR("ps_stat_get_iface(): Invalid iface 0x%x", iface_ptr, 0, 0);
    return E_INVALID_ARG;
  }

  LOG_MSG_INFO1("ps_stat_get_iface(): iface 0x%x:%d stat %d",
                iface_ptr->name, iface_ptr->instance, stat);

  if (iface_ptr->ps_iface_stat_override_f_ptr != NULL)
  {
    LOG_MSG_INFO1("ps_stat_get_iface(): Getting Stats Overrides", 0, 0, 0);
    iface_ptr->ps_iface_stat_override_f_ptr(iface_ptr,
                                            &stat_override,
                                            PS_IFACE_STAT_OP_TYPE_GET);
    switch (stat)
    {
      case PS_STAT_IFACE_ALL:
        iface_i_stats = (ps_stat_iface_i_s_type *) return_value;
        memcpy(iface_i_stats, &stat_override, sizeof(ps_stat_iface_i_s_type));
        break;

      case PS_STAT_IFACE_PKTS_RX:
        (*(uint32 *)return_value) = stat_override.pkts_rx;
        break;

      case PS_STAT_IFACE_PKTS_TX:
        (*(uint32 *)return_value) = stat_override.pkts_tx;
        break;

      case PS_STAT_IFACE_BYTES_RX:
        (*(uint64 *)return_value) = stat_override.bytes_rx;
        break;

      case PS_STAT_IFACE_BYTES_TX:
        (*(uint64 *)return_value) = stat_override.bytes_tx;
        break;
  ...
  {
    iface_ptr       = assoc_iface_ptr;
    assoc_iface_ptr = PS_IFACEI_GET_ASSOC_IFACE(iface_ptr);
  }

  if (iface_ptr->ps_iface_stat_get_adjustment_f_ptr != NULL)
  {
    LOG_MSG_INFO1("ps_stat_get_iface(): Getting stat adjustment from "
                  "iface 0x%x:%d", iface_ptr->name, iface_ptr->instance, 0);
    ...
    /*-------------------------------------------------------------------------
      Update results with adjustment
    -------------------------------------------------------------------------*/
    switch (stat)
    {
      case PS_STAT_IFACE_ALL:
        ...
        iface_i_stats->pkts_rx  += stat_adjustment.pkts_rx;
        iface_i_stats->pkts_tx  += stat_adjustment.pkts_tx;
        iface_i_stats->bytes_rx += stat_adjustment.bytes_rx;
        iface_i_stats->bytes_tx += stat_adjustment.bytes_tx;
        break;

      case PS_STAT_IFACE_PKTS_RX:
        (*(uint32 *)return_value) += stat_adjustment.pkts_rx;
        break;

      case PS_STAT_IFACE_PKTS_TX:
        (*(uint32 *)return_value) += stat_adjustment.pkts_tx;
        break;

      case PS_STAT_IFACE_BYTES_RX:
        (*(uint64 *)return_value) += stat_adjustment.bytes_rx;
        break;

      case PS_STAT_IFACE_BYTES_TX:
        (*(uint64 *)return_value) += stat_adjustment.bytes_tx;
        break;
