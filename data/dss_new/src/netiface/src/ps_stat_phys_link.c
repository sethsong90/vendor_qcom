...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_stat_phys_link.c#1 $ $DateTime: 2011/06/17 12:02:33 $ $Author: zhasan $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid stat value passed to "
              "ps_stat_get_phys_link_i_all",0,0,0);
    ASSERT(0);
  ...
  {
    LOG_MSG_ERROR("ps_stat_get_phys_link_is_primary: "
               "Null value of phys_link_ptr passed",0,0,0);
    ASSERT(0);
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (phys_link_ptr == NULL)
  {
    LOG_MSG_ERROR("ps_stat_get_phys_link_instance_all: "
              "Null value of phys_link_ptr passed",0,0,0);
    ASSERT(0);
    return E_NOT_ALLOWED ;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("Invalid handle to iface instance.  Handle: %d", handle, 0, 0);
  ...
  if (NULL == iface_ptr)
  {
    LOG_MSG_ERROR("Attempting to reset statistics on an invalid iface instance. "
              "Handle: %d", handle, 0, 0);
    return E_INVALID_ARG;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid handle to phys_link instance.  Handle: %d",
               handle, 0, 0);
    return E_INVALID_ARG;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    if (E_SUCCESS != ret)
    {
      LOG_MSG_ERROR("Unable to get phys link ctl blk for get stats.  Handle: %d",
                 handle, 0, 0);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (PS_STAT_INST_DESC_ALL == handle)
  {
    for (idx_iface = 0; idx_iface < MAX_SYSTEM_IFACES; ++idx_iface)
    {
           ...
           ++idx_phys_link)
      {
        ...
        {
          LOG_MSG_ERROR("Null phys link ptr", 0, 0, 0);
          return;
        }
        ...
        if (NULL == log_pkt_ptr)
        {
          LOG_MSG_ERROR("Log pkt alloc for phys link get stats failed",
                     0, 0, 0);
          return;
        }
  ...
  else
  {
    ...
    if (E_SUCCESS != ret)
    {
      LOG_MSG_ERROR("Unable to get phys link ctl blk for get stats %d",
                 handle, 0, 0);
      return;
    }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (ret != E_SUCCESS)
  {
    LOG_MSG_ERROR("Unable to get phys_link control block for reset.  Handle: %d",
               handle, 0, 0);
    return;
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR(" Instance pointer should NOT be NULL "
                "for PHYS_LINK stats",0,0,0);
    return E_INVALID_ARG ;
  }
