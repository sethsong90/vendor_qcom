...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_policy_mgr.c#1 $
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_FATAL_ERROR("Can't init Policy module, pool id %d",
              (int32)PS_MEM_PS_POLICY_TYPE, 0, 0);
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    /* Bogus inputs */
    LOG_MSG_INFO2("Bad inputs", 0, 0, 0);
  ...
  LOG_MSG_INFO1( "get priority: req - app_id [0x%x%x], profile [0x%x]",
  ...
  while (this_policy_ptr != NULL)
  {
    ...
    tmp_app_id_l = (uint32)(tmp_app_id & 0xFFFFFFFF);
    tmp_app_id_u = (uint32)(tmp_app_id >> 32);
    LOG_MSG_INFO1( "get priority: looking - app_id [0x%x%x], profile [0x%x]",
                   tmp_app_id_u, tmp_app_id_l, this_policy_ptr->profile_id );
    LOG_MSG_INFO1( "get priority: looking - prio [%d] --- pos [%d]", 
	            this_policy_ptr->priority, this_policy_ptr->position, 0 );
           ...
           this_policy_ptr->app_identifier == PS_POLICY_MGR_ANY_APP ) )
    {
      ...
      LOG_MSG_INFO1( "get priority: found - app_id [0x%x%x], profile [0x%lx]",
                      tmp_app_id_u, tmp_app_id_l, this_policy_ptr->profile_id );
      if ( priority >= max_prio )
      {
        ...
        LOG_MSG_INFO1( "get priority: found - new max prio [0x%x], app_id [0x%x%x]",
  ...
  if ( !match )
  {
    ...
    LOG_MSG_INFO1( "get priority: NO MATCH, app_id [0x%x%x], profile [0x%x], treat as legacy",
                    tmp_app_id_u, tmp_app_id_l, profile_id );  
  }
  else 
  {
    LOG_MSG_INFO1( "get priority: MATCH, app_id [0x%x%x], prio [0x%x]",
                    tmp_app_id_u, tmp_app_id_l, max_prio);
  }

  PS_LEAVE_CRIT_SECTION(&global_ps_crit_section);

  LOG_MSG_INFO2( "policy mgr lookup got prf [%d], prio [%d]", 
            prf_id, max_prio, 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if ((priority > PS_POLICY_MGR_PRIORITY_MAX) ||
      (priority <= PS_POLICY_MGR_PRIORITY_INVALID))
  {
    LOG_MSG_ERROR("Invalid priority %d", priority, 0, 0);
    return -1;
  }
  ...
  if (policy_list_size >= PS_POLICY_MGR_MAX_POLICY_ENTRIES )
  {
    ...
    LOG_MSG_ERROR("DB is full", 0, 0, 0);
    return -1;
  }
  ...
  {
    PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
    LOG_MSG_ERROR("Out of mem", 0, 0, 0);
    return -1;
  }
  ...
  tmp_app_id_l = (uint32)(tmp_app_id & 0xFFFFFFFF);
  tmp_app_id_u = (uint32)(tmp_app_id >> 32);
  LOG_MSG_INFO2( "add policy: app_id [0x%x%x], profile [0x%x]",
                  tmp_app_id_u, tmp_app_id_l, profile_id );
  LOG_MSG_INFO2( "add policy: prio [%d] --- pos [%d]", priority, tmp_rec_pos, 0 );
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_ERROR("get_profile_cback not set", 0, 0, 0);
