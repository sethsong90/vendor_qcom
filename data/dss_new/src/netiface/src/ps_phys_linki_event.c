...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/netiface/rel/11.03/src/ps_phys_linki_event.c#1 $
...
#define PS_PHYS_LINK_EVT_BUF_NUM          270
#define PS_PHYS_LINK_EVT_BUF_HIGH_WM      230
#define PS_PHYS_LINK_EVT_BUF_LOW_WM        90

#else

#ifdef FEATURE_DATAMODEM_DUAL_PROC_MODEM_EHRPD
#define PS_PHYS_LINK_EVT_BUF_NUM          250
#define PS_PHYS_LINK_EVT_BUF_HIGH_WM      220
#define PS_PHYS_LINK_EVT_BUF_LOW_WM        90
#else

#define PS_PHYS_LINK_EVT_BUF_NUM          220
#define PS_PHYS_LINK_EVT_BUF_HIGH_WM      180
#define PS_PHYS_LINK_EVT_BUF_LOW_WM        90
...
----------------------------------------------------------------------------*/
static int ps_phys_link_evt_buf_mem[PS_MEM_GET_TOT_SIZE_OPT
                                    (
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  /*-------------------------------------------------------------------------
    Initialize Pools
  -------------------------------------------------------------------------*/
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_PS_PHYS_LINK_EVT_CB_TYPE,
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  {
    LOG_MSG_INFO1("Invalid phy link buf 0x%p to reg evt %d",
             buf, event, 0);
    return -1;
  ...
  {
    LOG_MSG_INFO1("Not re-q'ing phys link buf 0x%p, evt %d",
             buf, event, 0);
    return -1;
  }
  ...
  {
    LOG_MSG_ERROR("Invalid phys link, 0x%p, is passed", this_phys_link_ptr, 0, 0);
    return -1;
  }

  PS_ENTER_CRIT_SECTION(&global_ps_crit_section);

  buf_ptr->event_q_ptr = (this_phys_link_ptr->phys_private.event_q_array) +
                         event;
  q_put((this_phys_link_ptr->phys_private.event_q_array) + event,
        &(buf_ptr->link));
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  else
  {
    ...
    ASSERT(0);
    LOG_MSG_ERROR("Invalid phys_link 0x%p is passed", this_phys_link_ptr, 0, 0);
    return;
  }
  ...
  {
    ASSERT(0);
    LOG_MSG_ERROR("Buf 0x%p (ev %d) not reg 0x%p phys_link",
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("PHYS LINK DOS ACK IND 0x%p", phys_link_ptr, 0, 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("PHYS LINK DOWN IND 0x%p, state %d",
          this_phys_link_ptr, PS_PHYS_LINKI_GET_STATE(this_phys_link_ptr), 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("PHYS LINK UP IND 0x%p, state %d",
          this_phys_link_ptr, PS_PHYS_LINKI_GET_STATE(this_phys_link_ptr), 0);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  LOG_MSG_INFO2("PHYS LINK GONE IND 0x%p, state %d",
          this_phys_link_ptr, PS_PHYS_LINKI_GET_STATE(this_phys_link_ptr), 0);
                                   ...
                                   PS_PHYS_LINK_CAPABILITY_FLOW_DECOUPLED))
  {
    ...
    {
      PS_BRANCH_LEAVE_CRIT_SECTION(&global_ps_crit_section);
      LOG_MSG_ERROR("Flows are still bound to phys link, 0x%p",
                this_phys_link_ptr, 0, 0);
      ASSERT(0);
      return;
    }
  }
