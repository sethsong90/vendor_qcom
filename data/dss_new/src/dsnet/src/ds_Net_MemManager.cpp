...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/dsnet/rel/11.03/src/ds_Net_MemManager.cpp#1 $
...
#define NETWORK_SIZE             ((sizeof(Network) + 3) & ~3)
...
#define QOS_JSON_SIZE            ((sizeof(QoSJson) + 3) & ~3)
#define QOS_DEFAULT_SIZE         ((sizeof(QoSDefault) + 3) & ~3)
#define QOS_DEFAULT_JSON_SIZE    ((sizeof(QoSDefaultJson) + 3) & ~3)
#define QOS_SECONDARY_SIZE       ((sizeof(QoSSecondary) + 3) & ~3)
#define QOS_SECONDARY_JSON_SIZE  ((sizeof(QoSSecondaryJson) + 3) & ~3)
...
#define QOS_MANAGER_JSON_SIZE    ((sizeof(QoSManagerJson) + 3) & ~3)
...
#define NETWORK_FACTORY_CLIENT_SIZE  ((sizeof(NetworkFactoryClient) + 3) & ~3)
...
#define QOS_SECONDARIES_INPUT_SIZE   ((sizeof(QoSSecondariesOutput) + 3) & ~3)
#define QOS_SECONDARIES_OUTPUT_SIZE   ((sizeof(QoSSecondariesInput) + 3) & ~3)
...
#define NETWORK_NUM_BUFS         (MAX_NETWORK_OBJS)
#define NETWORK_1X_NUM_BUFS      (MAX_NETWORK_1X_OBJS)
#define NETWORK_FACTORY_NUM_BUFS (1)
#define NETWORK_IPV6_NUM_BUFS    (MAX_NETWORK_OBJS)
...
#define QOS_JSON_NUM_BUFS        (MAX_QOS_DEFAULT_OBJS)
#define QOS_DEFAULT_NUM_BUFS     (MAX_QOS_DEFAULT_OBJS)
#define QOS_DEFAULT_JSON_NUM_BUFS     (MAX_QOS_DEFAULT_OBJS)
#define QOS_SECONDARY_NUM_BUFS   (MAX_QOS_SECONDARY_OBJS)
#define QOS_SECONDARY_JSON_NUM_BUFS   (MAX_QOS_SECONDARY_OBJS)
#define IPV6_ADDRESS_NUM_BUFS    (20)
#define TECH_UMTS_FACTORY_NUM_BUFS (MAX_NETWORK_UMTS_OBJS)
#define BEARER_TECH_NUM_BUFS     (20)
#define MCAST_MANAGER_NUM_BUFS   (MAX_NETWORK_OBJS)
#define MCAST_MANAGER_PRIV_NUM_BUFS  (MAX_NETWORK_OBJS)
#define QOS_MANAGER_NUM_BUFS     (MAX_NETWORK_OBJS)
#define QOS_MANAGER_JSON_NUM_BUFS     (MAX_NETWORK_OBJS)
...
#define IP_FILTER_REG_NUM_BUFS   (MAX_NETWORK_OBJS)
#define MBMS_JOIN_INFO_NUM_BUFS  (10)
#define MTPD_REG_NUM_BUFS        (10)
#define BCMCS_UPD_PARAM_NUM_BUFS (10)
#define NETWORK_FACTORY_CLIENT_NUM_OF_BUFS (MAX_NETWORK_OBJS)
...
#define NETWORK_HIGH_WM          (NETWORK_NUM_BUFS - 1)
...
#define QOS_JSON_HIGH_WM         (QOS_JSON_NUM_BUFS - 2)
#define QOS_DEFAULT_HIGH_WM      (QOS_DEFAULT_NUM_BUFS - 2)
#define QOS_DEFAULT_JSON_HIGH_WM      (QOS_DEFAULT_JSON_NUM_BUFS - 2)
#define QOS_SECONDARY_HIGH_WM    (MAX_QOS_SECONDARY_OBJS - 1)
#define QOS_SECONDARY_JSON_HIGH_WM    (MAX_QOS_SECONDARY_OBJS - 1)
...
#define QOS_MANAGER_JSON_HIGH_WM      (QOS_MANAGER_JSON_NUM_BUFS - 3)
...
#define IP_FILTER_REG_HIGH_WM    (MAX_NETWORK_OBJS - 2)
#define MBMS_JOIN_INFO_HIGH_WM   (8)
#define MTPD_REG_HIGH_WM         (8)
#define NETWORK_FACTORY_CLIENT_HIGH_WM (NETWORK_FACTORY_CLIENT_NUM_OF_BUFS - 1)
...
#define QOS_JSON_LOW_WM         (1)
#define QOS_DEFAULT_LOW_WM       (1)
#define QOS_DEFAULT_JSON_LOW_WM       (1)
#define QOS_SECONDARY_LOW_WM     (1)
#define QOS_SECONDARY_JSON_LOW_WM     (1)
...
#define QOS_MANAGER_JSON_LOW_WM       (1)
...
#define NETWORK_FACTORY_CLIENT_LOW_WM (0)
...
static int network_buf [PS_MEM_GET_TOT_SIZE_OPT(NETWORK_NUM_BUFS,
                                                NETWORK_SIZE)];
...
static int qos_json_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_JSON_NUM_BUFS,
                                                 QOS_JSON_SIZE)];

static int qos_default_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_DEFAULT_NUM_BUFS,
                                                    QOS_DEFAULT_SIZE)];

static int qos_default_json_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_DEFAULT_JSON_NUM_BUFS,
                                                    QOS_DEFAULT_JSON_SIZE)];

static int qos_secondary_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_SECONDARY_NUM_BUFS,
                                                      QOS_SECONDARY_SIZE)];

static int qos_secondary_json_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_SECONDARY_JSON_NUM_BUFS,
                                                      QOS_SECONDARY_JSON_SIZE)];
...
static int qos_manager_json_buf [PS_MEM_GET_TOT_SIZE_OPT(QOS_MANAGER_JSON_NUM_BUFS,
                                                    QOS_MANAGER_JSON_SIZE)];
...
static int network_factory_client_buf [PS_MEM_GET_TOT_SIZE_OPT(NETWORK_FACTORY_CLIENT_NUM_OF_BUFS,
                                                               NETWORK_FACTORY_CLIENT_SIZE)];
...
static int net_nat_session_buf [PS_MEM_GET_TOT_SIZE_OPT(NET_NAT_SESSION_NUM_BUFS,
                                                      NET_NAT_SESSION_SIZE)];

static int net_nat_manager_buf [PS_MEM_GET_TOT_SIZE_OPT(NET_NAT_MANAGER_NUM_BUFS,
                                                      NET_NAT_MANAGER_SIZE)];
...
static ps_mem_buf_hdr_type   * network_buf_hdr_ptr[NETWORK_NUM_BUFS];
static Network               * network_buf_ptr[NETWORK_NUM_BUFS];
...
static ps_mem_buf_hdr_type   * qos_json_buf_hdr_ptr[QOS_JSON_NUM_BUFS];
static QoSJson               * qos_json_buf_ptr[QOS_JSON_NUM_BUFS];
...
static ps_mem_buf_hdr_type   * qos_default_json_buf_hdr_ptr[QOS_DEFAULT_JSON_NUM_BUFS];
static QoSDefaultJson        * qos_default_json_buf_ptr[QOS_DEFAULT_JSON_NUM_BUFS];
...
static ps_mem_buf_hdr_type   * qos_secondary_json_buf_hdr_ptr[QOS_SECONDARY_JSON_NUM_BUFS];
static QoSSecondaryJson      * qos_secondary_json_buf_ptr[QOS_SECONDARY_JSON_NUM_BUFS];
...
static ps_mem_buf_hdr_type   * qos_manager_json_buf_hdr_ptr[QOS_MANAGER_JSON_NUM_BUFS];
static QoSManagerJson        * qos_manager_json_buf_ptr[QOS_MANAGER_JSON_NUM_BUFS];
...
static ps_mem_buf_hdr_type   * network_factory_client_buf_hdr_ptr[NETWORK_FACTORY_CLIENT_NUM_OF_BUFS];
static NetworkFactoryClient  * network_factory_client_buf_ptr[NETWORK_FACTORY_CLIENT_NUM_OF_BUFS];
...
/*-------------------------------------------------------------------------*/
  ...
  LOG_MSG_INFO1 ("Initing mem pools for DSNet objects", 0, 0, 0);
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_NETWORK,
                           network_buf,
                           NETWORK_SIZE,
                           NETWORK_NUM_BUFS,
                           NETWORK_HIGH_WM,
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_QOS_JSON,
                           qos_json_buf,
                           QOS_JSON_SIZE,
                           QOS_JSON_NUM_BUFS,
                           QOS_JSON_HIGH_WM,
                           ...
                           NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                           (int *) qos_json_buf_hdr_ptr,
                           (int *) qos_json_buf_ptr
#else
                           NULL,
                           NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_QOS_DEFAULT_JSON,
                           qos_default_json_buf,
                           QOS_DEFAULT_JSON_SIZE,
                           QOS_DEFAULT_JSON_NUM_BUFS,
                           QOS_DEFAULT_JSON_HIGH_WM,
                           QOS_DEFAULT_JSON_LOW_WM,
                           NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                           (int *) qos_default_json_buf_hdr_ptr,
                           (int *) qos_default_json_buf_ptr
#else
                           NULL,
                           NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_QOS_SECONDARY_JSON,
                           qos_secondary_json_buf,
                           QOS_SECONDARY_JSON_SIZE,
                           QOS_SECONDARY_JSON_NUM_BUFS,
                           QOS_SECONDARY_JSON_HIGH_WM,
                           QOS_SECONDARY_JSON_LOW_WM,
                           NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                           (int *) qos_secondary_json_buf_hdr_ptr,
                           (int *) qos_secondary_json_buf_ptr
#else
                           NULL,
                           NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_QOS_MANAGER_JSON,
                       qos_manager_json_buf,
                       QOS_MANAGER_JSON_SIZE,
                       QOS_MANAGER_JSON_NUM_BUFS,
                       QOS_MANAGER_JSON_HIGH_WM,
                       QOS_MANAGER_JSON_LOW_WM,
                       NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                       (int *) qos_manager_json_buf_hdr_ptr,
                       (int *) qos_manager_json_buf_ptr
#else
                       NULL,
                       NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_NETWORK_FACTORY_CLIENT_TYPE,
                            network_factory_client_buf,
                            NETWORK_FACTORY_CLIENT_SIZE,
                            NETWORK_FACTORY_CLIENT_NUM_OF_BUFS,
                            NETWORK_FACTORY_CLIENT_HIGH_WM,
                            NETWORK_FACTORY_CLIENT_LOW_WM,
                            NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                            (int *) network_factory_client_buf_hdr_ptr,
                            (int *) network_factory_client_buf_ptr
#else
                            NULL,
                            NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_NAT_SESSION,
                       net_nat_session_buf,
                       NET_NAT_SESSION_SIZE,
                       NET_NAT_SESSION_NUM_BUFS,
                       NET_NAT_SESSION_HIGH_WM,
                       NET_NAT_SESSION_LOW_WM,
                       NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                       (int *) net_nat_session_buf_hdr_ptr,
                       (int *) net_nat_session_buf_ptr
#else
                       NULL,
                       NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
  ...
  if (PS_MEM_POOL_INIT_OPT(PS_MEM_DS_NET_NAT_MANAGER,
                       net_nat_manager_buf,
                       NET_NAT_MANAGER_SIZE,
                       NET_NAT_MANAGER_NUM_BUFS,
                       NET_NAT_MANAGER_HIGH_WM,
                       NET_NAT_MANAGER_LOW_WM,
                       NULL,
#ifdef FEATURE_DATA_PS_MEM_DEBUG
                       (int *) net_nat_manager_buf_hdr_ptr,
                       (int *) net_nat_manager_buf_ptr
#else
                       NULL,
                       NULL
#endif /* FEATURE_DATA_PS_MEM_DEBUG */
...
bail:
  LOG_MSG_FATAL_ERROR ("Can't init DSNET pools", 0, 0, 0);
  ASSERT (0);
  return;
