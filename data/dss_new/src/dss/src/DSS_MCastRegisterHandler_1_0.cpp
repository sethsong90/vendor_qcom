...
/*===========================================================================
  ...
  $Header: //source/qcom/qct/modem/datamodem/interface/dss/rel/11.03/src/DSS_MCastRegisterHandler_1_0.cpp#1 $
...
void DSSMCastRegisterHandler1_0::EventOccurred()
{
   ...
   if (mEdClone->m_Ed->bReg)
   {
      ...
      if (AEE_SUCCESS != res) {
         LOG_MSG_ERROR("GetRegStateAndInfoCodeBCMCS1_0() failed: %d", res, 0, 0);
         return;
      }
...
AEEResult DSSMCastRegisterHandler1_0::RegisterIDL()
{
   ...
   LOG_MSG_INFO1("Registering to QDS_EV_REGISTRATION_STATE_PRIV_BCMCS1_0, MCastSessionPriv obj 0x%p", piNetMCast, 0, 0);
   IDS_ERR_RET(piNetMCast->OnStateChange(piSignal, MCastSessionPrivEvent::QDS_EV_REGISTRATION_STATE_PRIV_BCMCS1_0, &mRegObj));
   return AEE_SUCCESS;
}
