...
if [ $printhelp ]
then 
	echo "$0 : prepare and run dss_test_2"
...
# build command to send to run dss_test_peer_in udp server mode

command="run dss_test_peer -i $mip -p $pport"
cleanup="terminate"
...
# send command to master to run dss_test_peer in udp server mode
...
command="dss_test_2 -i $mip -p $pport -f $datafile -w 50"
