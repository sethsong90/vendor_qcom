                  ...
                  char *rsp, int rsp_len, int *out_len )
{
  ...
  servlen = std_strlen(serv_addr.sun_path) + sizeof(serv_addr.sun_family);
  ...
  length = read(sockfd, rsp, rsp_len-1);
...
int main(int argc, char** argv)
{
  ...
  else if (std_stricmp(DSI_NETCTRL_TEST_CALL_PARAM, *args) == 0)
  {
    ...
    case DSI_CALL_INFO_IP_VERSION:
      cmd.call_param.num_val = strtol(*args, &end, 10);
      if (*end != '\0')
        goto usage;
      break;
    ...
    case DSI_CALL_INFO_PASSWORD:
      ...
      cmd.call_param.num_val = std_strlen(*args);
      break;
  ...
  else if (std_stricmp(DSI_NETCTRL_TEST_QOS_REQUEST, *args) == 0)
  {
	...
	else
	{
      ...
      {
	args++;
        flow_ptr->umts_flow_desc.data_rate.guaranteed_rate = atoi( *args );
	args++;
        flow_ptr->umts_flow_desc.data_rate.max_rate = atoi( *args );
	...
	flow_ptr->umts_flow_desc.param_mask |= QMI_QOS_UMTS_FLOW_PARAM_TRAFFIC_CLASS;
      ...
      {
	args++;
        flow_ptr->cdma_flow_desc.profile_id = atoi( *args );
	...
	filter_ptr->ip_version = QMI_QOS_IP_VERSION_4;
      ...
      {
	args++;
        filter_ptr->filter_desc.tcp_src_ports.start_port = ntohl( 0 ); // TODO
	args++;
        filter_ptr->filter_desc.tcp_src_ports.range = ntohl( 1 ); // TODO
	filter_ptr->filter_desc.param_mask |= QMI_QOS_FILTER_PARAM_TCP_SRC_PORTS;
      ...
      {
	args++;
        filter_ptr->filter_desc.tcp_dest_ports.start_port = ntohl( 0 ); // TODO
	args++;
        filter_ptr->filter_desc.tcp_dest_ports.range = ntohl( 1 ); // TODO
	filter_ptr->filter_desc.param_mask |= QMI_QOS_FILTER_PARAM_TCP_DEST_PORTS;
      ...
      {
	args++;
        filter_ptr->filter_desc.udp_src_ports.start_port = ntohl( 0 ); // TODO
	args++;
        filter_ptr->filter_desc.udp_src_ports.range = ntohl( 1 ); // TODO
	filter_ptr->filter_desc.param_mask |= QMI_QOS_FILTER_PARAM_UDP_SRC_PORTS;
      ...
      {
	args++;
        filter_ptr->filter_desc.udp_dest_ports.start_port = ntohl( 0 ); // TODO
	args++;
        filter_ptr->filter_desc.udp_dest_ports.range = ntohl( 1 ); // TODO
