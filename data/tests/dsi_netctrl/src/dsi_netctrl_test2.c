...
void dsi_test_create_call(int tech, int ip_family, int profile)
{
  ...
  param_info.num_val = tech_map[tech].val;
...
void dsi_test_manage_qosflow(int index,  int qos_op)
{
  ...
  switch( qos_op )
  {
    case dsi_test_qos_op_request:
      ...
      qos_spec.tx_filter_req_array->filter_desc.tcp_src_ports.start_port = ntohs(4000);
...
int main(int argc, char *argv[])
{
  ...
  if (DSI_SUCCESS != dsi_init_ex(DSI_MODE_GENERAL, dsi_init_cb, 0x12345678))
  {
