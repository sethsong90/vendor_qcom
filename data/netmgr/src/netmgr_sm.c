  ...
  if( netmgr_main_cfg.debug ) {
    ...
    switch( debug_event ) {
      case STM_STATE_EXIT_FN:
        netmgr_log_low("netmgr_sm_debug_hook: enter state %s\n",
                       stm_get_state_name(sm, state_info));
        break;
      case STM_STATE_ENTRY_FN:
        netmgr_log_low("netmgr_sm_debug_hook: exit state %s\n",
                       stm_get_state_name(sm, state_info));
        break;
      default:
        break;
...
)
{
  ...
  if( NETMGR_SUCCESS !=
      netmgr_kif_iface_close( cmd_buf->data.link,
...
/*===========================================================================
  ...
  TRANSITION FUNCTION:  netmgr_sm_modem_late_connected
...
*/
/*=========================================================================*/
stm_state_t netmgr_sm_modem_late_connected
(
  stm_state_machine_t *sm,         /*!< State Machine instance pointer */
  void                *payload     /*!< Payload pointer */
)
{
  ...
  /* For dual-IP calls, the Modem generates PKT_SRVC_IND connect
   * events independently for IPV4 and IPV6 QMI clients.  This results
   * in race condition on invoking DHCP for IPV4 if the IPV6 connect
   * arrives first, as the state machine may already be in CONFIGURING
   * state and connect event was rejected.  Now assume such a IPV4
   * connect event is late-arriving, and treat as a configuration
   * action.  This should kickoff DHCP for IPV4 address acquisition,
   * but not change interface state. */

  /* For reverse rmnets, IPV6 address configuration is also performed in netmgr */
  if( NETMGR_IPV4_ADDR ==
      cmd_buf->data.info.connect_msg.addr_info_ptr->if_addr.type ||
      NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link))
  {
    /* Initiate KIF interface configuration process */
    if( NETMGR_SUCCESS != netmgr_kif_iface_configure( cmd_buf->data.link,
                                                      cmd_buf->data.info.connect_msg.addr_info_ptr->if_addr.type) ) {
      netmgr_log_err("failed on kif_iface_configure\n");
  ...
  NETMGR_LOG_FUNC_EXIT;
  return( next_state );
} /* netmgr_sm_modem_late_connected() */
...
)
{
  ...
  if( cmd_buf->repost_count < NETMGR_EXEC_MAX_CMD_REPOST_LIMIT )
  {
    ...
    repost_delay = ((1 << cmd_buf->repost_count) * NETMGR_EXEC_REPOST_STEP_MICROSECS);
...
)
{
      ...
      !NETMGR_KIF_IS_REV_RMNET_LINK(cmd_buf->data.link) )
  {
    ...
    if( NETMGR_SUCCESS !=
        netmgr_qmi_qos_get_flow_info( cmd_buf->data.link,
                                      qos_flow.flow_id,
                                      &qos_flow.priority,
                                      &qos_flow.datarate ) ) {
  ...
  if( NULL == event_info ) {
    ...
    if( modem_addr_ptr )
    {
      ...
      if( NETMGR_NLA_PARAM_PREFIXADDR & nl_addr_ptr->attr_info.param_mask ) {
        ...
        event_info->addr_info.addr.mask = ds_get_num_bits_set_count(modem_addr_ptr->if_mask);
...
)
{
  ...
  if( NETMGR_SUCCESS !=
      netmgr_kif_iface_reconfigure( cmd_buf->data.link,
                                    cmd_buf->data.info.connect_msg.addr_info_ptr ) ) {
    netmgr_log_err("failed on kif_iface_configure\n");
