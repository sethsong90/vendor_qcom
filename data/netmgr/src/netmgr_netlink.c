...
)
{
  ...
  switch( event_info->event ) {
    ...
    case NETMGR_USER_CMD:
      if ((*buffer = netmgr_malloc( size )) == NULL) {
  ...
  while( NLMSG_OK(nlh, buflen) ) {
    switch( nlh->nlmsg_type ) {
      ...
      case XFRM_MSG_NEWAE:
        ...
        if ( NETMGR_SUCCESS !=
            netmgr_nl_decode_xfrm_msg(buffer, buflen, &msg_ptr->xfrm_info) ) {
            return NETMGR_FAILURE;
        }
...
)
{
  ...
  */
  if( netmgr_nl_open_sock( sk_info, nl_type, nl_groups ) < 0)
  {
