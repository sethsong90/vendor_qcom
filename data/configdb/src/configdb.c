...
)
{
   ...
   else
   {
      ...
      cfgdb_xmlfile = strndup(xml_file, CFGDB_MAX_STRING_SIZE);
...
)
{
   ...
   if (NULL != cfgdb_root)
   {
      ...
      char *dup_param_id = strndup(param_id, CFGDB_MAX_STRING_SIZE);
      ...
      else
      {
         ...
         else if (0 != std_strnicmp(param_node->type, param_type, CFGDB_MAX_STRING_SIZE))
         {
...
)
{
   ...
   /* If there is no existing entry for the given type, create a new one */
            ...
            NULL == (node->type = strndup(type, CFGDB_MAX_STRING_SIZE)))
   {
   ...
   if (0 == std_strnicmp(node->type, CFGDB_ELE_LIST, CFGDB_ELE_LIST_LEN))
   {
      ...
      else
      {
         ...
         if (NULL == (size->name = strndup(CFGDB_SPL_ATTR_SIZE, CFGDB_SPL_ATTR_SIZE_LEN)) ||
             ...
             -1   == (asprintf_ret = asprintf(&size->value, "%ld", node->child_count)))
         {
            if (NULL != size->name) {
...
)
{
   ...
   while (FALSE == end_reached)
   {
      ...
      end = strchr(begin, CFGDB_PARAM_ID_SEPARATOR);
...
)
{
   ...
   {
      if (NULL != node->name &&
          0 == std_strnicmp(name, node->name, CFGDB_MAX_STRING_SIZE))
      {
         break;
      }
   }
...
)
{
   ...
   {
      if (0 == std_strnicmp(node->type, param_node->type, CFGDB_MAX_STRING_SIZE))
      {
