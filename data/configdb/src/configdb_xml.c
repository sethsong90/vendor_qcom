...
)
{
   ...
   /* If seek to the end failed or file size is greater than what we support */
            ...
            ((cur_pos = ftell(fp)) < 0 || cur_pos > CFGDB_XML_MAX_FILESIZE) )
   {
      fclose(fp);
   }
   else
...
)
{
   ...
   for (; attr != NULL; attr = attr->next)
   {
      ...
      {
         /* Copy the value */
         if (CFGDB_SUCCESS != (ret_val = configdb_util_dup_string(&cfgdb_node->name,
                                                                  &attr->value)))
         {
            break;
         }
      ...
      {
         /* Copy the value */
         if (CFGDB_SUCCESS != (ret_val = configdb_util_dup_string(&cfgdb_node->type,
                                                                  &attr->value)))
         {
            break;
         }
