...
)
{
  ...
  i=fwrite(iov->iov_base, 1, iov->iov_len, radish_raw_f);
...
void *iface_addr_start_traverse
(
  void
)
{
...
#define IFACE_START 45
  ...
  for (res = read(fd, ln, ADDR_BUF_LEN);
      res > 0 && !all_ifaces && !strstr(curifname, iface_name);
      res = read(fd, ln, ADDR_BUF_LEN)); // Do nothing
  ...
  if (res > 0 )
  {
    ...
    {
      strlcpy(iface_name, curifname, nmlen);
    }
...
)
{
  ...
  while ( (opt = getopt_long(argc, argv, "k:b:i:p:x", radish_opts, NULL)) != -1)
  {
    switch (opt)
    {
      ...
      case 'p': {
          ...
          memcpy(cur_prefix, optarg, slash - optarg);
...
)
{
  ...
  if (!ret) ret = t1->tv_nsec - t2->tv_nsec;
...
)
{
  ...
  if (evt)
  {
    LOGD("%s Timer expired. Event: %8.8x",__func__, (unsigned) evt);
...
)
{
  ...
  if (IN6_IS_ADDR_UNSPECIFIED(&cfg->ip6_addr))
  {
...
)
{
  ...
  if (!mevt)
  {
    ...
    paylen = (size_t) (end - (uint8_t *)mldp);
    ...
    mldp->max_response_delay = htons(1000);
...
)
{
  ...
  cfg->rcv_socket = radish_create_icmp6_packet_socket(cfg->iface_idx,
                                                      cfg->iface_name);
  if (cfg->rcv_socket < 0)
  {
  ...
  for (i = 0; i < IOV_PER_THREAD; i++)
  {
    ...
    LOGD("%s (%s): Buffer at %8.8x", __func__, 
         cfg->iface_name, (unsigned int) itm);
    ...
    for (j = 0; j < IOV_LEN; j++)
    {
      ...
      LOGD("%s (%s): -> [%d,%d]: %8.8x", __func__, 
           cfg->iface_name, i, j, 
           (unsigned int) itm->iov[j].iov_base);
...
)
{
  ...
  rndval = random();
  rndsecs = ((rndval * (max - min) ) / RAND_MAX ) + min;
...
)
{
  ...
  {
    LOGD("Randomized timer outside parameters: [%d, %d]: %d, %8.8x", 
         min, max, 
         rndsecs, 
         (unsigned) timer);
  }
...
)
{
  ...
  LOGD("Sending from %s%%%d to %s%%%d (len=%d) on socket %d",
...
)
{
  ...
  if (!addr || IN6_IS_ADDR_UNSPECIFIED(addr) || !cfg)
  {
  ...
  if (system(cmd) != 0)
  {
    LOGE("Unable to add a new prefix based route");
  }
...
)
{
  ...
  if (!addr || IN6_IS_ADDR_UNSPECIFIED(addr) || !iface)
  {
    LOGE("%s: invalid params", __func__);
    RADISH_LOG_FUNC_EXIT;
    return;
...
)
{
  ...
  hdr = ip6h;
  return icmp6_checksum(hdr, icmp6h, len);
}
    ...
    if (iface != cfg)
    {
      if (iface->iface_params.proxy)
      {
        ...
        if (itm)
        {
          for (i =0; i < IOV_LEN; i++)
          {
            RAD_LOGI("[%s] Copying from %8.8x to %8.8x", cfg->iface_name,
                     (unsigned int) pnd->iov[i].iov_base,
                     (unsigned int) itm->iov[i].iov_base);
            memcpy(itm->iov[i].iov_base, pnd->iov[i].iov_base, IOV_BUFF_LEN);
            itm->iov[i].iov_len = pnd->length;
            RAD_LOGI("%s: itm->iov[%d].iov_len = %d", __func__, i, itm->iov[i].iov_len);
          }
          newnd = nd_parse(itm->iov, pnd->length, pnd->ipv6hdr != NULL);
          if (!newnd)
          {
...
)
{
  ...
  if (cfg)
  {
    struct ipv6_mreq mreq= {.ipv6mr_interface = cfg->iface_idx,
...
)
{
  ...
  {
    LOGD("[%s] type: %d. code: %d checksum: %d. iov_len: %d "
         "icmp6h: %8.8x",
  ...
  if (!pnd->icmphdr)
  {
    pnd->proxy_cookie = radish_random_hash(iov);
  }
  else
  {
    ...
    case ICMPV6_TYPE_MLD2_LR:
      {
        ...
        {
          char addr[MAX_IPV6_PREFIX + 1];
          LOGD("%s: new record. l: %8.8x mc: %8.8x",
               __func__, (unsigned)l,
...
)
{
  ...
  if (FD_ISSET(cfg->wake_socket[1], &cfg->rfds))
  {
    ...
    if (cfg->iface_params.proxy)
    {
      ...
      {
        LOGE("null from proxy queue recv: %8.8x", (unsigned int)recv);
      }
...
)
{
  ...
  if (system(cmd) != 0)
  {
    LOGE("Cannot flush prefix route on the bridge interface");
  }
