...
)
{
            ...
            (curpos += curopt->h.length * 8), curopt = (union nd_options *) curpos)
    {
        ...
        LOGD("%s: calloc option %d (len %d): %8.8x", __func__,
        ...
        if(popt->option->h.type == ND_OPT_TYPE_PREF_INFO) {
          ...
          if (curpos + curopt->h.length * 8 > buffer + packet_size)
          {
            /*
             ...
             */
            num_trailing_zeroes =
              (curpos + curopt->h.length*8 - (buffer + packet_size));
            LOGD("trailing %d 0s detected, length will be amended to %d",
                 num_trailing_zeroes, packet_size + num_trailing_zeroes);
            ret->length = packet_size + num_trailing_zeroes;
            ret->iov[0].iov_len += num_trailing_zeroes;
            LOGD("pnd->length set to %d, iov_len set to %d",
    ...
    return ret;
error:
    if (ret) LOGD("%s: freeing parsed_nd: %8.8x\n", __func__, (unsigned) ret);
    nd_free(ret);
    RADISH_LOG_FUNC_EXIT;
    return NULL;
}

static int ra_init_header(struct parsed_nd *ndp, int code, int flags,
...
error:
    ...
    LOGD("%s: Freeing parsed_nd_option: %8.8x\n", __func__, (unsigned)popt);
    if (popt) {
        free(popt);
    }
    return -1;
...
int nd_add_opt_lla(struct parsed_nd *ndp, uint8_t *addr, size_t addrlen)
{
    ...
    LOGD("%s: Allocated parsed option %8.8x (len %d)", __func__, (unsigned)popt, optlen);
...
int nd_update_options(struct parsed_nd *ndp)
{
    ...
    ndp->length = (opstart + headsize + tmpsize) - pktstart;
    icmp->checksum = 0;
    if (tmpbuf)
      free(tmpbuf);
    return 0;
...
struct parsed_mldp2_lr *mldp2_lr_parse(struct iovec *iov, size_t packet_size, int has_ip_hdr)
{
    ...
    if (!ret)
    {
        LOGD("%s: Unable to allocate %d bytes", __func__, sizeof(struct parsed_mldp2_lr));
        goto error;
    }
    LOGD("%s: Allocated parsed_mldp2_lr: %8.8x\n", __func__, (unsigned) ret);
    ...
    LOGD("%s: Found %d records. curpos-buffer: %u packet size: %u", __func__, nrecs, ((size_t)(curpos-buffer)), packet_size);
    for (i = 0; i < (int) nrecs && ((size_t)(curpos - buffer)) < packet_size; i++) {
        ...
        LOGD("%s: Allocated parsed_mldp2_mcrecord: %8.8x\n", __func__, (unsigned) parsedrecord);
...
error:
    ...
    if (ret) {
        for (parsedrecord = PARSED_MLDP2_MCREC(rlist_pop_first(&ret->mcrecords));
            ...
            if (parsedrecord) {
                LOGD("%s: Freeing parsed_mldp2_mcrec: %8.8x\n", __func__, (unsigned)parsedrecord);
                free(parsedrecord);
            }
        }
    }
    if (ret) {
        LOGD("%s: Freeing parsed_mldp2_lr: %8.8x\n", __func__, (unsigned) ret);
        free(ret);
    }
    return NULL;
}
...
struct sockaddr_in6 *msg_get_ancilliary_addr(struct msghdr *msg)
{
    ...
    for(cmsg = CMSG_FIRSTHDR(msg) ; cmsg; cmsg = CMSG_NXTHDR(msg, cmsg)) {
        LOGD("%s cmsg level %d type %d len: %d. data len: %d", __func__,
                cmsg->cmsg_level,
