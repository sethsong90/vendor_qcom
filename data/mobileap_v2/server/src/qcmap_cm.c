...
/*!
@brief
  This function send a command  connect
  backhaul request, which is processed has a part NAS message processing .
...
*/
/*=========================================================================*/
void qcmap_auto_connect_handler(int sig, siginfo_t *si, void *uc)
{
  timer_t *tidp;
  tidp = si->si_value.sival_ptr;
  if( qcmap_cm_cb.cfg->wan_config.timerid_v4 == (long) *tidp )
  {
    ...
    qcmap_timer_enum_t timer_id = AUTO_CONNECT_V4;
    qcmap_cm_qmi_timer_ind(timer_id);
  }else
  {
    LOG_MSG_INFO1("IPv6 timer expired timer=%x",qcmap_cm_cb.cfg->wan_config.timerid_v6,0,0);
    qcmap_timer_enum_t timer_id = AUTO_CONNECT_V6;
...
void start_auto_connect(ip_version_enum_type family)
{
  ...
  sa.sa_sigaction = qcmap_auto_connect_handler;
