...
int main(int argc, char **argv)
{
 ...
 /* Initialize qcmap_cmdq module */
  ...
  if (QcMapMgr->get_mobileap_enable_cfg_flag())
  {
    timeout = calloc(1, sizeof(struct timeval));
    if(timeout !=NULL)
    {
      timeout->tv_sec  = QCMAP_QTI_MSG_TIMEOUT_S ;
      timeout->tv_usec = 0;
    }
    bootup_init = true;
  }
    ...
    while(1)
  {
    ...
    ret = select(os_params.max_fd+1, &master_fd_set, NULL, NULL, timeout);
    if (ret < 0)
    {
    ...
    if(ret == 0 && bootup_init )
    {
      if (QcMapMgr->get_mobileap_enable_cfg_flag())
      {
        ...
        {
          if (QcMapMgr->EnableWLAN(&qmi_err_num))
          {
    ...
    for (int i = 0; ( i <= os_params.max_fd ); i++)
    {
      ...
      {
           LOG_MSG_INFO1("some fd set, %d, qcmap_dsi_sockfd:%d, qcmap_cmdq_sockfd:%d\n\n", i, qcmap_dsi_sockfd, qcmap_cmdq_sockfd);
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QcMapMgr->DisableWLAN(&qmi_err_num))
  {
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (QcMapMgr->GetStaticNatEntries(&resp_msg.snat_config[0], &resp_msg.snat_config_len, &qmi_err_num))
  {
...
--------------------------------------------------------------------------*/
  ...
  if (!(retval=QcMapMgr->BringDownUSBLink(usb_link,&err_num)))
  {
    LOG_MSG_ERROR("Bring down of USB link failed for QTI",0,0,0);
    resp_msg.resp.result = QMI_RESULT_FAILURE_V01;
    resp_msg.resp.error = QMI_ERR_NO_EFFECT_V01;
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ...
  if (req_ptr = (qcmap_msgr_set_qcmap_bootup_cfg_req_msg_v01*)req_c_struct) {
   ...
   LOG_MSG_INFO1 ("qmi_qcmap_msgr_set_qcmap_bootup_cfg mobileap_enable = %d, mobileap_enable_vaild", req_ptr->mobileap_bootup_flag,req_ptr->mobileap_bootup_flag_valid,0);
   LOG_MSG_INFO1 ("qmi_qcmap_msgr_set_qcmap_bootup_cfg wlan_enable = %d, wlan_enable_vaild", req_ptr->wlan_bootup_flag,req_ptr->wlan_bootup_flag_valid,0);
