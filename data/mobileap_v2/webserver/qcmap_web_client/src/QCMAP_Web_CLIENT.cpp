...
)
{
  ...
  memset(tmp,0,MAX_BUF_LEN);
  if(strstr(values[FIREWALL_SETTINGS_GET_PAGE_PAGE],GETFIREWALLENTRIES)!= NULL)
  {
...
static int create_qcmap_webclient_socket()
{
  ...
  if ( (system("chmod 777 /etc/qcmap_webclient_cgi_file"))  !=QCMAP_CM_SUCCESS)
  {
...
{
  LOG_MSG_ERROR("main function weclient ", 0, 0, 0);
  int test_mode = 0, nbytes=0;
  ...
  /* Enable MobileAP so that we are ready to service CGI requests */
  LOG_MSG_ERROR("Enable mobile ap ", 0, 0, 0);
  ...
  while(1)
  {
    ...
    if (FD_ISSET(qcmap_webclient_sockfd, &initial_sockfd))
    {
      ...
      else
      {
        ...
        printf(" String is: %s \n",buf);
        #endif
        //parse the buffer and store values.
        Tokenizer(buf, fields, values);
        memset(buf,0,MAX_BUF_LEN);
        /* Check Page request is for MobileAP Settings */
        if(strstr(fields[MOBILE_AP_PAGE_PAGE],SETMOBILEAP) != NULL)
          SetMobileAP_Config(values,buf,their_addr,addr_len);
        else if(strstr(fields[MOBILE_AP_PAGE_PAGE],GETMOBILEAP) !=NULL)
        ...
        else if(strstr(fields[LAN_PAGE_PAGE],SETLAN_CONFIG) !=NULL)
          SetLanConfig(values,buf,their_addr,addr_len);
        else if(strstr(fields[LAN_PAGE_PAGE],GETLAN_CONFIG) !=NULL)
          GetLanConfig(values,buf,their_addr,addr_len);
        else if(strstr(fields[WWAN_IPV4_PAGE_PAGE],GETWWANIPV4) !=NULL)
          GetWWANIPV4(values,buf,their_addr,addr_len);
        else if(strstr(fields[WWAN_IPV4_PAGE_PAGE],SETWWANIPV4) !=NULL)
          SetWWANIPV4(values,buf,their_addr,addr_len);
        else if(strstr(fields[UPNP_DLNA_PAGE_PAGE],GETUPNPDLNA) !=NULL)
          GetUPNP_DLNA_Config(values,buf,their_addr,addr_len);
        else if(strstr(fields[WWAN_IPV4_PAGE_PAGE],SETUPNPDLNA) !=NULL)
          SetUPNP_DLNA_Config(values,buf,their_addr,addr_len);
        else if(strstr(fields[DATA_RATE_PAGE_PAGE],GETDATARATE) !=NULL)
          GetDatarate(values,buf,their_addr,addr_len);
        else if(strstr(fields[GET_MEDIA_DIR_PAGE_PAGE],GETMEDIA_DIR_LIST) !=NULL)
          GetMediaDir(values,buf,their_addr,addr_len);
        else if(strstr(fields[SET_MEDIA_DIR_PAGE_PAGE],SETMEDIA_DIR_LIST) !=NULL)
          SetMediaDir(values,buf,their_addr,addr_len);
        else if(strstr(fields[WWAN_STATS_PAGE_PAGE],WWAN_STATS) !=NULL)
        {
