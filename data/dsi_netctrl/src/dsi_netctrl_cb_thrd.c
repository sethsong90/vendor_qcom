      ...
      case QMI_WDS_SRVC_EMBMS_SAI_LIST_IND_MSG:
        {
          ...
          unsigned short *temp_list_ptr = NULL;
          ...
          if (ind_data->sai_list.param_mask & QMI_WDS_EMBMS_LIST_IND_SAI_LIST_PARAM_MASK)
          {
            ...
            if ( 0 != temp_size)
            {
              if( NULL == (temp_list_ptr = (unsigned short *)malloc(temp_size * sizeof(unsigned short))))
              {
                 reti = DSI_ERROR;
                 break;
              }
            ...
            cmd_buf->cmd_data.data_union.wds_ind.ind_data.sai_list.num_sai_per_group = temp_list_ptr;
