...
int dsi_mni_init_client(int conn_id)
{
  ...
  do
  {
    ...
    DSI_LOG_DEBUG("dsi_mni_init_clients: init wds srvc client"
    ...
    if( DSI_TRUE == dsi_qos_enable )
    {
      /* obtain qmi_qos service client for the given qmi port */
      qmi_ret = qmi_qos_srvc_init_client( DSI_GET_WDS_STR(conn_id),
                                          dsi_qmi_qos_ind_cb,
                                          (void *)conn_id,
                                          &qmi_err_code );
      if (qmi_ret < 0)
    ...
    DSI_LOG_DEBUG("dsi_mni_init: init iface [%d]: END", conn_id);
    ...
    else
    {
      if(list_query.param_mask == QMI_WDS_EMBMS_LIST_QUERY_RESP_TMGI_LIST_PARAM_MASK )
      {
        ...
        st_hndl->priv.embms_tmgi_list_info.embms_tmgi_actdeact_params.tmgi_list_len =
          (list_query.tmgi_list.tmgi_list_len > DSI_MAX_NUM_OF_TMGI_LIST)?
