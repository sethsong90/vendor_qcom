...
/* global mutex */
pthread_mutex_t dsi_global_mutex;
  ...
  for(count=0; count<DSI_MAX_DATA_CALLS; count++)
  {
    ...
    if (st != NULL && st->priv.dsi_iface_id == i)
    {
      ...
      if (ce_reason->reason_code != DSI_CE_REASON_UNKNOWN)
      {
        ...
        {
          st->priv.ce_reason[ipf].reason_code = ce_reason->reason_code;
          st->priv.ce_reason[ipf].reason_type = ce_reason->reason_type;
        }
      }
    }
  }
...
*/
/*=========================================================================*/
int dsi_is_qmiport_incall( int iface, int *inuse_ptr )
{
  ...
  *inuse_ptr = FALSE;
  ...
  do
  {
    ...
    if(!( DSI_IS_ID_VALID( iface ) &&
          DSI_GET_WDS_HNDL( iface ) != DSI_INVALID_WDS_HNDL ))
    {
      DSI_LOG_ERROR( "Invalid interface ID specified: %d", iface );
      break;
    }

    /* Retrieve IPv6 call statuses */
    qmi_ret = qmi_wds_set_client_ip_pref( DSI_GET_WDS_HNDL(iface),
    ...
    qmi_ret = qmi_wds_get_pkt_srvc_status( DSI_GET_WDS_HNDL(iface),
                                           &link_state_v6,
                                           &qmi_err_code );
    if( QMI_NO_ERR != qmi_ret )
    {
...
*/
/*=========================================================================*/
unsigned int dsi_map_qmi_to_dsi_tech(unsigned int qmi_tech)
{
  int i=0;
...
*/
/*=========================================================================*/
unsigned int dsi_map_dsi_to_qmi_tech(unsigned int dsi_tech)
{
  int i=0;
...
DSI_INLINE void dsi_detach_dsi_iface(dsi_store_t * st)
{
  ...
  if (DSI_GET_COUNT(st->priv.dsi_iface_id) == 0)
  {
    /* delete ipv6 profile if one was ever created */
...
void dsi_delete_ipv6_profile(int ipv6_iface)
{
  ...
  do
  {
    ...
    if (DSI_GET_IPV6_PROFILE(ipv6_iface) != DSI_INVALID_PROFILE_INDEX)
    {
      ...
      profile_id.profile_index = DSI_GET_IPV6_PROFILE(ipv6_iface);
...
static DSI_INLINE void dsi_invalidate_iface(int dsi_iface)
{
    ...
    DSI_SET_WDS_TXN(dsi_iface,DSI_INVALID_WDS_TXN);
...
void dsi_modem_out_of_service_hdlr(int conn_id)
{
  ...
  DSI_LOG_DEBUG("%s", "Extra log to make sure the change is below is applied");

  if (client_found == TRUE)
  {
    /* set dsi_inited to false */
    dsi_inited = DSI_FALSE;

    /* set netmgr_ready_queried to false */
...
static int dsi_configure_qos()
{
  ...
  do {
    ...
    DSI_LOG_INFO("QOS property is set to [%s] enable [%d]", args, dsi_qos_enable);
    ret = DSI_SUCCESS;
  }
  while (0);
...
/* allow multiple init from offtarget test suite */
#ifndef DSI_NETCTRL_OFFTARGET
    if (dsi_inited)
    {
  ...
  if (DSI_SUCCESS == ret)
  {
    DSI_LOG_DEBUG( "%s", "dsi_init: EXIT with success" );
  }
  else
  {
    DSI_LOG_DEBUG( "%s", "dsi_init: EXIT with error" );
  }
