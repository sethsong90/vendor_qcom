...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "dsi_embms_enable invalid arg, st_hndl [%x]",
    ...
    if(DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_start_f [%x] returned err",
...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "dsi_embms_activate_tmgi invalid arg, st_hndl [%x],"
    ...
    temp_len = (earfcnlist_len > DSI_SIZE_OF_EARFCNLIST)?
    ...
    {
      st_hndl->priv.embms_tmgi_list_info.embms_tmgi_actdeact_params.earfcn_list[itr] = earfcnlist[itr];
    }

    /* validate sailist length */
    temp_len = (sailist_len > DSI_EMBMS_SAI_PER_FREQ_MAX_SIZE) ? DSI_EMBMS_SAI_PER_FREQ_MAX_SIZE : sailist_len;
    ...
    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_embms_tmgi_activate_f [%x] returned err",
...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_deactivate invalid arg, st_hndl [%x],"
                     " or tmgi NULL", (unsigned int)st_hndl);
      break;
    }
    ...
    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_embms_tmgi_deactivate_f [%x] returned err",
...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_activate_deactivate invalid arg, st_hndl [%x],"
                     " or tmgi NULL", (unsigned int)st_hndl);
      break;
    }
    ...
    /* validate earfcnlist length */
    temp_len = (earfcnlist_len > DSI_SIZE_OF_EARFCNLIST) ? DSI_SIZE_OF_EARFCNLIST : earfcnlist_len;
    st_hndl->priv.embms_tmgi_list_info.embms_tmgi_actdeact_params.param_mask |=
    ...
    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_embms_tmgi_act_deactivate_f [%x] returned err",
...
)
{
  ...
  do
  {
    ...
    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "dsi_embms_tmgi_list_query invalid arg, st_hndl [%x]" ,
                     (unsigned int)st_hndl);
      break;
    }
    ...
    if (DSI_SUCCESS != ret)
    {
      DSI_LOG_ERROR("dsi_mni_vtbl.mni_embms_tmgi_list_query_f [%x] returned err",
    ...
    else
    {
      ...
      if(st_hndl->priv.embms_tmgi_list_info.embms_tmgi_actdeact_params.param_mask & QMI_WDS_EMBMS_LIST_QUERY_RESP_TMGI_LIST_PARAM_MASK )
      {
        ...
        embms_tmgi_ptr->embms_tmgi_actdeact_params.tmgi_list_len =
