...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "cannot proceed with reg/unreg, inval arg, st_hndl [%#x]",
    ...
    if (DSI_SUCCESS != reti)
    {
      DSI_LOG_ERROR("%s", "Handoff indication registration/unregisteration failed!");
      break;
    }
...
)
{
  ...
  do
  {
    ...
    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "start_data_call invalid arg, st_hndl [%x]",
                       (unsigned int)st_hndl );
      break;
    }
...
)
{
  ...
  do
  {
    ...
    if(!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "cannot stop, inval arg, st_hndl [%#x]",
                       (unsigned int)st_hndl );
      break;
    }
    ...
    else
    {
      ...
      if (0 == DSI_GET_COUNT(i))
      {
        ...
        /* local state cleanup occurs after response received */
      }
      else
      {
        DSI_LOG_DEBUG("interface [%d] has ref count [%d] on it",
                      i, DSI_GET_COUNT(i));
...
)
{
  ...
  do
  {
    ...
    {
      DSI_LOG_ERROR( "cannot set, inval arg, st_hndl [%#x], ident [%u], info [%x]",
                     (unsigned int)st, identifier, (unsigned int)info );
      break;
    }

    DSI_LOG_VERBOSE( "set param for dsi_hndl [%x], ident [%d], num_val [%d]",
    ...
    case DSI_CALL_INFO_TECH_PREF:
              ...
              DSI_RADIO_TECH_UMTS != dsi_tech)
      {
        ...
        * corresponding modem */
        ...
        break;
               ...
               (temp_tech = dsi_map_dsi_to_qmi_tech(dsi_tech)))
      {
      ...
      else
      {
        ...
        st->priv.start_nw_params.tech_pref = temp_tech;
      }
      break;
...
int dsi_get_device_name(dsi_hndl_t hndl, char * buf, int len)
{
  ...
  do
  {
    ...
    strlcpy(buf, DSI_GET_DEV_STR(i), len);
...
)
{
  ...
  do
  {
    ...
    if (!(DSI_IS_HNDL_VALID(st_hndl)))
    {
      DSI_LOG_ERROR( "inval arg, store hndl [%#x]", (unsigned int)st_hndl );
      break;
    }
    ...
    if (index == DSI_MAX_DATA_CALLS)
    {
      ...
      DSI_LOG_ERROR("rel_data_srvc_hndl: PANIC:Could not find " \
                    "the handle [%x] in the store table",
                    (unsigned int)hndl);
...
)
{
  ...
  do
  {
    ...
    */
    ...
    DSI_LOG_VERBOSE( "data store is at [%#x]", (unsigned int)st );
...
)
{
  ...
  do
  {
    ...
    if(0 > rc)
    {
      DSI_LOG_ERROR( "dsi_reset_pkt_stats: reset stats err,iface=%d, rc=%d, qmi_err=%d",
                    st->priv.dsi_iface_id,
                    rc,
                    qmi_err);
      break;
...
)
{
  ...
  do
  {
    ...
    /* QMI message returned successfully.Fill in the statistics */
