...
main(int argc, char ** argv)
{
  ...
  fprintf(stderr, "Test case to indicate Call Mgr disabling FMC when SM is "
     "in the CONFIGURING state and is waiting for the Tunnel Mgr's response\n");
  ...
  s = inet_pton(AF_INET, "192.168.1.100", (char*)&(addr->sin_addr));  
  ...
  if (s <= 0) {
    ...
    {
      fprintf(stderr,"Error in inet_pton IP address %d ret %d\n",
            addr->sin_addr.s_addr, s);
    }
   }

  tunnel_mgr_ds.is_tunnel_set = 1;
  tunnel_mgr_ds.dest_strm_id = 1;
  tunnel_mgr_ds.nat_present = 1;
  ...
  fprintf(stderr, "completed setting tunnel info on %d\n",
            tunnel_mgr_client_fd );

  bearer_val = DS_FMC_APP_FMC_BEARER_DISABLED;

  /*Trigger the state machine to tear down the call*/
  if( send(cm_clnt_hdl, &bearer_val, 
      sizeof(ds_fmc_app_fmc_bearer_status_type_t), 0 ) < 0)
  {
    fprintf(stderr, "send on cm_clnt_hdl=0x%08x failed\n",
  ...
  if(recv_buf == bearer_val)
  {
    fprintf(stderr, "recv_buf %d == bearer_val: %d\n",
	          recv_buf, bearer_val );
  }
  else
  {
    fprintf(stderr, "Undesired behaviour: recv_buf %d != bearer_val: %d\n",
	          recv_buf, bearer_val );
    close(tunnel_mgr_client_fd);
    close(cm_clnt_hdl);
    close(tm_srvr_hdl);
  ...
  memset(&tunnel_mgr_ds, 0, sizeof(ds_fmc_app_tunnel_mgr_ds_type));
