...
int main(int argc, char **argv)
{
  ...
  if (test_mode > 0)
  {
    ...
    while (TRUE)
    {
      ...
      printf(" 8. Set NAT Timeout           24. Start Embedded Call\n");
      printf(" 9. Set IPSEC VPN Passthrough 25. End Embedded Call\n");
      ...
      switch (opt)
      {
        ...
        case 24:
        {
          ...
          printf("   Please enter IP Family (IPV4 - 4, IPV6 - 6, IPV4V6 - 10 : ");
          fgets(scan_string, sizeof(scan_string), stdin);
          IP_Family = atoi(scan_string);
          printf("   Please select Technology (0-UMTS, 1-CDMA, 2-ANY) : ");
          fgets(scan_string, sizeof(scan_string), stdin);
          ...
          fgets(scan_string, sizeof(scan_string), stdin);
          umts_indx = atoi(scan_string);
          printf("   Please enter CDMA Profile Number : ");
          fgets(scan_string, sizeof(scan_string), stdin);
          cdma_indx = atoi(scan_string);
          if(QcMapMgr->StartEmbeddedCall(IP_Family, umts_indx, cdma_indx, Tech) == true)
            printf("  Embedded Call Started. ");
          else
            printf("  Failed to start Embedded Call. ");
          break;
        }
        case 25:
        {
          if(QcMapMgr->EndEmbeddedCall() == true)
            printf("   Embedded Call Ended. ");
          else
            printf("  Failed to end Embedded Call.");
          break;
        }
        case 26:
        {
