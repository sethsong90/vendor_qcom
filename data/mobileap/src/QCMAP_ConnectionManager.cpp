...
)
{
  ...
  case QCMAP_CM_EVENT_DISABLED:
    ...
    manager->EndEmbeddedCall();
    manager->DisableWLAN();
    break;
  case QCMAP_CM_EVENT_EMBD_CONNECTED:
    QCMAP_CM_LOG("QCMAP Embedded Call IPV4 Connected\n");
    break;
  case QCMAP_CM_EVENT_EMBD_IPv6_CONNECTED:
    QCMAP_CM_LOG("QCMAP Embedded Call IPV6 Connected\n");
    break;
  case QCMAP_CM_EVENT_EMBD_DISCONNECTED:
    QCMAP_CM_LOG("QCMAP Embedded Call IPV4 Disonnected\n");
    break;
  case QCMAP_CM_EVENT_EMBD_IPv6_DISCONNECTED:
    QCMAP_CM_LOG("QCMAP Embedded Call IPV6 Disconnected\n");
    break;
  default:
...
/* Start Embedded Call */
boolean QCMAP_ConnectionManager::StartEmbeddedCall
(
  int IP_Family,
...
)
{
  int err = 0;
  int ret_val = 0;
  ret_val = qcmap_cm_embdcall_enable(IP_Family, UMTS_PROFILE_INDX, CDMA_PROFILE_INDX, Tech,&err);
  QCMAP_CM_LOG( "\nEmbedded Call START  ret_val : %d, err : %d\n",ret_val,err);
  if((ret_val == QCMAP_CM_ERROR && err == QCMAP_CM_EWOULDBLOCK) || (ret_val == QCMAP_CM_SUCCESS))
  {
    QCMAP_CM_LOG( "\nEmbedded Call Initiating.\n");
    return true;
  }
  else
  {
    QCMAP_CM_LOG( "\nFailed to Start Embedded Call. Error : %d\n",err);
    return false;
  }
}

/* End Embedded Call */
boolean QCMAP_ConnectionManager::EndEmbeddedCall()
{
  int ret = 0;
  int qcmap_cm_errno = 0;
  ret = qcmap_cm_embdcall_disable(&qcmap_cm_errno);
  QCMAP_CM_LOG( "\nEmbedded Call END ret_val : %d, err : %d\n",ret,qcmap_cm_errno);
  if(qcmap_cm_errno != QCMAP_CM_SUCCESS)
  {
    QCMAP_CM_LOG( "\nFailed to End Embedded Call. Dss Error %d.\n",ret);
    return false;
  }
  else
    QCMAP_CM_LOG( "\nEmbedded Call Terminating.\n");
  return true;
}
