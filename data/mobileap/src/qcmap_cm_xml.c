       ...
       case XMLLIB_PARSETREE_NODE_ELEMENT:
       {
                                         ...
                                         HostAPDCfg_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->lan_config.interface[ssid].path_to_hostapd_conf, content_buf, content->len);
                                         ...
                                         DHCPLeaseTime_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->lan_config.interface[ssid].dhcp_lease_time, content_buf, content->len);
                                         ...
                                         SupplicantCfg_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->lan_config.interface[ssid].path_to_supplicant_conf, content_buf, content->len);
                                         ...
                                         ExternalAPSSID_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->lan_config.interface[ssid].external_ap_ssid, content_buf, content->len);
                                         ...
                                         EriConfig_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->wan_config.eri_config_file, content_buf, content->len);
             printf("EriConfig %s\n", config->wan_config.eri_config_file);
           }
         }
         else if (0 == qcmap_cm_util_icmp_string(&xml_node->payload.element.name,
                                         ...
                                         Firewall_TAG))
         {
           ...
           if (content)
           {
             memset(content_buf, 0, sizeof(content_buf));
             memcpy(content_buf, (void *)content->string, content->len);
             strlcpy(config->nat_config.firewall_config_file, content_buf, content->len);
