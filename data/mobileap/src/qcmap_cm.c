...
typedef struct
{
    ...
    dss_iface_ioctl_device_info_type dss_device_info;

    /* dss ipv6 related info */
    sint15                    ipv6_dss_handle;
    dss_net_policy_info_type  ipv6_net_policy;
    dss_iface_ioctl_device_info_type ipv6_dss_device_info;
...
static qcmap_cm_cb_t qcmap_cm_cb = {
  ...
  .handle                      = 1,
  .dss_handle                  = QCMAP_LAN_INVALID_QCMAP_HANDLE,
  .ipv6_dss_handle             = QCMAP_LAN_INVALID_QCMAP_HANDLE,
...
/*---------------------------------------------------------------------------
   Type representing an QCMAP EMBEDDED CB (QCMAP EMBEDDED control block);
---------------------------------------------------------------------------*/
typedef struct
{
    qcmap_cm_state_e          state;
...
/*---------------------------------------------------------------------------
   Mobile AP CM Embedded Call control block
---------------------------------------------------------------------------*/
static qcmap_embd_cb_t qcmap_embd_cb = {
  .state                       = QCMAP_CM_DISABLE,
  .ipv6_state                  = QCMAP_CM_V6_DISABLE,
  .handle                      = 1,
  .dss_handle                  = QCMAP_LAN_INVALID_QCMAP_HANDLE,
...
/*=========================================================================*/
static void
qcmap_cm_embd_lock (void)
{
  ds_assert(pthread_mutex_lock(&qcmap_embd_cb.qcmap_cm_mutex) == 0);
}

/*===========================================================================
  FUNCTION  qcmap_cm_embd_unlock
===========================================================================*/
/*!
@brief
  Wrapper function for releasing the shared lock used for serializing access
...
/*=========================================================================*/
static void
qcmap_cm_embd_unlock (void)
{
  ds_assert(pthread_mutex_unlock(&qcmap_embd_cb.qcmap_cm_mutex) == 0);
}
...
static void qcmap_cm_dss_net_embd_cb_fcn
(
...
static void
qcmap_cm_dss_sock_embd_cb_fcn
(
  sint15 dss_nethandle,                             /* Application ID      */
  sint15 sockfd,                                    /* Socket descriptor   */
  uint32 event_mask,                                /* Event occurred      */
  void  *sock_cb_user_data                          /* User call back data */
);
...
)
{
  ...
  LOG_MSG_INFO1_6("qcmap_cm_dss_net_cb_fcn: dss_nethandle %d dss_errno %d "
                "qcmap_cm_cb.state (V4) %d qcmap_cm_cb.ipv6_state (V6) %d", dss_nethandle, dss_errno,
                qcmap_cm_cb.state, qcmap_cm_cb.ipv6_state,0,0);

  if(dss_nethandle == qcmap_cm_cb.ipv6_dss_handle)
  {
    if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTING)
    {
        if(dss_errno == DS_ENETISCONN)
        {
          dss_iface_id_type if_id;
          dss_iface_ioctl_device_info_type device_info;
          sint15 dss_ioctl_errno;
          int rval;
          ...
          callback = TRUE;
          LOG_MSG_INFO1("Received DS_ENETISCONN in state V6_LAN_CONNECTING",0,0,0);
          LOG_MSG_INFO1("Transition from state V6_LAN_CONNECTING to V6_LAN_CONNECTED",0,0,0);

          if_id = dss_get_iface_id(qcmap_cm_cb.ipv6_dss_handle);
          ...
          LOG_MSG_INFO1("rval:%d dss_ioctl_errno %d",
                        rval, dss_ioctl_errno, 0);

          if(rval == QCMAP_CM_ERROR || dss_ioctl_errno == DS_EINVAL)
          {
            LOG_MSG_ERROR ("Couldn't get ipv6 rmnet name. rval %d dss_ioctl_err %d",
                           rval, dss_ioctl_errno, 0);
            strlcpy((char*) &device_info.device_name, QCMAP_V6_DEFAULT_DEVICE_NAME, 16);
          }

          LOG_MSG_INFO1("device_name:%s, interface index is:%d",
                        device_info.device_name, device_info.if_index, 0);
          memcpy(&qcmap_cm_cb.ipv6_dss_device_info, &device_info, sizeof(dss_iface_ioctl_device_info_type));
        ...
        else if (dss_errno == DS_ENETNONET)
        {
          ...
          callback = TRUE;
          LOG_MSG_INFO1("Received DS_ENETNONET in state V6_LAN_CONNECTING",0,0,0);
        ...
        else if(dss_errno == QCMAP_CM_IND_V6_WAN_CONNECTED)
        {
        ...
        else if(dss_errno == QCMAP_CM_IND_V6_WAN_DISCONNECTED)
        {
    ...
    else if (qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTED)
    {
      if (dss_errno == DS_ENETISCONN)
      {
          LOG_MSG_INFO1("Received DS_ENETISCONN in state V6_LAN_CONNECTED",0,0,0);
          LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_LAN_CONNECTED",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        qcmap_cm_lock();
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Disconnected",
                        qcmap_cm_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received event DS_ENETNONET in state V6_LAN_CONNECTED",0,0,0);
    ...
    else if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_LAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in state V6_LAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_LAN_DISCONNECTING",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        qcmap_cm_lock();
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Disconnected",
                        qcmap_cm_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received event DS_ENETNONET in state V6_LAN_DISCONNECTING",0,0,0);
    ...
    else if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_WAN_CONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in V6_WAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_WAN_CONNECTING",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in V6_WAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in state V6_WAN_CONNECTING",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V6_WAN_CONNECTED)
      {
        qcmap_cm_lock();
    ...
    else if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_WAN_CONNECTED)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in V6_WAN_CONNECTED",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_WAN_CONNECTED",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in V6_WAN_CONNECTED",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in state V6_WAN_CONNECTED",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V6_WAN_CONNECTED)
      {
    ...
    else if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_WAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in V6_WAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in V6_WAN_DISCONNECTING",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in V6_WAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore DS_ENETNONET in  V6_WAN_DISCONNECTING",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V6_WAN_CONNECTED)
      {
    ...
    else if(qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_ENABLE)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in V6_ENABLE",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in V6_ENABLE",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in V6_ENABLE",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in V6_ENABLE",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V6_WAN_CONNECTED)
      {
  ...
  if (dss_nethandle == qcmap_cm_cb.dss_handle)
  {
    if (qcmap_cm_cb.state == QCMAP_CM_LAN_CONNECTING)
    {
      if (dss_errno == DS_ENETISCONN)
      {
        dss_iface_id_type if_id;
        ...
        LOG_MSG_INFO1("Received DS_ENETISCONN in state LAN_CONNECTING",0,0,0);
        ...
        if_id = dss_get_iface_id(qcmap_cm_cb.dss_handle);
        memset(&device_info, 0, sizeof(device_info));
        ...
        if(rval == QCMAP_CM_ERROR || dss_ioctl_errno == DS_EINVAL)
        {
          LOG_MSG_ERROR ("Couldn't get ipv4 rmnet name. rval %d dss_ioctl_err %d",
                         rval, dss_ioctl_errno, 0);
          strlcpy((char*) &device_info.device_name, QCMAP_V4_DEFAULT_DEVICE_NAME, 16);
        }

        LOG_MSG_INFO1("device_name:%s, interface index is:%d",
                      device_info.device_name, device_info.if_index, 0);
        memcpy(&qcmap_cm_cb.dss_device_info, &device_info, sizeof(dss_iface_ioctl_device_info_type));


        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Connected",
                      qcmap_cm_cb.handle, 0, 0);
      }
      ...
      else if (dss_errno == DS_ENETNONET)
      {
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Connecting Fail",
                      qcmap_cm_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received DS_ENETNONET in state LAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Transition from state LAN_CONNECTING to ENABLE",0,0,0);
      }
      else if(dss_errno == QCMAP_CM_IND_V4_WAN_CONNECTED)
      {
      ...
      else if(dss_errno == QCMAP_CM_IND_V4_WAN_DISCONNECTED)
      {
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_LAN_CONNECTED)
    {
      if (dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received DS_ENETISCONN in state LAN_CONNECTED",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state LAN_CONNECTED",0,0,0);
      }
      else if (dss_errno == DS_ENETNONET)
      {
        qcmap_cm_lock();
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Disconnected",
                      qcmap_cm_cb.handle, 0, 0);
        callback = TRUE;
        LOG_MSG_INFO1("Received event DS_ENETNONET in state LAN_CONNECTED",0,0,0);
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_LAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in state LAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state LAN_DISCONNECTING",0,0,0);
      }
      else if (dss_errno == DS_ENETNONET)
      {
        qcmap_cm_lock();
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Disconnected",
                      qcmap_cm_cb.handle, 0, 0);
        callback = TRUE;
        LOG_MSG_INFO1("Received event DS_ENETNONET in state LAN_DISCONNECTING",0,0,0);
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_WAN_CONNECTING)
    {
      if (dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in WAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state WAN_CONNECTING",0,0,0);
      }      
      else if (dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in WAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in state WAN_CONNECTING",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V4_WAN_CONNECTED)
      {
        qcmap_cm_lock();
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_WAN_CONNECTED)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in WAN_CONNECTED",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state WAN_CONNECTED",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in WAN_CONNECTED",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in state WAN_CONNECTED",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V4_WAN_CONNECTED)
      {
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_WAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in WAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in WAN_DISCONNECTING",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
    ...
    else if (qcmap_cm_cb.state == QCMAP_CM_ENABLE)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in ENABLE",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in ENABLE",0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        LOG_MSG_INFO1("Received event DS_ENETNONET in ENABLE",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETNONET in ENABLE",0,0,0);
      }
      else if (dss_errno == QCMAP_CM_IND_V4_WAN_CONNECTED)
      {
...
/*===========================================================================
  FUNCTION  qcmap_cm_dss_sock_cb_fcn
===========================================================================*/
static void
qcmap_cm_dss_sock_cb_fcn
(
  sint15 dss_nethandle,                             /* Application ID      */
  sint15 sockfd,                                    /* Socket descriptor   */
  uint32 event_mask,                                /* Event occurred      */
    ...
    case QMI_QCMAP_WWAN_STATUS_IND_V01:
    {
      ...
      if (ind_data.wwan_status == QCMAP_WWAN_CONNECTED_V01)
      {
        if (ind_data.mobile_ap_handle == qcmap_cm_cb.mobile_ap_handle)
        {
          ...
          cmd_buf->data.dss_nethandle = qcmap_cm_cb.dss_handle;
          cmd_buf->data.iface_id = DSS_IFACE_ANY;
          cmd_buf->data.dss_errno = QCMAP_CM_IND_V4_WAN_CONNECTED;
        ...
        else if (ind_data.mobile_ap_handle == qcmap_cm_cb.ipv6_mobile_ap_handle)
        {
          LOG_MSG_INFO1("qcmap_cm_qmi_qcmap_ind: IPv6 WWAN Connected reconfig_required %d",
                        ind_data.reconfig_required, 0, 0);
          cmd_buf = qcmap_cmdq_get_cmd(); 
          cmd_buf->data.dss_nethandle = qcmap_cm_cb.ipv6_dss_handle;
          cmd_buf->data.iface_id = DSS_IFACE_ANY;
          cmd_buf->data.dss_errno = QCMAP_CM_IND_V6_WAN_CONNECTED;
          LOG_MSG_INFO1("qcmap_cmdq: queue commmand",0,0,0);
          /* Post command for processing in the command thread context */
          if( QCMAP_CMDQ_SUCCESS != qcmap_cmdq_put_cmd( cmd_buf ) ) 
          {
      ...
      else if (ind_data.wwan_status == QCMAP_WWAN_DISCONNECTED_V01)
      {
        if (ind_data.mobile_ap_handle == qcmap_cm_cb.mobile_ap_handle)
        {
          ...
          cmd_buf->data.dss_nethandle = qcmap_cm_cb.dss_handle;
          cmd_buf->data.iface_id = DSS_IFACE_ANY;
          cmd_buf->data.dss_errno = QCMAP_CM_IND_V4_WAN_DISCONNECTED;
          LOG_MSG_INFO1("qcmap_cmdq: queue commmand",0,0,0);
          /* Post command for processing in the command thread context */
          if( QCMAP_CMDQ_SUCCESS != qcmap_cmdq_put_cmd( cmd_buf ) ) 
          {
        ...
        else if (ind_data.mobile_ap_handle == qcmap_cm_cb.ipv6_mobile_ap_handle)
        {
          ...
          cmd_buf->data.dss_nethandle = qcmap_cm_cb.ipv6_dss_handle;
          cmd_buf->data.iface_id = DSS_IFACE_ANY;
          cmd_buf->data.dss_errno = QCMAP_CM_IND_V6_WAN_DISCONNECTED;
          LOG_MSG_INFO1("qcmap_cmdq: queue commmand",0,0,0);
          /* Post command for processing in the command thread context */
          if( QCMAP_CMDQ_SUCCESS != qcmap_cmdq_put_cmd( cmd_buf ) ) 
          {
...
)
{
  ...
  /* Open IPv4 DSS API */
  dss_init_net_policy_info(&qcmap_cm_cb.net_policy);
  qcmap_cm_cb.net_policy.iface.kind = DSS_IFACE_NAME;
  switch (qcmap_cm_cb.cfg->wan_config.tech)
  {
    case QCMAP_WAN_TECH_3GPP:
      qcmap_cm_cb.net_policy.iface.info.name = DSS_IFACE_UMTS;
      break;
    case QCMAP_WAN_TECH_3GPP2:
      qcmap_cm_cb.net_policy.iface.info.name = DSS_IFACE_CDMA_SN;
      break;
    case QCMAP_WAN_TECH_ANY:
    default:
      qcmap_cm_cb.net_policy.iface.info.name = DSS_IFACE_ANY;
      break;
  }
  qcmap_cm_cb.net_policy.family = DSS_AF_INET;
  qcmap_cm_cb.net_policy.umts.pdp_profile_num =
    qcmap_cm_cb.cfg->wan_config.profile_id.v4.umts_profile_index;
  qcmap_cm_cb.net_policy.cdma.data_session_profile_id =
    qcmap_cm_cb.cfg->wan_config.profile_id.v4.cdma_profile_index;

  qcmap_cm_cb.dss_handle = dsnet_get_handle(&qcmap_cm_dss_net_cb_fcn,
                                            (void *)0,
                                            &qcmap_cm_dss_sock_cb_fcn,
                                            (void *)0,
                                            &qcmap_cm_cb.net_policy, &dss_errno);

  LOG_MSG_INFO1("qcmap_cm_enable: dss_handle %X", qcmap_cm_cb.dss_handle, 0, 0);

  if (qcmap_cm_cb.dss_handle < 0)
  {
    *qcmap_cm_errno = QCMAP_CM_EOPNOTSUPP;
    qcmap_cm_unlock();
    LOG_MSG_ERROR("qcmap_cm_enable: Can not get dss net handle", 0, 0, 0);
    return qcmap_cm_handle;
  }

  /* Open IPv6 DSS API */
  if ((qcmap_cm_cb.cfg->wan_config.ip_family == QCMAP_IP_V6_V01) ||
      (qcmap_cm_cb.cfg->wan_config.ip_family == QCMAP_IP_V4V6_V01))
  {
    dss_init_net_policy_info(&qcmap_cm_cb.ipv6_net_policy);
    qcmap_cm_cb.ipv6_net_policy.iface.kind = DSS_IFACE_NAME;
    switch (qcmap_cm_cb.cfg->wan_config.tech)
    {
      case QCMAP_WAN_TECH_3GPP:
        qcmap_cm_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_UMTS;
        break;
      case QCMAP_WAN_TECH_3GPP2:
        qcmap_cm_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_CDMA_SN;
        break;
      case QCMAP_WAN_TECH_ANY:
      default:
        qcmap_cm_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_ANY;
        break;
    }
    qcmap_cm_cb.ipv6_net_policy.family = DSS_AF_INET6;
    qcmap_cm_cb.ipv6_net_policy.umts.pdp_profile_num =
      qcmap_cm_cb.cfg->wan_config.profile_id.v6.umts_profile_index;
    qcmap_cm_cb.ipv6_net_policy.cdma.data_session_profile_id =
      qcmap_cm_cb.cfg->wan_config.profile_id.v6.cdma_profile_index;

    qcmap_cm_cb.ipv6_dss_handle = dsnet_get_handle(&qcmap_cm_dss_net_cb_fcn,
                                              (void *)0,
                                              &qcmap_cm_dss_sock_cb_fcn,
                                              (void *)0,
                                              &qcmap_cm_cb.ipv6_net_policy, &dss_errno);

    LOG_MSG_INFO1("qcmap_cm_enable: ipv6_dss_handle %X", qcmap_cm_cb.ipv6_dss_handle, 0, 0);

    if (qcmap_cm_cb.ipv6_dss_handle < 0)
    {
      *qcmap_cm_errno = QCMAP_CM_EOPNOTSUPP;
      qcmap_cm_unlock();
      LOG_MSG_ERROR("qcmap_cm_enable: Can not get ipv6 dss net handle", 0, 0, 0);
      return qcmap_cm_handle;
    }
  }
  ...
  while(1)
  {
    rval = dsnet_start(qcmap_cm_cb.dss_handle, &dss_errno);
    if(rval == DSS_SUCCESS || dss_errno == DS_EWOULDBLOCK || count > QCMAP_DEFAULT_DSS_INIT_TIME)
      break;
    sleep(1);
    count++;
  }

  if (rval != DSS_SUCCESS && dss_errno != DS_EWOULDBLOCK)
  {
    LOG_MSG_INFO1("qcmap_cm_enable: dsnet_start rval %d dss_errno %d",
                  rval, dss_errno, 0);
    // Fake a NONET
    cmd_buf = qcmap_cmdq_get_cmd();
    cmd_buf->data.dss_nethandle = qcmap_cm_cb.dss_handle;
    cmd_buf->data.iface_id = DSS_IFACE_ANY;
    cmd_buf->data.dss_errno = DS_ENETNONET;
    LOG_MSG_INFO1("qcmap_cmdq: queue commmand",0,0,0);
    /* Post command for processing in the command thread context */
    if( QCMAP_CMDQ_SUCCESS != qcmap_cmdq_put_cmd( cmd_buf ) ) 
    {
  ...
  if (qcmap_cm_cb.ipv6_dss_handle >= 0)
  {
    rval = dsnet_start(qcmap_cm_cb.ipv6_dss_handle, &dss_errno);

    if (rval != DSS_SUCCESS && dss_errno != DS_EWOULDBLOCK)
    {
      LOG_MSG_INFO1("qcmap_cm_enable: dsnet_start ipv6 rval %d dss_errno %d",
                    rval, dss_errno, 0);
      // Fake a NONET
      cmd_buf = qcmap_cmdq_get_cmd(); 
      cmd_buf->data.dss_nethandle = qcmap_cm_cb.ipv6_dss_handle;
...
)
{
  ...
  else if (qcmap_cm_cb.state == QCMAP_CM_LAN_CONNECTED || qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTED)
  {
    ...
    dsnet_stop(qcmap_cm_cb.dss_handle, &dss_errno);

    if (qcmap_cm_cb.ipv6_dss_handle >= 0)
    {
      qcmap_cm_cb.ipv6_state = QCMAP_CM_V6_LAN_DISCONNECTING;
      dsnet_stop(qcmap_cm_cb.ipv6_dss_handle, &dss_errno);
    }
    qcmap_cm_unlock();
  ...
  else if(qcmap_cm_cb.state <= QCMAP_CM_ENABLE && qcmap_cm_cb.ipv6_state <= QCMAP_CM_V6_ENABLE)
  {
    ...
    if (qcmap_cm_cb.ipv6_dss_handle >= 0)
    {
      dsnet_release_handle(qcmap_cm_cb.ipv6_dss_handle, &dss_errno);
      qcmap_cm_cb.ipv6_dss_handle = QCMAP_LAN_INVALID_QCMAP_HANDLE;
    }

    dsnet_release_handle(qcmap_cm_cb.dss_handle, &dss_errno);
    qcmap_cm_cb.dss_handle = QCMAP_LAN_INVALID_QCMAP_HANDLE;
...
)
{
      ...
      qcmap_cm_cb.ipv6_state == QCMAP_CM_V6_WAN_CONNECTED)
  {
    ...
    qcmap_cm_unlock();
    LOG_MSG_INFO1("QCMAP CM Backhaul already connected",
                  0, 0, 0);
    return ret;
  }
...
)
{
  ...
  {
    strlcpy(qcmap_dev_name, qcmap_cm_cb.dss_device_info.device_name, PS_MAX_DEVICE_NAME_LEN);
  }
  else if (qcmap_dev_type == QCMAP_IP_V6_V01)
  {
    if (qcmap_cm_cb.ipv6_dss_handle >= 0)
    {
      strlcpy(qcmap_dev_name, qcmap_cm_cb.ipv6_dss_device_info.device_name, PS_MAX_DEVICE_NAME_LEN);
    }
    else
    {
      qcmap_cm_unlock();
...
)
{
  ...
  sint15              dss_ioctl_errno;
  sint15              dss_handle;
  dss_iface_ioctl_get_iface_stats_type stats_info;
  ...
  if (ip_family == IP_V4)
  {
    dss_handle = qcmap_cm_cb.dss_handle;
  }
  else if (ip_family == IP_V6)
  {
    dss_handle = qcmap_cm_cb.ipv6_dss_handle;
  }
  else
  {
  ...
  if_id = dss_get_iface_id(dss_handle);
  ...
  if(if_id == DSS_IFACE_INVALID_ID)
  {
    *qcmap_cm_errno = QCMAP_CM_EINVAL;
    ret = QCMAP_CM_ERROR;
    qcmap_cm_unlock();
    LOG_MSG_ERROR( "QCMAP CM Invalid iface ID %d", if_id, 0, 0);
    return ret;
  }
  ...
  if (rval != 0)
  {
    LOG_MSG_ERROR("DSS_IFACE_IOCTL_GET_IFACE_STATS Failed error %d",
                   dss_ioctl_errno,0,0);
    *qcmap_cm_errno = dss_ioctl_errno;
    ret = QCMAP_CM_ERROR;
    qcmap_cm_unlock();
    return ret;
  }
...
)
{
  ...
  if (rval != 0)
  {
    LOG_MSG_ERROR("DSS_IFACE_IOCTL_RESET_IFACE_STATS Failed error %d",
                   dss_ioctl_errno,0,0);
    *qcmap_cm_errno = dss_ioctl_errno;
    ret = QCMAP_CM_ERROR;
    qcmap_cm_unlock();
...
{
  pthread_mutexattr_t attr;
  qcmap_cm_embd_conf  net_profile;
  sint15 dss_errno = 0, rval = -2;

  LOG_MSG_INFO1("qcmap_cm_embdcall: enter", 0, 0, 0);
  net_profile.umts_profile_index = UMTS_PROFILE_INDX;
  net_profile.cdma_profile_index = CDMA_PROFILE_INDX;
  net_profile.tech = Tech;
  net_profile.ip_family = IP_Family;

  if (qcmap_embd_cb.state >= QCMAP_CM_LAN_CONNECTING ||
      qcmap_embd_cb.ipv6_state >= QCMAP_CM_V6_LAN_CONNECTING)
  {
    LOG_MSG_ERROR("Embedded Call already Initiated!", 0, 0, 0);
    return rval;
  }

  if(qcmap_embd_cb.init == FALSE)
  {
    LOG_MSG_INFO1("qcmap_cm_embdcall: init", 0, 0, 0);
    ...
    /* Set mutex to be reentry */
    pthread_mutexattr_settype(&attr, PTHREAD_MUTEX_RECURSIVE_NP);
    pthread_mutex_init(&qcmap_embd_cb.qcmap_cm_mutex, &attr);
    qcmap_embd_cb.init = TRUE;
  }

  qcmap_cm_embd_lock();

  if(qcmap_embd_cb.disable_in_process)
  {
    *qcmap_embd_errno = QCMAP_CM_EOPNOTSUPP;
    qcmap_cm_embd_unlock();
    LOG_MSG_ERROR( "qcmap_cm_embdcall fail because disable still in progress",
                                                                      0, 0, 0);
    return rval;
  }

  if (qcmap_embd_cb.state != QCMAP_CM_DISABLE &&
                            qcmap_embd_cb.ipv6_state != QCMAP_CM_V6_DISABLE)
  {
    *qcmap_embd_errno = QCMAP_CM_EOPNOTSUPP;
    qcmap_cm_embd_unlock();
    LOG_MSG_ERROR( "qcmap_cm_embdcall fail because state is not DISABLE", 0, 0, 0);
    return rval;
  }
  if
  (
    net_profile.ip_family== QCMAP_IP_V4V6_V01
                         ||
    net_profile.ip_family  == QCMAP_IP_V4_V01
  )
  {
    /* Open IPv4 DSS API */
      dss_init_net_policy_info(&qcmap_embd_cb.net_policy);
      qcmap_embd_cb.net_policy.iface.kind = DSS_IFACE_NAME;
      switch (net_profile.tech)
      {
        case QCMAP_WAN_TECH_3GPP:
          qcmap_embd_cb.net_policy.iface.info.name = DSS_IFACE_UMTS;
          break;
        case QCMAP_WAN_TECH_3GPP2:
          qcmap_embd_cb.net_policy.iface.info.name = DSS_IFACE_CDMA_SN;
          break;
        case QCMAP_WAN_TECH_ANY:
        default:
          qcmap_embd_cb.net_policy.iface.info.name = DSS_IFACE_ANY;
          break;
      }
      qcmap_embd_cb.net_policy.family = DSS_AF_INET;
      qcmap_embd_cb.net_policy.umts.pdp_profile_num =
                                                net_profile.umts_profile_index;
      qcmap_embd_cb.net_policy.cdma.data_session_profile_id =
                                                net_profile.cdma_profile_index;
      qcmap_embd_cb.dss_handle =
                                 dsnet_get_handle(&qcmap_cm_dss_net_embd_cb_fcn,
                                 (void *)0, &qcmap_cm_dss_sock_embd_cb_fcn,
                                 (void *)0, &qcmap_embd_cb.net_policy,
                                 &dss_errno);
      LOG_MSG_INFO1("qcmap_embdcall: dss_handle %X",
                                                 qcmap_embd_cb.dss_handle, 0, 0);
      if (qcmap_embd_cb.dss_handle < 0)
      {
        *qcmap_embd_errno = QCMAP_CM_EOPNOTSUPP;
        qcmap_cm_embd_unlock();
        LOG_MSG_ERROR("qcmap_embdcall: Can not get dss net handle", 0, 0, 0);
        return rval;
      }
  }
  /* Open IPv6 DSS API */
  ...
  (
    net_profile.ip_family  == QCMAP_IP_V4V6_V01
                          ||
    net_profile.ip_family  == QCMAP_IP_V6_V01
  )
  {
    dss_init_net_policy_info(&qcmap_embd_cb.ipv6_net_policy);
    qcmap_embd_cb.ipv6_net_policy.iface.kind = DSS_IFACE_NAME;
    switch (net_profile.tech)
    {
      case QCMAP_WAN_TECH_3GPP:
        qcmap_embd_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_UMTS;
        break;
      case QCMAP_WAN_TECH_3GPP2:
        qcmap_embd_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_CDMA_SN;
        break;
      case QCMAP_WAN_TECH_ANY:
      default:
        qcmap_embd_cb.ipv6_net_policy.iface.info.name = DSS_IFACE_ANY;
        break;
    }
    qcmap_embd_cb.ipv6_net_policy.family = DSS_AF_INET6;
    qcmap_embd_cb.ipv6_net_policy.umts.pdp_profile_num =
                                                net_profile.umts_profile_index;
    qcmap_embd_cb.ipv6_net_policy.cdma.data_session_profile_id =
                                                net_profile.cdma_profile_index;
    qcmap_embd_cb.ipv6_dss_handle =
                                 dsnet_get_handle(&qcmap_cm_dss_net_embd_cb_fcn,
                                 (void *)0, &qcmap_cm_dss_sock_embd_cb_fcn,
                                 (void *)0, &qcmap_embd_cb.ipv6_net_policy,
                                 &dss_errno);
    LOG_MSG_INFO1("qcmap_embdcall: ipv6_dss_handle %X",
                                           qcmap_embd_cb.ipv6_dss_handle, 0, 0);
    if (qcmap_embd_cb.ipv6_dss_handle < 0)
    {
      *qcmap_embd_errno = QCMAP_CM_EOPNOTSUPP;
      qcmap_cm_embd_unlock();
      LOG_MSG_ERROR("qcmap_cm_embdcall: Can not get ipv6 dss net handle",
                                                                       0, 0, 0);
      return rval;
    }
  }
  ...
  if (qcmap_embd_cb.dss_handle >= 0)
  {
    qcmap_embd_cb.state = QCMAP_CM_LAN_CONNECTING;
    *qcmap_embd_errno = QCMAP_CM_EWOULDBLOCK;
    rval = dsnet_start(qcmap_embd_cb.dss_handle, &dss_errno);
    if (rval != DSS_SUCCESS && dss_errno != DS_EWOULDBLOCK)
    {
      LOG_MSG_INFO1("qcmap_cm_enable: dsnet_start rval %d dss_errno %d",
                                                            rval, dss_errno, 0);
      // Fake a NONET
      qcmap_cm_dss_net_embd_cb_fcn(qcmap_embd_cb.dss_handle, DSS_IFACE_ANY,
                                                            DS_ENETNONET, NULL);
    }
  }
   /* Connecting IPv6 LAN */
  if (qcmap_embd_cb.ipv6_dss_handle >= 0)
  {
    qcmap_embd_cb.ipv6_state = QCMAP_CM_V6_LAN_CONNECTING;
    *qcmap_embd_errno = QCMAP_CM_EWOULDBLOCK;
    if ( net_profile.ip_family  == QCMAP_IP_V4V6_V01 )
      sleep(2); // temporary hack to delay the v6 call
    rval = dsnet_start(qcmap_embd_cb.ipv6_dss_handle, &dss_errno);
    if (rval != DSS_SUCCESS && dss_errno != DS_EWOULDBLOCK)
    {
      LOG_MSG_INFO1("qcmap_cm_enable: dsnet_start ipv6 rval %d dss_errno %d",
                                                            rval, dss_errno, 0);
      // Fake a NONET
      qcmap_cm_dss_net_embd_cb_fcn(qcmap_embd_cb.ipv6_dss_handle, DSS_IFACE_ANY,
                                                            DS_ENETNONET, NULL);
    }
  }
  return rval;
...
)
{
  ...
  LOG_MSG_INFO1("qcmap_cm_embdcall_disable: enter", 0, 0, 0);
  if
  (
    qcmap_embd_cb.state == QCMAP_CM_DISABLE
                        &&
    qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_DISABLE
  )
  {
     LOG_MSG_INFO1("No active Embedded Call available. No need to disable.",
                                                                       0, 0, 0);
     return dss_errno;
  }
  *qcmap_cm_errno = QCMAP_CM_SUCCESS;
  ds_assert(qcmap_cm_errno != NULL);
  qcmap_cm_embd_lock();
  if (qcmap_embd_cb.state == QCMAP_CM_LAN_CONNECTED ||
                       qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTED)
  {
    LOG_MSG_INFO1("In condition :QCMAP_CM_LAN_CONNECTED || QCMAP_CM_V6_LAN_CONNECTED", 0, 0, 0);
    qcmap_embd_cb.disable_in_process = TRUE;
    if (qcmap_embd_cb.dss_handle >= 0)
    {
      qcmap_embd_cb.state = QCMAP_CM_LAN_DISCONNECTING;
      dsnet_stop(qcmap_embd_cb.dss_handle, &dss_errno);
    }
    if (qcmap_embd_cb.ipv6_dss_handle >= 0)
    {
      qcmap_embd_cb.ipv6_state = QCMAP_CM_V6_LAN_DISCONNECTING;
      dsnet_stop(qcmap_embd_cb.ipv6_dss_handle, &dss_errno);
    }
    qcmap_cm_embd_unlock();
  }
  else if(qcmap_embd_cb.state <= QCMAP_CM_ENABLE ||
                                 qcmap_embd_cb.ipv6_state <= QCMAP_CM_V6_ENABLE)
  {
    LOG_MSG_INFO1("In condition :QCMAP_CM_ENABLE || QCMAP_CM_V6_ENABLE", 0, 0, 0);

    if ( qcmap_embd_cb.state <= QCMAP_CM_ENABLE )
    {
      qcmap_embd_cb.state = QCMAP_CM_DISABLE;
      if ( qcmap_embd_cb.dss_handle >= 0 )
      {
        dsnet_release_handle(qcmap_embd_cb.dss_handle, &dss_errno);
        qcmap_embd_cb.dss_handle = QCMAP_LAN_INVALID_QCMAP_HANDLE;
      }
    }

    if ( qcmap_embd_cb.ipv6_state <= QCMAP_CM_V6_ENABLE )
    {
      qcmap_embd_cb.ipv6_state = QCMAP_CM_V6_DISABLE;
      if(qcmap_embd_cb.ipv6_dss_handle >= 0)
      {
        dsnet_release_handle(qcmap_embd_cb.ipv6_dss_handle, &dss_errno);
        qcmap_embd_cb.ipv6_dss_handle = QCMAP_LAN_INVALID_QCMAP_HANDLE;
      }
    }

    LOG_MSG_INFO1("Disable: STATES V4 %d V6 %d", qcmap_embd_cb.state,
                                                   qcmap_embd_cb.ipv6_state, 0);
    qcmap_cm_embd_unlock();

    if ( qcmap_embd_cb.state == QCMAP_CM_DISABLE &&
         qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_DISABLE )
    {
      qcmap_embd_cb.disable_in_process = FALSE;
      LOG_MSG_INFO1("QCMAP Embedded Handle 0x%04x Disabled", qcmap_embd_cb.handle, 0, 0);
    }

  }
  else
  {
    LOG_MSG_INFO1("unknown Condition", 0, 0, 0);
    qcmap_embd_cb.disable_in_process = TRUE;
    qcmap_embd_cb.state = QCMAP_CM_LAN_DISCONNECTING;
    dsnet_stop(qcmap_embd_cb.dss_handle, &dss_errno);
    qcmap_embd_cb.dss_handle = QCMAP_LAN_INVALID_QCMAP_HANDLE;
...
/*============================================================
  FUNCTION  qcmap_cm_dss_embd_cb_fcn
=============================================================*/
/*!
@brief
    QCMAP CM DSS net callback
...
)
{
  ...
  int i;
  int err;
  int *qcmap_embd_errno = &err;
  boolean callback = FALSE;
  char command[200];

  LOG_MSG_INFO1_6("qcmap_cm_dss_net_embd_cb_fcn: dss_nethandle %d dss_errno %d "
                " qcmap_cm_embd_cb.ip_state (V4) %d qcmap_cm_embd_cb.ipv6_state (V6) %d",
                dss_nethandle, dss_errno,
                qcmap_embd_cb.state, qcmap_embd_cb.ipv6_state,0,0);
  if(dss_nethandle == qcmap_embd_cb.ipv6_dss_handle)
  {
    if(qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTING)
    {
        if(dss_errno == DS_ENETISCONN)
        {
          dss_iface_id_type if_id;
          ...
          qcmap_embd_cb.ipv6_state = QCMAP_CM_V6_LAN_CONNECTED;
          qcmap_cm_embd_unlock();
          event = QCMAP_CM_EVENT_EMBD_IPv6_CONNECTED;
          callback = TRUE;
          LOG_MSG_INFO1("Received DS_ENETISCONN in state V6_LAN_CONNECTING" ,0,0,0);
          LOG_MSG_INFO1("Transition from state V6_LAN_CONNECTING to"
                                                      " V6_LAN_CONNECTED",0,0,0);
          if_id = dss_get_iface_id(qcmap_embd_cb.ipv6_dss_handle);
          memset(&device_info, 0, sizeof(device_info));
          rval = dss_iface_ioctl(if_id,
                                 DSS_IFACE_IOCTL_GET_DEVICE_INFO,
                                 &device_info,
                                 &dss_ioctl_errno);
          LOG_MSG_INFO1("rval:%d dss_ioctl_errno %d", rval, dss_ioctl_errno, 0);
          if(rval == QCMAP_CM_ERROR || dss_ioctl_errno == DS_EINVAL)
          {
            LOG_MSG_ERROR ("Couldn't get ipv6 rmnet name. rval %d"
                                  " dss_ioctl_err %d", rval, dss_ioctl_errno, 0);
            strlcpy((char*) &device_info.device_name, QCMAP_V6_DEFAULT_DEVICE_NAME, 16);
          }
          LOG_MSG_INFO1("device_name:%s, interface index is:%d",
                        device_info.device_name, device_info.if_index, 0);
          memcpy(&qcmap_embd_cb.ipv6_dss_device_info, &device_info,
                                      sizeof(dss_iface_ioctl_device_info_type));
          /* Enable IPv6 RA acceptatance for the Embedded call. */
          snprintf(command, 200,
             "echo 2 > /proc/sys/net/ipv6/conf/%s/accept_ra", device_info.device_name);
          ds_system_call(command, strlen(command));
          LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Connected",
                                                    qcmap_embd_cb.handle, 0, 0);
        }
        else if (dss_errno == DS_ENETNONET)
        {
          qcmap_cm_embd_lock();
          ...
          qcmap_cm_embd_unlock();
          event = QCMAP_CM_EVENT_EMBD_IPv6_DISCONNECTED;
          callback = TRUE;
          LOG_MSG_INFO1("Received DS_ENETNONET in state V6_LAN_CONNECTING",
                                                                         0,0,0);
          LOG_MSG_INFO1("Transition from state V6_LAN_CONNECTING to V6_DISABLE",
                                                                         0,0,0);
          LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Connecting Fail",
                                                    qcmap_embd_cb.handle, 0, 0);
        }
    }
    else if (qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_LAN_CONNECTED)
    {
      if (dss_errno == DS_ENETISCONN)
      {
          LOG_MSG_INFO1("Received DS_ENETISCONN in state V6_LAN_CONNECTED",
                                                                         0,0,0);
          LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_LAN_CONNECTED",
                                                                         0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Disconnected",
                                                    qcmap_embd_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received event DS_ENETNONET in state V6_LAN_CONNECTED",
                                                                         0,0,0);
        LOG_MSG_INFO1("Transition from V6_LAN_CONNECTED to V6_ENABLE",0,0,0);
        if(qcmap_embd_cb.disable_in_process)
        {
          qcmap_cm_embdcall_disable(qcmap_embd_errno);
        }
      }
    }
    else if(qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_LAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in state"
                                                 " V6_LAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state V6_LAN_DISCONNECTING"
                                                                        ,0,0,0);
      }
      else if(dss_errno == DS_ENETNONET)
      {
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv6 LAN Disconnected",
                                                    qcmap_embd_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received event DS_ENETNONET in state"
                                                  " V6_LAN_DISCONNECTING",0,0,0);
        LOG_MSG_INFO1("Transition from V6_LAN_DISCONNECTING to V6_ENABLE",0,0,0);
        if(qcmap_embd_cb.disable_in_process)
        {
    ...
    else if(qcmap_embd_cb.ipv6_state == QCMAP_CM_V6_ENABLE)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in V6_ENABLE",0,0,0);
  ...
  if (dss_nethandle == qcmap_embd_cb.dss_handle)
  {
    if (qcmap_embd_cb.state == QCMAP_CM_LAN_CONNECTING)
    {
      if (dss_errno == DS_ENETISCONN)
      {
        ...
        qcmap_cm_embd_lock();
        qcmap_embd_cb.state = QCMAP_CM_LAN_CONNECTED;
        qcmap_cm_embd_unlock();
        event = QCMAP_CM_EVENT_EMBD_CONNECTED;
        callback = TRUE;
        LOG_MSG_INFO1("qcmap_cm_dss_net_embd_cb_fcn : Received DS_ENETISCONN in state LAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("QCMAP EMBEDDED CALL CONNECTED", 0,0,0);

        if_id = dss_get_iface_id(qcmap_embd_cb.dss_handle);
        memset(&device_info, 0, sizeof(device_info));
        rval = dss_iface_ioctl(if_id,
                               DSS_IFACE_IOCTL_GET_DEVICE_INFO,
                               &device_info,
        ...
        LOG_MSG_INFO1("device_name:%s, interface index is:%d",
                      device_info.device_name, device_info.if_index, 0);
        memcpy(&qcmap_embd_cb.dss_device_info, &device_info,
                                      sizeof(dss_iface_ioctl_device_info_type));
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Connected",
                                                    qcmap_embd_cb.handle, 0, 0);
      }
      else if (dss_errno == DS_ENETNONET)
      {
        ...
        qcmap_cm_embd_unlock();
        event = QCMAP_CM_EVENT_EMBD_DISCONNECTED;
        callback = TRUE;
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Connecting Fail",
                                                    qcmap_embd_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received DS_ENETNONET in state LAN_CONNECTING",0,0,0);
        LOG_MSG_INFO1("Transition from state LAN_CONNECTING to DISABLE",0,0,0);
      }
    }
    else if (qcmap_embd_cb.state == QCMAP_CM_LAN_CONNECTED)
    {
      if (dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received DS_ENETISCONN in state LAN_CONNECTED",0,0,0);
      ...
      {
        qcmap_cm_embd_lock();
        qcmap_embd_cb.state = QCMAP_CM_ENABLE;
        qcmap_cm_embd_unlock();
        event = QCMAP_CM_EVENT_EMBD_DISCONNECTED;
        callback = TRUE;
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Disconnected",
    ...
    else if (qcmap_embd_cb.state == QCMAP_CM_LAN_DISCONNECTING)
    {
      if(dss_errno == DS_ENETISCONN)
      {
        LOG_MSG_INFO1("Received event DS_ENETISCONN in state LAN_DISCONNECTING"
                                                                        ,0,0,0);
        LOG_MSG_INFO1("Ignore event DS_ENETISCONN in state LAN_DISCONNECTING",
                                                                         0,0,0);
      }
      else if (dss_errno == DS_ENETNONET)
      {
        ...
        LOG_MSG_INFO1("QCMAP AP Handle 0x%04x IPv4 LAN Disconnected",
                                                    qcmap_embd_cb.handle, 0, 0);
        LOG_MSG_INFO1("Received event DS_ENETNONET in state LAN_DISCONNECTING",
                                                                         0,0,0);
        LOG_MSG_INFO1("Transition from LAN_DISCONNECTING to ENABLE",0,0,0);
        if(qcmap_embd_cb.disable_in_process)
        {
  ...
  LOG_MSG_INFO1("QCMAP EMBEDDED CALL : Callback %d, Event %d",callback,event,0);
  if (callback)
    qcmap_cm_cb.qcmap_cm_callback(qcmap_cm_cb.handle, event,
                              qcmap_cm_cb.qcmap_cm_callback_user_data);
}
...
/*===========================================================================
  FUNCTION  qcmap_cm_dss_sock_embd_cb_fcn
===========================================================================*/
static void
qcmap_cm_dss_sock_embd_cb_fcn
(
