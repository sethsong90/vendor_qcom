...
/******************************************************************************
  ...
  DESCRIPTION
  Implementation file for QTI Netlink messaging functions.
...
--------------------------------------------------------------------------*/
  std_strlcpy(qti_netlink_conf->if_dev[0].dev_name,
              RNDIS_INTERFACE,
              IF_NAME_LEN);
  std_strlcpy(qti_netlink_conf->if_dev[1].dev_name,
              ECM_INTERFACE,
              IF_NAME_LEN);
...
/*-------------------------------------------------------------------------
  Bring up rndis0 and ecm0 interfaces to listen to netlink events on them.
  This was moved from QTI initializaton to here to allow USB to complete
  rndis0 and ecm0 enumeration.
--------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
  std_strlcpy(qti_netlink_conf->if_dev[0].dev_name, RNDIS_INTERFACE, IF_NAME_LEN);
  std_strlcpy(qti_netlink_conf->if_dev[1].dev_name, ECM_INTERFACE, IF_NAME_LEN);
  LOG_MSG_INFO1("Assigned device names %s,%s",
                qti_netlink_conf->if_dev[0].dev_name,
                qti_netlink_conf->if_dev[1].dev_name,0);
...
/*------------------------------------------------------------------------*/
  ...
  LOG_MSG_INFO1("Re-enable RNDIS and ECM interfaces", 0, 0, 0);
  if(interface == 0)
  {
    ds_system_call("ifconfig rndis0 up", strlen("ifconfig rndis0 up"));
  }
