...
#ifndef LOGE_IF
#define LOGE_IF(cond, ...) \
    ( (CONDITION(cond)) ? ((void)LOGE(LOG_TAG, __VA_ARGS__)) : (void)0 )
#endif
...
int DummySensor::ALSRegister()
{
    ...
    LOGE_IF(mDebug, "=== Beginning Reading Dummy ALS file %s ===", al_dummy);
    // open dummy file
    ...
    while (fgets(line, MAX_DUMMY_LINE_LEN * sizeof(char), dummy) &&
                i < MAX_DUMMY_ALS) {
        als_vals[i] = atoi(line);
        LOGE_IF(mDebug, "line %d = %d", i, als_vals[i]);
        i++;
    }
    max_als_vals = i;
    ...
    ret = 0;
    LOGE_IF(mDebug, "=== Finished Reading Dummy ALS file ===");
err_mem:
    //close dummy file
    fclose(dummy);
...
void DummySensor::processDummyWork(union sigval sigev_value)
{
    ...
    LOGE_IF(mDebug, "sensor->mALSValue = %d", sensor->mALSValue);
    sensor->mALSReady = 1;
