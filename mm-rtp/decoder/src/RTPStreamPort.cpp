                              ...
                              /*rout*/ int* pnRead)
{
  ...
  int32 nRet  = m_pRTPDataSource->readAt(mOffset, (void*)pBuf, nBufSize);
  *pnRead = (int)nRet;
  return DS_SUCCESS;
}
...
iStreamPort::DataSourceReturnCode RTPStreamPort::GetRTPBaseTimeUs(/*rout*/ int64* pBaseTimeUs)
{
  ...
  android::status_t eRet = m_pRTPDataSource->getRTPBaseTime((off64_t*)pBaseTimeUs);
...
iStreamPort::DataSourceReturnCode RTPStreamPort::GetNumBytesAvailable(/*rout*/ int64* pNumBytesAvailable)
{
  ...
  android::status_t eRet = m_pRTPDataSource->getSize((off64_t*)pNumBytesAvailable);
  return (android::OK == eRet ? DS_SUCCESS : DS_FAILURE);
}
...
iStreamPort::DataSourceReturnCode RTPStreamPort::GetAvailableOffset(/*rout*/ int64* pAvailableOffset,
                                          /*rout*/ bool* pbEOS)
{
  android::status_t eRet = m_pRTPDataSource->getSize((off64_t*)pAvailableOffset);
                                          ...
                                          /*rout*/ bool* pbEOS)
{
  ...
  android::status_t eRet = m_pRTPDataSource->getHeadOffset((off64_t*)pAvailableOffset);
  *pbEOS = FALSE;
  return (android::OK == eRet ? DS_SUCCESS : DS_FAILURE);
}
