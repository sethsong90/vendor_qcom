...
namespace android
{
  ...
  /*==========================================================================
   ...
   ===========================================================================*/
  ssize_t RTPDataSource::readAt(off64_t offset, void *data, size_t size)
  {
    int32 nBytes = 0;
    if(m_eRTPState == RTP_FETCHING_DATA)
    {
...
#endif /*RTP_READ_FROM_FILE*/
      {
        ...
        /*Don't call RTPParser read, if complete requested data is not available as RTPParser read is blocking call
              */
        if((offset + size) <= numBytesAvailable || numBytesAvailable >= TS_PKT_LEN)
        {
  ...
  void RTPDataSource::init(int32 port, int32 payloadType, int rtpSock, bool tcp)
  {
    ...
    {
      MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTPDataSource:error in creating RTP parser for payload type %lu",payloadType);
      return;
    }
  ...
  status_t RTPDataSource::start()
  {
    ...
    MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR, "RTPDataSource:payload buffer length %lld",nBufSize);
  ...
  int32 RTPDataSource::setSocketOptions(int32 socket)
  {
    ...
    if( 0 != setsockopt(socket, SOL_SOCKET, SO_RCVTIMEO,(struct timeval *)&tv,sizeof(struct timeval)) )
    {
    ...
    if( recvBufferSize < (RTP_SOCKET_RECEIVE_BUFFER_SIZE * 2) )
    {
      ...
      if( 0 != setsockopt(socket, SOL_SOCKET, SO_RCVBUF, &recvBufferSize, sizeof(recvBufferSize)) )
      {
  ...
  int32 RTPDataSource::receiveRTPPacket(uint8* rtpBuffer)
  {
    ...
    if(m_bTCPConnection)
    {
      ...
      nBytes = recvfrom(socket, rtpBuffer, 2/*framingheader */,
                             flags, NULL, NULL);

      uint16 nDataSize = (((uint16)rtpBuffer[0]) << 8) | ((uint16)rtpBuffer[1]);

      if(nDataSize <= RTP_PACKET_MAXIMUM_SIZE)
      {
         nBytes = recvfrom(socket, rtpBuffer, nDataSize,
                             flags, NULL, NULL);
      }
    }
    else
    {
      nBytes = recvfrom(socket, rtpBuffer, RTP_PACKET_MAXIMUM_SIZE,
                             flags, NULL, NULL);
    }
  ...
  int32  RTPDataSource::acceptTCPConnection(int32 tcpSocket)
  {
    ...
    if( 0 == listen(tcpSocket, 1) )
    {
      ...
      {
        acceptedSocket = accept(tcpSocket, NULL, NULL);
        if( acceptedSocket >= 0)
        {
  ...
  /*==========================================================================
    ...
    ===========================================================================*/
  status_t RTPDataSource::getRTPBaseTime(off64_t *baseTimeUs)
  {
    status_t status = OK;
  ...
  status_t RTPDataSource::initCheck() const
  {
    ...
    /*==========================================================================
      ...
      RETURN VALUE: returns OK for success
     ===========================================================================*/
    status_t RTPDataSource::getHeadOffset(off64_t *size)
    {
       int32 nStatus = -1;
  ...
  /*==========================================================================
    ...
    RETURN VALUE: returns OK for success
   ===========================================================================*/
  status_t RTPDataSource::getSize(off64_t *size)
  {
     int32 nStatus = -1;
