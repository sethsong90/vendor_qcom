    ...
    */
                                   ...
                                   "RTCP", &m_pRTCPThread) )
    {
        MM_MSG_PRIO1(MM_GENERAL, MM_PRIO_ERROR,
           "CRTPReceiver::CRTPReceiver Thread create failed %d!!!",m_pRTCPThread);
        m_pRTCPThread = NULL;
...
int CRTCPReceiver::RTCPThread( void )
{
    ...
    setsockopt(m_nSockfd, SOL_SOCKET, SO_RCVTIMEO,(struct timeval *)&tv,sizeof(struct timeval));
    ...
    while ( m_eState == RTCP_STATE_RUNNING )
    {
        ...
        if(nBytesRead > 0)
        {
            ...
            m_pCallback(RTP_RTCP_RR_EVENT, RTP_SUCCESS, (int)(&sData), m_pUserData);
        }
        MM_Timer_Sleep(1);
    }
...
int CRTCPReceiver::RecvPacket(uint8 *pData, uint32 nLen)
{
    ...
    iRecvResult = recvfrom(m_nSockfd, pData, nLen, 0, NULL, NULL);
