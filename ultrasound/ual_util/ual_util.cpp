                             ...
                             us_all_info *paramsStruct)
{
  ...
  while (NULL != line)
  {
    ...
    else if (strcmp(paramName, "usf_epos_on_screen_transform_origin") == 0)
    {
      strlcpy(paramsStruct->usf_epos_on_screen_transform_origin,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_epos_on_screen_transform_origin is: %s",
           __FUNCTION__,
           paramsStruct->usf_epos_on_screen_transform_origin);
    }
    else if (strcmp(paramName, "usf_epos_on_screen_transform_end_X") == 0)
    {
      strlcpy(paramsStruct->usf_epos_on_screen_transform_end_X,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_epos_on_screen_transform_end_X is: %s",
           __FUNCTION__,
           paramsStruct->usf_epos_on_screen_transform_end_X);
    }
    else if (strcmp(paramName, "usf_epos_on_screen_transform_end_Y") == 0)
    {
      strlcpy(paramsStruct->usf_epos_on_screen_transform_end_Y,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_epos_on_screen_transform_end_Y is: %s",
           __FUNCTION__,
           paramsStruct->usf_epos_on_screen_transform_end_Y);
    }
    ...
    else if (strcmp(paramName, "usf_pairing_socket_path") == 0)
    {
      strlcpy(paramsStruct->usf_pairing_socket_path,
              temp,
              FILE_PATH_MAX_LEN);
      LOGD("%s: usf_pairing_socket_path is: %s",
           __FUNCTION__,
           paramsStruct->usf_pairing_socket_path);
    }
    ...
    else if (strcmp(paramName, "usf_pairing_num_series_calib_files") == 0)
    {
      ret = sscanf(temp,
                   "%d",
                   &paramsStruct->usf_pairing_num_series_calib_files);
      LOGD("%s: usf_pairing_num_series_calib_files is: %d",
           __FUNCTION__,
           paramsStruct->usf_pairing_num_series_calib_files);
    }
...
*/
void ual_util_write_file(const char *file_path,
                          void *data,
                          uint32_t data_size)
{
