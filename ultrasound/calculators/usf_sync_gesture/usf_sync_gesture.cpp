...
void gesture_lib_init (us_all_info *paramsStruct)
{
  ...
  gesture_config->TxTransparentDataSize = paramsStruct->usf_tx_transparent_data_size;
  memcpy(&(gesture_config->TxTransparentDataSize),
         paramsStruct->usf_tx_transparent_data,
         sizeof(gesture_config->TxTransparentDataSize));
  ...
  LOGI("Pattern received begins with: %d,%d,%d,%d",
       gestureContext.m_pattern[0],
       gestureContext.m_pattern[1],
       gestureContext.m_pattern[2],
       gestureContext.m_pattern[3]);
...
/**
  ...
  result decision.
*/
QcUsGestureLibResultType get_gesture_output_decision(short *packet,
                                                     us_all_info *paramsStruct)
{
  ...
  */
  ...
  if (QC_US_GESTURE_LIB_RESULT_IDLE != gesture_lib_output.gesture)
  {
    return gesture_lib_output.gesture;
  }
  else if (QC_US_GESTURE_LIB_RESULT_IDLE != dsp_result)
  {
    return dsp_result;
  }
  return QC_US_GESTURE_LIB_RESULT_IDLE;
}
                       ...
                       us_all_info *paramsStruct)
{
  ...
  gesture_output.gesture = get_gesture_output_decision(packet,
                                                       paramsStruct);
    ...
    if (NULL != adapter)
    {
      int event = (adapter->get_event_mapping() == MAPPED) ?
        ...
        // Framework requests raw events
                  gesture_output.gesture;
      if (1 == adapter->send_event(event, event_source))
      {
