...
#define DISABLED_TS 1
#define ENABLED_TS 0
// Thresholds for enabling/disabling touchscreen. Units of 1/1000 cm
#define TS_HIGH_Z_THRESHOLD 4000
#define TS_LOW_Z_THRESHOLD 3500
#define NO_POINT_MAX 100
#define UNKNOWN_LAST_POINT_Z -1
// Smarter stand rotation angle shift for library reports
#define CMD_ROTATION_ANGLE_SHIFT 15
...
static const int MAX_PRESSURE = 1023;
...
*/
static int ts_state_enable = ENABLED_TS;
...
typedef struct
{
  ...
  long  channel_num;
  long  packet_num;
  long  payload [EPOS_PACKET_SIZE_DWORDS];
  long  checksum;
} EposPacket;
...
*/
static const uint32_t UNDEF_ACT_CHANNELS = 0xffffffff;

/**
  Active channels
*/
static int s_act_channels = UNDEF_ACT_CHANNELS;
/**
  Time to require active channels state
*/
static double s_act_channel_test_time;
...
*/
static void enable_disable_ts (int pressure_to_send,
                               long z_coord,
                               region_zone_t rz)
{
  bool send_to_ual = (eposParams.m_on_screen_send_points_to_ual ||
                      eposParams.m_off_screen_send_points_to_ual);
  if ((TS_HIGH_Z_THRESHOLD <= z_coord) ||
      // Point has negative z and outside on screen active zone.
      ((0 > z_coord) && (ON_SCREEN_WORK_ZONE != rz &&
                         ON_SCREEN_ACTIVE_ZONE_BORDER != rz)))
  {
    if (ts_state_enable == DISABLED_TS)
    {
      ts_state_enable = ENABLED_TS;
    }
  }
  else if ((pressure_to_send) || (TS_LOW_Z_THRESHOLD > z_coord))
  {
    // Disable ts only if ual and ts are enabled
    if (send_to_ual && ENABLED_TS == ts_state_enable)
    {
...
static int update_persistent_data()
{
  if (NULL == eposParams.m_calib_files[EPOS_PERSISTENT_FILE].calib_packet ||
      MAX_PERSIST_DATA_SIZE * sizeof(long) >
  ...
  eposParams.m_calib_files[EPOS_PERSISTENT_FILE].calib_packet_len =
        epos_lib_proxy->get_persistent_data(
          ...
          (long *)eposParams.m_calib_files[EPOS_PERSISTENT_FILE].calib_packet,
...
void init_transform_settings(us_all_info *paramsStruct)
{
  ...
  on_screen_plane_props.origin = paramsStruct->usf_epos_on_screen_transform_origin;
  on_screen_plane_props.point_end_x = paramsStruct->usf_epos_on_screen_transform_end_X;
  on_screen_plane_props.point_end_y = paramsStruct->usf_epos_on_screen_transform_end_Y;
...
static void load_coeffs(int file_num) {
  ...
  else
  {
    if ((rc = epos_lib_proxy->load_coeffs(
      ...
      eposParams.m_calib_files[file_num].calib_packet_len / sizeof(long))))
    {
      snprintf(msg,
             MAX_FILE_NAME_LEN,
...
void epos_init_smarter_stand()
{
  ...
  on_screen_origin = s_work_params.paramsStruct.usf_epos_on_screen_transform_origin;
  off_screen_origin = s_work_params.paramsStruct.usf_epos_off_screen_transform_origin;
  ...
  axis_direction.vector_mult_by_scalar(-(1 << CMD_ROTATION_ANGLE_SHIFT));;

  long origin[3] = {
    (long)axis_origin.get_element(X),
    (long)axis_origin.get_element(Y),
    (long)axis_origin.get_element(Z)
    };

  long direction[3] =
  {
    (long)axis_direction.get_element(X),
    (long)axis_direction.get_element(Y),
    (long)axis_direction.get_element(Z)
  };

  // Off screen and on screen planes are assumed to be parallel
  long screen_distance = abs(on_screen_origin.get_element(Z) -
  ...
  LOGD("Library smart stand params: %ld",
       screen_distance);
...
)
{
  ...
  long epos_points_per_pen, epos_max_pens, epos_workspace_size;
  ...
  s_act_channels = UNDEF_ACT_CHANNELS;
  s_act_channel_test_time = 0;
...
static uint16_t get_tool_rubber_mask(long type)
{
    ...
    long btn_mask = (ERASER_BUTTON_INDEX_BUTTON1 == eposParams.m_eraser_button_index) ?
                    COORD_SW1 : COORD_SW2;
    bool is_sb_pressed = !!(type & btn_mask);
                                         ...
                                         long type)
{
  ...
  {
    buttons_state_bitmap |= ((!!(type & COORD_SW2)) << BTN_STYLUS_SHIFT);
  }
  if (paramsStruct->req_buttons_bitmap & BTN_STYLUS2_MASK)
  {
    buttons_state_bitmap |= ((!!(type & COORD_SW1)) << BTN_STYLUS2_SHIFT);
  }
  if (paramsStruct->req_buttons_bitmap & BTN_TOOL_PEN_MASK)
  {
    buttons_state_bitmap |= (1 << BTN_TOOL_PEN_SHIFT); // Always send PEN tool type if requested (for now)
  }
  if (paramsStruct->req_buttons_bitmap & BTN_TOOL_RUBBER_MASK)
  {
    buttons_state_bitmap |= get_tool_rubber_mask(type);
  }
 ...
 * @param pressure the pen pressure value for the current point
*/
...
)
{
  ...
  // - stylus not touching the screen (nPressure <= 0)
  ...
  bool pen_proximity_condition = (pressure <= 0 && ts_state_enable);
...
)
{
  ...
  switch (event_type_ind)
  {
    ...
    case USF_TSC_PTR_EVENT_IND:
      pEvent->event_data.point_event.pressure = nPressure;
      if (force_buttons_up)
      {
      ...
      else
      {
        ...
        if (get_current_hover_icon_state(rz, nPressure))
        {
          pEvent->event_data.point_event.buttons_state_bitmap |=
...
static void get_act_channels(void *InWorkspace, double& time)
{
  ...
  if (!s_act_channel_test_time)
  { // first time entry
    s_act_channel_test_time = time + ACT_CHANNEL_TIME;
  }
  else
  {
    if (time >= s_act_channel_test_time)
    { // It's time to check active channels
      ...
      int ret = epos_lib_proxy->command(InWorkspace,
                                        (long*)&cmd_act_channels);
      if (!ret)
      {
        if (cmd_act_channels.args[0] != s_act_channels)
        {
          int num_of_blocked_mic = 0;
          s_act_channels = cmd_act_channels.args[0];

          for (int i = 0; i < s_work_params.paramsStruct.usf_tx_port_count; i++)
          {
            int act_channel_mask = 1 << i;
            bool mic_i_open = s_act_channels & act_channel_mask;
            if (!mic_i_open)
            {
              num_of_blocked_mic++;
            }
          ...
          LOGD("%s: ret=%d; act_channels=%d; num_of_blocked_mic=%d",
               __FUNCTION__,
               ret,
               s_act_channels,
               num_of_blocked_mic);
...
{
  static long s_last_battery_state = 0;
  long battery_state = 0;

  epos_lib_proxy->query_epoint(InWorkspace,
                               POINT_Battery,
                               0,
                               &battery_state,
                               sizeof(battery_state)/sizeof(long));
...
{
  // Record coords to coordFile
  int coord_count = s_work_params.paramsStruct.usf_epos_coord_count;
  if ((NULL != s_work_params.coordFile) &&
      (eposStats.m_nPointsCalculated < coord_count))
  {
    // coordsLeft is the number of coords left to record.
    int coordsLeft = coord_count - eposStats.m_nPointsCalculated;

    // Write valid points to file, but no more than coordsLeft
    for (int coord_idx = 0;
          (coordsLeft > 0) && (coord_idx < num_points); coord_idx++)
    {
    ...
    if (coordsLeft == 0)
    {
      LOGD("%s: Done recording EPOS coordinates",
         __FUNCTION__);
      fclose(s_work_params.coordFile);
...
==============================================================================*/
static void send_socket_event(usf_extended_epoint_t const &raw_point,
                              int pressure)
{
  int ret = m_epos_data_socket->send_epos_point(raw_point.X,
                                                raw_point.Y,
                                                raw_point.Z,
                                                raw_point.TiltX,
                                                raw_point.TiltY,
 ...
 *
 * @param outPoint The current point.
 *
...
{
  int pressure = (transformed_point.Type & EPOS_TYPE_PEN_DOWN_BIT) ? MAX_PRESSURE :
                                                                     MIN_PRESSURE;
  ...
  bool send_to_socket = (eposParams.m_on_screen_send_points_to_socket &&
                        ON_SCREEN_WORK_ZONE == rz) ||
                        (eposParams.m_off_screen_send_points_to_socket &&
                        OFF_SCREEN_WORK_ZONE == rz);
...
*/
int library_calculate_points(long *pPacket,
                             void *InWorkspace,
...
*/
static void epos_get_points (long *pPacket,
                             void *InWorkspace,
                             FeedbackInfo *OutFeedback)
{
  long last_point_Z = UNKNOWN_LAST_POINT_Z;
  bool change_ts_state = false;
  region_zone_t region_zone = NOT_IN_ANY_ACTIVE_ZONE;
  static int no_points = NO_POINT_MAX;
  int num_invalid_points = 0;
  region_zone_t last_valid_draw_area = NOT_IN_ANY_ACTIVE_ZONE;
  ...
  for (int i = 0; i < num_points; i++)
  {
    ...
    last_point_Z = outPoint.Z;
    last_valid_draw_area = region_zone;
  ...
  // If EPOS lib continuously returns 0 valid points, the touchscreen is enabled
  if (0 < num_points - num_invalid_points)
  { // There are valid points
    no_points = NO_POINT_MAX;
    change_ts_state = true;
  }
  else
  { // There are no valid points
    ...
    if (0 == no_points)
    {
      last_point_Z = TS_HIGH_Z_THRESHOLD;
      no_points = NO_POINT_MAX;
      change_ts_state = true;
      // If last point was in working zone and pressed, and there are no points
      // anymore, need to send up event.
      handle_outside_work_zone_event();
    }
  }
  if(true == change_ts_state)
  {
    enable_disable_ts(eposParams.m_nLastPoint.Type &
                       EPOS_TYPE_PEN_DOWN_BIT,
                      last_point_Z,
                      last_valid_draw_area);
  }
...
bool usf_epos_get_points(bool& b_us)
{
  ...
  for (int f = 0; f < s_work_params.numberOfFrames ; f++)
  {
    ...
    if (eposParams.m_nNextFrameSeqNum == -1)
    {
    ...
    {
      epos_get_points((long*)s_work_params.nextPacket,
                                   eposParams.m_epos_workspace,
                                   &fb_info);
...
void update_config_parameters()
{
  ...
  LOGD("Config was updated: on screen ual:%d, off screen ual:%d, on screen socket:%d, off screen socket:%d, event type:%d, off screen mode:%d, eraser button index:%d, eraser button mode:%d, on screen hover icon mode: %d, off screen hover icon mode: %d",
       eposParams.m_on_screen_send_points_to_ual,
       eposParams.m_off_screen_send_points_to_ual,
       eposParams.m_on_screen_send_points_to_socket,
       eposParams.m_off_screen_send_points_to_socket,
...
int main(void)
{
  ...
  timespec tsCur;
  clock_gettime(CLOCK_REALTIME, &tsCur);
  double timeBeforeFirstPoint = (double)tsCur.tv_sec; // sec
  ...
  while (rc && sb_run)
  {
    ...
    if (0 < timeout)
    {
      // Current time
      clock_gettime(CLOCK_REALTIME, &tsCur);
      double duration =  (double)tsCur.tv_sec - timeBeforeFirstPoint; // sec
      if (duration >= timeout)
      {
        LOGW("%s: Reach timeout for calibration.",
             __FUNCTION__);
        break;
      }
    }
