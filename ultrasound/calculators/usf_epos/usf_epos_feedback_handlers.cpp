 ...
 */
static int usf_epos_spd_snd_req_handler(long args[],
 ...
 */
typedef int (*usf_epos_fb_hdlr_func)(long args[],
                                     struct usf_epos_command *ret_cmd);
                              ...
                              FeedbackInfo *OutFeedback)
{
  ...
  LOGD("%s: Handling feedback: %ld arg0: %ld",
       __FUNCTION__,
  ...
  for (int i = 0; i < (int)ARRAY_SIZE(fb_handlers); ++i)
    ...
    if (fb_handlers[i].fb_num == OutFeedback->CommandOut[FEEDBACK_INDEX])
    {
      ...
      if (fb_handlers[i].func(OutFeedback->CommandOut + 1,
                              &ret_cmd))
      {
        LOGW("%s: Could not handle %ld feedback",
             __FUNCTION__,
   ...
   LOGW("%s: No handler found for %ld feedback",
        __FUNCTION__,
        OutFeedback->CommandOut[FEEDBACK_INDEX]);
}
