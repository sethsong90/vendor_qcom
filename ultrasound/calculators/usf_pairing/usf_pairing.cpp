...
void pairing_init()
{
  ...
  int rc;
  long pairing_points_per_pen, pairing_max_pens, pairing_workspace_size;
  EPoint *out_points;
                                                   ...
                                                   long ppl_res)
{
  ...
  if ((ERR_DUPL_POS_SERIES == find_ser_res) ||
      ...
      for (int f = 0; f < num_of_frames; ++f)
      {
        ...
        for (int m = 0; m < paramsStruct.usf_tx_port_count; ++m)
        {
          ...
          if ((last_series != fb_info.CurrentSeriesNumber) ||
              ...
              (1 == fb_info.Result))
          {
            LOGD("%s: Validation: %ld, Result: %ld, TX0 Series: %ld, Return value: %ld",
                 __FUNCTION__,
            ...
            if (1 == fb_info.Result)
            {
              LOGD("%s: Series found: Tx0: %ld, Tx1: %ld",
                 __FUNCTION__,
                 fb_info.CurrentSeriesNumber,
