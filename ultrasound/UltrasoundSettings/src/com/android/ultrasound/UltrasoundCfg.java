...
{
  private String cfgFileLocation = null;
  ...
  private LinkedHashMap<Integer, String> cfgFile =
  ...
  private final int WAIT_FOR_DEMON_DELAY_INTERVAL_MS = 500;
  ...
  private String[] commonParamsNames = new String[] {"usf_frame_file",
                                                     "usf_frame_count",
  ...
  /*===========================================================================
  INNER_CLASS:  CfgFileFilter
  ===========================================================================*/
  /**
   CfgFileFilter class is used in the Spinner that shows all the
   cfg files in the cfg file dir
  */
   class CfgFileFilter implements FileFilter
   {
     public boolean accept(File pathname) {
       if(pathname.getName().endsWith("cfg") == true)
       {
         return true;
       }
       return false;
  ...
  */
  public class FileCfgItemSelectedListener implements OnItemSelectedListener
  {
    public void onItemSelected(AdapterView<?> parent, View view, int pos, long id) {
      ...
      {
        Log.d(this.toString(),
                  "in onItemSelected in FileCfgItemSelectedListener");
        setCfgFileLocation(getCfgFilesDir() + parent.getItemAtPosition(pos).toString());
        updateLinkCfgFile();
  ...
  */
  protected void updateUI() {
    ...
    if ((SystemProperties.get("init.svc." + getDaemonName(), "stopped")).
        equals("stopped"))
    {
    ...
    if ((null == frameFileEditText) || (null == debugModeCheckbox) || (null == frameCountEditText))
    {
      return;
    }
    ...
    if (frameCountFile.exists() && frameCountFile.isFile())
    {
      ...
      if ((0 < len_1) &&
          (debugModeCheckbox.getCurrentTextColor() != Color.GREEN) &&
  ...
  */
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    int layoutId = getResources().getIdentifier(getXmlFileName(),
          ...
          else
          {
            ...
            else
            {
              ...
              catch (IOException e)
              {
                ...
                SystemService.stop(getDaemonName()); // Stop daemon with SIGKILL
              }
            }
            clearDSPVersion();
          }
    ...
    /* Finds cfg file. If no such file exist or no cfg dir
       exist then the UI will be empty. */
    try
    {
      File dir = new File(getCfgFilesDir());
      File[] children = dir.listFiles(new CfgFileFilter());
      ...
      String cfgFileName = UltrasoundUtil.getCfgFileLocationFromLinkFile(getCfgLinkFileLocation());
      Log.d(this.toString(),
            "Symbolic link: " + getCfgLinkFileLocation() +
            "->" + cfgFileName);
      // If cfgFileName is null then default file is choosen
      if (null != cfgFileName)
      {
        cfgFileName = cfgFileName.replaceFirst(getCfgExpressionDir(), "");
        setCfgFileLocation(getCfgFilesDir() + cfgFileName);
      }
      else
      {
      ...
      chooseFileSpinner = (Spinner)findViewById(R.id.choose_file_spinner);

      if (null != chooseFileSpinner) {
        ArrayAdapter<String> chooseFileAdapter = new ArrayAdapter<String>(this,
                                   android.R.layout.simple_spinner_item, childrenStrings);
        chooseFileAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    ...
    if ((null != debugModeCheckbox) && (null != frameCountEditText) && (null != frameFileEditText)) {
      debugModeCheckbox.setTextColor(Color.WHITE);
  ...
  void setFrameFileParams()
  {
    ...
    if (null != frameFileFormatSpinner) {
      ...
      frameFileFormatSpinner.
        setOnItemSelectedListener(new FileFormetItemSelectedListener());
      frameFileFormatSpinner.setEnabled(false);
  ...
  */
   ...
   for (String privateParamName : privateParams.keySet())
   {
     if (paramName.equals(privateParamName))
     {
       ...
       else if (privateParams.get(privateParamName).getClass() == Spinner.class)
       {
         ...
         if (-1 != intVal)
         { // The given number could be interpreted as int.
           ((Spinner) privateParams.get(privateParamName)).
               setSelection(intVal);
             return;
         }
         ArrayAdapter myAdap = (ArrayAdapter)
           ((Spinner) privateParams.get(privateParamName)).getAdapter();
         int spinnerPosition;
         for (spinnerPosition = 0; spinnerPosition < myAdap.getCount();
               spinnerPosition++)
         {
  ...
  private void restoreCfgFile()
  {
    ...
    File corrupted = new File(getCfgFileLocation());
    File backup = new File(getCfgFileLocation() + "_backup");
  ...
  */
  protected void readCfgFileAndSetDisp(boolean setDisp) {
    if (false == isFileValid(getCfgFileLocation()))
    {
      restoreCfgFile();
    }
    ...
    try
    {
      FileInputStream fstream = new FileInputStream(getCfgFileLocation());
      ...
      while (null != (strLine = br.readLine()))
      {
        ...
        // This line is param and value - need to update cfgFileParams dict
        if (st.countTokens() >= 2)
        {
  ...
  */
  protected void updateCfgFile() {
    try
    {
      ...
      FileWriter fstream = new FileWriter(getCfgFileLocation() + "_new");
      ...
      while (itr.hasNext())
      {
        ...
        // This line is param and value - find the value in cfgFileParams dict
        else
        {
    ...
    useNewFileNBackupOld(getCfgFileLocation());
  }
   ...
   */
  void writeMissingCfgParams(BufferedWriter out,
                             Set<String> writtenParams,
                             Map<String, String> cfgFileParams)
  {
    Set<String> symmDiff = getSymmetricDifference(writtenParams,
                                                  cfgFileParams.keySet());
    try
    {
   ...
   */
  public <T> Set<T> getSymmetricDifference (Set<T> set1,
                                             Set<T> set2)
  {
  ...
  */
  private boolean updateAutoStartFile(boolean startOnReboot,
                                   boolean checkAutoStartStatus) {
    try
    {
  ...
  */
  private void updateLinkCfgFile()
  {
    try
    {
      Process proc1 = Runtime.getRuntime().exec("rm " + getCfgLinkFileLocation());
      ...
      Process proc2 = Runtime.getRuntime().exec("ln -s " + getCfgFileLocation() +
                                               " " + getCfgLinkFileLocation());
  ...
  */
  protected void setOnKeyListener(final EditText editText,
                                  final String nameOfParam) {
  ...
  /**
    ...
    cfg file location.
  */
  protected void setCfgFileLocation(String cfgFileLocation) {
    ...
    Log.d(this.toString(),
          "Setting cfgFileLocation to " + cfgFileLocation);
  }
  ...
  ===========================================================================*/
  /**
    getCfgFileLocation() function returns the cfg file name from
    the local member cfgFileLocation.
  */
  protected String getCfgFileLocation() {
  ...
  */
  ...
  {
    // Do nothing.
    // Currently this update is needed only in epos and therefore the function
