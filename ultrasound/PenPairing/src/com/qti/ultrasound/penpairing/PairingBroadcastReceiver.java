...
public class PairingBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        Log.d(this.toString(),
              "Received broadcast intent: " + intent.toString());

        int penId;
        String penName;

        if (-1 == (penId = intent.getIntExtra(PEN_ID_INTENT_KEY,
                                              -1))) {
        ...
        SharedPreferences.Editor editor = context.getSharedPreferences(PairedPensActivity.NAME_SHARED_PREFERENCE,
                                                                       0)
                                                 .edit();
        ...
        setResultData("Inserted pen named '" + penName + "' with series "
                + penId + " to database");
    }
