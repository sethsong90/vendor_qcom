...
static int nominal_test(char *argv)
{
	...
	if ((smd_dev_p = open(SMD_DEV_NM, O_RDWR)) <= 0) {
		perror("Open");
		printf("cannot open file %s", SMD_DEV_NM);
		return -1;
	}
...
static void usage(int ret)
{
	printf("Usage: smd_pkt_loopback_test [OPTIONS] [PARAMS] [TEST_TYPE]...\n"
			...
			"over a SMD PKT device (/dev/smd_pkt_loopback).\n"
			"\n"
			"OPTIONS can be:\n"
...
static uint32_t parse_command(int argc, char *const argv[])
{
	...
	while ((command = getopt_long(argc, argv, "v:nasrhcwb:p:i:d:", longopts,
					NULL)) != -1) {
		switch (command) {
		case 'v':
			smd_verbose = 1;
