...
int mapFB(struct fbDev *thisFB)
{
	...
	if (thisFB->fb_finfo.smem_len <= 0) {
		printf("\nframe buffer is not supported\n");
		return 0;
	} else {
	thisFB->fb_buf = thisFB->fb_buf_back = mmap(NULL,
		thisFB->fb_finfo.smem_len, PROT_READ | PROT_WRITE,
	...
	if (thisFB->fb_buf == MAP_FAILED) {
		...
		if (thisFB->fb_finfo.smem_len < (2*(thisFB->fb_size))){
			FBTEST_MSG_DEBUG("WARNING: No space for Framebuffer backbuffer!\n");
		}else{
...
int parse_opt(int argc, char **argv)
{
	...
	*/
	...
	while ((opt = getopt_long(argc, argv, "uVvnarsb:m:t:p:D:h:w:S:d:r:f:c:H:W:i:j:x:y:X:Y:F:1:2:3:4:P:Q:R:T:I:B:k:O:AZ", longOptions, NULL)) != -1) {
