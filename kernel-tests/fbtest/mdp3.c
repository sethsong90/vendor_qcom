...
#ifdef CRC
	/* Capture CRC value using DSI MISR */
	if (thisFBTEST->crcmode!=-1) {
		if ((result=setMisr(DISPLAY_MISR_DSI0))<0) {
			VPRINT(verbose, "ERROR: SET MISR failed! result=%d\n",
				result);
			result = TEST_RESULT_FAIL;
		}
		if ((result=getMisr(DISPLAY_MISR_DSI0))<0) {
			VPRINT(verbose, "ERROR: GET MISR failed! result=%d\n",
				result);
			result = TEST_RESULT_FAIL;
		}
	}
