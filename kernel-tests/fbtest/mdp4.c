...
-----------------------------------------------------------------------------
Copyright (c) 2011-13 Qualcomm Technologies, Inc.  All Rights Reserved.
...
int writeback_init_start(int framesize)
{
	...
	if (testflag == FALSE) {//if test is not multiopt
		...
		if (allocMEM(framesize + (WRITEBACK_SIZE * 2))) {
			FBTEST_MSG_DEBUG("[WRITEBACK_INIT_START] allocMEM failed! (Line %d)\n",__LINE__);
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_INIT_START] MSMFB_WRITEBACK_INIT failed! line=%d err=%d\n",
		   __LINE__, result);
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_INIT_START] MSMFB_WRITEBACK_START failed! line=%d err=%d\n",
		   __LINE__, result);
		return TEST_RESULT_FAIL;
	}
...
int writeback_queueBuff(struct msmfb_data *fbdata,int offset)
{
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_QUEUE_BUFF] MSMFB_WRITEBACK_QUEUE_BUFFER failed! line=%d err=%d\n",
		   __LINE__, result);
		return TEST_RESULT_FAIL;
	}
	return 	TEST_RESULT_PASS;
...
int writeback_dqueue_dump_stop_terminate(struct msmfb_data *fbdata,int offset,char *testname)
{
	int result = TEST_RESULT_FAIL;
	char filename[200];
	char time[16];
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_DQUEUE_DUMP] MSMFB_WRITEBACK_DEQUEUE_BUFFER failed! line=%d err=%d\n",
		   __LINE__, result);
		return TEST_RESULT_FAIL;
	}
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_DQUEUE_DUMP] MSMFB_WRITEBACK_STOP failed! line=%d err=%d\n",
		   __LINE__, result);
		return TEST_RESULT_FAIL;
	}
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[WRITEBACK_DQUEUE_DUMP] MSMFB_WRITEBACK_TERMINATE failed! line=%d err=%d\n",
		   __LINE__, result);
		return TEST_RESULT_FAIL;
	}
	return 	TEST_RESULT_PASS;
...
int allMDP4Test(void)
{
	...
	if(thisFBTEST->testMode == USER){
		VPRINT(verbose, "\nallMDP4 test is not supported in USER mode. Try with Auto.\n");
...
int ditherTest(void){
	...
	if(fptr == NULL){
		FBTEST_MSG_DEBUG("[Dithering] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	...
	if (writeback_flag == TRUE) {
		...
		if (allocMEM(FB->fb_size)) {
			FBTEST_MSG_DEBUG("[Dithering] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"ditherTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
	...
	VPRINT(verbose, "Dither test over\n");
...
int deInterlaceTest(void){
	...
	fptr = fopen(thisFBTEST->fileParams.filenamePath, "rb");
	if(fptr == NULL){
		FBTEST_MSG_DEBUG("[De-Interlace] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	...
	if (writeback_flag == TRUE) {
		...
		//allocate PMEM buffer
		if (allocMEM(FB->fb_size)) {
			FBTEST_MSG_DEBUG("[De-Interlace] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	//copy image data to PMEM buffer
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"deinterlaceTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
		VPRINT(verbose, "Displaying Image on device...\n");
	...
	VPRINT(verbose, "De-Interlace test over\n");
...
int videoPlayTest(void){
	...
	if (writeback_flag == TRUE) {
		...
		if (allocMEM(framesize*2)) {
			FBTEST_MSG_DEBUG("[Video] allocMEM failed! (Line %d)\n", __LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	...
	if (writeback_flag == TRUE) {
		overlayp->dst_rect.w = WRITEBACK_WIDTH(FB->fb_vinfo.xres);
		overlayp->dst_rect.h = WRITEBACK_HEIGHT(FB->fb_vinfo.yres);
	} else {
	...
	VPRINT(verbose, "Playing video on device...\n");
	/*Play Video file*/
	for (frame = 0; frame < video.frames; frame++) {
		...
		if (err != framesize) {
			VPRINT(verbose, "[video] ERROR! RAW image read failed at frame %d (read %d of %d bytes)\n", frame, err, framesize);
			break;
		}
		...
		if (writeback_flag == TRUE) {
			result = writeback_dqueue_dump_stop_terminate(&fbdata, framesize*2, "videodeinterlaceTest");
			if (result == TEST_RESULT_FAIL)
				return TEST_RESULT_FAIL;
		}
	...
	VPRINT(verbose, "Video test over\n");
...
int fpsVsyncTest(void){
	...
	for (i = 0; i < (iterations / 2); i++) {
		...
		if (ioctl(fb_fd, FBIOPAN_DISPLAY, vinfo) < 0) {
			FBTEST_MSG_DEBUG("ERROR: FBIOPAN_DISPLAY failed, vsync ON,"
				"yoffset=%d. line=%d\n", vinfo->yoffset, __LINE__);
			result = -FBIOPAN_DISPLAY_FAILED;
		}
	...
	if (writeback_flag == FALSE) {
		if (allocMEM(framesize * 2)) {
			FBTEST_MSG_DEBUG("[FPSTEST] allocMEM failed! (Line %d)\n", __LINE__);
			return -PMEMALLOC_FAILED;
		}
	} else {
		if (allocMEM(framesize * 2 + (WRITEBACK_SIZE * 2))) {
	...
	for (i = 0; i < (iterations / 2); i++) {
		...
		if (ioctl(FB->fb_fd, FBIOPAN_DISPLAY, &(FB->fb_vinfo)) < 0) {
			...
			result = writeback_dqueue_dump_stop_terminate(&fbdata, framesize * 2, "fpsVsyncTest");
			if (result == TEST_RESULT_FAIL)
				return TEST_RESULT_FAIL;
...
int formatTest(void)
{
	...
	if (writeback_flag == TRUE) {
		...
		//allocate PMEM buffer
		if (allocMEM(size)) {
			FBTEST_MSG_DEBUG("[FORMAT] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	//copy image data to PMEM buffer
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"formatTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
		VPRINT(verbose, "Displaying Image on device...\n");
...
int doCrop(void){
	...
	fptr = fopen(thisFBTEST->fileParams.filenamePath, "rb");
	if(fptr == NULL){
		FBTEST_MSG_DEBUG("[CROP] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	...
	if (writeback_flag == TRUE) {
		...
		//allocate PMEM buffer
		if (allocMEM(size)) {
			FBTEST_MSG_DEBUG("[CROP] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	//copy image data to PMEM buffer
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"cropTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
...
int multiopTest(void){
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,
				wbOffset, "multiopTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
		sleep(5);
...
int colorkeyTest(void){
	...
	for(i=0; i < inFileCnt; i++){
		if(strcmp(ImgFileParams[i].filenamePath, "")){
			...
			if(fptr == NULL){
				FBTEST_MSG_DEBUG("[Color Key] Cannot open raw Image file!\n");
				return -FOPEN_FAILED;
			}
			//read image in buffer
	...
	if(inFileCnt < 2){
		...
		if (colorkeyflag && (thisFBTEST->fBuffer == FB0)) {
			for (i = 0; i < ImgFileParams[BG_PIPE].inResolution.height; i++) {
				memcpy(&FB->fb_buf[i*(FB->fb_finfo.line_length)],
				((unsigned short *) (&ImgBuff[BG_PIPE][i*\
			...
			/* allocate PMEM buffer */
			if (allocMEM(FB->fb_size)) {
				FBTEST_MSG_DEBUG("[Color Key] allocMEM failed! (Line %d)\n", __LINE__);
				return -PMEMALLOC_FAILED;
			}
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"colorkeyTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	} else {
		sleep(3);
...
int blendTest(void)
{
	...
	struct fbtest_params *thisFBTEST;
	struct fb_var_screeninfo *vinfo;
	struct inputFileParams ImgFileParams[MAX_FILE_TO_BLEND], tmpFileParams;
	thisFBTEST = &FBTEST;
	vinfo = &(FB->fb_vinfo);
	...
	memset(ImgFileParams, 0x00, 4*sizeof(struct inputFileParams));
	...
	switch(thisFBTEST->testMode) {
		case USER:
			...
			for(i=0; i < inFileCnt; i++)
			{
				...
				if(!lookup_val(format_name_table, ImgFileParams[i].inputFormat, 0)){
					...
					if(fg_yuv>MAX_YUV_PIPE_SUPPORT){
						...
						strcpy(thisFBTEST->blend.ImagePath[i], "");
					}
				}
				if (thisFBTEST->fBuffer == FB0) {
			...
			if (!bg_set) {
				...
				memset(&ImgFileParams[BG_PIPE], 0x00, sizeof(struct inputFileParams));
			}
		...
		case AUTO:
			...
			strcpy(thisFBTEST->blend.ImagePath[0], "");
			strcpy(thisFBTEST->blend.ImagePath[1], thisFBTEST->fileParams.filenamePath);
			strcpy(thisFBTEST->blend.ImagePath[2], thisFBTEST->fileParams.filenamePath);
			strcpy(thisFBTEST->blend.ImagePath[3], thisFBTEST->fileParams.filenamePath);
	...
	VPRINT(verbose, "Image Blend test over\n");
...
int doImageBlend(struct inputFileParams *ptrImgFileParams, unsigned int inFileCnt, int isAutoMode)
{
	...
	struct fbtest_params *thisFBTEST;
        struct inputFileParams ImgFileParams[MAX_FILE_TO_BLEND];
	thisFBTEST = &FBTEST;
	...
	memcpy(ImgFileParams, ptrImgFileParams, MAX_FILE_TO_BLEND*sizeof(struct inputFileParams));
	for (i=0; i < inFileCnt; i++) {
		if (strcmp(ImgFileParams[i].filenamePath, "")) {
			...
			fptr = fopen(ImgFileParams[i].filenamePath, "rb");
			if(fptr == NULL){
				FBTEST_MSG_DEBUG("[BLEND] Cannot open raw Image file!\n");
				return -FOPEN_FAILED;
			}
			//read image in buffer
			bytesRead = fread(ImgBuff[i], 1, ImgFrameSize[i], fptr);
	...
	if (thisFBTEST->fBuffer == FB0) {
		ImgDataOffset[FG1_PIPE] = 0;
		ImgDataOffset[FG2_PIPE] = ImgFrameSize[FG1_PIPE];
		ImgDataOffset[FG3_PIPE] = ImgFrameSize[FG1_PIPE]+ImgFrameSize[FG2_PIPE];
	} else {
	...
	zOrderVal[BG_PIPE] = 0;
	zOrderVal[FG1_PIPE] = 0;
	zOrderVal[FG2_PIPE] = 1;
	zOrderVal[FG3_PIPE] = 2;
	...
	if (writeback_flag == TRUE) {
		...
		if (allocMEM(FB->fb_size*4)) {
			FBTEST_MSG_DEBUG("[BLEND] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	...
	if(isAutoMode == 0){
		if(inFileCnt >=2){
			if (thisFBTEST->fBuffer == FB0) {
				for (i = 0; i < ImgFileParams[BG_PIPE].inResolution.height; i++) {
					memcpy(&FB->fb_buf[i*(FB->fb_finfo.line_length)],
					((unsigned short *) (&ImgBuff[BG_PIPE][i* ImgFileParams[BG_PIPE].inResolution.width*(int)getFormatBpp(ImgFileParams[BG_PIPE].inputFormat)])),
					ImgFileParams[BG_PIPE].inResolution.width*(int)getFormatBpp(ImgFileParams[BG_PIPE].inputFormat));
				}
			sleep(3);
			}
		}else{
			VPRINT(verbose, "Minimum 2 images required for Blend Operation\n");
			return -INVALID_PARAMS;
		}
	}
	...
	for (i = FG1_PIPE; i < inFileCnt; i++) {
		...
		VPRINT(verbose, "Blending Image %d: %s (Alpha Value = %d)\n",i, ImgFileParams[i].filenamePath, thisFBTEST->blend.ImgAlphaVal[i-1]);
		...
		for(j=0;j<=0xFF; j++){
			...
			overlay[i].src_rect.x = 0;
			overlay[i].src_rect.y = 0;
			overlay[i].src_rect.w  = (isAutoMode && (vinfo->xres == thisFBTEST->fileParams.inResolution.width))? ImgFileParams[i].inResolution.width-60: ImgFileParams[i].inResolution.width;
			overlay[i].src_rect.h  = (isAutoMode && (vinfo->yres == thisFBTEST->fileParams.inResolution.height))? ImgFileParams[i].inResolution.height-60: ImgFileParams[i].inResolution.height;
			overlay[i].dst_rect.x = (isAutoMode)? i*20: 0;
			overlay[i].dst_rect.y = (isAutoMode)? i*20: 0;
			overlay[i].dst_rect.w  = (isAutoMode && (vinfo->xres == thisFBTEST->fileParams.inResolution.width))? ImgFileParams[i].inResolution.width-60: ImgFileParams[i].inResolution.width;
			overlay[i].dst_rect.h  = (isAutoMode && (vinfo->yres == thisFBTEST->fileParams.inResolution.height))? ImgFileParams[i].inResolution.height-60: ImgFileParams[i].inResolution.height;
			overlay[i].z_order = zOrderVal[i];
			overlay[i].is_fg = thisFBTEST->blend.isPixBlend? 0: 1;
			if (isAutoMode) {
				overlay[i].alpha = thisFBTEST->blend.isPixBlend? 0xFF: j;
			} else {
				overlay[i].alpha = thisFBTEST->blend.ImgAlphaVal[i-1];
			}
			...
			if ((writeback_flag == TRUE) && ((inFileCnt - 1) == i) && (j == 0xFF)) {
				result = writeback_queueBuff(&fbdata,size);
				if (result == TEST_RESULT_FAIL)
					return TEST_RESULT_FAIL;
			}
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,size,"blendTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	}
	if(cleanup_overlay())
...
int moveTest(void)
{
	...
	switch(thisFBTEST->testMode) {
		...
		case AUTO:
			...
			result = doImageMove(startX, startY, endX, endY, HORIZONTAL);

			VPRINT(verbose, "IMAGE TRAVERSAL IN VERTICAL DIRECTION\n");
			result = doImageMove(startX, startY, endX, endY, VERTICAL);

			VPRINT(verbose, "IMAGE TRAVERSAL IN ACROSS BOUNDARY\n");
			result = doImageMove(startX, startY, endX, endY, BOUNDARY);
			...
			result = doImageMove(startX, startY, endX, endY, RANDOM);
...
int doImageMove(unsigned int startX, unsigned int startY, unsigned int endX, unsigned int endY, unsigned int traversal)
{
	...
	if ((imgPtr = fopen(thisFBTEST->fileParams.filenamePath,"rb")) == NULL) {
		FBTEST_MSG_DEBUG("[Move] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	if (writeback_flag == TRUE) {
		...
		//allocate PMEM buffer
		if (allocMEM(FB->fb_size)) {
			FBTEST_MSG_DEBUG("[Move] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	...
	switch(traversal){
		case USER_DEFINED:
			...
			traverseImage(startX, startY, endX, endY, &overlay,1);
		break;
		case HORIZONTAL:
			count = 0;
			endX = vinfo->xres - iWidth;
			for(i=0; i <= (vinfo->yres - iHeight); i = i + MOVE_SKIP_INTERVAL){
				if (i == (vinfo->yres - iHeight))
					count = 1;
				traverseImage(0, i, endX, i, &overlay,count);
			}
		break;
		...
		case VERTICAL:
			...
			for(i=0; i <= (vinfo->xres - iWidth); i = i + MOVE_SKIP_INTERVAL){
				if (i == (vinfo->xres - iWidth))
					count = 1;
				traverseImage(i, 0, i, endY, &overlay,count);
			}
		break;

		case BOUNDARY:
			endX = vinfo->xres - iWidth;
			traverseImage(0, 0, endX,0, &overlay,0);

			startX = vinfo->xres - iWidth;
			endX = vinfo->xres - iWidth;
			endY = vinfo->yres - iHeight;
			traverseImage(startX, 0, endX, endY, &overlay,0);

			startX = vinfo->xres - iWidth;
			startY = vinfo->yres - iHeight;
			endY = vinfo->yres - iHeight;
			traverseImage(startX, startY, 0, endY, &overlay,0);

			startY = vinfo->yres - iHeight;
			traverseImage(0, startY, 0,0, &overlay,1);
		break;

		case DIAGONALS:
			endX = vinfo->xres - iWidth;
			endY = vinfo->yres - iHeight;
			traverseImage(0, 0, endX, endY, &overlay,0);
			startX = vinfo->xres - iWidth;
			endY = vinfo->yres - iHeight;

			traverseImage(startX,0, 0, endY, &overlay,1);
...
int traverseImage(unsigned int startX, unsigned int startY, unsigned int endX, unsigned int endY, struct mdp_overlay * ptrOverlay,int count)
{
	int overlay_id;
	struct mdp_overlay overlay;
	struct msmfb_overlay_data ovdata;
	...
	for (i = 0; i <= loopCnt; i++){
		...
		if ((writeback_flag == TRUE) && (count == 1) && (loopCnt == i)) {
		...
		if ((writeback_flag == TRUE) && (count == 1) && (loopCnt == i)) {
			result = writeback_dqueue_dump_stop_terminate(&fbdata,frameSize,"moveTest");
			if (result == TEST_RESULT_FAIL)
				return TEST_RESULT_FAIL;
		}
	}
...
int rotateTest(void)
{
	...
	VPRINT(verbose, "Rotate test over\n");
			...
			int dstOffset)
{
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[ROTATE] rotator start ioctl failed result = %d\n", result);
		if (cleanup_overlay())
			return -OVERLAY_UNSET_FAILED;
	...
	if (result < 0) {
		FBTEST_MSG_DEBUG("[ROTATE] rotator ioctl failed result = %d\n", result);
		...
		if (result < 0)
			FBTEST_MSG_DEBUG("[ROTATE] rotator finish ioctl failed result = %d\n", result);
			return -ROT_FAILED;
	}
...
int doRotate(unsigned int cmdRotFlip, unsigned int cmdRotDeg)
{
	...
	if ((imgPtr = fopen(thisFBTEST->fileParams.filenamePath,"rb")) == NULL) {
		FBTEST_MSG_DEBUG("[ROTATE] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	...
	/*Call Unset to make sure SMPs are released*/
	if (cleanup_overlay())
		return -OVERLAY_UNSET_FAILED;
	overlay.id = MSMFB_NEW_REQUEST;
...
int scaleTest(void)
{
	...
	VPRINT(verbose, "Scale test over\n");
...
//This function does up/down and arbitrary scaling.
int doScale(unsigned int iWidth, unsigned int iHeight, unsigned int oWidth, unsigned int oHeight, unsigned int iFormat)
{
	...
	if ((imgPtr = fopen(thisFBTEST->fileParams.filenamePath,"rb")) == NULL) {
		FBTEST_MSG_DEBUG("[SCALE] Cannot open raw Image file!\n");
		return -FOPEN_FAILED;
	}
	...
	if (writeback_flag == TRUE) {
		...
		//allocate PMEM buffer
		if (allocMEM(FB->fb_size)) {
			FBTEST_MSG_DEBUG("[SCALE] allocMEM failed! (Line %d)\n",__LINE__);
			return -PMEMALLOC_FAILED;
		}
	}
	...
	while ((overlay.dst_rect.w != oWidth) ||
		...
		if ((writeback_flag == TRUE) && (count == 1)) {
			result = writeback_queueBuff(&fbdata,frameSize);
			if (result == TEST_RESULT_FAIL)
				return TEST_RESULT_FAIL;
		}
		...
		if (thisFBTEST->isStepScale) {
			count = 1;
			if ((overlay.dst_rect.w != oWidth) || (overlay.dst_rect.h != oHeight)) {
	...
	if (writeback_flag == TRUE) {
		result = writeback_dqueue_dump_stop_terminate(&fbdata,frameSize,"scaleTest");
		if (result == TEST_RESULT_FAIL)
			return TEST_RESULT_FAIL;
	}
	if(cleanup_overlay())
...
int hwCursorTest(void)
{
	...
	cursor_image.fg_color = 0x00000000;
	alpha = 0x63;
	...
	result = ioctl(FB->fb_fd, FBIOPAN_DISPLAY, &FB->fb_vinfo);
	if (result < 0) {
		FBTEST_MSG_DEBUG("\n[HWCURSOR] FBIOPAN_DISPLAY failed! (Line %d)\n",\
					__LINE__);
		return -FBIOPAN_DISPLAY_FAILED;
	}
	usleep(delay);
	result = ioctl(FB->fb_fd, MSMFB_CURSOR, &cursor_control);
	if (result < 0) {
	...
	row = 0;
	for(col = 0; col < FB->fb_vinfo.xres - CURSOR_WIDTH; col++)
	{
		result = ioctl(FB->fb_fd, FBIOPAN_DISPLAY, &FB->fb_vinfo);
		if (result < 0) {
			FBTEST_MSG_DEBUG("\n[HWCURSOR] FBIOPAN_DISPLAY failed! (Line %d)\n",\
...
int overlayScaleTest(void)
{
    ...
    */
    ...
    for(fcount = 0; fcount < FBLISTSIZE_GLOBAL; fcount++) {
        if(!setOverlay(&overlay, FBLIST_GLOBAL[fcount],
            FBLIST_GLOBAL[fcount]->fb_fmt,
            src_min_width, src_min_height, 0, 0, src_min_width, src_min_height,
            0, 0, FB_OVERLAY_RGB_1, 0xffffffff, pipes[fcount],
            MSMFB_NEW_REQUEST, 0)) {
            printf("Error: Could not perform initial setOverlay on FB%d\n",
                    fcount);
            return  TEST_RESULT_FAIL;
        }
        overlay_ids[fcount] = overlay.id;
...
int drawBGAlpha(struct fbDev* ptr_fb, int color)
{
    ...
    if (ioctl(ptr_fb->fb_fd, FBIOPAN_DISPLAY, &(ptr_fb->fb_vinfo)) < 0) {
        FBTEST_MSG_DEBUG("ERROR: FBIOPAN_DISPLAY failed! line=%d\n", __LINE__);
        return TEST_RESULT_FAIL;
    }
    return TEST_RESULT_PASS;
}
...
static int overlayStressTest(void)
{
	...
	struct mdp_overlay overlay;
	struct msmfb_overlay_data ovdata;
	xres_aligned = (FB->fb_vinfo.xres_virtual + 31) & (~31);
