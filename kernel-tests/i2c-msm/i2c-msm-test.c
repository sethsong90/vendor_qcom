...
static int (*test_func)(int) = test_eeprom;
...
/**
 * do_read: Read from device using a single byte offset
 * @return zero on success or negative error code
 */
static int do_read(int fd, uint16_t offset, uint8_t *buf, uint16_t count)
{
	uint8_t offset_data[] = {offset};
	...
	pr_verbose("Reading %d bytes at addr:0x%x offset:0x%x\n", count,
...
static int test_scan_bus(int fd)
{
	...
	pr_verbose("Scanning the bus...");

	for (i = 8; i < 120; ++i) {
		slave_address = i;
		if (!do_read(fd, 0, data, sizeof(data)))
...
static int test_eeprom(int fd)
{
	...
	for (j = 0; j < iterations; j++) {
		...
		for (i = 0; i < num_blocks; i++, offset += block_size) {
			...
			if (rc < 0) {
				pr_err(
				   "Could not read eeprom before writing\n");
				goto err_exit;
			}

			rc = write_eeprom(fd, offset, write_buf, block_size);
			if (rc < 0) {
				pr_err("Could not write random buffer\n");
				goto err_exit;
			}

			rc = read_eeprom(fd, offset, read_buf2, block_size);
			if (rc < 0) {
				pr_err("Could not read eeprom after writing\n");
				goto restore_eeprom;
			}

			if (memcmp(write_buf, read_buf2, block_size)) {
				pr_err(
				  "Buffers differ after writing and reading\n");
				if (verbose)
...
static int parse_args(int argc, char **argv)
{
	...
	int command;
	const char *optstr = "D:s:b:n:o:i:plw:r:c:avh";
