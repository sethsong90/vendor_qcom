...
public class TunerOemHook  {
    ...
    public static TunerOemHook getInstance(Context context, Handler listenerHandler) {
        if (mInstance == null) {
            ...
            /*
             * This registers TunerOemHook with QmiOemHook library so that it can
             * receive any Tuner specific responses/unsols
             */

            QmiOemHook.registerService(TUNER_SERVICE_ID,
                    listenerHandler, OEM_HOOK_UNSOL_IND);


            Log.v(LOG_TAG, "Registered TunerOemHook with QmiOemHook");
        } else {
    ...
    public synchronized void dispose() {
        ...
        if (mRefCount == 0) {
            ...
            QmiOemHook.unregisterService(TUNER_SERVICE_ID);
            mQmiOemHook.dispose();
            mInstance = null;
    ...
    public static Object handleMessage(Message msg) {
        switch (msg.what) {
            case OEM_HOOK_UNSOL_IND:
                ...
                return TunerOemHook.receive(map);
            default:
