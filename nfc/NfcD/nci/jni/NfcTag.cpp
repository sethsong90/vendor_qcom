...
void NfcTag::discoverTechnologies (tNFA_ACTIVATED& activationData)
{
    ...
    case NFA_PROTOCOL_NFC_DEP:
      if (in_dta_mode()) {
        mTechList [mNumTechList] = TARGET_TYPE_ISO14443_3A; //is TagTechnology.NFC_A by Java API TODO: CHECK IF THIS IS CORRECT!
        mTechHandles [mNumTechList] = rfDetail.rf_disc_id;
        mTechLibNfcTypes [mNumTechList] = rfDetail.protocol;
        //save the stack's data structure for interpretation later
        memcpy (&(mTechParams[mNumTechList]), &(rfDetail.rf_tech_param), sizeof(rfDetail.rf_tech_param));
        ...
        if ((rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_F) ||
            (rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_F_ACTIVE) ||
            (rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_LISTEN_F) ||
            (rfDetail.rf_tech_param.mode == NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE)
        )
        {
          mTechList [mNumTechList] = TARGET_TYPE_FELICA; //is TagTechnology.NFC_F by Java API TODO: CHECK IF THIS IS CORRECT!
        }
        mNumTechList++;
        if (checkP2pInitiatorOrTarget(rfDetail.rf_tech_param.mode)) {
          mTechList [mNumTechList] = 55;
        }
        else {
          mTechList [mNumTechList] = 57;
        }
        isNfcDep = true;
      }
      break;
...
void NfcTag::discoverTechnologies (tNFA_DISC_RESULT& discoveryData)
{
    ...
    case NFA_PROTOCOL_NFC_DEP:
      if (in_dta_mode()) {
        ...
        //if remote device supports tech F;
        //tech F is preferred because it is faster than tech A
        if ( (discovery_ntf.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_F) ||
            (discovery_ntf.rf_tech_param.mode == NFC_DISCOVERY_TYPE_POLL_F_ACTIVE) ||
            (discovery_ntf.rf_tech_param.mode == NFC_DISCOVERY_TYPE_LISTEN_F) ||
            (discovery_ntf.rf_tech_param.mode == NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE)
        )
        {
          mTechList [mNumTechList] = TARGET_TYPE_FELICA; //is TagTechnology.NFC_F by Java API
        }

        mNumTechList++;
        if (checkP2pInitiatorOrTarget(discovery_ntf.rf_tech_param.mode)) {
          mTechList [mNumTechList] = 55;
        }
        else {
          mTechList [mNumTechList] = 57;
...
void NfcTag::createNativeNfcTag (tNFA_ACTIVATED& activationData)
{
    ...
    /* nfc service should not clear the timer before it is started*/
#ifdef DTA // <DTA>
    if(!in_dta_mode() ) {
#endif // </DTA>
    sWaitMutex.lock ();
        ...
        case NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE:
            {
                ...
                ***/
                ...
                #ifdef DTA // <DTA>
                if (in_dta_mode()) {
                  if ( (&activationData != 0) && // try to avoid seg fault
                       (&activationData.params != 0) &&
                       (&activationData.params.t3t != 0) &&

                       (activationData.params.t3t.num_system_codes > 0)

                       && (activationData.params.t3t.p_system_codes != NULL)

                  ) //copy the first system Code
                  {
                    //UINT16 systemCode = *(activationData.params.t3t.p_system_codes);
                    UINT16 systemCode = 0xA5A5;
                    /* Only allows 1 code but should catch for condition where not set and pointer to system codes invalid */
                    if (activationData.params.t3t.num_system_codes < 2)
                    {
                        systemCode = *(activationData.params.t3t.p_system_codes);
                    }
                    result [8] = (UINT8) (systemCode >> 8);
                    result [9] = (UINT8) systemCode;
                    ALOGD ("%s: tech F; sys code=0x%X 0x%X", fn, result [8], result [9]);
                  }
                  ALOGD ("%s : DTA : tech F; sys code=0x%X 0x%X", fn, result [8], result [9]);
                }
                else {
                #endif // </DTA>
                if (activationData.params.t3t.num_system_codes > 0) //copy the first System Code
                {
                    ...
                    ALOGD ("%s: non DTA : tech F; sys code=0x%X 0x%X", fn, result [8], result [9]);
                #endif // </DTA>
                }
                pollBytes.reset(e->NewByteArray(len));
        ...
        case NFC_PROTOCOL_T2T:
            {
                ...
                #ifdef DTA // <DTA>
                // TODO: Necessary for DTA?
                if(in_dta_mode() ) {
            mTechParams [i].param.pa.sel_rsp = activationData.activate_ntf.rf_tech_param.param.pa.sel_rsp;
                }
                ALOGD ("mTechParams [i].param.pa.sel_rsp = %x: ",mTechParams [i].param.pa.sel_rsp);
                #endif // </DTA>
...
void NfcTag::selectP2p()
{
    ...
    for (int i = 0; i < mNumTechList; i++)
    {
        ...
        if ( (mTechParams[i].mode == NFC_DISCOVERY_TYPE_POLL_F) ||
             (mTechParams[i].mode == NFC_DISCOVERY_TYPE_POLL_F_ACTIVE)
              #ifdef DTA // <DTA>
           || (in_dta_mode() &&
            ( (mTechParams[i].mode == NFC_DISCOVERY_TYPE_LISTEN_F) ||
              (mTechParams[i].mode == NFC_DISCOVERY_TYPE_LISTEN_F_ACTIVE)) )
             #endif // </DTA>
         )
        {
            ...
            #ifdef DTA // <DTA>
            if (in_dta_mode() ) {
              checkP2pInitiatorOrTarget(mTechParams[i].mode);
            }
            #endif // </DTA>
    ...
    {
    #ifdef DTA // <DTA>
      if (in_dta_mode() && !isInitiator) {
        ALOGE ("%s: P2P device is TARGET, no need to select", fn);
      }
      else { // (!in_dta_mode() || isInitiator())
        if (!isActive) {
          ALOGE ("%s: P2P device is INITIATOR but in PASSIVE poll mode -> select", fn);
        }
        else {
          ALOGE ("%s: P2P device is INITIATOR in ACTIVE poll mode -> select", fn);
        }
    #endif // </DTA>
...
#ifdef DTA // <DTA>
bool NfcTag::checkP2pInitiatorOrTarget(UINT8 mode) {

  ALOGE ("checkP2pInitiatorOrTarget(): mode = %d", mode);

  isActive =
    mode == NFC_DISCOVERY_TYPE_POLL_A_ACTIVE ||
    mode == NFC_DISCOVERY_TYPE_POLL_F_ACTIVE;

  return isInitiator =
    mode == NFC_DISCOVERY_TYPE_POLL_A ||
    mode == NFC_DISCOVERY_TYPE_POLL_A_ACTIVE ||
    mode == NFC_DISCOVERY_TYPE_POLL_F ||
    mode == NFC_DISCOVERY_TYPE_POLL_F_ACTIVE;
...
/*******************************************************************************
 **
 ** Function:        handleP2pConnection
 **
 ** Description:     Handles P2P connection activation.
 **                  event: event code.
 **                  data: pointer to event data.
 **
 ** Returns:         None
 **
 *******************************************************************************/
void NfcTag::handleP2pConnection(UINT8 event, tNFA_CONN_EVT_DATA* data) {
  static const char fn [] = "NfcTag::handleP2pConnection";

  ALOGE ("%s: <DTA> NFA_ACTIVATED event processed", fn);
  tNFA_ACTIVATED& activated = data->activated;
  mProtocol = activated.activate_ntf.protocol;
  calculateT1tMaxMessageSize(activated);
  discoverTechnologies(activated);
