...
static jboolean nativeNfcTag_doWrite (JNIEnv* e, jobject, jbyteArray buf)
{
    ...
    else
    {
        ...
        if (in_dta_mode())
        {
            ...
            {
                 // TODO: Check whether free in NativeNfcManager is right location
                t1t_tag_data = (UINT8*)malloc(data_len+2); // FIXME: malloc without free
                if(t1t_tag_data != NULL)
                {
                    ...
                    ALOGD ("t1t_tag_data[data_len]:%X data_len=%d", t1t_tag_data[data_len],data_len);
                    data_len++;
                    ...
                    {
                        ALOGD (" %X %X", t1t_tag_data[p],p_data[p]);
                    }
