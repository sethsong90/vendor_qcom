...
public class NativeNfcTag implements TagEndpoint {
    ...
    // <DTA>
    @Override
    public synchronized void pausePresenceChecking() {
        if (mWatchdog != null) {
            Log.d(TAG,"Pausing presence check");
            mWatchdog.pause();
        }
    }
    @Override
    public synchronized void resumePresenceChecking() {
        if (mWatchdog != null) {
            Log.d(TAG,"Resuming presence check");
            mWatchdog.doResume();
        }
    }

    public synchronized void dta_fake_ndef_tech() {
        DTA_FLAG = true;
        Log.d(TAG,"[DTA] dta_fake_ndef_tech, fake tag capacity is 10000 and it supports read and write");
        addNdefTechnology(null,
                getConnectedHandle(),
                getConnectedLibNfcType(),
                getConnectedTechnology(),
                10000, 2);
        // 2 = tag support read and write
        // 10000 = capacity of tag (this should affect only Ndef.getMaxsize() and in DTA mode should be ok)
    }
    // </DTA>
    @Override
            ...
            for (int i = 0; i < mTechList.length; i++) {
                ...
                switch (mTechList[i]) {
                    ...
                    case TagTechnology.MIFARE_ULTRALIGHT: {
                        // <DTA>
                        if(DTA_FLAG) { // set by dta_fake_ndef_tech
                            // In DTA mode, do not try to detect UltraLight
                            if (DBG) Log.d(TAG, "[DTA] NativeNfcTag: getTechExtras, do not call isUltralightC");
                            //boolean isUlc = isUltralightC();
                            // </DTA>
                        } else {
                            if (DBG) Log.d(TAG, "Calling isUltralightC");
                        boolean isUlc = isUltralightC();
    ...
    public NdefMessage findAndReadNdef() {
        ...
        for (int techIndex = 0; techIndex < technologies.length; techIndex++) {
            ...
            byte[] buff = null;
            // TODO: This was wrong in puneet's branch, keep note if still working
            // <DTA>
            // Do not read NDEF automatically when in DTA mode
            if(DTA_FLAG) {
                Log.d(TAG, "[DTA] do_not_read NDEF");
                buff = null;
            } // </DTA>
            else
            { // <!DTA>
