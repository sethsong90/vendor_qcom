...
#define POLL_WAKE_DELAY         20
#define WRITE_DELAY             2
#define NFCC_RESET_DELAY        100
#define NFCC_INIT_DELAY         100
...
NFC_RETURN_CODE DT_Nfc_Open(DT_Nfc_sConfig_t *pDriverConfig, void **pdTransportHandle, void (*nci_cb) )
{
   ...
   if(((reset_status == TRUE) && (stored_restart_reson != NFCSERVICE_WATCHDOG_TIMER_EXPIRED))|| (current_mode == FTM_MODE))
   {
       ...
       if (retstatus != NFC_SUCCESS)
       {
           ALOGE ("DT:DT_Nfc_Open : can't initialise NFCC hardware");
           retstatus = NFC_FAILED;
           goto done_open;
       }
...
UDRV_API UINT16  DT_Nfc_Write(tUSERIAL_PORT port, UINT8 *p_data, UINT16 len)
{
    ...
    if ((   p_data[0] == NCI_PROP_CMD)
        && (p_data[1] == NCI_SLEEP_CMD)
        && (p_data[2] == NCI_SLEEP_PL)) /* sleep cmd */
    {
