...
tNFC_STATUS RW_T3tGetSystemCodes (void)
{
    ...
    {
#ifdef DTA // <DTA>
      /* If we are in DTA mode, send correct SENSF_REQ according to pattern number instead of system
       * code request
       * Extra SENSF_REQ after the first one (chip does that automatically) is needed in pattern numbers 1-8
       * In pattern numbers 1 and 2 the extra SENSF_REQ comes from RW_T3tDetectNdef -> no sending here
       * No DTA tests for Type 3 tags are run in patterns 0 and 9-13 -> default behaviour */
      switch (dta_Pattern_Number) {
        case 1:
        case 2:
          // With failed status the stack behaves correctly after no SENF_REQ is sent
          RW_TRACE_DEBUG0 ("T3T: Not sending SENSF_REQ");
          return (NFC_STATUS_FAILED);
          break;
        ...
        case 5:
        case 6:
          // Poll for NDEF system code (0012FC0000)
          RW_TRACE_DEBUG0 ("T3T: Sending SENSF_REQ: 12FC 00 00");
          if ((retval = (tNFC_STATUS) nci_snd_t3t_polling (T3T_SYSTEM_CODE_NDEF, 0, 0)) == NCI_STATUS_OK)
          {
            p_cb->rw_substate = RW_T3T_GET_SC_SST_POLL_NDEF;
            p_cb->cur_poll_rc = 0;
        ...
        case 7:
        case 8:
          // Send wildcard poll request for type 3 tags with max time slots (00FFFF010F)
          RW_TRACE_DEBUG0 ("T3T: Sending SENSF_REQ: FFFF 01 0F");
          if ((retval = (tNFC_STATUS) nci_snd_t3t_polling (0xFFFF, 0x01, 0x0F)) == NCI_STATUS_OK)
          {
            // Use the NDEF polling substate since that doesn't break the stack behavior
            // when sending extra poll (SENSF_REQ)
            p_cb->rw_substate = RW_T3T_GET_SC_SST_POLL_NDEF;
            p_cb->cur_poll_rc = 1;
            p_cb->flags |= RW_T3T_FL_W4_GET_SC_POLL_RSP;
        ...
        case -1:
        default:
          RW_TRACE_DEBUG0 ("T3T: Default SENSF_REQ behavior");
#endif // </DTA>
        if ((retval = (tNFC_STATUS) nci_snd_t3t_polling (0xFFFF, T3T_POLL_RC_SC, 0)) == NCI_STATUS_OK)
        {
