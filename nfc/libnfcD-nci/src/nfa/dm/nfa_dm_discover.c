...
static void nfa_dm_set_qnci_params (void)
{
    ...
    /* In DTA mode , this parameter is configured in pattern number 0*/
    /* In non DTA mode . default should be set */
    if (!(in_dta_mode()))
    {
...
static void nfa_dm_set_rf_listen_mode_raw_config (tNFA_DM_DISC_TECH_PROTO_MASK *p_disc_mask)
{
    ...
    */
        ...
        &&(p_cfg->la_enable)  )
    {
        ...
        {
            UINT8_TO_STREAM (p, 4);
            ARRAY_TO_STREAM (p, p_cfg->la_nfcid1, 4);
        }
#endif // </DTA>
...
#ifdef DTA // <DTA>
        if (in_dta_mode()) {
            NFA_TRACE_DEBUG0("\n nfa_dm_set_rf_listen_mode_raw_config : Setting DTA");
            p_cfg->ln_atr_res_config = 0x30;
        }
#endif // </DTA>
    ...
    case NFA_DM_RF_INTF_ACTIVATED_NTF:
        ...
        else
        {
            ...
            if (nfa_dm_disc_notify_activation (&(p_data->nfc_discover)) == NFA_STATUS_FAILED)
            {
                ...
                if (p_data->nfc_discover.activate.intf_param.type != NFC_INTERFACE_EE_DIRECT_RF)//dont deactivate in ee direct hack
                {
...
#ifdef DTA // <DTA>
	if ((in_dta_mode())) {
    /* Force LRT to 64 byte payload + No Gen. Bytes available */
    p_listen_cfg->ln_atr_res_config = 0x00;
    NFA_TRACE_DEBUG0 ("nfa_dm_start_excl_discovery : Force the General Bytes + Payload");
	}
#endif // </DTA>
