...
#ifdef DTA // <DTA>
    BOOLEAN dta_mode = in_dta_mode(); // store in_dta_mode()
    if (dta_mode) {
     NFA_TRACE_DEBUG0 ("[DTA] In DTA mode, exclusive rf discovery shall call normal NDEF callback. ");
    }
    if (!dta_mode && (p_cb->flags & NFA_DM_FLAGS_EXCL_RF_ACTIVE) && (p_cb->p_excl_ndef_cback))
#else // <!DTA>
