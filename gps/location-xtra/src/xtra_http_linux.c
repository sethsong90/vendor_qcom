...
//   char *ptr;
   ...
   *data_len = 0;
   ...
   if(_bytes_sent < 0)
   {
      ...
      XTRA_TRACE_ERROR( "%s Failed to send request to host %s port %d, errno=%d: %s",
            func_name, host, port, errno, strerror(errno));
   return FAIL;
   }
   ...
   if(bGetHeader && idx < 256) //response is normal then pass to process HTTPResponse
   {
      ...
      {
         XTRA_TRACE_ERROR("remote file size is zero\n");
         resetSocket(_soc);
         return FAIL;
      }
      ...
      if(!dataout)
      {
      XTRA_TRACE_ERROR("cannot malloc buffer size %d\n",*data_len);
      *data_len = 0;
      resetSocket(_soc);
      return FAIL;
      }
