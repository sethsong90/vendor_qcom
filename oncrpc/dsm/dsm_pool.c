...
/*===========================================================================
  ...
  $Header: //linux/pkgs/proprietary/oncrpc/main/source/dsm/dsm_pool.c#3 $
...
)
{
  ...
  for (i = DSM_MEM_LEVEL_DNE;i < DSM_MEM_LEVEL_MAX; i++)
  {
  ...
  for (i=0; i < table->item_count; i++)
  {
    ...
    *(uint32 *)&(item_ptr->data_ptr[item_ptr->size]) = DSM_TAIL_COOKIE;
...
)
{
  ...
  */
  ...
  table->free_stack = (dsm_item_type**)
    &(table->item_array)
    [ ( table->item_count *
        DSMI_POOL_OBJECT_SIZE(table->pool_item_size,table->pad_size)) -
      table->pad_size
...
)
{
  ...
  for(i = DSM_MEM_LEVEL_MIN + 1; i < DSM_MEM_LEVEL_MAX; i++)
  { 
  ...
  for(i = DSM_MEM_LEVEL_MIN + 2; 
      i < DSM_MEM_LEVEL_MAX &&
        table->mem_event_cb_list[i].level != DSM_MEM_LEVEL_INVALID; 
      i++)
  {
    for(j = DSM_MEM_LEVEL_MIN + 2; 
        j <= DSM_MEM_LEVEL_MAX - i &&
