...
/*===========================================================================
  ...
  $Header: //linux/pkgs/proprietary/oncrpc/main/source/dsm/dsm.c#3 $
...
)
{
  ...
  else
  {
    ...
    -----------------------------------------------------------------------*/
    while (table->next_level > DSM_MEM_LEVEL_MIN && 
	   table->free_count < 
             table->mem_event_cb_list[table->next_level].avail_item_count)
    {
      table->next_level=(dsm_mem_level_enum_type)(table->next_level - 1);
      if (table->mem_event_cb_list[table->next_level+1].mem_new_event_cb != 
          NULL)   
      {
	table->mem_event_cb_list[table->next_level+1].
	  mem_new_event_cb(pool_id,
			   table->mem_event_cb_list[table->next_level+1].level,
			   DSM_MEM_OP_NEW);
      }
    }
  }
...
/*- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
...
#ifdef FEATURE_DSM_MEM_CHK_EXPENSIVE_DOUBLE_FREE
 {
  ...
  ---------------------------------------------------------------------*/
  while (table->next_level+1 < DSM_MEM_LEVEL_MAX &&
         table->mem_event_cb_list[table->next_level+1].level >
         DSM_MEM_LEVEL_MIN &&
         table->free_count >=
         table->mem_event_cb_list[table->next_level+1].avail_item_count)
  {
    table->next_level=(dsm_mem_level_enum_type)(table->next_level + 1);
    if (table->mem_event_cb_list[table->next_level].mem_free_event_cb != 
        NULL)
    {
      table->mem_event_cb_list[table->next_level].mem_free_event_cb(
        item_ptr->pool_id,
        table->mem_event_cb_list[table->next_level].level,
        DSM_MEM_OP_FREE);
    }
  } 
...
)
{
...
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT(src_ptr->enqueued_packet_length == 0);
#endif
  ...
  ----------------------------------------------------------------------*/
    ...
    if((*dup_ptr)->dup_ptr != NULL)
    {
      dsmi_ref_inc((*dup_ptr)->dup_ptr);
    }

    (*dup_ptr)->data_ptr = src_ptr->data_ptr + offset;
    (*dup_ptr)->used = MIN(cnt,src_ptr->used - offset);
...
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT((*pkt_head_ptr)->enqueued_packet_length == 0);
#endif
...
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT((*data_item_ptr)->enqueued_packet_length == 0);
#endif
...
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT(*pkt_head_ptr == NULL || (*pkt_head_ptr)->enqueued_packet_length == 0);
#endif
...
)
{
  ...
  if(item_ptr != NULL &&                        /* Its not a null pointer */
     item_ptr->references == 1 &&               /* Its not DUPed */
     (!DSMI_DUP(item_ptr)) &&
...
)
{
  ...
  while(size != 0)
  {
    ...
    ------------------------------------------------------------------------*/
    if( *pkt_head_ptr != NULL &&
        ( DSMI_DUP(*pkt_head_ptr) ||
          DSMI_TAIL_SIZE(*pkt_head_ptr) == 0 ||
         (*pkt_head_ptr)->references != 1))
    {
      pkt_head_ptr = &(*pkt_head_ptr)->pkt_ptr;
    }
...
)
{
  ...
  {
    bytes_this_item = MIN(cnt, item_ptr->used);
    if(buf != NULL)
    {
...
)
{
  ...
  while(((*pkt_head_ptr) != NULL) && (cnt != 0))
  {
    /*-----------------------------------------------------------------
      ...
      smaller than used
    -----------------------------------------------------------------*/
    bytes_this_item = (*pkt_head_ptr)->used - offset;
    bytes_this_item = MIN(cnt, bytes_this_item);
...
)
{
...
#endif /* FEATURE_DSM_MEM_CHK_EXPENSIVE_ALWAYS_VERIFY_PACKET */
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT(packet_ptr == NULL || packet_ptr->enqueued_packet_length == 0);
#endif
  /*-------------------------------------------------------------------------
  ...
  {
    n = MIN(len, packet_ptr->used - (uint16)offset);
...
{ 
  ...
  while( *pkt_head_ptr != NULL && len > 0 )
  {
	...
	return ret;
      ...
      dsmi_free_buffer(*pkt_head_ptr,file,line);
...
#ifdef FEATURE_DSM_MEM_CHK_QUEUE_CACHE_LENGTH
  ASSERT( (*pkt_head_ptr) == NULL || 
          (*pkt_head_ptr)->enqueued_packet_length == 0);
#endif
...
{
  return dsm_extract_long(item_head_ptr, offset, value, 1);
} /* dsm_peek_byte() */
...
)
{
         ...
         offset >= (*pkt_head_ptr)->used )
  {
    ...
    {
      ERR_FATAL("dsm_split: Trying dsm_split_packet on a free item",0,0,0);
      return 0;
    }
#endif /* FEATURE_DSM_MEM_CHK */
  ...
  if(NULL != *pkt_head_ptr)
  {
	...
	return 0;
      ...
      (*pkt_tail_ptr)->used = (*pkt_head_ptr)->used - offset;
...
#ifdef DSM_KIND
      (*pkt_tail_ptr)->kind = kind;
#endif /* DSM_KIND */

      length += offset;
      
      /* Adjust current reference count, used and size */
      (*pkt_head_ptr)->used = offset;
	...
	dsmi_ref_inc((*pkt_head_ptr)->dup_ptr);
      }
      else
      {
	(*pkt_tail_ptr)->dup_ptr = *pkt_head_ptr;
        dsmi_ref_inc(*pkt_head_ptr);
      }
