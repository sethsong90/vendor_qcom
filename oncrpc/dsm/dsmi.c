...
/*===========================================================================
  ...
  $Header: //linux/pkgs/proprietary/oncrpc/main/source/dsm/dsmi.c#3 $
...
)
{
  ...
  /*-----------------------------------------------------------------
    ...
    -----------------------------------------------------------------*/
  if(*(uint32 *)((DSMI_ITEM_HEAD(item_ptr) - DSM_HEAD_COOKIE_LEN))
     != DSM_COOKIE
    )
  {
    ERR_FATAL("dsm_verify_buffer: Bad Header Cookie", 0, 0, 0);
  }

  /*-----------------------------------------------------------------
    Check that the tail cookie is OK.
    -----------------------------------------------------------------*/
  if(*(uint32*)DSMI_ITEM_TAIL(item_ptr) != DSM_TAIL_COOKIE)
  {
    ERR_FATAL("dsm_verify_buffer: Bad Trailer Cookie", 0, 0, 0);
  }
  ...
  /*-----------------------------------------------------------------
    ...
    -----------------------------------------------------------------*/
  if(DSM_POOL_ITEM_SIZE(item_ptr->pool_id) != 0)
  {
    if( item_ptr->data_ptr < DSMI_ITEM_HEAD(item_ptr) ||
        item_ptr->data_ptr > DSMI_ITEM_TAIL(item_ptr)
        )
    {
      ERR_FATAL("dsm_verify_buffer: DATA_PTR out of range: DATA_PTR 0x%x",
                (int)item_ptr->data_ptr,0,0
                );
    ...
    {     
      ERR_FATAL("dsm_verify_buffer: USED field is out of range: USED %d:MAX %d",
                item_ptr->used,
    ...
    {     
      ERR_FATAL("dsm_verify_buffer: SIZE field is incorrect: SIZE is %d:SIZE should be %d",
                item_ptr->size,
                (int)( DSMI_ITEM_TAIL(item_ptr) - (item_ptr)->data_ptr ),
                0);
    }
...
uint8 dsmi_ref_dec(dsm_item_type *item_ptr)
{
  ...
  {
#ifdef FEATURE_DSM_MEM_CHK
    ERR_FATAL("dsmi_ref_dec: Reference count already zero",0,0,0);
#endif /* FEATURE_DSM_MEM_CHK */
    ret = (uint8)~0;
  }
  else
...
)
{
  ...
  for (i=0; i < table->item_count; i++)
  {
    item_ptr = 
      (dsm_item_type *)
      &(table->item_array
        [i * DSMI_POOL_OBJECT_SIZE(table->pool_item_size,table->pad_size)]);
