...
/*=====================================================================
  ...
  $Id: //linux/pkgs/proprietary/modem-api/main/source/test/oncrpc/ping/src/ping_mdm_client_test_1000.c#3 $
...
static int ping_mdm_test_main(uint32 data_size_words, uint32 test_duration_ms, uint32 interval_ms)
{
  ...
  while( (sigs & PING_MDM_CLIENT_TEST_END_SIG) == 0 )
  {
    ...
    else if( sigs & PING_MDM_CLIENT_RESTART_SIG )
    {
      ...
      ping_mdm_client_register(num_cb_expected - num_cb_received,data_size_words,interval_ms,split_packet_ms);
...
int main(int argc, char *argv[])
{
  ...
  uint32 interval = 10;
  printf("\n\n Ping MDM FH CB (File Handle, Callback) Test, version %s\n",VersionStr);
  ...
  printf("Usage: ping_mdm_fh_cb_test test_duration_ms(%d msec) \n",(unsigned int)test_duration_ms);
  oncrpc_init();
  oncrpc_task_start();
