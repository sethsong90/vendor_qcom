...
/*=====================================================================
  ...
  $Id: //linux/pkgs/proprietary/modem-api/main/source/test/oncrpc/ping/src/ping_mdm_client_test_1000.c#6 $
...
04/26/2010   kr     Replaced signal handler with thread to identify test
                    timeout.
...
/*=====================================================================
     Forward Declaration
======================================================================*/
static int ping_mdm_test_main(uint32 data_size_words, uint32 test_duration_ms, uint32 interval_ms, uint32 num_tasks);
...
*/
/*=========================================================================*/
static int ping_mdm_test_main(uint32 data_size_words,
   ...
   uint32 num_tasks)
{
   ...
   while( (sigs & PING_MDM_CLIENT_TEST_END_SIG) == 0 )
   {
      ...
      else if( sigs & PING_MDM_CLIENT_TEST_CALLBACK_SIG )
      {
...
int main(int argc, char *argv[])
{
   ...
   printf("Usage: ping_mdm_client_test_1 test_duration_ms(%d msec) data_size_words(%d) interval(%d ms) num_tasks(%d)\n",
   ...
   rc = ping_mdm_test_main(data_size_words, test_duration_ms,interval,num_tasks);
   if( rc == 0 )
   {
      printf("PASS\n");
   }
