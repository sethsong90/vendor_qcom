...
static int ping_test_main(int test_length, int data_size_words, int test_duration_ms)
{
  ...
  CbReceived = 0;
  rc = ping_mdm_start_ping_lte_test(PING_MDM_TEST_1000,param,ping_mdm_ping_lte_results_cb);
  if(rc == 0)
  {
...
int main(int argc, char *argv[])
{
  ...
  printf("\n\n ping_mdm_lte_test_1000 version %s\n",VersionStr);
  printf("Usage: ping_mdm_lte_test_1000 test_duration_ms(%d msec) data_size_words(%d) test_length(%d) \n",
  (int)test_duration_ms, (int)data_size_words, (int)test_length);
  oncrpc_init();
  oncrpc_task_start();
