/******************************************************************************
  @file  oncrpc_mc_test_0002
  @brief Linux user-space oncrpc multi-thread test to verify multi-core
         safety of ONCRPC Stack

  DESCRIPTION
  This test proceeds with the main thread of execution making a
  ping_mdm_register_data_cb() RPC call first, followed by a series of
  ping_mdm_data() RPC calls. This will test how the ONCRPC stack handles
  concurrent write to/read from the RPC router. This will also test how the
  RPC router handles the concurrent call and callback for the same ONCRPC
  device. The ONCRPC stack and RPC router are thus tested for:
...
=======================================================================*/
/*
Description: Thread to perform a forward ping test
*/
/*=====================================================================*/
...
int main(int argc, char *argv[])
{
  ...
  printf("\nOncrpc Multi-Core Test 0002, version %s\n",VersionStr);
  printf("Usage: oncrpc_mc_test_0002 <num iterations> [%d] <test duration(ms)> [%d] <num threads> [%d] <data size words> [%d]\n\n",
  ...
  printf("ONCRPC_MC_TEST_0002 COMPLETE...\n");
  if(!rc)
    printf("PASS\n");
  else
    printf("FAIL\n");
