...
/*=====================================================================
  ...
  $Id: //linux/pkgs/proprietary/modem-api/main/source/test/oncrpc/ping/src/ping_lte_client_test_0001.c#8 $
...
static int ping_test_main(int test_duration_ms, int test_length)
{
  ...
  for(i = 1;i <= PING_LTE_MAX_WORDS;i = i<<1)
  {
    data_size_words = i;
    rc = ping_lte_client_common_data_test(test_length,data_size_words,test_duration_ms,0,&results);
    if(rc == 0)
    {
...
int main(int argc, char *argv[])
{
  ...
  uint32 num_loops;
  printf("\n\n %s version %s\n",__FILE__,VersionStr);
  printf("Usage: ping_client_test_0001 test_duration_ms(2000 msec)  \n");
  oncrpc_init();
  oncrpc_task_start();
