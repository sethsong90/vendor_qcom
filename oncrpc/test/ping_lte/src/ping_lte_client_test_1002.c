...
/*=====================================================================
  ...
  $Id: //linux/pkgs/proprietary/modem-api/main/source/test/oncrpc/ping/src/ping_lte_client_test_1002.c#4 $
...
#define PING_LTE_CLIENT_TEST_END_SIG       (0x02)
#define PING_LTE_CLIENT_TEST_CALLBACK_SIG_1  (0x04)
#define PING_LTE_CLIENT_TEST_CALLBACK_SIG_2  (0x08)
...
*/
/*=========================================================================*/
static boolean ping_lte_data_cb_function_1(uint32 *data, uint32 data_size_words, uint32 sum_cb)
{
  uint32 i;
  uint32 sum=0;
  num_cb_received[0]++;
  ...
  set_signal(PING_LTE_CLIENT_TEST_CALLBACK_SIG_1);
...
*/
/*=========================================================================*/
static boolean ping_lte_data_cb_function_2(uint32 *data, uint32 data_size_words, uint32 sum_cb)
{
  uint32 i;
  uint32 sum=0;
  num_cb_received[1]++;
  ...
  /* Wake up main thread */
  set_signal(PING_LTE_CLIENT_TEST_CALLBACK_SIG_2);
  return( sum == sum_cb );
}
...
*/
/*=========================================================================*/
static void ping_lte_client_data_unregister()
{
      ping_lte_un_register_data_cb(ping_lte_data_cb_function_1);
      ping_lte_un_register_data_cb(ping_lte_data_cb_function_2);
        registerd_state = 0;
}
...
static void ping_lte_client_register(uint32 num,uint32 size_words, uint32 interval_ms, uint32 split_packet_ms)
{
  if( registerd_state == 0 )
  {
    ...
    ping_lte_register_data_cb(
      ping_lte_data_cb_function_1,
      num,
      size_words,
      interval_ms,
      split_packet_ms);
    ping_lte_register_data_cb(
      ping_lte_data_cb_function_2,
      num,
      size_words,
      interval_ms,
      split_packet_ms);
...
static int ping_lte_test_main(uint32 data_size_words, uint32 test_duration_ms, uint32 interval_ms)
{
  ...
  while( (sigs & PING_LTE_CLIENT_TEST_END_SIG) == 0 )
  {
    ...
    {
      clr_flag(PING_LTE_CLIENT_TEST_END_SIG);
      ping_lte_client_data_unregister();
      ...
      ping_lte_client_register(num_cb_expected - num_cb_received[0],data_size_words,interval_ms,split_packet_ms);
    }else if( sigs & PING_LTE_CLIENT_TEST_CALLBACK_SIG_1 )
    {
  ...
  /* In this particular test, we tell the LTE MDM server to send large RPC calls continually and we abruptly
...
int main(int argc, char *argv[])
{
  ...
  printf("Usage: ping_lte_client_test_1 test_duration_ms(1000 msec) data_size_words(64) interval(1000 ms)\n");
